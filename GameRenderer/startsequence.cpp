...
StartSequence::StartSequence()
{
    ...
    RegisterCaption(  "intro_1" ,  x, y, size, 3, 15 );
    RegisterCaption(  "intro_2" ,  x, y+15, 20, 8, 15 );


    RegisterCaption(  "intro_3" ,  x, y+40, size, 30, 45 );
    RegisterCaption(  "intro_4" ,  x, y+50, size, 42, 45 );
    RegisterCaption(  "intro_5" ,  x, y, size, 54, 65 );
    RegisterCaption(  "intro_6" ,  x, y, size, 66, 74 );
    RegisterCaption(  "intro_7" ,  x, y+15, size, 72, 74 );
    RegisterCaption(  "intro_8" ,  x, y, size, 74, 90 );
    RegisterCaption(  "intro_9" ,  x, y+15, 15, 82, 90 );
    RegisterCaption(  "intro_10" , x, y+30, 15, 86, 90 );
}
...
void StartSequence::Render()
{
    ...
    for( int i = 0; i < m_captions.Size(); ++i )
    {
            ...
            timeNow <= caption->m_endTime )
        {
			...
			UnicodeString temp = LANGUAGEPHRASE(theString);
			int stringLength = temp.Length();
			int finishedLen = stringLength;//strlen(theString);

            if( maxTimeLength < stringLength )
            {
				temp.m_unicodestring[ maxTimeLength ] = '\x0';
				finishedLen = maxTimeLength;
            }
            
            glColor4f( 1.0f, 1.0f, 1.0f, 0.8f );             
            g_gameFont.DrawText2D( caption->m_x, caption->m_y, caption->m_size, temp );
