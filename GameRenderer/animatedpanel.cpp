...
AnimatedPanel::~AnimatedPanel() { m_objects.EmptyAndDelete(); }

int AnimatedPanel::NewObject()
{
  auto obj = new AnimatedPanelObject();
...
int AnimatedPanel::NewKeyframe(int objId, float timeIndex)
{
  ...
  AnimatedPanelObject* obj = m_objects[objId];
  auto kf = new AnimatedPanelKeyframe();
  *kf = keyframe;
...
int AnimatedPanel::GetKeyframe(int objId, float timeIndex, float maxTimeDiff)
{
  ...
  for (int j = 0; j < obj->m_keyframes.Size(); ++j)
  {
    ...
    if (abs(kf->m_timeIndex - timeIndex) <= maxTimeDiff)
      return j;
  }
...
bool AnimatedPanel::CalculateObjectProperties(int objId, float timeIndex, AnimatedPanelKeyframe& result)
{
  ...
  for (int j = 0; j < obj->m_keyframes.Size(); ++j)
  {
    ...
    if (kf->m_timeIndex <= timeIndex)
      if (!left || kf->m_timeIndex > left->m_timeIndex)
        left = kf;

    if (kf->m_timeIndex >= timeIndex)
      if (!right || kf->m_timeIndex < right->m_timeIndex)
        right = kf;
  }
  ...
  if (abs(right->m_timeIndex - left->m_timeIndex) < 0.01f)
  {
    result = *left;
    return true;
  }
...
AnimatedPanelObject::~AnimatedPanelObject() { m_keyframes.EmptyAndDelete(); }
...
AnimatedPanelKeyframe::AnimatedPanelKeyframe()
  : m_timeIndex(0.0f),
    ...
    m_angle(0.0f),
    m_flipHoriz(0),
    m_alpha(255) { strcpy(m_imageName, "darwinian.bmp"); }
...
void AnimatedPanelKeyframe::Read(TextReader* reader)
{
  while (reader->ReadLine())
  {
    ...
    if (stricmp(command, "TIMEINDEX") == 0)
      m_timeIndex = atof(data);
    else if (stricmp(command, "IMAGENAME") == 0)
      strcpy(m_imageName, data);
    else if (stricmp(command, "XPOS") == 0)
      m_x = atof(data);
    else if (stricmp(command, "YPOS") == 0)
      m_y = atof(data);
    else if (stricmp(command, "COLOUR") == 0)
      m_colour = atoi(data);
    else if (stricmp(command, "SCALE") == 0)
      m_scale = atof(data);
    else if (stricmp(command, "ANGLE") == 0)
      m_angle = atof(data);
    else if (stricmp(command, "FLIPHORIZ") == 0)
      m_flipHoriz = atoi(data);
    else if (stricmp(command, "ALPHA") == 0)
      m_alpha = atoi(data);
    else
      if (stricmp(command, "END") == 0)
        break;
  }
}
