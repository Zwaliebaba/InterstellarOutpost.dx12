...
int NetSocketListener::GetPort() const { return m_port; }

NetRetCode NetSocketListener::Bind()
{
  NetLockMutex lock(*m_mutex);

  if (static_cast<int>(m_sockfd) >= 0)
    return NetOk;
  ...
  else
  {
    ...
    NetHostDetails* pHostent = NetGetHostByName(m_interface.c_str()); if (!pHostent)
    {
    ...
    int errorCode = getaddrinfo(m_interface.c_str(), nullptr, &hints, &results);

    if (errorCode != 0 || results == nullptr)
    {
      NetDebugOut("NetSocketListener::Bind() failed to lookup address \"%s\": %s\n", m_interface.c_str(), gai_strerrorA(errorCode));

      NetCloseSocket(m_sockfd);
      m_sockfd = static_cast<SOCKET>(-1);
      m_binding = false;
      return NetFailed;
    }
...
void NetSocketListener::AppDebugOutNetSocketError(int _errorCode)
{
  ...
  default:
    DebugTrace("NetSocketListener(%s) : Error reading UDP data (errno=%d)\n", m_name.c_str(), _errorCode);
    break;
  }
#else
  // TODO
...
NetRetCode NetSocketListener::StartListening(NetCallBack functionPointer)
{
  ...
  if (functionPointer == static_cast<NetCallBack>(nullptr))
    return NetBadArgs;

  // Signal that we should be listening
  m_listening = true;

  auto packet = new NetUdpPacket();

  while (m_listening)
  {
    NetSocketLenType clientAddrLen = sizeof(packet->m_clientAddress);

    packet->m_length = recvfrom(m_sockfd, packet->m_data, MAX_PACKET_SIZE, 0, (struct sockaddr*)&packet->m_clientAddress, &clientAddrLen);
