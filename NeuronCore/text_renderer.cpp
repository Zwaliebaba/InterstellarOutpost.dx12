...
void TextRenderer::SetHorizScaleFactor(float factor) { m_horizScaleFactor = factor; }

void TextRenderer::SetHorizSpacingFactor(float factor) { m_horizSpacingFactor = factor; }
...
void TextRenderer::Shutdown()
{
  for (int i = 0; i < m_bmpnum; i++)
  {
    ...
    m_unicodebmpfiles[i].bitmapFile = nullptr;
  }
...
void TextRenderer::AddUnicodeBitmapInfo(int lower, int upper, const char* _name, int id, float linesOfCharacters, int width, int height)
{
  ...
  uni.width = width;
  uni.height = height;
  uni.bitmapFile = nullptr;
  uni.textHeight = ((1.0f / uni.lines) * (1.0f - (26.0f * (0.003f * (uni.height / 448)))));
  strncpy(uni.filename, _name, 256);

  //DebugTrace("Adding Unicode bmp info: %d:%d:%d", lower, upper, id);

  if (m_bmpnum >= m_bmparraysize)
  {
    //DebugTrace("Increasing Unicode info array size from %d to %d", m_bmparraysize, m_bmparraysize*2);
    auto newArray = new UnicodeFontInfo[m_bmparraysize * 2];
...
void TextRenderer::BuildUnicodeArray()
{
  ...
  {
    auto gluintArray = new GLuint[m_bmpnum];
    int counter = 0;
    for (int i = 0; i < m_bmpnum; i++)
    {
...
#define EXTENSION "bmp"
  ...
  auto unicodefiles = g_app->m_resource->ListResources("textures\\unicode\\", filter, false);

  size_t numFiles = unicodefiles.size();
  DebugTrace("{} unicode " EXTENSION "s found: {}\n", numFiles, m_filename);
  for (int i = 0; i < numFiles; i++)
  {
    ...
    //DebugTrace("Format is %s",format);

    if (sscanf(currentfilename, format, &lower, &upper) == 2)
    {
      AddUnicodeBitmapInfo(lower, upper, currentfilename, textureId, lines, width, height);
    }
  }

  DebugTrace("Finished building unicode textures.\n");
}
...
float TextRenderer::GetTexCoordY(wchar_t theChar)
{
  if (m_isunicode)
  {
    const float charHeight = 1.0f / m_unicodebmpfiles[m_currentbmpnum].lines; //14.0f;
...
int TextRenderer::GetTextureID(WCHAR theChar)
{
  ...
  if (theChar >= m_unicodebmpfiles[m_currentbmpnum].lowerBound && theChar <= m_unicodebmpfiles[m_currentbmpnum].upperBound)
    return m_currentbmpnum;

  for (int i = 0; i < m_bmpnum; i++)
  {
    if (theChar >= m_unicodebmpfiles[i].lowerBound && theChar <= m_unicodebmpfiles[i].upperBound)
      return i;
  }
...
void TextRenderer::DrawText2DUp(float _x, float _y, float _size, const UnicodeString& _text)
{
  ...
  for (unsigned int i = 0; i < numChars; ++i)
  {
    ...
    if (thisChar > 32)
    {
      ...
      glTexCoord2f(texX, texY + textHeight);
      glVertex2f(_x, _y + (thisChar < 0x3000 ? defaultHorizSize : horiSize));
      glTexCoord2f(texX + TEX_WIDTH, texY + textHeight);
      glVertex2f(_x, _y);
      glTexCoord2f(texX + TEX_WIDTH, texY);
      glVertex2f(_x + _size, _y);
      glTexCoord2f(texX, texY);
      glVertex2f(_x + _size, _y + (thisChar < 0x3000 ? defaultHorizSize : horiSize));
    }
...
void TextRenderer::DrawText2DDown(float _x, float _y, float _size, const UnicodeString& _text)
{
  ...
  if (m_renderShadow || m_renderOutline)
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_COLOR);
  else
    glBlendFunc(GL_SRC_ALPHA, GL_ONE);
      ...
      if (m_renderOutline)
      {
        for (int x = -1; x <= 1; ++x)
        {
          for (int y = -1; y <= 1; ++y)
          {
            ...
            {
              glTexCoord2f(texX, texY + textHeight);
              glVertex2f(_x + x + _size, _y + y);
              glTexCoord2f(texX + TEX_WIDTH, texY + textHeight);
              glVertex2f(_x + x + _size, _y + y + (thisChar < 0x3000 ? defaultHorizSize : horiSize));
              glTexCoord2f(texX + TEX_WIDTH, texY);
              glVertex2f(_x + x, _y + y + (thisChar < 0x3000 ? defaultHorizSize : horiSize));
              glTexCoord2f(texX, texY);
  ...
  for (unsigned int i = 0; i < numChars; ++i)
  {
    ...
    if (thisChar > 32)
    {
        ...
        for (int x = -1; x <= 1; ++x)
        {
          for (int y = -1; y <= 1; ++y)
          {
            if (x != 0 || y != 0)
            {
              ...
              glTexCoord2f(texX, texY + textHeight);
              glVertex2f(_x + xDiff, _y + yDiff);
              glTexCoord2f(texX + TEX_WIDTH, texY + textHeight);
              glVertex2f(_x + xDiff + (thisChar < 0x3000 ? defaultHorizSize : horiSize), _y + yDiff);
              glTexCoord2f(texX + TEX_WIDTH, texY);
              glVertex2f(_x + xDiff + (thisChar < 0x3000 ? defaultHorizSize : horiSize), _y + yDiff + _size);
              glTexCoord2f(texX, texY);
              glVertex2f(_x + xDiff, _y + yDiff + _size);
            }
          }
        }
      }
      else
      {
        ...
        glVertex2f(_x, _y);
        glTexCoord2f(texX + TEX_WIDTH, texY + textHeight);
        glVertex2f(_x + (thisChar < 0x3000 ? defaultHorizSize : horiSize), _y);
        glTexCoord2f(texX + TEX_WIDTH, texY);
        glVertex2f(_x + (thisChar < 0x3000 ? defaultHorizSize : horiSize), _y + _size);
        glTexCoord2f(texX, texY);
...
void TextRenderer::DrawText2D(float _x, float _y, float _size, const UnicodeString& _text) { DrawText2DSimple(_x, _y, _size, _text); }
...
namespace
{
  ...
  class SaveGLTex2DParamI
  {
    public:
      SaveGLTex2DParamI(GLenum _what)
        : m_what(_what) { glGetTexParameteriv(GL_TEXTURE_2D, _what, &m_value); }
  ...
  {
    public:
      SaveGLColour() { glGetFloatv(GL_CURRENT_COLOR, m_colour); }

      ~SaveGLColour() { glColor4fv(m_colour); }
...
void TextRenderer::DrawText3DSimple(const Vector3& _pos, float _size, const char* _text)
{
  UnicodeString u(_text);
  DrawText3DSimple(_pos, _size, u);
}

void TextRenderer::DrawText3DSimple(const Vector3& _pos, float _size, const UnicodeString& _text)
{
  // Store the GL state
  ...
  for (unsigned int i = 0; i < numChars; ++i)
  {
    ...
    if (thisChar > 32)
    {
      ...
      if (m_renderOutline)
      {
        for (int x = -1; x <= 1; ++x)
        {
          for (int y = -1; y <= 1; ++y)
          {
            if (x != 0 || y != 0)
            {
              Vector3 thisPos = pos + cam->GetUp() * y * _size * 0.05f + cam->GetRight() * x * _size * 0.05f;
              glTexCoord2f(texX, texY + textHeight);
              glVertex3dv((thisPos).GetData());
              glTexCoord2f(texX + TEX_WIDTH, texY + textHeight);
              glVertex3dv((thisPos + (thisChar < 0x3000 ? defaultHorizSize : horiSize)).GetData());
              glTexCoord2f(texX + TEX_WIDTH, texY);
              glVertex3dv((thisPos + (thisChar < 0x3000 ? defaultHorizSize : horiSize) - vertSize).GetData());
              glTexCoord2f(texX, texY);
              glVertex3dv((thisPos - vertSize).GetData());
            }
          }
        }
      }
      else
      {
        ...
        glTexCoord2f(texX + TEX_WIDTH, texY + textHeight);
        glVertex3dv((pos + (thisChar < 0x3000 ? defaultHorizSize : horiSize)).GetData());
        glTexCoord2f(texX + TEX_WIDTH, texY);
        glVertex3dv((pos + (thisChar < 0x3000 ? defaultHorizSize : horiSize) - vertSize).GetData());
        glTexCoord2f(texX, texY);
        glVertex3dv((pos - vertSize).GetData());
      }
    }
...
void TextRenderer::DrawText3D(const Vector3& _pos, float _size, const char* _text, ...)
{
  char buf[512];
  va_list ap;
  va_start(ap, _text);
...
void TextRenderer::DrawText3D(const Vector3& _pos, float _size, const UnicodeString& _text) { DrawText3DSimple(_pos, _size, _text); }

void TextRenderer::DrawText3DCentre(const Vector3& _pos, float _size, const char* _text, ...)
{
  char buf[512];
  va_list ap;
  va_start(ap, _text);
...
void TextRenderer::DrawText3DCentre(const Vector3& _pos, float _size, const UnicodeString& _text)
{
  WCHAR buf[512];
...
void TextRenderer::DrawText3DRight(const Vector3& _pos, float _size, const char* _text, ...)
{
  char buf[512];
  va_list ap;
  va_start(ap, _text);
...
void TextRenderer::DrawText3DRight(const Vector3& _pos, float _size, const UnicodeString& _text)
{
  WCHAR buf[512];
  swprintf(buf, sizeof(buf) / sizeof(WCHAR), _text.m_unicodestring);
...
void TextRenderer::DrawText3D(const Vector3& _pos, const Vector3& _front, const Vector3& _up, float _size, const char* _text, ...)
{
  char buf[512];
  va_list ap;
  va_start(ap, _text);
...
void TextRenderer::DrawText3D(const Vector3& _pos, const Vector3& _front, const Vector3& _up, float _size, const UnicodeString& _text)
{
  SaveGLFontDrawAttributes saveAttribs;
...
float TextRenderer::GetTextWidth(unsigned int _numChars, float _size) { return _numChars * _size * m_horizScaleFactor; }
...
bool TextRenderer::IsUnicode() { return m_isunicode; }

static int DecodeSurrogatePair(WCHAR _word1, WCHAR _word2)
{
  int ret = -1;
  // Check to see if this is a well formed surrogate pair
  if (_word1 >= 0xD800 && _word1 < 0xDC00 && _word2 >= 0xDC00 && _word2 < 0xE000)
  {
    _word1 ^= 0xD800;
    _word2 ^= 0xDC00;

    ret = (static_cast<int>(_word1) << 10) | _word2;
    ret |= 0x10000;
  }
