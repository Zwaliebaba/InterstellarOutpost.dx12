...
void LangTable::ParseLanguageFile(char const *_filename)
{
	...
	if( !in || !in->IsOpen() )
	{
		DebugTrace("WARNING: Can not find language file %s to parse. Soft failing\n", _filename);
		return;
	}
	...
	else DebugTrace("Did not have TextReader. No checking it'a unicodeness\n");
	...
	DebugTrace("Read %d strings: %f seconds\n", i, GetHighResTime() - startTime );
