...
int FastDArray <T>::PutData(const T& newdata)
{
	...
	ASSERT_TEXT(firstfree != -1, "firstfree(%d)", firstfree);
...
{
	ASSERT_TEXT(index < this->m_arraySize && index >= 0, "index(%d), m_arraySize(%d)", index, this->m_arraySize);
...
{
	ASSERT_TEXT(index < this->m_arraySize && index >= 0, "index(%d), m_arraySize(%d)", index, this->m_arraySize);
	ASSERT_TEXT(this->shadow[index] == 0, "index(%d)", index);
