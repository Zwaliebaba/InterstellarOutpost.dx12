...
void SystemInfo::GetLocaleDetails()
{
  ...
  if (!languageSuccess)
  {
    ...
    ASSERT_TEXT(GetLocaleInfoA(LOCALE_USER_DEFAULT, LOCALE_SENGLANGUAGE, m_localeInfo.m_language, size), "Couldn't get locale details");
  }

  size = GetLocaleInfoA(LOCALE_USER_DEFAULT, LOCALE_SENGCOUNTRY, nullptr, 0);
  m_localeInfo.m_country = new char[size + 1];
  ASSERT_TEXT(GetLocaleInfoA(LOCALE_USER_DEFAULT, LOCALE_SENGCOUNTRY, m_localeInfo.m_country, size), "Couldn't get country details");
...
void SystemInfo::GetDirectXVersion()
{
  ...
  long errCode = RegOpenKeyExA(HKEY_LOCAL_MACHINE, "SOFTWARE\\Microsoft\\DirectX", 0, KEY_READ, &hkey);
  ASSERT_TEXT(errCode == ERROR_SUCCESS, "Failed to get DirectX Version");
  errCode = RegQueryValueExA(hkey, "InstalledVersion", NULL, NULL, buf, &bufLen);
  ...
  else
  {
    ...
    errCode = RegQueryValueExA(hkey, "Version", NULL, NULL, buf, &bufLen);
    if (errCode == ERROR_SUCCESS)
      m_directXVersion = buf[3];
  }
