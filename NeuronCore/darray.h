...
{
	ASSERT_TEXT(index >= 0, "index(%d)", index);
...
{
	ASSERT_TEXT(index < m_arraySize && index >= 0, "index(%d), m_arraySize(%d)", index, m_arraySize);
	ASSERT_TEXT(shadow[index] != 0, "index(%d)", index);
...
{
	ASSERT_TEXT(_index < m_arraySize && _index >= 0, "_index(%d), m_arraySize(%d)", _index, m_arraySize);
	ASSERT_TEXT(shadow[_index] != 0, "_index(%d)", _index);
