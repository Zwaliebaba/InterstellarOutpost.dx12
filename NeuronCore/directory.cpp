...
Directory::Directory()
  : m_name(nullptr) { SetName("NewDirectory"); }
...
template <class T>
static bool DirectoryEqual(const DArray<T*>& _a, const DArray<T*>& _b)
{
  int size = max(_a.Size(), _b.Size());

  for (int i = 0; i < size; i++)
  {
    bool aValid = _a.ValidIndex(i), bValid = _b.ValidIndex(i);

    if (aValid != bValid)
      return false;

    if (aValid && bValid && !(*_a.GetData(i) == *_b.GetData(i)))
      return false;
  }
...
{
  return DirectoryEqual(m_subDirectories, _another.m_subDirectories) && DirectoryEqual(m_data, _another.m_data);
}

bool Directory::operator !=(const Directory& _another) const { return !(*this == _another); }
...
Directory* Directory::GetDirectory(const char* dirName) const
{
  ...
  ASSERT_TEXT(false, "Error getting directory %s", dirName);
  return nullptr;
}
...
Directory* Directory::AddDirectory(const char* dirName)
{
  ...
  ASSERT_TEXT(false, "Failed to get first SubDir from %s", dirName);
  return nullptr;
}
...
void Directory::CopyData(const Directory* dir, bool overWrite, bool directories)
{
  ...
  for (int i = 0; i < dir->m_data.Size(); ++i)
  {
    if (dir->m_data.ValidIndex(i))
    {
      ...
      {
        auto newData = new DirectoryData();
  ...
  if (directories)
  {
    for (int d = 0; d < dir->m_subDirectories.Size(); ++d)
    {
      if (dir->m_subDirectories.ValidIndex(d))
      {
        ...
        auto newDir = new Directory(*subDir);
        AddDirectory(newDir);
      }
    }
  }
...
char* Directory::GetFirstSubDir(const char* dirName)
{
  ...
  if (dirSize == 0)
    return nullptr;
  auto result = new char[dirSize + 1];
...
void Directory::CreateData(const char* dataName, Fixed value)
{
  ...
  if (GetData(dataName)) { RemoveData(dataName); }
...
void Directory::CreateData(const char* dataName, unsigned char value) { CreateData(dataName, static_cast<char>(value)); }
...
void Directory::CreateData(const char* dataName, const UnicodeString& _data) { CreateData(dataName, _data.m_unicodestring); }
...
int Directory::GetDataInt(const char* dataName) const
{
  ...
  if (data && data->IsInt())
    return data->m_int;
  DebugTrace("Int Data not found : %s\n", dataName);
...
float Directory::GetDataFloat(const char* dataName) const
{
  ...
  if (data && data->IsFloat())
    return data->m_float;
  DebugTrace("Float Data not found : %s\n", dataName);
  return DIRECTORY_SAFEFLOAT;
}

unsigned char Directory::GetDataUChar(const char* dataName) const { return static_cast<unsigned char>(GetDataChar(dataName)); }
...
char Directory::GetDataChar(const char* dataName) const
{
  ...
  if (data && data->IsChar())
    return data->m_char;
  // DebugTrace( "Char Data not found : %s\n", dataName );
...
char* Directory::GetDataString(const char* dataName) const
{
  ...
  DebugTrace("String Data not found : %s\n", dataName);
...
wchar_t* Directory::GetDataUnicode(const char* dataName) const
{
  DirectoryData* data = GetData(dataName);
  {
    ...
    DebugTrace("Unicode Data not found : %s\n", dataName);
...
bool Directory::GetDataUnicodeWithStringFallback(const char* dataName, UnicodeString& _dest)
{
  ...
  if (data)
  {
    ...
    if (data->IsString() && data->m_string)
    {
      _dest = data->m_string;
      return true;
    }
  ...
  DebugTrace("Unicode/String Data not found : %s\n", dataName);
...
bool Directory::GetDataBool(const char* dataName) const
{
  ...
  if (data && data->IsBool())
    return data->m_bool;
  DebugTrace("Bool Data not found : %s\n", dataName);
...
void* Directory::GetDataVoid(const char* dataName, int* _dataLen) const
{
  ...
  DebugTrace("Void data not found : %s\n", dataName);
...
{
  if (GetData(dataName))
    ASSERT_TEXT(false, "Attempted to create duplicate data %s at location\n%s", dataName, m_name);
...
void Directory::GetDataDArray(const char* dataName, DArray<int>* darray) const
{
  ...
  if (!dir)
  {
    ASSERT_TEXT(false, "Failed to find DArray named %s", dataName);
    return;
  }
  ...
  if (strcmp(tID, "DArray<int>") != 0 || size == DIRECTORY_SAFEINT)
    DebugTrace("Found object %s but it doesn't appear to be a DArray\n");
  ...
  for (int i = 0; i < size; ++i)
  {
    ...
    if (!thisIndex)
      DebugTrace("Error loading DArray %s : index %d is missing\n", dataName, i);
    else
    {
...
void Directory::GetDataLList(const char* dataName, LList<Directory*>* llist) const
{
  ...
  if (!dir)
  {
    DebugTrace("Failed to find LList named %s\n", dataName);
    return;
  }
  ...
  if (strcmp(tID, "LList<Directory *>") != 0 || size == DIRECTORY_SAFEINT)
    DebugTrace("Found object %s but it doesn't appear to be a LList\n");
  ...
  for (int i = 0; i < size; ++i)
  {
    ...
    Directory* thisIndex = dir->GetDirectory(indexName);
    if (!thisIndex)
      DebugTrace("Error loading LList %s : index %d is missing\n", dataName, i);
    else
    {
      if (thisIndex->m_subDirectories.ValidIndex(0))
      {
        ...
        DebugTrace("Error loading LList %s : could not find directory in index %d\n", dataName, i);
    }
  }
}
...
void Directory::GetDataLList(const char* dataName, LList<int>* llist) const
{
  ...
  if (!dir)
  {
    DebugTrace("Failed to find LList %s\n", dataName);
    return;
  }
...
bool Directory::Read(istream& input)
{
  ...
  {
    auto data = new DirectoryData();
    if (!data->Read(input))
    {
      DebugTrace("Warning: failed to read directory data from stream\n");
      delete data;
      return false;
    }
    m_data.PutData(data);
  ...
  for (int s = 0; s < numSubdirs; ++s)
  {
    ...
    {
      DebugTrace("Warning failed to read directory from stream\n");
      delete newDir;
      return false;
    }
...
void HexDumpData(const char* input, int length, int highlight)
{
  ...
  for (int i = 0; i < length; i += linewidth)
  {
    for (int j = 0; j < linewidth; j++)
    {
      if (i + j < length)
      {
        unsigned char ch = static_cast<unsigned char>(input[i + j]);

        DebugTrace("%c", (i + j == highlight) ? '[' : ' ');
        DebugTrace("%c", isprint(ch) ? ch : ' ');
        DebugTrace("%c", (i + j == highlight) ? ']' : ' ');
      }
      else
        DebugTrace("   ");
...
bool Directory::Read(const char* input, int length)
{
  ...
  if (!result)
  {
    DebugTrace("Failed to parse directory at position %d.\n", static_cast<int>(inputStream.tellg()));
    DebugTrace("Packet length: %d\n", length);
    DebugTrace("Packet data:\n");
...
DirectoryData::DirectoryData()
  : m_name(nullptr),
    ...
    m_intsLen(0),
    m_ullong(0),
    m_unicode(nullptr) { SetName("NewData"); }
...
DirectoryData::DirectoryData(const DirectoryData& _copyMe)
  : m_name(nullptr),
    ...
    m_void(nullptr),
    m_ints(nullptr),
    m_unicode(nullptr) { SetData(&_copyMe); }
...
{
#ifdef TARGET_OS_LINUX
  iconv_t utf32_to_utf8 = iconv_open("UTF8", "UTF32LE"); if (utf32_to_utf8 == (iconv_t) - 1)
  {
    perror("Failed to open iconv from UTF32 -> UTF8");
    return;
  } char* inBuf = reinterpret_cast<char*>(const_cast<wchar_t*>(x)); size_t inBytesLeft = amount == -1 ? wcslen(x) : amount; inBytesLeft *=
    4; // (4 bytes per UTF32)
...
void DirectoryData::WriteXML(ostream& o, int indent) const
{
  ...
  DEBUG_ASSERT(m_type != DIRECTORY_TYPE_FIXED); // fixed case not implemented
  switch (m_type)
  {
  ...
  case DIRECTORY_TYPE_CHAR:
    o << static_cast<int>(static_cast<unsigned char>(m_char));
    break;
  case DIRECTORY_TYPE_STRING:
    WriteXMLString(o, m_string);
    break;
  case DIRECTORY_TYPE_UNICODE:
    WriteXMLString(o, m_unicode);
    break;
...
void DirectoryData::DebugPrint(int indent) const
{
  ...
  case DIRECTORY_TYPE_VOID:
    DebugTrace("Raw Data, Length %d bytes", m_voidLen);
    break;

  case DIRECTORY_TYPE_INTS:
    DebugTrace("Int Data, Length %d bytes", m_voidLen);
    break;
...
#else
  DebugTrace("Directory::DebugPrint is hashed out\n");
#endif
}
...
bool DirectoryData::Read(istream& input)
{
...
#ifdef USE_FIXED
  case DIRECTORY_TYPE_FIXED:
    ReadNetworkValue(input, m_fixed); break;
#endif
  case DIRECTORY_TYPE_CHAR:
    m_char = input.get();
    break;
  ...
  case DIRECTORY_TYPE_VOID:
    delete[] m_void;
    m_void = static_cast<char*>(ReadVoidData(input, &m_voidLen));
    break;
...
void DirectoryData::Write(ostream& output) const
{
  ...
  output.put(static_cast<unsigned char>(m_type));
...
#ifdef USE_FIXED
  case DIRECTORY_TYPE_FIXED:
    WriteNetworkValue(output, m_fixed); break;
#endif
  case DIRECTORY_TYPE_CHAR:
    output.put(m_char);
    break;
...
bool DirectoryData::operator ==(const DirectoryData& _another) const
{
  ...
  case DIRECTORY_TYPE_VOID:
    return m_voidLen == _another.m_voidLen && memcmp(m_void, _another.m_void, m_voidLen) == 0;

  case DIRECTORY_TYPE_INTS:
    return m_intsLen == _another.m_intsLen && memcmp(m_ints, _another.m_ints, m_intsLen) == 0;
