...
static D3DVERTEXELEMENT9 s_customVertexDesc[] = {{0, 0, D3DDECLTYPE_FLOAT3, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_POSITION, 0},
...
struct MipMapTask
{
  MipMapTask(LPDIRECT3DTEXTURE9 _pTexture, int _texId);
  ~MipMapTask();
  ...
  bool m_complete;
  int m_levels;
  std::vector<IDirect3DSurface9*> m_pSurface;
...
static NetMutex s_texturesToMipmapMutex;
static std::list<MipMapTask*> s_texturesToMipmap;
static WorkQueue* s_workQueue = nullptr;
...
class CurrentAttributes : public CustomVertex
{
  public:
    ...
    bool polygonOffsetEnabled; //								(GL_POLYGON_OFFSET_FILL)
    float depthBias, slopeDepthBias; //							Polygon Offset parameters
...
static void InitialiseData()
{
  ...
  if (!s_workQueue)
  {
    s_workQueue = new WorkQueue;
    s_workQueue->Add(SetWorkingThreadProcessor);
  }
}
...
void SafeDeleteTexture(IDirect3DTexture9*& texture)
{
  if (texture)
  {
    texture->Release();
...
void ReleaseD3DPoolDefaultResources()
{
  ...
  for (int i = 0; i < s_textureIds.size(); i++)
    SafeDeleteTexture(s_textureIds[i]);
}

// d3d resources are mostly automatically created
//  (on misc places) first time they are needed
void CreateD3DResources() { CreateD3dPoolDefaultResources(); }
...
bool Direct3DInit(HWND _hWnd, bool _windowed, int _width, int _height, int _colourDepth, int _zDepth, bool _waitVRT)
{
  ...
  if (!g_pD3D)
  {
    if (nullptr == (g_pD3D = Direct3DCreate9(D3D_SDK_VERSION)))
    {
      ...
      ASSERT_TEXT(0, "Direct3D 9 not available.");
    }
  }
  ...
  */
  ...
  else
  {
    ...
    case 16:
      ...
      break;
    default: ASSERT_TEXT(0, "Unexpected colourDepth.");
    }
  }
  ...
  if (_zDepth)
  {
    ...
    case 32:
      ...
      break;
    default: ASSERT_TEXT(0, "Unexpected zDepth.");
    }
  }
  ...
  if (FAILED(g_pD3D->CreateDevice(
    ...
    // standard adapter
    D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, _hWnd, g_supportsHwVertexProcessing?D3DCREATE_HARDWARE_VERTEXPROCESSING:
    D3DCREATE_SOFTWARE_VERTEXPROCESSING,&g_d3dpp, &g_pd3dDevice )))
  {
    // non fatal end, res change can help
    //ASSERT_TEXT( 0, "Failed to set screen mode" );
    return false;
  }
...
void Direct3DSwapBuffers()
{
  ...
  // DebugTrace("Setting Camera View Matrix...\n" );
  D3DXMatrixIdentity(&mIdentity);
  g_pd3dDevice->SetVertexShaderConstantF(0, (FLOAT*)&mIdentity, 4);
  g_pd3dDevice->SetVertexShaderConstantF(4, (FLOAT*)&mIdentity, 4);
  // JAKHACK
...
{
  GL_TRACE_IMP(" glMultMatrixd((const double *)%p)", m)
...
{
  GL_TRACE_IMP(" gluLookAt(%10.4g, %10.4g, %10.4g, %10.4g, %10.4g, %10.4g, %10.4g, %10.4g, %10.4g)", eyex, eyey, eyez, centerx, centery,
               centerz, upx, upy, upz)

  D3DXVECTOR3 eye(static_cast<FLOAT>(eyex), static_cast<FLOAT>(eyey), static_cast<FLOAT>(eyez)), at(
                static_cast<FLOAT>(centerx), static_cast<FLOAT>(centery), static_cast<FLOAT>(centerz)), up(
                static_cast<FLOAT>(upx), static_cast<FLOAT>(upy), static_cast<FLOAT>(upz));
...
void gluPerspective(GLdouble fovy, GLdouble aspect, GLdouble zNear, GLdouble zFar)
{
  ...
  D3DXMATRIX m;
  D3DXMatrixPerspectiveFovRH(&m, D3DXToRadian(static_cast<FLOAT>(fovy)), static_cast<FLOAT>(aspect), static_cast<FLOAT>(zNear),
...
void glDisable(GLenum cap)
{
  ...
  case GL_FOG:
    g_fixedPipeline->m_fogEnabled = false;
    break;

  case GL_LIGHTING:
    g_fixedPipeline->m_lighting = false;
    break;
  ...
  case GL_LIGHT1:
    ...
    //case GL_LIGHT6:
    //case GL_LIGHT7:
    g_fixedPipeline->m_lightEnabled[cap - GL_LIGHT0] = false;
    break;
  ...
  case GL_POLYGON_OFFSET_FILL:
    s_currentAttribs.polygonOffsetEnabled = false;
...
void glEnable(GLenum cap)
{
  ...
  case GL_FOG:
    g_fixedPipeline->m_fogEnabled = true;
    break;

  case GL_LIGHTING:
    g_fixedPipeline->m_lighting = true;
    break;
  ...
  case GL_LIGHT1:
    ...
    //case GL_LIGHT6:
    //case GL_LIGHT7:
    g_fixedPipeline->m_lightEnabled[cap - GL_LIGHT0] = true;
    break;
  ...
  case GL_POLYGON_OFFSET_FILL:
    s_currentAttribs.polygonOffsetEnabled = true;
    g_pd3dDevice->SetRenderState(D3DRS_SLOPESCALEDEPTHBIAS, s_currentAttribs.slopeDepthBias);
    g_pd3dDevice->SetRenderState(D3DRS_DEPTHBIAS, s_currentAttribs.depthBias);
    break;
...
{
  g_pd3dDevice->DrawPrimitiveUP(PrimitiveType, PrimitiveCount, pVertexStreamZeroData, VertexStreamZeroStride);
}

void glEnd()
{
  //DebugTrace("glEnd: g_pd3dDevice=%x g_fixedPipeline=%x\n",(unsigned)g_pd3dDevice,(unsigned)g_fixedPipeline);
  GL_TRACE_IMP(" glEnd()")

  // Update the matrix if it has been modified (also updated when we switch matrix modes)
  s_pTargetMatrixStack->FastSetTransform();

  if (g_fixedPipeline && g_pd3dDevice == g_pd3dDeviceActual)
    g_fixedPipeline->Bind();

  switch (s_primitiveMode)
  {
  case GL_POINTS: DEBUG_ASSERT(s_currentVertexNumber>0);
    DrawPrimitiveUP(D3DPT_POINTLIST, s_currentVertexNumber, s_vertices, sizeof(CustomVertex));
    break;

  case GL_LINES: DEBUG_ASSERT(s_currentVertexNumber%2==0);
    DEBUG_ASSERT(s_currentVertexNumber>1);
    DrawPrimitiveUP(D3DPT_LINELIST, s_currentVertexNumber / 2, s_vertices, sizeof(CustomVertex));
    break;

  case GL_LINE_LOOP: DEBUG_ASSERT(s_currentVertexNumber>1);
    if (s_currentVertexNumber + 2 >= s_allocatedVertices)
      reallocateVertexList();
    s_vertices[s_currentVertexNumber] = s_vertices[0];
    DrawPrimitiveUP(D3DPT_LINESTRIP, s_currentVertexNumber, s_vertices, sizeof(CustomVertex));
    break;

  case GL_LINE_STRIP: DEBUG_ASSERT(s_currentVertexNumber>1);
    DrawPrimitiveUP(D3DPT_LINESTRIP, s_currentVertexNumber, s_vertices, sizeof(CustomVertex));
    break;

  case GL_TRIANGLE_STRIP:
  case GL_QUAD_STRIP:
    //DEBUG_ASSERT(s_currentVertexNumber>2);
    if (s_currentVertexNumber > 2) // invalid value here _could be_ cause of xander's problem
    {
      // draw smooth (tristrip)
      //DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, s_currentVertexNumber-2, s_vertices, sizeof(CustomVertex));
      // draw flat (expand from strip to list)
      static std::vector<CustomVertex> vertexList; // list made from strip in s_vertices
      vertexList.clear();
      for (unsigned i = 0; i < s_currentVertexNumber - 2; i++)
      {
        CustomVertex vertex;
        vertex = s_vertices[i + 2];
        vertexList.push_back(vertex);
        vertex.x = s_vertices[i].x;
        vertex.y = s_vertices[i].y;
        vertex.z = s_vertices[i].z;
        vertex.u = s_vertices[i].u;
        vertex.v = s_vertices[i].v;
        vertex.u2 = s_vertices[i].u2;
        vertex.v2 = s_vertices[i].v2;
        vertexList.push_back(vertex);
        vertex.x = s_vertices[i + 1].x;
        vertex.y = s_vertices[i + 1].y;
        vertex.z = s_vertices[i + 1].z;
        vertex.u = s_vertices[i + 1].u;
        vertex.v = s_vertices[i + 1].v;
        vertex.u2 = s_vertices[i + 1].u2;
        vertex.v2 = s_vertices[i + 1].v2;
        vertexList.push_back(vertex);
      }
      DrawPrimitiveUP(D3DPT_TRIANGLELIST, s_currentVertexNumber - 2, &vertexList[0], sizeof(CustomVertex));
    }
    break;

  case GL_TRIANGLE_FAN: DEBUG_ASSERT(s_currentVertexNumber>2);
    DrawPrimitiveUP(D3DPT_TRIANGLEFAN, s_currentVertexNumber - 2, s_vertices, sizeof(CustomVertex));
    break;

  case GL_TRIANGLES:
  case GL_QUADS:
    // Quads: Inefficient, perhaps we should have two buffers, a vertex list, and an index list
  ...
  {
    DEBUG_ASSERT(s_currentVertexNumber%3==0);
    int numTris = s_currentVertexNumber / 3;
    // could be 0 in PaintPixels, and it's difficult to filter it out sooner
    if (numTris)
      DrawPrimitiveUP(D3DPT_TRIANGLELIST, numTris, s_vertices, sizeof(CustomVertex));
  }
  break;

  default: GL_TRACE("-glEnd() not implemented for %s", glEnumToString(s_primitiveMode));
    break;
  }
}
...
void glBindTexture(GLenum target, GLuint texture)
{
  ...
  default: DEBUG_ASSERT(target == GL_TEXTURE_2D);
    break;
  }
}
...
{
  GL_TRACE_IMP(" glCopyTexImage2D(%s, %d, %s, %d, %d, %d, %d, %d)", glEnumToString(target), level, glEnumToString(internalFormat), x, y,
               width, height, border);

  LPDIRECT3DTEXTURE9& pTexture = s_textureIds[s_activeTextureState->target];
  if (pTexture == nullptr)
  {
    HRESULT hr = g_pd3dDeviceActual->CreateTexture(width, height, 1, D3DUSAGE_RENDERTARGET, g_d3dpp.BackBufferFormat, D3DPOOL_DEFAULT,
                                                   &pTexture, nullptr);
    DEBUG_ASSERT(hr != D3DERR_INVALIDCALL);
    DEBUG_ASSERT(hr != D3DXERR_INVALIDDATA);
  }
        ...
        if (SUCCEEDED(g_pd3dDevice->CreateOffscreenPlainSurface(width,height,D3DFMT_A8R8G8B8,D3DPOOL_SYSTEMMEM,&memsurface,NULL)))
        {
          if (SUCCEEDED(g_pd3dDevice->GetRenderTargetData(screenTexture->GetRenderTarget(), memsurface)))
          {
            ...
            if (SUCCEEDED(memsurface->LockRect(&lrect,NULL,D3DLOCK_READONLY)))
            {
              ...
              auto dst = static_cast<char*>(pixels);
              auto src = static_cast<char*>(lrect.pBits);
...
MipMapTask::MipMapTask(LPDIRECT3DTEXTURE9 _pTexture, int _texId)
  : m_complete(false),
    m_levels(_pTexture->GetLevelCount()),
    m_pSurface(_pTexture->GetLevelCount()),
    m_pTexture(_pTexture),
    m_texId(_texId)
{
  for (int i = 0; i < m_levels; i++)
    _pTexture->GetSurfaceLevel(i, &m_pSurface[i]);
}
...
MipMapTask::~MipMapTask()
{
  ...
  s_textureIds[m_texId] = m_pTexture;
}

void MipMapTask::GenerateMipmaps()
{
  for (int i = 1; i < m_levels; i++)
  {
    D3DXLoadSurfaceFromSurface(m_pSurface[i], nullptr, nullptr, m_pSurface[0], nullptr, nullptr, D3DX_FILTER_TRIANGLE, 0);
  }
...
bool GenerateMipmaps()
{
  NetLockMutex lock(s_texturesToMipmapMutex);

  // Generate one mipmap at a time
  std::list<MipMapTask*>::iterator i;

  for (i = s_texturesToMipmap.begin(); i != s_texturesToMipmap.end();)
  {
    MipMapTask* task = *i;

    if (task->m_complete)
    {
      i = s_texturesToMipmap.erase(i);
      delete task;
    }
    else
      ++i;
                            ...
                            bool mipmapping, bool _compressed = false)
{
  ...
  HRESULT rc = g_pd3dDevice->CreateTexture(width, height, mipmapping ? 0 : 1, D3DUSAGE_AUTOGENMIPMAP, // autogenmipmap ignored by xbox
                                           _compressed ? D3DFMT_DXT1 : D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, &pTexture, nullptr);
  DEBUG_ASSERT(rc != D3DERR_INVALIDCALL);
  DEBUG_ASSERT(rc != D3DXERR_INVALIDDATA);

  IDirect3DSurface9* pTopSurface;
  RECT rect = {0, 0, width, height};

  pTexture->GetSurfaceLevel(0, &pTopSurface);

  D3DXLoadSurfaceFromMemory(pTopSurface, nullptr, nullptr, data, D3DFMT_A8B8G8R8, width * 4, nullptr, &rect, D3DX_FILTER_NONE, 0);
  pTopSurface->Release();

  if (mipmapping)
  {
    auto task = new MipMapTask(pTexture, s_activeTextureState->target);

    pTexture = nullptr;

    s_workQueue->Add(&MipMapTask::GenerateMipmaps, task);

    NetLockMutex lock(s_texturesToMipmapMutex);
    s_texturesToMipmap.push_back(task);
  }
...
int gluBuild2DMipmaps(GLenum target, GLint components, GLint width, GLint height, GLenum format, GLenum type, const void* data,
                      bool _compressed)
{
  GL_TRACE_IMP(" gluBuild2DMipmaps(%s, %d, %d, %d, %s, %s, %p)", glEnumToString(target), components, width, height, glEnumToString(format),
               glEnumToString(type), data)
  return d3dCreateTexture(target, components, width, height, format, type, data, true, _compressed);
}
...
void glTexImage2D(GLenum target, GLint level, GLint internalformat, GLsizei width, GLsizei height, GLint border, GLenum format, GLenum type,
                  const GLvoid* pixels, bool _compressed)
{
  GL_TRACE_IMP(" glTexImage2D(%s, %d, %d, %d, %d, %d, %s, %s, %p)", glEnumToString(target), level, internalformat, width, height, border,
  ...
  DEBUG_ASSERT(internalformat == 4);
  DEBUG_ASSERT(format == GL_RGBA);

  d3dCreateTexture(target, 4, width, height, format, type, pixels, false, _compressed);
}
...
static DWORD glMinMagFilterToDirect3D(GLenum param)
{
  ...
  case GL_LINEAR_MIPMAP_LINEAR:
    return D3DTEXF_LINEAR;
  default: ASSERT_TEXT(FALSE, "Unknown Filter %u", param);
    return D3DTEXF_NONE;
  }
}
...
static DWORD glTextureWrapToDirect3D(GLenum param)
{
  ...
  default: ASSERT_TEXT(FALSE, "Unknown Texture Wrap mode %u", param);
    return D3DTADDRESS_WRAP;
  }
}
...
void glTexParameteri(GLenum target, GLenum pname, GLint param)
{
  ...
  case GL_TEXTURE_MIN_FILTER:
    ...
    g_pd3dDevice->SetSamplerState(s_activeTexture, D3DSAMP_MIPFILTER, param == GL_LINEAR_MIPMAP_LINEAR ? D3DTEXF_LINEAR : D3DTEXF_NONE);
    break;
  }
}
...
void glFogf_impl(GLenum pname, GLfloat param)
{
  ...
  case GL_FOG_END:
    g_fixedPipeline->m_fogEnd = param;
    break;
  ...
  case GL_FOG_START:
    g_fixedPipeline->m_fogStart = param;
    break;
...
void glFogi_impl(GLenum pname, GLint param)
{
  ...
  case GL_FOG_START:
    if (param > 0)
      glFogf_impl(pname, static_cast<float>(param) / INT_MAX);
    else
      glFogf_impl(pname, static_cast<float>(param) / INT_MIN);
    break;
  }
}

void glFogfv(GLenum pname, const GLfloat* params) { GL_TRACE_IMP(" glFogfv(%s, (const float *)%p) ignored", glEnumToString(pname), params) }
...
static GLenum direct3DFillModeToOpenGL(D3DFILLMODE mode)
{
  ...
  default: DEBUG_ASSERT(mode == D3DFILL_SOLID || mode == D3DFILL_WIREFRAME);
    return GL_FILL;
  }
}
...
void glGetFloatv(GLenum pname, GLfloat* params)
{
  ...
  case GL_FOG_START:
    params[0] = g_fixedPipeline->m_fogStart;
    break;

  case GL_FOG_END:
    params[0] = g_fixedPipeline->m_fogEnd;
    break;

  default: GL_TRACE("-glGetFloatv(%s, (float *)%p) not implemented", glEnumToString(pname), params);
    DEBUG_ASSERT(false);
    break;
  }
}
...
const D3DXMATRIX& getWorldViewMatrix() { return s_pModelViewMatrixStack->GetTransform(); }

const D3DXMATRIX& getProjMatrix() { return s_pProjectionMatrixStack->GetTransform(); }
...
void glGetDoublev(GLenum pname, GLdouble* params)
{
  ...
  default: GL_TRACE("-glGetDoublev(%s, (double *)%p) not implemented", glEnumToString(pname), params)
  break;
  }
}
               ...
               const GLint viewport[4], GLdouble* winx, GLdouble* winy, GLdouble* winz)
{
  ...
  *winz = win.z;
  ...
  GL_TRACE_IMP(" gluProject(%10.4g, %10.4g, %10.4g, %s, %s, %s, => %10.4g, %10.4g, %10.4g)", objx, objy, objz,
               glArrayToString(modelMatrix, 16), glArrayToString(projMatrix, 16), glArrayToString(viewport, 4), *winx, *winy, *winz)
  return true;
}
                 ...
                 const GLint viewport[4], GLdouble* objx, GLdouble* objy, GLdouble* objz)
{
  ...
  D3DXVECTOR4 win((winx - viewport[0] /* left */) / viewport[2] /* width */, (winy - viewport[1] /* top */) / viewport[3] /* height */,
                  winz, 1.0f);
  ...
  *objz = source.z / source.w;
  ...
  GL_TRACE_IMP(" gluUnProject(%10.4g, %10.4g, %10.4g, %s, %s, %s, => %10.4g, %10.4g, %10.4g,", winx, winy, winz,
               glArrayToString(modelMatrix, 16), glArrayToString(projMatrix, 16), glArrayToString(viewport, 4), *objx, *objy, *objz)
...
void glLightfv(GLenum light, GLenum pname, const GLfloat* params)
{
  ...
  D3DLIGHT9& lightInfo = g_fixedPipeline->m_state.m_light[lightIndex];
  ...
  case GL_POSITION:
    if (params[3] == 0.0f)
    {
      ...
      // normalized dir is required by fixed shader
      D3DXVec3Normalize(static_cast<D3DXVECTOR3*>(&lightInfo.Direction), static_cast<D3DXVECTOR3*>(&lightInfo.Direction));
    }
    else
    {
  ...
  default: GL_TRACE("-glLightfv(%s, %s, (const float *)%p) not implemented", glEnumToString(light), glEnumToString(pname), params);
    break;
  }
}
...
{
  GL_TRACE_IMP(" glLightModelfv(%s, (const float *)%p) ignored", glEnumToString(pname), params);
}
...
void glPolygonOffset(GLfloat _factor, GLfloat _units)
{
  s_currentAttribs.depthBias = _units;
  s_currentAttribs.slopeDepthBias = _factor;

  if (s_currentAttribs.polygonOffsetEnabled)
  {
    g_pd3dDevice->SetRenderState(D3DRS_SLOPESCALEDEPTHBIAS, s_currentAttribs.slopeDepthBias);
    g_pd3dDevice->SetRenderState(D3DRS_DEPTHBIAS, s_currentAttribs.depthBias);
  }
...
void glShadeModel(GLenum mode) { GL_TRACE_IMP(" glShadeModel(%s) not supported", glEnumToString(mode)) }
...
void glAlphaFunc(GLenum func, GLclampf ref)
{
  ...
  g_pd3dDevice->SetRenderState(D3DRS_ALPHAREF, static_cast<DWORD>(cref * 255.0f));
}
...
void glNewList(GLuint list, GLenum mode)
{
  ...
  s_pTargetMatrixStack = s_matrixMode == GL_MODELVIEW ? s_pModelViewMatrixStack : s_pProjectionMatrixStack;
...
void glEndList()
{
  ...
  DEBUG_ASSERT(s_pDisplayListDevice != NULL);
...
void glMaterialfv(GLenum face, GLenum pname, const GLfloat* params)
{
  ...
  {
  case GL_SPECULAR:
    g_fixedPipeline->m_materialSpecular[0] = params[0];
    g_fixedPipeline->m_materialSpecular[1] = params[1];
    g_fixedPipeline->m_materialSpecular[2] = params[2];
    g_fixedPipeline->m_materialSpecular[3] = params[3];
    break;

  case GL_DIFFUSE:
  case GL_AMBIENT_AND_DIFFUSE:
    g_fixedPipeline->m_materialDiffuse[0] = params[0];
    g_fixedPipeline->m_materialDiffuse[1] = params[1];
    g_fixedPipeline->m_materialDiffuse[2] = params[2];
    g_fixedPipeline->m_materialDiffuse[3] = params[3];
    break;

  case GL_SHININESS:
    g_fixedPipeline->m_materialShininess = params[0];
    break;
  }
}
...
GLboolean glIsEnabled(GLenum cap)
{
  ...
  case GL_FOG:
    return g_fixedPipeline->m_fogEnabled;
  ...
  case GL_LIGHT1:
    ...
    //case GL_LIGHT6:
    //case GL_LIGHT7:
    return g_fixedPipeline->m_lightEnabled[cap - GL_LIGHT0];

  case GL_LIGHTING:
    return g_fixedPipeline->m_lighting;
...
void glGetTexLevelParameteriv(GLenum target, GLint level, GLenum pname, GLint* params)
{
  ...
  g_pd3dDevice->GetTexture(s_activeTexture, &baseTexture);
  ...
  case GL_TEXTURE_WIDTH:
    *params = surfaceDesc.Width;
    break;

  case GL_TEXTURE_HEIGHT:
    *params = surfaceDesc.Height;
    break;
  }
}
...
void glTexEnvf(GLenum target, GLenum pname, GLfloat param)
{
  ...
  GL_TRACE("-glTexEnvf(%s, %s, %10.4g) not implemented", glEnumToString(target), glEnumToString(pname), static_cast<double>(param));
  DEBUG_ASSERT(false);
}
...
void glLineWidth(GLfloat width) { GL_TRACE_IMP(" glLineWidth(%10.4g)", (double)width); }
...
void glClipPlane(GLenum plane, const GLdouble* equation)
{
  ...
  // normalize the plane
  D3DXPLANE tempPlane(equation[0], equation[1], equation[2], equation[3]);
  D3DXPLANE viewSpacePlane;
  D3DXPlaneNormalize(&tempPlane, &tempPlane);

  //transform the plane into view space
  D3DXMATRIX tempMatrix = s_pModelViewMatrixStack->GetTransform();
  D3DXMatrixInverse(&tempMatrix, nullptr, &tempMatrix);
  ...
  tempMatrix = s_pProjectionMatrixStack->GetTransform();
  D3DXMatrixInverse(&tempMatrix, nullptr, &tempMatrix);
  D3DXMatrixTranspose(&tempMatrix, &tempMatrix);
  D3DXPLANE planeClip;
  D3DXPlaneTransform(&planeClip, &viewSpacePlane, &tempMatrix);
  float equationAsFloats[4] = {planeClip.a, planeClip.b, planeClip.c, planeClip.d};
...
void glScissor(GLint x, GLint y, GLsizei width, GLsizei height)
{
  ...
  rect.bottom = g_app->m_renderer->ScreenH() - y;
  rect.top = rect.bottom - height;

  g_pd3dDevice->SetScissorRect(&rect);
}

int Shader::SetSamplerGLTextureId(const char* name, int tex) { return SetSamplerDX(name, s_textureIds[tex]); }
