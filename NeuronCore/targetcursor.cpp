...
bool secondaryInputEnabled() {	
	return //g_inputManager->controlEvent( ControlShowMouseCursor ) ||
...
bool disableInputZ() {
	return EclGetWindows()->Size() != 0 || ChatInputWindow::IsChatVisible();
}
...
void TargetCursor::Advance() {
	...
	if ( !disableInputZ() && g_inputManager->controlEvent( ControlTargetMoveZ, details ) &&
