...
class InputDriver
{
  ...
  protected:
    ...
    condition_t getDefaultConditionID(const std::string& name, inputtype_t& type);
  ...
  // Return STATE_DONE if we managed to parse these tokens, and put the parsed information
    ...
    virtual InputParserState parseInputSpecification(const InputSpecTokens& tokens, InputSpec& spec) = 0;

    // Get input state. True if the input was triggered (input condition met). If true,
    // details are placed in details.
    virtual bool getInput(const InputSpec& spec, InputDetails& details) = 0;
    ...
    virtual bool getInputDescription(const InputSpec& spec, InputDescription& desc);

    // Get the name of the driver (debuggung purposes)
    virtual const std::string& getName() const;
};

std::ostream& operator<<(std::ostream& stream, const InputDriver& driver);
