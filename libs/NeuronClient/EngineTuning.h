...
public:
  NumVar(const std::string &path, float val, float minValue = -FLT_MAX, float maxValue = FLT_MAX, float stepSize = 1.0f,
         ActionCallback pfnCallback = EngineVar::DefaultActionHandler);
...
public:
  ...
  void SetListLength(int32_t listLength)
  {
...
public:
  ...
  DynamicEnumVar &operator=(int32_t val)
  {
    m_Value = Clamp(val);
    return *this;
  }
  ...
  virtual void Increment(void) override
  {
    m_Value = (m_Value + 1) % m_EnumCount;
    EngineVar::Increment();
  }

  virtual void Decrement(void) override
  {
    m_Value = (m_Value + m_EnumCount - 1) % m_EnumCount;
    EngineVar::Decrement();
  }
  ...
  void AddEnum(const std::wstring &enumLabel)
  {
    m_EnumLabels.push_back(enumLabel);
    m_EnumCount++;
  }
