...
void DspHandle::Initialize(SoundInstance* _parent) { m_parent = _parent; }
...
{
  static const char* types[] = {"Type2D", "Type3DStationary", "Type3DAttachedToObject", "TypeInEditor"};
...
{
  static const char* types[] = {"Polyphonic", "MonophonicRandom", "MonophonicNearest"};
...
{
  static const char* types[] = {"PlayOnce", "Loop", "LoopADSR"};
...
{
  static const char* types[] = {"Sample", "SampleGroupRandom"};
  ...
  switch (m_positionType)
  {
    ...
    case Type3DAttachedToObject:
    {
      ...
      if (obj->m_id.GetUnitId() == UNIT_BUILDINGS)
        m_pos = static_cast<Building*>(obj)->m_centerPos;
    ...
    case TypeInEditor:
    {
      LegacyVector3 relativePos(g_app->m_soundSystem->m_editorPos.x, 0.0f, g_app->m_soundSystem->m_editorPos.z);
      m_pos = relativePos;
      m_vel.Zero();
...
void SoundInstance::ForceParameter(SoundParameter& _param, float value)
{
  switch (_param.m_link)
  {
    ...
    {
      if (g_app->m_camera)
        m_pos = g_app->m_camera->GetPos() + g_app->m_camera->GetFront() * value;
      break;
    }
  }
}
...
int SoundInstance::GetChannelIndex()
{
  ...
  if (m_channelIndex >= 0 && m_channelIndex < g_app->m_soundSystem->m_numChannels && g_app->m_soundSystem->m_channels[
    m_channelIndex] == m_id)
