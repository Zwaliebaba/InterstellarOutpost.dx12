...
PipeInputDriver::PipeInputDriver()
  : m_specs(),
    lastError(nullErr) { setName("Pipe"); }
...
InputParserState PipeInputDriver::parseInputSpec(ostringstream& stream, InputSpecList& speclist)
{
  ...
  {
    speclist.push_back(std::make_unique<const InputSpec>(spec));
    stream.str("");
    return pState;
  }
  return STATE_CONJ_ERROR;
...
InputParserState PipeInputDriver::parseInputSpecification(const InputSpecTokens& tokens, InputSpec& spec)
{
  string s = "";
  unique_ptr<InputFilterWithArgs> filterWithArgs(NEW InputFilterWithArgs());
  ...
  for (string::const_iterator it = s.begin(); it != s.end(); ++it)
  {
    ...
    switch (*it)
    {
      ...
      case ']':
        ...
        break;
      case ',':
        split = (brackets == 1 && parsingBrackets == 1);
        break;
      default:
...
bool PipeInputDriver::getInput(const InputSpec& spec, InputDetails& details)
{
...
const string& PipeInputDriver::getLastParseError(InputParserState state) { return lastError; }
