...
void VariableGroup::Display(TextContext &Text, float leftMargin, EngineVar *highlightedTweak)
{
  ...
  for (auto iter = m_Children.begin(); iter != m_Children.end(); ++iter)
  {
    ...
    if (iter->second == highlightedTweak)
    {
      ...
      if (temp1 > 0.0f) EngineTuning::s_ScrollOffset += 0.2f * temp1;
      else if (temp2 < 0.0f)
    ...
    if (subGroup != nullptr)
    {
      ...
      if (subGroup->IsExpanded()) Text.DrawString("- ");
      else
        Text.DrawString("+ ");
...
EngineVar *VariableGroup::NextVariable(EngineVar *curVar)
{
  ...
  if (nextIter == m_Children.end()) return m_GroupPtr ? m_GroupPtr->NextVariable(this) : nullptr;
  else
...
EngineVar *EngineVar::NextVar(void)
{
  ...
  if (isGroup != nullptr && isGroup->IsExpanded()) next = isGroup->FirstVariable();

  if (next == nullptr) next = m_GroupPtr->NextVariable(this);
...
EngineVar *EngineVar::PrevVar(void)
{
  ...
  if (prev != nullptr && prev != m_GroupPtr)
  {
    ...
    if (isGroup != nullptr && isGroup->IsExpanded()) prev = isGroup->LastVariable();
  }
...
void NumVar::SetValue(FILE *file, const std::string &setting)
{
  ...
  if (fscanf_s(file, scanString.c_str(), &valueRead)) *this = valueRead;
}
...
void CallbackTrigger::DisplayValue(TextContext &Text) const
{
  ...
  if (m_BangDisplay > 0) --m_BangDisplay;
}
...
{
  if (!GameInput::IsPressed(button)) return;
  ...
  if (Floor(durationHeld * timeStretch) > Floor(oldDuration * timeStretch)) action();
}

void EngineTuning::Update(float frameTime)
{
  if (GameInput::IsFirstPressed(GameInput::kBackButton) || GameInput::IsFirstPressed(GameInput::kKey_back)) sm_IsVisible = !sm_IsVisible;

  if (!sm_IsVisible) return;
  ...
  if (sm_SelectedVariable == nullptr) return;
  ...
  if (GameInput::IsFirstPressed(GameInput::kAButton) || GameInput::IsFirstPressed(GameInput::kKey_return)) sm_SelectedVariable->Bang();
}
