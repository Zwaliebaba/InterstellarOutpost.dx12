...
class DspBlueprint
{
  public:
    ...
    char* GetParameter(int _param, float* _min = nullptr, float* _max = nullptr, float* _default = nullptr,
                       int* _dataType = nullptr);
};
...
class SoundSystem
{
  ...
  protected:
    ...
    static bool SoundLibraryMainCallback(unsigned int _channel, signed short* _data, unsigned int _numSamples,
  ...
  public:
    ...
    IsSoundSourceOK(
      const char*
      _soundName);						// Tests that file names and file formats are OK, returns an error code from the SoundSource enum
