...
ConjoinInputDriver::ConjoinInputDriver()
  : m_specs(),
    lastError(nullErr) { setName("Conjoin"); }

InputParserState ConjoinInputDriver::parseInputSpecification(const InputSpecTokens& tokens, InputSpec& spec)
{
  string s = "";
  ...
  for (int i = 0; i <= tokens.length(); ++i)
  {
    ...
    else
    {
      ...
      if (PARSE_SUCCESS(pState))
      {
        ...
        speclist->push_back(std::make_unique<const InputSpec>(partspec));
...
bool ConjoinInputDriver::getInput(const InputSpec& spec, InputDetails& details)
{
