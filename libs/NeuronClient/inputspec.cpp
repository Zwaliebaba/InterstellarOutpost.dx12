...
unsigned InputSpecTokens::length() const { return m_tokens.size(); }
...
unique_ptr<InputSpecTokens> InputSpecTokens::operator()(int _start, int _end) const
{
  vector<string> vec;
  vec.clear();
  ...
  if (_start < 0)
    _start += len; // Allow negatives to count from the last token
  if (_end < 0)
    _end += len;

  if (0 <= _start && _start < len && 0 <= _end && _end < len && _start <= _end)
  { // We're in range
    for (int idx = _start; idx <= _end; idx++)
      vec.push_back((*this)[idx]);
  }

  unique_ptr<InputSpecTokens> tokens(NEW InputSpecTokens(vec));
  return tokens;
}
