...
namespace Neuron::Math
{
  ...
  inline bool Frustum::IntersectSphere(BoundingSphere sphere) const
  {
    ...
    for (int i = 0; i < 6; ++i)
      if (m_FrustumPlanes[i].DistanceFromPoint(sphere.GetCenter()) + radius < 0.0f) return false;
    return true;
  }
  ...
  inline bool Frustum::IntersectBoundingBox(const AxisAlignedBox &aabb) const
  {
    for (int i = 0; i < 6; ++i)
    {
      ...
      if (p.DistanceFromPoint(farCorner) < 0.0f) return false;
    }
  ...
  inline Frustum operator*(const OrthogonalTransform &xform, const Frustum &frustum)
  {
    ...
    for (int i = 0; i < 8; ++i) result.m_FrustumCorners[i] = xform * frustum.m_FrustumCorners[i];

    for (int i = 0; i < 6; ++i) result.m_FrustumPlanes[i] = xform * frustum.m_FrustumPlanes[i];
  ...
  inline Frustum operator*(const AffineTransform &xform, const Frustum &frustum)
  {
    ...
    for (int i = 0; i < 6; ++i) result.m_FrustumPlanes[i] = BoundingPlane(XForm * Vector4(frustum.m_FrustumPlanes[i]));
  ...
  inline Frustum operator*(const Matrix4 &mtx, const Frustum &frustum)
  {
    ...
    for (int i = 0; i < 8; ++i) result.m_FrustumCorners[i] = Vector3(mtx * frustum.m_FrustumCorners[i]);
