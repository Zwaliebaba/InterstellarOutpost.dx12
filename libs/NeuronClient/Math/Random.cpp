...
uint32_t Math::RandomNumberGenerator::Rand()
{
  if (m_Pos == n)
  {
    for (uint32_t i = 0; i < n - m; ++i) m_State[i] = m_State[i + m] ^ Twiddle(m_State[i], m_State[i + 1]);
    for (uint32_t i = n - m; i < n - 1; ++i) m_State[i] = m_State[i + m - n] ^ Twiddle(m_State[i], m_State[i + 1]);
...
void Math::RandomNumberGenerator::SetSeed(uint32_t s)
{
  ...
  for (int i = 1; i < n; ++i) m_State[i] = 1812433253ul * (m_State[i - 1] ^ (m_State[i - 1] >> 30)) + i;
