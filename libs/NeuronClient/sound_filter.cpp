...
void DspResLowPass::CalcCoefs(float _frequency, float _resonance, float _gain)
{
  ...
  m_cosOmega = cos(omega);
  m_sinOmega = sin(omega);
...
void DspResLowPass::SetParameters(const float* _params)
{
...
void DspResLowPass::Process(signed short* _data, unsigned int _numSamples)
{
  ...
  m_yn1 += static_cast<float>(1.0E-26);
...
void DspBitCrusher::SetParameters(const float* _params) { m_bitRate = _params[0]; }
...
void DspBitCrusher::Process(signed short* _data, unsigned int _numSamples)
{
  ...
  for (int i = 0; i < _numSamples; ++i)
  {
    ...
    if (output > 32767)
      output = 32767;
    _data[i] = static_cast<signed short>(output);
  }
}
...
void DspGargle::ProcessTriangle(signed short* _data, unsigned int _numSamples)
{
  ...
  float inc = (2.0f * m_freq) / static_cast<float>(m_sampleRate);
...
void DspGargle::SetParameters(const float* _params)
{
    ...
    m_currentBufferIndex(0)
{
  ...
  int delayInSamples = static_cast<float>(_sampleRate) * (m_delay * 0.001f);
...
void DspEcho::SetParameters(const float* _params)
{
...
void DspEcho::Process(signed short* _data, unsigned int _numSamples)
{
  ...
  while (i < _numSamples)
  {
    ...
    {
      float temp = _data[i];
      float result = _data[i] * dryProportion + m_buffer[j] * wetProportion;
      if (result < -32766.0f)
        result = -32766.0f;
...
void DspReverb::SetParameters(const float* _params) {}
...
void DspReverb::Process(signed short* _data, unsigned int _numSamples)
{
  ...
  int i, j, k = 0, s;
  const int delaySize = 3 * m_sampleRate;
