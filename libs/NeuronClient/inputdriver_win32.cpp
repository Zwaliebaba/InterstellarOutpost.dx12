...
W32InputDriver::W32InputDriver()
{
  ...
  EventManager::AddEventProcessor(WndProc);

  g_win32InputDriver = this;
}

W32InputDriver::~W32InputDriver() { EventManager::RemoveEventProcessor(WndProc); }
...
bool W32InputDriver::getKeyInput(const InputSpec& spec, InputDetails& details)
{
...
bool W32InputDriver::getMouseInput(const InputSpec& spec, InputDetails& details)
{
  int button = -1;
...
LRESULT CALLBACK W32InputDriver::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  switch (message)
  {
    ...
    case WM_MOUSEMOVE:
    {
      ...
      short newPosY = static_cast<short>(lParam >> 16);
    ...
    case WM_SYSKEYDOWN:
    {
      int flags = static_cast<short>(HIWORD(lParam));
      if (wParam == KEY_ALT)
        g_keys[wParam] = 1;
    ...
    case WM_KEYDOWN:
    {
      ...
      Canvas::EclUpdateKeyboard(wParam, g_keys[KEY_SHIFT] == 1, g_keys[KEY_CONTROL] == 1, g_keys[KEY_ALT] == 1);
      break;
    }
...
bool W32InputDriver::acceptDriver(const string& name)
{
  if (name == "key")
...
control_id_t W32InputDriver::getControlID(const string& name)
{
...
control_id_t W32InputDriver::getKeyId(const string& keyName)
{
...
bool W32InputDriver::getInputDescription(const InputSpec& spec, InputDescription& desc)
{
  if (KEY_DRIVER == spec.handler_id)
  {
    ...
    if (keyInRange)
    {
      ...
      desc.noun = Strings::Get(desc.noun);
    }
    else
      desc.noun = "control_key_unknown";

    std::string prefCond;
    if (getDefaultVerb(spec.condition, INPUT_TYPE_BOOL, desc.verb) && getDefaultPrefsString(
      spec.condition, INPUT_TYPE_BOOL, prefCond))
    {
      desc.pref = "key ";
  ...
  if (MOUSE_DRIVER == spec.handler_id)
  {
    bool ret = true;
