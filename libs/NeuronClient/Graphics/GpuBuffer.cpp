...
void GpuBuffer::Create(const std::wstring &name, uint32_t NumElements, uint32_t ElementSize, const void *initialData)
{
  ...
  if (initialData) CommandContext::InitializeBuffer(*this, initialData, m_BufferSize);
...
void ByteAddressBuffer::CreateDerivedViews(void)
{
  ...
  if (m_SRV.ptr == D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN) m_SRV = AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
  ...
  if (m_UAV.ptr == D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN) m_UAV = AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
