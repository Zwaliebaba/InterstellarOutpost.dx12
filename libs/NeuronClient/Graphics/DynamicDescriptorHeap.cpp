...
std::vector<com_ptr<ID3D12DescriptorHeap>> DynamicDescriptorHeap::sm_DescriptorHeapPool[2];
                                                   ...
                                                   const std::vector<ID3D12DescriptorHeap *> &UsedHeaps)
{
  uint32_t idx = HeapType == D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER ? 1 : 0;
  std::lock_guard<std::mutex> LockGuard(sm_Mutex);
  for (auto iter = UsedHeaps.begin(); iter != UsedHeaps.end(); ++iter) sm_RetiredDescriptorHeaps[idx].push(std::make_pair(FenceValue, *iter));
}
...
void DynamicDescriptorHeap::DescriptorHandleCache::UnbindAllValid()
{
  ...
  while (_BitScanForward(&RootIndex, TableParams))
  {
    ...
    if (m_RootDescriptorTable[RootIndex].AssignedHandlesBitMap != 0) m_StaleRootParamsBitMap |= (1 << RootIndex);
  }
}
                                                                          ...
                                                                          const D3D12_CPU_DESCRIPTOR_HANDLE Handles[])
{
  ...
  for (UINT i = 0; i < NumHandles; ++i) CopyDest[i] = Handles[i];
