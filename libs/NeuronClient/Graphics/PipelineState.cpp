...
static map<size_t, com_ptr<ID3D12PipelineState>> s_GraphicsPSOHashMap;
static map<size_t, com_ptr<ID3D12PipelineState>> s_ComputePSOHashMap;
...
void GraphicsPSO::SetRenderTargetFormats(UINT NumRTVs, const DXGI_FORMAT *RTVFormats, DXGI_FORMAT DSVFormat, UINT MsaaCount, UINT MsaaQuality)
{
  ...
  for (UINT i = NumRTVs; i < m_PSODesc.NumRenderTargets; ++i) m_PSODesc.RTVFormats[i] = DXGI_FORMAT_UNKNOWN;
...
void GraphicsPSO::Finalize()
{
  ...
  {
    while (*PSORef == nullptr) this_thread::yield();
    m_PSO = *PSORef;
  }
}
