...
std::vector<com_ptr<ID3D12DescriptorHeap>> DescriptorAllocator::sm_DescriptorHeapPool;
...
bool DescriptorHeap::ValidateHandle(const DescriptorHandle &DHandle) const
{
  ...
  if (DHandle.GetGpuPtr() - m_FirstHandle.GetGpuPtr() != DHandle.GetCpuPtr() - m_FirstHandle.GetCpuPtr()) return false;
