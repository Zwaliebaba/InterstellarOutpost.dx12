...
#include "Shaders/Compiled/AoBlurUpsampleBlendOutCS.h"
#include "Shaders/Compiled/AoBlurUpsampleCS.h"
#include "Shaders/Compiled/AoBlurUpsamplePreMinBlendOutCS.h"
#include "Shaders/Compiled/AoBlurUpsamplePreMinCS.h"
#include "Shaders/Compiled/AoPrepareDepthBuffers1CS.h"
#include "Shaders/Compiled/AoPrepareDepthBuffers2CS.h"
#include "Shaders/Compiled/AoRender1CS.h"
#include "Shaders/Compiled/AoRender2CS.h"
#include "Shaders/Compiled/DebugSSAOCS.h"
#include "Shaders/Compiled/LinearizeDepthCS.h"
...
namespace SSAO
{
  void ComputeAO(ComputeContext &Context, ColorBuffer &Destination, ColorBuffer &DepthBuffer, const float TanHalfFovH)
  {
    ...
    if (ArrayCount == 1) Context.Dispatch2D(BufferWidth, BufferHeight, 16, 16);
    else
      Context.Dispatch3D(BufferWidth, BufferHeight, ArrayCount, 8, 8, 1);
  }

  void BlurAndUpsample(ComputeContext &Context, ColorBuffer &Destination, ColorBuffer &HiResDepth, ColorBuffer &LoResDepth,
                       ColorBuffer *InterleavedAO, ColorBuffer *HighQualityAO, ColorBuffer *HiResAO)
  {
    ...
    if (HiResAO == nullptr) shader = &s_BlurUpsampleFinal[HighQualityAO == nullptr ? 0 : 1];
    else
...
void SSAO::Render(GraphicsContext &GfxContext, const float *ProjMat, float NearClipDist, float FarClipDist)
{
  ...
  if (!Enable)
  {
    ...
    if (ComputeLinearZ) LinearizeZ(Context, Depth, LinearDepth, zMagic);
  ...
  if (AsyncCompute) Context.Finish();
  else
    EngineProfiling::EndBlock(&GfxContext);

  if (DebugDraw)
  {
    if (AsyncCompute) g_CommandManager.GetGraphicsQueue().StallForProducer(g_CommandManager.GetComputeQueue());
