                                       ...
                                       size_t *bitSize)
{
  ...
  if (!GetFileInformationByHandleEx(hFile.get(), FileStandardInfo, &fileInfo, sizeof(fileInfo))) return HRESULT_FROM_WIN32(GetLastError());
  ...
  // read the data in
  DWORD BytesRead = 0;
  if (!ReadFile(hFile.get(), ddsData.get(), FileSize.LowPart, &BytesRead, nullptr)) return HRESULT_FROM_WIN32(GetLastError());
  ...
  if (hdr->size != sizeof(DDS_HEADER) || hdr->ddspf.size != sizeof(DDS_PIXELFORMAT)) return E_FAIL;
  ...
  {
    if (MAKEFOURCC('D', 'X', '1', '0') == hdr->ddspf.fourCC) offset += sizeof(DDS_HEADER_DXT10);
  }
...
static DXGI_FORMAT GetDXGIFormat(const DDS_PIXELFORMAT &ddpf)
{
  ...
  {
    if (8 == ddpf.RGBBitCount) return DXGI_FORMAT_A8_UNORM;
  }
  ...
  switch (resDim)
  {
    case D3D12_RESOURCE_DIMENSION_TEXTURE1D:
    {
      ...
      if (SUCCEEDED(hr) && tex != nullptr)
      {
        ...
        if (texture != nullptr) { *texture = tex; }
        else
        {
                                    ...
                                    _In_ D3D12_CPU_DESCRIPTOR_HANDLE textureView)
{
  ...
  if ((header->ddspf.flags & DDS_FOURCC) && (MAKEFOURCC('D', 'X', '1', '0') == header->ddspf.fourCC))
  {
    ...
    switch (d3d10ext->resourceDimension)
    {
      case D3D12_RESOURCE_DIMENSION_TEXTURE1D:
        ...
        if ((header->flags & DDS_HEIGHT) && height != 1) return HRESULT_FROM_WIN32(ERROR_INVALID_DATA);
        height = depth = 1;
        break;
  ...
  if (mipCount > D3D12_REQ_MIP_LEVELS) return HRESULT_FROM_WIN32(ERROR_NOT_SUPPORTED);
                                                          ...
                                                          DDS_ALPHA_MODE *alphaMode)
{
  ...
  if (!d3dDevice || !ddsData) return E_INVALIDARG;

  // Validate DDS file in memory
  if (ddsDataSize < (sizeof(uint32_t) + sizeof(DDS_HEADER))) return E_FAIL;
  ...
  // Verify header to validate DDS file
  if (header->size != sizeof(DDS_HEADER) || header->ddspf.size != sizeof(DDS_PIXELFORMAT)) return E_FAIL;
  ...
  if (header->ddspf.flags & DDS_FOURCC)
  {
    if (MAKEFOURCC('D', 'X', '1', '0') == header->ddspf.fourCC) offset += sizeof(DDS_HEADER_DXT10);
  }

  // Must be long enough for all headers and magic value
  if (ddsDataSize < offset) return E_FAIL;

  HRESULT hr = CreateTextureFromDDS(d3dDevice, header, ddsData + offset, ddsDataSize - offset, maxsize, forceSRGB, texture, textureView);
  if (SUCCEEDED(hr))
  {
    if (texture != nullptr && *texture != nullptr) (*texture)->SetName(L"DDSTextureLoader");
...
_Use_decl_annotations_ HRESULT CreateDDSTextureFromFile(ID3D12Device *d3dDevice, const wchar_t *fileName, size_t maxsize, bool forceSRGB,
                                                        ID3D12Resource **texture, D3D12_CPU_DESCRIPTOR_HANDLE textureView, DDS_ALPHA_MODE *alphaMode)
{
  if (texture) *texture = nullptr;

  if (alphaMode) *alphaMode = DDS_ALPHA_MODE_UNKNOWN;

  if (!d3dDevice || !fileName) return E_INVALIDARG;
  ...
  if (SUCCEEDED(hr)) (*texture)->SetName(fileName);
