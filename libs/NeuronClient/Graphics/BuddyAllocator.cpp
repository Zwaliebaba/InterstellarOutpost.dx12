...
void BuddyBlock::InitFromResource(ByteAddressBuffer *pBuffer, uint32_t numElements, uint32_t elementSize, const void *initialData)
{
  ...
  if (initialData) CommandContext::InitializeBuffer(*pBuffer, initialData, numElements * elementSize, m_offset);
}
...
BuddyAllocator::BuddyAllocator(kBuddyAllocationStrategy allocationStrategy, D3D12_HEAP_TYPE heapType, size_t maxBlockSize, size_t MinBlockSize,
                               size_t baseOffset)
    : m_allocationStrategy(allocationStrategy), m_heapType(heapType), m_baseOffset(baseOffset), m_maxBlockSize(maxBlockSize),
      m_minBlockSize(MinBlockSize), m_pBackingHeap(nullptr)
#if defined(PROFILE) || defined(_DEBUG)
      ,
      m_SpaceUsed(0), m_InternalFragmentation(0)
#endif
{
...
{
  if (m_allocationStrategy == kBuddyAllocationStrategy::kPlacedResourceStrategy) m_pBackingHeap->Release();
  else
...
size_t BuddyAllocator::AllocateBlock(UINT order)
{
  ...
  if (order > m_maxOrder) throw(std::bad_alloc());// Can't allocate a block that large
