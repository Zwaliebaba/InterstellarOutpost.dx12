...
#include "Shaders/Compiled/PerfGraphBackgroundVS.h"
#include "Shaders/Compiled/PerfGraphPS.h"
#include "Shaders/Compiled/PerfGraphVS.h"
...
public:
  ...
  {
    for (uint32_t i = 0; i < debugVarCount; ++i) m_PerfTimesCPUBuffer.emplace_back(new float[NodeCount]);
  }
  ...
  {
    for (uint32_t i = 0; i < m_DebugVarCount; i++) m_PerfTimesCPUBuffer[i][frameID % m_NodeCount] = timeStamps[i];
  }
...
public:
  GraphVector(uint32_t MaxActiveGraphs, uint32_t DebugVarCount)
      : m_MaxActiveGraphs(MaxActiveGraphs), m_ActiveGraphs(0), m_DebugVarCount(DebugVarCount), m_MinAbs((float) PERF_GRAPH_ERROR), m_MaxAbs(0.0f),
        m_FrameOfMinAbs(0), m_FrameOfMaxAbs(0)
  {
  ...
  float GetGlobalPresetMax()
  {
    ...
    for (uint32_t i = 0; i < m_DebugVarCount; ++i)
      if (m_PresetMax[i] > max) max = m_PresetMax[i];
    return max;
  }
  ...
  {
    for (uint32_t i = 0; i < m_DebugVarCount; ++i) m_PresetMax[i] = maxArray[i];
  }
  ...
  void ManageMax(float *InputNode, uint32_t nodeCount, uint32_t FrameID)
  {
    ...
    {
      //Absolute min max
      if (FrameID - m_FrameOfMinAbs > nodeCount) m_MinAbs = (float) PERF_GRAPH_ERROR;

      if (FrameID - m_FrameOfMaxAbs > nodeCount) m_MaxAbs = 0.0f;
      ...
      //Relative min max
      if (FrameID - m_FrameOfMax[i] > nodeCount) m_Max[i] = 0.0f;

      if (FrameID - m_FrameOfMin[i] > nodeCount) m_Min[i] = (float) PERF_GRAPH_ERROR;
...
{
  if (type == GraphType::Profile) return ProfileGraphs.AddGraph(new PerfGraph(PROFILE_NODE_COUNT, 2));
...
{
  if (GraphID == PERF_GRAPH_ERROR) return false;

  if (Type == GraphType::Profile) return ProfileGraphs.Toggle(GraphID);
...
{
  if (Type == GraphType::Profile) return ProfileGraphs.GetColor(GraphID);
  else// Type == GraphType::Global
...
{
  if (GraphID == PERF_GRAPH_ERROR) return;
...
void PerfGraph::RenderGraph(GraphicsContext &Context, uint32_t vertexCount, D3D12_VIEWPORT &viewport, uint32_t debugVarCount, float topMargin)
{
  ...
  for (uint32_t i = 0; i < debugVarCount; ++i)
  {
    ...
    if (debugVarCount > 1) viewport.TopLeftY += viewport.Height + topMargin;
  }
}
