...
protected:
  ...
  static std::vector<com_ptr<ID3D12DescriptorHeap>> sm_DescriptorHeapPool;
...
public:
  ...
  {
    if (m_CpuHandle.ptr != D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN) m_CpuHandle.ptr += OffsetScaledByDescriptorSize;
    if (m_GpuHandle.ptr != D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN) m_GpuHandle.ptr += OffsetScaledByDescriptorSize;
  }
