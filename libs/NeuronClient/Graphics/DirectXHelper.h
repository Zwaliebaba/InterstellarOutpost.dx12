...
protected:
  ...
  ~GpuUploadBuffer()
  {
    if (m_resource.get()) m_resource->Unmap(0, nullptr);
  }
  ...
  void Allocate(ID3D12Device5 *device, UINT bufferSize, LPCWSTR resourceName = nullptr)
  {
    ...
    if (resourceName) m_resource->SetName(resourceName);
  }
...
template<class T> class ConstantBuffer : public GpuUploadBuffer
{
...
public:
  ConstantBuffer() : m_mappedConstantData(nullptr), m_alignedInstanceSize(0), m_numInstances(0) {}
...
template<class T> class StructuredBuffer : public GpuUploadBuffer
{
  T *m_mappedBuffers;
