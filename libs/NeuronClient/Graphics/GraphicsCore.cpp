...
using namespace Neuron::Client::Graphics;
...
void Core::CreateDeviceResources()
{
  ...
  if (m_depthBufferFormat != DXGI_FORMAT_UNKNOWN)
  {
    ...
    check_hresult(
        m_d3dDevice->CreateDescriptorHeap(&dsvDescriptorHeapDesc, IID_GRAPHICS_PPV_ARGS(m_dsvDescriptorHeap)));
    m_dsvDescriptorHeap->SetName(L"DeviceResources");
  }
  // Create a command allocator for each back buffer that will be rendered to.
  for (UINT n = 0; n < m_backBufferCount; n++)
  {
    check_hresult(m_d3dDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT,
                                                      IID_GRAPHICS_PPV_ARGS(m_commandAllocators[n])));
    wchar_t name[25] = {};
  ...
  check_hresult(m_d3dDevice->CreateFence(m_fenceValues[m_backBufferIndex], D3D12_FENCE_FLAG_NONE,
                                         IID_GRAPHICS_PPV_ARGS(m_fence)));
...
void Core::CreateWindowSizeDependentResources()
{
  ...
  if (m_swapChain)
  {
    ...
    if (hr == DXGI_ERROR_DEVICE_REMOVED || hr == DXGI_ERROR_DEVICE_RESET)
    {
      ...
      char buff[64] = {};
      sprintf_s(
          buff, "Device Lost on ResizeBuffers: Reason code 0x%08X\n",
  ...
  else
  {
    ...
    check_hresult(m_dxgiFactory->CreateSwapChainForHwnd(m_commandQueue.get(), m_window, &swapChainDesc,
  ...
  if (m_depthBufferFormat != DXGI_FORMAT_UNKNOWN)
  {
    ...
    const CD3DX12_CLEAR_VALUE depthOptimizedClearValue(m_depthBufferFormat, (m_options & c_ReverseDepth) ? 0.0f : 1.0f,
                                                       0u);
  ...
  *ppAdapter = nullptr;
  ...
  if (factory6 != nullptr)
  {
    for (UINT adapterIndex = 0; SUCCEEDED(factory6->EnumAdapterByGpuPreference(
             adapterIndex, DXGI_GPU_PREFERENCE_HIGH_PERFORMANCE, IID_GRAPHICS_PPV_ARGS(adapter)));
         adapterIndex++)
    {
