...
void ColorBuffer::GenerateMipMaps(CommandContext &BaseContext)
{
  ...
  for (uint32_t TopMip = 0; TopMip < m_NumMipMaps;)
  {
    ...
    if (m_Format == DXGI_FORMAT_R8G8B8A8_UNORM_SRGB) Context.SetPipelineState(Graphics::g_GenerateMipsGammaPSO[NonPowerOfTwo]);
    else
    ...
    if (TopMip + NumMips > m_NumMipMaps) NumMips = m_NumMipMaps - TopMip;
