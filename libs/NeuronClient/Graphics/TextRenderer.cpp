...
#include "Shaders/Compiled/TextAntialiasPS.h"
#include "Shaders/Compiled/TextShadowPS.h"
#include "Shaders/Compiled/TextVS.h"
...
namespace TextRenderer
{
  ...
  public:
    ...
    void LoadFromBinary(const wchar_t *fontName, const uint8_t *pBinary, const size_t binarySize)
    {
      ...
      for (uint16_t i = 0; i < NumGlyphs; ++i) m_Dictionary[wcharList[i]] = glyphData[i];
  ...
  const Font *GetOrLoadFont(const wstring &filename)
  {
    ...
    if (fontIter != LoadedFonts.end()) return fontIter->second.get();

    Font *newFont = new Font();
    if (filename == L"default") newFont->LoadFromBinary(L"default", g_pconsola24, sizeof(g_pconsola24));
    else
...
{
  if (m_EnableShadow == enable) return;
...
void TextContext::SetFont(const wstring &fontName, float size)
{
  ...
  if (size > 0.0f) m_VSParams.TextSize = size;
...
{
  if (m_VSParams.TextSize == size) return;
