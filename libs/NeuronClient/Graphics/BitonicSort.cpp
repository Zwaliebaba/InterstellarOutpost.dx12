...
#include "Shaders/Compiled/Bitonic32InnerSortCS.h"
#include "Shaders/Compiled/Bitonic32OuterSortCS.h"
#include "Shaders/Compiled/Bitonic32PreSortCS.h"
#include "Shaders/Compiled/Bitonic64InnerSortCS.h"
#include "Shaders/Compiled/Bitonic64OuterSortCS.h"
#include "Shaders/Compiled/Bitonic64PreSortCS.h"
#include "Shaders/Compiled/BitonicIndirectArgsCS.h"
...
template<typename T> inline void VerifySort(T *List, uint32_t ListLength, bool bAscending)
{
...
void TestBitonicSort(uint32_t ListSize, bool b64Bit, bool bAscending)
{
  ...
  if (b64Bit)
  {
    ...
    for (uint32_t i = 0; i < ListSize; ++i) BufferPtr64[i] = ((uint64_t) (uint32_t) Math::g_RNG.NextInt() << 32 | i);
  }
  else
  {
    uint32_t *BufferPtr32 = (uint32_t *) BufferPtr;
    for (uint32_t i = 0; i < ListSize; ++i) BufferPtr32[i] = (((uint32_t) Math::g_RNG.NextInt() & ~IndexMask) | i);
  }
  ...
  if (b64Bit) VerifySort((uint64_t *) BufferPtr, ListSize, bAscending);
  else
