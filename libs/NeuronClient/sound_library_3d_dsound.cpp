...
#define SOUNDASSERT(x, msg)     { if(x != DS_OK && x != S_FALSE) {  \
...
static constexpr DirectFXDescriptor s_dxDescriptors[] = {
...
class DirectSoundChannel
{
  public:
    ...
    LegacyVector3 m_pos;					// Value recorded on previous call of SetChannelPosition
...
void DirectSoundChannel::UpdateSimulatedPlayCursor(long _playCursor)
{
  ...
  if (_playCursor < 0)
  {
    ...
    m_simulatedPlayCursor += timeSinceLastUpdate * static_cast<double>(m_freq) * 2.0;
...
SoundLibrary3dDirectSound::SoundLibrary3dDirectSound()
  : SoundLibrary3d(),
    m_channels(nullptr),
    m_musicChannel(nullptr) { m_directSound = NEW DirectSoundData; }
...
SoundLibrary3dDirectSound::~SoundLibrary3dDirectSound()
{
  ...
  {
    auto buffer = static_cast<IDirectSoundBuffer8*>(m_channels[i].m_bufferInterface);
    errCode = buffer->Stop();
  ...
  auto buffer = static_cast<IDirectSoundBuffer8*>(m_musicChannel->m_bufferInterface);
  errCode = buffer->Stop();
...
IDirectSoundBuffer* SoundLibrary3dDirectSound::CreateSecondaryBuffer(int _numSamples)
{
  ...
  int flags = DSBCAPS_CTRL3D | DSBCAPS_CTRLFX | DSBCAPS_CTRLFREQUENCY | DSBCAPS_CTRLVOLUME | DSBCAPS_GETCURRENTPOSITION2;
  ...
  int errCode = m_directSound->m_device->CreateSoundBuffer(&dsbd, &buffer, nullptr);
  ...
  errCode = buffer->Lock(0,	        // Offset
                         ...
                         &buf1, &buf1Size, nullptr, nullptr, DSBLOCK_ENTIREBUFFER);
...
void SoundLibrary3dDirectSound::Initialize(int _mixFreq, int _numChannels, bool _hw3d, int _mainBufNumSamples,
                                           int _musicBufNumSamples)
{
  ...
  int errCode = DirectSoundCreate8(nullptr, &m_directSound->m_device, nullptr);
  ...
  {
    int flags = DSBCAPS_PRIMARYBUFFER | DSBCAPS_CTRL3D;
    if (Hardware3DSupport() && m_hw3dDesired)
      flags |= DSBCAPS_LOCHARDWARE;
    else
      flags |= DSBCAPS_LOCSOFTWARE;
  ...
  for (int i = 0; i < m_numChannels; ++i)
  {
    ...
    errCode = m_channels[i].m_bufferInterface->QueryInterface(IID_IDirectSound3DBuffer, (void**)&m_channels[i].m_buffer3DInterface);
...
bool SoundLibrary3dDirectSound::Hardware3DSupport() { return (m_directSound->m_caps.dwMaxHw3DStreamingBuffers > 0); }
...
int SoundLibrary3dDirectSound::GetCPUOverhead()
{
  ...
  for (int i = 0; i < m_numChannels; ++i)
  {
    ...
    else SOUNDASSERT(errCode, "Direct sound couldn't get CPU overhead");
...
void SoundLibrary3dDirectSound::SetChannel3DMode(int _channel, int _mode)
{
  ...
  if (_mode != channel->m_3DMode)
  {
    ...
    {
      case 0:
        errCode = buffer3d->SetMode(DS3DMODE_NORMAL, DS3D_IMMEDIATE);
        break;
      case 1:
        errCode = buffer3d->SetMode(DS3DMODE_HEADRELATIVE, DS3D_IMMEDIATE);
        break;
      case 2:
        errCode = buffer3d->SetMode(DS3DMODE_DISABLE, DS3D_IMMEDIATE);
        break;
    }
...
void SoundLibrary3dDirectSound::SetChannelPosition(int _channel, const LegacyVector3& _pos, const LegacyVector3& _vel)
{
  DirectSoundChannel* channel;
  if (_channel == m_musicChannelId)
    channel = m_musicChannel;
  ...
  LegacyVector3 listenerToOldPos(_pos - m_listenerPos);
  LegacyVector3 oldPosToNewPos(_pos - channel->m_pos);
...
void SoundLibrary3dDirectSound::SetChannelFrequency(int _channel, int _frequency)
{
  ...
  if (channel->m_freq != _frequency)
  {
    ...
    if (errCode == DSERR_BUFFERLOST)
      buffer->Restore();
    else SOUNDASSERT(errCode, "Direct sound couldn't set channel frequency");
  }
}
...
void SoundLibrary3dDirectSound::SetChannelMinDistance(int _channel, float _minDistance)
{
  ...
  if (!NearlyEquals(_minDistance, channel->m_minDist))
  {
    ASSERT_TEXT(_minDistance >= DS3D_DEFAULTMINDISTANCE && _minDistance <= DS3D_DEFAULTMAXDISTANCE,
                "Channel MinDistance must be between %2.2f and %2.2f." "You requested %2.2f", DS3D_DEFAULTMINDISTANCE,
                DS3D_DEFAULTMAXDISTANCE, _minDistance);
...
void SoundLibrary3dDirectSound::SetChannelVolume(int _channel, float _volume)
{
  ...
  if (!NearlyEquals(_volume, channel->m_volume))
  {
    ...
    if (calculatedVolume < -10000.0f)
      calculatedVolume = -10000.0f;
    ...
    if (errCode == DSERR_BUFFERLOST)
      buffer->Restore();
    else SOUNDASSERT(errCode, "Direct sound couldn't set buffer volume");
  }
}

void SoundLibrary3dDirectSound::SetListenerPosition(const LegacyVector3& _pos, const LegacyVector3& _front,
                                                    const LegacyVector3& _up, const LegacyVector3& _vel)
{
  m_listenerPos = _pos;
  ...
  errCode = listener->SetOrientation(_front.x, _front.y, _front.z, _up.x, _up.y, _up.z, DS3D_DEFERRED);
...
void SoundLibrary3dDirectSound::CommitChanges()
{
  ...
  if (errCode == DSERR_BUFFERLOST)
    return;
  else SOUNDASSERT(errCode, "Direct sound couldn't commit deferred listener changes");
}
...
void SoundLibrary3dDirectSound::PopulateBuffer(int _channel, int _fromSample, int _numSamples, bool _isMusic)
{
  ...
  int errCode = channel->m_bufferInterface->Lock(byteOffset, byteSize, &buf1, &size1, &buf2, &size2, 0);
  END_PROFILE(g_app->m_profiler, "LockBuf");
  if (errCode == DSERR_BUFFERLOST)
    channel->m_bufferInterface->Restore();
  else SOUNDASSERT(errCode, "Direct sound couldn't get a lock on a secondary buffer");
  ...
  {
    if (buf1)
      m_musicCallback(static_cast<signed short*>(buf1), size1 / 2, &channel->m_silenceRemaining);
    if (buf2)
      m_musicCallback(static_cast<signed short*>(buf2), size2 / 2, &channel->m_silenceRemaining);
  }
  else if (!_isMusic && m_mainCallback)
  {
    if (buf1)
      m_mainCallback(_channel, static_cast<signed short*>(buf1), size1 / 2, &channel->m_silenceRemaining);
    if (buf2)
      m_mainCallback(_channel, static_cast<signed short*>(buf2), size2 / 2, &channel->m_silenceRemaining);
  }
  ...
  {
    if (channel->m_dspFX[i].m_userFilter == nullptr)
      continue;

    if (buf1)
      channel->m_dspFX[i].m_userFilter->Process(static_cast<signed short*>(buf1), size1 / 2);
    if (buf2)
      channel->m_dspFX[i].m_userFilter->Process(static_cast<signed short*>(buf2), size2 / 2);
  }
  ...
  channel->m_channelHealth = 1.0f - (static_cast<float>(_numSamples) / static_cast<float>(channel->m_numBufferSamples));
}
...
void SoundLibrary3dDirectSound::AdvanceChannel(int _channel, int _frameNum)
{
  ...
  if (channel->m_simulatedPlayCursor == -1 || (_channel & 1) == (_frameNum & 1))
  {
    int errCode = channel->m_bufferInterface->GetCurrentPosition(&playCursor, nullptr);

    if (errCode == DSERR_BUFFERLOST)
      channel->m_bufferInterface->Restore();
    else SOUNDASSERT(errCode, "Direct sound couldn't get current position");
...
void SoundLibrary3dDirectSound::ResetChannel(int _channel)
{
  ...
  int errCode = channel->m_bufferInterface->GetCurrentPosition(&playCursor, &writeCursor);
  if (errCode == DSERR_BUFFERLOST)
...
void SoundLibrary3dDirectSound::EnableDspFX(int _channel, int _numFilters, const int* _filterTypes)
{
  ...
  auto buffer = static_cast<IDirectSoundBuffer8*>(channel->m_bufferInterface);
  ...
  for (int i = 0; i < _numFilters; ++i)
  {
    if (_filterTypes[i] <= DSP_DSOUND_WAVESREVERB)
    {
      ...
      DEBUG_ASSERT(
        desc[numDSoundFilters].guidDSFXClass == GUID_DSFX_STANDARD_CHORUS || desc[numDSoundFilters].guidDSFXClass ==
        GUID_DSFX_STANDARD_COMPRESSOR || desc[numDSoundFilters].guidDSFXClass == GUID_DSFX_STANDARD_DISTORTION || desc[
          numDSoundFilters].guidDSFXClass == GUID_DSFX_STANDARD_ECHO || desc[numDSoundFilters].guidDSFXClass ==
        GUID_DSFX_STANDARD_FLANGER || desc[numDSoundFilters].guidDSFXClass == GUID_DSFX_STANDARD_GARGLE || desc[numDSoundFilters].
        guidDSFXClass == GUID_DSFX_STANDARD_I3DL2REVERB || desc[numDSoundFilters].guidDSFXClass == GUID_DSFX_STANDARD_PARAMEQ ||
...
void SoundLibrary3dDirectSound::UpdateDspFX(int _channel, int _filterType, int _numParams, const float* _params)
{
  ...
  if (_filterType <= DSP_DSOUND_WAVESREVERB)
  {
    ...
    int errCode = buffer->GetObjectInPath(sfxClass, 0, interfaceClass, (void**)&effect);
    SOUNDASSERT(errCode, "Failed to get hold of DirectFX filter");

    switch (_filterType)
    {
      case DSP_DSOUND_CHORUS:
        errCode = static_cast<IDirectSoundFXChorus*>(effect)->SetAllParameters((DSFXChorus*)_params);
        break;
      case DSP_DSOUND_COMPRESSOR:
        errCode = static_cast<IDirectSoundFXCompressor*>(effect)->SetAllParameters((DSFXCompressor*)_params);
        break;
      case DSP_DSOUND_DISTORTION:
        errCode = static_cast<IDirectSoundFXDistortion*>(effect)->SetAllParameters((DSFXDistortion*)_params);
        break;
      case DSP_DSOUND_ECHO:
        errCode = static_cast<IDirectSoundFXEcho*>(effect)->SetAllParameters((DSFXEcho*)_params);
        break;
      case DSP_DSOUND_FLANGER:
        errCode = static_cast<IDirectSoundFXFlanger*>(effect)->SetAllParameters((DSFXFlanger*)_params);
        break;
      case DSP_DSOUND_GARGLE:
        errCode = static_cast<IDirectSoundFXGargle*>(effect)->SetAllParameters((DSFXGargle*)_params);
        break;
      case DSP_DSOUND_PARAMEQ:
        errCode = static_cast<IDirectSoundFXParamEq*>(effect)->SetAllParameters((DSFXParamEq*)_params);
        break;
      case DSP_DSOUND_WAVESREVERB:
        errCode = static_cast<IDirectSoundFXWavesReverb*>(effect)->SetAllParameters((DSFXWavesReverb*)_params);
        break;
      case DSP_DSOUND_I3DL2REVERB:
        errCode = static_cast<IDirectSoundFXI3DL2Reverb*>(effect)->SetAllParameters((DSFXI3DL2Reverb*)_params);
        errCode = static_cast<IDirectSoundFXI3DL2Reverb*>(effect)->SetQuality(_params[12]);
        break;
      default: ASSERT_TEXT(false, "Unknown filter type");
    }
