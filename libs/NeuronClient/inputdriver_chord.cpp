...
ChordInputDriver::ChordInputDriver()
  : m_specs(),
    lastError(nullErr) { setName("Chord"); }

InputParserState ChordInputDriver::parseInputSpecification(const InputSpecTokens& tokens, InputSpec& spec)
{
  string s;
...
bool ChordInputDriver::getInput(const InputSpec& spec, InputDetails& details)
{
