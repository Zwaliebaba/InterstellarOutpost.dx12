...
int Neuron::Client::Core::Startup(const wchar_t *_gameName, Windows::Foundation::Size _size, HINSTANCE _hInstance, int nCmdShow)
{
  Neuron::Core::Startup();
  ...
  m_hwnd =
      CreateWindowEx(0, L"Neuron", _gameName, style, CW_USEDEFAULT, CW_USEDEFAULT, windowWidth, windowHeight, nullptr, nullptr, m_instance, nullptr);
...
void Neuron::Client::Core::StartGame(const com_ptr<GameMain> &&_gameMain)
{
  m_main = _gameMain;
...
void Neuron::Client::Core::Shutdown()
{
  if (m_main)
  {
    m_main->Shutdown();
  ...
  Neuron::Core::Shutdown();
}

void Neuron::Client::Core::OnDeviceLost()
{
}

void Neuron::Client::Core::OnDeviceRestored()
{
}

Windows::Foundation::Point Neuron::Client::Core::OutputTopLeft()
{
  RECT rc;
    ...
    case WM_SYSKEYDOWN:
      if (_wParam == VK_RETURN && (_lParam & 0x60000000) == 0x20000000)
      {
        ...
        if (s_fullscreen)
        {
          ...
          const auto outputSize = Client::Core::OutputSize();
