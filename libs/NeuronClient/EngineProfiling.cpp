...
#include "Graphics/GameInput.h"
#include "Graphics/GpuTimeManager.h"
#include "Graphics/GraphRenderer.h"
...
public:
  StatHistory()
  {
    for (uint32_t i = 0; i < kHistorySize; ++i) m_RecentHistory[i] = 0.0f;
    for (uint32_t i = 0; i < kExtendedHistorySize; ++i) m_ExtendedHistory[i] = 0.0f;
    m_Average = 0.0f;
    m_Minimum = 0.0f;
    m_Maximum = 0.0f;
  }
  ...
  void RecordStat(uint32_t FrameIndex, float Value)
  {
    ...
    if (ValidCount > 0) m_Average /= (float) ValidCount;
    else
      m_Minimum = 0.0f;
  }
...
public:
  ...
  NestedTimingTree *GetChild(const wstring &name)
  {
    ...
    if (iter != m_LUT.end()) return iter->second;
  ...
  {
    if (m_IsExpanded && m_Children.size() > 0) return m_Children[0];
  ...
  {
    if (!m_IsExpanded || m_Children.size() == 0) return this;
  ...
  NestedTimingTree *NextChild(NestedTimingTree *curChild)
  {
    ...
    for (auto iter = m_Children.begin(); iter != m_Children.end(); ++iter)
    {
      if (*iter == curChild)
      {
        auto nextChild = iter;
        ++nextChild;
        if (nextChild != m_Children.end()) return *nextChild;
      }
    }

    if (m_Parent != nullptr) return m_Parent->NextChild(this);
    else
      return &sm_RootScope;
  }
  ...
  NestedTimingTree *PrevChild(NestedTimingTree *curChild)
  {
    ...
    {
      if (this == &sm_RootScope) return sm_RootScope.LastChild();
      else
        return this;
    }
  ...
  {
    if (sm_SelectedScope == this) GraphRenderer::SetSelectedIndex(m_GpuTimer.GetTimerIndex());
    if (EngineProfiling::Paused)
    {
      for (auto node: m_Children) node->GatherTimes(FrameIndex);
      return;
    }
...
namespace EngineProfiling
{
  ...
  void Update(void)
  {
    if (GameInput::IsFirstPressed(GameInput::kStartButton) || GameInput::IsFirstPressed(GameInput::kKey_space)) Paused = !Paused;
  ...
  {
    if (DrawPerfGraph) GraphRenderer::RenderGraphs(Context, GraphType::Global);
  }
...
void NestedTimingTree::Update(void)
{
  ...
  if (sm_SelectedScope == &sm_RootScope)
  {
    ...
    if (sm_SelectedScope == &sm_RootScope) return;
  }
  ...
  if (GameInput::IsFirstPressed(GameInput::kDPadLeft) || GameInput::IsFirstPressed(GameInput::kKey_left))
  {
    ...
    if (sm_CursorOnGraph) sm_CursorOnGraph = !sm_CursorOnGraph;
    else
  ...
  {
    if (sm_SelectedScope->m_IsExpanded == true && !sm_CursorOnGraph) sm_CursorOnGraph = true;
    else
  ...
  else if (GameInput::IsFirstPressed(GameInput::kAButton) || GameInput::IsFirstPressed(GameInput::kKey_return)) { sm_SelectedScope->Toggle(); }
}
...
void NestedTimingTree::DisplayNode(TextContext &Text, float leftMargin, float indent)
{
  ...
  {
    if (sm_SelectedScope == this && !sm_CursorOnGraph) Text.SetColor(Color(1.0f, 1.0f, 0.5f));
    else
      Text.SetColor(Color(1.0f, 1.0f, 1.0f));
    ...
    if (m_Children.size() == 0) Text.DrawString("  ");
  ...
  for (auto node: m_Children) node->DisplayNode(Text, leftMargin + indent, indent);
}

void NestedTimingTree::StoreToGraph(void)
{
  if (m_GraphHandle != PERF_GRAPH_ERROR) GraphRenderer::Update(XMFLOAT2(m_CpuTime.GetLast(), m_GpuTime.GetLast()), m_GraphHandle, GraphType::Profile);

  for (auto node: m_Children) node->StoreToGraph();
}
