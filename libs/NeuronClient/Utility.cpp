...
std::string Utility::ToLower(const std::string &str)
{
  ...
  std::locale loc;
  for (char &s: lower_case) s = std::tolower(s, loc);
  return lower_case;
}
...
std::wstring Utility::ToLower(const std::wstring &str)
{
  ...
  std::locale loc;
  for (wchar_t &s: lower_case) s = std::tolower(s, loc);
  return lower_case;
}
...
{
  size_t lastSlash;
  if ((lastSlash = filePath.rfind('/')) != std::string::npos) return filePath.substr(0, lastSlash + 1);
...
{
  size_t lastSlash;
  if ((lastSlash = filePath.rfind(L'/')) != std::wstring::npos) return filePath.substr(0, lastSlash + 1);
...
{
  size_t lastSlash;
  if ((lastSlash = filePath.rfind('/')) != std::string::npos) return filePath.substr(lastSlash + 1, std::string::npos);
  else if ((lastSlash = filePath.rfind('\\')) != std::string::npos)
...
{
  size_t lastSlash;
  if ((lastSlash = filePath.rfind(L'/')) != std::string::npos) return filePath.substr(lastSlash + 1, std::string::npos);
