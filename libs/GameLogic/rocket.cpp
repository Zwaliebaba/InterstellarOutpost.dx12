...
FuelBuilding::FuelBuilding(BuildingType _type)
  : Building(_type),
    ...
    m_currentLevel(0.0f)
{
  if (!s_fuelPipe)
  {
    s_fuelPipe = Resource::GetShape("fuelpipe.shp");
    DEBUG_ASSERT(s_fuelPipe);
  }
}

void FuelBuilding::Initialize(Building* _template)
{
...
FuelBuilding* FuelBuilding::GetLinkedBuilding()
{
  ...
  if (building)
  {
    if (building->m_buildingType == BuildingType::TypeFuelGenerator || building->m_buildingType == BuildingType::TypeFuelPipe || building->m_buildingType == BuildingType::TypeEscapeRocket ||
      building->m_buildingType == BuildingType::TypeFuelStation)
    {
...
void FuelBuilding::Render(float _predictionTime)
{
  ...
  if (fuelBuilding)
  {
    ...
    LegacyVector3 pipeVector = (theirPipePos - ourPipePos).Normalize();
...
FuelGenerator::FuelGenerator()
  : FuelBuilding(BuildingType::TypeFuelGenerator),
    m_pump(nullptr),
    m_pumpTip(nullptr),
    m_pumpMovement(0.0f),
    ...
    m_surges(0.0f)
{
  Building::SetShape(Resource::GetShape("fuelgenerator.shp"));

  m_pump = Resource::GetShape("fuelgeneratorpump.shp");
...
void FuelGenerator::RenderAlphas(float _predictionTime) { FuelBuilding::RenderAlphas(_predictionTime); }

const char* FuelGenerator::GetObjectiveCounter()
{
  static char buffer[256];
  sprintf(buffer, "%s %d%%", Strings::Get("objective_fuelpressure").c_str(), static_cast<int>(m_currentLevel * 100));
  return buffer;
}
...
FuelPipe::FuelPipe()
  : FuelBuilding(BuildingType::TypeFuelPipe)
{
  Building::SetShape(Resource::GetShape("fuelpipebase.shp"));
}
...
FuelStation::FuelStation()
  : FuelBuilding(BuildingType::TypeFuelStation),
    m_entrance(nullptr)
{
  Building::SetShape(Resource::GetShape("fuelstation.shp"));
      ...
      if (numOthers > 0)
      {
        ...
        if (team->m_others.ValidIndex(randomIndex))
        {
          ...
          if (entity && entity->m_entityType == EntityType::TypeDarwinian)
          {
            ...
            if (distance < 300.0f && (darwinian->m_state == Darwinian::StateIdle || darwinian->m_state ==
              Darwinian::StateWorshipSpirit))
...
void FuelStation::RenderAlphas(float _predictionTime)
{
  ...
  {
    auto rocket = static_cast<EscapeRocket*>(building);
    if ((rocket->m_state == EscapeRocket::StateCountdown && rocket->m_countdown <= 10.0f) || rocket->m_state ==
      EscapeRocket::StateFlight)
    {
...
EscapeRocket::EscapeRocket()
  : FuelBuilding(BuildingType::TypeEscapeRocket),
    m_booster(nullptr),
    m_shadowTimer(0.0f),
    m_cameraShake(0.0f),
...
{
  Building::SetShape(Resource::GetShape("rocket.shp"));
...
void EscapeRocket::Initialize(Building* _template)
{
  FuelBuilding::Initialize(_template);
...
{
  static char buffer[256];
  sprintf(buffer, "%s %d%%, %s %d%%", Strings::Get("objective_fuel").c_str(), static_cast<int>(m_fuel),
          Strings::Get("objective_passengers").c_str(), m_passengers);
  return buffer;
}
  ...
  if (!m_spawnCompleted)
  {
    if (m_countdown > 100.0f && m_countdown < 110.0f)
    {
      ...
      if (spawnBuilding)
      {
        ...
        g_app->m_location->SpawnEntities(spawnPos, 1, -1, EntityType::TypeDarwinian, 1, g_zeroVector, 40.0f);
      }
    }
  }
...
void EscapeRocket::AdvanceExploding()
{
  ...
  if (m_passengers)
  {
    ...
    WorldObjectId id = g_app->m_location->SpawnEntities(windowMat.pos, 0, -1, EntityType::TypeDarwinian, 1, vel, 0.0f);
