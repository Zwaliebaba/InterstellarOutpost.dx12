...
Darwinian::Darwinian()
  : Entity(EntityType::TypeDarwinian),
    m_state(StateIdle),
    m_promoted(false),
    ...
    m_shadowBuildingId(-1)
{
  ...
  Darwinian::LoadAssets();
}

fire_and_forget Darwinian::LoadAssets()
{
  StartLoading();
  co_await resume_background();
  m_darTex = TextureManager::LoadTexture(L"Sprites\\darwinian.dds");
  m_santaHatTex = TextureManager::LoadTexture(L"Sprites\\santahat.dds");
  FinishLoading();
}
...
bool Darwinian::Advance(Unit* _unit)
{
  ...
  {
    auto boxKite = dynamic_cast<BoxKite*>(g_app->m_location->GetEffect(m_boxKiteId));
    boxKite->Release();
...
bool Darwinian::AdvanceWatchingSpectacle()
{
  ...
  if (!building || (building->m_buildingType != BuildingType::TypeGodDish && building->m_buildingType !=
    BuildingType::TypeEscapeRocket))
  {
    m_state = StateIdle;
    return false;
  }
  ...
  LegacyVector3 targetDir = (targetPos - m_pos).Normalize();

  //
  // Is it a god dish?

  if (building->m_buildingType == BuildingType::TypeGodDish)
  {
    auto dish = dynamic_cast<GodDish*>(building);
  ...
  if (building->m_buildingType == BuildingType::TypeEscapeRocket)
  {
    auto rocket = dynamic_cast<EscapeRocket*>(building);
...
bool Darwinian::AdvanceApproachingArmour()
{
  ...
  auto armour = dynamic_cast<Armour*>(g_app->m_location->GetEntity(m_armourId));
...
{
  auto ant = dynamic_cast<ArmyAnt*>(g_app->m_location->GetEntity(m_threatId));
  if (!ant || ant->m_dead)
  {
    m_state = StateIdle;
...
bool Darwinian::AdvanceCombat()
{
  ...
  Entity* entity = (isEntity ? dynamic_cast<Entity*>(threat) : nullptr);
  ...
  auto effectThread = dynamic_cast<Effect*>(threat);
  if (!isEntity && effectThread && effectThread->m_effectType == EffectType::EffectGunTurretTarget)
  {
  ...
  bool soldier = m_id.GetTeamId() == 1 || g_app->m_globalWorld->m_research->CurrentLevel(GlobalResearch::TypeDarwinian) > 2;
  ...
  if (soldier && !m_scared)
  {
    ...
    {
      LegacyVector3 moveAwayVector = (m_pos - threat->m_pos).Normalize() * 30.0f;
      float angle = syncsfrand(XM_PI * 0.5f);
  ...
  if (isEntity)
  {
    ...
    bool hasGrenade = m_id.GetTeamId() == 1 || g_app->m_globalWorld->m_research->CurrentLevel(GlobalResearch::TypeDarwinian) > 3;
    if (hasGrenade)
    {
      ...
      if (m_grenadeTimer <= 0.0f)
      {
        ...
        if (distanceToTarget > 75.0f)
        {
          ...
          if (numPlayers == 0)
          {
            ...
            LegacyVector3 ourLandNormal = g_app->m_location->m_landscape.m_normalMap->GetValue(m_pos.x, m_pos.z);
            LegacyVector3 targetLandNormal = g_app->m_location->m_landscape.m_normalMap->GetValue(threat->m_pos.x, threat->m_pos.z);

            if (ourLandNormal.y > 0.7f && targetLandNormal.y > 0.7f)
            {
              bool grenadeRequired = entity->m_entityType == EntityType::TypeSporeGenerator || entity->m_entityType ==
                EntityType::TypeSpider || entity->m_entityType == EntityType::TypeTriffidEgg || entity->m_entityType ==
                EntityType::TypeSquadie || entity->m_entityType == EntityType::TypeArmour;

              if (grenadeRequired)
              {
                float targetHeight = entity->m_pos.y - g_app->m_location->m_landscape.m_heightMap->GetValue(
              ...
              {
                int numFriends = g_app->m_location->m_entityGrid->GetNumFriends(
                  threat->m_pos.x, threat->m_pos.z, 50.0f, m_id.GetTeamId());
                int numEnemies = g_app->m_location->m_entityGrid->GetNumEnemies(
                  threat->m_pos.x, threat->m_pos.z, 50.0f, m_id.GetTeamId());
            ...
            if (throwGrenade)
              g_app->m_location->ThrowWeapon(m_pos, threat->m_pos, EffectType::EffectThrowableGrenade, m_id.GetTeamId());
          }
        }
      }
...
bool Darwinian::AdvanceWorshipSpirit()
{
  ...
  if (areWeThere)
    m_front = (spirit->m_pos - m_pos).Normalize();
  ...
  if (areWeThere && !m_boxKiteId.IsValid())
  {
    ...
    for (int i = 0; i < g_app->m_location->m_effects.Size(); ++i)
    {
      ...
      {
        Effect* obj = g_app->m_location->m_effects[i];
        if (obj->m_id.GetUnitId() == UNIT_EFFECTS && obj->m_effectType == EffectType::EffectBoxKite)
        {
...
bool Darwinian::AdvanceFollowingOfficer()
{
  ...
  auto officer = static_cast<Officer*>(g_app->m_location->GetEntitySafe(m_officerId, EntityType::TypeOfficer));
...
bool Darwinian::AdvanceBoardingRocket()
{
  ...
  Building* building = g_app->m_location->GetBuilding(m_buildingId);
  if (!building || building->m_buildingType != BuildingType::TypeFuelStation)
  {
    m_state = StateIdle;
    return false;
  }
...
bool Darwinian::AdvanceAttackingBuilding()
{
  ...
  Building* building = g_app->m_location->GetBuilding(m_buildingId);
  if (!building || building->m_buildingType != BuildingType::TypeEscapeRocket)
  {
    m_state = StateIdle;
    return false;
  }
  ...
  if (distance < 200.0f)
  {
    ...
    targetPos.y += 50.0f;
    g_app->m_location->ThrowWeapon(m_pos, targetPos, EffectType::EffectThrowableGrenade, 1);
    m_state = StateIdle;
  }
...
bool Darwinian::SearchForArmour()
{
  ...
  if (team)
  {
    for (int i = 0; i < team->m_specials.Size(); ++i)
    {
      ...
      if (entity && !entity->m_dead && entity->m_entityType == EntityType::TypeArmour)
      {
...
bool Darwinian::SearchForOfficers()
{
  ...
  if (team)
  {
    ...
    for (int i = 0; i < team->m_specials.Size(); ++i)
    {
      WorldObjectId id = *team->m_specials.GetPointer(i);
      Entity* entity = g_app->m_location->GetEntity(id);
      if (entity && !entity->m_dead && entity->m_entityType == EntityType::TypeOfficer)
      {
    ...
    if (officers.Size() > 0)
    {
      ...
      auto officer = static_cast<Officer*>(g_app->m_location->GetEntitySafe(officerId, EntityType::TypeOfficer));
...
bool Darwinian::SearchForThreats()
{
  ...
  {
    Building* building = g_app->m_location->GetBuilding(m_buildingId);
    if (building && building->m_buildingType == BuildingType::TypeGunTurret)
    {
  ...
  for (int i = 0; i < g_app->m_location->m_effects.Size(); ++i)
  {
    if (g_app->m_location->m_effects.ValidIndex(i))
    {
      Effect* wobj = g_app->m_location->m_effects[i];
      if (wobj->m_effectType == EffectType::EffectThrowableGrenade || wobj->m_effectType == EffectType::EffectThrowableAirstrikeMarker || 
        wobj->m_effectType == EffectType::EffectGunTurretTarget || (wobj->m_effectType == EffectType::EffectSpamInfection && m_id.GetTeamId() == 0))
      {
  ...
  for (int i = 0; i < numFound; ++i)
  {
    ...
    Entity* entity = g_app->m_location->GetEntity(id);
    bool onFire = entity->m_entityType == EntityType::TypeDarwinian && static_cast<Darwinian*>(entity)->IsOnFire();

    if (!entity->m_dead && !onFire && entity->m_entityType != EntityType::TypeEgg)
    {
      ++numEnemies;
  ...
  if (entity && !entity->m_dead)
  {
    ...
    if (entity->m_entityType == EntityType::TypeSporeGenerator || entity->m_entityType == EntityType::TypeTripod || 
      entity->m_entityType == EntityType::TypeSpider || entity->m_entityType == EntityType::TypeSoulDestroyer || 
      entity->m_entityType == EntityType::TypeTriffidEgg || entity->m_entityType == EntityType::TypeSquadie || 
      entity->m_entityType == EntityType::TypeArmour)
      m_scared = true;
...
bool Darwinian::AdvanceToTargetPosition()
{
  ...
  LegacyVector3 targetDir = (m_wayPoint - m_pos).Normalize();
  ...
  m_pos = newPos;
  m_vel = (m_pos - oldPos) / SERVER_ADVANCE_PERIOD;
  m_front = (newPos - oldPos).Normalize();
...
LegacyVector3 Darwinian::PushFromObstructions(const LegacyVector3& pos, bool killem)
{
  ...
  for (int b = 0; b < buildings->Size(); ++b)
  {
    ...
    if (building)
    {
      if (building->m_buildingType == BuildingType::TypeLaserFence && static_cast<LaserFence*>(building)->IsEnabled())
      {
        ...
        {
          auto nextFence = static_cast<LaserFence*>(g_app->m_location->GetBuilding(
            static_cast<LaserFence*>(building)->GetBuildingLink()));
...
void Darwinian::Render(float _predictionTime, float _highDetail)
{
  ...
  if (!m_onGround)
    entityUp.RotateAround(m_front * g_gameTime * (m_id.GetUniqueId() % 10) * 0.1f);
  ...
  if (!m_dead || !m_onGround)
  {
    ...
    float health = static_cast<float>(m_stats[StatHealth]) / maxHealth;
    ...
    {
      glEnable(GL_BLEND);
      colour.a = 2.5f * static_cast<float>(m_stats[StatHealth]);
      colour.r *= 0.2f;
      colour.g *= 0.2f;
      colour.b *= 0.2f;
    }
  ...
  if (m_id.GetTeamId() == 0 && Location::ChristmasModEnabled() == 1)
  {
    ...
    {
      glBindTexture(GL_TEXTURE_2D, m_santaHatTex->m_ref);
      ...
      glVertex3fv((predictedPos - entityRight).GetData());
      glEnd();
      glBindTexture(GL_TEXTURE_2D, m_darTex->m_ref);
    }
  }
  ...
  if (m_boxKiteId.IsValid())
  {
    ...
    if (boxKite)
    {
      ...
      boxKite->m_up.Normalize();
      boxKite->m_front = m_front;
      boxKite->m_front.Normalize();
  ...
  if (m_dead)
  {
    ...
    entityRight *= size;
    unsigned char alpha = static_cast<float>(m_stats[StatHealth]) * 2.55f;
...
BoxKite::BoxKite()
  : Effect(EffectType::EffectBoxKite),
    m_state(StateHeld),
    m_birthTime(0.0f),
    m_deathTime(0.0f)
{
  m_shape = Resource::GetShape("boxkite.shp");
...
fire_and_forget BoxKite::LoadAssets()
{
  StartLoading();
  co_await resume_background();
  m_starburst = TextureManager::LoadTexture(L"textures\\starburst.dds");
  FinishLoading();
}
...
void BoxKite::Render(float _predictionTime)
{
  ...
  glBindTexture(GL_TEXTURE_2D, m_starburst->m_ref);
