    ...
    m_ordersBuildingId(-1)
{
  ...
  m_shape = Resource::GetShape("darwinian.shp");
...
bool Officer::AdvanceGivingOrders()
{
  if (m_orders == OrderGoto)
    m_front = (m_orderPosition - m_pos).Normalize();
  m_vel.Zero();
  return false;
}
...
{
  int numFound;
  WorldObjectId* ids = g_app->m_location->m_entityGrid->GetFriends(m_pos.x, m_pos.z, OFFICER_ABSORBRANGE, &numFound,
                                                                   m_id.GetTeamId());
  ...
  for (int i = 0; i < numFound; ++i)
  {
    ...
    if (entity && entity->m_entityType == EntityType::TypeDarwinian && !entity->m_dead)
    {
...
void Officer::SetWaypoint(const LegacyVector3& _wayPoint)
{
...
{
  static const char* orders[] = {"None", "Goto", "Follow"};

  return orders[_orderType];
}

OfficerOrders::OfficerOrders()
  : Effect(EffectType::EffectOfficerOrders),
    m_arrivedTimer(-1.0f) { }
