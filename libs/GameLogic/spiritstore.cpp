...
void SpiritStore::Initialize(int _initialCapacity, int _maxCapacity, LegacyVector3 _pos, float _sizeX, float _sizeY, float _sizeZ)
{
...
void SpiritStore::Render(float _predictionTime)
{
  ...
  glBindTexture(GL_TEXTURE_2D, Resource::GetTexture("textures\\triangleOutline.bmp", true, false));
...
void SpiritStore::AddSpirit(Spirit* _spirit)
{
  ...
  *target = *_spirit;
  target->m_pos = m_pos + LegacyVector3(syncsfrand(m_sizeX * 1.5f), syncsfrand(m_sizeY * 1.5f), syncsfrand(m_sizeZ * 1.5f));
