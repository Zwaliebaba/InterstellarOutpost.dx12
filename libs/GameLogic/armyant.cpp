...
ArmyAnt::ArmyAnt()
  : Entity(EntityType::TypeArmyAnt),
    m_scale(0),
    m_orders(NoOrders),
    m_targetFound(false),
    m_spiritId(-1)
{
  m_shapes[0] = Resource::GetShape("armyant.shp");
  m_shapes[1] = Resource::GetShape("armyant2.shp");
  m_shapes[2] = Resource::GetShape("armyant3.shp");
...
{
  if (AdvanceToTargetPosition())
  {
    if (Entity* targetEntity = g_app->m_location->GetEntity(m_targetId))
    {
      if (targetEntity->m_entityType == EntityType::TypeDarwinian) { m_orders = CollectEntity; }
      else { m_orders = AttackEnemy; }
    }
    else { OrderReturnToBase(); }
  }
  return false;
}
...
{
  Spirit* s = NULL;
  if (g_app->m_location->m_spirits.ValidIndex(m_spiritId)) { s = g_app->m_location->m_spirits.GetPointer(m_spiritId); }
...
bool ArmyAnt::AdvanceAttackEnemy()
{
  ...
  if (targetEntity->m_entityType == EntityType::TypeEngineer)
  {
    m_wayPoint.y = g_app->m_location->m_landscape.m_heightMap->GetValue(m_wayPoint.x, m_wayPoint.z);
  }
...
bool ArmyAnt::AdvanceReturnToBase()
{
  ...
  if (arrived)
  {
    ...
    if (!building)
    {
      ...
      if (!newBaseFound) { m_orders = BaseDestroyed; }
      return false;
    }
    else if (building && building->m_buildingType == BuildingType::TypeAntHill)
    {
...
{
  bool arrived = AdvanceToTargetPosition();
  if (arrived) { SearchForRandomPosition(); }
...
bool ArmyAnt::SearchForSpirits()
{
  ...
  for (int i = 0; i < g_app->m_location->m_spirits.Size(); ++i)
  {
    if (g_app->m_location->m_spirits.ValidIndex(i))
    {
      ...
      if (theDist <= ARMYANT_SEARCHRANGE && theDist < closest && (s->m_state == Spirit::StateBirth || s->m_state ==
...
{
  WorldObjectId enemyId = g_app->m_location->m_entityGrid->GetBestEnemy(m_pos.x, m_pos.z, 0.0f, ARMYANT_SEARCHRANGE,
                                                                        m_id.GetTeamId());
  Entity* enemy = g_app->m_location->GetEntity(enemyId);

  if (enemy && !enemy->m_dead && enemy->m_entityType != EntityType::TypeDarwinian)
  {
    m_targetId = enemyId;
...
bool ArmyAnt::SearchForAntHill()
{
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    if (g_app->m_location->m_buildings.ValidIndex(i))
    {
      ...
      if (building->m_buildingType == BuildingType::TypeAntHill && g_app->m_location->IsFriend(building->m_id.GetTeamId(), m_id.GetTeamId()))
      {
...
bool ArmyAnt::AdvanceToTargetPosition()
{
  ...
  LegacyVector3 actualDir = (m_wayPoint - m_pos).Normalize();
