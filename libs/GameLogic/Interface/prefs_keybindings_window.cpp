...
class ApplyKeybindingsButton : public DarwiniaButton
{
  void MouseUp() override
  {
    ...
    if (g_prefsManager->GetInt("ControlMethod") == 0) { g_app->m_taskManagerInterface = new TaskManagerInterfaceGestures(); }
    else { g_app->m_taskManagerInterface = new TaskManagerInterfaceIcons(); }
...
class ControlMethodDropDownMenu : public DropDownMenu
{
  void SelectOption(int _option) override
  {
    ...
    if (parent && _option != parent->m_controlMethod)
    {
      ...
      auto newMenu = static_cast<DropDownMenu *>(parent->GetButton(LANGUAGEPHRASE("newcontrols_prefsoption")));
      if (newMenu) newMenu->SelectOption(_option);
    }
    else { DropDownMenu::SelectOption(_option); }
  }
};
...
void PrefsKeybindingsWindow::Create()
{
  ...
  if (0 == m_controlMethod) s_indices = s_gesture_controls;
  else s_indices = s_icon_controls;
