...
MessageDialog::~MessageDialog() { for (int i = 0; i < m_numLines; ++i) { delete [] m_messageLines[i]; } }
...
void MessageDialog::Render(bool _hasFocus)
{
  ...
  for (int i = 0; i < m_numLines; ++i) { g_editorFont.DrawText2D(m_x + 10, m_y + GetMenuSize(32) + i * GetMenuSize(DEF_FONT_SIZE), GetMenuSize(DEF_FONT_SIZE), m_messageLines[i]); }
}