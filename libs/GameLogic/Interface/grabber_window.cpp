...
void GrabberWindow::Create()
{
    ...
    start->SetProperties( "Start", 10, 30, 70, 15, UnicodeString("Start"), UnicodeString("Start") );
    RegisterButton( start );

    StopRecordButton *stop = new StopRecordButton();
    stop->SetProperties( "Stop", 90, 30, 70, 15, UnicodeString("Stop"), UnicodeString("Stop") );
    RegisterButton( stop );
}
