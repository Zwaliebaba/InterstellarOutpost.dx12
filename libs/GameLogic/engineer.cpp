...
Engineer::Engineer()
  : Entity(EntityType::TypeEngineer),
    m_state(StateIdle),
    m_hoverHeight(15.0f),
    ...
    m_bridgeId(-1)
{
  ...
  m_shape = Resource::GetShape("engineer.shp");
...
void Engineer::SetWaypoint(const LegacyVector3& _wayPoint)
{
  m_retargetTimer = 0.0f;
...
bool Engineer::SearchForControlTowers()
{
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    if (g_app->m_location->m_buildings.ValidIndex(i))
    {
      ...
      if (building->m_buildingType == BuildingType::TypeControlTower)
      {
...
bool Engineer::SearchForBridges()
{
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    ...
    {
      Building* building = g_app->m_location->m_buildings[i];
      if (building->m_buildingType == BuildingType::TypeBridge)
      {
        auto bridge = static_cast<Bridge*>(building);
        LegacyVector3 pos, front;
        float theDist = (building->m_pos - m_pos).Mag();
        if (bridge->GetAvailablePosition(pos, front) && bridge->m_status < 100.0f && theDist <= ENGINEER_SEARCHRANGE && theDist <
          closest)
        {
          buildingIndex = i;
          closest = theDist;
...
void Engineer::ChangeHealth(int amount)
{
  if (!m_dead)
  {
    ...
    float fractionDead = 1.0f - m_stats[StatHealth] / EntityBlueprint::GetStat(EntityType::TypeEngineer, StatHealth);
...
bool Engineer::Advance(Unit* _unit)
{
  ...
  if (amIDead)
  {
    ...
    if (m_state == StateOperatingBridge)
    {
      ...
      if (bridge && bridge->m_buildingType == BuildingType::TypeBridge)
      {
        bridge->EndOperation();
  ...
  m_positionHistory.PutDataAtStart(NEW LegacyVector3(m_pos));
  int maxSpirits = GetMaxSpirits();
  while (m_positionHistory.ValidIndex(maxSpirits + 1))
  {
    LegacyVector3* position = m_positionHistory[maxSpirits + 1];
  ...
  Building* building = g_app->m_location->GetBuilding(m_buildingId);
  if (building && building->m_buildingType == BuildingType::TypeControlTower && m_positionId != -1 && m_state != StateReprogramming)
  {
  ...
  if (building && building->m_buildingType == BuildingType::TypeBridge && m_state != StateOperatingBridge)
  {
  ...
  if (building && building->m_buildingType == BuildingType::TypeResearchItem && m_state != StateResearching)
  {
...
{
  LegacyVector3 distance = (m_targetPos - m_pos);
  distance.y = 0.0f;
  ...
  if (distance.Mag() > 5.0f)
  {
    ...
    LegacyVector3 newPos = m_pos + actualDir * desiredSpeed * SERVER_ADVANCE_PERIOD;
    ...
    m_pos = newPos;
    float targetHeight = std::max(g_app->m_location->m_landscape.m_heightMap->GetValue(m_pos.x, m_pos.z), 0.0f) + m_hoverHeight;
...
bool Engineer::SearchForIncubator()
{
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    ...
    {
      Building* building = g_app->m_location->m_buildings[i];
      if (building->m_buildingType == BuildingType::TypeIncubator && g_app->m_location->IsFriend(building->m_id.GetTeamId(), m_id.GetTeamId()))
      {
...
bool Engineer::AdvanceToControlTower()
{
  ...
  DEBUG_ASSERT(building->m_buildingType == BuildingType::TypeControlTower);
...
bool Engineer::AdvanceResearching()
{
  ...
  if (!item || item->m_buildingType != BuildingType::TypeResearchItem || !item->NeedsReprogram())
  {
    g_app->m_soundSystem->StopAllSounds(m_id, "Engineer BeginReprogramming");
  ...
  m_targetFront = (toPos - m_pos).Normalize();
  LegacyVector3 targetFront = m_targetFront;
  m_front = (targetFront * SERVER_ADVANCE_PERIOD) + (m_front * (1.0f - SERVER_ADVANCE_PERIOD));

  LegacyVector3 oldPos = m_pos;
  LegacyVector3 targetPos = m_targetPos;
  LegacyVector3 rightAngle = m_targetFront ^ g_upVector;
  float scale = 2.0f;
...
bool Engineer::AdvanceReprogramming()
{
  ...
  LegacyVector3 consolePos;
  ct->GetConsolePosition(m_positionId, consolePos);

  LegacyVector3 targetFront = (consolePos - m_pos).Normalize();
  m_front = (targetFront * SERVER_ADVANCE_PERIOD) + (m_front * (1.0f - SERVER_ADVANCE_PERIOD));
...
void Engineer::BeginBridge(LegacyVector3 _to)
{
  ...
  LegacyVector3 bridgeSpan(_to - m_wayPoint);
  int numComponents = static_cast<int>(bridgeSpan.Mag() / 80.0f);
  LegacyVector3 componentSpan = bridgeSpan / static_cast<float>(numComponents);
  ...
  {
    auto component = static_cast<Bridge*>(Building::CreateBuilding(BuildingType::TypeBridge));
    ...
    component->m_front = (_to - m_wayPoint).Normalize();
    ...
    LegacyVector3 right = component->m_front ^ g_upVector;
...
void Engineer::Render(float predictionTime)
{
  ...
  if (m_state == StateReprogramming || m_state == StateOperatingBridge || m_state == StateResearching)
  {
    ...
    if (building)
    {
      ...
      else if (building->m_buildingType == BuildingType::TypeResearchItem)
      {
        auto item = static_cast<ResearchItem*>(building);
        LegacyVector3 end1, end2;
        item->GetEndPositions(end1, end2);
...
std::string Engineer::GetCurrentAction() const
{
  switch (m_state)
  {
    case StateIdle:
      return Strings::Get("engineer_idle");
    case StateToWaypoint:
      return Strings::Get("engineer_towaypoint");
    case StateToSpirit:
      return Strings::Get("engineer_tospirit");
    case StateToIncubator:
      return Strings::Get("engineer_toincubator");

    case StateToControlTower:
    case StateReprogramming:
      return Strings::Get("engineer_reprogramming");

    case StateToResearchItem:
    case StateResearching:
      return Strings::Get("engineer_researching");

    case StateToBridge:
    case StateOperatingBridge:
      return Strings::Get("engineer_bridge");
  }
