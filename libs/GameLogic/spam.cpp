...
static float SpamReloadTime() { return SPAM_RELOADTIME - (SPAM_RELOADTIME / 2.0) * g_app->m_difficultyLevel / 10.0; }

Spam::Spam()
  : Building(BuildingType::TypeSpam),
    m_timer(0.0f),
...
void Spam::Initialize(Building* _template)
{
...
void Spam::Damage(float _damage)
{
  ...
  int oldHealthBand = static_cast<int>(m_damage / 10.0f);
  Building::Damage(_damage);
  m_damage += _damage;
  int newHealthBand = static_cast<int>(m_damage / 10.0f);
...
void Spam::SpawnInfection()
{
  ...
  for (int i = 0; i < 20; ++i)
  {
    LegacyVector3 vel = g_upVector;
    vel += LegacyVector3(syncsfrand(1.0f), syncfrand(2.0f), syncsfrand(1.0f));
...
SpamInfection::SpamInfection()
  : Effect(EffectType::EffectSpamInfection),
    m_state(StateIdle),
...
{
  m_targetId = g_app->m_location->m_entityGrid->GetBestEnemy(m_pos.x, m_pos.z, SPAMINFECTION_MINSEARCHRANGE,
  ...
  for (int i = 0; i < g_app->m_location->m_spirits.Size(); ++i)
  {
    if (g_app->m_location->m_spirits.ValidIndex(i))
    {
      ...
      if (theDist <= SPAMINFECTION_MAXSEARCHRANGE && theDist >= SPAMINFECTION_MINSEARCHRANGE && theDist < nearest && s->m_state ==
        Spirit::StateFloating && s->m_pos.y > 10.0f)
      {
        index = i;
        nearest = theDist;
      }
...
void SpamInfection::AdvanceAttackingEntity()
{
  ...
  if (arrived)
  {
    if (m_targetId.GetTeamId() == 0)
    {
      ...
      {
        g_app->m_location->SpawnEntities(target->m_pos, 1, -1, EntityType::TypeDarwinian, 1, target->m_vel, 0.0f);
...
void SpamInfection::AdvanceAttackingSpirit()
{
  ...
  if (bool arrived = AdvanceToTargetPosition())
  {
    auto entityType = EntityType::TypeVirii;
    if (syncfrand(20.0f) < 1.0f)
      entityType = EntityType::TypeSpider;
...
bool SpamInfection::AdvanceToTargetPosition()
{
  ...
  LegacyVector3 targetVel = (m_targetPos - m_pos).SetLength(200.0f);
...
{
  LegacyVector3 predictedPos = m_pos + m_vel * _time;

  //RenderSphere( predictedPos, 200.0f, RGBAColor(255,0,0,255) );
  //RenderArrow( predictedPos, m_targetPos, 1.0f, RGBAColor(255,255,255,255) );
