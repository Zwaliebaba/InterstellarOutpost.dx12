    ...
    m_spiritId(-1)
{
  ...
  m_shape = Resource::GetShape("spider.shp");
...
void Spider::ChangeHealth(int _amount)
{
  ...
  if (!m_dead && _amount < -1)
  {
    ...
    float fractionDead = 1.0f - static_cast<float>(m_stats[StatHealth]) / EntityBlueprint::GetStat(EntityType::TypeSpider, StatHealth);
...
void Spider::StompFoot(const LegacyVector3& _pos)
{
  for (int p = 0; p < 3; ++p)
  {
...
void Spider::UpdateLegs()
{
  ...
  for (int i = 0; i < SPIDER_NUM_LEGS; ++i)
  {
    ...
    //		{
    //			LegacyVector3 const &legPos = m_legs[i]->m_foot.m_pos;
    //			StompFoot(legPos);
    //		}
...
float Spider::IsPathOK(const LegacyVector3& _dest)
{
  ...
  constexpr float sampleSeperation = 8.0f;
...
void Spider::DetectCollisions()
{
  ...
  for (int i = 0; i < numFound; ++i)
  {
    ...
    if (ent->m_entityType == EntityType::TypeSpider && ent->m_id != m_id)
    {
...
{
  LegacyVector3 toTarget = m_targetPos - m_pos;
  toTarget.y = 0.0f;
  float toTargetMag = toTarget.Mag();
  LegacyVector3 toTargetNormalized = toTarget / toTargetMag;

  float dotProd = toTargetNormalized * m_front;
  if (dotProd < 0.999f)
  {
    LegacyVector3 rotation = m_front ^ toTargetNormalized;
    if (dotProd < 0.9f)
...
bool Spider::AdvanceAttack()
{
  ...
  if (facingTarget)
  {
    ...
    else
    {
      ...
      LegacyVector3 up = (m_pounceTarget - m_pos).Normalize();
      up.y = 0.5f;
      up.Normalize();
      m_vel = up * force;
      ...
      LegacyVector3 forwards = (m_pounceTarget - m_pos);
      for (int i = 0; i < SPIDER_NUM_LEGS; ++i)
      {
...
bool Spider::AdvancePouncing()
{
  ...
  if (m_pos.y < landHeight + 1.0f)
  {
    ...
    float damage = 100.0f;
    int numFound;
    WorldObjectId* enemies = g_app->m_location->m_entityGrid->
                                    GetEnemies(m_pos.x, m_pos.z, squashRange, &numFound, m_id.GetTeamId());
    for (int i = 0; i < numFound; ++i)
    {
...
bool Spider::SearchForSpirits()
{
  ...
  for (int i = 0; i < g_app->m_location->m_spirits.Size(); ++i)
  {
    if (g_app->m_location->m_spirits.ValidIndex(i))
    {
      ...
      if (s->NumNearbyEggs() < 3 && s->m_pos.y > 10)
      {
        ...
        if (theDist <= SPIRIT_MAXSEARCHRANGE && theDist >= SPIRIT_MINSEARCHRANGE && theDist < nearest && s->m_state ==
  ...
  if (found)
  {
    ...
    LegacyVector3 usToThem = (found->m_pos - m_pos).Normalize() * 45.0f;
...
bool Spider::AdvanceEggLaying()
{
  ...
  if (arrived)
  {
    ...
    g_app->m_location->SpawnEntities(eggLayMat.pos, m_id.GetTeamId(), -1, EntityType::TypeEgg, 1, g_zeroVector, 0.0f);
...
void Spider::Render(float _predictionTime)
{
  ...
  {
    //RenderArrow(m_pos, m_pounceTarget, 1.0f, RGBAColor(255,0,0) );
  }
