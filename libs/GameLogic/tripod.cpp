...
Tripod::Tripod()
  : Entity(EntityType::TypeTripod),
    ...
    m_bodyVel(0, 0, 0)
{
  m_shape = Resource::GetShape("tripod.shp");
...
void Tripod::ChangeHealth(const int _amount)
{
...
{
  bool allOnGround = m_legs[0]->m_foot.m_state == EntityFoot::OnGround && m_legs[1]->m_foot.m_state == EntityFoot::OnGround &&
...
void Tripod::DoFallForTwoLegs()
{
  ...
  LegacyVector3 footToFoot;
  LegacyVector3 pointBetweenFeet;
  LegacyVector3 pointUnderBody;
  ...
  LegacyVector3 feetToBody(m_pos - pointBetweenFeet);
  float feetToBodyLen = feetToBody.Mag();
  LegacyVector3 footToFootDir(footToFoot);
  footToFootDir.Normalize();
  LegacyVector3 forcePerUnitMass(m_up ^ footToFootDir);
  ...
  LegacyVector3 axis(footToFootDir * -rotation * 0.1f);
  m_up.RotateAround(axis);
  //		m_front.RotateAround(axis);

  m_pos += m_bodyVel * SERVER_ADVANCE_PERIOD;
  LegacyVector3 newFeetToBody(m_pos - pointBetweenFeet);
...
WorldObjectId Tripod::FindEntityToAttack()
{
  ...
  int numFound;
  WorldObjectId* enemies = g_app->m_location->m_entityGrid->GetEnemies(m_pos.x, m_pos.z, ATTACK_SEARCH_RADIUS, &numFound,
                                                                       m_id.GetTeamId());
...
LegacyVector2 Tripod::ChooseDestination()
{
...
void Tripod::DoNavigation()
{
  ...
  LegacyVector2 pos(m_pos);
  LegacyVector2 delta(m_navData.m_targetPos - pos);
  ...
  if (m_navData.m_dir == -1)
  {
    ...
    LegacyVector2 deltaNorm(delta / deltaMag);
...
{
  LegacyVector3 toEnemy = m_attackTarget - m_pos;
  toEnemy.Normalize();
  LegacyVector3 toEnemyHorizontal(toEnemy);
  toEnemyHorizontal.HorizontalAndNormalize();
  LegacyVector3 up = g_upVector - toEnemyHorizontal;
  up.Normalize();
...
void Tripod::AdvanceWalk()
{
  ...
  // Fall
  {
    ...
    {
      m_bodyVel.Zero();
      m_front.HorizontalAndNormalize();
      m_up = g_upVector;
    }
  }
...
void Tripod::AdvancePreAttack()
{
  ...
  LegacyVector3 desiredUp = CalcAttackUpVector();
...
void Tripod::AdvanceAttack()
{
  ...
  LegacyVector3 right = m_up ^ g_upVector;
  //	m_front = m_up ^ right;
  //	m_front.Normalize();

  if (timeInAttack > 1.0f)
  {
    int t2 = static_cast<int>(timeInAttack * 10.0f);
    if (t2 & 1)
    {
      LegacyVector3 toEnemy = m_attackTarget - m_pos;
      toEnemy.Normalize();
...
bool Tripod::Advance(Unit* _unit)
{
  ...
  LegacyVector3 frontHoriNorm = m_front;
  frontHoriNorm.HorizontalAndNormalize();
...
void Tripod::Render(const float _predictionTime)
{
