...
ReceiverBuilding::ReceiverBuilding(BuildingType _type)
  : Building(_type),
    m_spiritLink(-1),
    m_spiritLocation(nullptr) {}

void ReceiverBuilding::Initialize(Building* _template)
{
  Building::Initialize(_template);

  m_spiritLink = dynamic_cast<ReceiverBuilding*>(_template)->m_spiritLink;
}
...
void ReceiverBuilding::RenderAlphas(float _predictionTime)
{
  ...
  if (spiritLink)
  {
    ...
    for (int i = 0; i < m_spirits.Size(); ++i)
    {
      float thisSpirit = m_spirits[i];
      thisSpirit += _predictionTime * 0.8f;
      thisSpirit = std::max(thisSpirit, 0.0f);
      thisSpirit = std::min(thisSpirit, 1.0f);
      LegacyVector3 thisSpiritPos = ourPos + (theirPos - ourPos) * thisSpirit;
...
SpiritProcessor* ReceiverBuilding::GetSpiritProcessor()
{
  ...
  if (!processor || processor->m_buildingType != BuildingType::TypeSpiritProcessor)
  {
    for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
    {
      if (g_app->m_location->m_buildings.ValidIndex(i))
      {
        ...
        if (building->m_buildingType == BuildingType::TypeSpiritProcessor)
        {
...
SpiritProcessor::SpiritProcessor()
  : ReceiverBuilding(BuildingType::TypeSpiritProcessor),
    m_timerSync(0.0f),
    m_numThisSecond(0),
    m_spawnSync(0.0f),
    m_throughput(0.0f)
{
  Building::SetShape(Resource::GetShape("spiritprocessor.shp"));
}

void SpiritProcessor::Initialize(Building* _building)
{
  ReceiverBuilding::Initialize(_building);
...
const char* SpiritProcessor::GetObjectiveCounter()
{
  ...
  sprintf(result, "%s : %2.2f", Strings::Get("objective_throughput").c_str(), m_throughput);
  return result;
}
...
ReceiverLink::ReceiverLink()
  : ReceiverBuilding(BuildingType::TypeReceiverLink)
{
  Building::SetShape(Resource::GetShape("receiverlink.shp"));
}

bool ReceiverLink::Advance() { return ReceiverBuilding::Advance(); }

ReceiverSpiritSpawner::ReceiverSpiritSpawner()
  : ReceiverBuilding(BuildingType::TypeReceiverSpiritSpawner)
{
  Building::SetShape(Resource::GetShape("receiverlink.shp"));
}
...
SpiritReceiver::SpiritReceiver()
  : ReceiverBuilding(BuildingType::TypeSpiritReceiver),
    m_headMarker(nullptr),
    m_headShape(nullptr),
    m_spiritLink(nullptr)
{
  Building::SetShape(Resource::GetShape("spiritreceiver.shp"));
  ...
  m_headShape = Resource::GetShape("spiritreceiverhead.shp");
  m_spiritLink = m_headShape->m_rootFragment->LookupMarker("MarkerSpiritLink");
}

void SpiritReceiver::Initialize(Building* _template)
{
  ...
  ReceiverBuilding::Initialize(_template);
}
...
bool UnprocessedSpirit::Advance()
{
  ...
  m_xaxisRate = std::min(m_xaxisRate, 2.0f);
  m_xaxisRate = std::max(m_xaxisRate, 0.0f);
  m_yaxisRate = std::min(m_yaxisRate, 2.0f);
  m_yaxisRate = std::max(m_yaxisRate, 0.0f);
  m_zaxisRate = std::min(m_zaxisRate, 2.0f);
  m_zaxisRate = std::max(m_zaxisRate, 0.0f);
