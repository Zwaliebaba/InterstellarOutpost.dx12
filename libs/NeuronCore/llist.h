...
class LList
{
  ...
  public:
    ...
    T GetData(int index) const;	// slow unless sequential
    T* GetPointer(int index);			// slow unless sequential
...
template <class T>
void LList<T>::PutData(const T& newdata) { PutDataAtEnd(newdata); }
...
template <class T>
int LList<T>::Size() const { return m_numItems; }
...
T LList<T>::GetData(int index) const
{
  ...
  if (m_previous && m_previousIndex != -1 && index == m_previousIndex)
  {
...
template <class T>
T LList<T>::operator [](int index) { return GetData(index); }

template <class T>
bool LList<T>::ValidIndex(int index) const { return (index >= 0 && index < m_numItems); }
