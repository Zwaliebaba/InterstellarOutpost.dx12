...
void DArray<T>::SetSize(int newsize)
{
  if (newsize > m_arraySize)
  {
    ...
    auto tempshadow = NEW char[m_arraySize];
...
template <class T>
void DArray<T>::SetStepSize(int newstepsize) { m_stepSize = newstepsize; }

template <class T>
void DArray<T>::SetStepDouble() { m_stepSize = -1; }
...
template <class T>
const T& DArray<T>::operator [](int _index) const
{
...
template <class T>
int DArray<T>::Size() const { return m_arraySize; }
