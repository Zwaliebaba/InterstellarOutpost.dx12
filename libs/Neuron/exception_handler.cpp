								...
								LPCTSTR lpszAppVersion)
{
	...
	// Use the profile
	if (lpszProfile && lpszProfile[0] != '\0')
	{
		strncpy(szModuleName, lpszProfile, sizeof(szModuleName));
		szModuleName[sizeof(szModuleName)-1] = '\0';

		pszFilePart = szModuleName + strlen(szModuleName);
	}
	...
	else
	{
		...
		if (lpszProfile && lpszProfile[0] != '\0')
		{
			lstrcat(szCommandLine, _T(" \""));	// surround profile with quotes

			// Encode '\' as '\\'
			TCHAR *szCommandLineTemp = szCommandLine + strlen(szCommandLine);
			LPCTSTR szTemp = lpszProfile;
			for(; *szTemp; szTemp++)
			{
				if (*szTemp == '\\')
					*(szCommandLineTemp++) = *szTemp;
				*(szCommandLineTemp++) = *szTemp;
			}
			*szCommandLineTemp = '\0';
