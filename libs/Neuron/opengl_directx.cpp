...
static WorkQueue *s_workQueue = NULL;
...
bool Direct3DInit(HWND _hWnd, bool _windowed, int _width, int _height, int _colourDepth, int _zDepth, bool _waitVRT)
{
	...
	if (!g_pD3D)
	{
		if( NULL == ( g_pD3D = Direct3DCreate9( D3D_SDK_VERSION ) ) )
		{
			...
			AppReleaseAssert(0,"Direct3D 9 not available.");
		}
	}
	...
	*/
	...
	if (_windowed) {
		...
		switch (_colourDepth) {
			...
			case 16:
				...
				break;
			default:
				AppReleaseAssert(0,"Unexpected colourDepth.");
		}
	}
	...
	if (_zDepth) {
		...
		switch (_zDepth) {
			...
			case 32:
				...
				break;
			default:
				AppReleaseAssert(0,"Unexpected zDepth.");
		}
	}
		...
		_hWnd, g_supportsHwVertexProcessing?D3DCREATE_HARDWARE_VERTEXPROCESSING:D3DCREATE_SOFTWARE_VERTEXPROCESSING,&g_d3dpp, &g_pd3dDevice ) ) )
    {
		...
		//AppReleaseAssert( 0, "Failed to set screen mode" );
        return false;
    }
...
void glClear (GLbitfield mask)
{
	...
	// have handled.
	AppDebugAssert( (mask & ~(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT)) == 0 );
}
...
void glMatrixMode (GLenum mode)
{
	...
	AppDebugAssert( mode == GL_MODELVIEW || mode == GL_PROJECTION );
...
void __stdcall OpenGLD3D::glActiveTextureD3D(int _target)
{
	...
	AppDebugAssert(_target >= 0 && _target < MAX_ACTIVE_TEXTURES);
...
void glEnd ()
{
	//AppDebugOut("glEnd: g_pd3dDevice=%x g_fixedPipeline=%x\n",(unsigned)g_pd3dDevice,(unsigned)g_fixedPipeline);
	GL_TRACE_IMP(" glEnd()")
	...
	switch (s_primitiveMode) {
		case GL_POINTS:
			AppDebugAssert(s_currentVertexNumber>0);
			DrawPrimitiveUP(D3DPT_POINTLIST, s_currentVertexNumber, s_vertices, sizeof(CustomVertex));
			break;

		case GL_LINES:
			AppDebugAssert(s_currentVertexNumber%2==0);
			AppDebugAssert(s_currentVertexNumber>1);
		...
		case GL_QUAD_STRIP:
			//AppDebugAssert(s_currentVertexNumber>2);
		...
		case GL_QUADS:
			...
			{
				AppDebugAssert(s_currentVertexNumber%3==0);
...
void glBindTexture ( GLenum target, GLuint texture )
{
	...
	switch (target) {
		...
		default:
			AppDebugAssert( target == GL_TEXTURE_2D );
			break;
	}
}
...
void glCopyTexImage2D (GLenum target, GLint level, GLenum internalFormat, GLint x, GLint y, GLsizei width, GLsizei height, GLint border)
{
	...
	if (pTexture == NULL) {
		...
		AppDebugAssert( hr != D3DXERR_INVALIDDATA );
	}
...
void MipMapTask::GenerateMipmaps()
{
	for( int i = 1; i < m_levels; i++ )
	{
		D3DXLoadSurfaceFromSurface(
			m_pSurface[i], NULL, NULL, 
			m_pSurface[0], NULL, NULL, 
...
static 
int d3dCreateTexture(GLenum target, GLint components, GLint width, GLint height, GLenum format, GLenum type, const void *data, bool mipmapping, bool _compressed = false )
{
	LPDIRECT3DTEXTURE9 &pTexture = s_textureIds[s_activeTextureState->target];
	...
	AppDebugAssert( width == 1 << (int) ceil(log((float) width)/log(2.0f)) );
	AppDebugAssert( height == 1 << (int) ceil(log((float) height)/log(2.0f)) );

	// We also enforce that the format is convenient
	AppDebugAssert( format == GL_RGBA );	
	AppDebugAssert( type == GL_UNSIGNED_BYTE );

	HRESULT rc = g_pd3dDevice->CreateTexture(width, height, mipmapping ? 0 : 1, 
		D3DUSAGE_AUTOGENMIPMAP, // autogenmipmap ignored by xbox
		_compressed ? D3DFMT_DXT1 : D3DFMT_A8R8G8B8, 
		D3DPOOL_MANAGED, &pTexture, NULL);
	AppDebugAssert( rc != D3DERR_INVALIDCALL );
	AppDebugAssert( rc != D3DXERR_INVALIDDATA );
	...
	D3DXLoadSurfaceFromMemory(pTopSurface, NULL, NULL, data, D3DFMT_A8B8G8R8, width * 4, NULL, &rect, D3DX_FILTER_NONE, 0);
	pTopSurface->Release();


	if( mipmapping )
	{
		MipMapTask *task = new MipMapTask( pTexture, s_activeTextureState->target );
...
int gluBuild2DMipmaps (GLenum target, GLint components, GLint width, GLint height, GLenum format, GLenum type, const void *data, bool _compressed)
{
...
void glTexImage2D (GLenum target, GLint level, GLint internalformat, GLsizei width, GLsizei height, GLint border, GLenum format, GLenum type, const GLvoid *pixels, bool _compressed)
{
	...
	AppDebugAssert( target == GL_TEXTURE_2D );
	AppDebugAssert( level == 0 );
	AppDebugAssert( internalformat == 4 );
	AppDebugAssert( format == GL_RGBA );
...
static DWORD glMinMagFilterToDirect3D(GLenum param)
{
	switch (param) {
		...
		default: 
			AppReleaseAssert(FALSE, "Unknown Filter %u", param);
			return D3DTEXF_NONE;
	}
}
...
static DWORD glTextureWrapToDirect3D(GLenum param)
{
	switch (param) {
		...
		default:
			AppReleaseAssert(FALSE, "Unknown Texture Wrap mode %u", param);
...
{
	GL_TRACE_IMP(" glFogfv(%s, (const float *)%p) ignored", glEnumToString(pname), params)
}
...
static GLenum direct3DFillModeToOpenGL( D3DFILLMODE mode )
{
	switch (mode) {
		...
		default:
			AppDebugAssert( mode == D3DFILL_SOLID || mode == D3DFILL_WIREFRAME );
			return GL_FILL;
	}
}
...
{
	return s_pModelViewMatrixStack->GetTransform();
}

const D3DXMATRIX& getProjMatrix()
{
	return s_pProjectionMatrixStack->GetTransform();
}
	...
	switch (pname) {
		case GL_POSITION:
			if (params[3] == 0.0f) {
				...
				D3DXVec3Normalize((D3DXVECTOR3*)&lightInfo.Direction,(D3DXVECTOR3*)&lightInfo.Direction); 
			}
			else {
...
{
	GL_TRACE_IMP(" glShadeModel(%s) not supported", glEnumToString(mode))
}
...
void glEndList ()
{
	...
	AppDebugAssert( s_pDisplayListDevice != NULL );
...
void glLineWidth ( GLfloat width )
{
	...
	g_pd3dDevice->SetRenderState( D3DRS_LINEWIDTH, * (DWORD *) &width );
}
...
void glFinish ()
{
    ...
    AppDebugAssert( hr != D3DERR_INVALIDCALL );
    AppDebugAssert( hr != D3DERR_WASSTILLDRAWING );
    AppDebugAssert( hr == D3D_OK );
...
int Shader::SetSamplerGLTextureId(const char* name, int tex)
{
	return SetSamplerDX(name,s_textureIds[tex]);
}

int Shader::SetSamplerGLFixedStage(const char* name, int tex)
{
	return SetSamplerGLTextureId(name,s_textureStates[tex].target);
}
