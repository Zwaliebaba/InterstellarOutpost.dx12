...
{   
	...
	AppDebugAssert(m_stream==NULL);
	AppDebugAssert(m_streamCompressed==NULL);
	AppDebugAssert(m_AVIFile==NULL);
    AppDebugAssert(m_pixels==NULL);
    AppDebugAssert(m_accumBuffer==NULL);
}
...
{
    AppDebugAssert( m_pixels );
    AppDebugAssert( m_accumBuffer );
	...
	float multiplier = 1.0 / m_motionBlurFactor;
...
void AVIGenerator::CheckSanity()
{
	...
	AppDebugAssert( stricmp(extension, "avi") == 0 );

	// checking that bitmap size are multiple of 4
	AppDebugAssert(m_bih.biWidth%4 == 0);
	AppDebugAssert(m_bih.biHeight%4 == 0);
}
