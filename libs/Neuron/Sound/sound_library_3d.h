...
protected:    
    ...
    bool                (*m_mainCallback) (unsigned int, signed short *, int, unsigned int, int *);
    bool                (*m_musicCallback) (signed short *, int, unsigned int, int *);
...
public:
    ...
    bool                m_forceVolumeUpdate;

public:
    SoundLibrary3d();
    virtual ~SoundLibrary3d();

    virtual bool Initialise         (int _mixFreq, int _numChannels, 
									 bool hw3d, int _mainBufNumSamples, int _musicBufNumSamples) = 0;
    void		 SetMainCallback    (bool (*_callback) (unsigned int, signed short *, int, unsigned int, int *));
    void		 SetMusicCallback   (bool (*_callback) (signed short *, int, unsigned int, int *));
	...
	virtual int  GetNumMainChannels () const { return m_numChannels; }
...
class SoundLibrary3dNoSound : public SoundLibrary3d
{
public:
	SoundLibrary3dNoSound() : SoundLibrary3d() {}
	~SoundLibrary3dNoSound() {}

    bool Initialise         (int _mixFreq, int _numChannels, 
							bool hw3d, int _mainBufNumSamples, int _musicBufNumSamples) { return true; };

    bool Hardware3DSupport	() { return false; }
    int  GetMaxChannels		() { return 0; }    
    int  GetCPUOverhead		() { return 0; }
    float GetChannelHealth  (int _channel) { return 1.0f; }					// 0.0 = BAD, 1.0 = GOOD
	int  GetChannelBufSize	(int _channel) const { return 0; }
	int  GetNumMainChannels () const { return m_numChannels; }
