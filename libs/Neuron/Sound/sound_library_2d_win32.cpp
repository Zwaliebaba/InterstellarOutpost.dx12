...
SoundLib2dBuf::SoundLib2dBuf()
{
	...
	if( g_systemInfo->m_audioInfo.m_preferredDevice != -1 )
		AppReleaseAssert(result == MMSYSERR_NOERROR, "Couldn't init buffer");
	...
	if (count < 4)
	{
		...
		if( g_systemInfo->m_audioInfo.m_preferredDevice != -1 )
			AppReleaseAssert(result == MMSYSERR_NOERROR, "Couldn't send sound data");
	}
}
...
{
	SoundLibrary2dWin32 *soundLibrary = (SoundLibrary2dWin32 *)g_soundLibrary2d;
	
	if (_msg != WOM_DONE) return;
	if (!s_device)	return;
	if (!soundLibrary || !soundLibrary->m_callback) return;

	soundLibrary->m_fillsRequested++;
}
...
:	m_callback(NULL),
	...
	m_fillsRequested(0)
{
	AppReleaseAssert(!g_soundLibrary2d, "SoundLibrary2d already exists");
	...
	if( g_systemInfo->m_audioInfo.m_preferredDevice != -1 )
	{
		...
		AppReleaseAssert(result == MMSYSERR_NOERROR, "Failed to open audio output device: \"%s\"", errString);
	}
...
void SoundLibrary2dWin32::TopupBuffer()
{
	...
	else
	{
		while (m_fillsRequested)
		{
			...
			m_callback(buf->m_buffer, m_samplesPerBuffer);
