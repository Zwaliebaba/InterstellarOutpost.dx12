...
const float XInputDriver::SCALE_JOYSTICK_L = 3.4f;
const float XInputDriver::SCALE_JOYSTICK_R = 2.7f;
...
static ControlAction s_controls[] = {
	...
	"lefttriggerb",			 INPUT_TYPE_BOOL,
	"righttriggerb",		 INPUT_TYPE_BOOL,
	NULL,                    NULL
};
...
:	m_controllerMask( 0x0F )
{
	setName( "XInput" );

	for( int i = 0; i < MaxControllers; i++ )
		m_controllers[i].Initialise( i );
}

void XInputDriver::Advance()
{
	for( int i = 0; i < MaxControllers; i++ )
		m_controllers[i].Advance();
}
...
{	
	for( int i = 0; i < MaxControllers; i++ )
	{
		if( (1 << i) & m_controllerMask )
		{
			if( m_controllers[i].getInput( spec, details ) )			
			{
				details.controller = i;
				return true;
			}
		}
	}
...
void XInputDriver::setControllerMask( unsigned controllerMask )
{
	AppDebugOut("XInputDriver setting controller mask to 0x%x\n", controllerMask );
...
//
// 
// Class XInputDriver::Controller

XInputDriver::Controller::Controller()
:	m_id( -1 )
{
...
void  XInputDriver::Controller::Initialise( int _id )
{
	m_id = _id;
}


bool XInputDriver::Controller::getInput( InputSpec const &spec, InputDetails &details )
{
	...
	switch ( spec.control_id ) {
		...
		case XInputLeftTrigger:
		case XInputRightTrigger:
		case XInputLeftTriggerB:
		case XInputRightTriggerB:
...
bool XInputDriver::Controller::isIdle()
{
...
bool XInputDriver::Controller::getButtonInput( WORD button, InputSpec const &spec, InputDetails &details )
{
...
bool XInputDriver::Controller::getThumbInput( InputSpec const &spec, InputDetails &details )
{
	...
	switch ( spec.condition ) {
		...
		case COND_MOVED:
			...
			return ( abs( details.x ) > SCALE_JOYSTICK_L || abs( details.y ) > SCALE_JOYSTICK_L );
...
bool XInputDriver::Controller::getTriggerInput( InputSpec const &spec, InputDetails &details )
{
	...
	if( spec.control_id == XInputLeftTriggerB || spec.control_id == XInputRightTriggerB )
		return val != 0;
...
void XInputDriver::Controller::correctDeadZones( XINPUT_STATE &state )
{
...
void XInputDriver::Controller::scaleInputs( XINPUT_STATE &state )
{
	XINPUT_GAMEPAD &gp = state.Gamepad;

	double temp;

	temp = gp.sThumbLX * SCALE_JOYSTICK_L / XINPUT_GAMEPAD_THUMB_MAX;
	gp.sThumbLX = floor( temp * temp * temp );

	temp = gp.sThumbLY * SCALE_JOYSTICK_L / XINPUT_GAMEPAD_THUMB_MAX;
	gp.sThumbLY = -floor( temp * temp * temp );

	temp = gp.sThumbRX * SCALE_JOYSTICK_R / XINPUT_GAMEPAD_THUMB_MAX;
	gp.sThumbRX = floor( temp * temp * temp );

	temp = gp.sThumbRY * SCALE_JOYSTICK_R / XINPUT_GAMEPAD_THUMB_MAX;
...
bool XInputDriver::Controller::getThumbDirection( control_id_t control, InputSpec const &spec,
                                      unsigned direction, InputDetails &details )
{
	...
	bool isPressed = ( details.x > SCALE_JOYSTICK_L );
	bool wasPressed = ( details.y > SCALE_JOYSTICK_L );
	double now = 0.0;
	bool rep = false;
...
void XInputDriver::Controller::Advance()
{
	DWORD dwResult;
	...
	dwResult = XInputGetState( m_id, &state );
