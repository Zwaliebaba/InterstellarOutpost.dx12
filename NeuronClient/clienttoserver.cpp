...
static NetCallBackRetType ClientToServerListenCallback(NetUdpPacket* udpdata)
{
  if (udpdata)
  {
    ...
    {
      DebugTrace("Client received bogus data, discarded (1)\n");
      delete letter;
    }
...
void ClientToServer::OpenConnections()
{
  ...
  {
    DebugTrace("Client failed to bind to port %d\n", port);
    DebugTrace("Client looking for any available port number...\n");
    delete m_listener;
  ...
  if (result != NetOk)
  {
    DebugTrace("ERROR : Client failed to bind to open Listener\n");
    delete m_listener;
    m_listener = nullptr;
    return;
  }
  ...
  DebugTrace("Client listening on port %d\n", GetLocalPort());
}
...
ClientToServer::~ClientToServer()
{
  ...
  for (int i = 0; i < GAMEOPTION_NUMGAMEOPTIONS; i++)
  {
    ...
    m_gameOptionArrayVersion[i] = nullptr;
  }
}
  ...
  while (m_outbox.Size())
  {
    if (m_connectionState > StateDisconnected)
    {
      ...
      else
      {
...
#ifdef NETWORKQUALITY_TESTING
        static NetworkQuality q(200, 0.25, 8000); NetRetCode result = q.Write(socket, byteStream, letterSize, &writtenData);
#else
        NetRetCode result = socket->WriteData(byteStream, letterSize, &writtenData);
#endif

        if (result != NetOk)
          DebugTrace("CLIENT write data bad result %d\n", static_cast<int>(result));
        if (writtenData != 0 && writtenData < letterSize)
          DebugTrace("CLIENT wrote less data than requested\n");
        ...
        {
          s_largest = totalSize;
          DebugTrace("CLIENT: Largest datagram sent : %d bytes\n", s_largest);
#ifdef TARGET_DEBUG
          letter->DebugPrint(0);
#endif
        }
...
void ClientToServer::Advance()
{
  ...
  if (m_connectionState == StateConnected && g_app->m_gameMode != App::GameModeCampaign && g_app->m_gameMode != App::GameModePrologue && (
    timeNow > m_retryTimerFast || timeNow > m_retryTimer || timeNow > m_retryTimerSlow) && !g_app->m_spectator)
  {
    ...
    else
    {
      ...
      {
        if ((!g_app->m_multiwinia->m_coopMode && g_app->m_multiwinia->m_gameType != Multiwinia::GameTypeAssault) || (g_app->m_atMainMenu &&
          !g_app->m_location))
      ...
      {
        DebugTrace("Requesting send viral achievement mask\n");
  ...
  if (m_outOfSyncClients.Size() > 0 && m_syncErrorSeqId != -1)
  {
    for (int i = 0; i < NUM_TEAMS; i++)
    {
      ...
      if (lobbyTeam.m_clientId != m_clientId)
      {
        ...
        {
          g_app->m_multiwinia->SaveIFrame(lobbyTeam.m_clientId, new IFrame(ftp->Data(), ftp->Size()));
...
Directory* ClientToServer::GetNextLetter()
{
  ...
  if (m_inbox.Size() > 0)
  {
    ...
    else if (seqId == g_lastProcessedSequenceId + 1 || seqId == -1)
    {
    ...
    {
      DebugTrace("Dropping duplicate letter %d (LPSID = %d)\n", seqId, g_lastProcessedSequenceId);
...
{
  int seqIdDifference = max(0, GetEstimatedServerSeqId() - g_lastProcessedSequenceId);
...
void ClientToServer::ReceiveLetter(Directory* letter, int _bandwidthUsed)
{
  ...
  if (strcmp(letter->m_name, NET_DARWINIA_MESSAGE) != 0 || !letter->HasData(NET_DARWINIA_SEQID, DIRECTORY_TYPE_INT))
  {
    DebugTrace("Client received bogus data, discarded (2)\n");
    delete letter;
    return;
  }
  ...
  double newStartTime = GetHighResTime() - static_cast<double>(seqId) * 0.1f;
  ...
  for (i = 0; i < m_inbox.Size(); ++i)
  {
    ...
    //{
    //	DebugTrace("lastValidSequenceId: %d => %d (lastProcessed = %d, thisSeqId = %d). m_clientId = %d\n", 
    //		old, m_lastValidSequenceIdFromServer, g_lastProcessedSequenceId, thisSeqId, m_clientId );
    //}

    if (thisLetter->m_subDirectories.Size() == 0 && thisLetter->HasData(NET_DARWINIA_NUMEMPTYUPDATES, DIRECTORY_TYPE_INT))
    {
      int numEmptyUpdates = thisLetter->GetDataInt(NET_DARWINIA_NUMEMPTYUPDATES);
      ASSERT(numEmptyUpdates != -1);
      m_lastValidSequenceIdFromServer = max(m_lastValidSequenceIdFromServer, thisSeqId + numEmptyUpdates - 1);

      //DebugTrace("lastValidSequenceId: %d => %d (lastProcessed = %d, thisSeqId = %d, numEmpty = %d). m_clientId = %d\n", 
...
void ClientToServer::SendLetter(Directory* letter)
{
  if (letter)
  {
    ASSERT(letter->HasData(NET_DARWINIA_COMMAND,DIRECTORY_TYPE_STRING));
...
void ClientToServer::ClientLeave()
{
  ...
  DebugTrace("CLIENT : Disconnected\n");
}

void ClientToServer::RequestToRegisterAsSpectator(int _teamID)
{
  DebugTrace("CLIENT : Requesting to become a spectator (TeamID = %d)...\n", _teamID);
  auto letter = new Directory();
  unsigned char cTeam = static_cast<unsigned char>(_teamID);
  unsigned char cClient = static_cast<unsigned char>(m_clientId);
...
{
  DebugTrace("CLIENT : Sending Team Scores...\n");
  ...
  letter->CreateData(NET_DARWINIA_TEAMID, static_cast<unsigned char>(_winnerId));
...
void DirectorySetVector3(Directory* _letter, const char* _field, const Vector3& _pos)
{
  char data[256];
  std::ostrstream s(data, 256);
  WriteNetworkValue(s, _pos);
  _letter->CreateData(_field, s.str(), s.tellp());
}

Vector3 DirectoryGetVector3(Directory* _letter, const char* _field)
{
  int len;
  auto data = static_cast<const char*>(_letter->GetDataVoid(_field, &len));
...
void ClientToServer::SendIAmAlive(unsigned char _teamId, const TeamControls& _teamControls)
{
  auto letter = new Directory();
  ...
  letter->CreateData(NET_DARWINIA_ALIVE_DATA, data, length);
...
bool ClientToServer::IsSequenceIdInQueue(int _seqId)
{
  ...
  for (int i = 0; i < m_inbox.Size(); ++i)
  {
    ...
    {
      int numEmptyUpdates = thisLetter->GetDataInt(NET_DARWINIA_NUMEMPTYUPDATES);
      if (_seqId >= thisSeqId && _seqId < thisSeqId + numEmptyUpdates)
      {
        found = true;
        break;
      }
...
{
  DebugTrace("CLIENT : Requesting TeamName %s for team %d...\n", _teamName.m_charstring, _teamId);
  ...
  DebugTrace("Setting Team name %s at %2.4f\n", _teamName.m_charstring, GetHighResTime());
...
{
  DebugTrace("CLIENT: Sending chat message");
...
{
  //DebugTrace("Setting Game Option Version %d %d to %d\n", _gameOption, _index, _version );
  if (_index == -1)
...
void ClientToServer::ReliableSetTeamColour(int _colourId) { m_requestedTeamDetails.m_colourId = _colourId; }
...
{
  if (_colourId != -1)
    DebugTrace("CLIENT : Requesting TeamColour %d for team %d...\n", _colourId, _teamId);
...
void ClientToServer::RequestAimBuilding(unsigned char _teamId, int _buildingId, const Vector3& _pos)
{
  auto letter = new Directory();
...
void ClientToServer::RequestTargetProgram(unsigned char _teamId, unsigned char _program, const Vector3& _pos)
{
  auto letter = new Directory();
...
{
  DebugTrace("Request terminate entity : team %d entity %d\n", static_cast<int>(_teamId), _entityId);

  auto letter = new Directory();

  letter->CreateData(NET_DARWINIA_COMMAND, NET_DARWINIA_TERMINATE);
  letter->CreateData(NET_DARWINIA_TEAMID, _teamId);
  letter->CreateData(NET_DARWINIA_PROGRAM, static_cast<unsigned char>(255));
...
{
  DebugTrace("Requesting previous\n");
  auto letter = new Directory();
...
void ClientToServer::RequestSelectDarwinians(unsigned char _teamId, const Vector3& _pos, float _radius)
{
  auto letter = new Directory();
...
void ClientToServer::RequestIssueDarwinanOrders(unsigned char _teamId, const Vector3& _pos, bool directRoute)
{
  auto letter = new Directory();

  letter->CreateData(NET_DARWINIA_COMMAND, NET_DARWINIA_ORDERDGS);
  letter->CreateData(NET_DARWINIA_TEAMID, _teamId);
  letter->CreateData(NET_DARWINIA_DIRECTROUTE, static_cast<char>(directRoute));
...
void ClientToServer::RequestOfficerOrders(unsigned char _teamId, const Vector3& _pos, bool directRoute, bool forceOnlyMove)
{
  auto letter = new Directory();
  ...
  letter->CreateData(NET_DARWINIA_TEAMID, _teamId);
  letter->CreateData(NET_DARWINIA_DIRECTROUTE, static_cast<char>(directRoute));
  letter->CreateData(NET_DARWINIA_FORCEONLYMOVE, static_cast<char>(forceOnlyMove));
...
void ClientToServer::RequestArmourOrders(unsigned char _teamId, const Vector3& _pos, bool deployAtWaypoint)
{
  auto letter = new Directory();
  ...
  letter->CreateData(NET_DARWINIA_FORCEONLYMOVE, static_cast<char>(!deployAtWaypoint));
...
void ClientToServer::RequestToggleEntity(unsigned char _teamId, int _entityId, bool secondary)
{
  ...
  letter->CreateData(NET_DARWINIA_TEAMID, _teamId);
  letter->CreateData(NET_DARWINIA_ENTITYID, _entityId);
  letter->CreateData(NET_DARWINIA_SECONDARYFUNCTION, static_cast<char>(secondary));
...
bool ClientToServer::IsLocalLetter(Directory* letter) { return letter->GetDataInt(NET_DARWINIA_CLIENTID) == m_clientId; }
...
int ClientToServer::GetNumSlicesToAdvance(bool catchUp)
{
  ...
  if (g_sliceNum != -1)
    numSlicesPending += (NUM_SLICES_PER_FRAME - g_sliceNum);
  ...
  numSlicesToAdvance = min(numSlicesToAdvance, 20);
  ...
  for (int i = 0; i < slicesToAdvance; ++i)
  {
    if (g_sliceNum == -1)
    {
      ...
      if (letter)
      {
        ...
        DEBUG_ASSERT(seqId == -1 || seqId == g_lastProcessedSequenceId + 1);
        m_lastServerLetterReceivedTime = GetHighResTime();

#ifdef TRACK_SYNC_RAND
        SyncRandLog("Seq ID: %d\n", g_lastProcessedSequenceId); if (debug)
        {
          std::ostringstream s;
...
#ifdef TRACK_SYNC_RAND
      if (debug)
        SyncRandLog("g_sliceNum = %d", g_sliceNum);
#endif
      if (loc)
      {
        if (!g_app->GamePaused())
...
#ifdef TRACK_SYNC_RAND
          if (debug)
            SyncRandLog("Advance Multiwinia");
#endif
...
#ifdef TRACK_SYNC_RAND
        if (debug)
          SyncRandLog("Incremement sliceNumber");
#endif
        g_sliceNum++;
      }
      else
...
#ifdef TRACK_SYNC_RAND
        if (debug)
          SyncRandLog("sliceNumber reset to -1");
#endif
        g_sliceNum = -1;
      }
    }
...
bool ClientToServer::ProcessServerLetters(Directory* letter)
{
  ...
  if (strcmp(letter->m_name, NET_DARWINIA_MESSAGE) != 0 || !letter->HasData(NET_DARWINIA_COMMAND))
  {
    DebugTrace("Client received bogus message, discarded (4)\n");
    return true;
  }
  ...
  if (strcmp(cmd, NET_DARWINIA_DISCONNECT) == 0)
  {
    // TO DO: Display a message to the user?
    int reason = letter->GetDataInt(NET_DARWINIA_DISCONNECT);

    DebugTrace("Received the disconnect message! Showing upsell\n");
    ...
    if (gmw)
    {
      DebugTrace("Going back to main menu\n");
      if (reason == Disconnect_ServerFull)
      {
        ...
        m_connectionState = StateDisconnected;
        DebugTrace("------------------> Been disconnected because server is full\n");
      }
      else if (reason == Disconnect_DuplicateKey)
      {
        ...
        m_connectionState = StateDisconnected;
        DebugTrace("------------------> Been disconnected because of duplicate key\n");
      }
      else if (reason == Disconnect_KickedByServer)
      {
        ...
        m_connectionState = StateDisconnected;
        DebugTrace("------------------> Has been kicked by the server\n");
      }
      else if (reason == Disconnect_BadPassword)
      {
        ...
        m_connectionState = StateDisconnected;
        DebugTrace("------------------> Kicked because of incorrect password\n");
      }
  ...
  if (strcmp(cmd, NET_DARWINIA_CLIENTID) == 0)
  {
    int clientId = letter->GetDataInt(NET_DARWINIA_CLIENTID);
    ...
    if (iSpectator != -1)
    {
      if (iSpectator)
      {
        ...
        m_connectionState = StateHandshaking;
  ...
  if (strcmp(cmd, NET_DARWINIA_TEAMASSIGN) == 0)
  {
    int clientId = letter->GetDataInt(NET_DARWINIA_CLIENTID);
  ...
  if (strcmp(cmd, NET_DARWINIA_SERVER_FULL) == 0)
  {
    int clientId = letter->GetDataInt(NET_DARWINIA_CLIENTID);
  ...
  if (strcmp(cmd, NET_DARWINIA_NETSYNCERROR) == 0)
  {
    int clientId = letter->GetDataInt(NET_DARWINIA_CLIENTID);
    int seqId = letter->GetDataInt(NET_DARWINIA_SYNCERRORSEQID);

    DebugTrace("CLIENT: Server informed us that client %d is out of sync at seq id %d\n", clientId, seqId);
  ...
  if (strcmp(cmd, NET_DARWINIA_NETSYNCFIXED) == 0)
  {
    int clientId = letter->GetDataInt(NET_DARWINIA_CLIENTID);
    DebugTrace("SYNCFIXED Server informed us that Client %d has repaired his Sync Error\n", clientId);
    SetSyncState(clientId, true);
    return true;
  }
  if (strcmp(cmd, NET_DARWINIA_SPECTATORASSIGN) == 0)
  {
    int clientId = letter->GetDataInt(NET_DARWINIA_CLIENTID);
    ...
    if (clientId == m_clientId)
    {
      DebugTrace("CLIENT: I am a spectator\n");
      m_bSpectator = true;
    }

    return true;
  }
  if (strcmp(cmd, NET_DARWINIA_FILESEND) == 0)
  {
...
bool ClientToServer::ProcessServerUpdates(Directory* letter)
{
  ...
  if (strcmp(letter->m_name, NET_DARWINIA_MESSAGE) != 0 || !letter->HasData(NET_DARWINIA_COMMAND, DIRECTORY_TYPE_STRING))
  {
    DebugTrace("ClientToServer received bogus message, discarded (8)\n");
    return false;
  }
  ...
  if (strcmp(cmd, NET_DARWINIA_UPDATE) != 0)
  {
    DebugTrace("ClientToServer received bogus message, discarded (9)\n");
    return false;
  }
  ...
  if (letter->m_subDirectories.Size() == 0 && letter->HasData(NET_DARWINIA_NUMEMPTYUPDATES, DIRECTORY_TYPE_INT))
  {
    ...
    {
      auto letterCopy = new Directory(*letter);
  ...
  for (int i = 0; i < letter->m_subDirectories.Size(); ++i)
  {
    if (letter->m_subDirectories.ValidIndex(i))
    {
      ...
      ASSERT(strcmp(update->m_name, NET_DARWINIA_MESSAGE) == 0);
      ASSERT(update->HasData( NET_DARWINIA_COMMAND, DIRECTORY_TYPE_STRING ));
      ...
      else if (strcmp(cmd, NET_DARWINIA_SET_PLAYERACHIEVEMENTS) == 0)
      {
        ...
        {
          DebugTrace("Update has player achievement mask\n");
          achievementMask = update->GetDataUChar(NET_DARWINIA_PLAYERACHIEVEMENTS);
          DebugTrace("Achievement mask is %d\n", achievementMask);
        }

        if (teamId >= 0 && teamId < NUM_TEAMS)
        {
          DebugTrace("Setting achievement mask to %d for team id %d\n", achievementMask, teamId);
      ...
      {
        DebugTrace("Game Option Array Element Int\n");
        ...
        if (version > GameOptionVersion(gameOption, index))
        {
          ...
          case GAMEOPTION_GAME_PARAM:
            if (!g_app->m_multiwinia->ValidGameOption(index))
              DebugTrace("Warning: Invalid game param %d\n", index);
            else
          ...
          case GAMEOPTION_GAME_RESEARCH_LEVEL:
            if (index < 0 || index >= GlobalResearch::NumResearchItems)
              DebugTrace("Warning: Game research %d is outside of allowed range\n", index);
            else
              g_app->m_globalWorld->m_research->m_researchLevel[index] = value;
            break;

          default:
            DebugTrace("Warning: Game array index option %d unknown\n", gameOption);
            break;
          }
        }
      }
        ...
        if (version > GameOptionVersion(gameOption))
        {
          ...
          case GAMEOPTION_GAME_TYPE:
            if (value < 0 || value >= Multiwinia::NumGameTypes)
              DebugTrace("Warning: Game type %d is outside of allowed range\n", value);
            else
            {
          ...
          case GAMEOPTION_AIMODE:
          {
            if (value < 0 || value > Multiwinia::NumAITypes)
              DebugTrace("Warning: AI Mode %d is outside of allowed range\n", value);
            else
          ...
          case GAMEOPTION_BASICCRATES: { g_app->m_multiwinia->m_basicCratesOnly = value; }
          break;

          case GAMEOPTION_SINGLEASSAULT: { g_app->m_multiwinia->m_assaultSingleRound = value; }
          break;

          default:
            DebugTrace("Warning: Game int option %d unknown\n", gameOption);
            break;
          }
        }
      }
      ...
      {
        DebugTrace("Game Option Array  Int\n");
        int gameOption = update->GetDataInt(NET_DARWINIA_GAMEOPTION_NAME);
        ...
        default:
          DebugTrace("Warning: Game array int option %d unknown\n", gameOption);
          break;
        }
      }
      else if (strcmp(cmd, NET_DARWINIA_SETGAMEOPTIONSTRING) == 0)
      {
        ...
        if (version > GameOptionVersion(gameOption))
        {
          DebugTrace("Update to game string option %d (we have version %d) and client gave version %d. %s ==> %s\n", gameOption,
                     GameOptionVersion(gameOption), version, g_app->m_requestedMap, value);
          SetGameOptionVersion(gameOption, -1, version);
          switch (gameOption)
          {
          case GAMEOPTION_MAPNAME:
            ...
            if (g_app->m_server)
            {
              ...
              if (GetServerPort() != -1 && strcmp(g_app->m_requestedMap, "null") != 0 && strlen(g_app->m_requestedMap) > 0)
          ...
          default:
            DebugTrace("Warning: Game string option %d unknown\n", gameOption);
            break;
          }
        }
        DebugTrace("Discarded update to game string option %d because we have version %d and client gave version %d\n", gameOption,
                   GameOptionVersion(gameOption), version);
      }
      else if (strcmp(cmd, NET_DARWINIA_START_GAME) == 0)
      {
        DebugTrace("CLIENT : Received Start Game\n");
        // Start the game 
      ...
      else if (strcmp(cmd, NET_DARWINIA_ALIVE) == 0)
      {
        ...
        int length;
        auto data = static_cast<char*>(update->GetDataVoid(NET_DARWINIA_ALIVE_DATA, &length));
      ...
      else if (strcmp(cmd, NET_DARWINIA_TOGGLE_READY) == 0)
      {
        ...
        else
        {
          ...
          DebugTrace("Team %d ready : %d\n", teamId, static_cast<int>(team->m_ready));
        }
      }
      ...
      else if (strcmp(cmd, NET_DARWINIA_AIMBUILDING) == 0)
      {
        ...
        if (building && building->m_id.GetTeamId() == teamId && building->m_type == Building::TypeRadarDish)
        {
      ...
      else if (strcmp(cmd, NET_DARWINIA_TERMINATE) == 0)
      {
        ...
        if (program != 255)
        {
          DebugTrace("Network update Terminate : team %d program %d\n", static_cast<int>(teamId), static_cast<int>(program));
          team->m_taskManager->TerminateTask(program);
        }
        else
        {
          DebugTrace("Network update Terminate : team %d entity %d\n", static_cast<int>(teamId), entityId);
          if (team->m_others.ValidIndex(entityId))
          {
            DebugTrace("Entity found\n");
      ...
      else if (strcmp(cmd, NET_DARWINIA_NEXTWEAPON) == 0)
      {
        ...
        if (entity)
        {
          if (entity->m_type == Entity::TypeOfficer)
            static_cast<Officer*>(entity)->SetNextMode();
          else if (entity->m_type == Entity::TypeArmour)
            static_cast<Armour*>(entity)->SetDirectOrders();
        }
      }
      else if (strcmp(cmd, NET_DARWINIA_PREVWEAPON) == 0)
      {
        ...
        {
          if (entity->m_type == Entity::TypeOfficer)
            static_cast<Officer*>(entity)->SetPreviousMode();
          else if (entity->m_type == Entity::TypeArmour)
            static_cast<Armour*>(entity)->SetDirectOrders();
        }
      }
      else if (strcmp(cmd, NET_DARWINIA_NONE) == 0)
      {
        ...
        {
          if (entity->m_type == Entity::TypeOfficer)
            static_cast<Officer*>(entity)->CancelOrders();
        }
      }
      else if (strcmp(cmd, NET_DARWINIA_FOLLOW) == 0)
      {
        ...
        {
          if (entity->m_type == Entity::TypeOfficer)
            static_cast<Officer*>(entity)->SetFollowMode();
        }
      }
      else if (strcmp(cmd, NET_DARWINIA_SHAMANSUMMON) == 0)
      {
        ...
        for (int i = 0; i < loc->m_teams[teamId]->m_specials.Size(); ++i)
        {
          ...
          if (id.GetUniqueId() == entityId)
            break;
        }
        auto shaman = static_cast<Shaman*>(loc->GetEntitySafe(id, Entity::TypeShaman));
        if (shaman)
      ...
      else if (strcmp(cmd, NET_DARWINIA_ORDERGOTO) == 0)
      {
        ...
        if (entity)
        {
          if (entity->m_type == Entity::TypeOfficer)
          {
            ...
            if (forceOnlyMove)
              officer->SetWaypoint(pos);
            else
              officer->SetOrders(pos, directRoute);
          }
          else if (entity->m_type == Entity::TypeArmour)
          {
            ...
            if (forceOnlyMove)
              armour->SetWayPoint(pos);
            else
              armour->SetOrders(pos);
        ...
        if (team->m_others.ValidIndex(entityId))
        {
          ...
          if (entity)
          {
            if (entity->m_type == Entity::TypeArmour)
            {
              ...
              if (secondary)
                armour->ToggleConversionAction();
              else
                armour->ToggleLoading();
            }
            else if (entity->m_type == Entity::TypeOfficer)
            {
              auto officer = static_cast<Officer*>(entity);

              if (officer->IsInFormationMode() || officer->m_orders == Officer::OrderGoto)
      ...
      else if (strcmp(cmd, NET_DARWINIA_REQUEST_TEAMNAME) == 0)
      {
        ...
        DebugTrace("Team name set to %s at %2.4f\n", teamName.m_charstring, GetHighResTime());
      ...
      else if (strcmp(cmd, NET_DARWINIA_SEND_CHATMESSAGE) == 0)
      {
        if (update->HasData(NET_DARWINIA_UNICODEMESSAGE) && update->HasData(NET_DARWINIA_CLIENTID))
        {
          ...
          else
          {
            if (g_app->m_location)
            {
              g_app->m_location->NewChatMessage(update->GetDataUnicode(NET_DARWINIA_UNICODEMESSAGE),
                                                update->GetDataUChar(NET_DARWINIA_CLIENTID));
            }
          }
        }
      }
      ...
      else if (strcmp(cmd, NET_DARWINIA_CHRISTMASMODE) == 0)
      {
        ...
        DebugTrace("ClientToServer received bogus message, discarded (11)\n");
    }
  }
  return true;
}
...
void ClientToServer::ProcessFTPLetter(Directory* _letter) { m_ftpManager->ReceiveLetter(_letter); }
