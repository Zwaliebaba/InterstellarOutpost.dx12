    ...
    while (m_outbox.Size())
    {
		if (m_connectionState > StateDisconnected) 
		{
			...
			else
			{
				...
				if( result != NetOk )           DebugTrace("CLIENT write data bad result %d\n", (int) result );
				if( writtenData != 0 &&
					writtenData < letterSize )  DebugTrace("CLIENT wrote less data than requested\n" );
...
void ClientToServer::Advance()
{
    ...
    if( m_connectionState == StateConnecting && timeNow > m_retryTimer )
    {
		...
		if(g_app->GetTestBedMode() != TESTBED_ON)
		{
			...
			{
				DebugTrace("CLIENT has exceeded maximum connection attempts.\n" );
				m_connectionState = StateDisconnected;
				return;
...
{
    int seqIdDifference = 
		max( 0, GetEstimatedServerSeqId() - g_lastProcessedSequenceId );
...
{
    if( _colourId != -1 ) DebugTrace( "CLIENT : Requesting TeamColour %d for team %d...\n", _colourId, _teamId );
...
bool ClientToServer::ProcessServerLetters( Directory *letter )
{
    ...
    else if( strcmp( cmd, NET_DARWINIA_TEAMASSIGN ) == 0 )
    {
        ...
        if( letter->HasData( NET_DARWINIA_CLIENTISDEMO ) )
        {
            ...
            DebugTrace( "Team %d with Client %d is DEMO PLAYER\n", teamId, clientId );
        }
