...
#ifdef HAVE_XAUDIO
#include "sound_library_3d_xaudio.h"
...
public:
	...
	bool Initialise( int _id, unsigned _mixFreq, float _packetDuration, unsigned _numPackets )
	{
		...
		if( FAILED( hr ) )
			DebugTrace("XAudioChannel::Initialise - Failed to create xaudio voice. Error code = %#X\n", hr );
	...
	{		
		//DebugTrace("XAudioChannel(%d)::SetVolume %d millibels\n", m_id, _millibels );
	...
	void SetFrequency( float _freq )
	{
		if( m_setFreq != _freq )
		{
			if( _freq > 0 && _freq < 48000 * 3 )
				...
				DebugTrace("XAudioChannel::SetFrequency - attempted to set bogus frequency: %f\n");
		}
	}
	...
	bool Hungry( short *&_samples, int &_numSamples )
	{
		...
		if( hr != S_OK )
		{
			DebugTrace("Error getting voice state: %#X\n", hr);
			return false;
		}
	...
	void DebugPrint()
	{
		...
		if( m_pSourceVoice )
		{
			DebugTrace("XAudioChannel, null voice\n" );
			return;
		}
		...
		DebugTrace( "XAudioChannel State -- |%s%s%s%s%s%s%s\n",
	...
	void Swallow()
	{
		//DebugTrace( "XAudioChannel::Swallow() called\n" );
		...
		if( FAILED( hr ) )
		{
			DebugTrace( "XAudioChannel::Swallow() - failed to submit audio packet. ErrorCode = %#X\n", hr );
		}
	...
	{		
		if( m_3dMode != 2 )
		{
			...
			{
				DebugTrace("Warning, m_minDist = 0\n, setting to 1" );
				m_minDistance = 1.0f;
			}
...
private:
	...
	void SetFrequencyActual( float _freq )
	{
		...
		if( FAILED( hr ) )
			DebugTrace("XAudioChannel::SetFrequency - Failed to set pitch on source voice. Error code = %#X\n", hr );
		m_setFreq = _freq;
	}
...
{    
	...
	DebugTrace( "Initialising XAudio 3D SoundLibrary with %d channels at mix frequency of %d.\n", 
...
void SoundLibrary3dXAudio::Advance()
{
	...
	{	
		...
		u << "]\n";
		DebugTrace( "%s", s.str().c_str() );
		DebugTrace( "%s", t.str().c_str() );
		DebugTrace( "%s", u.str().c_str() );
