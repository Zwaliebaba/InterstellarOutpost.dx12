...
void ReadTag( std::istream &_input, const char *_tag )
{
	...
	if( !matches )
	{
		DebugTrace("Remaining data: [");

		while (!_input.eof())
			DebugTrace("%c", _input.get());
	}
#endif
	//ASSERT_TEXT( matches, "Failed to read tag: %s\n", _tag );
}
    ...
    for( int t = 0; t < NUM_TEAMS; ++t )
    {
		...
		if( teamUsed )
		{
            ...
            for( int u = 0; u < unitsSize; ++u )
            {
				...
				ASSERT_TEXT( _input, "validUnitIndex" );
				...
				if( validUnitIndex )
				{
                    ...
                    for( int e = 0; e < entitiesSize; ++e )
                    {
						...
						ASSERT_TEXT( _input, "validEntityIndex" );
			...
			for( int e = 0; e < othersSize; ++e )
			{
				...
				ASSERT_TEXT( _input, "validOtherIndex" );
...
{	
	...
	ASSERT_TEXT(a.m_sequenceId == b.m_sequenceId, "IFrame::CalculateDiff - different sequence ids");
	// ASSERT_TEXT( aInput.eof(), "Failed to completely read iframe a" );
	// ASSERT_TEXT( bInput.eof(), "Failed to completely read iframe b" );
