      ...
      if (window)
      {
        ...
        else
        {
          if (strcmp(currentButton, "None") == 0)
          {
            ...
            else { if (window->m_movable) { EclSetWindowPosition(mouseDownWindow, mouseX - mouseDownWindowX, mouseY - mouseDownWindowY); } }
          }
        }
      }
    }
...
void EclRegisterWindow(EclWindow* window, EclWindow* parent)
{
  ...
  if (parent && window->m_x == 0 && window->m_y == 0)
  {
    ...
    if (left > window->m_w / 2)
      window->m_x = static_cast<int>(parent->m_x + parent->m_w * (float)AppRandom() / (float)APP_RAND_MAX);
    else
      window->m_x = static_cast<int>(parent->m_x - window->m_w * (float)AppRandom() / (float)APP_RAND_MAX);
    if (above > window->m_h / 2)
      window->m_y = static_cast<int>(parent->m_y + parent->m_h * (float)AppRandom() / (float)APP_RAND_MAX);
    else
      window->m_y = static_cast<int>(parent->m_y - window->m_h / 2 * (float)AppRandom() / (float)APP_RAND_MAX);
  }
...
bool EclMouseInButton(EclWindow* window, EclButton* button)
{
  ...
  return (EclMouseInWindow(window) && mouseX >= window->m_x + button->m_x && mouseX <= window->m_x + button->m_x + button->m_w && mouseY >=
    window->m_y + button->m_y && mouseY <= window->m_y + button->m_y + button->m_h);
}
...
LList<EclWindow*>* EclGetWindows() { return &windows; }
