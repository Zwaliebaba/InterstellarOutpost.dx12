...
namespace DirectX::LoaderHelpers
{
  ...
  inline size_t BitsPerPixel(_In_ DXGI_FORMAT fmt) noexcept
  {
    switch (fmt)
    {
      ...
      case DXGI_FORMAT_FORCE_UINT: default:
        return 0;
    }
  }
  ...
  //--------------------------------------------------------------------------------------
  inline HRESULT LoadTextureDataFromMemory(const byte_buffer_t& ddsData, const DDS_HEADER** header, const uint8_t** bitData,
                                           size_t* bitSize) noexcept
  {
    ...
    *bitSize = 0;
    ...
    if (ddsData.size() > UINT32_MAX)
      return E_FAIL;

    if (ddsData.size() < (sizeof(uint32_t) + sizeof(DDS_HEADER)))
      return E_FAIL;
    ...
    if ((hdr->ddspf.flags & DDS_FOURCC) && (MAKEFOURCC('D', 'X', '1', '0') == hdr->ddspf.fourCC))
    {
      ...
      if (ddsData.size() < (sizeof(uint32_t) + sizeof(DDS_HEADER) + sizeof(DDS_HEADER_DXT10)))
        return E_FAIL;
  ...
  //--------------------------------------------------------------------------------------
  inline HRESULT GetSurfaceInfo(_In_ size_t width, _In_ size_t height, _In_ DXGI_FORMAT fmt, _Out_opt_ size_t* outNumBytes,
...
#undef ISBITMASK
    ...
    if (header->ddspf.flags & DDS_FOURCC)
    {
      if (MAKEFOURCC('D', 'X', '1', '0') == header->ddspf.fourCC)
      {
        ...
        switch (mode)
        {
          ...
          case DDS_ALPHA_MODE_UNKNOWN: default:
            break;
        }
      }
  ...
  {
    const float origAR = static_cast<float>(origx) / static_cast<float>(origy);
    ...
    if (origx > origy)
    {
      ...
      {
        const float score = fabsf((static_cast<float>(x) / static_cast<float>(y)) - origAR);
        if (score < bestScore)
        {
          bestScore = score;
