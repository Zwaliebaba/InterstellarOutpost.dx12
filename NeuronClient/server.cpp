    ...
    while (m_outbox.Size())
    {
        ...
        {                
			...
			if( client != NULL )
			{
				...
				// DebugTrace("Sending letter %d to %s:%d [%s]\n", seqId, client->GetIP(), client->m_port, cmd);
...
void Server::NotifyNetSyncError( int _clientId, int _sequenceId )
{
    ...
    // DebugTrace( "SYNCERROR Server: Notified client %d he is out of sync\n", _clientId );
}
...
void Server::AuthenticateClient ( int _clientId )
{
    ...
    if( authResult < 0 )
    {
        ...
        DebugTrace( "Client %d failed basic key check\n", _clientId );
        return;
    }
        ...
        if( m_clients.ValidIndex(i) )
        {
            ...
            if( s2c->m_clientId != _clientId )
            {
                ...
                if( duplicate )
                {
                    ...
                    DebugTrace( "Client %d is using a duplicate key\n", _clientId );
                    return;
                }
            }
        }
	...
	else
	{
		...
		if(maxTeams != 0 && numTeams != 0)
		{
			if( numTeams >= maxTeams)
			{
				...
				//else
				//{
				//	DebugTrace( "Teams are already full, client %d joins as spectator\n", _clientId );
				//	RegisterSpectator( _clientId );
				//}
			}
...
bool Server::HandleDisconnectedClient( Directory *_message )
{
    ...
    DebugTrace( "Re-sent disconnect message to client %s\n", fromStr );
...
void Server::Advertise()
{
    ...
    if( advertiseOnWan || advertiseOnLan )
    {
		...
		//DebugTrace("Advertising server properties\n" );
