	...
	for( int i = 0; i < m_numBlocks; i++ )
	{
		...
		if( !b.m_timedOut )
		{
			if( now > b.m_sendTime + _roundTripTime * 3 )
			{
				...
				if( numTimedOut )
				{
					DebugTrace( "FTP %s timed out block %d\n", Name(), i );					
				}
#endif
			}
		}
...
void FTPManager::SendFile( const char *_filename, const char *_data, int _size )
{
	...
	{
		DebugTrace("FTPManager[%s]: Already sending file %s\n", m_name, _filename );
	}
	else 
	{
		FTP *ftp = new FTP( _filename, _data, _size );
		m_sending.PutData( _filename, ftp );
		DebugTrace("FTPManager[%s]: Starting to send file %s (%d bytes, %d blocks)\n", m_name, _filename, _size, ftp->NumBlocks() );
	}
}
...
#ifdef FTP_MANAGER_VERBOSE_DEBUG
	if (numPackets > 0)	
		DebugTrace("FTPManager[%s]: Sent %d packets\n", m_name, numPackets);
#endif
...
void FTPManager::ReceiveLetterData( Directory *_letter )
{
	...
	if( !ftp ) 
	{
		...
		{
#ifdef FTP_MANAGER_VERBOSE_DEBUG
			DebugTrace( "FTPManager[%s]: Discarding duplicate transmission of file %s\n", m_name, filename );
#endif
			return;
		}
		...
		DebugTrace( "FTPManager[%s]: Receiving file %s (%d bytes, %d blocks)\n", m_name, filename, fileSize, ftp->NumBlocks() );
	}
...
#ifdef FTP_MANAGER_VERBOSE_DEBUG
			DebugTrace( "FTPManager[%s]: Received block %d/%d for file %s\n", m_name, blockNumber, ftp->NumBlocks(), filename );
#endif

			if (ftp->Complete())
				DebugTrace( "FTPManager[%s]: Finished receiving file %s\n", m_name, filename );
		}
	}
}
...
void FTPManager::ReceiveLetterAck( Directory *_letter )
{
	...
	if( ftp )
	{
...
#ifdef FTP_MANAGER_VERBOSE_DEBUG
		DebugTrace( "FTPManager[%s]: Received acks for file %s, blocks: ", m_name, filename );
#endif 
		...
		if( ftp->Complete() )
		{
			DebugTrace( "FTPManager[%s]: Finished sending %s\n", m_name, filename );
		...
		{
#ifdef FTP_MANAGER_VERBOSE_DEBUG
			DebugTrace( "FTPManager[%s]: Round trip time for file %s = %f\n", m_name, filename, maxRoundTripTime );
#endif 
