...
void NetworkValue::FromLatinString(const char* _buf) { FromUnicodeString(UnicodeString(_buf)); }
...
bool NetworkIntAsBool::Get() const { return m_networkInt->Get() != 0; }

void NetworkIntAsBool::Set(bool _x) { m_networkInt->Set(_x); }
...
void NetworkString::ToUnicodeString(UnicodeString& _to) { _to = UnicodeString(Get()); }

void NetworkString::FromUnicodeString(const UnicodeString& _from) { Set(_from.m_charstring); }

// NetworkUnicodeString

void NetworkUnicodeString::ToUnicodeString(UnicodeString& _to) { _to = Get(); }

void NetworkUnicodeString::FromUnicodeString(const UnicodeString& _from) { Set(_from); }
...
void NetworkChar::FromUnicodeString(const UnicodeString& _from)
{
  ...
  int value;
  if (swscanf(_from.m_unicodestring, L"%d", &value) == 1)
    Set(static_cast<unsigned char>(value));
}
...
void NetworkGameOptionInt::Set(int _value) { g_app->m_clientToServer->RequestSetGameOptionInt(m_gameOption, Bound(_value)); }
...
NetworkInt& NetworkIntArray::operator [](int _index) { return *m_array[_index]; }

// NetworkGameParam

int NetworkGameParam::Get() const { return g_app->m_multiwinia->m_params[m_index]; }

void NetworkGameParam::Set(int _value)
{
  g_app->m_clientToServer->RequestSetGameOptionArrayElementInt(GAMEOPTION_GAME_PARAM, m_index, Bound(_value));
}

// NetworkGameResearchLevel

int NetworkGameResearchLevel::Get() const { return g_app->m_globalWorld->m_research->m_researchLevel[m_index]; }

void NetworkGameResearchLevel::Set(int _value)
{
  g_app->m_clientToServer->RequestSetGameOptionArrayElementInt(GAMEOPTION_GAME_RESEARCH_LEVEL, m_index, Bound(_value));
}

// NetworkGameOptionBool

void NetworkGameOptionBool::Set(int _value) { g_app->m_clientToServer->RequestSetGameOptionInt(m_gameOption, Bound(_value)); }

// NetworkGameOptionString

void NetworkGameOptionString::Set(const char* _value) { g_app->m_clientToServer->RequestSetGameOptionString(m_gameOption, _value); }
...
void SetMapId(int _value, int _gameType)
{
  ...
  if (maps.ValidIndex(_value)) { g_app->m_clientToServer->RequestSetGameOptionString(GAMEOPTION_MAPNAME, maps[_value]->m_fileName); }
}

// NetworkGameOptionMapId

int NetworkGameOptionMapId::Get() const { return GetMapId(g_app->m_requestedMap, g_app->m_multiwinia->m_gameType); }

void NetworkGameOptionMapId::Set(int _value) { SetMapId(_value, g_app->m_multiwinia->m_gameType); }
