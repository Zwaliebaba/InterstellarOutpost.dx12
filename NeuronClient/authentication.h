...
enum
{
  AuthenticationUnknownGame = -10,
  // The game of the key is not legitimate
  AuthenticationWrongPlatform = -9,
  // Trying to use Ambrosia key on PC, or vice versa
  AuthenticationKeyDead = -8,
  // The key has been replaced with another key
  AuthenticationVersionNotFound = -7,
  // The version was not found
  AuthenticationKeyInactive = -6,
  // The key was found, but has not been made active yet
  AuthenticationVersionRevoked = -5,
  // Version is disabled
  AuthenticationKeyNotFound = -4,
  // The key could not be found in the list of known keys
  AuthenticationKeyRevoked = -3,
  // The key is no longer valid
  AuthenticationKeyBanned = -2,
  // The key has been banned due to misuse
  AuthenticationKeyInvalid = -1,
  // The key is not algorithmically correct
  AuthenticationUnknown = 0,
  // The key has not yet been tested
...
void Authentication_GetKeyHash(const char* _sourceKey, char* _destKey, int _hashToken);
...
void Authentication_RequestStatus(const char* _key, int _gameType, int _keyId = -1, const char* _ip = nullptr);
// The IP is used ONLY for logging
