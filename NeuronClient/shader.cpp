...
#define CHECK_HR_FILE(hr,name) {if(FAILED(hr)) {DebugTrace("Shader load failed: %s",name);DEBUG_ASSERT(SUCCEEDED(hr));return;}}
...
struct MemFile
{
	MemFile(const char* filename)
	{
		...
		if(!reader)
		{
			DebugTrace("MemFile load failed: %s",filename); DEBUG_ASSERT(0);
			return;
		}
...
void Shader::Bind()
{
	// DebugTrace("Shader: binding vertex shader %s\n", m_vertexFilename );
	HRESULT hr;
