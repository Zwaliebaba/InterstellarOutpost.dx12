...
#include "sound_library_2d_win32.h"
...
bool SoundEventBlueprint::TeamMatch(int _teamId)
{
  ...
  case SoundInstance::MyTeamOnly:
    return (g_app->m_globalWorld && _teamId == g_app->m_globalWorld->m_myTeamId);
...
SoundSourceBlueprint::~SoundSourceBlueprint() { m_events.EmptyAndDelete(); }
...
char* SoundSourceBlueprint::GetSoundSourceName(int _type)
{
  char* names[] = {"Laser", "Grenade", "Rocket", "AirStrikeBomb", "Spirit", "Sepulveda", "Gesture", "Ambience", "Music", "Interface",
                   "CratePowerup", "MultiwiniaInterface"};
...
DspBlueprint::~DspBlueprint() { m_params.EmptyAndDelete(); }
...
void SoundSystem::Initialise()
{
  ...
  m_PreloadQueue = new WorkQueue(); m_PreloadQueue->Add(&SoundSystem::BackgroundSoundLoad, this);
#endif
...
void SoundSystem::RestartSoundLibraryInternal()
{
...
#else
  g_soundLibrary2d = new SoundLibrary2dSDL; g_soundLibrary3d = new SoundLibrary3dSoftware();
#endif
  ...
  if (!success && hw3d)
  {
    DebugTrace("Failed to set requested sound settings\n");
    auto dialog = new MessageDialog("Error", "Failed to set requested sound settings");
  ...
  ASSERT_TEXT(success, "Couldn't start the sound library");

  m_numChannels = g_soundLibrary3d->GetNumMainChannels();

  if (g_systemInfo->m_audioInfo.m_preferredDevice != -1)
    ASSERT_TEXT(m_numChannels > 0, "Too few channels");
...
bool SoundSystem::IsInitialized() { return m_isInitialized; }
...
bool SoundSystem::SoundLibraryMainCallback(unsigned int _channel, signed short* _data, int _dataCapacity, unsigned int _numSamples,
                                           int* _silenceRemaining)
{
  ...
  if (instance && instance->m_cachedSampleHandle)
  {
    ...
    DEBUG_ASSERT(_numSamples <= _dataCapacity);
    ...
    if (numSamplesWritten < _numSamples)
    {
      ...
      if (instance->m_loopType == SoundInstance::Looped || instance->m_loopType == SoundInstance::LoopedADSR)
      {
        while (numSamplesRemaining > 0)
        {
          ...
          if (looped)
          {
            DEBUG_ASSERT((loopStart - _data) + numSamplesRemaining <= _dataCapacity);
...
#ifdef SOUND_EDITOR
  SaveBlueprints("sounds_new.txt"); bool result = AreFilesIdentical("sounds.txt", "sounds_new.txt"); if (result == false)
  {
...
void SoundSystem::ParseSoundEvent(TextReader* _in, SoundSourceBlueprint* _source, const char* _entityName)
{
  ...
  while (stricmp(fieldName, "END") != 0)
  {
    ...
    else if (stricmp(fieldName, "TEAMMATCHTYPE") == 0)
      seb->m_instance->m_teamMatchType = atoi(_in->GetNextToken());
...
void SoundSystem::ParseSampleGroup(TextReader* _in, SampleGroup* _group)
{
  while (true)
  {
    ...
    StrToLower(sample);
    auto extensionRemoved = RemoveExtension(sample);
...
{
  if (!g_app->m_soundsLoaded || g_loadingScreen->IsRendering() || NetGetCurrentThreadId() != g_app->m_mainThreadId)
    return true;
...
void SoundSystem::TriggerEntityEvent(Entity* _entity, char* _eventName)
{
  ...
  if (m_entityBlueprints.ValidIndex(_entity->m_type))
  {
    ...
    for (int i = 0; i < sourceBlueprint->m_events.Size(); ++i)
    {
      ...
      if (stricmp(seb->m_eventName, _eventName) == 0 && seb->TeamMatch(_entity->m_id.GetTeamId()))
      {
...
void SoundSystem::TriggerBuildingEvent(Building* _building, const char* _eventName)
{
  ...
  if (m_buildingBlueprints.ValidIndex(_building->m_type))
  {
    ...
    {
      SoundEventBlueprint* seb = sourceBlueprint->m_events[i];
      if (stricmp(seb->m_eventName, _eventName) == 0 && seb->TeamMatch(_building->m_id.GetTeamId()))
      {
        DEBUG_ASSERT(seb->m_instance);
        auto instance = new SoundInstance();
        instance->Copy(seb->m_instance);
...
void SoundSystem::TriggerOtherEvent(WorldObject* _other, char* _eventName, int _type)
{
  ...
  if (m_otherBlueprints.ValidIndex(_type))
  {
    ...
    {
      SoundEventBlueprint* seb = sourceBlueprint->m_events[i];
      if (stricmp(seb->m_eventName, _eventName) == 0 && seb->TeamMatch(_other ? _other->m_id.GetTeamId() : 255))
      {
        // We have a match
        DEBUG_ASSERT(seb->m_instance);
        auto instance = new SoundInstance();
...
void SoundSystem::TriggerOtherEvent(const Vector3& pos, char* _eventName, int _type, int _teamId)
{
  if (!m_channels)
    return;
  if (NotReadyForGameSoundsYet())
  ...
  if (m_otherBlueprints.ValidIndex(_type))
  {
    ...
    {
      SoundEventBlueprint* seb = sourceBlueprint->m_events[i];
      if (stricmp(seb->m_eventName, _eventName) == 0 && seb->TeamMatch(_teamId))
      {
        // We have a match
        DEBUG_ASSERT(seb->m_instance);
        auto instance = new SoundInstance();
...
static const char* g_soundSourceErrors[SoundSystem::SoundSourceNumErrors] = {nullptr, "Sound sample not mono",
