...
#ifdef _DEBUG
#define SOUNDASSERT(x, y, ...)  ASSERT(x == DS_OK || x == S_FALSE)
#else
#define SOUNDASSERT(x, y, ...)  SOUNDDEBUG(x, y, __VA_ARGS__)
#endif // _DEBUG

#define SOUNDDEBUG(x, y, ...)   DEBUG_ASSERT(x == DS_OK || x == S_FALSE)
...
{    
    ...
    ASSERT_TEXT( g_systemInfo->m_directXVersion >= 9.0f, APP_NAME " requires DirectX9 or Greater" );
...
{
    //
    // Superior to 3 because GetMaxChannels remove 2 channels and 1 channel is reserved for music

    return( m_directSound->m_caps.dwMaxHw3DStreamingBuffers > 3 );
}
...
void SoundLibrary3dDirectSound::SetChannel3DMode( int _channel, int _mode )
{
	...
	if (_mode != channel->m_3DMode)
	{
		...
		SOUNDASSERT(errCode, "Direct sound couldn't set channel 3D mode (%d)", _mode );
	}
}
...
{   
    ...
    if( _up != g_zeroVector && _front != g_zeroVector )
    {
        errCode = listener->SetOrientation( _front.x, _front.y, _front.z, 
                                            _up.x, _up.y, _up.z, DS3D_DEFERRED );

        SOUNDASSERT( errCode, "Direct sound couldn't set listener orientation, front(%f, %f, %f), top(%f, %f, %f)",
                     _front.x, _front.y, _front.z, _up.x, _up.y, _up.z );
    }
...
void SoundLibrary3dDirectSound::CommitChanges()
{
	...
	if (errCode == DSERR_BUFFERLOST)
		...
		SOUNDASSERT( errCode, "Direct sound couldn't commit deferred listener changes, errCode(%d)", errCode);
}
...
void SoundLibrary3dDirectSound::PopulateBuffer(int _channel, int _fromSample, int _numSamples, bool _isMusic)
{
    ...
    if (_isMusic && m_musicCallback)
	{
        if( buf1 )
        {
            m_musicCallback( (signed short *)buf1, size1/2, size1/2, &channel->m_silenceRemaining );
        }
        if( buf2 )
        {
            m_musicCallback( (signed short *)buf2, size2/2, size2/2, &channel->m_silenceRemaining );
        }
	}
	else if(!_isMusic && m_mainCallback)
    {
        if( buf1 )
        {
            m_mainCallback(_channel, (signed short *)buf1, size1/2, size1/2, &channel->m_silenceRemaining);
        }
        if( buf2 )
        {
            m_mainCallback(_channel, (signed short *)buf2, size2/2, size2/2, &channel->m_silenceRemaining);
        }
    }
	END_PROFILE( "FillBuf");
