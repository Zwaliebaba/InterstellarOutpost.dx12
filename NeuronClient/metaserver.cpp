...
struct GameServer
{
  ...
  GameServer()
    : m_updateTime(0.0f),
      ...
      m_receivedWan(false),
      m_receivedLan(false),
      m_data(nullptr) { m_identity[0] = '\x0'; }
};

static NetLib* s_netLib = nullptr;

static NetSocketListener* s_socketListener = nullptr;
static NetSocketSession* s_socketToWAN = nullptr;
static NetSocketSession* s_socketToLAN = nullptr;
...
static Directory* s_serverProperties = nullptr;
static NetMutex s_serverPropertiesMutex;

static Directory* s_clientProperties = nullptr;
...
bool MetaServer_SendToMetaServer(Directory* _directory) { return false; }
...
void ReceiveMetaServerData(Directory* _directory)
{
  ...
  DebugTrace("Received data from MetaServer : %s\n", thisDataType);

  bool found = false;
  MetaServerData* target = nullptr;
...
{
  if (strcmp(_directory->GetName(), NET_METASERVER_MESSAGE) != 0 || !_directory->HasData(NET_METASERVER_COMMAND, DIRECTORY_TYPE_STRING))
  {
    DebugTrace("Received bogus message, discarded (6)\n");
    delete _directory;
    return;
  }
  ...
  {
    MetaServer_UpdateServerList(_directory->GetDataString(NET_METASERVER_IP), _directory->GetDataInt(NET_METASERVER_PORT), true,
                                _directory);
  ...
  {
    DebugTrace("Received bogus message from the MetaServer, discarded (7)\n");
    delete _directory;
  }
}
...
static NetCallBackRetType MetaserverListenCallback(NetUdpPacket* udpdata)
{
  if (udpdata)
  {
    ...
    {
      DebugTrace("Received bogus message, discarded (8)\n");
      Directory test;
...
void MetaServer_Connect(const char* _metaServerIp, int _port) { MetaServer_Connect(_metaServerIp, PORT_METASERVER_LISTEN, _port); }
...
void MetaServer_Connect(const char* _metaServerIp, int _metaServerPort, int _port)
{
  ...
  {
    DebugTrace("MetaServer failed to open listener on Port %d\n", _port);
    DebugTrace("MetaServer looking for any free Port number...\n");
  ...
  {
    delete s_socketListener;
    s_socketListener = nullptr;
    DebugTrace("ERROR: Failed to connect to MetaServer\n");
    return;
  }

  NetStartThread(ListenThread);
  DebugTrace("Connection established to MetaServer, listening on Port %d\n", s_socketListener->GetPort());
...
void MetaServer_Disconnect()
{
  ...
  DebugTrace("Disconnected from MetaServer\n");
}

bool MetaServer_IsConnected() { return (s_connectedToWAN); }
...
void MetaServer_SetServerProperties(Directory* _properties)
{
  ...
  auto propertiesCopy = new Directory();
...
static void SendServerUnregisterLAN()
{
  ...
  NetLockMutex lock(s_serverPropertiesMutex);
  {
    ...
    if (s_serverProperties && s_serverProperties->HasData(NET_METASERVER_LOCALPORT) && s_socketToLAN)
    {
      int port = s_serverProperties->GetDataInt(NET_METASERVER_LOCALPORT);
      DebugTrace("SendServerUnregisterLan. Port = %d\n", port);
...
{
  DebugTrace("MetaServer : LAN registration enabled\n");
  ...
  while (s_registeringOverLAN)
  {
    ...
    if (!success)
    {
      DebugTrace("MetaServer encountered error sending data\n");
      break;
    }

    s_lanRegisterEvent.Wait(PERIOD_SERVER_REGISTER_LAN);
  }
  DebugTrace("MetaServer : LAN registration disabled\n");
...
void MetaServer_StartRegisteringOverLAN()
{
  ...
  if (!s_socketToLAN && s_socketListener)
  {
    ...
    DebugTrace("MetaServer enabling UDP broadcast for LAN discovery...");
  ...
  if (s_socketToLAN && !s_registeringOverLAN)
  {
    ...
    DebugTrace("ERROR: Failed to open the LAN broadcast socket\n");
}
...
bool MetaServer_IsRegisteringOverWAN() { return (s_connectedToWAN && s_registeringOverWAN); }

bool MetaServer_IsRegisteringOverLAN() { return (s_socketToLAN != nullptr); }
...
bool MetaServer_HasReceivedListWAN() { return !s_awaitingServerListWAN; }
...
void MetaServer_UpdateServerList(char* _ip, int _port, bool _lanOrWan, Directory* _server)
{
  ...
  for (int i = 0; i < s_serverList.Size(); ++i)
  {
    ...
    if (authKey && thisServer->m_data && thisServer->m_data->HasData(NET_METASERVER_AUTHKEY))
    {
      const char* thisAuthKey = thisServer->m_data->GetDataString(NET_METASERVER_AUTHKEY);

      authKeyMatch = (authKey && thisAuthKey && strcmp(thisAuthKey, authKey) == 0);
    }

    if (strcmp(thisServer->m_identity, serverIdentity) == 0 || authKeyMatch)
    {
      ...
      if (_lanOrWan == 0)
        thisServer->m_data->CreateData(NET_METASERVER_LANGAME, true);
      s_serverListMutex.Unlock();
      return;
    }
  }
  ...
  if (_lanOrWan == 0)
    server->m_data->CreateData(NET_METASERVER_LANGAME, true);
...
void MetaServer_DelistServer(const char* _ip, int _port, Directory* _server)
{
  ...
  for (int i = 0; i < s_serverList.Size(); ++i)
  {
    ...
    {
      if (authKey && thisServer->m_data->HasData(NET_METASERVER_AUTHKEY))
      {
        const char* thisAuthKey = thisServer->m_data->GetDataString(NET_METASERVER_AUTHKEY);
...
void MetaServer_SetServerTTL(int _seconds) { s_serverTTL = _seconds; }
...
Directory* MetaServer_GetServer(char* _ip, int _port, bool _authRequired)
{
  ...
  {
    GameServer* server = s_serverList[i];
    bool authOk = !_authRequired || server->m_authResult == AuthenticationAccepted;
...
LList<Directory*>* MetaServer_GetServerList(bool _authRequired, bool _wanServers, bool _lanServers, int _maxListSize)
{
  ...
  auto copyServerList = new LList<Directory*>();
  ...
  {
    GameServer* server = s_serverList[i];
    if (!_authRequired || server->m_authResult == AuthenticationAccepted)
    {
      bool lanMatch = (server->m_receivedLan && _lanServers);
      bool wanMatch = (server->m_receivedWan && _wanServers);
...
Directory* MetaServer_RequestData(const char* _dataType, const char* _lang)
{
  ...
  for (int i = 0; i < s_metaServerData.Size(); ++i)
  {
    ...
    if (stricmp(data->m_dataType, _dataType) == 0)
    {
      ...
      if (data->m_timeOfExpire > 0.0f && timeNow >= data->m_timeOfExpire)
      {
        ...
        if (data->m_data)
        {
          ...
          data->m_data->CreateData(NET_METASERVER_DATAEXPIRED, static_cast<int>(currentExpire + timeToWait));
        }
        else
          data->m_retries++;
      }
  ...
  if (sendRequest && s_connectedToWAN)
  {
    ...
    DebugTrace("Requesting data from MetaServer : %s\n", _dataType);
  }
...
int MetaServer_GetServerTTL()
{
  ...
  if (serverTTLDir && serverTTLDir->HasData(NET_METASERVER_DATA_SERVERTTL, DIRECTORY_TYPE_STRING))
...
void MetaServer_DelistServers(const char* _authKey)
{
  s_serverListMutex.Lock();
