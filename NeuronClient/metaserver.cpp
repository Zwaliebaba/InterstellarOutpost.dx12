...
{
#ifdef WAN_PLAY_ENABLED
    DebugTrace( "MetaServer : WAN registration enabled\n" );
    ...
    DebugTrace( "MetaServer : WAN registration disabled\n" );
#else
    #ifndef WAN_PLAY_IF_NOT_ENABLED_NO_MESSAGE
        DebugTrace( "MetaServer WARNING : WAN Play is disabled\n" );
    #endif
#endif
...
static NetCallBackRetType ServerRegisterLANThread(void *ignored)
{
	...
	SendServerUnregisterLAN();
        ...
        DebugTrace( "MetaServer WARNING : LAN Play is disabled\n" );
    #endif
#endif
...
void MetaServer_RequestServerListWAN( int _gameType )
{
    ...
    if( !s_clientProperties )
    {
        DebugTrace( "MetaServer failed to RequestServerList over WAN : client properties not set\n" );
        return;
    }
    ...
    {
        DebugTrace( "MetaServer refused to RequestServerList over WAN : authentication key invalid\n" );
        return;
    }
...
void MetaServer_RequestServerDetails( char *_ip, int _port )
{
    ...
    if( !s_clientProperties )
    {
        DebugTrace( "MetaServer failed to RequestServerDetails over WAN : client properties not set\n" );
        return;
    }
