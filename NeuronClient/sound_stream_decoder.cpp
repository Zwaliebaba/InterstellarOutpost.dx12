	...
	m_fileType(TypeUnknown)
{
	...
	{
		ASSERT_TEXT(0, "Unknown sound file format %s", m_in->m_filename.c_str());
	}
}
...
unsigned int SoundStreamDecoder::Read(signed short *_data, unsigned int _numSamples)
{
	...
	{
	case TypeUnknown:
		ASSERT_TEXT(0, "Unknown format of sound file %s", m_in->m_filename.c_str());
	case TypeWav:
