...
void ClientEngine::Startup(const wchar_t* _gameName, Windows::Foundation::Size _size, HINSTANCE _hInstance, int nCmdShow)
{
  ...
  if constexpr (SHOW_BORDER)
  {
    ...
    windowWidthAdd = (GetSystemMetrics(SM_CXSIZEFRAME) + paddedBorder) * 2.0f;
    windowHeightAdd = GetSystemMetrics(SM_CYCAPTION) + (GetSystemMetrics(SM_CYSIZEFRAME) + paddedBorder) * 2.0f;
  }
  else
  {
    style = WS_POPUP;
