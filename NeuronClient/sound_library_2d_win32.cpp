...
{
  auto soundLibrary = static_cast<SoundLibrary2dWin32*>(g_soundLibrary2d);

  if (_msg != WOM_DONE)
    return;
  if (!s_device)
    return;
  if (!soundLibrary || !soundLibrary->m_callback)
    return;
...
void SoundLibrary2dWin32::SetCallback(void (*_callback)(StereoSample*, unsigned int)) { m_callback = _callback; }
...
void SoundLibrary2dWin32::StartRecordToFile(const char* _filename)
{
  m_wavOutput = fopen(_filename, "wb");
  ASSERT_TEXT(m_wavOutput != NULL, "Couldn't create wave outout file {}", _filename);
}
