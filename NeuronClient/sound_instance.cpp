...
void DspHandle::Advance()
{
  ...
  for (int i = 0; i < MAX_PARAMS; ++i)
  {
    ...
    {
    case 0:
      params[i] = m_params[i].GetOutput();
      break;
    case 1:
      *((long*)&params[i]) = static_cast<long>(m_params[i].GetOutput());
      break;
    default: ASSERT_TEXT(false, "Unknown datatype");
    }
  }
...
bool SoundInstanceId::operator ==(const SoundInstanceId& w) const { return (m_index == w.m_index && m_uniqueId == w.m_uniqueId); }

const SoundInstanceId& SoundInstanceId::operator =(const SoundInstanceId& w)
{
  m_index = w.m_index;
...
void SoundInstance::SetSoundName(const char* _name)
{
  if (_name)
...
void SoundInstance::SetEventName(const char* _entityName, const char* _eventName)
{
  ...
  m_eventName = static_cast<char*>(malloc(strlen(_entityName) + strlen(_eventName) + 2));
...
{
  static char* types[] = {"Type2D", "Type3DStationary", "Type3DAttachedToObject", "TypeInEditor"};
...
{
  static char* types[] = {"Polyphonic", "MonophonicRandom", "MonophonicNearest"};
...
{
  static char* types[] = {"PlayOnce", "Loop", "LoopADSR"};
...
{
  static char* types[] = {"Sample", "SampleGroupRandom"};
...
{
  static char* types[] = {"AllTeams", "MyTeamOnly", "EnemyTeamOnly"};
...
{
  static char* types[] = {"Attack", "Decay", "Sustain", "Release"};
...
void SoundInstance::Copy(SoundInstance* _copyMe)
{
  ...
  m_eventName = _strdup(_copyMe->m_eventName);
...
float GetFrequencyModifier(int positionType)
{
  if (g_app->Multiplayer())
  {
    ...
    float result = sqrt(max(s_gametimer, 0.01f));
    return result;
  }
  return 1.0f;
}
...
bool SoundInstance::StartPlaying(int _channelIndex)
{
  ...
  Update3DPosition();
  //DebugTrace("Channel %d: SetChannelPosition Pos = (%f, %f, %f) Vel = (%f, %f, %f)\n", 
  //	m_channelIndex, 
...
bool SoundInstance::Advance()
{
  ...
  {
    DebugTrace("Bogus sound Frequency calculated. Details\n");
    DebugTrace(" m_cachedSampleHandle->m_cachedSample->m_freq = %f\n", m_cachedSampleHandle->m_cachedSample->m_freq);
    DebugTrace(" m_freq.GetOutput() = %f\n", m_freq.GetOutput());
    //DebugTrace(" frequency1 = %f\n", frequency1 );
    //DebugTrace(" gametimer1 = %f\n", gametimer1 );
    //DebugTrace(" s_gametimer = %f\n", s_gametimer );
    DebugTrace(" g_gameTimer.GetGameSpeed() = %f\n", g_gameTimer.GetGameSpeed());
    //DebugTrace(" sqrt( max( s_gametimer, 0.01f ) ) = %f\n", sqrt( max( s_gametimer, 0.01f ) ) );
    DebugTrace(" frequency = %f\n", frequency);
  }
  else
    g_soundLibrary3d->SetChannelFrequency(m_channelIndex, frequency);
...
bool SoundInstance::Update3DPosition()
{
  ...
  case Type3DStationary: { return false; }
  ...
  case Type3DAttachedToObject:
  {
    ...
    if (obj->m_id.GetUnitId() == UNIT_BUILDINGS)
      m_pos = static_cast<Building*>(obj)->m_centrePos;
  ...
  case TypeInEditor:
  {
    Vector3 relativePos(g_app->m_soundSystem->m_editorPos.x, 0.0f, g_app->m_soundSystem->m_editorPos.z);
    m_pos = relativePos;
    m_vel.Zero();
...
void SoundInstance::ForceParameter(SoundParameter& _param, float value)
{
  ...
  case SoundParameter::LinkedToHeightAboveGround:
  {
    ...
    if (g_app->m_location)
      landHeight = g_app->m_location->m_landscape.m_heightMap->GetValue(m_pos.x, m_pos.z);
  ...
  {
    if (g_app->m_camera) { m_pos = g_app->m_camera->GetPos() + g_app->m_camera->GetFront() * value; }
    break;
  }
  }
}
...
bool SoundInstance::UpdateParameter(SoundParameter& _param)
{
  ...
  if (_param.m_type == SoundParameter::TypeLinked)
  {
    ...
    case SoundParameter::LinkedToHeightAboveGround:
    {
      float landHeight = 0.0f;
      if (g_app->m_location)
        landHeight = g_app->m_location->m_landscape.m_heightMap->GetValue(pos.x, pos.z);
...
int SoundInstance::GetChannelIndex()
{
  ...
  if (m_channelIndex >= 0 && m_channelIndex < g_app->m_soundSystem->m_numChannels && g_app->m_soundSystem->m_channels[m_channelIndex] ==
    m_id)
...
WorldObject* SoundInstance::GetAttachedObject()
{
  ...
  if (g_app->m_locationId != -1)
  {
    ...
    if (!g_app->m_location->GetWorldObject(m_objId))
...
char* SoundInstance::GetDescriptor()
{
  ...
  const char* inEditor = m_positionType == TypeInEditor ? " editor" : "       ";
