...
{
  return (EclGetWindows()->Size() == 0 && !g_app->m_taskManagerInterface->IsVisible());
}

bool disableInputZ() { return EclGetWindows()->Size() != 0 || ChatInputWindow::IsChatVisible(); }

void TargetCursor::Advance()
{
  InputDetails details;
  if ((g_inputManager->controlEvent(ControlTargetMove, details) || (secondaryInputEnabled() && g_inputManager->
    controlEvent(ControlTargetMoveSecondary, details))) && INPUT_TYPE_2D == details.type)
  {
  ...
  if (!disableInputZ() && g_inputManager->controlEvent(ControlTargetMoveZ, details) && INPUT_TYPE_1D == details.type)
  {
