...
#endif
{
    ...
    ASSERT_TEXT( 0 < m_numChannels && m_numChannels <= 8 &&
...
CachedSample::~CachedSample()
{
	if( m_lockedCount > 0 )
	{
		DebugTrace( "Warning: Deleting locked cached sample %s\n", m_sampleName.c_str() );
	}
...
void CachedSample::Unlock()
{
	if( m_lockedCount <= 0 )
	{
		DebugTrace("Warning: Attempt to unlock cached sample %s more times than it has been locked\n", m_sampleName.c_str() );
		return;
	}
...
void CachedSample::Preload(bool lowcpu)
{
	...
	{
#ifdef VERBOSE_DEBUG
		DebugTrace("Preloading sound: %s, %s\n", m_sampleName.c_str(), m_name);
#endif
	...
	{
		DebugTrace("Skipping long sound: %s, %s\n", m_sampleName.c_str(), m_name);	
	}
...
void CachedSample::Read(signed short *_data, unsigned int _startSample, unsigned int _numSamples)
{
	...
	ASSERT_TEXT(m_numChannels == 1, "%s is not mono audio\n", m_name);
	if(_data)
