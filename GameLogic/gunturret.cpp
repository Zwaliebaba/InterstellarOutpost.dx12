    ...
    m_fireTimer(0.0)
{
  ...
  constexpr char barrelMountName[] = "MarkerBarrel";
  m_barrelMount = m_turret->m_rootFragment->LookupMarker(barrelMountName);
  ASSERT_TEXT(m_barrelMount, "GunTurret: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", barrelMountName,
              m_turret->m_name);

  constexpr char scoreMarkerName[] = "MarkerKills";
  m_scoreMarker = m_turret->m_rootFragment->LookupMarker(scoreMarkerName);
  ASSERT_TEXT(m_scoreMarker, "GunTurret: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", scoreMarkerName,
              m_turret->m_name);
  ...
  for (int i = 0; i < GUNTURRET_NUMBARRELS; ++i)
  {
    ...
    ASSERT_TEXT(m_barrelEnd[i], "GunTurret: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", name, m_barrel->m_name);
  ...
  if (g_app->Multiplayer())
  {
    ...
    for (int i = 0; i < MULTIWINIA_NUM_TEAMCOLOURS + 2; ++i)
    {
      for (int k = 0; k < 3; ++k)
      {
        ...
        if (k > 0 && i < MULTIWINIA_NUM_TEAMCOLOURS)
          colour = g_app->m_multiwinia->GetGroupColour(k - 1, i);
        if (!shape)
        {
...
void GunTurret::Initialise(Building* _template)
{
  ...
  m_state = static_cast<GunTurret*>(_template)->m_state;
  m_targetPos = static_cast<GunTurret*>(_template)->m_targetPos;
  m_targetRadius = static_cast<GunTurret*>(_template)->m_targetRadius;
  m_targetForce = static_cast<GunTurret*>(_template)->m_targetForce;
...
bool GunTurret::ClearLOS(const Vector3& _pos)
{
  double stepSize = 30.0;
  double dist = (m_pos - _pos).Mag();
  int numSteps = dist / stepSize;
  Vector3 diff = (_pos - m_pos) / numSteps;
  Vector3 pos = _pos;
  ...
  for (int i = 0; i < numSteps; ++i)
  {
    ...
    for (int j = 0; j < buildingIds->Size(); ++j)
    {
      ...
      if (building)
      {
        ...
        if (building->DoesRayHit(m_pos, dir, dist))
          return false;
      }
    }
    pos += diff;
...
bool GunTurret::SearchForTargets()
{
  ...
  if (m_id.GetTeamId() != 255)
  {
    if (m_state == GunTurretTypeMortar)
      m_targetId = g_app->m_location->m_entityGrid->GetBestEnemy(m_pos.x + m_targetPos.x, m_pos.z + m_targetPos.z, 0.0, m_targetRadius,
                                                                 m_id.GetTeamId());
    else
    {
      /* m_targetId = g_app->m_location->m_entityGrid->GetBestEnemy( m_pos.x, m_pos.z, 
                                                                   ...
                                                                   m_id.GetTeamId() );   */
      int numFound;
      g_app->m_location->m_entityGrid->GetEnemies(s_neighbours, m_pos.x, m_pos.z, GetMaxRange(), &numFound, m_id.GetTeamId());
      ...
      for (int i = 0; i < numFound; ++i)
      {
        ...
        if (dist >= GetMinRange() && dist < bestDist)
        {
          if (ClearLOS(e->m_pos))
          {
            bestDist = dist;
...
void GunTurret::SearchForRandomPos()
{
  if (m_target == g_zeroVector)
  {
    ...
    m_target = m_pos + Vector3(cos(angle) * distance, height, sin(angle) * distance);
  }
  else
  {
...
void GunTurret::RecalculateOwnership()
{
  ...
  for (int i = 0; i < NUM_TEAMS; ++i)
  {
    ...
    if (winningTeam != -1 && totalOccupiedPorts != teamCount[winningTeam])
    {
      //winningTeam = -1;
      multipleTeams = true;
    }
  ...
  {
    if (m_state == GunTurretTypeSubversion && !g_app->m_location->m_teams[winningTeam]->IsFuturewinianTeam())
    {
      Damage(-1000);
      return;
    }
    if (winningTeam == g_app->m_location->GetFuturewinianTeamId() && m_state != GunTurretTypeSubversion)
    {
      Damage(-1000);
      return;
    }
...
void GunTurret::PrimaryFire()
{
  ...
  for (int i = 0; i < 2; ++i)
  {
    ...
    {
      auto d = static_cast<Darwinian*>(g_app->m_location->GetEntitySafe(GetPortOccupant(m_nextBarrel), Entity::TypeDarwinian));
      ...
      int type = m_state;
      if (type == GunTurretTypeStandard && d && d->m_subversionTaskId != -1)
        type = GunTurretTypeSubversion;

      switch (type)
      {
      case GunTurretTypeStandard:
        g_app->m_location->FireTurretShell(shellPos - shellVel * SERVER_ADVANCE_PERIOD * 0.5f, shellVel, m_id.GetTeamId(),
                                           m_id.GetUniqueId());
        break;
      case GunTurretTypeRocket:
        g_app->m_location->FireRocket(shellPos, rocketTarget, m_id.GetTeamId(), 1.0f, m_id.GetUniqueId());
        break;
      case GunTurretTypeMortar:
        g_app->m_location->ThrowWeapon(shellPos, mortarTarget, EffectThrowableGrenade, m_id.GetTeamId(), force, 1000.0f, true);
        break;
      case GunTurretTypeFlame:
        g_app->m_location->LaunchFireball(shellPos, fireVel, 1.0f, m_id.GetUniqueId());
        break;
      case GunTurretTypeSubversion:
        g_app->m_location->FireLaser(shellPos, shellVel / 4.0f, m_id.GetTeamId(), true);
      }
      ...
      if (m_shellEject[m_nextBarrel])
      {
        ...
        if (type == GunTurretTypeStandard)
        {
          auto shell = new TurretEmptyShell(pos, -shellMat.r, vel);
    ...
    if (m_state == GunTurretTypeRocket)
      break;
  }
  ...
  if (fired)
  {
    ...
    case GunTurretTypeStandard:
      ...
      break;
    case GunTurretTypeFlame:
      g_app->m_soundSystem->TriggerBuildingEvent(this, "FireFlame");
      break;
    case GunTurretTypeMortar:
      g_app->m_soundSystem->TriggerBuildingEvent(this, "FireGrenade");
      break;
    case GunTurretTypeRocket:
      g_app->m_soundSystem->TriggerBuildingEvent(this, "FireRocket");
      break;
    }
...
double GunTurret::GetMinRange()
{
  ...
  {
    if (g_app->Multiplayer())
      return GUNTURRET_MINRANGE * 0.5;
    return GUNTURRET_MINRANGE;
  }

  case GunTurretTypeRocket:
    return ROCKETTURRET_MINRANGE;
  case GunTurretTypeFlame:
    return FLAMETURRET_MINRANGE;
  case GunTurretTypeSubversion:
    return SUBVERSIONTURRET_MINRANGE;
  }
...
double GunTurret::GetMaxRange()
{
  ...
  {
    if (g_app->Multiplayer())
      return GUNTURRET_MAXRANGE * 0.75;
    return GUNTURRET_MAXRANGE;
  }

  case GunTurretTypeRocket:
    return ROCKETTURRET_MAXRANGE;
  case GunTurretTypeFlame:
    return FLAMETURRET_MAXRANGE;
  case GunTurretTypeSubversion:
    return SUBVERSIONTURRET_MAXRANGE;
  }
...
bool GunTurret::Advance()
{
  ...
  if (g_gameTimer.GetGameTime() < m_hadTargetTimer + 1.0f && !underPlayerControl)
    primaryFire = true;
  ...
  else
  {
    ...
    if (m_retargetTimer <= 0.0)
    {
      ...
      if (g_app->Multiplayer())
        m_retargetTimer = GUNTURRET_MULTIWINIA_RETARGETTIMER;
      if (m_state == GunTurretTypeMortar)
        m_target = m_pos + m_targetPos;
    }
    ...
    if (m_targetId.IsValid())
    {
      ...
      else
      {
        ...
        m_target += Vector3(sin(theTime * 3) * 20, abs(sin(theTime * 2) * 10), cos(theTime * 3) * 20);

        Vector3 targetFront = (m_target - m_pos);
        targetFront.HorizontalAndNormalise();
        double angle = acos(targetFront * m_turretFront);

        primaryFire = (angle < 0.3);
        if (m_friendlyFireWarning)
          primaryFire = false;
      }
    }
  ...
  if (m_id.GetTeamId() >= 0 && m_id.GetTeamId() < NUM_TEAMS)
  {
    ...
    if (m_fireTimer > 0.0)
      m_fireTimer -= SERVER_ADVANCE_PERIOD;
    if (primaryFire)
      PrimaryFire();
  }
    ...
    if (timeSinceLastFire > 0.2)
    {
      ...
      if (m_temperature > 0.0)
      {
        for (int i = 0; i < 4; ++i)
        {
          ...
          if (m_state == GunTurretTypeFlame)
            smokePos += barrelMat.f * 4;

          int numParticles = syncfrand(static_cast<int>(1.0 + m_temperature / 10.0));
...
void GunTurret::Render(double _predictionTime)
{
  ...
  if (m_id.GetTeamId() >= 0 && m_id.GetTeamId() < NUM_TEAMS)
  {
    ...
    char caption[256];
    if (m_friendlyFireWarning && fmodf(g_gameTime, 0.6f) < 0.3f)
      sprintf(caption, "FF!");
    else
      sprintf(caption, "%d", kills);
...
void GunTurret::RenderPorts()
{
  ...
  for (int i = 0; i < GetNumPorts(); ++i)
  {
    ...
    if (!m_statusMarkers[i])
    {
      char name[64];
      sprintf(name, "MarkerStatus0%d", i + 1);
      ASSERT_TEXT(m_statusMarkers[i], "GunTurret: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", name, m_shape->m_name);
    }
...
bool GunTurret::DoesRayHit(const Vector3& _rayStart, const Vector3& _rayDir, double _rayLen, Vector3* _pos, Vector3* norm)
{
  //
  ...
  if (m_turret->RayHit(&ray, turretMat, true))
    return true;
  ...
  barrelFront.Normalise();
  Matrix34 barrelMat(barrelFront, predictedBarrelUp, barrelPos);
  if (m_barrel->RayHit(&ray, barrelMat, true))
    return true;
...
char* GunTurret::GetTurretTypeName(int _type)
{
  ...
  case GunTurretTypeRocket:
    return "buildingname_rocketturret";
  case GunTurretTypeMortar:
    return "buildingname_grenadeturret";
  case GunTurretTypeFlame:
    return "buildingname_flameturret";
  case GunTurretTypeSubversion:
    return "buildingname_subversionturret";
  }
...
char* GunTurret::GetTurretModelName(int _type)
{
  if (g_app->Multiplayer())
  {
    ...
    {
    case GunTurretTypeFlame:
      return "cannon_flame.shp";
      break;
    case GunTurretTypeRocket:
      return "cannon_rocket.shp";
      break;
    case GunTurretTypeSubversion:
      return "battlecannonbarrel.shp";
      break;
    default:
      return "cannon_bullets.shp";
      break;
    }
  }
  return "battlecannonbarrel.shp";
