...
public:
    ...
    void MouseUp()
    {
        ...
        ASSERT_TEXT( resolution != NULL, "Cannot get resolution id(%d)",
                          parent->m_resId );
        ...
        ASSERT_TEXT( parent->m_resId != -1, "Cannot get resolution id for width(%d) and height(%d)",
...
:   GameOptionsWindow( "dialog_screenoptions" )
{
    ...
    ASSERT_TEXT( m_resId != -1, "Cannot get resolution id for width(%d) and height(%d)",
                      g_prefsManager->GetInt(SCREEN_WIDTH_PREFS_NAME),
                      g_prefsManager->GetInt(SCREEN_HEIGHT_PREFS_NAME) );
