...
void MultiwiniaZone::Initialise(Building* _template)
{
  ...
  m_size = static_cast<MultiwiniaZone*>(_template)->m_size;
  m_centrePos = m_pos + Vector3(0, m_radius / 2, 0);

  auto zone = static_cast<MultiwiniaZone*>(_template);
...
void MultiwiniaZone::Advance_KingOfTheHill()
{
  ...
  if (GetNetworkTime() > m_recountTimer)
  {
    ...
    for (int i = 0; i < numFound; ++i)
    {
      ...
      if (entity && entity->m_type == Entity::TypeDarwinian && !entity->m_dead && !static_cast<Darwinian*>(entity)->IsOnFire() &&
        static_cast<Darwinian*>(entity)->m_state != Darwinian::StateInsideArmour)
      {
...
void MultiwiniaZone::Advance_Blitzkrieg()
{
  ...
  if (GetNetworkTime() > m_recountTimer)
  {
    ...
    if (newTeamId != 255)
    {
      double fractionOwned = static_cast<double>(highestCount) / static_cast<double>(m_totalCount);
    ...
    if (m_blitzkriegOwnership < 0.0)
      m_blitzkriegOwnership = 0.0;
    if (m_blitzkriegOwnership > 100.0)
      m_blitzkriegOwnership = 100.0;

    if (m_blitzkriegOwnership > oldBlitzkriegOwnership)
      m_blitzkriegUpOrDown = +1;
    else if (m_blitzkriegOwnership < oldBlitzkriegOwnership)
      m_blitzkriegUpOrDown = -1;
    else
      m_blitzkriegUpOrDown = 0;

    if (m_blitzkriegUnderAttackMessageTimer > 0.0)
      m_blitzkriegUnderAttackMessageTimer -= SERVER_ADVANCE_PERIOD;
    int baseTeam = GetBaseTeamId();
    if (baseTeam != -1 && m_blitzkriegUnderAttackMessageTimer <= 0.0 && !g_app->m_multiwinia->m_eliminated[baseTeam])
    {
      if (baseTeam == m_id.GetTeamId() && m_id.GetTeamId() == g_app->m_globalWorld->m_myTeamId && g_app->m_location->m_entityGrid->
        GetNumEnemies(m_pos.x, m_pos.z, m_size, m_id.GetTeamId()) > 0)
      {
    ...
    auto t = static_cast<AITarget*>(g_app->m_location->GetBuilding(AI::FindNearestTarget(m_pos)));
    if (t && t->m_type == TypeAITarget)
    {
      for (int i = 0; i < NUM_TEAMS; ++i)
      {
        double p = 1.0;
        if (g_app->m_multiwinia->m_aiType == Multiwinia::AITypeHard || g_app->m_multiwinia->m_aiType == Multiwinia::AITypeStandard)
          p = GetBlitzkriegPriority(m_id.GetUniqueId(), i);

        t->m_priority[i] = p;
        if (p > 0.0)
          t->m_aiObjectiveTarget[i] = true;
        else
          t->m_aiObjectiveTarget[i] = false;
      }
    }
...
{
  for (int i = 0; i < NUM_TEAMS; ++i)
    if (s_blitzkriegBaseZone[i] == m_id.GetUniqueId())
      return i;
  return -1;
}
...
double MultiwiniaZone::GetBlitzkriegPriority(int _zoneId, int _teamId)
{
  ...
  double priority = 0.0;
  auto zone = static_cast<MultiwiniaZone*>(g_app->m_location->GetBuilding(_zoneId));
  ...
  if (zone)
  {
    ...
    case BLITZKRIEGSCORE_UNOCCUPIED:
    {
      ...
      {
        if (zone->m_id.GetTeamId() != 255 && !g_app->m_multiwinia->m_eliminated[zone->m_id.GetTeamId()])
          priority = 1.0;
      }
      break;
    }
    ...
    case BLITZKRIEGSCORE_UNDERTHREAT:
    {
      for (int i = 0; i < zone->m_blitzkriegLinks.Size(); ++i)
      {
        if (zone->m_blitzkriegLinks[i] == s_blitzkriegBaseZone[_teamId] && zone->m_id.GetTeamId() != _teamId)
          priority = 1.0;
      }
      break;
    }
  ...
  if (!zone->IsBlitzkriegBaseZone() && zone->m_id.GetTeamId() == _teamId)
    priority = 0.0;
    ...
    if (GetNetworkTime() > m_recountTimer)
    {
      ...
      for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
      {
        if (g_app->m_location->m_buildings.ValidIndex(i))
        {
          ...
          if (building && building->m_type == TypeStatue && building->m_id.GetTeamId() == m_id.GetTeamId())
          {
            double distance = (building->m_pos - m_pos).Mag();
            if (distance < 30)
            {
              static_cast<Statue*>(building)->Destroy();
...
bool MultiwiniaZone::Advance()
{
  ...
  if (m_aiTarget)
  {
    for (int i = 0; i < NUM_TEAMS; ++i)
    {
      ...
      if (m_id.GetTeamId() != i && m_id.GetTeamId() != -1)
      {
  ...
  case Multiwinia::GameTypeKingOfTheHill:
    Advance_KingOfTheHill();
    break;
  case Multiwinia::GameTypeCaptureTheStatue:
    Advance_CaptureTheStatue();
    break;
...
void MultiwiniaZone::RenderZoneEdge(double startAngle, double totalAngle, RGBAColour colour, bool animated)
{
  ...
  for (int i = 0; i <= numSteps; ++i)
  {
    ...
    double xDiff = m_size * sin(angle);
    double zDiff = m_size * cos(angle);
  ...
  if (animated)
  {
    ...
    for (int i = 0; i <= numSteps; ++i)
    {
      ...
      pos.y = g_app->m_location->m_landscape.m_heightMap->GetValue(pos.x, pos.z);
      pos.y -= 20;
      glTexCoord2f(i + timeFactor, 0);
      glVertex3dv(pos.GetData());
      pos.y += 100;
      glTexCoord2f(i + timeFactor, 1);
      glVertex3dv(pos.GetData());
      angle += totalAngle / static_cast<double>(numSteps);
    }
    glEnd();
...
void MultiwiniaZone::RenderBlitzkrieg()
{
  ...
  double camDist = (g_app->m_camera->GetPos() - m_pos).Mag();
  if (camDist > 100)
    size *= 1.0 + scaleFactor * (camDist - 100) / 1500;
  ...
  if (teamId == 255)
    teamId = m_blitzkriegCaptureTeam;
...
bool MultiwiniaZone::DoesShapeHit(Shape* _shape, Matrix34 _transform) { return false; }
...
{
  for (int i = 0; i < NUM_TEAMS; ++i)
    if (m_id.GetUniqueId() == s_blitzkriegBaseZone[i])
      return true;
...
bool MultiwiniaZone::IsBlitzkriegZoneLocked()
{
  ...
  for (int i = 0; i < m_blitzkriegLinks.Size(); ++i)
  {
    auto link = static_cast<MultiwiniaZone*>(g_app->m_location->GetBuilding(m_blitzkriegLinks[i]));
    if (link && link->m_type == TypeMultiwiniaZone)
    {
      if (link->m_id.GetTeamId() != 255 && link->m_id.GetTeamId() != m_id.GetTeamId())
      {
        locked = false;
        break;
      }
...
int MultiwiniaZone::GetNumZones(int _teamId)
{
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    if (g_app->m_location->m_buildings.ValidIndex(i))
    {
      ...
      if (b->m_type == TypeMultiwiniaZone && b->m_id.GetTeamId() == _teamId)
        num++;
    }
  }
...
void MultiwiniaZone::SetBuildingLink(int _buildingId)
{
  ...
  for (int i = 0; i < m_blitzkriegLinks.Size(); ++i)
    if (m_blitzkriegLinks[i] == _buildingId)
      return;
