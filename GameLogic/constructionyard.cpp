    ...
    m_alpha(0.0)
{
    ...
    for( int i = 0; i < YARD_NUMPRIMITIVES; ++i )
    {
        ...
        ASSERT_TEXT( m_primitives[i], "ConstructionYard: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", name, m_shape->m_name );
    }
    ...
    for( int i = 0; i < YARD_NUMRUNGSPIKES; ++i )
    {
        ...
        ASSERT_TEXT( m_rungSpikes[i], "ConstructionYard: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", name, m_rung->m_name );
    }
}
...
Matrix34 ConstructionYard::GetRungMatrix1()
{
    if( m_numSurges > 0 )
    {
        double rungHeight = 55.0 + sin(g_gameTime) * 10.0 * m_fractionPopulated;
        Vector3 rungPos = m_pos + Vector3(0,rungHeight,0);
        Vector3 front = m_front;
        front.RotateAroundY( cos(g_gameTime * 0.5) * 0.5 * m_fractionPopulated );
...
Matrix34 ConstructionYard::GetRungMatrix2()
{
    if( m_numSurges > 0 )
    {
        double rungHeight = 110.0 + sin(g_gameTime * 0.8) * 15.0 * m_fractionPopulated;
        Vector3 rungPos = m_pos + Vector3(0,rungHeight,0);
        Vector3 front = m_front;
        front.RotateAroundY( cos(g_gameTime * 0.4) * 0.6 * m_fractionPopulated );
...
void ConstructionYard::Render( double _predictionTime )
{
    ...
    for( int i = 0; i < m_numPrimitives; ++i )
    {
        ...
        prim.pos.y += sin(g_gameTime+i) * 5.0;
...
void ConstructionYard::RenderAlphas( double _predictionTime )
{
    ...
    {        
        ...
        double size = 30.0 * sin(timeIndex+i*13);
        size = max( size, 5.0 );
    ...
    if( m_timer > 0.0 )
    {
        ...
        for( int r = 0; r < 2; ++r )
        {
            ...
            for( int i = 0; i < YARD_NUMRUNGSPIKES; ++i )
            {
                ...
                {
                    double size = sin(timeIndex+r+i) * j * 5.0;
                    glColor4f( 0.6, 0.2, 0.1, m_alpha);
...
:   Building()
{
    ...
    for( int i = 0; i < DISPLAYSCREEN_NUMRAYS; ++i )
    {
        ...
        ASSERT_TEXT( m_rays[i], "DisplayScreen: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", name, m_shape->m_name );
    }
}
