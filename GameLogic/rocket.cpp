...
Vector3 FuelBuilding::GetFuelPosition()
{
  if (!m_fuelMarker)
  {
    constexpr char fuelMarkerName[] = "MarkerFuel";
    m_fuelMarker = m_shape->m_rootFragment->LookupMarker(fuelMarkerName);
    ASSERT_TEXT(m_fuelMarker, "FuelBuilding: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", fuelMarkerName,
                m_shape->m_name);
  }
...
void FuelBuilding::ProvideFuel(double _level)
{
  ...
  if (_level < m_currentLevel)
    factor2 *= 3.0; // Decrease faster than increase
  ...
  if (m_currentLevel > 0.0f)
  {
    ...
    if (fuelBuilding)
    {
      ...
      for (int i = 0; i < maxLoops; ++i)
      {
        glBegin(GL_QUADS);
        glTexCoord2f(tx, 0);
        glVertex3dv((startPos - rightAngle).GetData());
        glTexCoord2f(tx, 1);
        glVertex3dv((startPos + rightAngle).GetData());
        glTexCoord2f(tx + tw, 1);
        glVertex3dv((endPos + rightAngle).GetData());
    ...
    m_surges(0.0f)
{
  ...
  constexpr char pumpTipName[] = "MarkerTip";
  m_pumpTip = m_pump->m_rootFragment->LookupMarker(pumpTipName);
  ASSERT_TEXT(m_pumpTip, "FuelGenerator: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", pumpTipName, m_pump->m_name);
}
...
Vector3 FuelGenerator::GetPumpPos()
{
  ...
  pumpPos.y += abs(cos(m_pumpMovement)) * pumpHeight;
...
void FuelGenerator::GetObjectiveCounter(UnicodeString& _dest)
{
  ...
  swprintf(buffer, sizeof(buffer) / sizeof(wchar_t), L"%ls %d%%", LANGUAGEPHRASE("objective_fuelpressure").m_unicodestring,
           static_cast<int>(m_currentLevel * 100));
    ...
    m_aiTarget(nullptr)
{
  ...
  constexpr char entranceName[] = "MarkerEntrance";
  m_entrance = m_shape->m_rootFragment->LookupMarker(entranceName);
  ASSERT_TEXT(m_entrance, "FuelStation: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", entranceName, m_shape->m_name);
}
  ...
  if (building && building->m_type == TypeEscapeRocket)
  {
    ...
    if ((rocket->m_state == EscapeRocket::StateCountdown && rocket->m_countdown <= 10.0f) || rocket->m_state == EscapeRocket::StateFlight)
    {
      ...
      for (int i = 0; i < 3; ++i)
      {
        ...
        glTexCoord2f(1, 0);
        glVertex3dv((ourPos - lineTheirPos).GetData());
        glTexCoord2f(1, 1);
        glVertex3dv((ourPos + lineTheirPos).GetData());
        glColor4f(1.0f, 0.4f, 0.2f, 0.2f);
        glTexCoord2f(0, 1);
        glVertex3dv((pos + lineTheirPos).GetData());
        glTexCoord2f(0, 0);
        glVertex3dv((pos - lineTheirPos).GetData());
        glEnd();
      }
    ...
    m_refuelRate(0.0f)
{
  ...
  ASSERT_TEXT(m_booster, "EscapeRocket: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", boosterName, m_shape->m_name);
  ...
  for (int i = 0; i < 3; ++i)
  {
    ...
    ASSERT_TEXT(m_window[i], "EscapeRocket: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", name, m_shape->m_name);
  }
}
...
void EscapeRocket::GetObjectiveCounter(UnicodeString& _dest)
{
  ...
  swprintf(buffer, sizeof(buffer) / sizeof(wchar_t), L"%ls %d%%, %ls %d%%", LANGUAGEPHRASE("objective_fuel").m_unicodestring,
           static_cast<int>(m_fuel), LANGUAGEPHRASE("objective_passengers").m_unicodestring, static_cast<int>(m_passengers));
  _dest = UnicodeString(buffer);
}
...
void EscapeRocket::AdvanceReady()
{
  ...
  if (!m_spawnCompleted)
  {
    ...
    if (myTeam && spawnpoint && spawnpoint->m_type == TypeSpawnPoint)
      gotSpawnPoints |= spawnpoint->m_id.GetTeamId() == 0;
...
void EscapeRocket::AdvanceFlight()
{
  ...
  double thrust = sqrt(m_pos.y - landHeight) * 2;
...
void EscapeRocket::AdvanceExploding()
{
  ...
  if (g_app->Multiplayer() && m_passengers == 0 && m_fuel <= minFuel)
      ...
      for (int i = 0; i < team->m_others.Size(); ++i)
      {
        if (team->m_others.ValidIndex(i))
        {
          ...
          if (entity && entity->m_type == Entity::TypeDarwinian)
          {
            ...
            else { watchSpectacle = (t == 0 && darwinian->m_state == Darwinian::StateIdle && (syncrand() % 10) < 2); }
  ...
  if (g_app->Multiplayer())
  {
    if (m_state < StateExploding && m_fuel > 33.0f)
    {
      ...
      if (m_attackTimer <= 0.0f)
      {
        ...
        if (numFound > 0)
        {
          int numAttackers = 1 + syncrand() % (1 + static_cast<int>(numFound * 0.05));
          for (int i = 0; i < numAttackers; ++i)
          {
            int chosenIndex = syncrand() % numFound;
            WorldObjectId attackerId = s_neighbours[chosenIndex];
            auto darwinian = static_cast<Darwinian*>(g_app->m_location->GetEntitySafe(attackerId, Entity::TypeDarwinian));
            if (darwinian && !darwinian->m_dead && !darwinian->IsOnFire())
...
bool EscapeRocket::DoesSphereHit(const Vector3& _pos, double _radius) { return false; }

bool EscapeRocket::DoesShapeHit(Shape* _shape, Matrix34 _transform) { return false; }
