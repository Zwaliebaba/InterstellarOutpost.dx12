    ...
    m_forceTeamMatch(false)
{
  ...
  constexpr char dishName[] = "Dish";
  m_dish = m_shape->m_rootFragment->LookupFragment(dishName);
  ASSERT_TEXT(m_dish, "RadarDish: Can't get Fragment(%s) from shape(%s), probably a corrupted file\n", dishName, m_shape->m_name);

  constexpr char upperMountName[] = "UpperMount";
  m_upperMount = m_shape->m_rootFragment->LookupFragment(upperMountName);
  ASSERT_TEXT(m_upperMount, "RadarDish: Can't get Fragment(%s) from shape(%s), probably a corrupted file\n", upperMountName,
              m_shape->m_name);

  constexpr char focusMarkerName[] = "MarkerFocus";
  m_focusMarker = m_shape->m_rootFragment->LookupMarker(focusMarkerName);
  ASSERT_TEXT(m_focusMarker, "RadarDish: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", focusMarkerName,
              m_shape->m_name);
}
...
void RadarDish::Initialise(Building* _template)
{
  ...
  auto r = static_cast<RadarDish*>(_template);
...
bool RadarDish::Advance()
{
  ...
  if (m_newlyCreated)
  {
    ...
    {
      auto r = static_cast<RadarDish*>(g_app->m_location->GetBuilding(m_validLinkList[0]));
      if (r && r->m_type == TypeRadarDish)
        Aim(r->GetStartPoint());
    }
  ...
  if (!m_horizontallyAligned)
  {
    ...
    if (fabsf(rotationAxis.y) > M_PI / 3000.0)
      m_upperMount->m_angVel.y = signf(rotationAxis.y) * sqrt(fabsf(rotationAxis.y));
    else
    {
      m_horizontallyAligned = true;
  ...
  if (m_horizontallyAligned && !m_verticallyAligned)
  {
    ...
    constexpr double maxAngle = 0.3;
    constexpr double minAngle = -0.2;
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    ...
    RadarDish* otherDish = static_cast<RadarDish*>(building);
...
{
  DebugTrace("Aiming radar dish at %f,%f,%f\n", _worldPos.x, _worldPos.y, _worldPos.z);
  if (m_validLinkList.Size() > 0)
  {
    ...
    {
      if (!m_validLinkList.ValidIndex(i))
        continue;
...
void RadarDish::Render(double _predictionTime) { Building::Render(_predictionTime); }
...
void RadarDish::RenderSignal(double _predictionTime, double _radius, double _alpha)
{
  ...
  int numSteps = static_cast<int>(distance / 200.0);
  ...
  {
    Vector3 deltaFrom = 1.2 * delta * s / numSteps;
    Vector3 deltaTo = 1.2 * delta * (s + 1) / numSteps;
    ...
    for (int r = 0; r <= numRadii; ++r)
    {
      ...
      gglMultiTexCoord2fARB(GL_TEXTURE0_ARB, texXInner + 10.0 / static_cast<double>(numSteps), (r) / numRadii);
      gglMultiTexCoord2fARB(GL_TEXTURE1_ARB, texXOuter + distance / (200.0 * static_cast<double>(numSteps)), (r) / numRadii);
      glVertex3dv((currentPos + deltaTo).GetData());

      currentPos.RotateAround(deltaNorm * (2.0 * M_PI / numRadii));
    }

    texXInner += 10.0 / static_cast<double>(numSteps);
    texXOuter += distance / (200.0 * static_cast<double>(numSteps));
  }
...
bool RadarDish::Connected() { return (m_receiverId != -1 && m_signal > 0.0); }

int RadarDish::GetConnectedDishId() { return m_receiverId; }

bool RadarDish::ReadyToSend() { return (Connected() && Teleport::ReadyToSend()); }

Vector3 RadarDish::GetStartPoint() { return GetDishPos(0.0); }

Vector3 RadarDish::GetEndPoint() { return GetDishPos(0.0) + GetDishFront(0.0) * m_range; }
...
bool RadarDish::UpdateEntityInTransit(Entity* _entity)
{
  ...
  if (distTravelled >= m_range)
  {
    ...
    {
      auto d = static_cast<Darwinian*>(_entity);
    ...
    {
      auto pointMan = static_cast<Squadie*>(_entity);
      if (pointMan && pointMan->m_type == Entity::TypeInsertionSquadie)
      {
        ...
        if (g_app->m_location->m_teams[teamId]->m_units.ValidIndex(unitId))
        {
          ...
          {
            auto is = static_cast<InsertionSquad*>(unit);
...
bool RadarDish::DoesRayHit(const Vector3& rayStart, const Vector3& rayDir, float rayLen, Vector3* pos, Vector3* norm)
{
...
void RadarDish::AdvanceAITarget()
{
  if (m_signal > 0.0)
  {
    ...
    {
      auto r = static_cast<RadarDish*>(g_app->m_location->GetBuilding(m_receiverId));
      if (r && r->m_type == TypeRadarDish)
      {
        if (r->m_aiTarget)
        {
...
void RadarDish::SetBuildingLink(int _buildingId) { m_validLinkList.PutData(_buildingId); }

void RadarDish::Read(TextReader* _in, bool _dynamic)
{
  Building::Read(_in, _dynamic);
  if (_in->TokenAvailable())
    m_forceConnection = atoi(_in->GetNextToken());
  if (_in->TokenAvailable())
...
void RadarDish::AddValidLink(int _id) { m_validLinkList.PutData(_id); }

void RadarDish::ClearLinks() { m_validLinkList.Empty(); }

bool RadarDish::NotAligned() { return (!m_horizontallyAligned && !m_verticallyAligned); }
...
bool RadarDish::ValidReceiverDish(int _buildingId)
{
  if (m_forceTeamMatch)
  {
    ...
    if (m_id.GetTeamId() == b->m_id.GetTeamId())
      return true;
    return false;
  }
  if (m_validLinkList.Size() == 0)
    return true;
  for (int i = 0; i < m_validLinkList.Size(); ++i)
    if (_buildingId == m_validLinkList[i])
      return true;
  return false;
}
