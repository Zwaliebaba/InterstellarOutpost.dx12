...
void Bridge::SetBridgeType(int _type)
{
  ...
  constexpr char signalName[] = "MarkerSignal";
  m_signal = m_shape->m_rootFragment->LookupMarker(signalName);
  ASSERT_TEXT(m_signal, "Bridge: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", signalName, m_shape->m_name);
}
    ...
    if (building && building->m_type == TypeBridge)
    {
      ...
      if (m_status > 0.0 && bridge->m_status > 0.0)
      {
        ...
        else
        {
          ...
          glColor4ub(col.r, col.g, col.b, static_cast<unsigned char>(255.0 * m_status / 100.0));
        }
...
bool Bridge::ReadyToSend()
{
  ...
  return (m_status > 0.0 && nextBridge && m_bridgeType == BridgeTypeEnd && nextBridge->m_type == TypeBridge && Teleport::ReadyToSend());
}
...
bool Bridge::UpdateEntityInTransit(Entity* _entity)
{
  ...
  if (m_status > 0.0 && nextBridge && nextBridge->m_type == TypeBridge && nextBridge->m_status > 0.0)
  {
    Matrix34 theirMat(nextBridge->m_front, g_upVector, nextBridge->m_pos);
