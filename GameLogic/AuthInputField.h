...
class AuthInputField : public InputField
{
  public:
    ...
    void Keypress(int keyCode, bool shift, bool ctrl, bool alt, unsigned char ascii) override
    {
      ...
      if (len < AUTHENTICATION_KEYLEN - 1 || keyCode == KEY_BACKSPACE)
      {
      ...
      // CMD-V paste support
      if (keyCode == KEY_V && ctrl) { parent->PasteFromClipboard(); }
#endif
    }

    void MouseUp() override
    {
      if (strcmp(static_cast<LocalString*>(m_value)->Get(), "authkey not found") == 0)
        static_cast<LocalString*>(m_value)->Set(nullptr);
