    ...
    m_panic(0.0)
{
  ...
  /*if( !s_shapeTail || !s_shapeHead )
  {
      ...
      ASSERT_TEXT( s_tailMarker, "SoulDestroyer: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", tailMarkerName, s_shapeHead->m_name );
  }
...
SoulDestroyer::~SoulDestroyer() { s_numSoulDestroyers--; }
...
void SoulDestroyer::Begin()
{
  if (!s_shapeHead[m_id.GetTeamId()] || !s_shapeTail[m_id.GetTeamId()])
  {
    ...
    constexpr char tailMarkerName[] = "MarkerTail";
    s_tailMarker[m_id.GetTeamId()] = s_shapeHead[m_id.GetTeamId()]->m_rootFragment->LookupMarker(tailMarkerName);
    ASSERT_TEXT(s_tailMarker[m_id.GetTeamId()], "SoulDestroyer: Can't get Marker(%s) from shape(%s), probably a corrupted file\n",
...
bool SoulDestroyer::ChangeHealth(int _amount, int _damageType)
{
  ...
  if (!m_dead && _amount < 0)
  {
    ...
    double fractionDead = 1.0 - static_cast<double>(m_stats[StatHealth]) / EntityBlueprint::GetStat(TypeSoulDestroyer, StatHealth);
...
void SoulDestroyer::Attack(const Vector3& _pos)
{
  int numFound;
...
{
  WorldObjectId targetId = g_app->m_location->m_entityGrid->GetBestEnemy(m_pos.x, m_pos.z, 0.0, SOULDESTROYER_MAXSEARCHRANGE,
                                                                         m_id.GetTeamId());
...
bool SoulDestroyer::SearchForTargetEnemy()
{
  ...
  if (abs(m_pos.y - landHeight) < 50.0)
  {
    m_targetEntity.SetInvalid();
    return false;
  }
...
bool SoulDestroyer::AdvanceToTargetPosition()
{
  ...
  if (!m_targetEntity.IsValid())
  {
    ...
    targetDir.RotateAround(right1 * sin(GetNetworkTime() * 6.0) * 1.5);
  }
...
void SoulDestroyer::RenderSpirit(const Vector3& _pos, double _alpha)
{
  Vector3 pos = _pos;
...
void Zombie::Render(double _predictionTime)
{
  ...
  glBegin(GL_QUADS);
  glTexCoord2i(0, 0);
  glVertex3dv((predictedPos - size * predictedRight - size * predictedUp).GetData());
  glTexCoord2i(1, 0);
  glVertex3dv((predictedPos + size * predictedRight - size * predictedUp).GetData());
  glTexCoord2i(1, 1);
  glVertex3dv((predictedPos + size * predictedRight + size * predictedUp).GetData());
