...
void Teleport::SetShape(Shape* _shape)
{
  ...
  constexpr char entranceName[] = "MarkerTeleportEntrance";
  m_entrance = m_shape->m_rootFragment->LookupMarker(entranceName);
  ASSERT_TEXT(m_entrance, "Teleport: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", entranceName, m_shape->m_name);
}
...
bool Teleport::Advance()
{
  ...
  for (int i = 0; i < m_inTransit.Size(); ++i)
  {
    ...
    auto ent = static_cast<Entity*>(obj);
    if (ent)
    {
...
void Teleport::RenderSpirit(const Vector3& _pos, int _teamId)
{
  Vector3 pos = _pos;
...
bool Teleport::ReadyToSend() { return (m_timeSync <= 0.0); }
    ...
    if (!_relay)
    {
      ...
      if (oldUnit)
      {
        ...
        if (newUnitId == -1)
        {
          ...
          newUnit = g_app->m_location->m_teams[_id.GetTeamId()]->NewUnit(oldUnit->m_troopType, oldUnit->m_entities.NumUsed(), &newUnitId,
                                                                         m_pos);
          ...
          if (oldUnit->m_troopType == Entity::TypeInsertionSquadie)
          {
            ...
            auto task = new Task(newUnit->m_teamId);
            ...
            if (g_app->Multiplayer())
              task->m_lifeTimer = oldTimer;
            bool success = g_app->m_location->m_teams[_id.GetTeamId()]->m_taskManager->RegisterTask(task);
            if (success)
              g_app->m_location->m_teams[_id.GetTeamId()]->m_taskManager->SelectTask(task->m_id);

            static_cast<InsertionSquad*>(newUnit)->m_weaponType = static_cast<InsertionSquad*>(oldUnit)->m_weaponType;
            static_cast<InsertionSquad*>(newUnit)->m_controllerId = static_cast<InsertionSquad*>(oldUnit)->m_controllerId;
            static_cast<InsertionSquad*>(oldUnit)->m_controllerId = -1;
            Task* controller = g_app->m_location->m_teams[_id.GetTeamId()]->m_taskManager->GetTask(
              static_cast<InsertionSquad*>(newUnit)->m_controllerId);
            if (controller)
            {
