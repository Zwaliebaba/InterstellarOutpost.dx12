...
void Crate::AdvanceFloating()
{
  ...
  targetPos.x += cos(timeIndex) * swayDistance;
...
{
  return (_type == CrateRewardSquad || _type == CrateRewardEngineer || _type == CrateRewardArmour || _type == CrateRewardAirstrike || _type
    == CrateRewardDarwinians || _type == CrateRewardGunTurret || _type == CrateRewardForest);
}

bool Crate::IsAdvancedCrate(int _type) { return (!IsBasicCrate(_type)); }

bool Crate::IsValid(int _type)
{
#ifndef INCLUDE_CRATES_ADVANCED
  if (IsAdvancedCrate(_type)) { return false; }
#endif

  if (g_app->m_multiwinia->m_basicCratesOnly && IsAdvancedCrate(_type))
    return false;

  if (_type == CrateRewardHarvester || _type == CrateRewardEngineer || _type == CrateRewardSpawnMania)
  {
  ...
  bool armourReinforcements = (g_app->m_multiwinia->m_gameType == Multiwinia::GameTypeRocketRiot || g_app->m_location->m_levelFile->
    m_levelOptions[LevelFile::LevelOptionTrunkPortArmour] > 0);

  if (armourReinforcements && _type == CrateRewardArmour)
    return false;
  ...
  {
    if (_type == CrateRewardRocketTurret || _type == CrateRewardSpaceShip)
      return false;
  }

  if (s_spaceShipUsed && _type == CrateRewardSpaceShip)
    return false;

  if (_type == CrateRewardCrateMania && s_crates >= 4)
    return false;
  ...
  if (_type == CrateRewardAntNest)
  {
    ...
    {
      if (g_app->m_location->m_teams[i] == nullptr || g_app->m_location->m_teams[i]->m_teamType == TeamTypeUnused)
        continue;
...
void Crate::CaptureCrate()
{
  ...
  while (true)
  {
    if (rewardId != CrateRewardSpam && rewardId != CrateRewardRandomiser && rewardId != CrateRewardPlague && rewardId !=
      CrateRewardSpaceShip && rewardId != CrateRewardTriffids &&
  ...
  switch (rewardId)
  {
  case CrateRewardSquad:
    team->m_taskManager->RunTask(GlobalResearch::TypeSquad);
    break;
  case CrateRewardHarvester:
    team->m_taskManager->RunTask(GlobalResearch::TypeHarvester);
    break;
  case CrateRewardEngineer:
    team->m_taskManager->RunTask(GlobalResearch::TypeEngineer);
    break;
  case CrateRewardArmour:
    team->m_taskManager->RunTask(GlobalResearch::TypeArmour);
    break;
  case CrateRewardAirstrike:
    team->m_taskManager->RunTask(GlobalResearch::TypeAirStrike);
    break;
  case CrateRewardNuke:
    team->m_taskManager->RunTask(GlobalResearch::TypeNuke);
    break;
  case CrateRewardSubversion:
    team->m_taskManager->RunTask(GlobalResearch::TypeSubversion);
    break;
  case CrateRewardBooster:
    team->m_taskManager->RunTask(GlobalResearch::TypeBooster);
    break;
  case CrateRewardHotFeet:
    team->m_taskManager->RunTask(GlobalResearch::TypeHotFeet);
    break;
  case CrateRewardSpam:
    team->m_taskManager->RunTask(GlobalResearch::TypeInfection);
    break;
  case CrateRewardDarkForest:
    team->m_taskManager->RunTask(GlobalResearch::TypeDarkForest);
    break;
  case CrateRewardAntNest:
    team->m_taskManager->RunTask(GlobalResearch::TypeAntsNest);
    break;
  //case CrateRewardPlague:             team->m_taskManager->RunTask( GlobalResearch::TypePlague );         break;
  case CrateRewardEggs:
    team->m_taskManager->RunTask(GlobalResearch::TypeEggSpawn);
    break;
  case CrateRewardMeteor:
    team->m_taskManager->RunTask(GlobalResearch::TypeMeteorShower);
    break;
  ...
  case CrateRewardDarwinians:
    RunDarwinians(m_pos, m_id.GetTeamId());
    messageShown = true;
    break;
  case CrateRewardRandomiser:
    RunRandomiser(m_pos, m_id.GetTeamId());
    messageShown = true;
    break;
  case CrateRewardSpawnMania:
    RunSpawnMania();
    messageShown = true;
    break;
  case CrateRewardBitzkreig:
    RunBitzkreig();
    messageShown = true;
    break;
  case CrateRewardCrateMania:
    RunCrateMania();
    messageShown = true;
    break;
  case CrateRewardForest:
  ...
  case CrateRewardRocketDarwinians:
    ...
    messageShown = true;
    break;
  case CrateRewardSpaceShip:
    RunSpaceShip();
    messageShown = true;
    break;
  case CrateRewardTriffids:
    RunTriffids(m_pos, monsterTeam);
    messageShown = true;
    break;
  case CrateRewardSlowDownTime:
    RunSlowDownTime();
    messageShown = true;
    break;
  case CrateRewardSpeedUpTime:
    RunSpeedUpTime();
    messageShown = true;
    break;
#ifdef INCLUDE_THEBEAST
  case CrateRewardTheBeast:
    RunTheBeast(m_pos); messageShown = true; break;
#endif

  default: ASSERT_TEXT(false, "Undefined Crate Reward in CaptureCrate(): id = %d", rewardId);
    break;
  }
...
void Crate::CaptureCrate(int _crateId, int _teamId)
{
  ...
  case CrateRewardForest:
    team->m_taskManager->RunTask(GlobalResearch::TypeMagicalForest);
    break;
  case CrateRewardDarkForest:
    team->m_taskManager->RunTask(GlobalResearch::TypeDarkForest);
    break;
  ...
  default: ASSERT_TEXT(false, "Undefined Crate Reward in RunCaptureCrate(int): id = %d", _crateId);
    break;
  }
}
...
void Crate::RunUncapturedCrate(int _teamId)
{
...
#endif
    if (!s_spaceShipUsed && g_app->m_multiwinia->m_gameType != Multiwinia::GameTypeAssault)
  ...
  {
  case CrateRewardAirstrike:
    RunAirStrike(m_pos, monsterTeam);
    break;
  //case CrateRewardNuke:           RunNuke( m_pos, _teamId );           break;        
  case CrateRewardDarwinians:
    RunDarwinians(m_pos, _teamId);
    break;
  case CrateRewardSpam:
    RunInfection(m_pos, monsterTeam);
    break;
  case CrateRewardForest:
    RunMagicalForest(m_pos, monsterTeam);
    break;
  case CrateRewardDarkForest:
    RunDarkForest(m_pos, monsterTeam);
    break;
  case CrateRewardRandomiser:
    RunRandomiser(m_pos, monsterTeam);
    break;
  case CrateRewardAntNest:
    RunAntNest(m_pos, monsterTeam);
    break;
  case CrateRewardEggs:
    RunEggs(m_pos, monsterTeam);
    break;
  case CrateRewardMeteor:
    RunMeteorShower(m_pos, monsterTeam);
    break;
  case CrateRewardSpaceShip:
    RunSpaceShip();
    break;
  case CrateRewardTriffids:
    RunTriffids(m_pos, monsterTeam);
    break;
  case CrateRewardPlague:
    RunPlague(m_pos, monsterTeam);
    break;
  case CrateRewardNapalmStrike:
    RunAirStrike(m_pos, monsterTeam, true);
    break;
  case CrateRewardSlowDownTime:
    RunSlowDownTime();
    break;
...
#ifdef INCLUDE_THEBEAST
  case CrateRewardTheBeast:
    RunTheBeast(m_pos); break;
#endif

  default: ASSERT_TEXT(false, "Undefined Crate Reward in RunUncapturedCrate(): id = %d", rewardId);
    break;
  }
}

void Crate::StopCaptureSounds() { g_app->m_soundSystem->StopAllSounds(m_id, "Crate Capturing_"); }

void Crate::AdvanceOnGround()
{
  if (!m_aiTarget)
  {
    m_aiTarget = static_cast<AITarget*>(Building::CreateBuilding(Building::TypeAITarget));
    ...
    m_aiTarget->Initialise(&targetTemplate);
  ...
  if (m_id.GetTeamId() == currentMajority && m_id.GetTeamId() != 255) // current owner still in control, advance normally
  {
    bool includes[NUM_TEAMS];
    memset(includes, false, sizeof(bool) * NUM_TEAMS);
    includes[m_id.GetTeamId()] = true;

    int prevIntCaptured = static_cast<int>(5.0 * m_captureTimer / CRATE_CAPTURE_TIME);
    ...
    if (m_captureTimer >= CRATE_CAPTURE_TIME)
    {
      ...
      noAdvanced = true;
#endif
      if (m_rewardId != -1 || syncfrand(1.0) < 0.9 || noAdvanced)
        CaptureCrate();
      else
  ...
  else
  {
    ...
    if (m_captureTimer <= 0.0 && m_id.GetTeamId() != currentMajority)
    {
    ...
    for (int t = 0; t < NUM_TEAMS; ++t)
    {
      if (g_app->m_location->m_teams[t]->m_teamType != TeamTypeUnused)
      {
        ...
        for (int i = 0; i < numFound; ++i)
        {
          ...
          if (entity && entity->m_type == Entity::TypeDarwinian && !entity->m_dead && static_cast<Darwinian*>(entity)->m_state !=
            Darwinian::StateInsideArmour)
            ++entitiesCounted;
        }
...
void Crate::RenderBubble(double _predictionTime)
{
  ...
  if (m_onGround)
  {
    if (m_bubbleTimer > 0.0)
    {
      ...
      if (m_onGround)
        predictedHealth -= 40.0f * _predictionTime;
      else
        predictedHealth -= 20.0f * _predictionTime;
      ...
      for (int i = 0; i < 3; ++i)
      {
        ...
        glVertex3dv((fragmentPos + right * size - up * size).GetData());
  ...
  else
  {
    ...
    glBegin(GL_QUADS);
    glTexCoord2i(0, 0);
    glVertex3dv((pos - right * size - up * size).GetData());
    glTexCoord2i(0, 1);
    glVertex3dv((pos - right * size + up * size).GetData());
    glTexCoord2i(1, 1);
    glVertex3dv((pos + right * size + up * size).GetData());
...
void Crate::RenderGroundMarker(double _predictionTime)
{
  ...
  glTexCoord2i(1, 1);
  glVertex3dv((pos + rightAngle * scale).GetData());
  glTexCoord2i(1, 0);
  glVertex3dv((pos - front * scale + rightAngle * scale).GetData());
  glTexCoord2i(0, 0);
  glVertex3dv((pos - front * scale).GetData());
  glEnd();
...
bool Crate::IsInView()
{
  ...
  Vector3 pos = m_pos;
  pos.y = g_app->m_location->m_landscape.m_heightMap->GetValue(pos.x, pos.z);
  if (g_app->m_camera->PosInViewFrustum(pos))
    return true;
...
Vector3 Crate::PushFromObstructions(Vector3 _pos)
{
  ...
  for (int b = 0; b < buildings->Size(); ++b)
  {
    ...
    if (building)
    {
      ...
      if (building->DoesSphereHit(result, m_radius))
        hit = true;
...
bool Crate::DoesSphereHit(const Vector3& _pos, double _radius) { return false; }

bool Crate::DoesShapeHit(Shape* _shape, Matrix34 _transform) { return false; }

bool Crate::DoesRayHit(const Vector3& _rayStart, const Vector3& _rayDir, double _rayLen, Vector3* _pos, Vector3* norm) { return false; }
...
char* Crate::GetName(int _type)
{
  ...
  case CrateRewardSpeedUpTime:
    return "SpeedUp";
#ifdef INCLUDE_THEBEAST
  case Crate::CrateRewardTheBeast:
    return "TheBeast";
#endif
  }
...
void Crate::TriggerSoundEvent(int _crateType, const Vector3& pos, int _teamId)
{
  auto eventName = GetName(_crateType);
...
void Crate::GenerateTrees(Vector3 _pos)
{
  ...
  Tree* existing = nullptr;
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    if (g_app->m_location->m_buildings.ValidIndex(i))
    {
      if (g_app->m_location->m_buildings[i]->m_type == TypeTree)
      {
  ...
  {
    Building* newBuilding = CreateBuilding(TypeTree);
    ...
    auto newTree = static_cast<Tree*>(newBuilding);
    newTree->m_seed = static_cast<int>(syncfrand(99999));
    if (newTree->m_height > 100)
      newTree->m_height = 100;
...
WorldObjectId Crate::RunAirStrike(const Vector3& _pos, int _teamId, bool _special)
{
  //DebugTrace("Creating AirStrike markers\n");
  ...
  for (int i = 0; i < numMarkers; ++i)
  {
    ...
    targetPos.y = g_app->m_location->m_landscape.m_heightMap->GetValue(targetPos.x, targetPos.z);
    weaponId = g_app->m_location->ThrowWeapon(_pos, targetPos, EffectThrowableAirstrikeMarker, _teamId);
    AirStrikeMarker* marker = static_cast<AirStrikeMarker*>(g_app->m_location->m_effects[weaponId]);
...
void Crate::RunNuke(const Vector3& _pos, int _teamId)
{
#ifdef INCLUDE_CRATES_ADVANCED
  ...
  for (int i = 0; i < 10; ++i)
  {
    ...
    while (g_app->m_location->m_landscape.m_heightMap->GetValue(pos.x, pos.z) > 0.0) { pos.Set(FRAND(worldX), 0.0, FRAND(worldZ)); }

    WorldObjectId id = g_app->m_location->SpawnEntities(pos, _teamId, -1, Entity::TypeNuke, 1, Vector3(), 0.0);

    auto nuke = static_cast<Nuke*>(g_app->m_location->GetEntitySafe(id, Entity::TypeNuke));
    if (nuke)
    {
      Vector3 targetPos = _pos;
      if (i == 0)
...
void Crate::RunSubversion(const Vector3& _pos, int _teamId, int _taskId)
{
  ...
  for (int i = 0; i < numFound; ++i)
  {
    WorldObjectId id = s_neighbours[i];
    Entity* entity = g_app->m_location->GetEntity(id);
    if (entity && entity->m_type == Entity::TypeDarwinian && entity->m_id.GetTeamId() == _teamId)
      static_cast<Darwinian*>(entity)->GiveMindControl(_taskId);
  }
...
void Crate::RunBooster(const Vector3& _pos, int _teamId, int _taskId)
{
  int numFound;
  bool include[NUM_TEAMS];
  memset(include, false, sizeof(include));
  ...
  for (int i = 0; i < numFound; ++i)
  {
    ...
    Entity* entity = g_app->m_location->GetEntity(id);
    if (entity && entity->m_type == Entity::TypeDarwinian && entity->m_id.GetTeamId() == _teamId)
      static_cast<Darwinian*>(entity)->SetBooster(_taskId);
  }
...
void Crate::RunHotFeet(const Vector3& _pos, int _teamId, int _taskId)
{
#ifdef INCLUDE_CRATES_ADVANCED
  int numFound;
  bool include[NUM_TEAMS];
  ...
  for (int i = 0; i < numFound; ++i)
  {
    ...
    Entity* entity = g_app->m_location->GetEntity(id);
    if (entity && entity->m_type == Entity::TypeDarwinian && entity->m_id.GetTeamId() == _teamId)
      static_cast<Darwinian*>(entity)->GiveHotFeet(_taskId);
  }
...
WorldObjectId Crate::RunGunTurret(const Vector3& _pos, int _teamId, int _turretType)
{
...
void Crate::RunInfection(const Vector3& _pos, int _teamId)
{
#ifdef INCLUDE_CRATES_ADVANCED
  for (int i = 0; i < 100; ++i)
  {
...
void Crate::RunMagicalForest(const Vector3& _pos, int _teamId)
{
  ...
  UnicodeString msg;
  GetNameTranslated(CrateRewardForest, msg);
...
void Crate::RunDarkForest(const Vector3& _pos, int _teamId)
{
#ifdef INCLUDE_CRATES_ADVANCED
  ...
  if (_teamId == g_app->m_location->GetMonsterTeamId())
  {
    GetNameTranslated(CrateRewardDarkForest, msg);
    UnicodeString temp(LANGUAGEPHRASE("multiwinia_exclamation"));
    temp.ReplaceStringFlag(L'T', msg);
    msg = temp;
  }
...
void Crate::RunAntNest(const Vector3& _pos, int _teamId)
{
  ...
  for (int i = 0; i < 2; ++i)
  {
    ...
    Building* newBuilding = CreateBuilding(TypeAntHill);
    int id = g_app->m_location->m_buildings.PutData(newBuilding);
    newBuilding->Initialise(&antHillTemplate);
    newBuilding->SetDetail(1);
  ...
  if (_teamId == g_app->m_location->GetMonsterTeamId())
  {
    GetNameTranslated(CrateRewardAntNest, msg);
    UnicodeString temp(LANGUAGEPHRASE("multiwinia_exclamation"));
    temp.ReplaceStringFlag(L'T', msg);
    msg = temp;
  }
...
void Crate::RunPlague(const Vector3& _pos, int _teamId)
{
#ifdef INCLUDE_CRATES_ADVANCED
  int numFound;
...
void Crate::RunEggs(const Vector3& _pos, int _teamId)
{
#ifdef INCLUDE_CRATES_ADVANCED
  ...
  for (int i = 0; i < numEggs; ++i)
  {
    ...
    auto triffidEgg = static_cast<TriffidEgg*>(g_app->m_location->GetEntitySafe(eggId, Entity::TypeTriffidEgg));
    if (triffidEgg)
    {
      int randomChance = -1;
      ...
      while (true)
      {
        ...
        if (randomChance != Triffid::SpawnSpirits && randomChance != Triffid::SpawnEggs && randomChance != Triffid::SpawnSoulDestroyer &&
          randomChance != Triffid::SpawnAnts && randomChance != Triffid::SpawnDarwinians)
          break;
      }
...
void Crate::RunMeteorShower(const Vector3& _pos, int _teamId)
{
#ifdef INCLUDE_CRATES_ADVANCED
  ...
  for (int i = 0; i < numMeteors; ++i)
  {
    ...
    auto m = static_cast<Meteor*>(g_app->m_location->GetEntitySafe(id, Entity::TypeMeteor));
    if (m)
...
void Crate::RunDarwinians(const Vector3& _pos, int _teamId)
{
...
void Crate::RunRandomiser(const Vector3& _pos, int _teamId)
{
#ifdef INCLUDE_CRATES_ADVANCED
  auto r = new Randomiser();
...
void Crate::RunSpawnMania()
{
  ...
  UnicodeString msg;
  GetNameTranslated(CrateRewardSpawnMania, msg);
  UnicodeString temp(LANGUAGEPHRASE("multiwinia_exclamation"));
  temp.ReplaceStringFlag(L'T', msg);
  msg = temp;
...
void Crate::RunBitzkreig()
{
  ...
  UnicodeString temp;
  GetNameTranslated(CrateRewardBitzkreig, temp);
  auto msg = UnicodeString(temp.Length() + 2, temp.m_unicodestring, temp.Length());
...
void Crate::RunCrateMania()
{
  ...
  UnicodeString temp;
  GetNameTranslated(CrateRewardCrateMania, temp);
  auto msg = UnicodeString(temp.Length() + 2, temp.m_unicodestring, temp.Length());
  msg.m_unicodestring[temp.Length()] = L'!';
  msg.m_unicodestring[temp.Length() + 1] = L'\0';
...
void Crate::RunRage(const Vector3& _pos, int _teamId, int _taskId)
{
  int numFound;
  bool include[NUM_TEAMS];
  memset(include, false, sizeof(include));
  ...
  for (int i = 0; i < numFound; ++i)
  {
    ...
    Entity* entity = g_app->m_location->GetEntity(id);
    if (entity && entity->m_type == Entity::TypeDarwinian && entity->m_id.GetTeamId() == _teamId)
      static_cast<Darwinian*>(entity)->GiveRage(_taskId);
  }
...
void Crate::RunTriffids(const Vector3& _pos, int _teamId)
{
#ifdef INCLUDE_CRATES_ADVANCED
  ...
  for (int i = 0; i < numTriffids; ++i)
  {
    ...
    for (int s = 0; s < Triffid::NumSpawnTypes; ++s)
    {
      ...
      if (s == Triffid::SpawnSpirits || s == Triffid::SpawnEggs)
        triffidTemplate.m_spawn[s] = false;
    }

    auto triffid = static_cast<Triffid*>(Building::CreateBuilding(Building::TypeTriffid));
    triffid->Initialise(&triffidTemplate);
  ...
  UnicodeString msg;
  GetNameTranslated(CrateRewardTriffids, msg);
  UnicodeString temp(LANGUAGEPHRASE("multiwinia_exclamation"));
  temp.ReplaceStringFlag(L'T', msg);
  msg = temp;
...
void Crate::RunTheBeast(const Vector3& _pos)
{
  ...
  auto portal = static_cast<Portal*>(Building::CreateBuilding(Building::TypePortal));
  portal->Initialise(&portalTemplate);
...
char* Crate::LogState(char* _message)
{
  ...
  sprintf(s_result, "%sCRATE Id[%d] pos[%5.2f,%5.2f,%5.2f], vel[%5.2f] radius[%5.2f]", (_message) ? _message : "", m_id.GetUniqueId(),
          m_pos.x, m_pos.y, m_pos.z, m_vel.x + m_vel.y + m_vel.z, m_radius);
