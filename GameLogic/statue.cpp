...
void Statue::Initialise(Building* _template)
{
  ...
  {
  case 0:
    SetShape(g_app->m_resource->GetShape("carrystatue.shp"));
    break;
  case 1:
    SetShape(g_app->m_resource->GetShape("carrysquaddie.shp"));
    break;
  case 2:
    SetShape(g_app->m_resource->GetShape("carryglobe.shp"));
    break;
  case 3:
    SetShape(g_app->m_resource->GetShape("carrytelescope.shp"));
    break;
  case 4:
    SetShape(g_app->m_resource->GetShape("carryweights.shp"));
    break;
  case 5:
    SetShape(g_app->m_resource->GetShape("carrycube.shp"));
    break;
  case 6:
    SetShape(g_app->m_resource->GetShape("carryteapot.shp"));
    break;
  case 7:
    SetShape(g_app->m_resource->GetShape("carrybillboard.shp"));
...
bool Statue::Advance()
{
  ...
  {
    DebugTrace("Statue bad waypoint\n");
    double timeNow = GetNetworkTime();

    if (timeNow >= m_waypointCheck)
    {
      DebugTrace("Looking for new waypoint, team = %d\n", m_id.GetTeamId());
      m_waypointCheck = timeNow + 1.0;

      int requiredTeamId = m_id.GetTeamId();
      if (m_id.GetTeamId() == g_app->m_location->GetMonsterTeamId() || m_id.GetTeamId() == g_app->m_location->GetFuturewinianTeamId())
        requiredTeamId = 255;
      ...
      for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
      {
        if (g_app->m_location->m_buildings.ValidIndex(i))
        {
          ...
          if (building && building->m_type == TypeMultiwiniaZone && building->m_id.GetTeamId() == requiredTeamId)
          {
      ...
      if (nearestId != -1)
      {
        ...
        DebugTrace("Statue set waypoint to building with teamId %d\n", building->m_id.GetTeamId());
        m_routeWayPointId = -1;
      }
    }
  }
