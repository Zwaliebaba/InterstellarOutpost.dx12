...
class GameServerButton : public GameMenuButton
{
  public:
    GameServerButton(int _serverNumber)
      : GameMenuButton(""),
        m_serverNumber(_serverNumber) { sprintf(m_name, "server%d", _serverNumber); }
    ...
    bool IsServerFull(Directory* server)
    {
      ...
      if (m_numHumanPlayers == '?' || !isRunning)
        m_numHumanPlayers = m_numPlayers;

      return m_numPlayers == m_maxPlayers;
    }

    bool IsLanGame(Directory* server) { return (server->HasData(NET_METASERVER_LANGAME)); }
    ...
    void MouseUp() override
    {
      ...
      else
      {
        ...
        if (mapIndex == -1) { g_app->m_clientToServer->m_requestCustomMap = true; }
        else { g_app->m_clientToServer->m_requestCustomMap = false; }
#endif
    ...
    void Render(int realX, int realY, bool highlighted, bool clicked) override
    {
      ...
      if (server->HasData(NET_METASERVER_HASPASSWORD))
      {
        ...
        glTexCoord2i(1, 1);
        glVertex2f(lockX + m_h * 0.8f, lockY);
        glTexCoord2i(1, 0);
        glVertex2f(lockX + m_h * 0.8f, lockY + m_h * 0.8f);
        glTexCoord2i(0, 0);
        glVertex2f(lockX, lockY + m_h * 0.8f);
        glEnd();
      ...
      for (int i = 0; i < m_maxPlayers; ++i)
      {
        ...
        if (i < m_numPlayers)
        {
          if (i < m_numHumanPlayers)
            glColor4ubv(col.GetData());
          else
            glColor4ubv((col * 0.6f).GetData());
    ...
    bool SpecialScroll(int direction) override
    {
      auto parent = static_cast<GameMenuWindow*>(m_parent);
      int realNum = GetServerIndex();
      if (direction == 1 && m_serverNumber == 0 && parent->m_serverList->ValidIndex(realNum + SERVERS_ON_SCREEN))
      {
      ...
      else if (direction == -1 && m_serverNumber + 1 == SERVERS_ON_SCREEN)
      {
