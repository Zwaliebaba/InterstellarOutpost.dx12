...
void HelpAndOptionsWindow::Create()
{
  ...
  button->SetShortProperties("multiwinia_menu_options", buttonX, yPos += buttonH + gap, buttonW, buttonH,
                             LANGUAGEPHRASE("multiwinia_menu_settings"));
...
CrateHelpWindow::~CrateHelpWindow() { m_crateButtons.Empty(); }
...
void CrateHelpWindow::Create()
{
  ...
  {
    auto cButton = new CrateButton(i, m_crateFontSize);
    sprintf(name, "crate_tooltip_%s", c.GetName(i));
    if (LANGUAGEPHRASE(name) == UnicodeString(name))
      ohnoes = ohnoes + UnicodeString(name) + UnicodeString("\n");
    sprintf(name, "crate_%s", c.GetName(i));

    cButton->SetShortProperties(name, m_crateX, m_crateY, m_crateW, m_crateH, cButton->m_caption);
  ...
  auto closebutton = new MenuCloseButton("close");
...
void CrateButton::Render(int realX, int realY, bool highlighted, bool clicked)
{
  ...
  auto chw = static_cast<CrateHelpWindow*>(m_parent);
  int offset = 0;
  if (chw->m_scrollBar)
    offset = chw->m_scrollBar->m_currentValue;

  float loadPos = (GetHighResTime() - m_startTime) * 750;
  loadPos = min(loadPos, m_yOffset);
  int position = realY - (offset * 4) + loadPos;

  if (position < chw->m_scrollBar->m_y - (m_imageHeight / 10) - m_h || position > chw->m_scrollBar->m_y - (m_imageHeight / 10) + chw->
    m_scrollBar->m_h)
    return;
  ...
  glBegin(GL_QUADS);
  glTexCoord2i(0, 1);
  glVertex2f(iconCentre.x - delta, iconCentre.y - delta);
  glTexCoord2i(1, 1);
  glVertex2f(iconCentre.x + delta, iconCentre.y - delta);
  glTexCoord2i(1, 0);
  glVertex2f(iconCentre.x + delta, iconCentre.y + delta);
...
class GeneralNextPageButton : public GameMenuButton
{
  public:
    ...
    void MouseUp() override
    {
      auto parent = static_cast<GeneralHelpWindow*>(m_parent);
...
void GeneralHelpWindow::Create()
{
  ...
  sprintf(text1, "multiwinia_helppage_general%d_mac", m_page); if (!ISLANGUAGEPHRASE(text1))
    sprintf(text1, "multiwinia_helppage_general%d", m_page);
#else
  sprintf(text1, "multiwinia_helppage_general%d", m_page);
#endif

  auto image = new ImageButton(image1);
  image->SetShortProperties("image1", buttonX, yPos, imageW, imageH, UnicodeString());
  RegisterButton(image);

  auto text = new TextButton(text1);
  ...
  auto prev = new GeneralNextPageButton();
  ...
  auto next = new GeneralNextPageButton();
  ...
  auto closebutton = new MenuCloseButton("close");
  closebutton->SetShortProperties("multiwinia_menu_back", buttonX, yPos += buttonH + gap, buttonW, buttonH,
...
class NextPageButton : public GameMenuButton
{
  public:
    ...
    void MouseUp() override
    {
      auto parent = static_cast<GameTypeHelpWindow*>(m_parent);
      parent->ChangePage(parent->m_page += m_dir);
    }
};
...
void GameTypeHelpWindow::Create()
{
...

  {
    ...
    if (g_app->m_langTable->DoesPhraseExist(stringId))
    {
      ...
      auto text = new TextButton(stringId);
