...
void Portal::AdvanceFinalSummon()
{
  ...
  {
  case SummonStateLoweringPortal:
    AdvanceStateLowering();
    break;
  case SummonStatePortalExpand:
    AdvanceStateExpanding();
    break;
  case SummonStateThrash:
    AdvanceStateThrashing();
    break;
  case SummonStateExplode:
    AdvanceStateExploding();
    break;
  case SummonStateCoolDown:
    AdvanceStateCooldown();
    break;
  }
...
void Portal::AdvanceStateExpanding()
{
  ...
  if (m_portalRadius >= 100.0)
  {
    ...
    for (int i = 0; i < NUM_TENTACLES; ++i)
    {
      ...
      pos.x += cos(degrees) * (m_portalRadius - 10.0);
      pos.z += sin(degrees) * (m_portalRadius - 10.0);
      ...
      {
        auto t = static_cast<Tentacle*>(unit->m_entities[j]);
...
{
  if (m_portalCentre != g_zeroVector)
    return m_portalCentre;
...
void Portal::RenderAlphas(double _predictionTime)
{
  ...
  for (int s = 0; s < steps; ++s)
  {
    if (rand() % 20 == 0)
    {
      ...
      centrePos += sin(d) * right * m_portalRadius;
      centrePos += cos(d) * m_front * m_portalRadius;
  ...
  for (int i = 0; i < 15; ++i)
  {
    ...
    centrePos += sin(d) * right * frand(m_portalRadius);
    centrePos += cos(d) * m_front * frand(m_portalRadius);
    centrePos.y = g_app->m_location->m_landscape.m_heightMap->GetValue(centrePos.x, centrePos.z);
...
void Portal::RenderExplosionBuildup(double _predictionTime)
{
  ...
  for (int i = 0; i < maxBlobs; ++i)
  {
    ...
    pos.y += abs(cos(timeIndex + i) * cos(i * 20) * 64);
    pos.z += cos(timeIndex + i) * i * 1.7;

    double size = 20.0 * sin(timeIndex + i * 13);
...
{
  if (m_finalSummonState == SummonStateThrash || m_finalSummonState == SummonStateExplode)
  {
    Vector3 from = m_pos;
...
bool Portal::DoesSphereHit(const Vector3& _pos, double _radius) { return false; }

bool Portal::DoesShapeHit(Shape* _shape, Matrix34 _transform) { return false; }

bool Portal::DoesRayHit(const Vector3& _rayStart, const Vector3& _rayDir, double _rayLen, Vector3* _pos, Vector3* norm)
{
