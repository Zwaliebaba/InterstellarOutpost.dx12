    ...
    void MouseUp() override
    {
      ...
      if (parent->m_gameType == Multiwinia::GameTypeAssault)
      {
          ...
          ValidIndex(parent->m_requestedMapId))
        {
          ...
          if (numPlayers == 3)
          {
            if (m_colourIndex == 0 && num > 0)
              taken = true;
            else
              if (m_colourIndex == 1 && num == 2)
                taken = true;
          }
          else
          {
      ...
      if (!taken)
      {
        ...
        static_cast<GameMenuWindow*>(m_parent)->m_newPage = GameMenuWindow::PageGameSetup;
      }
    }
    ...
    void Render(int realX, int realY, bool highlighted, bool clicked) override
    {
      ...
      if (parent->m_gameType == Multiwinia::GameTypeAssault)
      {
        ...
        for (int i = 0; i < NUM_TEAMS; ++i)
        {
          ...
          if (g_app->m_multiwinia->m_teams[i].m_colourId == m_colourIndex && g_app->m_globalWorld->m_myTeamId != i)
            num++;

          if (i == g_app->m_globalWorld->m_myTeamId && g_app->m_multiwinia->m_teams[i].m_colourId == m_colourIndex)
            clicked = true;
        }
