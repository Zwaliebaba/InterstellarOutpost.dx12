...
LList<UnicodeString*> Engineer::s_buildingIds;
LList<UnicodeString*> Engineer::s_researchIds;
...
Engineer::~Engineer()
{
  if (g_app->m_gameMode == App::GameModeMultiwinia && m_id.GetTeamId() != 255)
  {
...
void Engineer::SetWaypoint(const Vector3& _wayPoint)
{
  m_retargetTimer = 0.0;
    ...
    for (int i = 0; i < g_app->m_location->m_spirits.Size(); ++i)
    {
      if (g_app->m_location->m_spirits.ValidIndex(i))
      {
        ...
        if (theDist <= ENGINEER_SEARCHRANGE && theDist < closest && (s->m_state == Spirit::StateBirth || s->m_state ==
          Spirit::StateFloating) && s->m_broken < 0.0)
        {
          found = s;
          spiritId = i;
          closest = theDist;
...
bool Engineer::SearchForControlTowers()
{
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    if (g_app->m_location->m_buildings.ValidIndex(i))
    {
      ...
      {
        ControlTower* ct = static_cast<ControlTower*>(building);
        Vector3 pos, front;
        if ((ct->m_id.GetTeamId() != m_id.GetTeamId() || ct->m_ownership < 100.0) && ct->GetAvailablePosition(pos, front) != -1)
        {
          double theDist = (building->m_pos - m_pos).Mag();
...
bool Engineer::SearchForBridges()
{
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    if (g_app->m_location->m_buildings.ValidIndex(i))
    {
      ...
      {
        Bridge* bridge = static_cast<Bridge*>(building);
        Vector3 pos, front;
        double theDist = (building->m_pos - m_pos).Mag();
        if (bridge->GetAvailablePosition(pos, front) && bridge->m_status < 100.0 && theDist <= ENGINEER_SEARCHRANGE && theDist < closest)
        {
          buildingIndex = i;
          closest = theDist;
        }
...
bool Engineer::SearchForResearchItems()
{
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    if (g_app->m_location->m_buildings.ValidIndex(i))
    {
      ...
      {
        ResearchItem* item = static_cast<ResearchItem*>(building);
        double theDist = (building->m_pos - m_pos).Mag();
  ...
  if (buildingIndex != -1)
  {
    Building* building = g_app->m_location->m_buildings[buildingIndex];
    m_buildingId = building->m_id.GetUniqueId();
    Vector3 usToThem = (building->m_pos - m_pos).SetLength(35.0);
...
bool Engineer::ChangeHealth(int amount, int _damageType)
{
  ...
  if (!m_dead)
  {
    int healthBandBefore = static_cast<int>(m_stats[StatHealth] / 50.0);
    ...
    int healthBandAfter = static_cast<int>(m_stats[StatHealth] / 50.0);

    double fractionDead = 1.0 - m_stats[StatHealth] / EntityBlueprint::GetStat(TypeEngineer, StatHealth);
...
bool Engineer::Advance(Unit* _unit)
{
  ...
  if (bridge && bridge->m_type == Building::TypeBridge)
  {
    double range = (bridge->m_pos - m_pos).Mag();
    if (range > 100.0)
      EndBridge();
  }
...
bool Engineer::AdvanceToTargetPos()
{
  ...
  if (distance.Mag() > 5.0)
  {
    ...
    desiredSpeed = min(desiredSpeed, static_cast<double>(m_stats[StatSpeed]));
    ...
    m_pos = newPos;
    double targetHeight = max(g_app->m_location->m_landscape.m_heightMap->GetValue(m_pos.x, m_pos.z), 0.0) + m_hoverHeight;
...
bool Engineer::SearchForRandomPosition()
{
  ...
  m_targetPos = m_pos + Vector3(sin(angle) * distance, 0.0, cos(angle) * distance);
...
bool Engineer::SearchForIncubator()
{
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    if (g_app->m_location->m_buildings.ValidIndex(i))
    {
      ...
      if (building->m_type == Building::TypeSpawnPoint && g_app->m_gameMode == App::GameModeMultiwinia && g_app->m_location->
        IsFriend(building->m_id.GetTeamId(), m_id.GetTeamId()))
      {
        double distance = (building->m_pos - m_pos).Mag();
        if (distance < nearest)
        {
...
bool Engineer::AdvanceToIncubator()
{
  ...
  if (!building)
  {
    ...
    building = g_app->m_location->GetBuilding(m_buildingId);
    if (!building)
    {
  ...
  {
    auto incubator = static_cast<Incubator*>(building);
  ...
  //
  // Deal with spawn point

  if (building && building->m_type == Building::TypeSpawnPoint)
  {
    auto spawnPoint = static_cast<SpawnPoint*>(building);
...
bool Engineer::AdvanceToControlTower()
{
  ...
  if ((ct->m_id.GetTeamId() == m_id.GetTeamId() && ct->m_ownership >= 100.0) || positionId == -1)
  {
  ...
  if (positionId != -1)
  {
    ...
    if (arrived)
    {
      ...
      if (g_app->IsSinglePlayer())
      {
        ...
        char buf[256];
        sprintf(buf, "%d-%d", g_app->m_locationId, ct->m_id.GetUniqueId());
        UnicodeString tempS(buf);
        for (int i = 0; i < s_buildingIds.Size(); i++)
        {
          UnicodeString* string = s_buildingIds.GetData(i);
          if (string)
          {
            if (doneAlready = wcscmp(string->m_unicodestring, tempS.m_unicodestring) == 0)
              break;
          }
        }
...
bool Engineer::AdvanceResearching()
{
  ...
  double time = g_gameTime + m_id.GetIndex();
  Vector3 toPos = end1;
  toPos += (end2 - end1) / 2.0;
  toPos += (end2 - end1) * sin(time) * 0.5;

  for (int i = 0; i < 2; ++i)
  {
    Vector3 particleVel = m_pos - toPos;
    particleVel += Vector3(sfrand() * 15.0, frand() * 10.0, sfrand() * 15.0);
    g_app->m_particleSystem->CreateParticle(toPos, particleVel, Particle::TypeBlueSpark);
  }

  //
  // Make us double around a bit while we work

  m_targetFront = (toPos - m_pos).Normalise();
  Vector3 targetFront = m_targetFront;
  m_front = (targetFront * SERVER_ADVANCE_PERIOD) + (m_front * (1.0 - SERVER_ADVANCE_PERIOD));

  Vector3 oldPos = m_pos;
  Vector3 targetPos = m_targetPos;
  Vector3 rightAngle = m_targetFront ^ g_upVector;
  double scale = 2.0;
  targetPos += m_targetFront * sin(GetNetworkTime() + m_id.GetUniqueId() * 10.0) * scale;
  targetPos += rightAngle * cos(GetNetworkTime() + m_id.GetUniqueId() * 10.0) * scale * 1.5;

  m_pos = (targetPos * SERVER_ADVANCE_PERIOD) + (m_pos * (1.0 - SERVER_ADVANCE_PERIOD));
  m_vel = (m_pos - oldPos) / SERVER_ADVANCE_PERIOD;
...
void Engineer::BeginBridge(Vector3 _to)
{
  ...
  int numComponents = static_cast<int>(bridgeSpan.Mag() / 80.0);
  Vector3 componentSpan = bridgeSpan / numComponents;
  ...
  for (int i = numComponents; i >= 0; --i)
  {
    ...
    component->m_pos = m_wayPoint + componentSpan * i;
    component->m_pos += Vector3(SFRAND(15.0), 0.0,SFRAND(15.0));
    ...
    if (i < numComponents && i > 0)
      component->m_front.RotateAroundY(0.5 * M_PI);
...
bool Engineer::AdvanceToResearchItem()
{
  ...
  if (arrived)
  {
    ...
    if (g_app->IsSinglePlayer())
    {
      ...
      bool doneAlready = false;
      char buf[256];
      sprintf(buf, "%d-%d", g_app->m_locationId, item->m_id.GetUniqueId());
      UnicodeString tempS(buf);
      for (int i = 0; i < s_researchIds.Size(); i++)
      {
        UnicodeString* string = s_researchIds.GetData(i);
        if (string)
        {
          if (doneAlready = wcscmp(string->m_unicodestring, tempS.m_unicodestring) == 0)
            break;
      ...
      if (!doneAlready)
      {
        g_app->m_markerSystem->RegisterMarker_Research(item->m_id);
        s_researchIds.PutData(new UnicodeString(tempS));
      }
    }
  }
...
void Engineer::RenderShape(double predictionTime)
{
  Vector3 predictedPos = m_pos + m_vel * predictionTime;
  if (m_onGround)
  {
    predictedPos.y = max(g_app->m_location->m_landscape.m_heightMap->GetValue(predictedPos.x, predictedPos.z), 0.0 /*sea level*/) +
      m_hoverHeight;
  }
...
void Engineer::Render(double predictionTime)
{
  //
  ...
  if (!m_dead)
  {
    ...
    BeginRenderShadow();
    RenderShadow(predictedPos, 10.0);
    EndRenderShadow();
  }
  ...
  if (m_state == StateReprogramming || m_state == StateOperatingBridge || m_state == StateResearching)
  {
    ...
    if (building)
    {
      ...
      glColor4f(0.2, 0.4, 1.0, abs(sin(g_gameTime * 3.0)));
...
char* Engineer::GetCurrentAction()
{
  ...
  case StateToWaypoint:
    return "engineer_towaypoint";
  case StateToSpirit:
    return "engineer_tospirit";
  case StateToIncubator:
    return "engineer_toincubator";

  case StateToControlTower:
  case StateReprogramming:
    return "engineer_reprogramming";

  case StateToResearchItem:
  case StateResearching:
    return "engineer_researching";

  case StateToBridge:
  case StateOperatingBridge:
    return "engineer_bridge";
  }

  return "engineer_idle";
}

bool Engineer::IsSelectable() { return true; }

void Engineer::RunAI(AI* _ai)
{
