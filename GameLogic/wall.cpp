...
void Wall::Initialise(Building* _template)
{
  ...
  m_objectiveLink = static_cast<Wall*>(_template)->m_objectiveLink;
}
...
bool Wall::Advance()
{
  ...
  {
    auto ai = static_cast<AIObjective*>(g_app->m_location->GetBuilding(m_objectiveLink));
    if (ai && ai->m_type == TypeAIObjective)
    {
      m_id.GetUniqueId();
      m_registered = true;
    }
...
bool Wall::DoesSphereHit(const Vector3& _pos, double _radius)
{
  if (m_shape)
  {
    Vector3 right = m_front ^ g_upVector;
...
bool Wall::DoesRayHit(const Vector3& _rayStart, const Vector3& _rayDir, double _rayLen, Vector3* _pos, Vector3* norm)
{
  if (m_shape)
  {
...
void Wall::SetBuildingLink(int _buildingId) { m_objectiveLink = _buildingId; }

int Wall::GetBuildingLink() { return m_objectiveLink; }
