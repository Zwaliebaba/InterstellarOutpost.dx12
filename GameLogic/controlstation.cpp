...
void ControlStation::Initialise(Building* _template)
{
  ...
  m_controlBuildingId = static_cast<ControlStation*>(_template)->m_controlBuildingId;
}
...
void ControlStation::RecalculateOwnership()
{
  ...
  {
    if (teamCount[i] > num && winningTeam == -1)
      winningTeam = i;
    else if (winningTeam != -1 && teamCount[i] > num && teamCount[i] > teamCount[winningTeam])
      winningTeam = i;
  }
...
void ControlStation::SetBuildingLink(int _buildingId) { m_controlBuildingId = _buildingId; }

int ControlStation::GetBuildingLink() { return m_controlBuildingId; }
...
void ControlStation::RenderAlphas(double _predictionTime)
{
  ...
  if (target && m_id.GetTeamId() >= 0 && m_id.GetTeamId() < NUM_TEAMS && !target->m_destroyed)
  {
    ...
    {
      float timeOffset = fmodf(GetHighResTime() * 3, 1.0f) * 1 / static_cast<float>(numSteps);
      Vector3 thisFromPos = fromPos + (targetPos - fromPos) * (timeOffset + j / static_cast<float>(numSteps));
      Vector3 thisToPos = fromPos + (targetPos - fromPos) * (timeOffset + (j + 1) / static_cast<float>(numSteps));

      float alpha = 255 - 255 * j / static_cast<float>(numSteps);
