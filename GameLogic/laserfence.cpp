    ...
    m_scale(1.0)
{
  ...
  constexpr char marker1Name[] = "MarkerFence01";
  m_marker1 = m_shape->m_rootFragment->LookupMarker(marker1Name);
  ASSERT_TEXT(m_marker1, "LaserFence: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", marker1Name, m_shape->m_name);

  constexpr char marker2Name[] = "MarkerFence02";
  m_marker2 = m_shape->m_rootFragment->LookupMarker(marker2Name);
  ASSERT_TEXT(m_marker2, "LaserFence: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", marker2Name, m_shape->m_name);
}
...
void LaserFence::Initialise(Building* _template)
{
  ...
  if (m_mode == ModeEnabled || m_mode == ModeAlwaysOn)
    m_status = 1.0;
...
void LaserFence::RenderAlphas(double predictionTime)
{
  ...
  if (m_mode != ModeDisabled && m_mode != ModeNeverOn)
  {
    ...
    if (m_nextLaserFenceId != -1)
    {
      ...
      Vector3 bottomLeft(m_pos - Vector3(0, ourFenceHeight / 3, 0)), topLeft(m_pos + Vector3(0, ourFenceHeight, 0)),
              topRight(nextFence->m_pos + Vector3(0, theirFenceHeight, 0)), bottomRight(
                nextFence->m_pos - Vector3(0, theirFenceHeight / 3, 0));
...
{
  if (m_mode != ModeEnabled && m_mode != ModeNeverOn && m_mode != ModeAlwaysOn)
    m_mode = ModeEnabling;
...
{
  if (m_mode != ModeDisabled && m_mode != ModeNeverOn && m_mode != ModeAlwaysOn)
...
bool LaserFence::IsEnabled() { return (m_mode == ModeEnabled) || (m_mode == ModeEnabling) || (m_mode == ModeAlwaysOn); }
...
void LaserFence::Electrocute(const Vector3& _pos) { g_app->m_soundSystem->TriggerBuildingEvent(this, "Electrocute"); }

bool LaserFence::DoesSphereHit(const Vector3& _pos, double _radius)
{
  if (m_mode == ModeDisabled)
  {
...
bool LaserFence::DoesRayHit(const Vector3& _rayStart, const Vector3& _rayDir, double _rayLen, Vector3* _pos, Vector3* _norm)
{
  if (m_mode == ModeDisabled)
  {
...
{
  if (m_mode != ModeAlwaysOn)
  {
    m_solarLinked = true;
    if (GetBuildingLink() != -1)
    {
      auto fence = static_cast<LaserFence*>(g_app->m_location->GetBuilding(GetBuildingLink()));
