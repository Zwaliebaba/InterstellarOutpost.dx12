...
{
  SetType(TypeAI);
  s_entityPopulation--; // AIs dont increase the population, and they never die
...
AI::~AI() { m_rrObjectiveList.EmptyAndDelete(); }

bool AI::ChangeHealth(int _amount, int _damageType) { return false; }
...
void AI::Begin()
{
  ...
  DebugTrace("AI Node graph rebuilt in %dms\n", static_cast<int>(timeTaken * 1000.0));
}

int AI::FindTargetBuilding(int _fromTargetId, int _fromTeamId)
{
  AITarget* fromBuilding = static_cast<AITarget*>(g_app->m_location->GetBuilding(_fromTargetId));
  ...
  if (g_app->Multiplayer())
  {
    ...
    if (g_app->m_multiwinia->m_gameType == Multiwinia::GameTypeAssault || g_app->m_multiwinia->m_gameType == Multiwinia::GameTypeRocketRiot)
      randomMovement = false;
  }
  else
  ...
  if (randomMovement)
  {
    ...
    if (numNeighbours > 0)
    {
      ...
      while (true)
      {
        ...
        AITarget* t = static_cast<AITarget*>(g_app->m_location->GetBuilding(fromBuilding->m_neighbours[neighbourIndex]->m_neighbourId));
        if (fromBuilding->m_radarTarget)
          if (t && t->m_radarTarget)
            return _fromTargetId;
        if (t && t->m_waterTarget)
          continue;
        if (fromBuilding->m_neighbours[neighbourIndex]->m_laserFenceId == -1)
          break;
        LaserFence* fence = static_cast<LaserFence*>(g_app->m_location->GetBuilding(
          fromBuilding->m_neighbours[neighbourIndex]->m_laserFenceId));
        if (!fence || fence->m_id.GetTeamId() == m_id.GetTeamId())
          break;
  ...
  else
  {
    ...
    for (int i = 0; i < fromBuilding->m_neighbours.Size(); ++i)
    {
      ...
      AITarget* target = static_cast<AITarget*>(g_app->m_location->GetBuilding(toBuildingId));
      DEBUG_ASSERT(target && target->m_type == Building::TypeAITarget);

      if (target->m_waterTarget)
        continue;

      double thisPriority = target->m_priority[_fromTeamId];
      if (thisPriority > bestPriority)
      {
        LaserFence* fence = static_cast<LaserFence*>(g_app->m_location->GetBuilding(fromBuilding->m_neighbours[i]->m_laserFenceId));
        if (!fence || g_app->m_location->IsFriend(fence->m_id.GetTeamId(), _fromTeamId) || !fence->IsEnabled())
        {
          ...
          if (target->m_turretTarget)
            ...
            turretTarget = false;
          if (fromBuilding->m_radarTarget && target->m_radarTarget)
            id = _fromTargetId;
        }
      }

      if (target->m_turretTarget && !g_app->m_location->IsFriend(target->m_id.GetTeamId(), _fromTeamId) && target->m_id.GetTeamId() != 255)
    ...
    if (turretTarget)
    {
      ...
      {
        int sanity = 0;
        AITarget* best = static_cast<AITarget*>(g_app->m_location->GetBuilding(id));
        while (sanity++ < 1000)
        {
          NeighbourInfo* info = fromBuilding->m_neighbours[turretList[syncrand() % turretList.Size()]];
          AITarget* target = static_cast<AITarget*>(g_app->m_location->GetBuilding(info->m_neighbourId));
          if ((target->m_pos - best->m_pos).Mag() < 200.0)
          {
            LaserFence* fence = static_cast<LaserFence*>(g_app->m_location->GetBuilding(info->m_laserFenceId));
...
int AI::FindNearestTarget(const Vector3& _fromPos, bool _evaluateCliffs, bool _allowSteepDownhill)
{
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    if (g_app->m_location->m_buildings.ValidIndex(i))
    {
      ...
      {
        AITarget* target = static_cast<AITarget*>(building);
  ...
  while (numRemaining > 0)
  {
    ...
    if (team->m_others.ValidIndex(index))
    {
      ...
      if (entity && entity->m_type == TypeDarwinian && entity->m_onGround)
      {
        ...
        if (darwinian->m_wallHits > 10 && darwinian->m_state != Darwinian::StateInsideArmour)
        {
          ...
          Darwinian::StateWatchingSpectacle)
        {
          ...
          if (nearestTarget)
          {
            ...
            if (distance > 70.0)
            {
              ...
              targetPos.x += radius * sin(theta);
              targetPos.z += radius * cos(theta);
...
void AI::AdvanceSinglePlayerAI()
{
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    ...
    {
      Building* building = g_app->m_location->m_buildings[i];
      if (building->m_type == Building::TypeAITarget && g_app->m_location->IsFriend(building->m_id.GetTeamId(), m_id.GetTeamId()))
      {
        AITarget* target = static_cast<AITarget*>(building);
        ...
        if ((idleCount > limit && noAttack) || !target->IsImportant())
  ...
  for (int i = 0; i < m_wellDefendedIds.Size(); ++i)
  {
    ...
    AITarget* target = static_cast<AITarget*>(g_app->m_location->m_buildings[buildingIndex]);
    ...
    {
      AITarget* targetBuilding = static_cast<AITarget*>(g_app->m_location->GetBuilding(targetBuildingId));
      int enemyCount = targetBuilding->m_enemyCount[m_id.GetTeamId()];
      double sendChance = 1.5 * static_cast<double>(enemyCount) / static_cast<double>(numIdle);
      ...
      for (int j = 0; j < numFound; ++j)
      {
        ...
        if (darwinian && syncfrand(1.0) < sendChance && (darwinian->m_state == Darwinian::StateIdle || darwinian->m_state ==
            Darwinian::StateWorshipSpirit || darwinian->m_state == Darwinian::StateWatchingSpectacle) && !darwinian->IsInFormation() &&
          // Dont take Darwinians out of a formation
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    if (g_app->m_location->m_buildings.ValidIndex(i))
    {
      ...
      if (building->m_type == Building::TypeAITarget && g_app->m_location->IsFriend(building->m_id.GetTeamId(), m_id.GetTeamId()))
      {
        ...
        {
          AITarget* neighbour = static_cast<AITarget*>(g_app->m_location->GetBuilding(target->m_neighbours[n]->m_neighbourId));
        ...
        bool noAttack = enemyCount < friendCount * 0.33;
        if (g_app->m_multiwinia->m_aiType != Multiwinia::AITypeEasy && g_app->m_multiwinia->m_gameType != Multiwinia::GameTypeAssault)
        {
          if (enemyCount > 0)
          {
        ...
        if (g_app->m_multiwinia->m_gameType == Multiwinia::GameTypeAssault && target->m_radarTarget && g_app->m_location->IsAttacking(
          m_id.GetTeamId()))
          limit *= 3;
  ...
  for (int i = 0; i < m_wellDefendedIds.Size(); ++i)
  {
    ...
    for (int j = 0; j < g_app->m_location->m_buildings.Size(); ++j)
    {
      if (g_app->m_location->m_buildings.ValidIndex(j))
      {
        ...
        if (b->m_type == Building::TypeSpawnPoint && (target->m_pos - b->m_pos).Mag() < 200.0 && b->m_id.GetTeamId() == m_id.GetTeamId() &&
    ...
    if (targetBuildingId != -1)
    {
      ...
      if (g_app->m_multiwinia->m_gameType == Multiwinia::GameTypeAssault)
      {
        ...
        for (int i = 0; i < team->m_specials.Size(); ++i)
        {
          ...
          Armour* a = static_cast<Armour*>(g_app->m_location->GetEntitySafe(*team->m_specials.GetPointer(i), TypeArmour));
          if (a)
          {
            if ((a->m_pos - target->m_pos).Mag() < 100.0 && a->m_state == Armour::StateUnloading)
              sendChance = 0.0;
          }
        }
      }
      ...
      if (useOfficer)
      {
        //if(SearchForOfficers(target->m_pos, 200.0 ) == 0 )
        {
          ...
          for (int j = 0; j < numFound; ++j)
          {
            auto d = static_cast<Darwinian*>(g_app->m_location->GetEntitySafe(s_neighbours[j], TypeDarwinian));
            if (d && d->m_state == Darwinian::StateIdle)
            {
              team->m_taskManager->TargetTask(id, d->m_pos);
              auto o = static_cast<Officer*>(g_app->m_location->GetEntitySafe(team->m_taskManager->m_mostRecentEntity, TypeOfficer));
              if (o)
              {
                Vector3 targetPos = targetBuilding->m_pos;
                double positionError = 20.0;
                ...
                {
                  LList<int>* nearbyBuildings = g_app->m_location->m_obstructionGrid->GetBuildings(
                    targetBuilding->m_pos.x, targetBuilding->m_pos.z);
      ...
      else
      {
        ...
        for (int j = 0; j < numFound; ++j)
        {
          if (g_app->m_multiwinia->m_aiType != Multiwinia::AITypeEasy && target->m_scoreZone != -1 && target->m_friendCount[m_id.
            GetTeamId()] - numSent < limit)
          {
          ...
          if (darwinian && syncfrand(1.0) < sendChance && (darwinian->m_state == Darwinian::StateIdle || darwinian->m_state ==
              Darwinian::StateWorshipSpirit || darwinian->m_state == Darwinian::StateWatchingSpectacle) && !darwinian->IsInFormation() &&
            // Dont take Darwinians out of a formation
            darwinian->m_onGround && darwinian->m_closestAITarget == target->m_id.GetUniqueId())
          {
            if (targetBuilding->m_radarTarget)
            {
              Vector3 pos;
...
void AI::AdvanceAssaultAI()
{
  if (g_app->m_multiwinia->m_aiType != Multiwinia::AITypeHard)
    m_timer = 3.0;
  if (!AIObjective::s_objectivesInitialised)
    return;
  ...
  if (m_currentObjective == -1)
  {
    ...
    for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
    {
      if (g_app->m_location->m_buildings.ValidIndex(i))
      {
        ...
        {
          if (static_cast<AIObjective*>(b)->m_active)
          {
    ...
    if (activeObjectives.Size() > 0 && m_currentObjective == -1)
    {
      if (!attacking && activeObjectives.Size() > 1)
      {
        ...
        {
          AIObjective* objective = static_cast<AIObjective*>(g_app->m_location->GetBuilding(activeObjectives[i]));
          if (objective)
          {
            for (int j = 0; j < objective->m_objectiveMarkers.Size(); ++j)
            {
              AIObjectiveMarker* marker = static_cast<AIObjectiveMarker*>(g_app->m_location->GetBuilding(objective->m_objectiveMarkers[j]));
              if (marker)
              {
                bool underAttack = g_app->m_location->m_entityGrid->AreEnemiesPresent(
                  marker->m_pos.x, marker->m_pos.z, 450.0, m_id.GetTeamId());
                if (underAttack)
                {
  ...
  if (m_currentObjective != -1)
  {
    AIObjective* currentObjective = static_cast<AIObjective*>(g_app->m_location->GetBuilding(m_currentObjective));
    ...
    if (attacking)
    {
      ...
      for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
      {
        if (g_app->m_location->m_buildings.ValidIndex(i))
        {
          WallBuster* b = static_cast<WallBuster*>(g_app->m_location->m_buildings[i]);
          if (b->m_type == Building::TypeWallBuster && b->m_id.GetTeamId() == m_id.GetTeamId())
            b->RunAI(this);
        }
      }
    }
  ...
  if (m_radarReposTimer <= 0.0)
  {
    ...
    for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
    {
      if (g_app->m_location->m_buildings.ValidIndex(i))
      {
        ...
        {
          RadarDish* dish = static_cast<RadarDish*>(b);
          if (b->m_id.GetTeamId() == m_id.GetTeamId())
          {
            bool usable = true;
            int num = 0;
    ...
    for (int i = 0; i < ourRadars.Size(); ++i)
    {
      ...
      for (int j = 0; j < radars.Size(); ++j)
      {
        ...
        if (dotProd < 0.0)
        {
          if (g_app->m_location->IsVisible(d->GetDishPos(0.0), radars[j]->GetDishPos(0.0)) && d->
            ValidReceiverDish(radars[j]->m_id.GetUniqueId()))
          {
            ...
            {
              AITarget* t = static_cast<AITarget*>(g_app->m_location->GetBuilding(FindNearestTarget(radars[j]->m_pos, false, true)));
...
int AI::SearchForOfficers(Vector3 _pos, double _range, bool _ignoreGotoOfficers, bool _gotoOnly)
{
  ...
  for (int i = 0; i < team->m_specials.Size(); ++i)
  {
    ...
    if (e && e->m_type == TypeOfficer && !e->m_dead)
    {
      if ((e->m_pos - _pos).Mag() <= _range)
      {
        if (_gotoOnly)
        {
          if (static_cast<Officer*>(e)->m_orders == Officer::OrderGoto)
            num++;
        }
        else if (!_ignoreGotoOfficers || static_cast<Officer*>(e)->m_orders != Officer::OrderGoto)
          num++;
      }
    }
  }
...
{
  Team* team = g_app->m_location->m_teams[m_id.GetTeamId()];
  if (g_app->m_multiwinia->m_gameType == Multiwinia::GameTypeRocketRiot || g_app->m_multiwinia->m_gameType == Multiwinia::GameTypeAssault ||
    g_app->m_multiwinia->m_gameType == Multiwinia::GameTypeBlitzkreig)
    return; // no officers in rocket riot or assault
    ...
    if (g_app->m_location->m_buildings.ValidIndex(i))
    {
      ...
      if (building->m_type == Building::TypeAITarget && g_app->m_location->IsFriend(building->m_id.GetTeamId(), m_id.GetTeamId()))
      {
        ...
        if ( //target->IsImportant() &&
          needsDefending && friendCount >= limit &&
          ...
          numOfficers == 0)
        {
          ...
          for (int j = 0; j < numFound; ++j)
          {
            ...
            if (e && e->m_type == TypeDarwinian)
            {
              auto d = static_cast<Darwinian*>(e);
              if (d->m_state != Darwinian::StateBeingSacraficed && d->m_state != Darwinian::StateCapturedByAnt && d->m_state !=
                Darwinian::StateInsideArmour && d->m_state != Darwinian::StateOperatingPort)
              {
...
void AI::AdvanceCurrentTasks()
{
  ...
  for (int i = 0; i < team->m_taskManager->m_tasks.Size(); ++i)
  {
    ...
    if (t)
    {
      if (t->m_state != Task::StateRunning && t->m_startTimer <= 0.0)
      {
        switch (t->m_type)
        {
...
int CountPanels(const Vector3& _pos, const double _range)
{
  int num = 0;
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    ...
    {
      Building* b = g_app->m_location->m_buildings[i];
      if (b->m_type == Building::TypeSolarPanel && (b->m_pos - _pos).Mag() <= _range)
        num++;
    }
  }
...
void AI::AdvanceRocketRiotAI()
{
  ...
  if (m_rrObjectiveList.Size() == 0)
  {
    for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
    {
      ...
      {
        Building* b = g_app->m_location->m_buildings[i];
        if (b && b->m_type == Building::TypeAIObjectiveMarker && b->m_id.GetTeamId() == m_id.GetTeamId())
        {
          double range = static_cast<AIObjectiveMarker*>(b)->m_scanRange;
          int numPanels = CountPanels(b->m_pos, range);
          auto info = new RRSolarPanelInfo;
          info->m_pos = b->m_pos;
...
{
  AITarget* target = nullptr;
  int maxEnemies = 0;
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    if (g_app->m_location->m_buildings.ValidIndex(i))
    {
      AITarget* t = static_cast<AITarget*>(g_app->m_location->m_buildings[i]);
      if (t && t->m_type == Building::TypeAITarget && g_app->m_location->m_teams[m_id.GetTeamId()]->m_taskManager->
        IsValidTargetArea(_taskId, t->m_pos))
      {
...
void AI::RunAirStrike(int _taskId)
{
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    ...
    {
      Building* b = g_app->m_location->m_buildings[i];
      if (!g_app->m_location->m_teams[m_id.GetTeamId()]->m_taskManager->IsValidTargetArea(_taskId, b->m_pos))
        continue;
      if (b && b->m_type == Building::TypeAITarget && b->m_id.GetTeamId() != g_app->m_location->GetMonsterTeamId())
      {
        AITarget* t = static_cast<AITarget*>(g_app->m_location->m_buildings[i]);
        if (t->m_enemyCount[m_id.GetTeamId()] > maxEnemies)
        {
      ...
      {
        if (b->m_id.GetTeamId() != 255 && !g_app->m_location->IsFriend(m_id.GetTeamId(), b->m_id.GetTeamId()))
...
void AI::RunNuke(int _taskId)
{
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    ...
    {
      AITarget* t = static_cast<AITarget*>(g_app->m_location->m_buildings[i]);
      if (t && t->m_type == Building::TypeAITarget && t->m_id.GetTeamId() != g_app->m_location->GetMonsterTeamId() && g_app->m_location->
        m_teams[m_id.GetTeamId()]->m_taskManager->IsValidTargetArea(_taskId, t->m_pos))
      {
        if (t->m_enemyCount[m_id.GetTeamId()] > maxEnemies)
        {
          maxEnemies = t->m_enemyCount[m_id.GetTeamId()];
...
void AI::RunPowerup()
{
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    if (g_app->m_location->m_buildings.ValidIndex(i))
    {
      ...
      {
        AITarget* target = static_cast<AITarget*>(b);
        if (target->m_friendCount[m_id.GetTeamId()] > 30 && target->m_enemyCount[m_id.GetTeamId()] > target->m_friendCount[m_id.
          GetTeamId()])
        {
          int numFound;
          double range = 100.0;
          if (target->m_scoreZone != -1)
          {
            MultiwiniaZone* zone = static_cast<MultiwiniaZone*>(g_app->m_location->GetBuilding(target->m_scoreZone));
...
void AI::RunGunTurretRocketRiot(int _taskId)
{
  ...
  EscapeRocket* rocket = static_cast<EscapeRocket*>(g_app->m_location->GetBuilding(lobbyTeam->m_rocketId));
  ...
  //if( rocket->m_fuel >= 10.0 )
  {
    ...
    for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
    {
      if (g_app->m_location->m_buildings.ValidIndex(i))
      {
        ...
        if (b->m_type == Building::TypeGunTurret)
        {
          if (b->m_id.GetTeamId() == m_id.GetTeamId() && (rocket->m_pos - b->m_pos).Mag() < 300.0)
            numTurretsFound++;
        }

        if (b->m_type == Building::TypeTrunkPort && b->m_id.GetTeamId() != m_id.GetTeamId())
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    ...
    {
      Building* b = g_app->m_location->m_buildings[i];
      if (b->m_type == Building::TypeAITarget && b->m_id.GetTeamId() == m_id.GetTeamId())
  ...
  if (ownedTargets.Size() > 0)
  {
    ...
    int numFound;
    g_app->m_location->m_entityGrid->GetFriends(s_neighbours, t->m_pos.x, t->m_pos.z, DARWINIAN_SEARCHRANGE_ARMOUR, &numFound,
                                                m_id.GetTeamId());
    ...
    {
      Entity* e = g_app->m_location->GetEntitySafe(s_neighbours[i], TypeDarwinian);
      if (e)
      {
        pos += e->m_pos;
        num++;
...
void AI::RunGunTurret(int _taskId)
{
  ...
  if (g_app->m_multiwinia->m_gameType == Multiwinia::GameTypeRocketRiot && g_app->m_multiwinia->m_aiType > Multiwinia::AITypeEasy)
  {
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    ...
    {
      Building* b = g_app->m_location->m_buildings[i];
      if (b && b->m_type == Building::TypeAITarget && g_app->m_location->m_teams[m_id.GetTeamId()]->m_taskManager->
        IsValidTargetArea(_taskId, b->m_pos))
      {
        if (b->m_id.GetTeamId() == m_id.GetTeamId())
        {
          AITarget* t = static_cast<AITarget*>(b);
          int hostileTarget = -1;
          int neutralTarget = -1;
          ...
          {
            AITarget* n = static_cast<AITarget*>(g_app->m_location->GetBuilding(t->m_neighbours[j]->m_neighbourId));
            if (n)
            {
          ...
          if (hostileTarget != -1)
            targetBuilding = hostileTarget;
          else
            if (neutralTarget != -1)
              targetBuilding = neutralTarget;
        }
      }
    }
  }
  ...
  if (b)
  {
    if (b->m_type == Building::TypeAITarget)
    {
      ...
      for (int i = 0; i < numFound; ++i)
      {
        auto e = static_cast<Darwinian*>(g_app->m_location->GetEntitySafe(s_neighbours[i], TypeDarwinian));
        if (e)
        {
          if (e->m_state != Darwinian::StateInsideArmour && e->m_state != Darwinian::StateOnFire && e->m_onGround && !e->m_dead)
          {
            pos += e->m_pos;
            num++;
          }
...
{
  MultiwiniaZone* zone = static_cast<MultiwiniaZone*>(g_app->m_location->
                                                             GetBuilding(MultiwiniaZone::s_blitzkriegBaseZone[m_id.GetTeamId()]));
  if (zone && zone->m_type == Building::TypeMultiwiniaZone)
  {
    int numTurrets = 0;
    ...
    for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
    {
      ...
      {
        Building* building = g_app->m_location->m_buildings[i];
        if ((building->m_pos - zone->m_pos).Mag() < zone->m_size && building->m_type == Building::TypeGunTurret)
        {
          numTurrets++;
          turretPos = building->m_pos;
        }

        if (building->m_type == Building::TypeMultiwiniaZone && building->m_id.GetUniqueId() != MultiwiniaZone::s_blitzkriegBaseZone[m_id.
          GetTeamId()] && building->m_id.GetTeamId() == m_id.GetTeamId())
    ...
    else
    {
      if (zoneList.Size() > 0)
      {
        ...
        MultiwiniaZone* b = static_cast<MultiwiniaZone*>(g_app->m_location->GetBuilding(buildingId));
...
void AI::RunSpam(int _taskId)
{
  ...
  int acceptableTarget = -1;
  // an acceptable target is one which the enemy controls but is close to an area we control. used if there are no good targets available
...
void AI::RunForest(int _taskId)
{
  ...
  if (hasHarvester)
  {
    for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
    {
      ...
      {
        Building* b = g_app->m_location->m_buildings[i];
        if (b && b->m_type == Building::TypeAITarget && b->m_id.GetTeamId() == m_id.GetTeamId())
        {
          AITarget* target = static_cast<AITarget*>(b);
...
void AI::RunAntNest(int _taskId)
{
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    ...
    {
      Building* b = g_app->m_location->m_buildings[i];
      if (b && b->m_type == Building::TypeSpawnPoint && !g_app->m_location->IsFriend(m_id.GetTeamId(), b->m_id.GetTeamId()) && b->m_id.
        GetTeamId() != 255 && b->m_id.GetTeamId() != g_app->m_location->GetMonsterTeamId() && g_app->m_location->m_teams[m_id.GetTeamId()]->
        m_taskManager->IsValidTargetArea(_taskId, b->m_pos))
...
void AI::RunEggs(int _taskId)
{
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    ...
    {
      Building* b = g_app->m_location->m_buildings[i];
      if (b && b->m_type == Building::TypeAITarget && !g_app->m_location->IsFriend(m_id.GetTeamId(), b->m_id.GetTeamId()) && b->m_id.
        GetTeamId() != 255 && b->m_id.GetTeamId() != g_app->m_location->GetMonsterTeamId() && g_app->m_location->m_teams[m_id.GetTeamId()]->
        m_taskManager->IsValidTargetArea(_taskId, b->m_pos))
...
void AI::RunPlague(int _taskId)
{
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    if (g_app->m_location->m_buildings.ValidIndex(i))
    {
      ...
      if (b && b->m_type == Building::TypeAITarget)
      {
        if (!g_app->m_location->IsFriend(m_id.GetTeamId(), b->m_id.GetTeamId()) && b->m_id.GetTeamId() != g_app->m_location->
          GetMonsterTeamId())
        {
          AITarget* t = static_cast<AITarget*>(b);
          bool good = true;
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    if (g_app->m_location->m_buildings.ValidIndex(i))
    {
      ...
      if (b && b->m_type == Building::TypeAITarget && !g_app->m_location->IsFriend(m_id.GetTeamId(), b->m_id.GetTeamId()) && b->m_id.
        ...
        m_taskManager->IsValidTargetArea(_taskId, b->m_pos))
      {
        if (g_app->m_location->m_entityGrid->GetNumFriends(b->m_pos.x, b->m_pos.z, area, m_id.GetTeamId()) < 30 && g_app->m_location->
          m_entityGrid->GetNumEnemies(b->m_pos.x, b->m_pos.z, area, m_id.GetTeamId()) > 50)
        {
          g_app->m_location->m_teams[m_id.GetTeamId()]->m_taskManager->TargetTask(_taskId, b->m_pos);
          break;
        }
...
AITarget::~AITarget() { m_neighbours.EmptyAndDelete(); }
...
double AITarget::GetRange()
{
  ...
  if (g_app->Multiplayer())
  {
    ...
    if (m_scoreZone != -1)
    {
      MultiwiniaZone* zone = static_cast<MultiwiniaZone*>(g_app->m_location->GetBuilding(m_scoreZone));
      if (zone && zone->m_type == Building::TypeMultiwiniaZone)
        range = zone->m_size;
    }
  }
...
bool AITarget::IsImportant(int& _buildingId)
{
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    ...
    {
      Building* b = g_app->m_location->m_buildings[i];
      if (b->m_ports.Size() > 0 || b->m_type == Building::TypeCrate || b->m_type == Building::TypeRadarDish || (g_app->m_multiwinia->
        m_gameType == Multiwinia::GameTypeRocketRiot && b->m_type == Building::TypeTrunkPort) || (g_app->m_multiwinia->m_gameType !=
        Multiwinia::GameTypeCaptureTheStatue && b->m_type == Building::TypeMultiwiniaZone))
      {
...
bool AITarget::NeedsDefending(int _teamId)
{
  ...
  if (m_scoreZone != -1 && g_app->m_multiwinia->m_gameType != Multiwinia::GameTypeCaptureTheStatue)
    return true;

  for (int i = 0; i < m_neighbours.Size(); ++i)
  {
    AITarget* t = static_cast<AITarget*>(g_app->m_location->GetBuilding(m_neighbours[i]->m_neighbourId));
    if (t && t->m_type == Building::TypeAITarget)
    {
      if (!g_app->m_location->IsFriend(m_id.GetTeamId(), t->m_id.GetTeamId()) && t->m_id.GetTeamId() != 255 && t->m_enemyCount[m_id.
        GetTeamId()] > 30 && numOfficers == 0)
...
void AITarget::RecalculatePriorityAssault()
{
  for (int t = 0; t < NUM_TEAMS; ++t)
  {
    ...
    if (!m_aiObjectiveTarget[t])
    {
      ...
      for (int i = 0; i < m_neighbours.Size(); ++i)
      {
        LaserFence* fence = static_cast<LaserFence*>(g_app->m_location->GetBuilding(m_neighbours[i]->m_laserFenceId));
        if (!fence || g_app->m_location->IsFriend(fence->m_id.GetTeamId(), t) || !fence->IsEnabled())
        {
          int neighbourId = m_neighbours[i]->m_neighbourId;
          AITarget* target = static_cast<AITarget*>(g_app->m_location->GetBuilding(neighbourId));
      ...
      if (num > 0)
      {
        prioritySum /= static_cast<double>(num) + 1; // (m_neighbours.Size() + 1);
      ...
      if (m_crateTarget)
      {
        ...
        {
          AI* ai = AI::s_ai[t];
          AIObjective* objective = static_cast<AIObjective*>(g_app->m_location->GetBuilding(ai->m_currentObjective));
...
void AITarget::RecalculatePriorityCTS()
{
  for (int t = 0; t < NUM_TEAMS; ++t)
  {
    ...
    if (num > 0)
    {
      prioritySum /= static_cast<double>(num);
      m_priority[t] = prioritySum;
    }

    if (g_app->m_location->IsFriend(m_id.GetTeamId(), t))
      m_priority[t] /= 2.0;
    ...
    if (m_statueTarget != -1)
    {
      Statue* statue = static_cast<Statue*>(g_app->m_location->GetBuilding(m_statueTarget));
      if (statue)
      {
        if (statue->m_id.GetTeamId() != t || statue->m_numLifters<statue->m_maxLifters || m_enemyCount[t] > 10)
          m_priority[t] = 1.0;
      }
    }
...
void AITarget::RecalculatePriorityStandard()
{
  for (int t = 0; t < NUM_TEAMS; ++t)
  {
    ...
    {
      if (importantBuilding->m_id.GetTeamId() != t && importantBuilding->m_type != Building::TypeTrunkPort)
        grabIt = true;
    }
    if (grabIt)
    {
    ...
    m_priority[t] += static_cast<double>(m_neighbours.Size()) * 0.01;
    m_priority[t] += m_priorityModifier;
    ...
    if (g_app->Multiplayer())
    {
      if (m_scoreZone != -1)
      {
        MultiwiniaZone* b = static_cast<MultiwiniaZone*>(g_app->m_location->GetBuilding(m_scoreZone));
        if (b)
        {
          if (g_app->m_multiwinia->m_gameType == Multiwinia::GameTypeKingOfTheHill)
          {
            ...
            if (b->m_id.GetTeamId() != t || b->m_totalCount - b->m_teamCount[t] > 10)
              m_priority[t] = 0.95;
          }
        }
      }
...
void AITarget::RecalculatePriorityRocketRiot()
{
  for (int t = 0; t < NUM_TEAMS; ++t)
  {
    ...
    if (importantBuilding)
    {
      ...
      if (importantBuilding->m_type == Building::TypeTrunkPort && g_app->m_multiwinia->m_gameType == Multiwinia::GameTypeRocketRiot &&
    ...
    else
    {
      ...
      prioritySum /= static_cast<double>(num) + 1;
      m_priority[t] = prioritySum;
      if (g_app->m_location->IsFriend(m_id.GetTeamId(), t))
        m_priority[t] /= 2.0;
    }
...
int AITarget::GetBuildingLink() { return m_linkId; }
...
void AISpawnPoint::Initialise(Building* _template)
{
  ...
  auto spawnPoint = (AISpawnPoint*)_template;
...
bool AISpawnPoint::PopulationLocked()
{
  ...
  if (m_populationLock == -1)
  {
    ...
    for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
    {
      if (g_app->m_location->m_buildings.ValidIndex(i))
      {
        ...
        {
          SpawnPopulationLock* lock = static_cast<SpawnPopulationLock*>(building);
          double distance = (building->m_pos - m_pos).Mag();
  ...
  {
    SpawnPopulationLock* lock = static_cast<SpawnPopulationLock*>(g_app->m_location->GetBuilding(m_populationLock));
...
bool AISpawnPoint::Advance()
{
  ...
  if (!m_online)
  {
    ...
    {
      BlueprintStore* bpStore = static_cast<BlueprintStore*>(building);
    ...
    {
      BlueprintConsole* console = static_cast<BlueprintConsole*>(building);
