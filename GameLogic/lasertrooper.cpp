...
void LaserTrooper::Render(double predictionTime, int teamId)
{
  ...
  //	}
  //	entityUp.FastRotateAround(m_front, sin(wobble) * 0.1);
  entityUp.Normalise();
  ...
  if (!m_dead)
  {
    ...
    if (alpha > 0.5)
    {
      ...
      glBegin(GL_QUADS);
      glTexCoord2f(0.0, 1.0);
      glVertex3dv((predictedPos - entityFront * 1.5 + entityUp).GetData());
      glTexCoord2f(1.0, 1.0);
      glVertex3dv((predictedPos + entityFront * 1.5 + entityUp).GetData());
      glTexCoord2f(1.0, 0.0);
      glVertex3dv((predictedPos + entityFront * 1.5).GetData());
      glTexCoord2f(0.0, 0.0);
      glVertex3dv((predictedPos - entityFront * 1.5).GetData());
      glEnd();
