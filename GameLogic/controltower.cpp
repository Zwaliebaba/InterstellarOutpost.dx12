    ...
    m_checkTargetTimer(0.0)
{
  ...
  constexpr char lightPosName[] = "MarkerLightPos";
  m_lightPos = m_shape->m_rootFragment->LookupMarker(lightPosName);
  ASSERT_TEXT(m_lightPos, "ControlTower: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", lightPosName, m_shape->m_name);

  constexpr char dishPosName[] = "MarkerDishPos";
  m_dishPos = m_shape->m_rootFragment->LookupMarker(dishPosName);
  ASSERT_TEXT(m_dishPos, "ControlTower: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", dishPosName, m_shape->m_name);
  ...
  for (int i = 0; i < 3; ++i)
  {
    ...
    ASSERT_TEXT(m_reprogrammer[i], "ControlTower: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", markerName,
                m_shape->m_name);

    sprintf(markerName, "MarkerConsole%d", i);
    m_console[i] = m_shape->m_rootFragment->LookupMarker(markerName);
    ASSERT_TEXT(m_console[i], "ControlTower: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", markerName,
                m_shape->m_name);
  }
...
void ControlTower::RenderAlphas(double _predictionTime)
{
  ...
  if (static_cast<int>(GetHighResTime()) > s_lastRecalculation)
  {
    ...
    if (nearest < 200.0)
      s_desiredDistanceScale = 1.0;
    else
      s_desiredDistanceScale = 0.1;
  }

  if (s_desiredDistanceScale > s_distanceScale)
  {
    s_distanceScale = (s_desiredDistanceScale * SERVER_ADVANCE_PERIOD * 0.1) + (s_distanceScale * (1.0 - SERVER_ADVANCE_PERIOD * 0.1));
  }
  else
  {
    s_distanceScale = (s_desiredDistanceScale * SERVER_ADVANCE_PERIOD * 0.03) + (s_distanceScale * (1.0 - SERVER_ADVANCE_PERIOD * 0.03));
  }
  ...
  for (int i = 0; i < 10; ++i)
  {
    Vector3 thisUp1 = Vector3(0, -20, 0) + controlUp * i;
    Vector3 thisUp2 = Vector3(0, -20, 0) + controlUp * (i + 1);

    int alpha = 255 - 255 * static_cast<double>(i) / 10.0;
    int alpha2 = 255 - 255 * static_cast<double>(i + 1) / 10.0;

    alpha *= abs(sin(g_gameTime * 2 + static_cast<double>(i) / 5.0));
    alpha2 *= abs(sin(g_gameTime * 2 + static_cast<double>(i + 1) / 5.0));

    alpha *= s_distanceScale;
    alpha2 *= s_distanceScale;

    double y = static_cast<double>(i) / 10.0;
    double h = 1.0 / 10.0;
    ...
    glTexCoord2f(y, 0);
    glVertex3dv((lightPos - camR * w + thisUp1).GetData());
    glTexCoord2f(y, 1);
    glVertex3dv((lightPos + camR * w + thisUp1).GetData());
    ...
    glTexCoord2f(y + h, 1);
    glVertex3dv((lightPos + camR * w + thisUp2).GetData());
    glTexCoord2f(y + h, 0);
    glVertex3dv((lightPos - camR * w + thisUp2).GetData());
    glEnd();
  }
