...
class GameTypeButton : public GameMenuButton
{
  public:
    ...
    void Render(int realX, int realY, bool highlighted, bool clicked) override
    {
      ...
      if (g_app->m_globalWorld->m_myTeamId != 255 || g_app->m_spectator || parent->m_settingUpCustomGame)
      {
        ...
        if (highlighted)
          static_cast<GameMenuWindow*>(m_parent)->m_highlightedGameType = m_gameType;
      }
    }

    void MouseUp() override
    {
      auto main = static_cast<GameMenuWindow*>(EclGetWindow(/*"GameMenu"*/"multiwinia_mainmenu_title"));
      ...
      if (g_app->m_globalWorld->m_myTeamId != 255 || g_app->m_spectator || main->m_settingUpCustomGame)
      {
