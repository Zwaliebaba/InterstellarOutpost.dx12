    ...
    m_numStartingSpirits(0)
{
  ...
  constexpr char spiritCentreName[] = "MarkerSpirits";
  m_spiritCentre = m_shape->m_rootFragment->LookupMarker(spiritCentreName);
  ASSERT_TEXT(m_spiritCentre, "Incubator: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", spiritCentreName,
              m_shape->m_name);

  constexpr char exitName[] = "MarkerExit";
  m_exit = m_shape->m_rootFragment->LookupMarker(exitName);
  ASSERT_TEXT(m_exit, "Incubator: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", exitName, m_shape->m_name);

  constexpr char dockName[] = "MarkerDock";
  m_dock = m_shape->m_rootFragment->LookupMarker(dockName);
  ASSERT_TEXT(m_dock, "Incubator: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", dockName, m_shape->m_name);

  constexpr char spiritEntrance0Name[] = "MarkerSpiritIncoming0";
  m_spiritEntrance[0] = m_shape->m_rootFragment->LookupMarker(spiritEntrance0Name);
  ASSERT_TEXT(m_spiritEntrance[0], "Incubator: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", spiritEntrance0Name,
              m_shape->m_name);

  constexpr char spiritEntrance1Name[] = "MarkerSpiritIncoming1";
  m_spiritEntrance[1] = m_shape->m_rootFragment->LookupMarker(spiritEntrance1Name);
  ASSERT_TEXT(m_spiritEntrance[1], "Incubator: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", spiritEntrance1Name,
              m_shape->m_name);

  constexpr char spiritEntrance2Name[] = "MarkerSpiritIncoming2";
  m_spiritEntrance[2] = m_shape->m_rootFragment->LookupMarker(spiritEntrance2Name);
  ASSERT_TEXT(m_spiritEntrance[2], "Incubator: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", spiritEntrance2Name,
              m_shape->m_name);
  ...
  if (m_id.GetTeamId() != 255)
  {
    if (m_spirits.NumUsed() > 0)
    {
      ...
      if (m_timer <= 0.0)
      {
        ...
        {
          m_timer = INCUBATOR_PROCESSTIME;
          double overCrowded = static_cast<double>(m_spirits.NumUsed()) / 10.0;
          m_timer -= overCrowded;
          if (m_timer < 0.5)
            m_timer = 0.5;
        }
...
void Incubator::SpawnEntity()
{
  ...
  if (teamId < 0 || teamId >= NUM_TEAMS || g_app->m_location->m_teams[teamId]->m_teamType == TeamTypeUnused)
    return;
