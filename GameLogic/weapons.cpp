...
void ThrowableWeapon::Render(double _predictionTime)
{
  ...
  int numFlashes = static_cast<int>(GetNetworkTime() - m_birthTime);
...
double ThrowableWeapon::GetApproxMaxRange(double _maxForce) { return _maxForce + (_maxForce * 0.75) + (_maxForce * 0.75 * 0.75); }

void ThrowableWeapon::RemoveForce() { m_force = 0.0; }

void ThrowableWeapon::ResetForce() { m_force = 1.0; }

// ****************************************************************************
//  Class Grenade
// ****************************************************************************

Grenade::Grenade(const Vector3& _startPos, const Vector3& _front, double _force)
  : ThrowableWeapon(EffectThrowableGrenade, _startPos, _front, _force),
    m_life(3.0),
    m_power(25.0),
    m_explodeOnImpact(false),
    m_collected(false) { m_colour.Set(255, 100, 100); }
...
AirStrikeMarker::AirStrikeMarker(const Vector3& _startPos, const Vector3& _front, double _force)
...
bool AirStrikeMarker::Advance()
{
  ...
  if (GetNetworkTime() > m_birthTime + 2.0f && m_force <= 0.1f)
  {
    ...
    {
      // DebugTrace("Creating AirStrike Unit at %2.2f\n", GetNetworkTime() );
...
ControllerGrenade::ControllerGrenade(const Vector3& _startPos, const Vector3& _front, double _force)
...
bool ControllerGrenade::Advance()
{
  ...
  if (GetNetworkTime() > m_birthTime + 3.0)
  {
    ...
    for (int i = 0; i < 50; ++i)
    {
      auto thisVector = Vector3(50, 0, 0);
      thisVector.RotateAroundY(2.0 * M_PI * static_cast<double>(i) / 50.0);
...
void Rocket::Explode()
{
  ...
  if (m_firework)
  {
    ...
    {
      Vector3 vel(sfrand(30.0f), 10.0f + frand(10.0f), sfrand(30.0f));
      float size = 120.0f + frand(120.0f);
      g_app->m_particleSystem->CreateParticle(m_pos + g_upVector * 15.0f * 0.3f, vel, Particle::TypeExplosionCore, size, colour);
    }
  }
  ...
  auto g = static_cast<GunTurret*>(g_app->m_location->GetBuilding(m_fromBuildingId));
...
bool Laser::Advance()
{
  if (m_fromTeamId == -1 || m_fromTeamId == 255)
    return true;
  m_life -= SERVER_ADVANCE_PERIOD;
  if (m_life <= 0.0)
    return true;
  ...
  else
  {
    ...
    if (!m_harmless)
    {
      ...
      for (int i = 0; i < numFound; ++i)
      {
        ...
        double verticalHit = g_app->IsSinglePlayer() || abs(entity->m_pos.y - m_pos.y) < 5.0;
        ...
        if (verticalHit && PointSegDist2D(Vector2(entity->m_pos), Vector2(rayStart), Vector2(rayEnd)) < 5.0)
        {
  ...
  //entity->ChangeHealth(-1);
...
else*/
          {
            ...
            {
              WorldObjectId id = g_app->m_location->SpawnEntities(entity->m_pos, m_fromTeamId, -1, Entity::TypeDarwinian, 1, entity->m_vel,
                                                                  0.0);
            ...
            else
            {
              ...
              bool success = entity->ChangeHealth(static_cast<int>(-damage), Entity::DamageTypeLaser);
              ...
              else if (!dead && !entity->m_dead)
              {
                if (entity->m_type == Entity::TypeDarwinian)
                {
                  ...
                  g_app->m_soundSystem->TriggerOtherEvent(entity->m_pos, "InjuredEntity", SoundSourceBlueprint::TypeLaser,
                                                          m_id.GetTeamId());
                }
              }
              if (success)
              {
                ...
                if (entity->m_type != Entity::TypeInsertionSquadie && entity->m_type != Entity::TypeVirii && entity->m_type !=
                  Entity::TypeShaman)
                {
                  entity->m_front = push;
                  entity->m_front.Normalise();
                }
                if (entity->m_type != Entity::TypeVirii && entity->m_type != Entity::TypeShaman)
                  entity->m_vel += push;
...
void Laser::Render(double predictionTime)
{
  ...
  if (!g_app->m_multiwinia->GameOver() && !g_app->GamePaused())
  {
    if (camDistSqd < 200.0)
  ...
  for (int i = 0; i < numFound; ++i)
  {
    ...
    if (abs(distance - radius) < 10.0)
    {
      ...
      if (!dead && ent->m_dead)
      {
        ...
        {
          if ((ent->m_type == Entity::TypeDarwinian && m_teamId >= 0 && m_teamId < NUM_TEAMS) || g_app->IsSinglePlayer())
      ...
      {
        if (ent->m_type != Entity::TypeInsertionSquadie && ent->m_type != Entity::TypeOfficer && ent->m_type != Entity::TypeShaman)
        {
...
void Shockwave::Render(double predictionTime)
{
  ...
  {
    double xDiff = radius * sin(angle);
    double zDiff = radius * cos(angle);
  ...
  if (m_size - predictedLife < 0.1 && !g_app->m_multiwinia->GameOver() && !g_app->GamePaused())
  {
  ...
  if (m_size - predictedLife < 1.0)
  {
    ...
    double size = (m_size * 500.0) / sqrt(distToBang);
    ...
    size = (m_size * 1000.0) / sqrt(distToBang);
...
MuzzleFlash::MuzzleFlash()
  : WorldObject(),
    m_size(1.0),
    m_life(1.0) { m_type = EffectMuzzleFlash; }

MuzzleFlash::MuzzleFlash(const Vector3& _pos, const Vector3& _front, double _size, double _life)
  : WorldObject(),
    m_front(_front),
    m_size(_size),
    m_life(_life)
...
void MuzzleFlash::Render(double _predictionTime)
{
  ...
  for (int i = 0; i < 5; ++i)
  {
    ...
    glVertex3dv((fromPos + rightAngle).GetData());
    glTexCoord2i(1, 1);
    glVertex3dv((fromPos + toPosToFromPos + rightAngle).GetData());
    glTexCoord2i(1, 0);
    glVertex3dv((fromPos + toPosToFromPos - rightAngle).GetData());
  }
  glEnd();
  ...
  : WorldObject()
{
  ...
  constexpr char boosterName[] = "MarkerBooster";
  m_booster = m_shape->m_rootFragment->LookupMarker(boosterName);
  ASSERT_TEXT(m_booster, "Missile: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", boosterName, m_shape->m_name);

  int rocketResearch = g_app->m_globalWorld->m_research->CurrentLevel(GlobalResearch::TypeRocket);
  m_life = 5.0 + rocketResearch * 5.0;
}

bool Missile::AdvanceToTargetPosition(const Vector3& _pos)
{
...
bool Missile::Advance()
{
  ...
  auto tank = static_cast<Tank*>(g_app->m_location->GetEntitySafe(m_tankId, Entity::TypeTank));
  if (tank)
...
void Missile::Explode() { g_app->m_location->Bang(m_pos, 20.0, 100.0); }
...
TurretShell::TurretShell(double _life)
  : WorldObject(),
    ...
    m_fromBuildingId(-1) { m_type = EffectGunTurretShell; }
...
bool TurretShell::Advance()
{
      ...
      if (entity->RayHit(oldPos, rayDir))
      {
        ...
        if (!dead && entity->m_dead && m_id.GetTeamId() >= 0 && m_id.GetTeamId() < NUM_TEAMS)
        {
          ...
          if (entity->m_type == Entity::TypeArmour)
            numKills += static_cast<Armour*>(entity)->GetNumPassengers();
          ...
          if (building && building->m_type == Building::TypeGunTurret)
            static_cast<GunTurret*>(building)->RegisterKill(entity->m_id.GetTeamId(), numKills);
        }
...

  {
    ...
    for (int b = 0; b < buildings->Size(); ++b)
    {
      ...
      if (building)
      {
        bool allowedHit = (building->m_type == Building::TypeTree || building->m_type == Building::TypeAntHill || building->m_type ==
          Building::TypeTriffid);
  ...
  if (m_pos.y <= landHeight || midPoint.y <= midLandHeight)
  {
    Vector3 groundPos;
...
void TurretShell::Render(double predictionTime)
{
  ...
  if (building && building->m_type == Building::TypeGunTurret)
  {
    ...
    if (myTeam && myTeam->m_currentBuildingId == m_fromBuildingId)
  ...
  glBegin(GL_LINES);
  glColor4f(1.0f, 1.0f, 1.0f, alpha);
  glVertex3dv(predictedPos.GetData());
  glColor4f(1.0f, 1.0f, 1.0f, 0.0f);
  glVertex3dv((predictedPos - m_vel * lineLen).GetData());
  glEnd();
  glShadeModel(GL_FLAT);
  glLineWidth(1.0);
}
...
Fireball::Fireball()
  : WorldObject(),
    ...
    m_fromBuildingId(-1) { m_type = EffectFireball; }
...
bool Fireball::Advance()
{
  ...
  m_radius = 20.0 * std::min(1.0, abs(1.0 - m_life));

  int numFound;
  g_app->m_location->m_entityGrid->GetNeighbours(s_neighbours, m_pos.x, m_pos.z, m_radius, &numFound);

  bool registerKills = false;
  auto building = static_cast<GunTurret*>(g_app->m_location->GetBuilding(m_fromBuildingId));
  if (building && building->m_type == Building::TypeGunTurret)
    registerKills = true;
  ...
  for (int i = 0; i < numFound; ++i)
  {
    ...
    if (d && !Damaged(d->m_id.GetUniqueId()))
    {
      ...
      if (oldState != Darwinian::StateOnFire && d->m_state == Darwinian::StateOnFire && registerKills)
...
{
  if (g_gameTime - m_birthTime < 0.3 && !g_app->m_multiwinia->GameOver())
  {
    double distance = (g_app->m_camera->GetPos() - m_pos).Mag();
    double distanceFactor = 1.0 - (distance / 500.0);
    if (distanceFactor < 0.0)
  ...
  double startAngle = static_cast<int>(g_gameTime) % static_cast<int>(2.0 * M_PI);
  ...
  for (int i = 0; i <= numSteps; ++i)
  {
    double colMod = 0.25 + (abs(sin(angle * 2 + timeFactor)) * 0.75);
    double alpha = 0.2 + (abs(sin(angle * 2 + timeFactor)) * 0.8);
    ...
    double xDiff = (m_radius + (m_radius * 0.3 * _predictionTime)) * sin(angle);
    double zDiff = (m_radius + (m_radius * 0.3 * _predictionTime)) * cos(angle);
    Vector3 pos = m_pos + Vector3(xDiff, 5, zDiff);
    ...
    pos.y += 200;
    glVertex3dv(pos.GetData());
    angle += totalAngle / static_cast<double>(numSteps);
  }
  glEnd();
  ...
  for (int i = 0; i <= numSteps; ++i)
  {
    double alpha = 0.5 + (abs(sin(angle * 2 + timeFactor)) * 0.5);
    ...
    pos.y = g_app->m_location->m_landscape.m_heightMap->GetValue(pos.x, pos.z);
    pos.y -= 20;
    glTexCoord2f(i + g_gameTime, 0);
    glVertex3dv(pos.GetData());
    pos.y += 400;
    glTexCoord2f(i + g_gameTime, 1);
    glVertex3dv(pos.GetData());
    angle += totalAngle / static_cast<double>(numSteps);
  }
  glEnd();
  ...
  {
    double colMod = 0.25 + (abs(sin(angle * 2 + timeFactor)) * 0.75);
    double alpha = 0.2 + (abs(sin(angle * 2 + timeFactor)) * 0.7);
    //if( m_radius > g_app->m_location->m_landscape.GetWorldSizeX() * 0.75 )
    {
      alpha *= 1.0 - (m_radius / g_app->m_location->m_landscape.GetWorldSizeX());
    }
...
LandMine::LandMine()
  : WorldObject() { m_shape = g_app->m_resource->GetShape("flying_egg.shp"); }
...
TurretEmptyShell::TurretEmptyShell(const Vector3& pos, const Vector3& front, const Vector3& vel)
{
  m_pos = pos;
  m_front = front;
  m_vel = vel;
...
bool TurretEmptyShell::Advance()
{
  if (m_force > 0.2)
  {
    ...
    axis += Vector3(fmod(m_id.GetUniqueId() * 0.1, 1.0), fmod(m_id.GetUniqueId() * 0.3, 1.0), fmod(m_id.GetUniqueId() * 0.7, 1.0));
