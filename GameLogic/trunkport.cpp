    ...
    m_populationLock(-1)
{
  ...
  constexpr char destination1Name[] = "MarkerDestination1";
  m_destination1 = m_shape->m_rootFragment->LookupMarker(destination1Name);
  ASSERT_TEXT(m_destination1, "TrunkPort: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", destination1Name,
              m_shape->m_name);

  constexpr char destination2Name[] = "MarkerDestination2";
  m_destination2 = m_shape->m_rootFragment->LookupMarker(destination2Name);
  ASSERT_TEXT(m_destination2, "TrunkPort: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", destination2Name,
              m_shape->m_name);
}
...
{
  m_heightMapSize = static_cast<int>(TRUNKPORT_HEIGHTMAP_MAXSIZE / (double)_detail);
  ...
  constexpr char markerName[] = "MarkerSurface";
  ShapeMarker* marker = m_shape->m_rootFragment->LookupMarker(markerName);
  ASSERT_TEXT(marker, "TrunkPort: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", markerName, m_shape->m_name);
  ...
  for (int x = 0; x < m_heightMapSize; ++x)
  {
    ...
    {
      double fractionX = static_cast<double>(x) / static_cast<double>(m_heightMapSize - 1);
      double fractionZ = static_cast<double>(z) / static_cast<double>(m_heightMapSize - 1);
  ...
  if (m_openTimer > 0.0 || g_app->Multiplayer())
  {
    ...
    for (int x = 0; x < m_heightMapSize; ++x)
    {
      for (int z = 0; z < m_heightMapSize; ++z)
      {
        ...
        double wave1 = cos(centreDif * 0.15);
        double wave2 = cos(centreDif * 0.05);

        Vector3 thisDif = m_front * sin(g_gameTime * 2) * wave1 * (1.0 - fractionOut) * 15 * timeScale;
        thisDif += m_front * sin(g_gameTime * 2.5) * wave2 * (1.0 - fractionOut) * 15 * timeScale;
        thisDif += g_upVector * cos(g_gameTime) * wave1 * timeScale * 10 * (1.0 - fractionOut);
        difMap[x][z] = thisDif;
      }
    }
    ...
    for (int x = 0; x < m_heightMapSize - 1; ++x)
    {
      for (int z = 0; z < m_heightMapSize - 1; ++z)
      {
        ...
        double fractionX = static_cast<double>(x) / static_cast<double>(m_heightMapSize);
        double fractionZ = static_cast<double>(z) / static_cast<double>(m_heightMapSize);
        double width = 1.0 / m_heightMapSize;

        glTexCoord2f(fractionX, fractionZ);
        glVertex3dv(thisPos1.GetData());
        glTexCoord2f(fractionX + width, fractionZ);
        glVertex3dv(thisPos2.GetData());
        glTexCoord2f(fractionX + width, fractionZ + width);
        glVertex3dv(thisPos3.GetData());
        glTexCoord2f(fractionX, fractionZ + width);
        glVertex3dv(thisPos4.GetData());
      }
    }
