...
ViriiUnit::ViriiUnit(int teamId, int unitId, int numEntities, const Vector3& _pos)
...
void ViriiUnit::Render(double _predictionTime)
{
  ...
  else
  {
    ...
    if (entityDetail == 1 && rangeToCam > 1000.0)
      ...
      if (entityDetail == 2 && rangeToCam > 500.0)
        viriiDetail = 2;
...
Virii::Virii()
  : Entity(),
    ...
    m_spiritId(-1),
    m_historyTimer(0.0),
    m_prevPosTimer(0.0) { m_retargetTimer = syncfrand(2.0); }
...
bool Virii::Advance(Unit* _unit)
{
  ...
  if (m_positionHistory.Size() > 0)
  {
    ...
    if (!recorded)
    {
      ...
      if (abs(m_pos.y - lastRecordedHeight) > 3.0)
...
bool Virii::AdvanceToTargetPos(const Vector3& _pos)
{
...
bool Virii::AdvanceAttacking()
{
  ...
  if (arrived)
  {
    ...
    for (int i = 0; i < 3; ++i)
    {
      auto vel = Vector3(SFRAND(15.0), 15.0, SFRAND(15.0));
...
WorldObjectId Virii::FindNearbyEgg(const Vector3& _pos)
{
  int numFound;
...
bool Virii::SearchForIdleDirection()
{
  ...
  if (chanceOfReturn >= 1.0 || syncfrand(1.0) <= chanceOfReturn)
  {
    ...
    if (newDirection.x < -0.5)
      ...
      newDirection.x = 0.0;
    if (newDirection.z < -0.5)
      newDirection.z = -1.0;
    else if (newDirection.z > 0.5)
      newDirection.z = 1.0;
    else
      newDirection.z = 0.0;
  ...
  while (attempts < 3)
  {
    ...
    int x = (static_cast<int>(syncfrand(3.0)) - 1);
    int z = (static_cast<int>(syncfrand(3.0)) - 1);
...
Vector3 Virii::AdvanceDeadPositionVector(int _index, const Vector3& _pos, double _time)
{
...
void Virii::Render(double predictionTime, int teamId, int _detail)
{
  ...
  double health = static_cast<double>(m_stats[StatHealth]) / EntityBlueprint::GetStat(m_type, StatHealth);
...
#define glowTexH        128.0/512.0
  ...
  for (int i = 0; i < lastIndex; i++)
  {
    ...
    const Vector3& glowDiff = prevPos.m_glowDiff;
    ...
    glTexCoord2f(0.0, wormTexYpos);
    glVertex3dv((prevPos.m_pos - wormRightAngle).GetData());
    glTexCoord2f(wormTexW, wormTexYpos);
    glVertex3dv((prevPos.m_pos + wormRightAngle).GetData());
    wormTexYpos += (distance * 6) / 512.0;
    glTexCoord2f(wormTexW, wormTexYpos);
    glVertex3dv((pos + wormRightAngle).GetData());
    glTexCoord2f(0.0, wormTexYpos);
    glVertex3dv((pos - wormRightAngle).GetData());
    ...
    if (_detail < 4)
    {
      ...
      glTexCoord2f(glowTexXpos, 0.0);
      glVertex3dv((prevPos.m_pos - glowRightAngle + glowDiff).GetData());
      glTexCoord2f(1.0, 0.0);
      glVertex3dv((prevPos.m_pos + glowRightAngle + glowDiff).GetData());

      glTexCoord2f(1.0, glowTexH);
      glVertex3dv((pos + glowRightAngle - glowDiff).GetData());
