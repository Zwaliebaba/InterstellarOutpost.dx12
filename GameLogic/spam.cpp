...
static inline double SpamReloadTime() { return SPAM_RELOADTIME - (SPAM_RELOADTIME / 2.0) * g_app->m_difficultyLevel / 10.0; }
...
void Spam::Damage(double _damage)
{
  ...
  int oldHealthBand = static_cast<int>(m_damage / 10.0);
  Building::Damage(_damage);
  m_damage += _damage;
  int newHealthBand = static_cast<int>(m_damage / 10.0);
...
bool SpamInfection::SearchForSpirits()
{
  ...
  if (syncfrand(2.0) < 1.0 && !g_app->Multiplayer())
    return false;
  ...
  for (int i = 0; i < g_app->m_location->m_spirits.Size(); ++i)
  {
    if (g_app->m_location->m_spirits.ValidIndex(i))
    {
      ...
      if (theDist <= SPAMINFECTION_MAXSEARCHRANGE && theDist >= SPAMINFECTION_MINSEARCHRANGE && theDist < nearest && s->m_state ==
        Spirit::StateFloating && s->m_pos.y > 10.0)
      {
        index = i;
        nearest = theDist;
      }
...
void SpamInfection::AdvanceAttackingSpirit()
{
  ...
  if (arrived)
  {
    ...
    if (syncfrand(20.0) < 1.0)
      entityType = Entity::TypeSpider;
    if (g_app->Multiplayer())
    {
      if (Virii::s_viriiPopulation[teamId] >= VIRII_POPULATIONLIMIT && entityType == Entity::TypeVirii)
        noSpawn = true;
    }
...
void SpamInfection::Render(double _time)
{
  ...
  for (int j = 0; j < numRepeats; ++j)
  {
    ...
    {
      double alpha = 1.0 - i / static_cast<double>(maxLength);
      alpha *= 0.75;
      ...
      glVertex3dv((thisPos - rightAngle).GetData());
      glTexCoord2f(0.2, 1.0);
      glVertex3dv((thisPos + rightAngle).GetData());
      glTexCoord2f(0.8, 1.0);
      glVertex3dv((lastPos + rightAngle).GetData());
