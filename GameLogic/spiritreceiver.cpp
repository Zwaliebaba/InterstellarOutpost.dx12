...
Vector3 ReceiverBuilding::GetSpiritLocation()
{
  if (!m_spiritLocation)
  {
    constexpr char spiritLocationName[] = "MarkerSpiritLink";
    m_spiritLocation = m_shape->m_rootFragment->LookupMarker(spiritLocationName);
    ASSERT_TEXT(m_spiritLocation, "ReceiverBuilding: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", spiritLocationName,
                m_shape->m_name);
  }
...
void ReceiverBuilding::BeginRenderUnprocessedSpirits()
{
  ...
  g_app->m_camera->SetupProjectionMatrix(s_nearPlaneStart * 1.1, g_app->m_renderer->GetFarPlane());

}

void ReceiverBuilding::RenderUnprocessedSpirit(const Vector3& _pos, double _life)
{
  ...
  glBegin(GL_QUADS);
  glTexCoord2i(0, 0);
  glVertex3dv((position + camUp * 3 * scale).GetData());
  glTexCoord2i(1, 0);
  glVertex3dv((position + camRight * 3 * scale).GetData());
  glTexCoord2i(1, 1);
  glVertex3dv((position - camUp * 3 * scale).GetData());
  glTexCoord2i(0, 1);
  glVertex3dv((position - camRight * 3 * scale).GetData());
  ...
  glBegin(GL_QUADS);
  glTexCoord2i(0, 0);
  glVertex3dv((position + camUp * 1 * scale).GetData());
  glTexCoord2i(1, 0);
  glVertex3dv((position + camRight * 1 * scale).GetData());
  glTexCoord2i(1, 1);
  glVertex3dv((position - camUp * 1 * scale).GetData());
  glTexCoord2i(0, 1);
  glVertex3dv((position - camRight * 1 * scale).GetData());
  ...
  if (buildingDetail == 1)
  {
    ...
    glBegin(GL_QUADS);
    glTexCoord2i(0, 0);
    glVertex3dv((position + camUp * 30 * scale).GetData());
    glTexCoord2i(1, 0);
    glVertex3dv((position + camRight * 30 * scale).GetData());
    glTexCoord2i(1, 1);
    glVertex3dv((position - camUp * 30 * scale).GetData());
    glTexCoord2i(0, 1);
    glVertex3dv((position - camRight * 30 * scale).GetData());
...
void ReceiverBuilding::RenderUnprocessedSpirit_basic(const Vector3& _pos, double _life)
{
  Vector3 position = _pos;
  Vector3 camUp = g_app->m_camera->GetUp();
  Vector3 camRight = g_app->m_camera->GetRight();
...
void ReceiverBuilding::RenderUnprocessedSpirit_detail(const Vector3& _pos, double _life)
{
  Vector3 position = _pos;
  Vector3 camUp = g_app->m_camera->GetUp();
  Vector3 camRight = g_app->m_camera->GetRight();
...
void SpiritProcessor::GetObjectiveCounter(UnicodeString& _dest)
{
  ...
  swprintf(result, sizeof(result) / sizeof(wchar_t), L"%ls : %2.2", LANGUAGEPHRASE("objective_throughput").m_unicodestring, m_throughput);
    ...
    m_spiritLink(nullptr)
{
  ...
  constexpr char headMarkerName[] = "MarkerHead";
  m_headMarker = m_shape->m_rootFragment->LookupMarker(headMarkerName);
  ASSERT_TEXT(m_headMarker, "SpiritReceiver: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", headMarkerName,
              m_shape->m_name);
  ...
  ASSERT_TEXT(m_spiritLink, "SpiritReceiver: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", spiritLinkName,
              m_headShape->m_name);
}
...
void SpiritReceiver::RenderPorts()
{
  ...
  for (int i = 0; i < GetNumPorts(); ++i)
  {
    ...
    if (!m_statusMarkers[i])
    {
      char name[64];
      sprintf(name, "MarkerStatus0%d", i + 1);
      ASSERT_TEXT(m_statusMarkers[i], "SpiritReceiver: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", name,
                  m_shape->m_name);
    }
...
bool UnprocessedSpirit::Advance()
{
  ...
  if (m_zaxisRate < 0.0)
    m_zaxisRate = 0.0;
  m_hover.x = sin(m_positionOffset) * m_xaxisRate;
  m_hover.y = sin(m_positionOffset) * m_yaxisRate;
  m_hover.z = sin(m_positionOffset) * m_zaxisRate;
