    ...
    m_signal(0.0)
{
  ...
  ASSERT_TEXT(m_focusMarker, "FeedingTube: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", focusMarkerName,
              m_shape->m_name);
}
...
void FeedingTube::Render(double _predictionTime) { Building::Render(_predictionTime); }
...
Vector3 FeedingTube::GetStartPoint() { return GetDishPos(0.0); }
