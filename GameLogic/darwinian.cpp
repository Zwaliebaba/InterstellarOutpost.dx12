...
void Darwinian::Begin()
{
  ...
  if (m_id.GetTeamId() == g_app->m_location->GetFuturewinianTeamId() || (futurewinianTeam != -1 && futurewinianTeam == g_app->m_location->
    GetTeamPosition(m_id.GetTeamId())))
    m_futurewinian = true;
}

bool Darwinian::ChangeHealth(int _amount, int _damageType)
{
  if (m_state == StateInsideArmour || m_state == StateBeingSacraficed)
  {
...
bool Darwinian::SearchForNewTask()
{
  ...
  if (m_followingPlayerOrders && m_state != StateIdle)
  {
  ...
  case StateIdle:
    ...
    if (!newTargetFound)
      newTargetFound = SearchForCarryableBuilding();
    if (!newTargetFound)
      newTargetFound = SearchForCrates();
    if (!newTargetFound)
...
bool Darwinian::Advance(Unit* _unit)
{
  ...
  if (!amIDead && !m_dead && m_onGround && m_state != StateOnFire)
  {
    ...
    case StateAttackingBuilding:
      ...
      break;
    case StateCarryingBuilding:
      amIDead = AdvanceCarryingBuilding();
      break;
    case StateFollowingShaman:
      amIDead = AdvanceFollowingShaman();
      break;
    case StateOpeningCrate:
      amIDead = AdvanceOpeningCrate();
      break;
    case StateRaisingFlag:
      amIDead = AdvanceRaisingFlag();
      break;

    case StateApproachingCarriableBuilding:
      amIDead = AdvanceApproachingCarriableBuilding();
      break;
    }
  }

  if (m_state == StateBeingSacraficed)
    amIDead = AdvanceSacraficing();
  if (m_state == StateInVortex)
    amIDead = AdvanceInVortex();
  if (m_state == StateBeingAbducted)
    amIDead = AdvanceBeingAbducted();

  if (m_boxKiteId.IsValid() && (m_state != StateWorshipSpirit || m_dead))
  {
    auto boxKite = static_cast<BoxKite*>(g_app->m_location->GetEffect(m_boxKiteId));
    if (boxKite)
      boxKite->Release();
  ...
  if (m_controllerId != -1 && !m_dead)
  {
    ...
    Task* task = nullptr;
    if (g_app->Multiplayer())
      task = team->m_taskManager->GetTask(m_controllerId);
    else
      task = g_app->m_location->GetMyTaskManager()->GetTask(m_controllerId);

    InsertionSquad* controller = nullptr;
    if (task)
      controller = static_cast<InsertionSquad*>(g_app->m_location->GetUnit(task->m_objId));

    if (task && controller)
      controller->m_numControlledThisFrame++;
  }

  if (!m_onGround && m_state != StateBeingSacraficed && m_state != StateBeingAbducted)
    AdvanceInAir(_unit);
  ...
  if (m_pos.y < 0.0 && m_state != StateInsideArmour)
  {
  ...
  if (m_hotFeetTaskId != -1)
  {
    ...
    {
      m_hotFeetTaskId = -1;
      m_stats[StatSpeed] = EntityBlueprint::GetStat(TypeDarwinian, StatSpeed);
    }
  }
  ...
  if (g_app->m_multiwinia->m_gameType == Multiwinia::GameTypeAssault && !m_dead)
  {
    if (PulseWave::s_pulseWave)
    {
      if (!g_app->m_location->IsFriend(PulseWave::s_pulseWave->m_id.GetTeamId(), m_id.GetTeamId()))
      {
        ...
        {
          if (g_app->m_location->GetTeamPosition(PulseWave::s_pulseWave->m_id.GetTeamId()) == g_app->m_location->m_levelFile->m_levelOptions
            [LevelFile::LevelOptionFuturewinianTeam])
          {
            g_app->m_location->SpawnEntities(m_pos, PulseWave::s_pulseWave->m_id.GetTeamId(), -1, TypeDarwinian, 1, m_vel, 0.0);
            amIDead = true;
            m_dead = true;
          }
          else
...
int Darwinian::FindNearestUsefulBuilding(double& distance)
{
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    if (g_app->m_location->m_buildings.ValidIndex(i))
    {
      ...
      if (building->m_type == Building::TypeSpawnPoint || building->m_type == Building::TypeMultiwiniaZone || building->m_type ==
        Building::TypeCrate || building->m_type == Building::TypeStatue || building->m_type == Building::TypeGunTurret || building->m_type
        == Building::TypeSolarPanel)
      {
...
WorldObjectId Darwinian::FindNearestUsefulOfficer(double& distance)
{
  ...
  for (int i = 0; i < team->m_specials.Size(); ++i)
  {
    ...
    if (entity && !entity->m_dead && entity->m_type == TypeOfficer)
    {
      ...
      if (officer->m_orders == Officer::OrderGoto || officer->m_orders == Officer::OrderFollow)
      {
...
bool Darwinian::AdvanceIdle()
{
  ...
  if (g_app->Multiplayer())
  {
    ...
    if (m_idleTimer <= 0.0)
    {
      ...
      if (nearestBuildingId != -1 && distance < 500)
        newTargetFound = true;
      ...
      if (newTargetFound)
      {
        ...
        m_wayPoint.x += radius * sin(theta);
        m_wayPoint.z += radius * cos(theta);
...
bool Darwinian::AdvanceWatchingSpectacle()
{
  ...
  targetPos += Vector3(sin(g_gameTime) * 30.0, cos(g_gameTime) * 20.0, sin(g_gameTime) * 25.0);
...
bool Darwinian::AdvanceApproachingCarriableBuilding()
{
  ...
  auto carryable = static_cast<CarryableBuilding*>(g_app->m_location->GetBuilding(m_buildingId));

  if (!carryable || !CarryableBuilding::IsCarryableBuilding(carryable->m_type))
  {
    m_state = StateIdle;
    m_buildingId = -1;
    return false;
...
bool Darwinian::AdvanceCarryingBuilding()
{
  ...
  if (lifting)
  {
    ...
    double percentLifted = carryable->m_numLifters / static_cast<double>(carryable->m_maxLifters);
    if (percentLifted > 1.0)
      percentLifted = 1.0;
    int soundFrequency = 15 + static_cast<int>((1.0 - percentLifted) * 20.0);
...
bool Darwinian::AdvanceApproachingArmour()
{
  ...
  auto armour = static_cast<Armour*>(g_app->m_location->GetEntity(m_armourId));
...
bool Darwinian::AdvanceInsideArmour()
{
  ...
  double theta = syncfrand(M_PI * 2);
  m_pos.x += radius * sin(theta);
  m_pos.z += radius * cos(theta);
    m_vel = ( pos - m_pos );
...
{
  auto ant = static_cast<ArmyAnt*>(g_app->m_location->GetEntity(m_threatId));
  if (!ant || ant->m_dead)
  {
    m_state = StateIdle;
...
bool Darwinian::AdvanceCombat()
{
  ...
  if (!g_app->Multiplayer() && m_id.GetTeamId() == 1)
    soldier = true;

  auto officer = static_cast<Officer*>(g_app->m_location->GetEntitySafe(m_officerId, TypeOfficer));
  ...
  if (!inFormation) //|| !isEntity )
  {
    if (soldier && !m_scared)
    {
      ...
      if (distance < DARWINIAN_FEARRANGE / 2.0 && m_controllerId == -1 && heightDiff > -25)
      {
  ...
  if (isEntity)
  {
    ...
    if (soldier)
    {
      ...
      if (officer)
      {
        ...
        if (static_cast<int>(fmodf(timeFactor + formationIndex, formationSize / 4.0)) == 0)
          fireInFormation = true;

        if (HasRage() && static_cast<int>(fmod(timeFactor + formationIndex, formationSize / 16.0)) == 0)
          fireInFormation = true;
      }
      else
      {
        fireNormally = (attackRate >= 1.0 && (syncrand() % static_cast<int>(attackRate)) == 0);
        if (HasRage())
          fireNormally = true;
      }
      ...
      {
        if (m_subversionTaskId != -1 || m_futurewinian)
    ...
    if (hasGrenade)
    {
      ...
      if (m_grenadeTimer <= 0.0)
      {
        ...
        if (distanceToTarget > 75.0 || m_insane)
        //if( distanceToTarget > 50.0 )
        {
          ...
          if (numPlayers == 0)
          {
            ...
            if (ourLandNormal.y > 0.7 && targetLandNormal.y > 0.7 && heightDiff > -50)
            {
              bool grenadeRequired = entity->m_type == TypeSporeGenerator || entity->m_type == TypeSpider || entity->m_type ==
                TypeTriffidEgg || entity->m_type == TypeInsertionSquadie || entity->m_type == TypeArmour || entity->m_type == TypeTank;

              if (grenadeRequired)
              {
                double targetHeight = entity->m_pos.y - g_app->m_location->m_landscape.m_heightMap->GetValue(
                  entity->m_pos.x, entity->m_pos.z);
                if (targetHeight < 40.0)
                  throwGrenade = true;
              }
              else
              {
            ...
            if (entity->m_type == TypeSpaceShip)
              throwGrenade = false;
...
Vector3 GetCrateOffset(int id)
{
  ...
  Vector3 result(sin(timeIndex * 2.5) * 3, cos(timeIndex) * 6, sin(timeIndex * 1.5) * 4);
...
bool Darwinian::AdvanceOpeningCrate()
{
  ...
  //if( int( g_gameTimer.GetGameTime() * 10 ) % m_id.GetUniqueId() == 0 )
  {
    ...
    if (distance < CRATE_SCAN_RANGE * 1.1)
    {
      ...
      double percentDone = static_cast<Crate*>(building)->GetPercentCaptured();
...
bool Darwinian::AdvanceRaisingFlag()
{
  ...
  auto zone = static_cast<MultiwiniaZone*>(building);
...
bool Darwinian::AdvanceUnderControl()
{
  ...
  if (m_officerTimer <= 0.0)
  {
    ...
    Vector3 focusPos = controller->m_focusPos;
...
bool Darwinian::AdvanceBoardingRocket()
{
  ...
  auto rocket = static_cast<EscapeRocket*>(g_app->m_location->GetBuilding(station->GetBuildingLink()));
...
bool Darwinian::AdvanceAttackingBuilding()
{
  ...
  if (distance < 200.0)
  {
    ...
    targetPos.y += 50.0;
    g_app->m_location->ThrowWeapon(m_pos, targetPos, EffectThrowableGrenade, 1);
    m_state = StateIdle;
  }
...
bool Darwinian::SearchForRandomPosition()
{
  ...
  if (syncfrand(1) < 0.7)
  {
    ...
    m_wayPoint = m_pos + Vector3(sin(angle) * distance, 0.0, cos(angle) * distance);
...
bool Darwinian::SearchForFlags()
{
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    if (g_app->m_location->m_buildings.ValidIndex(i))
    {
      ...
      {
        auto zone = static_cast<MultiwiniaZone*>(b);
...
{
  if (m_state == StateBeingSacraficed || m_state == StateInsideArmour || m_state == StateOnFire || m_state == StateOperatingPort /*||
...
void Darwinian::FollowRoute()
{
  ...
  if (m_state != StateFollowingOrders && m_state != StateFollowingOfficer)
    return;
    ...
    for (int i = 0; i < team->m_specials.Size(); ++i)
    {
      ...
      if (entity && !entity->m_dead && entity->m_type == TypeOfficer)
      {
        ...
        if (distance < DARWINIAN_SEARCHRANGE_OFFICERS && officer->m_orders == Officer::OrderGoto)
          officers.PutData(id);
        else if (officer->m_orders == Officer::OrderFollow && distance < DARWINIAN_SEARCHRANGE_OFFICERFOLLOW &&
          /*distance > 50.0 &&*/ distance < nearest)
        {
          if (!officer->m_formation || !officer->FormationFull())
          {
            nearest = distance;
            nearestId = id;
          }
...
bool Darwinian::SearchForShamans()
{
  ...
  if (team)
  {
    ...
    for (int i = 0; i < team->m_specials.Size(); ++i)
    {
      WorldObjectId id = *team->m_specials.GetPointer(i);
      Entity* entity = g_app->m_location->GetEntity(id);
      if (entity && !entity->m_dead && entity->m_type == TypeShaman)
      {
        auto shaman = static_cast<Shaman*>(entity);
        double distance = (shaman->m_pos - m_pos).Mag();
        if (shaman->CallingDarwinians() && distance < DARWINIAN_SEARCHRANGE_OFFICERFOLLOW && distance < nearest)
        {
          nearest = distance;
          nearestId = id;
        }
...
void Darwinian::GiveOrders(const Vector3& _targetPos, int routeId)
{
  m_orders = _targetPos;
  m_ordersBuildingId = -1;
  m_ordersSet = true;
  ...
  {
    auto building = static_cast<CarryableBuilding*>(g_app->m_location->GetBuilding(m_buildingId));

    if (building && CarryableBuilding::IsCarryableBuilding(building->m_type))
    {
...
bool Darwinian::SearchForThreats()
{
  ...
  m_threatRange = (DARWINIAN_SEARCHRANGE_THREATS * threatRangeChange) + (m_threatRange * (1.0 - threatRangeChange));
  ...
  double maxGrenadeRangeSqd = pow(DARWINIAN_SEARCHRANGE_GRENADES, 2);
  ...
  if (!HasRage())
  {
    for (int i = 0; i < g_app->m_location->m_effects.Size(); ++i)
    {
      if (g_app->m_location->m_effects.ValidIndex(i))
      {
        ...
        if (wobj->m_type == EffectThrowableGrenade || wobj->m_type == EffectThrowableAirstrikeMarker || (!g_app->Multiplayer() && wobj->
          m_type == EffectGunTurretTarget && m_id.GetTeamId() != wobj->m_id.GetTeamId()) || (wobj->m_type == EffectSpamInfection && m_id.
          GetTeamId() != wobj->m_id.GetTeamId()))
        {
  ...
  if (syncrand() % 500 == 0)
  {
    ...
    for (int i = 0; i < buildings->Size(); ++i)
    {
      ...
      if (b)
      {
        if ((b->m_type == Building::TypeAntHill || b->m_type == Building::TypeTriffid) && b->m_id.GetTeamId() != m_id.GetTeamId())
        {
          double dist = (m_pos - b->m_pos).Mag();
          if (dist < searchRange && dist < nearest)
          {
            nearest = dist;
            id = i;
          }
  ...
  if (m_controllerId != -1)
  {
    ...
    if (task)
      controller = g_app->m_location->GetUnit(task->m_objId);
    if (controller && controller->m_troopType == TypeInsertionSquadie)
    {
      Vector3 focusPos = static_cast<InsertionSquad*>(controller)->m_focusPos;
  ...
  {
    std::vector<WorldObjectId>* ids = g_app->m_location->m_entityGridCache->GetEnemies(
      searchPos.x, searchPos.z, searchRange, &numFound, m_id.GetTeamId());
    ...
    for (int i = 0; i < numFound; ++i)
    {
      ...
      if (entity)
      {
        ...
        bool beingSacraficed = entity->m_type == TypeDarwinian && static_cast<Darwinian*>(entity)->m_state == StateBeingSacraficed;

        if (!entity->m_dead && !onFire && !beingSacraficed && entity->m_type != TypeEgg && entity->m_type != TypeHarvester && entity->m_type
          != TypeEngineer && (entity->m_type != TypeArmour || static_cast<Armour*>(entity)->GetSpeed() < 15.0))
        {
          ++numEnemies;
  ...
  if (entity && !entity->m_dead)
  {
    ...
    if (m_threatId != threatId)
    {
      ...
      else
      {
        ...
        if (frand(200) < 1.0 && !m_startedThreatSound && (gameTimeNow - lastAttackHorn) > 4.0)
        {
    ...
    m_threatRange = sqrt(nearestThreatSqd);
    END_PROFILE("SearchThreats");
    return true;
  }
...
bool Darwinian::SearchForPorts()
{
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    if (g_app->m_location->m_buildings.ValidIndex(i))
    {
      ...
      {
        if (building->GetNumPortsOccupied() < building->GetNumPorts() && g_app->m_location->IsWalkable(m_pos, building->m_pos))
  ...
  for (int p = 0; p < chosenBuilding->GetNumPorts(); ++p)
  {
    ...
    if (g_app->Multiplayer())
      limit = 1;
    if (!chosenBuilding->GetPortOccupant(p).IsValid() && chosenBuilding->GetPortOperatorCount(p, m_id.GetTeamId()) < limit)
...
bool Darwinian::SearchForCarryableBuilding()
{
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    if (g_app->m_location->m_buildings.ValidIndex(i))
    {
      ...
      {
        auto carriable = static_cast<CarryableBuilding*>(building);

        if (building->m_id.GetTeamId() == m_id.GetTeamId() && carriable->m_numLifters >= carriable->m_maxLifters)
          continue;
...
{
  if (m_paralyzeTimer > 0.0)
    return false; // we're paralyzed, no moving!
    ...
    if (building)
    {
      if (building->m_type == Building::TypeLaserFence && static_cast<LaserFence*>(building)->IsEnabled())
      {
        if (!g_app->m_location->IsFriend(building->m_id.GetTeamId(), m_id.GetTeamId()))
        {
          ...
          else if (building->DoesSphereHit(result, closest))
          {
            ...
            while (building->DoesSphereHit(result, closest)) { result -= pushForce; }
            //m_avoidObstruction = result;
            //m_state = StateIdle;
      ...
      else if (building->m_type == Building::TypeWall)
      {
        if (building->DoesSphereHit(result, 4.0))
        {
          ...
          while (building->DoesSphereHit(result, 1.0)) { result -= pushForce; }
...
bool Darwinian::IsInView() { return g_app->m_camera->PosInViewFrustum(m_pos); }
...
bool Darwinian::AdvanceOnFire()
{
  ...
  if (!m_dead && m_onGround && syncfrand(10) < 2.0 / g_gameTimer.GetGameSpeed())
    ChangeHealth(-2);
...
bool Darwinian::AdvanceSacraficing()
{
  ...
  {
    auto p = static_cast<Portal*>(g_app->m_location->GetBuilding(m_targetPortal));
...
bool Darwinian::AdvanceInVortex()
{
  if (DustBall::s_vortexPos != g_zeroVector)
  {
    ...
    targetPos.x += cos(timeIndex) * radius;
    targetPos.z += sin(timeIndex) * radius;
...
{
  m_onGround = false;
  auto ship = static_cast<SpaceShip*>(g_app->m_location->GetEntity(m_spaceshipId));
  if (ship && ship->m_type == TypeSpaceShip && ship->m_state == SpaceShip::ShipStateAbducting)
  {
    ...
    if ((m_pos - ship->m_pos).Mag() < 20.0 || m_pos.y > ship->m_pos.y)
    {
...
/*bool Darwinian::AdvanceJumpPadLaunch()
{
    ...
    if( pad && pad->m_type == Building::TypeJumpPad )
    {
        ...
        point += (cos(angle)) * (g_upVector * radius);
        point += (sin(angle)) * (direction * radius);
...
void Darwinian::AdvanceHotFeet()
{
  ...
  for (int b = 0; b < buildings->Size(); ++b)
  {
    ...
    if (building)
    {
      if (building->m_type == Building::TypeTree && building->DoesSphereHit(m_pos, 2.0))
      {
...
bool Darwinian::AdvanceInfection()
{
  ...
  if (m_infectionTimer <= 0.0)
  {
    ...
    for (int i = 0; i < numFound; ++i)
    {
      ...
      {
        auto d = static_cast<Darwinian*>(entity);
        if (syncrand() % 2 == 0)
        {
...
void Darwinian::OrbitPortal()
{
  ...
  auto portal = static_cast<Portal*>(g_app->m_location->GetBuilding(m_targetPortal));
  ...
  targetPos.z += sin(timeIndex) * radius;
  targetPos.y -= size;
  targetPos.y += sin(timeIndex * 3.0) * 10.0;
  ...
  if ((m_pos - vortexPos).Mag() <= portal->m_portalRadius + 5.0)
    m_burning = true;
}

void Darwinian::SetFire()
{
  if (m_state != StateInsideArmour && m_state != StateBeingSacraficed && m_boosterTaskId == -1 && m_id.GetTeamId() != g_app->m_location->
    GetMonsterTeamId())
  {
...
void Darwinian::Render(double _predictionTime, double _highDetail)
{
  ...
  if (_highDetail > 0.0 && camDist < 1300)
  {
    ...
    else if (!m_onGround && m_state != StateBeingSacraficed && m_state != StateInWater && m_state != StateBeingAbducted)
      predictedPos.y += 3.0;
    ...
    if (m_state == StateWorshipSpirit || m_state == StateOperatingPort || m_state == StateWatchingSpectacle || m_state == StateOpeningCrate)
    {
      entityUp.RotateAround(m_front * sin(g_gameTimer.GetAccurateTime()) * 0.3);
      entityUp.Normalise();
    }
  }

  if (!m_onGround && m_state != StateBeingSacraficed && m_state != StateInWater && !future)
  {
    Vector3 rotateAmount = m_front * g_gameTimer.GetAccurateTime() * (m_id.GetUniqueId() % 10) * 0.1;

    if (m_id.GetUniqueId() % 2 == 0)
      entityUp.RotateAround(rotateAmount);
    else
      entityUp.RotateAround(rotateAmount * -1.0);
  }
  ...
  {
    if (camDist > 100)
      size *= 1.0 + scaleFactor * (camDist - 100) / 1000;
  ...
  if (_highDetail > 0.0 && m_shadowBuildingId == -1 && !m_dead && m_state != StateBeingAbducted && camDist < maxShadowDistance)
  {
    if (m_futurewinian)
  ...
  if (!m_dead || !m_onGround)
  {
    ...
    double health = static_cast<double>(m_stats[StatHealth]) / maxHealth;
    if (health > 1.0)
      health = 1.0;
    ...
    {
      glEnable(GL_BLEND);
      colour.a = 2.5 * static_cast<double>(m_stats[StatHealth]);
      colour.r *= 0.2;
      colour.g *= 0.2;
      colour.b *= 0.2;
    }

    if (m_state == StateOnFire || m_burning)
    {
      //colour.r *= 0.01;
      //colour.g *= 0.01;
      //colour.b *= 0.01;
    ...
    if (m_id.GetTeamId() != g_app->m_location->GetMonsterTeamId() && !m_futurewinian)
    {
      glColor4ubv(colour.GetData());
      glBegin(GL_QUADS);
      glTexCoord2i(0, 1);
      glVertex3dv((predictedPos - entityRight + entityUp).GetData());
      glTexCoord2i(1, 1);
      glVertex3dv((predictedPos + entityRight + entityUp).GetData());
      glTexCoord2i(1, 0);
      glVertex3dv((predictedPos + entityRight).GetData());
      glTexCoord2i(0, 0);
      glVertex3dv((predictedPos - entityRight).GetData());
      glEnd();
    }
    ...
    if (future)
    {
      ...
      glBegin(GL_QUADS);
      glTexCoord2i(0, 1);
      glVertex3dv((predictedPos - entityRight * factor + entityUp * factor).GetData());
      glTexCoord2i(1, 1);
      glVertex3dv((predictedPos + entityRight * factor + entityUp * factor).GetData());
      glTexCoord2i(1, 0);
      ...
      right *= scale;
      up *= scale;*/
      double alpha = abs(sin(g_gameTime * 2.0)) * 0.3;
      RGBAColour col = g_app->m_location->m_teams[m_id.GetTeamId()]->m_colour;
      ...
      glBegin(GL_QUADS);
      glTexCoord2i(0, 0);
      glVertex3dv((bubblePos - right * bubbleSize - up * bubbleSize).GetData());
      glTexCoord2i(0, 1);
      glVertex3dv((bubblePos - right * bubbleSize + up * bubbleSize).GetData());
      glTexCoord2i(1, 1);
      glVertex3dv((bubblePos + right * bubbleSize + up * bubbleSize).GetData());
    ...
    if (m_id.GetTeamId() == g_app->m_globalWorld->m_myTeamId)
    {
      ...
      if ((m_controllerId != -1 || m_underPlayerControl) && !m_dead)
      {
        SetBlend(GL_SRC_ALPHA, GL_ONE);
        glEnable(GL_BLEND);
        glDisable(GL_DEPTH_TEST);
        double alpha = abs(sin(g_gameTime * 5.0));
        if (alpha < 0.3)
          alpha = 0.3;
    ...
    if (m_boosterTaskId != -1 && !m_dead)
    {
      ...
      right *= scale;
      up *= scale;*/
      double alpha = abs(sin(g_gameTime * 2.0)) * 0.5;
      RGBAColour col = g_app->m_location->m_teams[m_id.GetTeamId()]->m_colour;
...

  {
    ...
    if (crossProduct > 0.95)
    {
      ...
      glBegin(GL_QUADS);
      glTexCoord2i(0, 1);
      glVertex3dv((predictedPos - camRight + entityUp).GetData());
      glTexCoord2i(1, 1);
      glVertex3dv((predictedPos + camRight + entityUp).GetData());
      glTexCoord2i(1, 0);
      glVertex3dv((predictedPos + camRight).GetData());
      glTexCoord2i(0, 0);
      glVertex3dv((predictedPos - camRight).GetData());
      glEnd();
      SetTexture("sprites/darwinian.bmp");
  ...
  if (m_subversionTaskId != -1 && !m_dead)
  {
    ...
    double baseSize = 2.8;
    double waveSize = baseSize + ((g_gameTime) - static_cast<int>(g_gameTime));
  ...
  if (m_dead)
  {
    ...
    entityRight.Normalise();
    entityRight *= size;
    unsigned char alpha = static_cast<double>(m_stats[StatHealth]) * 2.55;
    ...
    for (int i = 0; i < 3; ++i)
    {
      ...
      if (i == 0)
        fragmentPos.x += 10.0 - predictedHealth / 10.0;
      if (i == 1)
        fragmentPos.x += 10.0 - predictedHealth / 10.0;
      if (i == 1)
        fragmentPos.z += 10.0 - predictedHealth / 10.0;
      if (i == 2)
        fragmentPos.x -= 10.0 - predictedHealth / 10.0;
      ...
      glVertex3dv((fragmentPos - entityRight + entityUp).GetData());
      glTexCoord2f(right, bottom);
      glVertex3dv((fragmentPos + entityRight + entityUp).GetData());
      glTexCoord2f(right, top);
      glVertex3dv((fragmentPos + entityRight).GetData());
...
void Darwinian::FireSubversion(const Vector3& _pos)
{
  if (m_reloading == 0.0)
  {
    Vector3 toPos = _pos;
...
bool Darwinian::HasRage() { return (m_rageTaskId != -1 || g_app->m_location->m_bitzkreigTimer > 0.0); }

char* Darwinian::LogState(char* _message)
{
  static char s_result[1024];

  sprintf(s_result, "%sDARWINIAN Id[%d] state[%d] pos[%5.2f,%5.2f,%5.2f] vel[%5.2f] front[%5.2f] waypoint[%5.2f,%5.2f,%5.2f] boxId[%d]",
          (_message) ? _message : "", m_id.GetUniqueId(), m_state, m_pos.x, m_pos.y, m_pos.z, m_vel.x + m_vel.y + m_vel.z,
          m_front.x + m_front.y + m_front.z, m_wayPoint.x, m_wayPoint.y, m_wayPoint.z, m_boxKiteId.GetUniqueId());
...
bool BoxKite::Advance()
{
  if (m_state == StateReleased)
  {
    ...
    m_vel.x = sin(theTime + m_id.GetIndex());
    m_vel.z = sin(theTime + m_id.GetIndex());
...
void BoxKite::Render(double _predictionTime)
{
  ...
  glBegin(GL_QUADS);
  glTexCoord2i(0, 0);
  glVertex3dv((predictedPos - camRight + camUp).GetData());
  glTexCoord2i(1, 0);
  glVertex3dv((predictedPos + camRight + camUp).GetData());
  glTexCoord2i(1, 1);
  glVertex3dv((predictedPos + camRight - camUp).GetData());
...
char* BoxKite::LogState(char* _message)
{
  ...
  sprintf(s_result, "%sBOXKITE Id[%d] state[%d] pos[%5.2f], vel[%5.2f]", (_message) ? _message : "", m_id.GetUniqueId(), m_state,
