...
SpawnBuildingLink::~SpawnBuildingLink() { m_spirits.EmptyAndDelete(); }
...
void SpawnBuilding::RenderSpirit(const Vector3& _pos, int teamId)
{
  Vector3 pos = _pos;
...
{
  return (_type == TypeSpawnPoint || _type == TypeSpawnLink || _type == TypeSpawnPointMaster);
}
    ...
    if (building && IsSpawnBuilding(building->m_type))
    {
      ...
      for (int j = 0; j < link->m_spirits.Size(); ++j)
      {
        ...
        if (predictedProgress >= 0.0 && predictedProgress <= 1.0)
        {
          ...
          if (targetBuilding)
            targetTeamId = targetBuilding->m_id.GetTeamId();
...
Vector3 SpawnBuilding::GetSpiritLink()
{
  if (!m_spiritLink)
  {
    ...
    constexpr char spiritLinkName[] = "MarkerSpiritLink";
    m_spiritLink = m_shape->m_rootFragment->LookupMarker(spiritLinkName);
    ASSERT_TEXT(m_spiritLink, "SpawnBuilding: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", spiritLinkName,
                m_shape->m_name);
  }
...
void SpawnBuilding::Render(double _predictionTime) { Building::Render(_predictionTime); }
    ...
    m_activationTimer(30.0)
{
  ...
  constexpr char doorMarkerName[] = "MarkerDoor";
  m_doorMarker = m_shape->m_rootFragment->LookupMarker(doorMarkerName);
  ASSERT_TEXT(m_doorMarker, "SpawnPoint: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", doorMarkerName,
              m_shape->m_name);
...
void SpawnPoint::RecalculateOwnership()
{
  ...
  {
    if (teamCount[i] > 0 && winningTeam == 255)
      winningTeam = i;
    else
    {
      if (winningTeam != 255 && teamCount[i] > 2 && teamCount[i] > teamCount[winningTeam])
        winningTeam = i;
    }

    if (winningTeam != 255 && teamCount[i] > 0)
    {
    ...
    if (winningTeam != 255 && teamCount[winningTeam] < 3)
      winningTeam = 255;
  }
  ...
  {
    if (winningTeam == 255)
      g_app->m_soundSystem->TriggerBuildingEvent(this, "TurnOff");
    else
      g_app->m_soundSystem->TriggerBuildingEvent(this, "TurnOn");
  }
...
void SpawnPoint::SpawnDarwinian()
{
  ...
  auto darwinian = static_cast<Darwinian*>(g_app->m_location->GetEntitySafe(spawned, Entity::TypeDarwinian));
  if (darwinian)
    ...
    if (m_id.GetTeamId() != 255 && !PopulationLocked())
    {
      ...
      if (m_spawnTimer <= 0.0 || g_app->m_location->m_spawnManiaTimer > 0.0)
      {
        ...
        if (g_app->Multiplayer())
        {
          ...
          ++m_spawnCounter;
          if (m_spawnCounter >= static_cast<int>(20 * CalculateHandicap(m_id.GetTeamId())))
          {
            m_spawnCounter = 0;
...
void SpawnPoint::Render(double _predictionTime) { SpawnBuilding::Render(_predictionTime); }
...
void SpawnPoint::RenderAlphas(double _predictionTime)
{
  ...
  for (int i = 0; i < maxBlobs; ++i)
  {
    ...
    pos += Vector3(0, 25, 0);
    pos.x += sin(timeIndex + i) * i * 0.7;
    pos.y += cos(timeIndex + i) * sin(i * 10) * 12;
    pos.z += cos(timeIndex + i) * i * 0.7;

    double size = 10.0 + sin(timeIndex + i * 10) * 10.0;
    size = max(size, 5.0);
...
{
  if (g_app->m_location->m_levelFile->m_levelOptions[LevelFile::LevelOptionNoHandicap] > 0)
    return 1.0;
  if (_teamId == 255)
    return 0.0;
  if (s_numSpawnPoints[0] > 0)
    return 1.0; // There are still some unoccupied spawn points on the level - no handicap
  ...
  {
    if (g_app->m_location->m_teams[i - 1]->m_teamType == TeamTypeUnused)
      continue;
  ...
  if (numTeams == 0)
    return 1.0; // dont bother with handicap if the player is by himself for whatever reason

  double average = totalSpawnPoints / numTeams;

  if (average > s_numSpawnPoints[_teamId + 1] * 1.5)
  {
    double factor = 1 - ((s_numSpawnPoints[_teamId + 1] - 1) / total);
    return (1.0 + (handicap / 100.0 * factor));
  }
...
bool SpawnPopulationLock::Advance()
{
  ...
  if (g_app->Multiplayer() && g_app->m_location->m_levelFile->m_populationCap != -1)
    return false;
