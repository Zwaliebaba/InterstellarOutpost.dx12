...
Nuke::Nuke()
  : Entity(),
    ...
    m_subTimer(2.5),
    m_launchTimer(-1.0f),
    m_renderMarker(false) { m_type = TypeNuke; }

Nuke::~Nuke() { m_history.EmptyAndDelete(); }
...
bool Nuke::AdvanceLaunched()
{
  ...
  else
  {
    ...
    if (m_casualtyMessageTimer <= 0.0f)
      return true;
  }
  return false;
}
...
void Nuke::RenderHistory(double _predictionTime)
{
  if (m_history.Size() > 0 && m_id.GetTeamId() != 255)
  {
    ...
    for (int i = 0; i < m_history.Size(); ++i)
    {
      ...
      lastPos += diff * 0.1f;
      if (m_id.GetTeamId() != g_app->m_location->GetMonsterTeamId())
        colour.a = 255 - 255 * static_cast<float>(i) / static_cast<float>(m_history.Size());
