...
void CloneLab::Initialise(Building* _template) { Incubator::Initialise(_template); }
...
{
  if (!m_aiTarget && NumSpiritsInside() > 0)
    InitialiseAITarget();
  ...
  if (NumSpiritsInside() == 0 && m_aiTarget)
  {
...
void CloneLab::Read(TextReader* _in, bool _dynamic) { Incubator::Read(_in, _dynamic); }
