...
class ApplyOtherButton : public GameMenuButton
{
  public:
    ...
    void MouseUp() override
    {
      ...
      if (g_prefsManager->GetInt(OTHER_USEPORTFORWARDING) != parent->m_usePortForwarding || g_prefsManager->GetInt(OTHER_SERVERPORT) !=
        parent->m_serverPort || g_prefsManager->GetInt(OTHER_CLIENTPORT) != parent->m_clientPort)
      {
    ...
    void Render(int realX, int realY, bool highlighted, bool clicked) override
    {
      ...
      for (int i = 0; i < parent->m_languages.Size(); ++i)
      {
        if (stricmp(parent->m_languages[i], g_prefsManager->GetString(OTHER_LANGUAGE)) == 0)
          if (parent->m_language == i)
            langMatch = true;
      }

      if (parent->m_helpEnabled == g_prefsManager->GetInt(OTHER_HELPENABLED) && langMatch && g_prefsManager->GetInt(OTHER_USEPORTFORWARDING)
        == parent->m_usePortForwarding && g_prefsManager->GetInt(OTHER_SERVERPORT) == parent->m_serverPort && g_prefsManager->
        GetInt(OTHER_CLIENTPORT) == parent->m_clientPort)
        m_inactive = true;
      else
        m_inactive = false;
...
void PrefsOtherWindow::ListAvailableLanguages()
{
  ...
  {
    const char* lang = fileList[i].c_str();
    char* dot = (char*)strrchr(lang, '.');
    if (dot)
      *dot = '\x0';
    m_languages.PutData(_strdup(lang));
  }
}
...
void PrefsOtherWindow::Create()
{
  ...
  auto usePortforwarding = new GameMenuCheckBox();
  usePortforwarding->SetShortProperties("dialog_useportforwarding", x, y += border, buttonW, buttonH,
                                        LANGUAGEPHRASE("dialog_useportforwarding"));
  ...
  auto serverPort = new GameMenuInputField();
  ...
  auto clientPort = new GameMenuInputField();
  ...
  auto language = new GameMenuDropDown();
