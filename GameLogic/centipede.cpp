...
void Centipede::Begin()
{
  ...
  if (!m_next.IsValid())
  {
    ...
    if (myUnit)
    {
      double size = 0.2 * pow(1.1, myUnit->m_entities.Size());
      size = min(size, 10.0);
...
bool Centipede::Advance(Unit* _unit)
{
  ...
  if (m_next.IsValid())
  {
    ...
    auto centipede = static_cast<Centipede*>(g_app->m_location->GetEntitySafe(m_next, TypeCentipede));
  ...
  if (maxHealth > 255)
    maxHealth = 255;
  double healthFraction = static_cast<double>(m_stats[StatHealth]) / maxHealth;
  ...
  {
    auto prev = static_cast<Centipede*>(g_app->m_location->GetEntitySafe(m_prev, TypeCentipede));
    if (prev)
    {
...
void Centipede::Attack(const Vector3& _pos)
{
  int numFound;
...
int Centipede::GetSize()
{
  ...
  if (m_prev.IsValid())
  {
    auto c = static_cast<Centipede*>(g_app->m_location->GetEntitySafe(m_prev, Entity::TypeCentipede));
    while (c && c->m_prev.IsValid())
    {
      count++;
      c = static_cast<Centipede*>(g_app->m_location->GetEntitySafe(c->m_prev, Entity::TypeCentipede));
    }
  }
  return count;
}
...
void Centipede::EatSpirits()
{
  ...
  if (m_numSpiritsEaten >= required)
  {
    ...
    while (true)
    {
      auto centipede = static_cast<Centipede*>(g_app->m_location->GetEntitySafe(tail->m_prev, TypeCentipede));
      if (!centipede)
        break;
      tail = centipede;
    }
    ...
    if (g_app->Multiplayer())
      numRequiredToEat = CENTIPEDE_NUMSPIRITSTOREGROW_MP;
