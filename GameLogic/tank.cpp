...
Tank::Tank()
  : Entity(),
    m_shapeTurret(nullptr),
    m_markerTurret(nullptr),
    m_markerBarrelEnd(nullptr),
    m_speed(0.0),
    m_attackTimer(0.0),
    ...
    m_mineTimer(0.0) { SetType(TypeTank); }
...
void Tank::Begin()
{
  ...
  constexpr char markerTurretName[] = "MarkerTurret";
  m_markerTurret = m_shape->m_rootFragment->LookupMarker(markerTurretName);
  ASSERT_TEXT(m_markerTurret, "Tank: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", markerTurretName, m_shape->m_name);

  constexpr char markerEntranceName[] = "MarkerEntrance";
  m_markerEntrance = m_shape->m_rootFragment->LookupMarker(markerEntranceName);
  ASSERT_TEXT(m_markerEntrance, "Tank: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", markerEntranceName,
              m_shape->m_name);

  constexpr char markerBarrelEndName[] = "MarkerBarrelEnd";
  m_markerBarrelEnd = m_shapeTurret->m_rootFragment->LookupMarker(markerBarrelEndName);
  ASSERT_TEXT(m_markerBarrelEnd, "Tank: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", markerBarrelEndName,
              m_shapeTurret->m_name);
...
bool Tank::ChangeHealth(int _amount, int _damageType)
{
  ...
  if (!m_dead)
  {
    ...
    double fraction = _amount / EntityBlueprint::GetStat(TypeTank, StatHealth);
...
bool Tank::IsLoading() { return (m_state == StateLoading && m_numPassengers < TANK_CAPACITY); }

bool Tank::IsUnloading()
{
  return (m_state == StateUnloading && m_numPassengers > 0 && GetHighResTime() >= m_previousUnloadTimer + TANK_UNLOADPERIOD);
}

void Tank::AddPassenger() { ++m_numPassengers; }
...
void Tank::SetMissileTarget(const Vector3& _startRay, const Vector3& _rayDir)
{
  //
  ...
  if (hit) { m_missileTarget = landscapeHit; }
}

void Tank::SetMissileTarget(const Vector3& _target) { m_missileTarget = _target; }

Vector3 Tank::GetMissileTarget() { return m_missileTarget; }
...
void Tank::Render(double _predictionTime)
{
  ...
  predictedPos.y += sin(g_gameTime + m_id.GetUniqueId()) * 2;
  Vector3 predictedUp = g_app->m_location->m_landscape.m_normalMap->GetValue(predictedPos.x, predictedPos.z);
  predictedUp.x += sin((g_gameTime + m_id.GetUniqueId()) * 2) * 0.05;
  predictedUp.z += cos(g_gameTime + m_id.GetUniqueId()) * 0.05;
  ...
  if (m_renderDamaged)
  {
    ...
    if (frand() > 0.5)
      bodyMat.r *= (1.0 + sin(g_gameTime) * 0.5);
    else
      bodyMat.f *= (1.0 + sin(g_gameTime) * 0.5);
  }
  ...
  if (m_renderDamaged)
  {
    ...
    if (frand() > 0.5)
      turretMat.r *= (1.0 + sin(g_gameTime) * 0.5);
    else
      turretMat.f *= (1.0 + sin(g_gameTime) * 0.5);
  }
...
bool Tank::IsSelectable() { return true; }
  ...
  if (m_wayPoint == g_zeroVector)
  {
    ...
    for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
    {
      if (g_app->m_location->m_buildings.ValidIndex(i))
      {
        ...
        if (b->m_type == Building::TypeAITarget && b->m_id.GetTeamId() != m_id.GetTeamId() && b->m_id.GetTeamId() != 255)
        {
          if ((m_pos - b->m_pos).Mag() < dist && (m_pos - b->m_pos).Mag() > 50.0)
          {
