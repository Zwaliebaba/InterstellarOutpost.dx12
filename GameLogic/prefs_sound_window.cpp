...
class RestartSoundButton : public GameMenuButton
{
  public:
    ...
    void Render(int realX, int realY, bool highlighted, bool clicked) override
    {
      ...
      if (parent->m_soundLib == 0)
      {
        const char* lib = g_prefsManager->GetString(SOUND_LIBRARY, nullptr);
        if (!lib || strcmp(lib, "software") == 0)
          soundLibMatch = true;
      }
      else
      {
        const char* lib = g_prefsManager->GetString(SOUND_LIBRARY, nullptr);
        if (!lib || strlen(lib) == 0 || strcmp(lib, "dsound") == 0)
          soundLibMatch = true;
      }

      int mixFreq = g_prefsManager->GetInt(SOUND_MIXFREQ);
      if (parent->m_mixFreq == g_prefsManager->GetInt(SOUND_MIXFREQ, 22050) && parent->m_numChannels == g_prefsManager->GetInt(
          SOUND_CHANNELS, 16) &&
#ifdef USE_SOUND_HW3D
...
class HW3DDropDownMenu : public GameMenuCheckBox
{
  public:
    void Render(int realX, int realY, bool highlighted, bool clicked)
    {
      ...
      if (available) { GameMenuCheckBox::Render(realX, realY, highlighted, clicked); }
      else
      {
    ...
    {
      bool available = g_app->m_soundSystem->IsInitialized() && g_soundLibrary3d->Hardware3DSupport();
      if (available) { GameMenuCheckBox::MouseUp(); }
    }
};
#endif
...
PrefsSoundWindow::PrefsSoundWindow()
  : GameOptionsWindow("dialog_soundoptions"),
    m_menuSoundLib(nullptr),
    m_menuMixFreq(nullptr),
    m_menuNumChannels(nullptr),
#ifdef USE_SOUND_MEMORY
...
#ifdef USE_SOUND_MEMORY
  m_menuMemoryUsage = new GameMenuDropDown(); m_menuMemoryUsage->
    SetShortProperties("dialog_memoryusage", x, y += border, buttonW, buttonH, LANGUAGEPHRASE("dialog_memoryusage")); m_menuMemoryUsage->
    AddOption("dialog_high", 1); m_menuMemoryUsage->AddOption("dialog_medium", 2); m_menuMemoryUsage->AddOption("dialog_low", 3);
  m_menuMemoryUsage->RegisterInt(&m_memoryUsage); m_menuMemoryUsage->m_fontSize = fontSize; m_menuMemoryUsage->m_cycleMode = true;
  RegisterButton(m_menuMemoryUsage); m_buttonOrder.PutData(m_menuMemoryUsage);
#endif
...
#ifdef USE_SOUND_HW3D
  m_menuHw3d = new HW3DDropDownMenu(); m_menuHw3d->
    SetShortProperties("dialog_hw3dsound", x, y += border, buttonW, buttonH, LANGUAGEPHRASE("dialog_hw3dsound")); m_menuHw3d->
    RegisterInt(&m_useHardware3D); m_menuHw3d->m_fontSize = fontSize; RegisterButton(m_menuHw3d); m_buttonOrder.PutData(m_menuHw3d);
#endif

#ifdef USE_SOUND_DSPEFFECTS
  m_menuDspEffects = new GameMenuCheckBox(); m_menuDspEffects->SetShortProperties("dialog_realtimeeffects", x, y += border, buttonW,
                                                                                  buttonH, LANGUAGEPHRASE("dialog_realtimeeffects"));
  m_menuDspEffects->RegisterInt(&m_dspEffects); m_menuDspEffects->m_fontSize = fontSize; RegisterButton(m_menuDspEffects); m_buttonOrder.
    PutData(m_menuDspEffects);
#endif
...
void PrefsSoundWindow::RefreshValues()
{
  ...
  if (m_menuSoundLib)
    m_menuSoundLib->SelectOption(m_soundLib);
  if (m_menuMixFreq)
    m_menuMixFreq->SelectOption(m_mixFreq);
  if (m_menuNumChannels)
    m_menuNumChannels->SelectOption(m_numChannels);
#ifdef USE_SOUND_MEMORY
  if (m_menuMemoryUsage)
    m_menuMemoryUsage->SelectOption(m_memoryUsage);
#endif
  if (m_menuSwapStereo)
    m_menuSwapStereo->Set(m_swapStereo);
#ifdef USE_SOUND_HW3D
  if (m_menuHw3d)
    m_menuHw3d->Set(m_useHardware3D);
#endif
#ifdef USE_SOUND_DSPEFFECTS
  if (m_menuDspEffects)
    m_menuDspEffects->Set(m_dspEffects);
#endif
}
...
void PrefsSoundWindow::Render(bool _hasFocus)
{
  ...
  if (g_profiler)
  {
    ...
    if (element->m_lastNumCalls > 0)
    {
      ...
      wchar_t temp[512];
      swprintf(temp, sizeof(temp) / sizeof(wchar_t), L"%ls %d%%", LANGUAGEPHRASE("dialog_cpuusage").m_unicodestring,
               static_cast<int>(occup));
  ...
  swprintf(tempString, sizeof(tempString) / sizeof(wchar_t), L"%ls %2.1f Mb (unused %2.1f)",
           LANGUAGEPHRASE("dialog_memoryusage").m_unicodestring, totalMemoryUsage / MB, unusedMemoryUsage / MB);
