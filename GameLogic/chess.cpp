...
void ChessBase::Initialise(Building* _template)
{
  ...
  m_width = static_cast<ChessBase*>(_template)->m_width;
  m_depth = static_cast<ChessBase*>(_template)->m_depth;
}
...
bool ChessBase::Advance()
{
  ...
  if (m_timer <= 0.0)
  {
    ...
    if (m_sparePoints > 0.3)
    {
      ...
      if (thisScale > m_sparePoints)
        thisScale = m_sparePoints;
      if (thisScale < 0.1)
        thisScale = 0.1;
    ...
    for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
    {
      ...
      {
        auto chessPiece = static_cast<ChessPiece*>(g_app->m_location->m_buildings[i]);
        if (chessPiece->m_type == TypeChessPiece && !g_app->m_location->IsFriend(chessPiece->m_id.GetTeamId(), m_id.GetTeamId()))
        {
...
void ChessBase::SpawnPiece(double _scale)
{
  ...
  auto building = static_cast<ChessPiece*>(Building::CreateBuilding(Building::TypeChessPiece));
  g_app->m_location->m_buildings.PutData(building);
  building->m_scale = _scale;
  building->m_chessBaseId = m_id.GetUniqueId();
  building->Initialise(&carryTemplate);
    ...
    m_damage(0.0)
{
  ...
  CarryableBuilding::SetShape(g_app->m_resource->GetShape("chesspiece.shp"));
}
...
void ChessPiece::SetWaypoint(const Vector3& _waypoint)
{
...
void ChessPiece::HandleSuccess()
{
  ...
  auto base = static_cast<ChessBase*>(g_app->m_location->GetBuilding(m_chessBaseId));
  ...
  g_app->m_multiwinia->AwardPoints(m_id.GetTeamId(), static_cast<int>(10 * m_scale));
