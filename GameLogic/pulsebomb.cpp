...
void PulseBomb::Initialise(Building* _template)
{
  ...
  m_startTime = static_cast<PulseBomb*>(_template)->m_startTime;
  s_pulseBombs[m_id.GetTeamId()] = m_id.GetUniqueId();
  for (int i = 0; i < static_cast<PulseBomb*>(_template)->m_links.Size(); ++i)
    SetBuildingLink(static_cast<PulseBomb*>(_template)->m_links[i]);

  if (g_app->m_multiwinia->m_pulseTimer > 0.0)
    m_startTime = g_app->m_multiwinia->m_pulseTimer;

  if (s_defaultTime == 0.0)
    s_defaultTime = m_startTime;

  g_app->m_multiwinia->m_timeRemaining = static_cast<int>(m_startTime);
}
...
bool PulseBomb::Advance()
{
  ...
  for (int i = 0; i < m_links.Size(); ++i)
  {
    ...
    if (b && b->m_type == TypeControlStation)
      static_cast<ControlStation*>(b)->LinkToPulseBomb(m_id.GetUniqueId());
  }
  ...
  for (int i = 0; i < m_links.Size(); ++i)
  {
    ...
    if (b && b->m_type == TypeControlStation)
    {
      if (g_app->m_location->IsFriend(b->m_id.GetTeamId(), m_id.GetTeamId()) || b->m_id.GetTeamId() == 255)
      {
  ...
  if (g_app->m_multiwinia->m_timeRemaining <= 0 && m_detonateTime == 0.0)
  {
    ...
    for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
    {
      if (g_app->m_location->m_buildings.ValidIndex(i))
      {
        ...
        if (b->m_type == TypeTrunkPort && b->m_id.GetTeamId() != m_id.GetTeamId())
...
void PulseBomb::RenderAlphas(double _predictionTime)
{
  ...
  for (int i = 0; i < maxBlobs; ++i)
  {
    ...
    double size = 2.0;
    double factor = 200.0;
    if (g_app->m_multiwinia->GetPreciseTimeRemaining() >= 0.0)
      size += sin(timeIndex + i * 10) * factor * (1.0 - (g_app->m_multiwinia->GetPreciseTimeRemaining() / m_startTime));
    size = max(size, 2.0);
...
void PulseBomb::RenderCountdown(double _predictionTime)
{
  if (g_app->m_multiwinia->m_timeRemaining <= 10 && !m_destroyed)
  {
    ...
    Vector3 textPos = screenPos + screenRight * screenSize * 0.5 + screenUp * screenSize * 0.5;
...
void PulseBomb::Destroy()
{
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    ...
    {
      Building* b = g_app->m_location->m_buildings[i];
      if (b->m_type == TypeTrunkPort && b->m_id.GetTeamId() != defenderTeamId)
        b->m_id.SetTeamId(255);
    }
  }
...
bool PulseBomb::DoesSphereHit(const Vector3& _pos, double _radius)
{
  if (m_shape)
  {
