    ...
    m_spiritId(-1)
{
  ...
  //ASSERT_TEXT( m_eggLay, "Spider: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", eggLayName, m_shape->m_name );
...
void Spider::Begin()
{
  ...
  constexpr char eggLayName[] = "MarkerEggLay";
  m_eggLay = m_shape->m_rootFragment->LookupMarker(eggLayName);
  ASSERT_TEXT(m_eggLay, "Spider: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", eggLayName, m_shape->m_name);
...
bool Spider::ChangeHealth(int _amount, int _damageType)
{
  ...
  if (!m_dead && _amount < -1)
  {
    ...
    double fractionDead = 1.0 - static_cast<double>(m_stats[StatHealth]) / EntityBlueprint::GetStat(TypeSpider, StatHealth);
...
{
  if (GetNetworkTime() < m_nextLegMoveTime)
    return -1;
...
void Spider::StompFoot(const Vector3& _pos)
{
  //
  // Damage everyone nearby

  int numFound;
  g_app->m_location->m_entityGrid->GetEnemies(s_neighbours, _pos.x, _pos.z, FOOT_DAMAGE_RADIUS, &numFound, m_id.GetTeamId());
...
void Spider::UpdateLegs()
{
  ...
  for (int i = 1; i < 3; ++i)
  {
    double diffBest = abs(m_vel.Mag() - m_parameters[stage].m_idealSpeed);
    double diffCurrent = abs(m_vel.Mag() - m_parameters[i].m_idealSpeed);
...
double Spider::IsPathOK(const Vector3& _dest)
{
  Vector3 toDest(_dest - m_pos);
  double distToDest = toDest.Mag();
  constexpr double sampleSeperation = 8.0;
...
void Spider::DetectCollisions()
{
  ...
  pos += m_vel;
  int numFound;
  g_app->m_location->m_entityGrid->GetNeighbours(s_neighbours, pos.x, pos.z, 22.0, &numFound);
...
bool Spider::FaceTarget()
{
  ...
  if (dotProd < 0.999)
  {
    ...
    if (dotProd < 0.9)
      ...
      rotation.SetLength(TURN_RATE / 2.0);
...
bool Spider::AdvanceAttack()
{
  ...
  if (facingTarget)
  {
    ...
    {
      double force = sqrt(distance) * 24.0;
