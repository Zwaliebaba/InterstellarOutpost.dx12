...
class CreateMapButton : public GameMenuButton
{
  public:
    ...
    {
      auto parent = static_cast<NewMapWindow*>(m_parent);
      char filename[512];
...
class SelectMapButton : public GameMenuButton
{
  public:
    ...
    void MouseUp() override
    {
      ...
      auto fileDialog = new MultiwiniaFileDialog("Select File", m_parent->m_name, m_path, m_filter, false);
...
void MultiwiniaWindow::Create()
{
  ...
  auto selectMap = new SelectMapButton();
  ...
  auto newmap = new NewMapWindowButton();
...
NewMapWindow::NewMapWindow()
  : GameOptionsWindow("new_map"),
    m_mapFilename(m_fileString, 256) { m_mapFilename.Set("mp_newmap.txt"); }
...
void NewMapWindow::Create()
{
  ...
  CreateMenuControl("multiwinia_editor_mapfilename", InputField::TypeString, &m_mapFilename, y += buttonH + gap, 0, nullptr, x, buttonW,
                    fontSize);
  m_buttonOrder.PutData(GetButton("multiwinia_editor_mapfilename"));

  auto createMap = new CreateMapButton();
