...
PowerBuilding::~PowerBuilding() { m_surges.EmptyAndDelete(); }
...
Vector3 PowerBuilding::GetPowerLocation()
{
  if (!m_powerLocation)
  {
    constexpr char powerLocationName[] = "MarkerPowerLocation";
    m_powerLocation = m_shape->m_rootFragment->LookupMarker(powerLocationName);
    ASSERT_TEXT(m_powerLocation, "PowerBuilding: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", powerLocationName,
                m_shape->m_name);
  }
...
void PowerBuilding::RenderAlphas(double _predictionTime)
{
  ...
  if (powerLink)
  {
    ...
    glBegin(GL_QUADS);
    glTexCoord2f(0.0, 0);
    glVertex3dv((ourPos - ourPosRight * 3).GetData());
    glTexCoord2f(0.0, 1);
    glVertex3dv((ourPos + ourPosRight * 3).GetData());
    glTexCoord2f(1.0, 1);
    glVertex3dv((theirPos + theirPosRight * 3).GetData());
    glTexCoord2f(1.0, 0);
    glVertex3dv((theirPos - theirPosRight * 3).GetData());
    glEnd();
    ...
    glBegin(GL_QUADS);
    glTexCoord2f(0.1, 0);
    glVertex3dv((ourPos - ourPosRight).GetData());
    glTexCoord2f(0.1, 1);
    glVertex3dv((ourPos + ourPosRight).GetData());
    glTexCoord2f(0.9, 1);
    glVertex3dv((theirPos + theirPosRight).GetData());
    glTexCoord2f(0.9, 0);
    glVertex3dv((theirPos - theirPosRight).GetData());
    glEnd();
    ...
    for (int i = 0; i < m_surges.Size(); ++i)
    {
      ...
      if (g_app->Multiplayer())
        predictedPos = thisSurge->m_percent + _predictionTime * 2;
      if (predictedPos < 0.0)
        predictedPos = 0.0;
      if (predictedPos > 1.0)
        predictedPos = 1.0;
      ...
      glBegin(GL_QUADS);
      glTexCoord2i(0, 0);
      glVertex3dv((thisSurgePos - camUp * 1.5 - camRight * 1.5).GetData());
      glTexCoord2i(1, 0);
      glVertex3dv((thisSurgePos - camUp * 1.5 + camRight * 1.5).GetData());
      glTexCoord2i(1, 1);
      glVertex3dv((thisSurgePos + camUp * 1.5 + camRight * 1.5).GetData());
      ...
      glBegin(GL_QUADS);
      glTexCoord2i(0, 0);
      glVertex3dv((thisSurgePos - camUp - camRight).GetData());
      glTexCoord2i(1, 0);
      glVertex3dv((thisSurgePos - camUp + camRight).GetData());
      glTexCoord2i(1, 1);
      glVertex3dv((thisSurgePos + camUp + camRight).GetData());
    ...
    if (g_app->Multiplayer())
    {
      ...
      for (int i = 0; i < m_surges.Size(); ++i)
      {
        ...
        {
          Team* team = g_app->m_location->m_teams[thisSurge->m_teamId];
          if (!team || team->m_monsterTeam || team->m_futurewinianTeam)
            continue;
...
bool PowerBuilding::Advance()
{
  for (int i = 0; i < m_surges.Size(); ++i)
  {
    ...
    if (g_app->Multiplayer())
      amountToAdd = SERVER_ADVANCE_PERIOD * 2;
    ...
    m_throughput(0.0)
{
  ...
  constexpr char counterName[] = "MarkerCounter";
  m_counter = m_shape->m_rootFragment->LookupMarker(counterName);
  ASSERT_TEXT(m_counter, "Generator: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", counterName, m_shape->m_name);
}
...
void Generator::GetObjectiveCounter(UnicodeString& _dest)
{
  ...
  swprintf(result, sizeof(result) / sizeof(wchar_t), L"%ls : %d Gq/s", LANGUAGEPHRASE("objective_output").m_unicodestring,
           static_cast<int>(m_throughput * 10));
...
void Generator::Render(double _predictionTime)
{
  ...
  g_gameFont.DrawText3D(counterMat.pos, counterMat.f, counterMat.u, 7.0, "%d", static_cast<int>(m_throughput * 10.0));
...
void PylonStart::RenderAlphas(double _predictionTime) { PowerBuilding::RenderAlphas(_predictionTime); }
...
void PylonEnd::TriggerSurge(double _initValue, int teamId)
{
  ...
  if (building && building->m_type == TypeLaserFence && g_app->m_location->IsFriend(teamId, building->m_id.GetTeamId()))
  {
    ...
    m_startingTeam(-1)
{
  ...
  for (int i = 0; i < SOLARPANEL_NUMGLOWS; ++i)
  {
    ...
    ASSERT_TEXT(m_glowMarker[i], "SolarPanel: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", name, m_shape->m_name);
  }
  ...
  for (int i = 0; i < SOLARPANEL_NUMSTATUSMARKERS; ++i)
  {
    ...
    ASSERT_TEXT(m_statusMarkers[i], "SolarPanel: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", name, m_shape->m_name);
  }
}

bool SolarPanel::IsOperating() { return m_operating; }
...
bool SolarPanel::Advance()
{
  ...
  if (GetNumPorts() > 0)
    fractionOccupied = static_cast<double>(GetNumPortsOccupied()) / static_cast<double>(GetNumPorts());

  if (m_startingTeam != 255 && m_id.GetTeamId() != 255 && m_startingTeam != m_id.GetTeamId() && GetNumPorts() > 0 && g_app->m_multiwinia->
...
void SolarPanel::RenderAlphas(double _predictionTime)
{
  ...
  if (fractionOccupied > 0.0 && m_id.GetTeamId() != 255)
  {
    ...
    double alphaValue = 0.5 + abs(sin(g_gameTime)) * 0.5;
    ...
    for (int i = 0; i < SOLARPANEL_NUMGLOWS; ++i)
    {
      ...
      glBegin(GL_QUADS);
      glTexCoord2i(0, 0);
      glVertex3dv((thisGlow.pos - thisGlow.r * glowHeight * 0.5 + thisGlow.f * glowWidth * 0.5).GetData());
      glTexCoord2i(0, 1);
      glVertex3dv((thisGlow.pos + thisGlow.r * glowHeight * 0.5 + thisGlow.f * glowWidth * 0.5).GetData());
      glTexCoord2i(1, 1);
      glVertex3dv((thisGlow.pos + thisGlow.r * glowHeight * 0.5 - thisGlow.f * glowWidth * 0.5).GetData());
    ...
    for (int i = 0; i < SOLARPANEL_NUMGLOWS; ++i)
    {
      ...
      glBegin(GL_QUADS);
      glTexCoord2i(0, 0);
      glVertex3dv((thisGlow.pos - thisGlow.r * glowHeight + thisGlow.f * glowWidth).GetData());
      glTexCoord2i(0, 1);
      glVertex3dv((thisGlow.pos + thisGlow.r * glowHeight + thisGlow.f * glowWidth).GetData());
      glTexCoord2i(1, 1);
      glVertex3dv((thisGlow.pos + thisGlow.r * glowHeight - thisGlow.f * glowWidth).GetData());
...
void PowerSplitter::Initialise(Building* _template)
{
  ...
  auto splitter = static_cast<PowerSplitter*>(_template);
...
void PowerSplitter::TriggerSurge(double _initValue, int teamId)
{
  for (int i = 0; i < m_links.Size(); ++i)
  {
    ...
    if (building && building->m_type == TypePylon && building->m_id.GetTeamId() == teamId)
    {
      static_cast<PowerBuilding*>(building)->TriggerSurge(_initValue, teamId);
      break;
    }
  }
}

void PowerSplitter::RenderAlphas(double _predictionTime) { PowerBuilding::RenderAlphas(_predictionTime); }

void PowerSplitter::SetBuildingLink(int _buildingId) { m_links.PutData(_buildingId); }
