...
void Tentacle::Begin()
{
  ...
  if (!m_next.IsValid())
  {
    ...
    if (myUnit)
    {
      double size = 1.0; // * pow(1.1, myUnit->m_entities.Size() );
      size = min(size, 30.0);

      Tentacle* prev = nullptr;

      for (int i = 0; i < myUnit->m_entities.Size(); ++i)
      {
        if (myUnit->m_entities.ValidIndex(i))
        {
          auto tentacle = static_cast<Tentacle*>(myUnit->m_entities[i]);
...
bool Tentacle::ChangeHealth(int _amount, int _damageType) { return false; }

bool Tentacle::Advance(Unit* _unit)
{
  if (!g_app->m_location->GetBuilding(m_portalId))
    return true;
  ASSERT_TEXT(_unit, "Tentacles must be created in a unit");
  ...
  if (m_prev.IsValid())
  {
    ...
    auto tentacle = static_cast<Tentacle*>(g_app->m_location->GetEntitySafe(m_prev, TypeTentacle));
    if (tentacle)
    {
      ...
      if (success)
      {
        ...
        if (trailPos.y - (m_size / 2.0) >= g_app->m_location->m_landscape.m_heightMap->GetValue(trailPos.x, trailPos.z) || trailPos.y >
          m_pos.y || m_state == TentacleStateSwipeAcross)
        {
  ...
  else
  {
    ...
    switch (m_state)
    {
    case TentacleStateIdle:
      AdvanceIdle();
      break;
    case TentacleStateSlamGround:
      AdvanceSlam();
      break;
    case TentacleStateSwipeAcross:
      AdvanceSwipe();
      break;
    }
  }
...
bool Tentacle::AdvanceIdle()
{
  if (m_targetDirection == g_zeroVector)
  {
    ...
    Vector3 direction = g_upVector + Vector3(sin(timeIndex) * 0.1, 0.0, sin(timeIndex) * 0.1);
    direction.Normalise();
    ...
    if (syncrand() % 50 == 0)
    {
      auto p = static_cast<Portal*>(g_app->m_location->GetBuilding(m_portalId));
      if (p && p->m_type == Building::TypePortal && p->m_finalSummonState != Portal::SummonStateExplode)
      {
...
void Tentacle::Attack(const Vector3& _pos)
{
  int numFound;
  ...
  for (int i = 0; i < numFound; ++i)
  {
    ...
    if (entity->m_type != TypeTentacle && entity->m_type != TypeShaman)
    {
...
void Tentacle::SetPortalId(int _id) { m_portalId = _id; }
...
void Tentacle::Render(double _predictionTime)
{
  ...
  if (!m_next.IsValid() && m_state == TentacleStateIdle && (m_direction ^ g_upVector).Mag() < 0.2)
  {
    auto p = static_cast<Portal*>(g_app->m_location->GetBuilding(m_portalId));
    if (p)
    {
...
bool Tentacle::IsInView() { return g_app->m_camera->SphereInViewFrustum(m_pos + m_centrePos, m_radius * 2.0); }
