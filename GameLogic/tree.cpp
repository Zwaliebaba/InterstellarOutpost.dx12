...
Tree::Tree()
  : Building(),
    ...
    m_corrupted(false) { m_type = TypeTree; }
...
void Tree::Initialise(Building* _template)
{
  ...
  if (m_evil)
    g_app->m_soundSystem->TriggerBuildingEvent(this, "CreateEvil");
  else if (magic)
    g_app->m_soundSystem->TriggerBuildingEvent(this, "CreateMagic");
  else
    g_app->m_soundSystem->TriggerBuildingEvent(this, "CreateNormal");
}
...
bool Tree::Advance()
{
  ...
  if (m_onFire > 0.0)
  {
    ...
    for (int i = 0; i < numFire; ++i)
    {
      ...
      auto fireSpawn = Vector3(distance, height, 0);
    ...
    for (int b = 0; b < g_app->m_location->m_buildings.Size(); ++b)
    {
      if (g_app->m_location->m_buildings.ValidIndex(b))
      {
        ...
        if (building->m_type == TypeAntHill)
        {
          ...
          if (theirRadius + ourRadius >= distance)
            static_cast<AntHill*>(building)->Burn();
        }
      }
    }
    ...
    for (int i = 0; i < num; ++i)
    {
      ...
      if (e)
      {
        if (e->m_type == Entity::TypeDarwinian)
          static_cast<Darwinian*>(e)->SetFire();
        else
  ...
  if (m_onFire > -0.3 && m_onFire <= 0.0)
  {
    if (m_leafDropRate > 0)
    {
      ...
      if (rand() % (51 - m_leafDropRate) == 0)
      {
        ...
        fireSpawn += Vector3(sfrand(actualHeight * 1.0), sfrand(actualHeight * 0.25), sfrand(actualHeight * 1.0));
        g_app->m_particleSystem->CreateParticle(fireSpawn, g_zeroVector, Particle::TypeLeaf, -1.0,
    ...
    if (!m_evil && m_spiritDropRate > 0)
    {
      ...
      {
        double actualHeight = GetActualHeight(0.0);
        auto pos = Vector3(FRAND(actualHeight), actualHeight, 0);
        pos.RotateAroundY(syncfrand(2.0 * M_PI));
        pos += m_pos;

        int teamId = -1;
        while (teamId == -1 || teamId == g_app->m_location->GetMonsterTeamId() || teamId == g_app->m_location->GetFuturewinianTeamId())
        {
  ...
  if (m_evil)
  {
    ...
    if (syncfrand(10.0) < 0.2)
    {
      ...
      auto darwinian = static_cast<Darwinian*>(g_app->m_location->GetEntitySafe(id, Entity::TypeDarwinian));
      if (darwinian && darwinian->m_state != Darwinian::StateOperatingPort)
      {
        bool killed = false;
        bool dead = darwinian->m_dead;
        darwinian->ChangeHealth(-999);
        if (!dead && darwinian->m_dead)
          killed = true;
    ...
    if (GetActualHeight(0.0f) == m_height)
    {
      ...
      if (m_corruptCheckTimer <= 0.0f)
      {
        ...
        for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
        {
          ...
          {
            auto t = static_cast<Tree*>(g_app->m_location->m_buildings[i]);
            if (t && t->m_type == TypeTree)
            {
              if (!t->m_evil && !t->m_corrupted && (m_pos - t->m_pos).Mag() < GetActualHeight(0.0f))
              {
                //t->m_evil = true;
...
Vector3 PushFromObstructions(Vector3 _pos)
{
  ...
  for (int b = 0; b < buildings->Size(); ++b)
  {
    ...
    {
      bool hit = false;
      if (building->DoesSphereHit(result, 1.0))
        hit = true;
...
void Tree::CreateAnotherTree()
{
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    if (g_app->m_location->m_buildings.ValidIndex(i))
    {
      ...
      if (building->m_id != m_id && building->m_type == TypeTree)
      {
...
double Tree::GetActualHeight(double _predictionTime)
{
  ...
  double actualHeight = m_height * (1.0 - abs(predictedOnFire));
  if (actualHeight < m_height * 0.01)
    actualHeight = m_height * 0.01;
...
void Tree::Generate()
{
  ...
  if (treeDetail > 1)
  {
    ...
    alpha *= pow(1.3, treeDetail);
    alpha = min(alpha, 255);
    m_leafColourArray[3] = alpha;
  }

#ifdef USE_DISPLAY_LISTS
  m_branchDisplayListId = g_app->m_resource->CreateDisplayListAsync(nullptr, Method<Tree>(&Tree::GenerateBranches, this));

  m_leafDisplayListId = g_app->m_resource->CreateDisplayListAsync(nullptr, Method<Tree>(&Tree::GenerateLeaves, this));
#endif
...
void Tree::RenderAlphas(double _predictionTime)
{
  ...
  if (m_spiritDropRate > 0)
  {
    ...
    double factor = abs(sin(GetHighResTime() + m_id.GetUniqueId() * 10));
  ...
  if (m_corruptCheckTimer <= 0.0f && !m_evil)
  {
    ...
    {
      m_branchColourArray[0] -= static_cast<float>(m_branchColourArray[0]) * (factor);
      m_branchColourArray[1] -= static_cast<float>(m_branchColourArray[1]) * (factor);
      m_branchColourArray[2] -= static_cast<float>(m_branchColourArray[2]) * (factor);
  ...
  if (m_onFire > 0.0f)
  {
    ...
    m_branchColourArray[0] -= static_cast<float>(m_branchColourArray[0]) * (fireFactor);
    m_branchColourArray[1] -= static_cast<float>(m_branchColourArray[1]) * (fireFactor);
    m_branchColourArray[2] -= static_cast<float>(m_branchColourArray[2]) * (fireFactor);
  ...
  else if (m_onFire < 0.0)
  {
    if (abs(m_onFire) > 0.5)
      ...
      m_leafColourArray[3] *= (1.0 - abs(m_onFire) * 2.0);
  }
  ...
  if (m_onFire > 0.0)
  {
    ...
    for (int i = 0; i < 4; ++i)
    {
      ...
      pos.x += sin(timeIndex + i) * i * 1.7;
      pos.y += cos(timeIndex + i);
      pos.z += cos(timeIndex + i) * i * 1.7;
      ...
      glBegin(GL_QUADS);
      glTexCoord2i(0, 0);
      glVertex3dv((pos - camUp * size - camRight * size).GetData());
      glTexCoord2i(1, 0);
      glVertex3dv((pos + camUp * size - camRight * size).GetData());
      glTexCoord2i(1, 1);
      glVertex3dv((pos + camUp * size + camRight * size).GetData());
...
bool Tree::DoesSphereHit(const Vector3& _pos, double _radius)
{
  if (SphereSphereIntersection(m_pos, 10.0, _pos, _radius))
    return true;
...
bool Tree::DoesRayHit(const Vector3& _rayStart, const Vector3& _rayDir, double _rayLen, Vector3* _pos, Vector3* _norm)
{
...
void Tree::RenderBranch(Vector3 _from, Vector3 _to, int _iterations, bool _calcRadius, bool _renderBranch, bool _renderLeaf)
{
  ...
  _to += Vector3(sin(g_gameTime*(7-_iterations))*0.005 * _iterations,
                 sin(g_gameTime) * 0.01,
                 sin(g_gameTime * (7-_iterations))*0.005 * _iterations );
  */
  ...
  if ((_iterations == 0 && _renderLeaf) || (_iterations != 0 && _renderBranch))
  {
    ...
    glVertex3dv((_from - camRightA).GetData());
    glTexCoord2f(0.0, 1.0);
    glVertex3dv((_from + camRightA).GetData());
    glTexCoord2f(1.0, 1.0);
    glVertex3dv((_to + camRightA).GetData());
    glTexCoord2f(1.0, 0.0);
    glVertex3dv((_to - camRightA).GetData());

    glTexCoord2f(0.0, 0.0);
    glVertex3dv((_from - camRightB).GetData());
    glTexCoord2f(0.0, 1.0);
    glVertex3dv((_from + camRightB).GetData());
    glTexCoord2f(1.0, 1.0);
    glVertex3dv((_to + camRightB).GetData());
    glTexCoord2f(1.0, 0.0);
    glVertex3dv((_to - camRightB).GetData());
  }
  ...
  for (int i = 0; i < numBranches; ++i)
  {
    ...
    Vector3 thisTo = thisFrom + thisRightAngle * thickness * 0.4 * m_pushOut + thisBranch * (1.0 - distance) * m_pushUp;
...
double Tree::GetFireAmount() { return m_onFire; }

bool Tree::IsOnFire() { return (m_onFire > 0.0); }

double Tree::GetBurnRange() { return GetActualHeight(0.0) / 2.0; }

bool Tree::IsInView() { return (g_app->m_camera->SphereInViewFrustum(m_pos, m_height)); }
...
char* Tree::LogState(char* message)
{
  ...
  sprintf(s_result,
          "%sTREE Id[%d] pos[%5.2f,%5.2f,%5.2f], vel[%5.2f] front[%5.2f] centre[%5.2f] radius[%5.2f] height[%5.2f] hitcheckCentre[%5.2f] hitcheckRadius[%5.2f]",
          (message) ? message : "", m_id.GetUniqueId(), m_pos.x, m_pos.y, m_pos.z, m_vel.x + m_vel.y + m_vel.z,
          m_front.x + m_front.y + m_front.z, m_centrePos.x + m_centrePos.y + m_centrePos.z, m_radius, m_height,
          // radius is a function of hitCheckRadius and height
          m_hitcheckCentre.x + m_hitcheckCentre.y + m_hitcheckCentre.z, m_hitcheckRadius);
