...
// *** Render
void FenceSwitch::Render(double predictionTime) { Building::Render(predictionTime); }
...
Vector3 FenceSwitch::GetConnectionLocation()
{
  ...
  {
    constexpr char connectionLocationName[] = "MarkerConnectionLocation";
    m_connectionLocation = m_shape->m_rootFragment->LookupMarker(connectionLocationName);
    ASSERT_TEXT(m_connectionLocation, "FenceSwitch: Can't get Marker(%s) from shape(%s), probably a corrupted file\n",
