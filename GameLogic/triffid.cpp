    ...
    m_triggerRadius(100.0)
{
  ...
  constexpr char launchPointName[] = "MarkerLaunchPoint";
  m_launchPoint = m_shape->m_rootFragment->LookupMarker(launchPointName);
  ASSERT_TEXT(m_launchPoint, "Triffid: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", launchPointName, m_shape->m_name);

  constexpr char stemName[] = "MarkerTriffidStem";
  m_stem = m_shape->m_rootFragment->LookupMarker(stemName);
  ASSERT_TEXT(m_stem, "Triffid: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", stemName, m_shape->m_name);
...
Matrix34 Triffid::GetHead(double _predictionTime)
{
  ...
  if (m_damage > 0.0)
  {
    _front.RotateAroundY(sin(timer) * m_variance * 0.f);
    Vector3 right = _front ^ _up;

    double pitchVariation = 1.0 + sin(timer) * 0.1;
    _front.RotateAround(right * m_pitch * 0.5 * pitchVariation);

    _pos += Vector3(sin(timer / 1.3) * 4, sin(timer / 1.5), sin(timer / 1.2) * 3);
  ...
  else
  {
    ...
    _front.RotateAround(right * sin(timer * 5.0) * m_variance);
    _up.RotateAroundY(sin(timer * 3.5) * m_variance);

    _pos += Vector3(sin(timer * 3.3) * 8, sin(timer * 4.5), sin(timer * 3.2) * 6);
  }

  Matrix34 result(_front, _up, _pos);

  // Ordering important for sync
  double size = sin(timer);
  size *= 0.1;
  size += 1.0;
  size *= m_size;
...
bool Triffid::DoesRayHit(const Vector3& _rayStart, const Vector3& _rayDir, double _rayLen, Vector3* _pos, Vector3* _norm)
{
...
void Triffid::Initialise(Building* _template)
{
  ...
  double timeAdd = syncrand() % static_cast<int>(m_reloadTime);
  timeAdd += 10.0;
...
bool Triffid::Advance()
{
  ...
  if (m_damage <= 0.0)
  {
    ...
    fireSpawn += Vector3(sfrand(10.0 * m_size), sfrand(10.0 * m_size), sfrand(10.0 * m_size));
    ...
    fireSpawn = m_pos + Vector3(sfrand(10.0 * m_size), sfrand(10.0 * m_size), sfrand(10.0 * m_size));
    g_app->m_particleSystem->CreateParticle(fireSpawn, g_zeroVector, Particle::TypeFire, fireSize);
  ...
  double healthFraction = m_damage / 50.0;
  double timeIndex = g_gameTime + m_id.GetUniqueId() * 10;
  m_renderDamaged = (frand(0.75) * (1.0 - abs(sin(timeIndex)) * 1.2) > healthFraction);
...
char* Triffid::GetSpawnName(int _spawnType)
{
  static char* names[NumSpawnTypes] = {"SpawnVirii", "SpawnCentipedes", "SpawnSpider", "SpawnSpirits", "SpawnEggs", "SpawnTriffidEggs",
                                       "SpawnDarwinians", "SpawnAnts", "SpawnSoulDestroyer", "SpawnSporeGenerator"};
...
void TriffidEgg::Spawn()
{
  ...
  {
    if ((g_app->m_multiwinia->m_gameType != Multiwinia::GameTypeAssault && SoulDestroyer::s_numSoulDestroyers < 2) || g_app->
      IsSinglePlayer())
    {
      if (g_app->Multiplayer())
      ...
      if (egg)
      {
        while (egg->m_spawnType = syncrand() % (Triffid::NumSpawnTypes - 1))
        {
          if (g_app->Multiplayer())
          {
            ...
            {
              if (g_app->m_multiwinia->m_gameType == Multiwinia::GameTypeAssault || SoulDestroyer::s_numSoulDestroyers >= 2)
                continue;
            }
          }
  ...
  case Triffid::SpawnAnts:
  {
    ...
    for (int i = 0; i < numAnts; ++i)
    {
      Vector3 vel = g_upVector + Vector3(SFRAND(1), 0.0, SFRAND(1));
      vel.SetLength(10.0 + syncfrand(20.0));
      WorldObjectId id = g_app->m_location->SpawnEntities(m_pos, teamId, -1, TypeArmyAnt, 1, vel, 0.0, 0.0);
      Entity* entity = g_app->m_location->GetEntity(id);
      entity->m_front.y = 0.0;
      entity->m_front.Normalise();
      entity->m_onGround = false;
...
void TriffidEgg::Render(double _predictionTime)
{
  ...
  age = max(age, 0.0);
  age = min(age, 1.0);
  double size = m_size + abs(sin(timeNow * 2.0)) * (1.0 - age) * 0.4;
...
void TriffidEgg::SetTimer(double _time) { m_timerSync = GetNetworkTime() + _time; }
