...
void Triffid::Render( double _predictionTime )
{
    ...
    if( m_renderDamaged && !g_app->m_editing && m_damage > 0.0 )
    {
        ...
        if      ( thefrand > 0.7 ) mat.f *= ( 1.0 - sin(timeIndex) * 0.5 );
        else if ( thefrand > 0.4 ) mat.u *= ( 1.0 - sin(timeIndex) * 0.2 );    
        else                        mat.r *= ( 1.0 - sin(timeIndex) * 0.5 );
        glEnable( GL_BLEND );
            ...
            g_app->m_locationEditor->m_selectionId == m_id.GetUniqueId() )
        {
            ...
            if( m_useTrigger )
            {
                ...
                for( int i = 0; i < numSteps; ++i )
                {
                    ...
                    Vector3 thisPos = triggerPos + Vector3( sin(angle)*m_triggerRadius, 0.0,
                                                            cos(angle)*m_triggerRadius );
