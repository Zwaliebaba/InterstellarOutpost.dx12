...
void AntHill::Damage(double _damage)
{
  ...
  if (m_health > 0)
  {
    int healthBandBefore = static_cast<int>(m_health / 20.0);
    m_health += _damage;
    int healthBandAfter = static_cast<int>(m_health / 20.0);

    if (healthBandAfter != healthBandBefore)
    {
      Matrix34 mat(m_front, g_upVector, m_pos);
      g_explosionManager.AddExplosion(m_shape, mat, 1.0 - static_cast<double>(m_health) / 100.0);
...
bool AntHill::SearchForScoutArea(Vector3& _pos)
{
  ...
  double theta = syncfrand(M_PI * 2);
  scoutPos.x += radius * sin(theta);
  scoutPos.z += radius * cos(theta);
...
void AntHill::SpawnAnt(Vector3& waypoint, WorldObjectId targetId)
{
  ...
  double theta = syncfrand(M_PI * 2);
  ant->m_wayPoint.x += radius * sin(theta);
  ant->m_wayPoint.z += radius * cos(theta);
...
bool AntHill::Advance()
{
  ...
  if (!g_app->m_location->m_teams)
    return false;
  if (g_app->m_location->m_teams[m_id.GetTeamId()]->m_teamType != TeamTypeCPU)
    return false;
  ...
  int maxObjectives = 3;
  if (g_app->Multiplayer())
    maxObjectives = m_numAntsInside / 10;
  if (maxObjectives < 1)
    maxObjectives = 1;
  if (maxObjectives > 6)
    maxObjectives = 6;
  ...
  if (!popLocked && m_objectives.Size() > 0 && GetNetworkTime() > m_spawnTimer && m_numAntsInside > 0)
  {
  ...
  if (m_numSpiritsInside > 0 && GetNetworkTime() > m_eggConvertTimer)
  {
    m_numSpiritsInside--;
    m_numAntsInside += 3;
  ...
  double healthFraction = static_cast<double>(m_health) / 100.0;
...
void AntHill::Render(double _predictionTime)
{
  ...
  if (m_renderDamaged)
  {
    ...
    if (thefrand > 0.7)
      mat.f *= (1.0 - sin(timeIndex) * 0.7);
    else if (thefrand > 0.4)
      mat.u *= (1.0 - sin(timeIndex) * 0.5);
    else
      mat.r *= (1.0 - sin(timeIndex) * 0.7);
    glEnable(GL_BLEND);
