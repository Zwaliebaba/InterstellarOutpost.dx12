...
void BlueprintBuilding::Initialise(Building* _template)
{
  ...
  constexpr char markerName[] = "MarkerBlueprint";
  m_marker = m_shape->m_rootFragment->LookupMarker(markerName);
  ASSERT_TEXT(m_marker, "BlueprintBuilding: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", markerName, m_shape->m_name);
...
void BlueprintBuilding::RenderAlphas(double _predictionTime)
{
  ...
  if (link)
  {
    ...
    if (fabs(infected - linkInfected) < 0.01)
      glColor4f(infected, 0.7 - infected * 0.7, 0.0, 0.1 + fabs(sin(ourTime)) * 0.2);
    else
      glColor4f(infected, 0.7 - infected * 0.7, 0.0, 0.5 + fabs(sin(ourTime)) * 0.5);
    ...
    glBegin(GL_QUADS);
    glTexCoord2i(0, 0);
    glVertex3dv((ourPos - rightAngle).GetData());
    glTexCoord2i(0, 1);
    glVertex3dv((ourPos + rightAngle).GetData());
    glTexCoord2i(1, 1);
    glVertex3dv((theirPos + rightAngle).GetData());
    glTexCoord2i(1, 0);
    glVertex3dv((theirPos - rightAngle).GetData());
    glEnd();
...
void BlueprintBuilding::SendBlueprint(int _segment, bool _infected)
{
  ...
  if (_infected)
    m_infected += SERVER_ADVANCE_PERIOD * 10.0;
  else
    m_infected -= SERVER_ADVANCE_PERIOD * 10.0;
...
void BlueprintStore::GetObjectiveCounter(UnicodeString& _dest)
{
  ...
  swprintf(result, sizeof(result) / sizeof(wchar_t), L"%ls %d%%", LANGUAGEPHRASE("objective_totalinfection").m_unicodestring,
           static_cast<int>(100.0 * totalInfection / double(BLUEPRINTSTORE_NUMSEGMENTS * 100.0)));
...
void BlueprintStore::SendBlueprint(int _segment, bool _infected)
{
  ...
  if (_infected)
    oldValue += SERVER_ADVANCE_PERIOD * 1.0;
  else
    oldValue -= SERVER_ADVANCE_PERIOD * 1.0;
...
bool BlueprintStore::Advance()
{
  ...
  if (clean > 0)
  {
    double infectionChange = (fullyInfected * 0.9) / static_cast<double>(clean);
...
void BlueprintStore::GetDisplay(Vector3& _pos, Vector3& _right, Vector3& _up, double& _size)
{
  ...
  front.RotateAroundY(sin(g_gameTime) * 0.3);

  Vector3 up = g_upVector;
  up.RotateAround(m_front * cos(g_gameTime) * 0.1);
...
bool BlueprintRelay::Advance()
{
  ...
  m_pos.x += sin(ourTime / 1.5) * 1.0;
  m_pos.y += sin(ourTime / 1.3) * 1.0;
  m_pos.z += cos(ourTime / 1.7) * 1.0;
