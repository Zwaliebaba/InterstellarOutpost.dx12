...
{
  m_directions[0].x = 0.5;
  m_directions[0].y = 0.86603;
  m_directions[1].x = 1.0;
  m_directions[1].y = 0.0;
  m_directions[2].x = 0.5;
  m_directions[2].y = -0.86603;
  m_directions[3].x = -0.5;
  m_directions[3].y = -0.86603;
  m_directions[4].x = -1.0;
  m_directions[4].y = 0.0;
  m_directions[5].x = -0.5;
  m_directions[5].y = 0.86603;
}
...
{
  bool allOnGround = m_legs[0]->m_foot.m_state == EntityFoot::OnGround && m_legs[1]->m_foot.m_state == EntityFoot::OnGround && m_legs[2]->
...
WorldObjectId Tripod::FindEntityToAttack()
{
  ...
  int numFound;
  g_app->m_location->m_entityGrid->GetEnemies(s_neighbours, m_pos.x, m_pos.z, ATTACK_SEARCH_RADIUS, &numFound, m_id.GetTeamId());
...
Vector2 Tripod::ChooseDestination()
{
  ...
  int numFound;
  g_app->m_location->m_entityGrid->GetEnemies(s_neighbours, m_pos.x, m_pos.z, NAVIGATION_SEARCH_RADIUS, &numFound, m_id.GetTeamId());
  Vector2 pos;
...
void Tripod::DoNavigation()
{
  ...
  double theta = asin(cross / deltaMag);
  if (theta < -M_PI / 6.0)
  {
    m_navData.m_dir++;
...
void Tripod::AdvanceAttack()
{
  ...
  if (timeInAttack > 1.0)
  {
    int t2 = static_cast<int>(timeInAttack * 10.0);
    if (t2 & 1)
    {
      Vector3 toEnemy = m_attackTarget - m_pos;
      toEnemy.Normalise();
      constexpr double speed = 80.0;
      right.SetLength(4.0);
