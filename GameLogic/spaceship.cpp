...
void SpaceShip::AdvanceAbducting(bool _atTarget)
{
  ...
  for (int i = 0; i < numFound; ++i)
  {
    ...
    if (d && d->m_id.GetTeamId() != g_app->m_location->GetFuturewinianTeamId() && d->m_state != Darwinian::StateInsideArmour)
    {
      Vector3 myPos = m_pos;
      myPos.y = d->m_pos.y;
...
void SpaceShip::AdvanceDropping()
{
  ...
  if (m_abductees > 0)
  {
    m_lastDrop = g_app->m_location->SpawnEntities(m_pos, g_app->m_location->GetFuturewinianTeamId(), -1, TypeDarwinian, 1, g_zeroVector,
                                                  0.0f);
    auto d = static_cast<Darwinian*>(g_app->m_location->GetEntitySafe(m_lastDrop, Entity::TypeDarwinian));
    if (d)
      d->m_onGround = false;
    m_abductees--;
  }
...
void SpaceShip::AdvanceEngines()
{
  ...
  for (int e = 1; e < 5; ++e)
  {
    ...
    ASSERT_TEXT(engine, "SpaceShip: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", markerName, m_shape->m_name);
...
bool SpaceShip::Advance(Unit* _unit)
{
  ...
  if (m_state != ShipStateLeaving)
  {
    if (m_state != ShipStateDropping && m_state != ShipStateWaiting)
    {
  ...
  switch (m_state)
  {
  case ShipStateAbducting:
    AdvanceAbducting(atTarget);
    break;
  case ShipStateDropping:
    AdvanceDropping();
    break;
...
void SpaceShip::Render(double predictionTime)
{
  ...
  if (m_state == ShipStateAbducting || m_state == ShipStateDropping)
...
void SpaceShip::RenderBeam(double _predictionTime)
{
  ...
  if (m_state == ShipStateAbducting)
  {
    ...
    while (y > totalHeight) { y -= SPACESHIP_HOVERHEIGHT; }
  }
  else
  {
    y = totalHeight - g_gameTime * 50;
    while (y < groundHeight) { y += SPACESHIP_HOVERHEIGHT; }
  }
...
void SpaceShip::SetWaypoint(const Vector3& _wayPoint) { m_waypoint = _wayPoint; }

bool SpaceShip::ChangeHealth(int amount, int _damageType) { return false; }

void SpaceShip::IncreaseAbductees() { m_abductees++; }
