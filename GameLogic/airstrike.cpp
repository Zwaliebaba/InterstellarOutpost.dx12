...
AirstrikeUnit::AirstrikeUnit(int teamId, int unitId, int numEntities, const Vector3& _pos)
    ...
    m_state(StateApproaching)
{
  ...
  //DebugTrace("New AirStrike Unit created with %d invaders, unit id = %d, x:%2.2f y:%2.2f z:%2.2f\n", numEntities, unitId, _pos.x, _pos.y, _pos.z );
}
...
bool AirstrikeUnit::Advance(int _slice)
{
  ...
  //DebugTrace("Updating AirStrike Unit\n");
...
void AirstrikeUnit::Render(double _predictionTime) { Unit::Render(_predictionTime); }
...
bool SpaceInvader::Advance(Unit* _unit)
{
  if (m_dead)
    return true;
  //DebugTrace("Upating SpaceInvader %d, formation index %d ", m_id.GetUniqueId(), m_formationIndex );
  ...
  if (targetPos != g_zeroVector)
  {
    ...
    //DebugTrace("target position x:%2.2f y:%2.2f z:%2.2f\n", targetPos.x, targetPos.y, targetPos.z );
  }
...
bool SpaceInvader::ChangeHealth(int _amount, int _damageType)
{
  ...
  if (!m_dead)
  {
    ...
    double fractionDead = 1.0f - m_stats[StatHealth] / EntityBlueprint::GetStat(TypeEngineer, StatHealth);
