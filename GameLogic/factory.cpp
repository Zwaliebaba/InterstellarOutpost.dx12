...
void Factory::Initialise(Building* _template)
{
  ...
  constexpr char markerSpiritStoreName[] = "MarkerSpiritStore";
  ShapeMarker* markerSpiritStore = m_shape->m_rootFragment->LookupMarker(markerSpiritStoreName);
  ASSERT_TEXT(markerSpiritStore, "Factory: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", markerSpiritStoreName,
              m_shape->m_name);
...
void Factory::AdvanceStateCreating()
{
  ...
  for (int i = 0; i < numToCreate; ++i)
  {
    if (m_spiritStore.NumSpirits() > 0)
    {
      ...
      Vector3 vel(SFRAND(1.0), SFRAND(1.0), 5.0);
