...
InsertionSquad::InsertionSquad(int teamId, int unitId, int numEntities, const Vector3& _pos)
...
void InsertionSquad::SetWayPoint(const Vector3& _pos)
{
  m_wayPoint = _pos;
  m_focusPos = _pos;
...
{
  int grenadeLevel = g_app->m_globalWorld->m_research->CurrentLevel(GlobalResearch::TypeGrenade), rocketLevel = g_app->m_globalWorld->
        m_research->CurrentLevel(GlobalResearch::TypeRocket), airStrikeLevel = g_app->m_globalWorld->m_research->
                                                                                      CurrentLevel(GlobalResearch::TypeAirStrike),
      controllerLevel = g_app->m_globalWorld->m_research->CurrentLevel(GlobalResearch::TypeController);

  switch (m_weaponType)
  {
  case GlobalResearch::TypeGrenade:
    DebugTrace("Current on weapon type grenade\n");
    m_weaponType = rocketLevel > 0
  ...
  case GlobalResearch::TypeRocket:
    DebugTrace("Current on weapon type rocket\n");
    m_weaponType = airStrikeLevel > 0
      ? GlobalResearch::TypeAirStrike
      : controllerLevel > 0
      ? GlobalResearch::TypeController
      : grenadeLevel > 0
  ...
  case GlobalResearch::TypeAirStrike:
    DebugTrace("Current on weapon type airstrike\n");
    m_weaponType = controllerLevel > 0
      ? GlobalResearch::TypeController
      : grenadeLevel > 0
      ? GlobalResearch::TypeGrenade
      : rocketLevel > 0
  ...
  case GlobalResearch::TypeController:
    DebugTrace("Current on weapon type controller\n");
    m_weaponType = grenadeLevel > 0
      ? GlobalResearch::TypeGrenade
      : rocketLevel > 0
      ? GlobalResearch::TypeRocket
      : airStrikeLevel > 0
    ...
    break;
  default:
    DebugTrace("Cycling secondary weapon on squad has caused poopers\n");
    m_weaponType = GlobalResearch::TypeGrenade;
    break;
  }
  DebugTrace("Weapon type is now %d\n", m_weaponType);
}
...
void InsertionSquad::Attack(Vector3 pos, bool withGrenade)
{
  ...
  for (int i = 0; i < m_entities.Size(); ++i)
  {
    ...
    {
      auto ent = static_cast<Squadie*>(m_entities[i]);
      if (ent->m_enabled && !ent->m_dead && ent->m_reloading == 0.0)
        canAttack.PutData(i);
    }
  }
  ...
  if (canAttack.Size() > 0)
  {
    ...
    double timeToWait = reloadTime / static_cast<double>(canAttack.Size());
...
void InsertionSquad::DirectControl(const TeamControls& _teamControls)
{
  ...
  if (_teamControls.m_leftButtonPressed || (_teamControls.m_consoleController && _teamControls.m_directUnitMove))
  {
...
bool InsertionSquad::IsSelectable() { return true; }
...
void InsertionSquad::RunAI(AI* _ai)
{
  ...
  if (m_aiTimer <= 0.0)
  {
    ...
    else
    {
      ...
      auto t = static_cast<AITarget*>(g_app->m_location->GetBuilding(_ai->FindNearestTarget(m_centrePos)));
      if (t && t->m_type == Building::TypeAITarget)
      {
        ...
        {
          auto target = static_cast<AITarget*>(g_app->m_location->GetBuilding(t->m_neighbours[i]->m_neighbourId));
          if (target)
          {
    ...
    m_retargetTimer(0.0)
{
  ...
  constexpr char laserName[] = "MarkerLaser";
  m_laser = m_shape->m_rootFragment->LookupMarker(laserName);
  ASSERT_TEXT(m_laser, "Squadie: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", laserName, m_shape->m_name);

  constexpr char brassName[] = "MarkerBrass";
  m_brass = m_shape->m_rootFragment->LookupMarker(brassName);
  ASSERT_TEXT(m_brass, "Squadie: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", brassName, m_shape->m_name);

  /*
      const char eye1Name[] = "MarkerEye1";
      m_eye1 = m_shape->m_rootFragment->LookupMarker( eye1Name );
      ASSERT_TEXT( m_eye1, "Squadie: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", eye1Name, m_shape->m_name );
  
      const char eye2Name[] = "MarkerEye2";
      m_eye2 = m_shape->m_rootFragment->LookupMarker( eye2Name );
      ASSERT_TEXT( m_eye2, "Squadie: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", eye2Name, m_shape->m_name );
  */
}
...
bool Squadie::Advance(Unit* _theUnit)
{
  ...
  if (!m_dead && m_onGround && m_inWater < 0.0)
  {
    ...
    if (distance > 0.01 || _unit->m_teleportId != -1)
    {
      ...
      if (factor > 2.0)
        factor = 2.0;
      if (landNormal.y < 0.6 && factor < 1.0)
        factor = 0.0;
      m_vel *= factor;
    ...
    m_angVel.x = 0.0;
    m_angVel.z = 0.0;
    constexpr double maxTurnRate = 20.0; // radians per second
...
void Squadie::Attack(const Vector3& _pos)
{
  if (m_reloading == 0.0)
  {
    //
...
bool Squadie::HasSecondaryWeapon() { return (m_secondaryTimer <= 0.0); }

void Squadie::FireSecondaryWeapon(const Vector3& _pos)
{
  auto squad = static_cast<InsertionSquad*>(g_app->m_location->GetUnit(m_id));
  DEBUG_ASSERT(squad);
