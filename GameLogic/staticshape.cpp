...
void StaticShape::Initialise(Building* _template)
{
  ...
  auto staticShape = static_cast<StaticShape*>(_template);
...
bool StaticShape::DoesRayHit(const Vector3& _rayStart, const Vector3& _rayDir, double _rayLen, Vector3* _pos, Vector3* norm)
{
  if (m_shape)
  {
...
bool StaticShape::DoesSphereHit(const Vector3& _pos, double _radius)
{
  if (m_shape)
  {
