...
Armour::Armour()
  : Entity(),
    ...
    m_conversionOrder(StateIdle) { SetType(TypeArmour); }
...
void Armour::Begin()
{
  ...
  constexpr char markerEntranceName[] = "MarkerEntrance";
  m_markerEntrance = m_shape->m_rootFragment->LookupMarker(markerEntranceName);
  ASSERT_TEXT(m_markerEntrance, "Armour: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", markerEntranceName,
              m_shape->m_name);

  /*
    const char markerFlagName[] = "MarkerFlag";
    m_markerFlag        = m_shape->m_rootFragment->LookupMarker( markerFlagName );
    ASSERT_TEXT( m_markerFlag, "Armour: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", markerFlagName, m_shape->m_name );
  */
...
{
  if (_damageType == DamageTypeLaser)
    return false;
  ...
  if (!m_dead)
  {
    ...
    int healthBandBefore = static_cast<int>(oldHealth / 20.0);
    int healthBandAfter = static_cast<int>(newHealth / 20.0);

    if (healthBandBefore != healthBandAfter || newHealth == 0)
    {
      double fractionDead = 1.0 - static_cast<double>(newHealth) / EntityBlueprint::GetStat(TypeArmour, StatHealth);
      fractionDead *= 0.5;
      if (newHealth == 0)
        fractionDead = 1.0;
...
void Armour::AdvanceToTargetPos()
{
  ...
  double angle = acos(toTarget * m_front);
  ...
  if (distance > 100.0 && angle < 1.0)
  {
    ...
    m_speed = min(m_speed, static_cast<double>(m_stats[StatSpeed]));
  }
...
{
  int numFound;
  g_app->m_location->m_entityGrid->GetNeighbours(s_neighbours, m_pos.x, m_pos.z, 40.0, &numFound);
  ...
  for (int i = 0; i < numFound; ++i)
  {
    ...
    if (ent && ent->m_type == TypeArmour && ent->m_id != m_id && !ent->m_dead)
    {
...
bool Armour::Advance(Unit* _unit)
{
  ...
  if (!g_app->Multiplayer() || m_vel.Mag() < 5)
    DetectCollisions();
  ...
  int numSmoke = 1 + static_cast<int>(velocity / 20.0);
...
void Armour::RunBlitzkriegAI(AI* _ai)
{
  ...
  if (g_app->m_multiwinia->m_aiType == Multiwinia::AITypeEasy)
    m_aiTimer = 20.0;
  if (g_app->m_multiwinia->m_aiType == Multiwinia::AITypeStandard)
    m_aiTimer = 10.0;
  ...
  if (numPassengers > 20 && m_vel.Mag() == 0.0)
  {
    ...
    if (!wait)
    {
      ...
      for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
      {
        if (g_app->m_location->m_buildings.ValidIndex(i))
        {
          auto zone = static_cast<MultiwiniaZone*>(g_app->m_location->m_buildings[i]);
          if (zone && zone->m_type == Building::TypeMultiwiniaZone)
          {
            if (zone->m_id.GetTeamId() == m_id.GetTeamId())
            {
              ...
              {
                auto linkedZone = static_cast<MultiwiniaZone*>(g_app->m_location->GetBuilding(zone->m_blitzkriegLinks[j]));
      ...
      if (!stop && g_app->m_multiwinia->m_teams[m_id.GetTeamId()].m_score == BLITZKRIEGSCORE_UNDERTHREAT)
      {
        ...
        if (aiType == Multiwinia::AITypeEasy)
          doIt = true;
        if (aiType == Multiwinia::AITypeStandard)
          doIt = (syncfrand(1.0) < 0.5);
        if (doIt)
        {
          validZones.Empty();
          auto base = static_cast<MultiwiniaZone*>(g_app->m_location->GetBuilding(MultiwiniaZone::s_blitzkriegBaseZone[m_id.GetTeamId()]));
          if (base && base->m_type == Building::TypeMultiwiniaZone)
          {
            ...
            {
              auto zone = static_cast<MultiwiniaZone*>(g_app->m_location->GetBuilding(base->m_blitzkriegLinks[i]));
              if (zone && zone->m_type == Building::TypeMultiwiniaZone)
              {
                if (zone->m_id.GetTeamId() != m_id.GetTeamId())
...
void Armour::GetBlitzkriegDropPoint(Building* _b)
{
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    ...
    {
      double range = 200.0;
      if (_b->m_type == Building::TypeMultiwiniaZone)
        range = static_cast<MultiwiniaZone*>(_b)->m_size * 1.2;
      Building* building = g_app->m_location->m_buildings[i];
      if ((building->m_pos - _b->m_pos).Mag() < range && building->m_type == Building::TypeGunTurret && building->m_id.GetTeamId() != m_id.
        GetTeamId())
      {
        numTurrets++;
...
void Armour::RunMobileBombAI(AI* _ai)
{
  ...
  if (m_suicideBuildingId == -1)
  {
    ...
    for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
    {
      ...
      {
        Building* b = g_app->m_location->m_buildings[i];
        if (b->m_type == Building::TypeGunTurret || b->m_type == Building::TypeSpawnPoint)
        {
          if (b->m_id.GetTeamId() != 255 && b->m_id.GetTeamId() != m_id.GetTeamId())
          {
...
void Armour::FindPickupPoint(AI* _ai)
{
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    if (g_app->m_location->m_buildings.ValidIndex(i))
    {
      ...
      {
        auto marker = static_cast<AIObjectiveMarker*>(b);
        if (marker->m_pickupAvailable && marker->m_armourObjective == 1 && (m_pos - marker->m_pos).Mag() > DARWINIAN_SEARCHRANGE_ARMOUR *
          1.5) // make sure we dont move 3 inches to the left and pick up all the guys we just dropped off
        {
          bool include[NUM_TEAMS];
          memset(include, false, sizeof(include));
          include[m_id.GetTeamId()] = true;
          if (g_app->m_location->m_entityGrid->GetNumNeighbours(marker->m_pos.x, marker->m_pos.z, DARWINIAN_SEARCHRANGE_ARMOUR, include) >
            25)
          {
...
void Armour::RunAssaultAI(AI* _ai)
{
  ...
  {
    auto objective = static_cast<AIObjective*>(g_app->m_location->GetBuilding(_ai->m_currentObjective));
      ...
      if (numApproaching == 0)
      {
        ...
        if (objective)
        {
          ...
          else
          {
            ...
            {
              auto marker = static_cast<AIObjectiveMarker*>(g_app->m_location->GetBuilding(objective->m_objectiveMarkers[i]));
              if (marker)
              {
                if (g_app->m_location->IsAttacking(m_id.GetTeamId()) && !g_app->m_location->IsFriend(
                  marker->m_id.GetTeamId(), m_id.GetTeamId()))
          ...
          auto marker = static_cast<AIObjectiveMarker*>(g_app->m_location->GetBuilding(index));
          if (marker)
...
void Armour::RunStandardAI(AI* _ai)
{
  ...
  if (m_numPassengers == 0 && m_state == StateIdle)
  {
    ...
    for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
    {
      ...
      {
        Building* b = g_app->m_location->m_buildings[i];
        if (b->m_type == Building::TypeSpawnPoint && b->m_id.GetTeamId() == m_id.GetTeamId())
        {
  ...
  if (m_numPassengers > 50)
  {
    if (m_conversionPoint == g_zeroVector)
    {
      ...
      for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
      {
        ...
        {
          Building* b = g_app->m_location->m_buildings[i];
          if (b->m_type == Building::TypeSpawnPoint && b->m_id.GetTeamId() != m_id.GetTeamId() && b->m_id.GetTeamId() != 255)
          {
            double dist = (m_pos - b->m_pos).Mag();
            if (dist < distance)
            {
...
bool Armour::PreventLoadOverlap()
{
  if (m_numPassengers == 0)
  {
    ...
    for (int i = 0; i < numFound; ++i)
    {
      auto a = static_cast<Armour*>(g_app->m_location->GetEntitySafe(s_neighbours[i], Entity::TypeArmour));
      if (a)
      {
        if (a == this)
          continue;

        if (a->m_id.GetTeamId() == m_id.GetTeamId() && a->m_state == StateLoading)
        {
          m_state = StateIdle;
          return true;
        }
...
bool Armour::RRValidPickupPoint(const Vector3& _pos)
{
  Team* team = g_app->m_location->m_teams[m_id.GetTeamId()];
  for (int i = 0; i < team->m_specials.Size(); ++i)
  {
    WorldObjectId id = *team->m_specials.GetPointer(i);
    auto a = static_cast<Armour*>(g_app->m_location->GetEntitySafe(id, Entity::TypeArmour));
    if (a)
    {
      if ((a->m_conversionPoint - _pos).Mag() < 100.0)
        return false;

      if (a->m_state == StateLoading && (a->m_pos - _pos).Mag() < 100.0)
        return false;
    }
  }
...
void Armour::FindIdleDarwinians()
{
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    if (g_app->m_location->m_buildings.ValidIndex(i))
    {
      ...
      {
        auto t = static_cast<AITarget*>(b);
        if (t->m_idleCount[m_id.GetTeamId()] > num && (t->m_pos - rocket->m_pos).Mag() > 400.0 && RRValidPickupPoint(t->m_pos))
        {
...
void Armour::RRFindPickupPoint()
{
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    ...
    {
      Building* b = g_app->m_location->m_buildings[i];
      if ((b->m_type == Building::TypeTrunkPort && b->m_id.GetTeamId() == m_id.GetTeamId()) || (b->m_type == Building::TypeAITarget &&
        rocket && (b->m_pos - rocket->m_pos).Mag() < 250.0) || (b->m_type == Building::TypeSpawnPoint && b->m_id.GetTeamId() == m_id.
        GetTeamId()))
      {
        int numFriends = g_app->m_location->m_entityGrid->GetNumFriends(b->m_pos.x, b->m_pos.z, 200.0, m_id.GetTeamId());
        if (numFriends > 20)
...
void Armour::RRFindDropoffPoint(AI* _ai)
{
  ...
  for (int i = 0; i < NUM_TEAMS; ++i)
  {
    ...
    auto theirRocket = static_cast<EscapeRocket*>(g_app->m_location->GetBuilding(g_app->m_multiwinia->m_teams[i].m_rocketId));
    if (theirRocket)
    {
      if (theirRocket->m_fuel >= 75.0 && theirRocket->m_fuel > rocket->m_fuel)
      {
        rocketAttack = true;
  ...
  if (buildingId != -1)
  {
    ...
    if (b)
    {
      ...
      if (g_app->m_multiwinia->m_aiType > Multiwinia::AITypeEasy)
      {
        pos.x += cos(syncfrand(M_PI*2.0)) * 100.0;
        pos.z += sin(syncfrand(M_PI*2.0)) * 100.0;
...
{
  if (!g_app->m_location->m_landscape.IsInLandscape(_orders))
    return;
  m_newOrdersTimer = 0.0;
...
{
  if (!g_app->m_location->m_landscape.IsInLandscape(_wayPoint))
    return;
...
void Armour::AISetConversionPoint(const Vector3& _pos)
{
...
bool Armour::IsLoading() { return (m_state == StateLoading && m_numPassengers < Capacity() && m_newOrdersTimer > 1.0); }

bool Armour::IsUnloading()
{
  return (m_state == StateUnloading && m_numPassengers > 0 && m_newOrdersTimer > 1.0 && GetNetworkTime() >= m_previousUnloadTimer +
    ARMOUR_UNLOADPERIOD);
}
...
int Armour::GetNumPassengers() { return m_numPassengers + m_passengerOverflow; }

double Armour::GetSpeed() { return m_speed; }

bool Armour::IsSelectable() { return true; }
...
void Armour::Render(double _predictionTime)
{
  ...
  if (m_smoothFront == g_zeroVector)
    m_smoothFront = predictedFront;
  if (m_smoothUp == g_zeroVector)
    m_smoothUp = predictedUp;
  if (m_smoothYpos == 0.0)
    m_smoothYpos = predictedPos.y;
...
bool Armour::IsRouteClear(const Vector3& _pos)
{
  double jump = 20.0;
  ...
  for (int i = 0; i < steps; ++i)
  {
    ...
    for (int b = 0; b < buildings->Size(); ++b)
    {
      ...
      if (building && building->m_type == Building::TypeLaserFence && !g_app->m_location->IsFriend(
        building->m_id.GetTeamId(), m_id.GetTeamId()))
      {
        auto fence = static_cast<LaserFence*>(building);

        if (fence->IsEnabled() && building->DoesRayHit(pos, dir, jump))
          return false;
      }
    }
...
bool Armour::IsRouteClear(const Vector3& _fromPos, const Vector3& _toPos)
{
  double jump = 20.0;
  ...
  for (int i = 0; i < steps; ++i)
  {
    ...
    {
      Building* building = g_app->m_location->GetBuilding(buildings->GetData(b));
      if (building && building->m_type == Building::TypeLaserFence && building->DoesRayHit(pos, dir, jump))
        return false;
    }
    pos += jump * dir;
  }
...
void Armour::CreateRoute(const Vector3& _pos)
{
...
void Armour::RegisterPassenger() { m_registeredPassengers++; }

bool Armour::RayHit(const Vector3& rayStart, const Vector3& rayDir)
{
  double gameTime = g_gameTimer.GetGameTime();

  Vector3 predictedPos = m_pos;
  predictedPos.y += sin(gameTime + m_id.GetUniqueId()) * 2;

  Vector3 predictedUp = m_up;
  predictedUp.x += sin((gameTime + m_id.GetUniqueId()) * 2) * 0.05;
  predictedUp.z += cos(gameTime + m_id.GetUniqueId()) * 0.05;
