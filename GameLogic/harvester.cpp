...
void Harvester::SetWaypoint(const Vector3& _wayPoint)
{
  m_retargetTimer = 0.0f;
...
int Harvester::GetNumSpirits() { return m_spirits.Size(); }
  ...
  if (m_spirits.Size() < GetMaxSpirits())
  {
    ...
    for (int i = 0; i < g_app->m_location->m_spirits.Size(); ++i)
    {
      if (g_app->m_location->m_spirits.ValidIndex(i))
      {
        ...
        if (theDist <= ENGINEER_SEARCHRANGE && theDist < closest && s->m_state == Spirit::StateFloating && s->m_broken < 0.0f)
        {
          found = s;
          pos = s->m_pos;
          closest = theDist;
...
bool Harvester::Advance(Unit* _unit)
{
  ...
  case StateHarvesting:
    amIDead = AdvanceHarvesting();
    break;
  case StateToSpawnPoint:
    amIDead = AdvanceToSpawnPoint();
    break;
  }
  ...
  if (!m_movedThisAdvance)
  {
    ...
    double targetHeight = max(g_app->m_location->m_landscape.m_heightMap->GetValue(m_pos.x, m_pos.z), 0.0) + GetHoverHeight();
  ...
  for (int i = 0; i < m_spirits.Size(); ++i)
  {
    ...
    if (g_app->m_location->m_spirits.ValidIndex(spiritId))
    {
      ...
      if (s && s->m_state == Spirit::StateHarvested)
      {
        ...
        targetPos.x += cos(timeIndex + i) * radius;
        targetPos.z += sin(timeIndex + i) * radius;
        targetPos.y = m_pos.y;
        ...
        if (!inRange)
        {
          if (abs(s->m_pos.y - targetPos.y) < 5.0 && (s->m_pos - targetPos).Mag() < 50.0)
          {
...
void Harvester::AdvanceGrenades()
{
  ...
  for (int i = 0; i < m_grenades.Size(); ++i)
  {
    if (g_app->m_location->m_effects.ValidIndex(m_grenades[i]))
    {
      ...
      targetPos.x += cos(timeIndex + i * 50.0) * radius;
      targetPos.z += sin(timeIndex + i * 50.0) * radius;
...
bool Harvester::SearchForSpawnPoint()
{
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    ...
    {
      Building* building = g_app->m_location->m_buildings[i];
      if (building->m_type == Building::TypeSpawnPoint && g_app->m_location->IsFriend(building->m_id.GetTeamId(), m_id.GetTeamId()))
      {
        double distance = (building->m_pos - m_pos).Mag();
...
{
  auto sp = static_cast<SpawnPoint*>(g_app->m_location->GetBuilding(m_buildingId));

  if (!sp || sp->m_type != Building::TypeSpawnPoint)
  {
    bool found = SearchForSpawnPoint();
    sp = static_cast<SpawnPoint*>(g_app->m_location->GetBuilding(m_buildingId));
...
void Harvester::RenderBeam(double _predicionTime)
{
  ...
  while (y > totalHeight) { y -= m_hoverHeight; }
...
{
  double timeIndex = GetNetworkTime() + m_id.GetUniqueId() * 20;
  return m_hoverHeight + sin(timeIndex) * (m_hoverHeight * 0.05);
}
