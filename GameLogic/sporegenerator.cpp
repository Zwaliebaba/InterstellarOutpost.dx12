    ...
    m_state(StateIdle)
{
  ...
  ASSERT_TEXT( m_eggMarker, "SporeGenerator: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", eggMarkerName, m_shape->m_name );
  ...
  for( int i = 0; i < SPOREGENERATOR_NUMTAILS; ++i )
  {
      ...
      ASSERT_TEXT( m_tail[i], "SporeGenerator: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", name, m_shape->m_name );
  }*/
}
...
void SporeGenerator::Begin()
{
  ...
  constexpr char eggMarkerName[] = "MarkerEggs";
  m_eggMarker = m_shape->m_rootFragment->LookupMarker(eggMarkerName);
  ASSERT_TEXT(m_eggMarker, "SporeGenerator: Can't get Marker(%s) from shape(%s), probably a corrupted file\n", eggMarkerName,
              m_shape->m_name);
...
bool SporeGenerator::ChangeHealth(int _amount, int _damageType)
{
  ...
  if (!m_dead && _amount < -1)
  {
    ...
    double fractionDead = 1.0 - static_cast<double>(m_stats[StatHealth]) / EntityBlueprint::GetStat(TypeSporeGenerator, StatHealth);
...
bool SporeGenerator::AdvanceToTargetPosition()
{
  ...
  m_vel.RotateAround(right * sin(GetNetworkTime() * 3.0) * SERVER_ADVANCE_PERIOD * 1.5);
...
bool SporeGenerator::AdvanceEggLaying()
{
  ...
  if (distance < 50.0)
  {
    ...
    if (m_eggTimer <= 0.0)
    {
      m_eggTimer = 2.0 + syncfrand(2.0) - 1.0 * static_cast<double>(g_app->m_difficultyLevel) / 10.0;
...
void SporeGenerator::RenderTail(const Vector3& _from, const Vector3& _to, double _size)
{
