...
static ControlName s_controls[] = {ControlCameraLeft, false, "control_event_left", ControlCameraRight, false, "control_event_right",
                                   ControlCameraForwards, false, "control_event_forwards", ControlCameraBackwards, false,
                                   "control_event_backwards", ControlCameraUp, false, "control_event_up", ControlCameraDown, false,
                                   "control_event_down", ControlCameraZoom, false, "control_event_zoom", ControlUnitDeselect, true,
                                   "control_event_deselect", ControlSkipMessage, true, "control_event_skipmessage", ControlGesturesChatLog,
                                   true, "control_event_chatlog", ControlIconsChatLog, true, "control_event_iconschatlog",
                                   ControlIconsTaskManagerDisplay, true, "control_event_iconstaskmanagerdisplay",
                                   ControlIconsTaskManagerEndTask, true, "control_event_iconstaskmanagerendtask", ControlNull, false,
                                   nullptr};
...
class RestoreDefaultsButton : public DarwiniaButton
{
  public:
    ...
    {
      TextFileReader reader(InputPrefs::GetSystemPrefsPath().c_str());
      if (reader.IsOpen())
      {
...
class ChangeKeybindingButton : public DarwiniaButton
{
  public:
    ...
    void Render(int x, int y, bool highlighted, bool clicked) override
    {
      ...
      int time = static_cast<int>(g_gameTime * 3.0);
...
class ControlMethodDropDownMenu : public DropDownMenu
{
  void SelectOption(int _option) override
  {
    ...
    if (parent && _option != parent->m_controlMethod)
    {
      ...
      auto newMenu = static_cast<DropDownMenu*>(parent->GetButton("newcontrols_prefsoption"));
      if (newMenu)
...
PrefsKeybindingsWindow::PrefsKeybindingsWindow()
  : DarwiniaWindow("dialog_inputoptions")
{
  unsigned i;
...
void PrefsKeybindingsWindow::Create()
{
  ...
  controlMethod->SetShortProperties("newcontrols_prefsoption", x, y += border, buttonW, buttonH, LANGUAGEPHRASE("newcontrols_prefsoption"));
  controlMethod->AddOption("newcontrols_prefs_gestures", 0);
  controlMethod->AddOption("newcontrols_prefs_icons", 1);
  ...
  box->SetShortProperties("invert", 10, y + h, m_w - 20, (num_controls * h) + border, UnicodeString("invert"));
  RegisterButton(box);
  ...
  for (unsigned j = 0; j < num_controls; ++j)
  {
    ...
    but->SetShortProperties(s_controls[i].name, x, y += h, buttonW, buttonH, LANGUAGEPHRASE(s_controls[i].name));
    but->m_fontSize = GetMenuSize(15);
    but->m_centered = true;
    const char* keyName = m_bindings[i]->noun.c_str();
    but->m_caption = UnicodeString(keyName);
    RegisterButton(but);
    m_buttonOrder.PutData(but);
  }

#ifdef TARGET_OS_MACOSX
  DropDownMenu* numMouseButtons = new DropDownMenu(); numMouseButtons->SetShortProperties(" ", x, y += h, buttonW, buttonH, "", "");
  numMouseButtons->AddOption("1", 1); numMouseButtons->AddOption("3", 3); numMouseButtons->RegisterInt(&m_numMouseButtons);
  ...
  restore->SetShortProperties("dialog_restoredefaults", border, y - h, m_w - border * 2, buttonH, LANGUAGEPHRASE("dialog_restoredefaults"));
  ...
  cancel->SetShortProperties("dialog_close", border, y, buttonW, buttonH, LANGUAGEPHRASE("dialog_close"));
  ...
  apply->SetShortProperties("dialog_apply", m_w - buttonW - border, y, buttonW, buttonH, LANGUAGEPHRASE("dialog_apply"));
...
void PrefsKeybindingsWindow::Render(bool _hasFocus)
{
  ...
  {
    int i = s_indices[j];
    const char* eventName = s_controls[i].name;
    g_editorFont.DrawText2D(x, y += h, size, LANGUAGEPHRASE(eventName));
  }
}
