...
void Officer::Begin()
{
  ...
  constexpr char flagMarkerName[] = "MarkerFlag";
...
void Officer::Render(double _predictionTime)
{
  ...
  if (m_enabled && !m_dead)
  {
    if (m_id.GetTeamId() != g_app->m_globalWorld->m_myTeamId || g_app->m_hideInterface)
      RenderFlag(_predictionTime);
  }

  if (m_id.GetTeamId() == g_app->m_globalWorld->m_myTeamId && m_orderRouteId != -1)
  {
    //g_app->m_location->m_routingSystem.SetRouteIntensity( m_orderRouteId, 0.5 );
  }
}

void Officer::RenderSpirit(const Vector3& _pos)
{
  Vector3 pos = _pos;
...
void Officer::RenderShield(double _predictionTime)
{
  ...
  for (int i = 0; i < m_shield; ++i)
  {
    ...
    spiritPos.x += sin(timeIndex * 1.8 + i * 2.0) * 10.0;
    spiritPos.y += cos(timeIndex * 2.1 + i * 1.2) * 6.0;
    spiritPos.z += sin(timeIndex * 2.4 + i * 1.4) * 10.0;
...
void Officer::RenderFlag(double _predictionTime)
{
  ...
  if (g_app->Multiplayer())
  {
    ...
    if (camDist > 100)
      size *= 1.0f + scaleFactor * (camDist - 100) / 1500;
...
void Officer::CalculateBoundingSphere(Vector3& centre, double& radius)
{
  ...
  if (scaleFactor > 0.0)
  {
    double camDist = (g_app->m_camera->GetPos() - m_pos).Mag();
    if (camDist > 100)
      amountToScaleBy *= 1.0 + scaleFactor * (camDist - 100) / 1000;
  }
...
Vector3 Officer::GetFormationPositionFromIndex(const Vector3& pos, int positionIndex, const Vector3& front, int count)
{
  //
  ...
  Vector3 targetPos = pos + front * row * rowSpread + right * column * columnSpread;
...
bool Officer::IsInFormationMode() { return (m_orders == OrderFollow && m_formation); }
...
bool Officer::AdvanceToWaypoint()
{
  ...
  if (m_formation)
    m_orderPosition = m_pos + m_front * 100;
...
bool Officer::Advance(Unit* _unit)
{
  ...
  if (m_orders == OrderGoto)
  {
    ...
    if (triggerOrder)
    {
      ...
      {
        auto orders = new MultiwiniaOfficerOrders();
        orders->m_pos = m_pos + Vector3(0, 2, 0);
        orders->m_wayPoint = m_orderPosition;
  ...
  if (m_targetFront != g_zeroVector && m_state != StateToWaypoint)
  {
  ...
  if (m_orderRouteId != -1 && m_orders == OrderGoto)
  {
    Route* route = g_app->m_location->m_routingSystem.GetRoute(m_orderRouteId);
...
void Officer::RunAI(AI* _ai)
{
  ...
  if (m_orders == OrderGoto)
  {
    ...
    auto thisTarget = static_cast<AITarget*>(g_app->m_location->GetBuilding(currentTarget));
    if (thisTarget)
    {
          ...
          if (targetBuilding->m_radarTarget)
          {
            ...
            for (int i = 0; i < nearbyBuildings->Size(); ++i)
            {
              ...
              if (building->m_type == Building::TypeRadarDish)
              {
                ...
                static_cast<Teleport*>(building)->GetEntrance(entrancePos, entranceFront);
  ...
  int numFound;
  g_app->m_location->m_entityGrid->GetEnemies(s_neighbours, m_pos.x, m_pos.z, DARWINIAN_SEARCHRANGE_THREATS * 3.0, &numFound,
                                              m_id.GetTeamId());
  ...
  if (m_state == StateToWaypoint)
    return;

  auto t = static_cast<AITarget*>(g_app->m_location->GetBuilding(_ai->FindNearestTarget(m_pos)));
  if (t)
  {
    bool moveOn = true;
    if (t->IsImportant())
    {
      ...
      if (t->NeedsDefending(m_id.GetTeamId()))
      {
        ...
        {
          auto thisTarget = static_cast<AITarget*>(g_app->m_location->GetBuilding(t->m_neighbours[j]->m_neighbourId));
          if (thisTarget && thisTarget->m_id.GetTeamId() != m_id.GetTeamId() && thisTarget->m_id.GetTeamId() != 255)
    ...
    if (moveOn && FormationFull()) // no sense going anywhere else with a half full formation
    {
      ...
      for (int j = 0; j < g_app->m_location->m_buildings.Size(); ++j)
      {
        ...
        {
          auto thisTarget = static_cast<AITarget*>(g_app->m_location->m_buildings[j]);
          if (!thisTarget || thisTarget->m_type != Building::TypeAITarget)
            continue;
          if (!g_app->m_location->IsWalkable(m_pos, thisTarget->m_pos, true))
            continue;

          bool valid = false;
          if (thisTarget->m_id.GetTeamId() != m_id.GetTeamId() && thisTarget->m_id.GetTeamId() != 255)
          {
            // owned by the enemy
            valid = true;
          }

          if (thisTarget->IsImportant() && thisTarget->m_id.GetTeamId() == 255)
          {
            // valuable, uncaptured building (spawn point, multiwinia zone etc)
            valid = true;
          }

          if (valid && (thisTarget->m_pos - m_pos).Mag() < nearest)
          {
      ...
      if (id != -1)
      {
        ...
        if (building)
          SetWaypoint(building->m_pos);
      }
    }
  }
}

void Officer::SetWaypoint(const Vector3& _wayPoint)
{
...
bool Officer::IsThereATeleportClose(const Vector3& _orders)
{
...
void Officer::SetOrders(const Vector3& _orders, bool directRoute)
{
  if (g_app->IsSinglePlayer() && !g_app->m_location->IsWalkable(m_pos, _orders))
  {
  ...
  else
  {
    ...
    else if (!formationToggle)
    {
      ...
      m_orderRouteId = g_app->m_location->m_routingSystem.GenerateRoute(m_pos, _orders, directRoute);
      if (m_id.GetTeamId() == g_app->m_globalWorld->m_myTeamId)
      {
...
{
  DebugTrace("Setting previous mode\n");
  if (g_app->Multiplayer())
    return;
...
OfficerOrders::OfficerOrders()
  : WorldObject(),
    m_arrivedTimer(-1.0) { m_type = EffectOfficerOrders; }
...
MultiwiniaOfficerOrders::MultiwiniaOfficerOrders()
  : WorldObject(),
    ...
    m_routeWayPointId(-1),
    m_arrivedTimer(-1.0),
    m_dropArrowTimer(0.0) { m_type = EffectOfficerOrders; }
...
bool MultiwiniaOfficerOrders::Advance()
{
  ...
  else
  {
    ...
    if (timeNow >= m_dropArrowTimer)
    {
      ...
      auto orders = new OfficerOrderTrail();
...
{
  if (!g_app->m_location->IsFriend(g_app->m_globalWorld->m_myTeamId, m_id.GetTeamId()))
    return;
...
OfficerOrderTrail::OfficerOrderTrail()
  : WorldObject(),
    m_birthTime(0.0) { m_type = EffectOfficerOrderTrail; }
...
void OfficerOrderTrail::Render(double _time)
{
  ...
  glBegin(GL_QUADS);
  glTexCoord2i(0, 1);
  glVertex3dv((m_pos - camRight + camUp).GetData());
  glTexCoord2i(1, 1);
  glVertex3dv((m_pos + camRight + camUp).GetData());
  glTexCoord2i(1, 0);
  glVertex3dv((m_pos + camRight - camUp).GetData());
