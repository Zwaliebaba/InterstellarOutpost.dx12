  ...
  if (g_prefsManager->GetInt("TextLanguageTest", 0) == 1)
  {
    ...
    for (int i = 0; i < phrases->Size(); ++i)
    {
      if (phrases->ValidIndex(i))
      {
        ...
        if (!result) { fprintf(output, "ERROR : Invalid keycode with string : '%s'\n", phrase->m_key); }
            ...
            strstr(phrase->m_string, "[TAB]"))
        {
          ...
          if (!ISLANGUAGEPHRASE(iconsId)) { fprintf(output, "WARNING : StringID found that may require ICONS version : '%s'\n", phrase->m_key); }
        }
        //#endif
      }
    }
...
bool Sepulveda::BuildCaption(const char *_baseString, char *_dest)
{
...
void Sepulveda::Advance()
{
  ...
  if (m_caption[0] != '\0')
  {
    ...
    else { m_history[0]->m_timeSync = GetHighResTime(); }
  }
  ...
  if (m_caption[0] == '\0' && m_msgQueue.Size() > 0)
  {
    ...
    auto sepulvedaCaption = std::unique_ptr<SepulvedaCaption>(new SepulvedaCaption());
  ...
  if (chatLog)
  {
    ...
    for (int i = 0; i < m_history.Size(); ++i) { m_history[i]->m_timeSync = max(m_history[i]->m_timeSync, timeNow - 3.0f); }
  }
...
void Sepulveda::Say(char *_stringId)
{
  ...
  if (numMouseButtons == 1)
  {
    ...
    if (ISLANGUAGEPHRASE(phrase1mb)) { actualStringId = NewStr(phrase1mb); }
  }
  ...
  if (controlMethod == 1)
  {
    ...
    if (ISLANGUAGEPHRASE(phrase)) { actualStringId = NewStr(phrase); }
  }

  if (!actualStringId) { actualStringId = NewStr(_stringId); }
...
void Sepulveda::HighlightPosition(const Vector3 &_pos, float _radius, char *_highlightName)
{
  //
  ...
  if (!found)
  {
    auto highlight = new SepulvedaHighlight(_highlightName);
...
bool Sepulveda::IsHighlighted(const Vector3 &_pos, float _radius, char *_highlightName)
{
  for (int i = 0; i < m_highlights.Size(); ++i)
  {
    SepulvedaHighlight *highlight = m_highlights[i];
    if (!_highlightName || strcmp(_highlightName, highlight->m_name) == 0) { if (highlight->m_pos == _pos && highlight->m_radius == _radius) { return true; } }
  }
...
void Sepulveda::ClearHighlights(char *_highlightName)
{
  ...
  {
    SepulvedaHighlight *highlight = m_highlights[i];
    if (!_highlightName || strcmp(_highlightName, highlight->m_name) == 0) { highlight->Stop(); }
  }
}
...
{
  if (m_caption[0] != '\0') { m_caption[0] = '\0'; }
...
void Sepulveda::DemoGesture(const char *_gestureDemoName, float _startDelay)
{
...
bool Sepulveda::IsTalking() { return (m_caption[0] != '\0' || m_msgQueue.Size() > 0); }
...
{
  if (m_caption[0] == '\0') { return 0.0f; }
  int totalLen = strlen(m_caption);
  int narratorSpeed = g_prefsManager->GetInt("TextSpeed", 15);
  float timeToWait = 0.6f * static_cast<float>(totalLen) / static_cast<float>(narratorSpeed);
...
float Sepulveda::CalculatePauseTime()
{
  ...
  int totalLen = strlen(m_caption);
  int narratorSpeed = g_prefsManager->GetInt("TextSpeed", 15);
  float timeToWait = static_cast<float>(totalLen) / static_cast<float>(narratorSpeed);
...
void Sepulveda::RenderGestureDemo()
{
  ...
  if (timeSinceStart > totalDuration) { timeSinceStart = totalDuration; }
...
void Sepulveda::SetCutsceneMode(bool _cutsceneMode) { m_cutsceneMode = _cutsceneMode; }


bool Sepulveda::IsInCutsceneMode() { return m_cutsceneMode; }
...
void Sepulveda::RenderTextBoxCutsceneMode()
{
  ...
  glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
  if (g_app->m_script->m_permitEscape) { g_gameFont.DrawText2DRight(textBoxX + textBoxW, 590.0f, 7.0f, LANGUAGEPHRASE("sepulveda_pressreturn_skipall")); }
  ...
  else if (m_caption[0] != '\0') { g_gameFont.DrawText2DRight(textBoxX + textBoxW, 590.0f, 7.0f, LANGUAGEPHRASE("sepulveda_pressreturn")); }
...
void Sepulveda::RenderTextBox()
{
  ...
  if (s_highestY < s_smoothHighestY) { s_smoothHighestY = s_highestY; }
  else { s_smoothHighestY = s_smoothHighestY * 0.95f + s_highestY * 0.05f; }
  ...
  else if (m_caption[0] != '\0') { g_gameFont.DrawText2DRight(textBoxX + textBoxW, 590.0f, 7.0f, LANGUAGEPHRASE("sepulveda_pressreturn"), m_msgQueue.Size()); }
...
void Sepulveda::Render()
{
  ...
  if (m_gestureDemo) { RenderGestureDemo(); }
  ...
  if (!g_app->m_taskManagerInterface->m_visible)
  {
    ...
    {
      if (m_cutsceneMode && m_fade > 0.01f) { RenderTextBoxCutsceneMode(); }
      else
      {
        m_cutsceneMode = false;
        RenderTextBox();
        ...
        bool chatLog = g_app->m_sepulveda->ChatLogVisible();
        if (chatLog) { RenderScrollBar(); }
        else { m_scrollbarOffset = 0.0f; }
      }
    }
  }
  else { RenderTextBoxTaskManagerMode(); }
...
void Sepulveda::RenderScrollBar()
{
  ...
  float fractionX = mouseX / static_cast<float>(g_app->m_renderer->ScreenW());
  float fractionY = mouseY / static_cast<float>(g_app->m_renderer->ScreenH());
...
SepulvedaHighlight::SepulvedaHighlight(char *_name)
  : m_radius(0.0f),
    m_alpha(0.0f),
    m_ended(false),
    m_name(nullptr) { m_name = NewStr(_name); }


SepulvedaHighlight::~SepulvedaHighlight() { delete [] m_name; }


void SepulvedaHighlight::Start(const Vector3 &_pos, float _radius)
{
  m_pos = _pos;
  m_radius = _radius;
  m_ended = false;
...
void SepulvedaHighlight::Stop() { m_ended = true; }
...
bool Sepulveda::ChatLogVisible()
{
  ...
  if (g_prefsManager->GetInt("ControlMethod") == 1 &&
      g_inputManager->controlEvent(ControlIconsChatLog)) { return true; }

  if (g_prefsManager->GetInt("ControlMethod") == 0 &&
      g_inputManager->controlEvent(ControlGesturesChatLog)) { return true; }
