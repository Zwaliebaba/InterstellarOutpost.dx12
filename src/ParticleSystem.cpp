...
// *** Initialize
void Particle::Initialize(const LegacyVector3& _pos, const LegacyVector3& _vel, int _typeId, float _size)
{
  ...
  const ParticleType& type = m_types[_typeId];
  float amount = frand();
...
{
  const ParticleType& type = m_types[m_typeId];
  ...
  if (m_typeId == TypeExplosionDebris)
  {
    if (frand() < 0.3f)
    {
      ...
      g_app->m_particleSystem->CreateParticle(pos, vel, TypeRocketTrail, m_size / 2.0f);
  ...
  if (!particleCreated)
  {
    if (m_typeId == TypeSpark || m_typeId == TypeBrass || m_typeId == TypeBlueSpark || m_typeId == TypeLeaf)
    {
...
void Particle::Render(float _predictionTime)
{
  ...
  if (m_typeId == TypeMissileTrail)
  {
    ...
    float fraction = static_cast<float>(alpha) / 100.0f;
