...
void LogEntityGridError(WorldObjectId _id, const LegacyVector3& _pos, int _error)
{
  ...
  auto theError = NEW EntityGridError();
  theError->m_id = _id;
  theError->m_pos = _pos;
...
void EntityGrid::EnsureMaxNeighbours(int _maxNeighbours)
{
  if (_maxNeighbours > m_maxNeighbours)
  {
    ...
    auto newNeighbours = NEW WorldObjectId[newMaxNeighbours];
...
// *** GetGridIndexX
int EntityGrid::GetGridIndexX(float _worldX) { return static_cast<int>(_worldX * m_cellSizeXRecip); }

// *** GetGridIndexZ
int EntityGrid::GetGridIndexZ(float _worldZ) { return static_cast<int>(_worldZ * m_cellSizeZRecip); }
...
void EntityGrid::RemoveObject(WorldObjectId _objectID, float _worldX, float _worldZ, float _radius)
{
  ...
  if (_radius == 0.0f)
  {
    ...
    if (!success)
      LogEntityGridError(_objectID, LegacyVector3(_worldX, 0.0f, _worldZ), 2);
  }
  else
  {
    int leftMostCell = GetGridIndexX(_worldX - _radius / 2);
    ...
    for (int x = leftMostCell; x <= rightMostCell; ++x)
    {
      for (int z = upMostCell; z <= downMostCell; ++z)
      {
        ...
        if (!success)
          LogEntityGridError(_objectID, LegacyVector3(x * m_cellSizeX, 0.0f, z * m_cellSizeZ), 2);
      }
    }
  }
}
...
// *** UpdateObject
void EntityGrid::UpdateObject(WorldObjectId _objectId, float _oldWorldX, float _oldWorldZ, float _newWorldX, float _newWorldZ,
                              float _radius)
{
  ...
  if (oldIndexX != newIndexX || oldIndexZ != newIndexZ || _radius > 0.0f)
  {
...
// *** GetEnemies
WorldObjectId* EntityGrid::GetEnemies(float _worldX, float _worldZ, float _range, int* _numFound, unsigned char _myTeam)
{
...
WorldObjectId EntityGrid::GetBestEnemy(float _worldX, float _worldZ, float _minRange, float _maxRange, unsigned char _myTeam)
{
  int numFound;
...
// *** GetFriends
WorldObjectId* EntityGrid::GetFriends(float _worldX, float _worldZ, float _range, int* _numFound, unsigned char _myTeam)
{
  bool include[NUM_TEAMS];
...
// *** GetNeighbours
WorldObjectId* EntityGrid::GetNeighbours(float _worldX, float _worldZ, float _range, int* _numNeighbours)
{
  bool include[NUM_TEAMS];
...
// *** GetNeighbours
WorldObjectId* EntityGrid::GetNeighbours(float _worldX, float _worldZ, float _range, int* _numFound, bool _includeTeam[NUM_TEAMS])
{
  int numFoundSoFar = 0;
      ...
      for (int t = 0; t < NUM_TEAMS; ++t)
      {
        if (_includeTeam[t])
        {
          ...
          for (int i = 0; i < ogc->m_arraySize; i++)
          {
            ...
            if (!obj)
            {
              LogEntityGridError(objId, LegacyVector3(x * m_cellSizeX, 0.0f, z * m_cellSizeZ), 1);
              continue;
            }
...
void EntityGrid::Render()
{
  ...
  float cellSizeX = g_app->m_location->m_landscape.GetWorldSizeX() / static_cast<float>(m_numCellsX);
  float cellSizeZ = g_app->m_location->m_landscape.GetWorldSizeZ() / static_cast<float>(m_numCellsZ);
  ...
  {
    float worldX = (static_cast<float>(x) / static_cast<float>(m_numCellsX)) * g_app->m_location->m_landscape.GetWorldSizeX();
    for (z = 0; z < m_numCellsZ; ++z)
    {
      for (int t = 0; t < NUM_TEAMS; ++t)
      {
        ...
        if (numEntities > 0)
        {
          float worldZ = (static_cast<float>(z) / static_cast<float>(m_numCellsZ)) * g_app->m_location->m_landscape.GetWorldSizeZ();
          float worldY = g_app->m_location->m_landscape.m_heightMap->GetValue(worldX, worldZ) + 10.0f;
          worldY = 100.0f + t * 30.0f;

          float alpha = 128;
          RGBAColor col = g_app->m_location->m_teams[t].m_colour;
