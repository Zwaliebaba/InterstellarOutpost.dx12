...
// *** GetPowerOfTwo
int LandscapeTile::GetPowerOfTwo(int x) { return static_cast<int>(ceil(Log2(x))); }
  ...
  if (res > 0)
  {
    ...
    for (int z = 0; z < res; ++z)
    {
      for (int x = 0; x < res; ++x)
      {
        ...
        *target = 'A' + actualVal / 16;
        *(target + 1) = 'A' + actualVal - actualVal / 16 * 16;
...
float LandscapeTile::GenerateNoise(float _halfSize, float _height)
{
  ...
  val *= 0.1f + powf(fabsf(_height), m_lowlandSmoothingFactor) * 0.15f;
  return val;
}
...
void LandscapeTile::GenerateDiamondMidpoint(int _x, int _z, int _halfSize)
{
  ...
  {
    case 0:
      newHeight = m_heightMap->GetData(_x - _halfSize, _z) + m_heightMap->GetData(_x + _halfSize, _z) + m_heightMap->
        GetData(_x, _z - _halfSize) + m_heightMap->GetData(_x, _z + _halfSize);
      newHeight *= 0.25f;
      break;
    case 1:
      if (darwiniaRandom() & 1)
  ...
  newHeight += GenerateNoise(static_cast<float>(_halfSize), newHeight);
...
void LandscapeTile::Generate(LandscapeDef* _def)
{
  ...
  // Initialize value of all height samples
...
void Landscape::MergeTileIntoLandscape(const LandscapeTile* _tile)
{
  ...
  _tile->m_heightMap->m_cellSizeX = _tile->m_size / static_cast<float>(_tile->m_heightMap->GetNumColumns());
  _tile->m_heightMap->m_cellSizeY = _tile->m_size / static_cast<float>(_tile->m_heightMap->GetNumRows());
  ...
  for (unsigned short z = 0; z < numCells; ++z)
  {
    ...
    float tileZ = m_heightMap->GetRealY(static_cast<float>(z + posZ));

    for (unsigned short x = 0; x < numCells; ++x)
    {
      //			float tileX = posX + (float)x * m_heightMap->m_cellSizeX;
      float tileX = m_heightMap->GetRealX(static_cast<float>(x + posX));
...
void Landscape::GenerateNormals()
{
  ...
  for (unsigned short z = 0; z < m_heightMap->GetNumRows(); ++z)
  {
    ...
    for (unsigned short x = 0; x < m_heightMap->GetNumColumns(); ++x)
    {
      ...
      LegacyVector3 normA = (vectW ^ vectN).Normalize();
      LegacyVector3 normB = (vectE ^ vectS).Normalize();
      LegacyVector3 norm = (normA + normB).Normalize();
...
void Landscape::Init(LandscapeDef* _def, bool _justMakeTheHeightMap)
{
  ...
  if (!m_heightMap || m_heightMap->m_cellSizeX != _def->m_cellSize || m_heightMap->m_cellSizeY != _def->m_cellSize || m_worldSizeX
    != _def->m_worldSizeX || m_worldSizeZ != _def->m_worldSizeZ)
  {
    delete m_heightMap;
    delete m_normalMap;
    m_heightMap = NEW SurfaceMap2D<float>(_def->m_worldSizeX, _def->m_worldSizeZ, 0.0f, 0.0f, _def->m_cellSize, _def->m_cellSize,
                                          m_outsideHeight);
    m_normalMap = NEW SurfaceMap2D<LegacyVector3>(_def->m_worldSizeX, _def->m_worldSizeZ, 0.0f, 0.0f, _def->m_cellSize,
                                                  _def->m_cellSize, g_upVector);
  }
...
bool Landscape::IsInLandscape(const LegacyVector3& _pos)
{
...
bool Landscape::RayHit(const LegacyVector3& _rayStart, const LegacyVector3& _rayDir, LegacyVector3* _result) const
{
  if (!m_heightMap)
    return false;

  LegacyVector3 rayDirNormalized = _rayDir;
  ...
  if (x0 >= 0 && x0 < m_heightMap->GetNumColumns() && z0 >= 0 && z0 <= m_heightMap->GetNumRows())
    return UnsafeRayHit(_rayStart, rayDirNormalized, _result);

  // *** Find out which quadrant the 2D projection of the ray goes in ***

  LegacyVector2 segIntersectResult;
  if (rayDirNormalized.x > 0.0f)
  {
    if (rayDirNormalized.z > 0.0f)
    {
      ...
      LegacyVector2 segStart(0.0f, 0.0f);
      LegacyVector2 segEnd(GetWorldSizeX(), 0.0f);
      bool intersects = SegRayIntersection2D(segStart, segEnd, _rayStart, rayDirNormalized, &segIntersectResult);
      if (!intersects)
      {
        LegacyVector2 segStart(0.0f, 0.0f);
        LegacyVector2 segEnd(0.0f, GetWorldSizeZ());
        intersects = SegRayIntersection2D(segStart, segEnd, _rayStart, rayDirNormalized, &segIntersectResult);
        if (!intersects)
          return false;
      }
    }
    else
    {
      ...
      LegacyVector2 segStart(0.0f, GetWorldSizeZ());
      LegacyVector2 segEnd(GetWorldSizeX(), GetWorldSizeZ());
      bool intersects = SegRayIntersection2D(segStart, segEnd, _rayStart, rayDirNormalized, &segIntersectResult);
      if (!intersects)
      {
        LegacyVector2 segStart(0.0f, 0.0f);
  ...
  else
  {
    if (rayDirNormalized.z > 0.0f)
    {
      ...
      if (!intersects)
      {
        LegacyVector2 segStart(GetWorldSizeX(), 0.0f);
        LegacyVector2 segEnd(GetWorldSizeX(), GetWorldSizeZ());
        intersects = SegRayIntersection2D(segStart, segEnd, _rayStart, rayDirNormalized, &segIntersectResult);
        if (!intersects)
          return false;
  ...
  float lambda = (segIntersectResult.x - _rayStart.x) / rayDirNormalized.x;
  float segIntersectResultY = _rayStart.y + lambda * rayDirNormalized.y;

  return UnsafeRayHit(LegacyVector3(segIntersectResult.x, segIntersectResultY, segIntersectResult.y), rayDirNormalized, _result);
}
...
// ASSUMES: that _result is a pointer to a valid LegacyVector3
bool Landscape::UnsafeRayHit(const LegacyVector3& _rayStart, const LegacyVector3& _rayDir, LegacyVector3* _result) const
{
  ...
  if (_rayDir.x > 0.0f && _rayDir.z > 0.0f)
  {
    ...
    while (gridX < m_heightMap->GetNumColumns() && gridZ < m_heightMap->GetNumRows())
    {
      ...
      LegacyVector3 segStart(m_heightMap->GetRealX(gridX), 0.0f, m_heightMap->GetRealY(gridZ + 1));
      LegacyVector3 segEnd(m_heightMap->GetRealX(gridX + 1), 0.0f, m_heightMap->GetRealY(gridZ + 1));
  ...
  else if (_rayDir.x < 0.0f && _rayDir.z < 0.0f)
  {
    ...
    while (gridX >= 0 && gridZ >= 0)
    {
      ...
      LegacyVector3 segStart(m_heightMap->GetRealX(gridX), 0.0f, m_heightMap->GetRealY(gridZ));
      LegacyVector3 segEnd(m_heightMap->GetRealX(gridX + 1), 0.0f, m_heightMap->GetRealY(gridZ));
      bool intersects = SegRayIntersection2D(segStart, segEnd, _rayStart, _rayDir);
      if (intersects)
        gridZ--;
      else
...
bool Landscape::RayHitCell(int x0, int z0, const LegacyVector3& _rayStart, const LegacyVector3& _rayDir,
                           LegacyVector3* _result) const
{
  LegacyVector3 corner1(m_heightMap->GetRealX(x0), m_heightMap->GetData(x0, z0), m_heightMap->GetRealY(z0));
  LegacyVector3 corner2(m_heightMap->GetRealX(x0), m_heightMap->GetData(x0, z0 + 1), m_heightMap->GetRealY(z0 + 1));
  LegacyVector3 corner3(m_heightMap->GetRealX(x0 + 1), m_heightMap->GetData(x0 + 1, z0), m_heightMap->GetRealY(z0));

  if (RayTriIntersection(_rayStart, _rayDir, corner1, corner2, corner3, 1e10, _result))
    return true;

  LegacyVector3 corner4(m_heightMap->GetRealX(x0 + 1), m_heightMap->GetData(x0 + 1, z0 + 1), m_heightMap->GetRealY(z0 + 1));
