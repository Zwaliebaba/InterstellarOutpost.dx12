...
{
  if (_immediate) { g_app->m_camera->SetFOV(_fov); }
  else { g_app->m_camera->SetTargetFOV(_fov); }
}
...
void Script::RunCommand_LocationFocus(const char *_locationName, float _fov)
{
  ...
  if (!g_app->m_camera->IsInMode(Camera::ModeSphereWorldScripted)) { g_app->m_camera->RequestMode(Camera::ModeSphereWorldScripted); }
...
{
  if (g_app->m_camera->IsAnimPlaying()) { g_app->m_camera->StopAnimation(); }

  if (g_app->m_location) { g_app->m_camera->RequestMode(Camera::ModeFreeMovement); }
  else { g_app->m_camera->RequestMode(Camera::ModeSphereWorld); }
}
...
void Script::RunCommand_Say(char *_stringId) { g_app->m_sepulveda->Say(_stringId); }


void Script::RunCommand_ShutUp() { g_app->m_sepulveda->ShutUp(); }
...
void Script::RunCommand_Highlight(int _buildingId) { g_app->m_sepulveda->HighlightBuilding(_buildingId, "ScriptHighlight"); }


void Script::RunCommand_ClearHighlights() { g_app->m_sepulveda->ClearHighlights("ScriptHighlight"); }
...
void Script::RunCommand_TriggerSound(const char *_event)
{
  ...
  if (g_app->m_soundSystem->NumInstancesPlaying(WorldObjectId(), eventName) == 0) { g_app->m_soundSystem->TriggerOtherEvent(nullptr, (char *) _event, SoundSourceBlueprint::TypeMusic); }
}
...
void Script::RunCommand_DemoGesture(const char *_name) { g_app->m_sepulveda->DemoGesture(_name, 2.0f); }
...
void Script::RunCommand_RestoreResearch() { g_app->m_globalWorld->m_research->m_researchLevel[GlobalResearch::TypeDarwinian] = m_darwinianResearchLevel; }
...
void Script::RunCommand_ShowDemoEndSequence() { g_app->m_demoEndSequence = new DemoEndSequence(); }
...
void Script::RunCommand_ActivateTrunkPort(int _buildingId, bool _fullActivation)
{
  ...
  {
    if (_fullActivation) { b->ReprogramComplete(); }
    else
    {
...
void Script::Advance()
{
  if (g_inputManager->controlEvent(ControlSkipCutscene)) { if (Skip()) return; }
  ...
  if (m_waitForRocket)
  {
    ...
    if (rocket->m_state == m_rocketState &&
        ...
        static_cast<int>(rocket->m_countdown) > m_rocketData) { return; }
  }

  if (m_requestedLocationId != -1)
  {
    if (g_app->m_locationId != m_requestedLocationId) { return; }
    m_requestedLocationId = -1;
  }
  ...
  if (m_in)
  {
    if (m_in->ReadLine()) { AdvanceScript(); }
    else
    {
      delete m_in;
      m_in = nullptr;
...
void Script::TestScript(char *_filename)
{
  ...
  while (m_in->ReadLine())
  {
    ...
    switch (opCode)
    {
      ...
      case OpHighlight:
        if (!g_app->m_location->GetBuilding((int) nextFloat)) { ReportError(levelFile, "HighlightBuilding called with invalid buildingId: %d", (int) nextFloat); }
        break;
