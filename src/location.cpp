...
void Location::Init(std::string_view _missionFilename, std::string_view _mapFilename)
{
...
void Location::LoadLevel(std::string_view missionFilename, std::string_view mapFilename)
{
...
void Location::InitLights()
{
  for (int i = 0; i < m_levelFile->m_lights.Size(); i++)
  {
    ...
    light->SetColor(levelLight->m_colour);
  ...
  for (int i = 0; i < m_levelFile->m_buildings.Size(); i++)
  {
    ...
    if (existing)
    {
      ASSERT_TEXT(
        ...
        "Existing building type = {}, new building type = {}", m_levelFile->m_mapFilename, m_levelFile->m_missionFilename,
        Building::GetTypeName(existing->m_buildingType), Building::GetTypeName(building->m_buildingType));
    }
    Building* newBuilding = Building::CreateBuilding(building->m_buildingType);
    m_buildings.PutData(newBuilding);
    newBuilding->Initialize(building);
  }
}
...
WorldObjectId Location::SpawnEntities(const LegacyVector3& _pos, unsigned char _teamId, int _unitId, EntityType _type,
                                      int _numEntities, const LegacyVector3& _vel, float _spread, float _range, int _routeId,
                                      int _routeWaypointId)
{
...
LegacyVector3 Location::FindValidSpawnPosition(const LegacyVector3& _pos, float _spread)
{
  int tries = 10;

  while (tries > 0)
  {
    LegacyVector3 randomPos = _pos;
    ...
    bool onMap = randomPos.x >= 0 && randomPos.z >= 0 && randomPos.x < m_landscape.GetWorldSizeX() && randomPos.z < m_landscape.
      GetWorldSizeZ();
...
WorldObject* Location::GetEffect(WorldObjectId _id)
{
  ...
  {
    auto* wobj = m_effects[_id.GetIndex()];
...
Entity* Location::GetEntity(const LegacyVector3& _rayStart, const LegacyVector3& _rayDir)
{
...
Entity* Location::GetEntitySafe(WorldObjectId _id, EntityType _type)
{
  WorldObject* wobj = GetEntity(_id);
  auto ent = dynamic_cast<Entity*>(wobj);

  if (ent && ent->m_entityType == _type)
    return ent;
...
bool Location::IsVisible(const LegacyVector3& _from, const LegacyVector3& _to)
{
  LegacyVector3 rayDir = (_to - _from).Normalize();
...
bool Location::IsWalkable(const LegacyVector3& _from, const LegacyVector3& _to, bool _evaluateCliffs)
{
  ...
  LegacyVector3 diff = (_to - _from) / static_cast<float>(numSteps);
  float distanceUnderWater = 0.0f;

  LegacyVector3 position = _from;
  LegacyVector3 oldPosition = _from;
...
void Location::AdvanceBuildings(int _slice)
{
  ...
  for (int i = startIndex; i <= endIndex; ++i)
  {
    if (m_buildings.ValidIndex(i))
    {
      ...
      START_PROFILE(g_app->m_profiler, Building::GetTypeName( building->m_buildingType ));
      bool removeBuilding = building->Advance();
      END_PROFILE(g_app->m_profiler, Building::GetTypeName( building->m_buildingType ));
...
{
  auto b1 = (DepthSortedBuilding*)elem1;
  auto b2 = (DepthSortedBuilding*)elem2;
...
void Location::InitialiseTeam(unsigned char _teamId, unsigned char _teamType)
{
  ...
  for (int i = 0; i < m_levelFile->m_instantUnits.Size(); i++)
  {
    ...
    if (iu->m_inAUnit)
    {
      Unit* newUnit = team->NewUnit(iu->m_entityType, iu->m_number, &unitId, pos);
      newUnit->SetWayPoint(pos);
      newUnit->m_routeId = iu->m_routeId;
      iu->m_routeId = -1;
    }

    WorldObjectId spawnedId = SpawnEntities(pos, iu->m_teamId, unitId, iu->m_entityType, iu->m_number, g_zeroVector, iu->m_spread, -1.0f,
    ...
    if (iu->m_entityType == EntityType::TypeDarwinian && iu->m_number == 1 && iu->m_state == Darwinian::StateFollowingOrders)
    {
      auto darwinian = static_cast<Darwinian*>(g_app->m_location->GetEntitySafe(spawnedId, EntityType::TypeDarwinian));
      if (darwinian)
        darwinian->GiveOrders(targetPos);
    }
    if (iu->m_entityType == EntityType::TypeOfficer)
    {
      auto officer = static_cast<Officer*>(g_app->m_location->GetEntitySafe(spawnedId, EntityType::TypeOfficer));
      if (iu->m_state == Officer::OrderGoto)
        officer->SetOrders(targetPos);
      else
        officer->m_orders = iu->m_state;
    }
    if (iu->m_entityType == EntityType::TypeArmour)
    {
      auto armour = static_cast<Armour*>(g_app->m_location->GetEntitySafe(spawnedId, EntityType::TypeArmour));
  ...
  if (_teamType == Team::TeamTypeLocalPlayer)
  {
    for (int i = 0; i < m_levelFile->m_runningPrograms.Size(); ++i)
    {
      ...
      if (program->m_entityType == EntityType::TypeEngineer)
      {
        LegacyVector3 pos(program->m_positionX[0], 0, program->m_positionZ[0]);
        pos.y = m_landscape.m_heightMap->GetValue(pos.x, pos.z);
        WorldObjectId objId = SpawnEntities(pos, _teamId, -1, EntityType::TypeEngineer, 1, g_zeroVector, 0.0f);
        auto engineer = static_cast<Engineer*>(GetEntitySafe(objId, EntityType::TypeEngineer));
      ...
      if (program->m_entityType == EntityType::TypeSquadie)
      {
        LegacyVector3 pos(program->m_positionX[0], 0, program->m_positionZ[0]);
        pos.y = m_landscape.m_heightMap->GetValue(pos.x, pos.z);

        int unitId;
        auto squad = static_cast<InsertionSquad*>(GetMyTeam()->NewUnit(EntityType::TypeSquadie, program->m_count, &unitId,
                                                                       pos));

        LegacyVector3 waypoint(program->m_waypointX, 0, program->m_waypointZ);
        ...
        SpawnEntities(pos, _teamId, unitId, EntityType::TypeSquadie, program->m_count, g_zeroVector, 10.0f);
...
std::optional<std::shared_ptr<WorldObject>> Location::GetWorldObject(const LegacyVector3& _startRay, const LegacyVector3& _rayDir,
  unsigned char _teamId, float _maxDistance, float* _range) const
{
  float closestRangeSqd = FLT_MAX;
	std::optional<std::shared_ptr<WorldObject>> objectFound = std::nullopt;

  for (auto object : m_globalWorldObjects)
  {
    LegacyVector3 hitPos;

    bool rayHit = RaySphereIntersection(_startRay, _rayDir, object->m_centerPos, object->m_radius, _maxDistance, &hitPos);

    if (rayHit)
    {
      float centerPosX, centerPosY, rayHitX, rayHitY;
      g_app->m_camera->Get2DScreenPos(object->m_centerPos, &centerPosX, &centerPosY);
...
void Location::UpdateTeam(unsigned char teamId, const TeamControls& teamControls)
{
  ...
  if (unit)
  {
    ...
    if (unitMove)
    {
      ...
      unit->m_targetDir = (teamControls.m_mousePos - unit->m_centerPos).Normalize();
      unit->RecalculateOffsets();
  ...
  else
  {
    ...
    if (entity)
    {
      ...
      switch (entity->m_entityType)
      {
        ...
        {
          auto eng = dynamic_cast<Engineer*>(entity);
          if (unitMove)
        ...
        case EntityType::TypeOfficer:
        {
          ...
          if (primaryFire)
            officer->SetOrders(teamControls.m_mousePos);
        ...
        case EntityType::TypeArmour:
        {
          ...
          if (unitMove)
            armour->SetWayPoint(teamControls.m_mousePos);
          if (primaryFire)
...
int Location::GetUnitId(const LegacyVector3& startRay, const LegacyVector3& direction, unsigned char team, float* _range)
{
  if (team == 255)
    return -1;
...
WorldObjectId Location::GetEntityId(const LegacyVector3& startRay, const LegacyVector3& direction, unsigned char teamId,
                                    float* _range)
{
  if (teamId == 255)
...
int Location::GetBuildingId(const LegacyVector3& rayStart, const LegacyVector3& rayDir, unsigned char teamId, float _maxDistance,
                            float* _range)
{
  float closestRangeSqd = FLT_MAX;
  int buildingId = -1;
  ...
  for (int i = 0; i < m_buildings.Size(); i++)
  {
    if (m_buildings.ValidIndex(i))
    {
      ...
      if (building->m_buildingType != BuildingType::TypeControlTower && teamMatch)
      {
        LegacyVector3 hitPos;
        bool rayHit = false;
...
void Location::ThrowWeapon(const LegacyVector3& _pos, const LegacyVector3& _target, EffectType _type, unsigned char _fromTeamId)
{
  ...
  LegacyVector3 front = (_target - _pos).Normalize();
  front.y = 1.0f;
  front.Normalize();
  ...
  switch (_type)
  {
    case EffectType::EffectThrowableGrenade:
      weapon = NEW Grenade(_pos, front, force);
      break;
    case EffectType::EffectThrowableAirstrikeMarker:
      weapon = NEW AirStrikeMarker(_pos, front, force);
      break;
    case EffectType::EffectThrowableControllerGrenade:
  ...
  LegacyVector3 flashFront = front;
  auto mf = NEW MuzzleFlash(_pos, flashFront, 20.0f, 3.0f);
  int index = m_effects.PutData(mf);
  mf->m_id.Set(_fromTeamId, UNIT_EFFECTS, index, -1);
  mf->m_id.GenerateUniqueId();
}

void Location::FireRocket(const LegacyVector3& _pos, const LegacyVector3& _target, unsigned char _teamId)
{
  auto r = NEW Rocket(_pos, _target);
  ...
  LegacyVector3 flashFront = _target - _pos;
  flashFront.Normalize();
  auto mf = NEW MuzzleFlash(_pos, flashFront, 20.0f, 3.0f);
  int index = m_effects.PutData(mf);
...
void Location::FireTurretShell(const LegacyVector3& _pos, const LegacyVector3& _vel)
{
  ...
  auto shell = NEW TurretShell(lifeTime);
  shell->m_pos = _pos;
  shell->m_vel = _vel;
...
void Location::FireLaser(const LegacyVector3& _pos, const LegacyVector3& _vel, unsigned char _teamId)
{
  ...
  LegacyVector3 flashFront = _vel;
  flashFront.Normalize();
  auto mf = NEW MuzzleFlash(_pos, flashFront, 20.0f * lifetime, 1.0f);
  int index = m_effects.PutData(mf);
  mf->m_id.Set(_teamId, UNIT_EFFECTS, index, -1);
  mf->m_id.GenerateUniqueId();
}
...
void Location::Bang(const LegacyVector3& _pos, float _range, float _damage)
{
  ...
  int numDebris = std::max(1.0f, _range * _damage * 0.005f);
  ...
  for (int i = 0; i < numFound; ++i)
  {
    ...
    LegacyVector3 push(entity->m_pos - _pos);
    push.Normalize();
...
void Location::CreateShockwave(const LegacyVector3& _pos, float _size, unsigned char _teamId)
{
  auto s = NEW Shockwave(_teamId, _size);
  s->m_pos = _pos;
...
{
  float fogCol[] = {
    g_app->m_backgroundColor.r / 255.0f,
    g_app->m_backgroundColor.g / 255.0f,
    g_app->m_backgroundColor.b / 255.0f,
    0
  };
...
int Location::ChristmasModEnabled()
{
  ...
  if (theTime->tm_mon == 11 && (theTime->tm_mday == 25 || theTime->tm_mday == 26))
  {
