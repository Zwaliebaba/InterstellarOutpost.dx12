...
void Renderer::RenderFadeOut()
{
  ...
  if (m_fadeDelay > 0.0f) { m_fadeDelay -= timeIncrement; }
  else
  {
...
void Renderer::RenderFrame(bool withFlip)
{
  ...
  if (g_app->m_location) { glClearColor(col->r / 255.0f, col->g / 255.0f, col->b / 255.0f, col->a / 255.0f); }
  else { glClearColor(0.05f, 0.0f, 0.05f, 0.1f); }
  ...
  if (g_app->m_editing)
  {
    ...
    else { g_app->m_globalWorld->Render(); }
  }
  else
  {
    if (g_app->m_locationId != -1)
...
#ifdef USE_DIRECT3D
  if (renderPixelShaderPref == 1) { if (g_deformEffect && deformStarted) g_deformEffect->Stop(); }
#endif
  ...
  if (m_renderDarwinLogo >= 0.0f)
  {
    ...
    if (timeNow > m_renderDarwinLogo + 10) { m_renderDarwinLogo = -1.0f; }
    else if (timeNow < m_renderDarwinLogo + 3) { alpha = (timeNow - m_renderDarwinLogo) / 3.0f; }
    else if (timeNow > m_renderDarwinLogo + 8) { alpha = 1.0f - (timeNow - m_renderDarwinLogo - 8) / 2.0f; }
    else { alpha = 1.0f; }
...
void Renderer::SetupProjMatrixFor3D() const
{
  ...
  else
  {
    gluPerspective(g_app->m_camera->GetFov(),
                   static_cast<float>(m_screenW) / static_cast<float>(m_screenH),// Aspect ratio
...
void Renderer::FPSMeterAdvance()
{
  ...
  {
    if (currentTime > endOfSecond + 2.0) { endOfSecond = currentTime + 1.0; }
    else { endOfSecond += 1.0; }
...
void Renderer::PreRenderPixelEffect()
{
  ...
  for (int y = 0; y < PIXEL_EFFECT_GRID_RES; ++y) { for (int x = 0; x < PIXEL_EFFECT_GRID_RES; ++x) { m_pixelEffectGrid[y][x] = 1e9; } }
              ...
              if (distance < cutoff)
              {
                for (int j = 0; j < unit->m_entities.Size(); ++j)
                {
                  if (unit->m_entities.ValidIndex(j))
                  {
                    ...
                    bool rendered = false;
                    if (j <= unit->m_entities.GetLastUpdated()) { rendered = entity->RenderPixelEffect(g_predictionTime); }
                    else { rendered = entity->RenderPixelEffect(g_predictionTime + SERVER_ADVANCE_PERIOD); }
                    if (rendered)
                    {
      ...
      for (int i = 0; i < g_app->m_location->m_teams[t].m_others.Size(); ++i)
      {
        if (g_app->m_location->m_teams[t].m_others.ValidIndex(i))
        {
          ...
          if (entity->IsInView())
          {
            ...
            {
              bool rendered = false;
              if (i <= g_app->m_location->m_teams[t].m_others.GetLastUpdated()) { rendered = entity->RenderPixelEffect(g_predictionTime); }
              else { rendered = entity->RenderPixelEffect(g_predictionTime + SERVER_ADVANCE_PERIOD); }
              if (rendered)
              {
                float distance = (entity->m_pos - g_app->m_camera->GetPos()).Mag();
  ...
  if (false)
  {
    ...
    for (int y = 0; y < yCellsUsed; ++y)
    {
      for (int x = 0; x < PIXEL_EFFECT_GRID_RES; ++x)
      {
        ...
        if (dist < 1e9)
        {
          g_editorFont.DrawText2DCentre(static_cast<float>(x) * scaleX + offsetX,
                                        static_cast<float>(y) * scaleY + offsetY,
                                        12, "%.0f", dist);
        }
      }
    }
...
void Renderer::RasteriseSphere(const Vector3 &_pos, float _radius)
{
  ...
  for (int y = y1; y < y2; ++y) { for (int x = x1; x < x2; ++x) { if (nearestZ < m_pixelEffectGrid[x][y]) { m_pixelEffectGrid[x][y] = nearestZ; } } }
}
...
void Renderer::MarkUsedCells(const ShapeFragment *_frag, const Matrix34 &_transform)
{
  ...
  {
    if (!g_app->m_camera->SphereInViewFrustum(worldPos, _frag->m_radius)) return;
  }

  if (_frag->m_radius > 0.0f) { RasteriseSphere(worldPos, _frag->m_radius); }
