...
#include "input/inputdriver_conjoin.h"
#include "input/inputdriver_chord.h"
...
void UpdateAdvanceTime()
{
  ...
  else
  {
    ...
    if (g_advanceTime > 0.25f) { g_advanceTime = 0.25f; }
    g_gameTime = realTime;
...
void UpdateTargetFrameRate(int _currentSlice)
{
  ...
  if (g_targetFrameRate < 2.0f) { g_targetFrameRate = 2.0f; }
  else if (g_targetFrameRate > 85.0f) { g_targetFrameRate = 85.0f; }
}
...
bool ProcessServerLetters(ServerToClientLetter *letter)
{
  switch (letter->m_type)
  {
    ...
    case ServerToClientLetter::TeamAssign:
      ...
      if (letter->m_ip == g_app->m_clientToServer->GetOurIP_Int()) { g_app->m_location->InitialiseTeam(letter->m_teamId, letter->m_teamType); }
      else { g_app->m_location->InitialiseTeam(letter->m_teamId, Team::TeamTypeRemotePlayer); }
      return true;
...
bool HandleCommonConditions()
{
  ...
  if (!controllerPlugged && g_inputManager->getInputMode() == INPUT_MODE_GAMEPAD) curWindowHasFocus = false;
  ...
  {
    // TODO: This
    if (g_app->m_requestedLocationId != -1) DebugKeyBindings::ReallyQuitButton();
    else g_app->m_requestQuit = true;
  }
#endif
...
void LocationGameLoop()
{
  ...
  while (true)
  {
    ...
    {
      if (g_app->m_script && g_app->m_script->IsRunningScript()) { g_app->m_sepulveda->PlayerSkipsMessage(); }
      else
      {
        if (WindowsOnScreen()) RemoveAllWindows();
        else if (g_app->m_taskManagerInterface->m_visible) { g_app->m_taskManagerInterface->m_visible = false; }
        else
        {
    ...
    if (HandleCommonConditions()) continue;
    ...
    if (iAmAServer)
    {
      if (timeNow > nextServerAdvanceTime)
      {
        ...
        if (timeNow > nextServerAdvanceTime) { nextServerAdvanceTime = timeNow + SERVER_ADVANCE_PERIOD; }
      }
    }

    if (!WindowsOnScreen()) teamControls.Advance();
    ...
    if (iAmAClient)
    {
      ...
      if (timeNow > nextIAmAliveMessage)
      {
        ...
        if (team)
        {
          ...
          if (g_inputManager->getInputMode() == INPUT_MODE_KEYBOARD &&
              teamControls.m_primaryFireTarget)
            orderGiven = true;
          if (g_inputManager->getInputMode() == INPUT_MODE_GAMEPAD &&
              teamControls.m_secondaryFireDirected)
            orderGiven = true;
          ...
          if (checkMouse)
          {
            ...
            if (idUnderMouse.GetUnitId() == UNIT_BUILDINGS)
            {
              ...
              if (building && building->m_type == Building::TypeRadarDish) { teamControls.m_mousePos = building->m_pos; }
            }
          }
        }
        ...
        if (g_app->m_taskManagerInterface->m_visible ||
            ...
            chatLog || entityUnderMouse) { teamControls.ClearFlags(); }

        g_app->m_clientToServer->SendIAmAlive(g_app->m_globalWorld->m_myTeamId, teamControls);

        nextIAmAliveMessage += IAMALIVE_PERIOD;
        if (timeNow > nextIAmAliveMessage) { nextIAmAliveMessage = timeNow + IAMALIVE_PERIOD; }
      ...
      for (int i = 0; i < slicesToAdvance; ++i)
      {
        ...
        if (g_sliceNum != -1)
        {
          ...
          if (g_sliceNum < NUM_SLICES_PER_FRAME - 1) { g_sliceNum++; }
          else
          {
            g_sliceNum = -1;
      ...
      if (g_app->m_tutorial) g_app->m_tutorial->Advance();
      ...
      if (g_app->m_renderer->m_fps < 15) { g_app->m_soundSystem->Advance(); }
    }
  }
  ...
  if (g_app->m_globalWorld->GetLocationName(g_app->m_locationId)) { g_app->m_globalWorld->TransferSpirits(g_app->m_locationId); }
...
void InitialiseInputManager()
{
  ...
  g_inputManager->addDriver(new AliasInputDriver());
  {
    ...
    {
      if (localeInputPrefsReader->IsOpen()) { g_inputManager->parseInputPrefs(*localeInputPrefsReader, true); }
    ...
    {
      if (userInputPrefsReader->IsOpen()) { g_inputManager->parseInputPrefs(*userInputPrefsReader, true); }
...
void EnterLocation()
{
  ...
  if (!g_app->m_editing)
  {
    if (g_prefsManager->GetInt("RecordDemo") == 2) { if (g_app->m_server) { g_app->m_server->LoadHistory("ServerHistory.dat"); } }
    else
    {
      if (iAmAServer)
      {
...
void EnterGlobalWorld()
{
  ...
  if (g_app->m_editing) { GlobalWorldEditorLoop(); }
  else { GlobalWorldGameLoop(); }
}
...
void MainMenuLoop()
{
  ...
  while (g_app->m_atMainMenu)
  {
    ...
    if (!g_app->m_gameMenu->m_menuCreated) { if (g_app->m_renderer->IsFadeComplete()) { g_app->m_gameMenu->CreateMenu(); } }
  }
...
void RunTheGame()
{
  ...
  if (g_prefsManager->GetInt("CurrentGameMode", 1) == 0) { g_app->LoadPrologue(); }
  else { g_app->LoadCampaign(); }
  ...
  while (true)
  {
    ...
    if (g_app->m_requestedLocationId != -1) { EnterLocation(); }
