...
void Team::Initialize(int _teamId)
{
  m_teamId = _teamId;
...
void Team::SelectUnit(int _unitId, int _entityId, int _buildingId)
{
  ...
  if (m_currentUnitId == -1 && m_currentBuildingId == -1 && m_others.ValidIndex(m_currentEntityId))
  {
    ...
    if (entity && entity->m_entityType == EntityType::TypeOfficer)
...
Unit* Team::NewUnit(EntityType _troopType, int _numEntities, int* _unitId, const LegacyVector3& _pos)
{
  *_unitId = m_units.GetNextFree();
  Unit* unit = nullptr;

  if (_troopType == EntityType::TypeSquadie)
    unit = NEW InsertionSquad(m_teamId, *_unitId, _numEntities, _pos);
  else if (_troopType == EntityType::TypeSpaceInvader)
    unit = NEW AirstrikeUnit(m_teamId, *_unitId, _numEntities, _pos);
  else if (_troopType == EntityType::TypeVirii)
...
Entity* Team::NewEntity(EntityType _troopType, int _unitId, int* _index)
{
  if (_unitId == -1)
  {
...
int Team::NumEntities(EntityType _troopType)
{
  int result = 0;
  int i;
  ...
  for (i = 0; i < m_others.Size(); ++i)
  {
    if (m_others.ValidIndex(i))
    {
      ...
      if (ent->m_entityType == _troopType)
        ++result;
    }
  }
    ...
    for (int i = startIndex; i <= endIndex; i++)
    {
      if (m_others.ValidIndex(i))
      {
        ...
        if (ent->m_enabled)
        {
          ...
          DEBUG_ASSERT(strcmp(g_app->m_profiler->m_currentElement->m_name.c_str(), "Advance Others") == 0);
#endif
...
void Team::RenderVirii(float _predictionTime)
{
  ...
  for (int i = 0; i <= m_others.Size(); i++)
  {
    if (m_others.ValidIndex(i))
    {
      ...
      {
        auto virii = dynamic_cast<Virii*>(entity);
        if (virii->IsInView())
        {
...
void Team::RenderOthers(float _predictionTime)
{
  ...
  for (int i = 0; i <= lastUpdated; i++)
  {
    ...
    {
      Entity* entity = m_others.GetData(i);
      if (entity->m_entityType != EntityType::TypeVirii && entity->m_entityType != EntityType::TypeDarwinian && entity->IsInView())
      {
        START_PROFILE(g_app->m_profiler, Entity::GetTypeName( entity->m_entityType ));
        entity->Render(_predictionTime);
        END_PROFILE(g_app->m_profiler, Entity::GetTypeName( entity->m_entityType ));
      }
    }
  }
...
unsigned short TeamControls::GetFlags() const
{
  return (m_unitMove ? 0x0001 : 0) | (m_directUnitMove ? 0x0002 : 0) | (m_primaryFireTarget ? 0x0004 : 0) | (
    m_secondaryFireTarget ? 0x0008 : 0) | (m_primaryFireDirected ? 0x0010 : 0) | (m_secondaryFireDirected ? 0x0020 : 0) | (
...
void TeamControls::Advance()
{
  ...
  m_primaryFireDirected |= g_inputManager->controlEvent(ControlUnitPrimaryFireDirected) && !g_inputManager->controlEvent(
    ControlCameraRotate);
