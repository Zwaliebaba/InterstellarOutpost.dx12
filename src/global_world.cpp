...
GlobalBuilding::GlobalBuilding()
  ...
  m_teamId(-1),
  m_locationId(-1),
  m_buildingType(BuildingType::TypeTrunkPort),
  m_online(false),
  m_link(-1),
  m_shape(nullptr) {
  m_shape = Resource::GetShape("trunkport.shp");
}
...
void GlobalResearch::EvaluateLevel(int _type)
{
  ...
  if (currentLevel < 4)
  {
    ...
    if (m_researchProgress[_type] >= requiredProgress)
    {
      ...
      sprintf(sepStringId, "research_%s_v%d", GetTypeName(_type).c_str(), m_researchLevel[_type]);
      _strupr(sepStringId);
...
std::string GlobalResearch::GetTypeName(int _type)
{
  const char* names[] = {
    "Darwinian",
    "Officer",
...
std::string GlobalResearch::GetTypeNameTranslated(int _type)
{
  return Strings::Get("researchname_" + GetTypeName(_type));
}
...
{
  //#ifndef DEMOBUILD
  m_shapeOuter = Resource::GetShape("globalworld_outer.shp");
  m_shapeMiddle = Resource::GetShape("globalworld_middle.shp");
  m_shapeInner = Resource::GetShape("globalworld_inner.shp");
  //#endif
}

void SphereWorld::AddLocation(int _locationId)
{
  // Initialize the sphere world to
  ...
  // Initialize the spirits for the new worlds.
...
void SphereWorld::RenderTrunkLinks()
{
  ...
  for (int i = 0; i < g_app->m_globalWorld->m_buildings.Size(); ++i)
  {
    ...
    if (building->m_buildingType == BuildingType::TypeTrunkPort && building->m_link != -1)
    {
...
void SphereWorld::RenderIslands()
{
  ...
  for (int i = 0; i < g_app->m_globalWorld->m_locations.Size(); ++i)
  {
    ...
    if (loc->m_available)
    {
      ...
      int numRedraws = 5;
      if (!loc->m_missionCompleted && loc->m_missionFilename != "null" && fmodf(g_gameTime, 1.0f) < 0.7f)
        numRedraws = 10;
  ...
  for (int i = 0; i < g_app->m_globalWorld->m_locations.Size(); ++i)
  {
    ...
    {
      LegacyVector3 islandPos = g_app->m_globalWorld->GetLocationPosition(loc->m_id);
      std::string islandName = g_app->m_globalWorld->GetLocationNameTranslated(loc->m_id);
      std::ranges::transform(islandName, islandName.begin(), ::toupper);
...
void GlobalWorld::Advance()
{
  ...
  if (g_inputManager->controlEvent(ControlSelectLocation) && m_locationRequested == -1 && Canvas::EclGetWindows()->Size() == 0)
  {
    LegacyVector3 rayStart, rayDir;
    g_app->m_camera->GetClickRay(g_target->X(), g_target->Y(), &rayStart, &rayDir);
  ...
  else if (m_locationRequested == -1 && Canvas::EclGetWindows()->Size() == 0)
  {
    LegacyVector3 rayStart, rayDir;
    g_app->m_camera->GetClickRay(g_target->X(), g_target->Y(), &rayStart, &rayDir);
    ...
    if (locId != -1)
    {
      ...
      locY = ClientEngine::OutputSize().Height - locY;
...
GlobalLocation* GlobalWorld::GetLocation(std::string_view _name)
{
...
int GlobalWorld::GetLocationId(std::string_view _name)
{
  for (int i = 0; i < m_locations.Size(); ++i)
  {
    GlobalLocation* loc = m_locations[i];
...
int GlobalWorld::GetLocationIdFromMapFilename(std::string_view _mapFilename)
{
  auto mapName = _mapFilename.find_first_of("map_");
  if (mapName == std::string_view::npos)
    return -1;

  mapName += 4;
  const auto extension = _mapFilename.find_first_of('.');
  return GetLocationId(_mapFilename.substr(mapName, extension - mapName));  // also remove the .txt
}

std::string GlobalWorld::GetLocationName(int _id)
{
  if (GlobalLocation* loc = GetLocation(_id))
    return loc->m_name;
  return {};
}

std::string GlobalWorld::GetLocationNameTranslated(int _id)
{
  const GlobalLocation* loc = GetLocation(_id);
  if (!loc)
    return {};

  return Strings::Get("location_" + loc->m_name);
}
...
void GlobalWorld::AddLocation(GlobalLocation* _location)
{
  if (_location->m_id == -1)
  {
    _location->m_id = m_nextLocationId;
    m_nextLocationId++;
  }
  else if (_location->m_id >= m_nextLocationId)
    m_nextLocationId = _location->m_id + 1;

  m_locations.PutData(_location);
  m_sphereWorld->AddLocation(_location->m_id);
}

void GlobalWorld::AddBuilding(GlobalBuilding* _building)
{
  if (_building->m_id == -1)
  {
...
void GlobalWorld::ParseLocations(TextReader* _in)
{
  while (_in->ReadLine())
  {
    ...
    location->m_name = location->m_name.substr(0, location->m_name.size() - 4);
...
void GlobalWorld::ParseBuildings(TextReader* _in)
{
  while (_in->ReadLine())
  {
    ...
    building->m_buildingType = (BuildingType)atoi(_in->GetNextToken());
...
void GlobalWorld::AddLevelBuildingToGlobalBuildings(Building* _building, int _locId)
{
  if (_building->m_isGlobal)
  {
    ...
    if (!gb)
    {
      ...
      gb->m_buildingType = _building->m_buildingType;
      gb->m_locationId = _locId;
      gb->m_id = _building->m_id.GetUniqueId();
      gb->m_teamId = _building->m_id.GetTeamId();
      m_buildings.PutData(gb);

      if (_building->m_buildingType == BuildingType::TypeTrunkPort)
        gb->m_link = dynamic_cast<TrunkPort*>(_building)->m_targetLocationId;
    }
    gb->m_pos = _building->m_pos;
  }
}

void GlobalWorld::LoadGame(std::string_view _filename)
{
  TextReader* in = in = Resource::GetTextReader(_filename);
  ...
  for (int i = 0; i < m_locations.Size(); i++)
  {
    ...
    char filter[256];
    sprintf(filter, "mission_%s*.txt", GetLocationName(loc->m_id).c_str());
    ...
    for (auto missionName : Resource::ListResources("levels\\", filter, false))
    {
      ...
      for (int b = 0; b < levFile.m_buildings.Size(); ++b)
      {
        ...
        if (building->m_buildingType == BuildingType::TypeAntHill || building->m_buildingType == BuildingType::TypeTriffid || building->m_buildingType ==
          BuildingType::TypeIncubator)
        {
          if (!building->m_dynamic)
          {
            DebugTrace("{} found on level {} should be dynamic (otherwise save games wont work)\n",
              Building::GetTypeName(building->m_buildingType), GetLocationName(loc->m_id));
          }
        }
      }
...
void GlobalWorld::LoadLocations(std::string_view _filename)
{
  ...
  while (in->ReadLine())
  {
    ...
    if (GlobalLocation* location = GetLocation(locIndex))
