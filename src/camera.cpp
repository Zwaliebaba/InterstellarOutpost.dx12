...
void Camera::AdvanceDebugMode()
{
  ...
  if (g_app->m_editing) { m_targetFov = 60.0f; }

  float advanceTime = g_advanceTime;
  Vector3 right = m_front ^ m_up;

  float speedSideways = g_app->m_globalWorld->GetSize() / 30.0f;
  if (g_app->m_locationId != -1) { speedSideways = g_app->m_location->m_landscape.GetWorldSizeX() / 30.0f; }
...
void Camera::AdvanceSphereWorldIntroMode()
{
  ...
  if (runningTime < 30.0f) { targetPos.Set(-1000000, 4000000, 397000); }
  else { targetPos.Set(1000, 500, 1000); }
  Vector3 targetFront = targetPos - m_pos;
  float distance = targetFront.Mag();

  float forwardSpeed = 3000.0f;

  if (distance < 2000000 && runningTime > 30) { forwardSpeed = (distance / 1070000.0f) * 3000.0f; }
...
void Camera::AdvanceSphereWorldOutroMode()
{
  ...
  if (runningTime > 30.0f) { targetPos.Set(-965852, 1720000, 2600000); }
  ...
  targetFront = -m_pos;
  if (runningTime > 30.0f) { targetFront = m_vel; }
...
float Camera::DistanceToBlockage(const Vector3 &_dir, const float _maxDist)
{
  ...
  for (int i = 1; i <= numSteps; ++i)
  {
    ...
    if (landHeight + MIN_GROUND_CLEARANCE > m_height) { return static_cast<float>(i) * distStep; }
  }
...
{
  if (g_app->m_renderer->m_renderingPoster) { return; }
  ...
  if (!g_app->m_taskManagerInterface->m_visible)
  {
    ...
    if (keyForward || keyBackward || keyLeft || keyRight) { g_app->m_helpSystem->PlayerDoneAction(HelpSystem::CameraMovement); }
...
void Camera::AdvanceBuildingFocusMode()
{
  ...
  if (m_pos.y < landHeight + 10.0f) { m_pos.y = landHeight + 10.0f; }
}
...
void Camera::AdvanceAutomaticTracking()
{
  ...
  if (numInputs > 0) avgCameraTarget = avgCameraTarget / static_cast<float>(numInputs);
  else avgCameraTarget = m_pos;
...
void Camera::UpdateControlVector()
{
  ...
  else if (fabs(angle) >= angleThreshold) { m_skipDirectionCalculation = true; }
}
...
bool Camera::AdvanceManualRotateCamera(Vector3 &cameraTarget)
{
      ...
      !g_inputManager->controlEvent(ControlUnitPrimaryFireDirected))
  {
    ...
    if (g_inputManager->controlEvent(ControlCameraRotateLeft)) { deltaX = rotSpeed; }

    if (g_inputManager->controlEvent(ControlCameraRotateRight)) { deltaX = -rotSpeed; }
    ...
    if (newUp.y > 0.1f) { front.RotateAround(right * rotRight); }
...
{
  if (g_app->m_taskManagerInterface->m_visible) return;

  UpdateEntityTrackingMode();

  if (!g_app->m_location || !m_entityTrack) goto finishMode;
  ...
  if (!entity || entity->m_dead) goto finishMode;

  Task *currentTask = g_app->m_taskManager->GetCurrentTask();
  if (currentTask && currentTask->m_state != Task::StateRunning) goto finishMode;

  if (!currentTask && entity->m_type != Entity::TypeOfficer) goto finishMode;

  if (!m_objectId.IsValid()) goto finishMode;
...
float Camera::DirectDistanceToBlockage(const Vector3 &_from, const Vector3 &_to, const float _maxDist)
{
  ...
  for (int i = 1; i <= numSteps; ++i)
  {
    ...
    if (landHeight > y) { return static_cast<float>(i) * distStep; }
  }
...
void Camera::AdvanceEntityFollowMode()
{
  ...
  float radius = 15.0f;
  if (m_distFromEntity < radius) { m_distFromEntity = radius; }
  else if (m_distFromEntity > 5000.0f) { m_distFromEntity = 5000.0f; }
  ...
  Vector3 newUp(m_up);
  newUp.RotateAround(right * rotRight);
  if (newUp.y > 0.1f) { m_front.RotateAround(right * rotRight); }
...
bool Camera::PosInViewFrustum(const Vector3 &_pos)
{
  ...
  if (angle < fovInRadians - tolerance) { return false; }
  return true;
}
...
void Camera::AdvanceComponentZoom()
{
  ...
  if (g_app->m_taskManagerInterface->m_visible ||
      IsInMode(ModeEntityTrack)) { return; }
  ...
  if (m_mode == ModeSphereWorldScripted) { change = 10.0f; }

  if (m_mode == ModeMoveToTarget ||
      m_mode == ModeDoNothing ||
      m_mode == ModeBuildingFocus) { change = 1.0f; }
...
void Camera::AdvanceComponentMouseWheelHeight()
{
  ...
  if (g_app->m_location)
  {
    ...
    if (m_mode == ModeTurretAim) { m_height = max(m_height, MIN_GROUND_CLEARANCE); }
    else { if (landheight + MIN_GROUND_CLEARANCE > m_height) { m_height = landheight + MIN_GROUND_CLEARANCE; } }
...
{
  m_debugMode++;
  if (m_debugMode >= DebugModeNumStates) { m_debugMode = 0; }
}
...
void Camera::RequestEntityTrackMode(const WorldObjectId &_id)
{
  ...
  if (entity) { m_targetPos = entity->m_pos; }
}
...
void Camera::UpdateEntityTrackingMode()
{
  if (g_prefsManager && g_inputManager)
  {
    ...
    if (camTracking != 1 && camTracking != 2)
    {
      ...
      if (g_inputManager->getInputMode() == INPUT_MODE_GAMEPAD) camTracking = 2;
    }
