...
{
  if (g_inputManager->controlEvent(ControlSelectLocation) && !g_app->m_taskManagerInterface->m_visible)
  {
    WorldObjectId id;
...
void LocationInput::AdvanceTeamControl()
{
  ...
  if (g_inputManager->controlEvent(ControlUnitSetTarget) && // TODO: Really?
    ...
    !g_app->m_taskManagerInterface->m_visible && !taskStarted)
  {
    WorldObjectId id;
  ...
  if (team->m_currentUnitId == -1)
  {
    if (team->m_currentEntityId == -1)
    {
      ...
      else
      {
        ...
        else if (building->m_buildingType == BuildingType::TypeRadarDish)
    ...
    else
    {
      ...
      else if (ent->m_entityType == EntityType::TypeOfficer)
      {
        if (g_app->m_taskManagerInterface->ControlEvent(TaskManagerInterface::TMTerminate))
        {
      ...
      else if (ent->m_entityType == EntityType::TypeArmour)
      {
...
{
  if (g_app->m_globalWorld->m_myTeamId != 255 && Canvas::EclGetWindows()->Size() == 0)
    AdvanceTeamControl();
}
...
void LocationInput::Render()
{
  ...
  if (g_app->m_location->GetMyTeam())
  {
    ...
    g_editorFont.DrawText2D(12, 19, DEF_FONT_SIZE, "You are TEAM %d", g_app->m_globalWorld->m_myTeamId);
  }
