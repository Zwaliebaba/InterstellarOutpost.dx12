    ...
    if (id.IsValid())
    {
      if (id.GetUnitId() == UNIT_BUILDINGS)
      {
        ...
        if (!currentTask)
        {
          ...
          {
            auto fs = static_cast<FenceSwitch *>(building);
            fs->Switch();
          }
        }
      }
      else { g_app->m_clientToServer->RequestSelectUnit(id.GetTeamId(), id.GetUnitId(), id.GetIndex(), -1); }
    }
  }
}
...
void LocationInput::AdvanceTeamControl()
{
  ...
  if (g_inputManager->controlEvent(ControlUnitDeselect))
  {
    if (objectSelected)
    {
      ...
      if (team->m_currentUnitId != -1) { g_app->m_helpSystem->PlayerDoneAction(HelpSystem::SquadDeselect); }
      else if (team->m_currentEntityId != -1) { g_app->m_helpSystem->PlayerDoneAction(HelpSystem::EngineerDeselect); }
    }
  }
  ...
  if (team->m_currentUnitId == -1)
  {
    ...
    {
      if (team->m_currentBuildingId == -1) { AdvanceNoSelection(); }
      else
      {
        Building *building = g_app->m_location->GetBuilding(team->m_currentBuildingId);
        if (!building) { team->m_currentBuildingId = -1; }
        else if (building->m_type == Building::TypeRadarDish) { AdvanceRadarDishControl(building); }
        else if (building->m_type == Building::TypeGunTurret)
        {
    ...
    else
    {
      ...
      if (!ent) { team->m_currentEntityId = -1; }
      else if (ent->m_type == Entity::TypeOfficer)
      {
        ...
        if (g_inputManager->controlEvent(ControlWeaponCycleLeft)) { officer->SetPreviousMode(); }
        else if (g_inputManager->controlEvent(ControlWeaponCycleRight)) { officer->SetNextMode(); }
      }
      else if (ent->m_type == Entity::TypeArmour)
      {
        ...
        {
          auto armour = static_cast<Armour *>(ent);
      ...
      if (unit->m_troopType == Entity::TypeInsertionSquadie)
      {
        if (!g_app->m_taskManagerInterface->m_visible)
        {
          ...
          if (weaponList.Size() > 1)
          {
            ...
            {
              currentWeapon--;
              if (currentWeapon < 0) { currentWeapon = weaponList.Size() - 1; }
            }

            if (g_inputManager->controlEvent(ControlWeaponCycleRight))
            {
              currentWeapon++;
              if (currentWeapon >= weaponList.Size()) { currentWeapon = 0; }
            }
...
void LocationInput::Render()
{
  ...
  if (g_app->m_location->GetMyTeam())
  {
    ...
    g_editorFont.DrawText2D(12, 19, DEF_FONT_SIZE,
                            "You are TEAM %d", static_cast<int>(g_app->m_globalWorld->m_myTeamId));
  }
