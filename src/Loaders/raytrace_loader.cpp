...
#define REFRACTION_COEFFICIENT 1.4f
    ...
    float theta = iv_acos( -_incomingDir * _hitNormal );
    float thetaPrime = iv_asin( iv_sin(theta) / REFRACTION_COEFFICIENT );

    //Vector3 internalAngle = (iv_cos(thetaPrime) * X) + (iv_sin(thetaPrime) * Y);
    //_outgoingPoint = _hitPoint + 2.0f * _sphereRadius * iv_cos(thetaPrime) * internalAngle;

//    Vector3 internalAngle( -iv_cos( thetaPrime), -iv_sin( thetaPrime ), 0.0f );
//    Vector3 centrePoint( -_sphereRadius, 0, 0 );
//    Vector3 exitPoint = 2.0f * _sphereRadius * iv_cos(thetaPrime) * internalAngle;
//    Vector3 exitNormal = (exitPoint - centrePoint).Normalise();
//    Vector3 exitVector = exitNormal;
//    exitVector.RotateAroundZ( -theta );

    Vector3 incomingRay( -iv_cos(theta), -iv_sin(theta), 0.0f );
    Vector3 innerRay = incomingRay;
    innerRay.RotateAroundZ( thetaPrime - theta );
    float innerDistance = 2.0f * _sphereRadius * iv_cos(thetaPrime);
...
void RayTraceLoader::AdvanceObjects(float _advanceTime)
{
    ...
    float zPos = 700.0f - iv_sqrt( m_time - m_startTime ) * 150.0f;
    zPos = max( zPos, 0.0f );
...
//    }
    ...
    float yPos = -7.0f + iv_sin(m_time/6.0f) * 10.0f;
    ...
    if( m_cameraMode == 2 )
    {
        ...
        m_cameraPos.y = 10.0f + iv_sin(m_time * 0.8f) * 10.0f;
        m_cameraPos.SetLength( 100.0f + iv_sin(m_time*0.5f) * 40.0f );
...
void RayTraceLoader::RenderMessage()
{
    if( m_time > m_startTime + 30.0f )
    {
        ...
        float screenX = 160;
        screenX += iv_sin(m_time*2.0f) * 0.5f;

        for( int i = 0; i < 2; ++i )
        {
            float yPos = 20;                                    
            yPos += iv_sin(m_time*3.0f) * 0.5f;
            ...
            g_gameFont.DrawText2DCentre( screenX, yPos+=texH,       texH*4,APP_NAME );
...
void RayTraceLoader::RenderHelp()
{
    ...
    for( int i = 0; i < 2; ++i )
    {
		...
		wchar_t temp[512];
		int tempLen = sizeof(temp)/sizeof(wchar_t);

        g_gameFont.DrawText2D( screenX, yPos+=texH, texH*2,     LANGUAGEPHRASE("bootloader_raytrace_10") );
		swprintf(temp, tempLen, L"X - C  %15ls %dx%d", LANGUAGEPHRASE("bootloader_raytrace_11").m_unicodestring, m_resolution*160, m_resolution*120 );
        g_gameFont.DrawText2D( screenX, yPos+=texH*3, texH, UnicodeString(temp) );
		swprintf(temp, tempLen, L"O - P  %15ls %2.2f", LANGUAGEPHRASE("bootloader_raytrace_12").m_unicodestring, m_reflectiveness );
        g_gameFont.DrawText2D( screenX, yPos+=texH, texH, UnicodeString(temp) );
		swprintf(temp, tempLen, L"K - L  %15ls %2.2f", LANGUAGEPHRASE("bootloader_raytrace_13").m_unicodestring, m_pixelWave );
        g_gameFont.DrawText2D( screenX, yPos+=texH, texH, UnicodeString(temp) );
		swprintf(temp, tempLen, L"N - M  %15ls %2.2f", LANGUAGEPHRASE("bootloader_raytrace_14").m_unicodestring );
        g_gameFont.DrawText2D( screenX, yPos+=texH, texH, UnicodeString(temp) );
		swprintf(temp, tempLen, L"U - I  %15ls %2.2f", LANGUAGEPHRASE("bootloader_raytrace_15").m_unicodestring, m_pixelBlur );
        g_gameFont.DrawText2D( screenX, yPos+=texH, texH, UnicodeString(temp) );
		swprintf(temp, tempLen, L"V - B  %15ls %2.2f", LANGUAGEPHRASE("bootloader_raytrace_16").m_unicodestring, m_brightness );
        g_gameFont.DrawText2D( screenX, yPos+=texH, texH, UnicodeString(temp) ); 
		swprintf(temp, tempLen, L"1      %ls", LANGUAGEPHRASE("bootloader_raytrace_17").m_unicodestring );
        g_gameFont.DrawText2D( screenX, yPos+=texH*2, texH, UnicodeString(temp) );
		swprintf(temp, tempLen, L"2      %ls", LANGUAGEPHRASE("bootloader_raytrace_18").m_unicodestring );
        g_gameFont.DrawText2D( screenX, yPos+=texH, texH, UnicodeString(temp) ); 
		swprintf(temp, tempLen, L"3      %ls", LANGUAGEPHRASE("bootloader_raytrace_19").m_unicodestring );
        g_gameFont.DrawText2D( screenX, yPos+=texH, texH, UnicodeString(temp) ); 
		swprintf(temp, tempLen, L"LMB    %ls", LANGUAGEPHRASE("bootloader_raytrace_20").m_unicodestring );
        g_gameFont.DrawText2D( screenX, yPos+=texH, texH, UnicodeString(temp) ); 
		swprintf(temp, tempLen, L"RMB    %ls", LANGUAGEPHRASE("bootloader_raytrace_21").m_unicodestring );
        g_gameFont.DrawText2D( screenX, yPos+=texH, texH, UnicodeString(temp) );
		swprintf(temp, tempLen, L"WASD   %ls", LANGUAGEPHRASE("bootloader_raytrace_22").m_unicodestring );
        g_gameFont.DrawText2D( screenX, yPos+=texH, texH, UnicodeString(temp) );
		swprintf(temp, tempLen, L"QE     %ls", LANGUAGEPHRASE("bootloader_raytrace_23").m_unicodestring );
        g_gameFont.DrawText2D( screenX, yPos+=texH, texH, UnicodeString(temp) );
...
void RayTraceLoader::RenderRays()
{
    ...
    for( int y = 0; y < screenH-1; ++y )
    {
        ...
        for( int x = 0; x < screenW; ++x )
        {
            ...
            if( m_pixelWave > 0.0f )
            {
                dx = iv_sin(m_time * 5.0f + y * 0.1f) * m_pixelWave;
                dy = iv_sin(m_time * 5.0f + x * 0.1f) * m_pixelWave;
            }
