...
void CreditsLoader::GenerateCharsCentre( float _x, float _y, float _size, const UnicodeString &_string )
{
	float texWidth = g_gameFont.GetTextWidth( _string.Length(), _size );

    GenerateChars( _x - texWidth/2.0f, _y, _size, _string );
}


int CreditsLoader::AddString( float _x, float _y, float _size, const UnicodeString &_string )
{
    //
    ...
    for( int j = 0; j < m_items.Size(); ++j )
    {
        if( m_items.ValidIndex(j) )
        {
			...
			if( thisItem->m_string.StrCmp(_string, true) == 0 &&
    ...
    {        
        ...
        item->m_size = _size;
        item->m_used = true;
        item->m_pos = GenerateOffscreenPosition( 1000.0f + (AppRandom() % 200) );        
        itemId = m_items.PutData( item );
    }

    float texW = g_gameFont.GetTextWidth( _string.Length(), _size );
...
void CreditsLoader::GenerateWords( float _x, float _y, float _size, const UnicodeString &_string )
{
    wchar_t stringCopy[256];
	wcscpy( stringCopy, _string.m_unicodestring );
    
    wchar_t *copyPos = stringCopy;
    float x = _x;

    while( true )
    {
        wchar_t *space = wcschr( copyPos, L' ' );
        if( space ) *space = L'\x0';

        int itemId = AddString( x, _y, _size, UnicodeString(copyPos) );
        ...
        {
            darwinian->m_pos = GenerateOffscreenPosition( 200.0f + (AppRandom() % 100) );
        }
        ...
        x += g_gameFont.GetTextWidth( wcslen(copyPos)+1, _size );
        copyPos = (space+1);
    }
}


void CreditsLoader::GenerateWordsCentre( float _x, float _y, float _size, const UnicodeString &_string )
{
	float texWidth = g_gameFont.GetTextWidth( _string.Length(), _size );
...
void CreditsLoader::GenerateChars( float _x, float _y, float _size, const UnicodeString &_string )
{
    float x = _x;

	for( int i = 0; i < _string.Length(); ++i )    
    {
		wchar_t thisChar = _string.m_unicodestring[i];
        if( thisChar != L' ' )
        {
            wchar_t buffer[64];
            swprintf( buffer, sizeof(buffer)/sizeof(wchar_t), L"%c", thisChar);

            int itemId = AddString( x, _y, _size, UnicodeString(buffer) );
            CreditsItem *item = m_items[itemId];
            CreditsDarwinian *darwinian = new CreditsDarwinian();
    ...
    for( int i = 0; i < m_items.Size(); ++i )
    {
        if( m_items.ValidIndex(i) )
        {
            ...
            if( !item->m_used )
            {
                ...
                CreditsDarwinian *darwinian = new CreditsDarwinian();
                darwinian->m_pos = GenerateOffscreenPosition( 40 + (AppRandom() % 50) );
...
Vector2 CreditsLoader::GenerateOffscreenPosition( float _distanceOut )
{
    ...
    switch( AppRandom() % 4 )
    {
        case 0 :    result.Set( -1 * _distanceOut, AppRandom() % 600 );         break;
        case 1 :    result.Set( AppRandom() % 800, -1 * _distanceOut );         break;
        case 2 :    result.Set( 800 + _distanceOut, AppRandom() % 600 );        break;
        case 3 :    result.Set( AppRandom() % 800, 600 + _distanceOut );        break;
    }
...
#ifdef OBERONBUILD
            GenerateWordsCentre( 400, 400, 30, LANGUAGEPHRASE("bootloader_credits_15") );
#endif
            break;
