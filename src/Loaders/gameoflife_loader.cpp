...
{    
    ...
    while( !g_inputManager->controlEvent( ControlSkipMessage ) )
    {
        ...
        m_zoom = 0.08f + iv_pow( GetHighResTime() - startTime, 2.0f ) / 500;
        ...
        if( m_allDead > 0.0f )
        {
			...
			swprintf(tempString, sizeof(tempString)/sizeof(wchar_t), L"%ls %d", LANGUAGEPHRASE("bootloader_life_17").m_unicodestring, m_totalAge);
            g_gameFont.DrawText2DCentre( 0.0f, screenH/3+screenW/100.0f, screenW/100.0f, UnicodeString(tempString) );
