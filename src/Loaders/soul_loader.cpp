    ...
    for( int i = 0; i < NUMMESSAGES; ++i )
    {
        ...
        if( timePassed >= startTime[i] && timePassed <= endTime )
        {
            ...
            for( int j = 0; j < 4; ++j )
            {
                ...
                pos += Vector3( iv_sin(m_time+j), iv_cos(m_time/2+j), iv_sin(m_time/3+j*2) ) * thisSize * 0.06f;

                char msgId[256];
                sprintf( msgId, "bootloader_soul_%d", i );
                g_gameFont.DrawText3DCentre( pos, thisSize, LANGUAGEPHRASE(msgId) );
            }
        }
    }
}
...
void SoulLoaderSpirit::Advance( float _time )
{
    ...
    vel.x = iv_sin( m_positionOffset ) * m_xaxisRate;
    vel.z = iv_sin( m_positionOffset ) * m_zaxisRate;            
...
{    
    ...
    glBegin( GL_QUADS );
        glVertex3dv( (m_pos - camUp*size).GetData() );
        glVertex3dv( (m_pos + camRight*size).GetData() );
        glVertex3dv( (m_pos + camUp*size).GetData() );
        glVertex3dv( (m_pos - camRight*size).GetData() );
    glEnd();    
