...
class LevelFile
{
  ...
  void ParseMWMFile(const char* _filename);
  ...
  public:
    enum // level options
    {
      LevelOptionNoHandicap = 0,
      // Handicap is overridden to 0 regardless of game option
      LevelOptionInstantSpawnPointCapture,
      // spawn points have no 30 second capture timer and become active instantly
      LevelOptionTrunkPortReinforcements,
      // activates the RocketRiot style trunk port reinforcements on a level
      LevelOptionTrunkPortArmour,
      // activates trunk port armour spawns ala rocket riot
      LevelOptionNoGunTurrets,
      // disables GunTurret tasks from TrunkPorts on RocketRiot
      LevelOptionAttackerReinforcements,
      // defines the number of darwinians attacking teams in assault receive as reinforcements
      LevelOptionDefenderReinforcements,
      // defines the number of darwinians the defending team receieves
      LevelOptionDefenderReinforcementType,
      // specify what type of reinforcement the defending team receives. replaces the TrunkPortArmour option (which must be active for this option to have effect). uses globalresearch type names
      LevelOptionAttackerReinforcementType,
      // specify what type of reinforcements the attacking team recieves
      LevelOptionForceRadarTeamMatch,
      // 2 radar dishes must be owned by the same team before they will connect
      LevelOptionNoRadarControl,
      // radar dishes cannot be manually aimed
      LevelOptionAirstrikeCaptureZones,
      // the number of flags you need to have captured in blitzkrieg before your gunturrets are replaced with airstrikes
      LevelOptionFuturewinianTeam,
      // the team id specified here has Futurewinians instead of darwinians
      LevelOptionAttackReinforcementMode,
      // sets a specific option to the reinforcement task (ie, napalm strike, flame turret)
      LevelOptionDefendReinforcementMode,
      // sets a specific option to the reinforcement task (ie, napalm strike, flame turret)
      NumLevelOptions
    };
    ...
    int m_populationCap;
    int m_defenderPopulationCap;
    // used to enforce defender/attack ratios in Assault by limiting the number of darwinians per defending team
    int m_numPlayers;
    ...
    static int CalculeCRC(const char* _mapFilename);
};
