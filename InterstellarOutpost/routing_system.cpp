...
RoutingSystem::RoutingSystem() { m_walkableRoute = false; }
...
class WalkableRSCache : public RoutingSystemCache<bool>
{
  public:
    ...
    {
      return g_app->m_location->IsWalkable(m_nodes[_from]->m_waypoint.GetPos(), m_nodes[_to]->m_waypoint.GetPos(),
                                           m_nodes[_from]->m_checkCliffs && m_nodes[_to]->m_checkCliffs);
    }
};
...
class WalkableDistanceRSCache : public RoutingSystemCache<double>
{
  public:
    ...
    {
      return g_app->m_location->GetWalkingDistance(m_nodes[_from]->m_waypoint.GetPos(), m_nodes[_to]->m_waypoint.GetPos(), false);
    }
};

int RoutingSystem::GetNearestNode(const Vector3& pos, bool requireWalkable)
{
  int nearestId = -1;
  ...
  for (int i = 0; i < m_nodes.Size(); ++i)
  {
    if (m_nodes.ValidIndex(i))
    {
      ...
      {
        int fence = -1;
        if (!requireWalkable || g_app->m_location->IsWalkable(pos, fromNode->m_waypoint.GetPos(), true, false, &fence))
        {
          if (fence == -1)
          {
            nearest = dist;
...
int RoutingSystem::GenerateRoute(const Vector3& fromPos, const Vector3& toPos, bool directRoute, bool permitClimbing, bool noRounding)
{
  ...
  if (targetNodeId == -1)
    targetNodeId = GetNearestNode(toPos, false);
  if (startNodeId == -1)
    startNodeId = GetNearestNode(fromPos, false);
  ...
  if (complexRoutePossible)
  {
    ...
    straightLineWalkable = g_app->m_location->IsWalkable(fromPos, toPos, true, permitClimbing) && g_app->m_location->
      IsWalkable(fromPos - rightAngle, toPos - rightAngle, true, permitClimbing) && g_app->m_location->IsWalkable(
        fromPos + rightAngle, toPos + rightAngle, true, permitClimbing);
  ...
  if (!straightLineWalkable && complexRoutePossible)
  {
    ...
    while (true)
    {
      ...
      {
        DebugTrace("Error in route planning!\n");
  ...
  if (route->m_wayPoints.Size() > 2 && !noRounding)
  {
    ...
    for (int i = 1; i < route->m_wayPoints.Size() - 1; ++i)
    {
      ...
      for (int j = numSteps - 1; j >= 0; --j)
      {
        ...
        Vector3 thisPos = fromPos + (midPos - fromPos) * (j / static_cast<double>(numSteps));

        Vector3 rightAngle = (thisPos - toPos) ^ g_upVector;
        rightAngle.SetLength(40);

        if (!g_app->m_location->IsWalkable(thisPos, toPos, true, true) || !g_app->m_location->
                                                                                  IsWalkable(thisPos - rightAngle, toPos - rightAngle, true,
                                                                                    true) || !g_app->m_location->IsWalkable(
          thisPos + rightAngle, toPos + rightAngle, true, true))
          break;
  ...
  if (straightLineWalkDistance >= 0 && (!climbing || permitClimbing))
  {
  ...
  if (timeTaken > 10)
    DebugTrace("Performance warning : Route planned in %2.2ms\n", timeTaken);
...
void RoutingSystem::HighlightRoute(int routeId, int teamId)
{
  ...
  for (int i = 0; i < route->m_wayPoints.Size() - 1; ++i)
  {
    ...
    {
      Vector3 thisPos = from + (to - from) * (j / static_cast<double>(numSteps));
...
void RoutingSystem::RenderNodes()
{
  for (int i = 0; i < m_nodes.Size(); ++i)
  {
    if (m_nodes.ValidIndex(i))
    {
      ...
      g_editorFont.DrawText3DCentre(fromNode->m_waypoint.GetPos() + g_upVector * 20, 10, "%d", fromNode->m_routeTable.Size());
      g_editorFont.DrawText3DCentre(fromNode->m_waypoint.GetPos() + g_upVector * 40, 10, "%d", i);

      for (int j = 0; j < fromNode->m_routeTable.Size(); ++j)
      {
        RoutingTableEntry* entry = fromNode->m_routeTable[j];

        if (entry->m_nextNode == entry->m_targetNode && m_nodes.ValidIndex(entry->m_targetNode))
        {
          ...
          {
            Vector3 position = fromNode->m_waypoint.GetPos() + (toNode->m_waypoint.GetPos() - fromNode->m_waypoint.GetPos()) * (i /
...
void RoutingSystem::Render()
{
  ...
  for (int j = 0; j < m_routes.Size(); ++j)
  {
    if (m_routes.ValidIndex(j))
    {
      ...
      if (!g_app->m_location->m_landscape.IsInLandscape(routeEndPoint))
        continue;
      ...
      if (route->m_alpha > 0.0)
      {
        ...
        if (e)
        {
          m_walkableRoute = e->m_type == Entity::TypeArmour || e->m_type == Entity::TypeEngineer || e->m_type == Entity::TypeHarvester;
        }
        ...
        if (!m_walkableRoute)
        {
          ...
          for (int i = 0; i < route->m_wayPoints.Size() - 1; ++i)
          {
            if (!g_app->m_location->IsWalkable(route->GetWayPoint(i)->GetPos(), route->GetWayPoint(i + 1)->GetPos()))
            {
              nonWalkable = i;
              break;
            }
        ...
        for (int i = 0; i < route->m_wayPoints.Size() - 1; ++i)
        {
          if (i == nonWalkable)
            m_walkableRoute = false;
          RenderRouteLine(route->GetWayPoint(i)->GetPos(), route->GetWayPoint(i + 1)->GetPos(), route->m_alpha);
        }
      }
    }
  }
...
void RoutingSystem::RenderRouteLine(const Vector3& fromPos, const Vector3& toPos, double intensity)
{
  ...
  RGBAColour colour = m_walkableRoute ? g_app->m_location->GetMyTeam()->m_colour : RGBAColour(0, 0, 0, 100);
  ...
  for (int i = 0; i < numSteps; ++i)
  {
    double thisAlpha2 = thisAlpha * (0.5 + sin(GetHighResTime() * 10 + i * 0.7) * 0.5);
...
RouteNode::~RouteNode() { m_routeTable.EmptyAndDelete(); }
...
void RouteNode::AddRoute(int targetNodeId, int nextNodeId, double totalDistance)
{
  ...
  for (int i = 0; i < m_routeTable.Size(); ++i)
  {
    if (m_routeTable[i]->m_targetNode == targetNodeId && m_routeTable[i]->m_totalDistance <= totalDistance)
    {
...
// *** Constructor
WayPoint::WayPoint(int _type, const Vector3& _pos)
  : m_type(_type),
    m_pos(_pos),
    m_buildingId(-1) {}
...
Vector3 WayPoint::GetPos()
{
  ...
  else if (m_type == TypeBuilding)
  {
    ...
    if (building)
    {
      ...
      auto teleport = (Teleport*)building;
      Vector3 pos, front;
...
// *** SetPos
void WayPoint::SetPos(const Vector3& _pos) { m_pos = _pos; }
...
void Route::AddWayPoint(const Vector3& _pos)
{
...
int Route::GetIdOfNearestWayPoint(const Vector3& _pos)
{
  int idOfNearest = -1;
...
int Route::GetIdOfNearestEdge(const Vector3& _pos, double* _dist)
{
  int idOfNearest = 0;
...
Vector3 Route::GetEndPoint() { return m_wayPoints[m_wayPoints.Size() - 1]->GetPos(); }
    ...
    if (m_nodes.ValidIndex(i))
    {
      ...
      for (int j = 0; j < m_nodes.Size(); ++j)
      {
        if (m_nodes.ValidIndex(j))
        {
          ...
          if (distance < 600 && walkableCache(i, j))
          {
            double walkDistance = walkableDistanceCache(i, j);
