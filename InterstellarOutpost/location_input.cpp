...
void LocationInput::AdvanceRadarDishControl(Building* _building)
{
  if (g_inputManager->controlEvent(ControlUnitSelect))
  {
    ...
    if (building && building->m_type == Building::TypeRadarDish && building != _building)
    {
      ...
      g_app->m_clientToServer->RequestAimBuilding(g_app->m_globalWorld->m_myTeamId, _building->m_id.GetUniqueId(), dish->GetStartPoint());
    }
    else
    {
      g_app->m_clientToServer->RequestAimBuilding(g_app->m_globalWorld->m_myTeamId, _building->m_id.GetUniqueId(),
...
bool LocationInput::GetObjectUnderMouse(WorldObjectId& _id, int _teamId)
{
  ...
  if (building)
  {
    bool selectableType = (building->m_type == Building::TypeRadarDish || building->m_type == Building::TypeGunTurret || building->m_type ==
  ...
  if (buildId != -1 && buildDist < entDist && buildDist < unitDist && ((b && b->m_id.GetTeamId() == _teamId) || (_teamId == 255) || (b && b
    ->m_id.GetTeamId() == 255)))
  {
    // Building is nearest
...
void LocationInput::SelectObjectUnderMouse(WorldObjectId& id)
{
  ...
  bool somethingSelected = currentTask || team->GetMyEntity() || team->GetMyUnit() || team->m_numDarwiniansSelected > 0;
  ...
  if (!cantSelectBecauseSomethingAlreadySelected)
  {
    ...
    if (id.IsValid())
    {
      if (id.GetUnitId() == UNIT_BUILDINGS)
      {
        ...
        if (building && building->m_id.GetTeamId() == team->m_teamId)
        {
          ...
          else if (building->m_type == Building::TypeGunTurret || building->m_type == Building::TypeChessPiece)
          {
          ...
          {
            FenceSwitch* fs = static_cast<FenceSwitch*>(building);
            fs->Switch();
            doneIt = true;
          }
        }
      ...
      else
      {
        ...
        if ((entity && entity->m_type != Entity::TypeDarwinian && entity->IsSelectable() && entity != team->GetMyEntity()) || (unit && unit
          ->IsSelectable() && unit != team->GetMyUnit()))
        {
    ...
    else
    {
      ...
      if (mwControlMethod == 1)
      {
        ...
        bool gamepadMove = (INPUT_MODE_GAMEPAD == g_inputManager->getInputMode() && myEntity && (myEntity->m_type == Entity::TypeOfficer ||
  ...
  for (int i = 0; i < team->m_others.Size(); ++i)
  {
    if (team->m_others.ValidIndex(i))
    {
      ...
      if (entity && entity->m_type == Entity::TypeDarwinian)
      {
        ...
        if (darwinian->m_underPlayerControl && darwinian->m_state == Darwinian::StateCarryingBuilding)
        {
          Building* building = g_app->m_location->GetBuilding(darwinian->m_buildingId);
          if (building && building->m_type == Building::TypeWallBuster)
            ++count;
        }
      }
    }
...
void LocationInput::AdvanceTeamControl()
{
  ...
  if (team->m_taskManager)
  {
    ...
    if (team->m_taskManager->m_mostRecentTaskId != -1 && g_prefsManager->GetInt("HelpEnabled", 1))
    {
  ...
  if (!taskStarted && !g_app->m_taskManagerInterface->IsVisible() && !g_app->m_camera->IsInMode(Camera::ModeEntityTrack) && !g_app->
    m_location->GetMyTeam()->GetMyEntity() && !g_app->m_location->GetMyTeam()->GetMyUnit() && g_app->IsSinglePlayer())
  {
    if (g_inputManager->controlEvent(ControlIconsTaskManagerQuickUnitLeft) && g_app->m_globalWorld->m_research->HasResearch(
    ...
    else if (g_inputManager->controlEvent(ControlIconsTaskManagerQuickUnitRight) && g_app->m_globalWorld->m_research->HasResearch(
    ...
    else if (g_inputManager->controlEvent(ControlIconsTaskManagerQuickUnitUp) && g_app->m_globalWorld->m_research->HasResearch(
  ...
  if (controlUnitSelect && !g_app->m_taskManagerInterface->IsVisible())
  ...
  if (controlUnitToggle && !g_app->m_taskManagerInterface->IsVisible() && !taskStarted && team->m_numDarwiniansSelected == 0 && !g_app->
    m_multiwinia->GameInGracePeriod() && !g_app->m_multiwinia->GameOver() && !g_app->GamePaused())
  {
    if (entityUnderMouse)
    {
      bool allowToggle = team->m_currentUnitId == -1 && team->m_currentEntityId == -1 && team->m_currentBuildingId == -1;
      ...
      if (allowToggle)
      {
        if (entityUnderMouse->m_type == Entity::TypeArmour)
        {
          ...
          if (togglingConversionOrders && team->GetMyEntity() == entityUnderMouse)
          {
        ...
        if (entityUnderMouse->m_type == Entity::TypeOfficer)
        {
  ...
  if (controlDarwinianPromote && !taskStarted && ((team->m_numDarwiniansSelected == 0 && !g_app->m_multiwinia->GameInGracePeriod() && !g_app
    ->m_multiwinia->GameOver() && !g_app->GamePaused()) || g_app->IsSinglePlayer()))
  {
    if (team->m_currentUnitId == -1 && team->m_currentEntityId == -1 && team->m_currentBuildingId == -1)
    {
      Vector3 mousePos = g_app->m_userInput->GetMousePos3d();
      WorldObjectId id = Task::FindDarwinian(mousePos, g_app->m_globalWorld->m_myTeamId);
      Darwinian* darwinian = static_cast<Darwinian*>(g_app->m_location->GetEntitySafe(id, Entity::TypeDarwinian));
...

  {
    if (idUnderMouse.GetUnitId() == UNIT_BUILDINGS)
    {
      ...
      if (building)
      {
        ...
        {
          Vector3 front;
          static_cast<RadarDish*>(building)->GetEntrance(highlightedObjectPos, front);
        }
    ...
    else
    {
      ...
      if (entity)
      {
        if (entity->m_type == Entity::TypeArmour && static_cast<Armour*>(entity)->GetNumPassengers() < static_cast<Armour*>(entity)->
          Capacity())
  ...
  if (!g_app->IsSinglePlayer() && !g_app->m_taskManagerInterface->IsVisible() && !taskStarted && team->m_currentUnitId == -1 && team->
    m_currentEntityId == -1 && team->m_currentBuildingId == -1)
  {
    if (team->m_numDarwiniansSelected > 0 && team->m_selectionCircleCentre == g_zeroVector)
    {
    ...
    {
      if (g_inputManager->controlEvent(ControlBeginSelectionCircle) && team->m_selectionCircleCentre == g_zeroVector && !officerUndermouse
        && !g_app->m_multiwinia->GameInGracePeriod() && !circleBlocked)
      {
      ...
      if (g_inputManager->controlEvent(ControlEnlargeSelectionCircle))
      {
        ...
        if (team->m_selectionCircleRadius > 100)
          team->m_selectionCircleRadius = 100;

        double timeNow = GetNetworkTime();
        if (timeNow > team->m_lastOrdersSet)
        {
          team->m_lastOrdersSet = timeNow;
          g_app->m_clientToServer->RequestSelectDarwinians(g_app->m_globalWorld->m_myTeamId, team->m_selectionCircleCentre,
                                                           team->m_selectionCircleRadius);
        }
  ...
  {
    if (!g_app->m_taskManagerInterface->IsVisible() && !taskStarted && team->m_numDarwiniansSelected > 0 && team->m_selectionCircleCentre ==
      g_zeroVector)
    {
  ...
  bool objectSelected = team->m_currentUnitId != -1 || team->m_currentEntityId != -1 || team->m_currentBuildingId != -1 || team->
  ...
  {
    if (g_app->IsSinglePlayer() || (!g_app->m_multiwinia->GameInGracePeriod() && !g_app->m_multiwinia->GameOver() && !g_app->GamePaused()))
    {
      if (g_inputManager->controlEvent(ControlUnitCreate))
      {
        ...
        g_app->m_clientToServer->RequestTargetProgram(g_app->m_globalWorld->m_myTeamId,
                                                      g_app->m_location->GetMyTaskManager()->m_currentTaskId, mousePos);
      }

      if (g_inputManager->controlEvent(ControlIssueSpecialOrders) && !increasingSelectionCircle)
      {
        Task* currentTask = g_app->m_location->GetMyTaskManager()->GetCurrentTask();
        bool placingGamepadTask = currentTask && currentTask->m_state == Task::StateStarted && INPUT_MODE_GAMEPAD == g_inputManager->
          getInputMode();
    ...
    else
    {
      ...
      else if (ent->m_type == Entity::TypeOfficer)
      {
        ...
        else
        {
          ...
          if (g_inputManager->controlEvent(ControlIssueSpecialOrders) || (g_inputManager->getInputMode() == INPUT_MODE_GAMEPAD &&
            specialMove))
          {
...
void LocationInput::Advance()
{
  ...
  {
    if (g_inputManager->controlEvent(ControlToggleRendering)) { g_app->ToggleTestBedRendering(); }
  }
#endif

  bool chatLog = g_app->m_camera->ChatLogVisible();

  if (g_app->m_globalWorld->m_myTeamId != 255 && EclGetWindows()->Size() == 0 && !chatLog)
    AdvanceTeamControl();

  if (g_app->Multiplayer())
  {
#ifndef TESTBED_ENABLED
    if (g_app->m_multiwinia->GameInGracePeriod() && g_inputManager->controlEvent(ControlToggleReady) && EclGetWindows()->Size() == 0)
    ...
    if (g_app->m_multiwinia->GameOver())
    {
      ...
      if (g_app->GetTestBedMode() == TESTBED_ON)
      {
        if (EclGetWindows()->Size() == 0)
        {
          ...
          if (g_app->GetTestBedState() != TESTBED_GAME_OVER_DELAY) { g_app->TestBedLogWrite(); }
      ...
      {
        if (g_inputManager->controlEvent(ControlToggleReady) && EclGetWindows()->Size() == 0)
        {
          g_app->m_requestedLocationId = -1;
          g_app->m_atMainMenu = true;
        }
    ...
    if (g_inputManager->controlEvent(ControlLobbyChat) && !ChatInputWindow::IsChatVisible() && !m_chatToggledThisUpdate && g_inputManager->
      getInputMode() != INPUT_MODE_GAMEPAD)
    {
      if (EclGetWindows()->Size() == 0)
      {
