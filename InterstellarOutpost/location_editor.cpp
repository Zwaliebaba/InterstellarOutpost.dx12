...
#include "buildings_window.h"
#include "camera_mount_window.h"
#include "camera_anim_window.h"
#include "editor_window.h"
#include "instant_unit_window.h"
#include "landscape_window.h"
#include "lights_window.h"
#include "tree_window.h"
#include "multiwiniaoptions_window.h"
...
#define INSTANT_UNIT_SIZE_FACTOR 6.0
...
int LocationEditor::DoesRayHitInstantUnit(Vector3 const &rayStart, Vector3 const &rayDir)
{
	...
	for (int i = 0; i < location->m_levelFile->m_instantUnits.Size(); i++)
	{
		...
		bool result = RaySphereIntersection(rayStart, rayDir, pos, 
											sqrt(iu->m_number) * INSTANT_UNIT_SIZE_FACTOR);
		if (result)
		{
			return i;
		}
...
void LocationEditor::AdvanceModeLight()
{
	...
	if (location->m_lights.ValidIndex(m_selectionId) &&  g_inputManager->controlEvent( ControlTileDrag ) )
	{
		...
		front.RotateAroundY((float)g_target->dX() * 0.05);
  	...
  	if ( g_inputManager->controlEvent( ControlTileSelect ) )
	{
		...
		if (win && win->m_newNodeArmed)
		{
			...
			if (mountId != -1)
			{
				...
				node->m_mountName = _strdup(mount->m_name);
...
void LocationEditor::RenderUnit(InstantUnit *_iu)
{
	...
	// Render troops
	int maxX = (int)sqrt(_iu->m_number);
    ...
    if( m_mode == ModeInstantUnit )
    {
        ...
        {
            float xDiff = _iu->m_spread * sin(angle);
            float zDiff = _iu->m_spread * cos(angle);
...
void LocationEditor::RenderModeInstantUnit()
{
	if (m_selectionId != -1)
	{
		...
		RenderSphere(pos, sqrt(iu->m_number) * INSTANT_UNIT_SIZE_FACTOR);	
	}
}
