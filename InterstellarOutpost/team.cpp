...
void Team::SelectUnit(int _unitId, int _entityId, int _buildingId)
{
  ...
  if (m_teamId == g_app->m_globalWorld->m_myTeamId)
  {
    ...
    if (b && b->m_type == Building::TypeGunTurret && !g_app->m_camera->IsInMode(Camera::ModeTurretAim))
      g_app->m_camera->RequestTurretAimMode(b);
  }

  Building* b = g_app->m_location->GetBuilding(m_currentBuildingId);
  if (b && b->m_type == Building::TypeGunTurret)
    static_cast<GunTurret*>(b)->m_fireTimer = 0.5f;
}
...
Unit* Team::GetMyUnit()
{
  if (m_currentUnitId == -1 || !m_units.ValidIndex(m_currentUnitId))
    return NULL;
  if (m_units.ValidIndex(m_currentUnitId))
    return m_units[m_currentUnitId];
  return NULL;
}

Entity* Team::RayHitEntity(const Vector3& _rayStart, const Vector3& _rayEnd)
{
  // Hit against Units
...
Entity* Team::GetMyEntity()
{
  if (m_currentEntityId == -1)
    return NULL;
  if (m_others.ValidIndex(m_currentEntityId))
    return m_others[m_currentEntityId];
  return NULL;
}

Unit* Team::NewUnit(int _troopType, int _numEntities, int* _unitId, const Vector3& _pos)
{
...
void Team::Advance(int _slice)
{
  if (m_teamType > TeamTypeUnused)
  {
    ...
    if (_slice == 0)
    {
      ...
      for (int unit = 0; unit < m_units.Size(); ++unit)
      {
        if (m_units.ValidIndex(unit))
        {
          ...
          bool WatchEntities(); if (WatchEntities())
    ...
    if (_slice == 0)
    {
      ...
      m_selectedDarwinianCentre = m_newSelectedDarwinianCentre / static_cast<double>(m_numDarwiniansSelected);
...
#ifdef TRACK_SYNC_RAND
          SyncRandLogText(ent->LogState("ENTITY AFTER: ")); bool WatchEntities(); WatchEntities();
#endif
          END_PROFILE(entityName);

          if (ent->m_type == Entity::TypeDarwinian && !ent->m_dead && !amIdead && static_cast<Darwinian*>(ent)->m_underPlayerControl)
          {
...
void Team::RenderVirii(double _predictionTime)
{
  ...
  g_app->m_camera->SetupProjectionMatrix(nearPlaneStart * 1.05, g_app->m_renderer->GetFarPlane());
  ...
  for (int i = 0; i <= m_others.Size(); i++)
  {
    if (m_others.ValidIndex(i))
    {
      ...
      {
        Virii* virii = static_cast<Virii*>(entity);
        if (virii->IsInView())
        {
          double rangeToCam = (virii->m_pos - g_app->m_camera->GetPos()).Mag();
          int viriiDetail = 1;
          if (entityDetail == 1 && rangeToCam > 1000.0)
            viriiDetail = 2;
          else if (entityDetail == 2 && rangeToCam > 1000.0)
            viriiDetail = 3;
          else if (entityDetail == 2 && rangeToCam > 500.0)
            viriiDetail = 2;
          else if (entityDetail == 3 && rangeToCam > 1000.0)
            viriiDetail = 4;
          else if (entityDetail == 3 && rangeToCam > 600.0)
            viriiDetail = 3;
          else
            if (entityDetail == 3 && rangeToCam > 300.0)
              viriiDetail = 2;
...
void Team::RenderDarwinians(double _predictionTime)
{
  ...
  if (entityDetail <= 1)
    highDetailDistanceSqd = 99999.9;
  else if (entityDetail == 2)
    highDetailDistanceSqd = 1000.0;
  else
    if (entityDetail >= 3)
      highDetailDistanceSqd = 500.0;
...
UnicodeString Team::GetTeamName()
{
  ...
  else
  {
    ...
    else
    {
      ...
      {
      case 0:
        name = LANGUAGEPHRASE("multiwinia_team_green");
        break;
      case 1:
        name = LANGUAGEPHRASE("multiwinia_team_red");
        break;
      case 2:
        name = LANGUAGEPHRASE("multiwinia_team_yellow");
        break;
      case 3:
        name = LANGUAGEPHRASE("multiwinia_team_blue");
        break;
      case 4:
        name = LANGUAGEPHRASE("multiwinia_team_orange");
        break;
      case 5:
        name = LANGUAGEPHRASE("multiwinia_team_purple");
        break;
      case 6:
        name = LANGUAGEPHRASE("multiwinia_team_cyan");
        break;
      case 7:
        name = LANGUAGEPHRASE("multiwinia_team_pink");
        break;

      default:
        name = UnicodeString("BAD TEAM NAME");
        break;
      }
...
// ****************************************************************************
//  Class TeamControls — client-side Advance() only
//  Pure data methods (GetFlags, SetFlags, Clear, Pack, Unpack) are now in
//  NeuronCore/team_controls.cpp
// ****************************************************************************
...
void TeamControls::Advance()
{
  ...
  m_targetDirected |= g_inputManager->controlEvent(ControlTargetMoveSecondary);
  ...
  if (g_inputManager->controlEvent(ControlUnitMove, details))
  {
    ...
    DEBUG_ASSERT(SHRT_MIN <= waypoint.x && waypoint.x <= SHRT_MAX);
    DEBUG_ASSERT(SHRT_MIN <= waypoint.y && waypoint.y <= SHRT_MAX);
  ...
  if (g_inputManager->controlEvent(ControlUnitTargetDirected))
  {
    ...
    Squadie* pointMan = static_cast<Squadie*>(g_app->m_camera->m_trackingEntity);
    if (pointMan)
    {
...
{
  bool match = (g_app->m_location->m_levelFile->m_levelOptions[LevelFile::LevelOptionFuturewinianTeam] == g_app->m_location->
    GetTeamPosition(m_teamId));
...
void Team::Eliminate()
{
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    if (g_app->m_location->m_buildings.ValidIndex(i))
    {
      ...
      if (building->m_type == Building::TypeTrunkPort && building->m_id.GetTeamId() == m_teamId)
