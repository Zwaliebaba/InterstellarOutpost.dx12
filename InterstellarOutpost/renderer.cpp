...
void Renderer::Initialise()
{
  ...
  bool windowed = static_cast<bool>(g_prefsManager->GetInt("ScreenWindowed", 1));
  ...
  bool success = g_windowManager->CreateWin(m_screenW, m_screenH, windowed, colourDepth, refreshRate, zDepth, true, true, APP_NAME_W);
  ...
  if (!success)
  {
    ...
    success = g_windowManager->CreateWin(m_screenW, m_screenH, windowed, colourDepth, refreshRate, zDepth, true, true, APP_NAME_W);
...
void Renderer::RenderFrame(bool withFlip)
{
  ...
  {
    if ((g_app->m_gameMode != App::GameModeMultiwinia && !g_eventHandler->WindowHasFocus()) || g_app->m_userRequestsPause)
      RenderPaused();
  }
...
{
  GLint returnVal;
  glGetIntegerv(static_cast<GLenum>(pname), &returnVal);
  return returnVal;
}
...
void Renderer::CheckOpenGLState(bool fullCheck, int lineNumber) const
{
  ...
  if (glError != GL_NO_ERROR)
    printf("glError: %d, %d\n", glError, lineNumber);
  DEBUG_ASSERT(glError == GL_NO_ERROR);
...
void Renderer::Clip(int _x, int _y, int _w, int _h)
{
  ...
  //    DebugTrace(" Projected coordinates are: x1 = %f, y1 = %f, x2 = %f, y2 = %f\n", x1, y1, x2, y2 );

  glScissor(x1, y2, x2 - x1, y1 - y2);
  glEnable(GL_SCISSOR_TEST);
}

void Renderer::EndClip() { glDisable(GL_SCISSOR_TEST); }
...
void Renderer::RenderMenuTransition(float _f)
{
  ...
  glTexCoord2f(_p, d3dOneMinus(1 - _p));
  glVertex2f(left, top);
  glTexCoord2f(1 - _p, d3dOneMinus(1 - _p));
  glVertex2f(right, top);
  glTexCoord2f(1 - _p, d3dOneMinus(_p));
  glVertex2f(right, bottom);
  glTexCoord2f(_p, d3dOneMinus(_p));
...
bool Renderer::IsMenuTransitionComplete() const { return m_menuTransitionState == MenuTransitionFinished; }
