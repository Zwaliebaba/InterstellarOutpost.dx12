    ...
    m_numTris(0)
{
  ...
  for (int j = 0; j < _frag->m_numTriangles; ++j)
  {
    ...
    constexpr float minFragLife = EXPLOSION_LIFETIME * MIN_FRAG_LIFE;
...
void Explosion::Render()
{
  ...
  for (int i = 0; i < m_numTris; ++i)
  {
    ...
    const Vector3 norm(m_tris[i].m_tumbler->m_rotMat * m_tris[i].m_norm);
    const Vector3 v1(m_tris[i].m_tumbler->m_rotMat * m_tris[i].m_v1 + m_tris[i].m_pos);
    const Vector3 v2(m_tris[i].m_tumbler->m_rotMat * m_tris[i].m_v2 + m_tris[i].m_pos);
    const Vector3 v3(m_tris[i].m_tumbler->m_rotMat * m_tris[i].m_v3 + m_tris[i].m_pos);

    glColor4ub(m_tris[i].m_colour.r, m_tris[i].m_colour.g, m_tris[i].m_colour.b, (1.0 - age) * 255);

    glNormal3dv(norm.GetDataConst());

    glTexCoord2i(0, 0);
    glVertex3dv(v1.GetDataConst());
    glTexCoord2i(0, 1);
    glVertex3dv(v2.GetDataConst());
    glTexCoord2i(1, 1);
    glVertex3dv(v3.GetDataConst());
  }
  glEnd();
}
