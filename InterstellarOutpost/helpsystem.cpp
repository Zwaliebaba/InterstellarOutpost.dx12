...
namespace {
	...
	class TaskShutdownActionHelp : public ActionHelp
	{
		bool IsActionAvailable()
		{
			return( g_app->m_location->GetMyTeam()->m_taskManager->CapacityUsed() == 
                g_app->m_location->GetMyTeam()->m_taskManager->Capacity() );
		}
	...
	class TaskReselectActionHelp : public ActionHelp
	{
		bool IsActionAvailable()
		{
			...
			return( g_app->m_location->GetMyTeam()->m_taskManager->m_tasks.Size() > 0 && !somethingSelected );
		}
	...
	class SquadSummonActionHelp : public ActionHelp
	{
		bool IsActionAvailable()
		{
			bool spaceAvailable = g_app->m_location->GetMyTeam()->m_taskManager->CapacityUsed() < g_app->m_location->GetMyTeam()->m_taskManager->Capacity();
	...
	class OfficerCreateActionHelp : public ActionHelp
	{
		...
		void GiveActionHelp()
		{
			...
			g_app->m_sepulveda->DemoGesture( "officer.txt", 2.0 );
		}
...
HelpSystem::~HelpSystem()
{
	// There are some shared items, let's not delete them twice
	m_actionHelp[UseGunTurret] = NULL;
	m_actionHelp[UseRadarDish] = NULL;

	m_actionHelp.EmptyAndDelete();
	m_buildingHelp.EmptyAndDelete();
}
...
void HelpSystem::RunDefaultHelp()
{
    ...
    if( g_app->m_location && team )
    {
        ...
        if( !helpGiven )
        {
            int numRunningTasks = g_app->m_location->GetMyTaskManager()->CapacityUsed();
            int maxTasks = g_app->m_location->GetMyTaskManager()->Capacity();
...
void HelpSystem::Render()
{
		...
		g_inputManager->getInputMode() == INPUT_MODE_KEYBOARD )
    {
        ...
        g_gameFont.DrawText2DCentre( g_app->m_renderer->ScreenW()/2.0, 15, 15, LANGUAGEPHRASE("help_pressh") );
