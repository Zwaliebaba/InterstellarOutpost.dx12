  ...
  if (res > 0)
  {
    ...
    for (int z = 0; z < res; ++z)
    {
      for (int x = 0; x < res; ++x)
      {
        ...
        *target = 'A' + static_cast<int>(actualVal / 16);
        *(target + 1) = 'A' + actualVal - static_cast<int>(actualVal / 16) * 16;
...
double LandscapeTile::GenerateNoise(double _halfSize, double _height)
{
  ...
  double val = Landscapesfrand(pow(length * 10.0, m_fractalDimension));
  val *= m_compensatedHeightScale;
  val *= 0.1 + static_cast<double>(pow(fabsf(_height), m_lowlandSmoothingFactor)) * 0.15;
  return val;
}
...
void LandscapeTile::GenerateDiamondMidpoint(int _x, int _z, int _halfSize)
{
  ...
  case 0:
    ...
    break;
  case 1:
    if (LandscapeAppRandom() & 1) { newHeight = m_heightMap->GetData(_x - _halfSize, _z) + m_heightMap->GetData(_x + _halfSize, _z); }
    else { newHeight = m_heightMap->GetData(_x, _z + _halfSize) + m_heightMap->GetData(_x, _z - _halfSize); }
    newHeight *= 0.5;
    break;
  case 2:
  {
  ...
  newHeight += GenerateNoise(_halfSize, newHeight);
...
void LandscapeTile::GenerateSquareMidpoint(int _x, int _z, int _halfSize)
{
  ...
  {
  case 0:
    newHeight = m_heightMap->GetData(_x - _halfSize, _z - _halfSize) + m_heightMap->GetData(_x - _halfSize, _z + _halfSize) + m_heightMap->
      GetData(_x + _halfSize, _z - _halfSize) + m_heightMap->GetData(_x + _halfSize, _z + _halfSize);
    newHeight *= 0.25;
    break;
  case 1:
    ...
    else { newHeight = m_heightMap->GetData(_x - _halfSize, _z + _halfSize) + m_heightMap->GetData(_x + _halfSize, _z - _halfSize); }
    newHeight *= 0.5;
    break;
  case 2:
  {
...
void LandscapeTile::Generate(LandscapeDef* _def)
{
  // Do we need to throw away our height map?
  {
    ...
    reqCells = pow(2, reqCells) + 1;
    if (!m_heightMap || reqCells != m_heightMap->GetNumColumns() || reqCells != m_heightMap->GetNumRows())
    {
      delete m_heightMap;
      m_heightMap = new SurfaceMap2D<double>(reqCells, reqCells, 0.0, 0.0, 1.0, 1.0, // Cell size of 1.0 is correct here
                                             m_outsideHeight);
    }
  }
  ...
  {
    double fracDimModifier = 30.7 * exp(-6.5 * m_fractalDimension);
    double smoothingModifier = 15.353 * exp(-3.1 * m_lowlandSmoothingFactor);
...
void LandscapeTile::LandscapeAppSeedRandom(unsigned int seed) { m_holdrand = static_cast<long>(seed); }

int LandscapeTile::LandscapeAppRandom() { return (((m_holdrand = m_holdrand * 214013L + 2531011L) >> 16) & 0x7fff); }

double LandscapeTile::Landscapefrand(double range)
{
  return range * (static_cast<double>(LandscapeAppRandom()) / static_cast<double>(APP_RAND_MAX));
}

double LandscapeTile::Landscapesfrand(double range)
{
  return (0.5 - static_cast<double>(LandscapeAppRandom()) / static_cast<double>(APP_RAND_MAX)) * range;
}
...
{
  unsigned short posX = static_cast<double>(_tile->m_posX / m_heightMap->m_cellSizeX) + 0.5;
  int posY = _tile->m_posY;
  unsigned short posZ = static_cast<double>(_tile->m_posZ / m_heightMap->m_cellSizeY) + 0.5;

  // Calculate num cells that this tile will occupy in the main landscape
  int numCells = static_cast<double>(_tile->m_size) / m_heightMap->m_cellSizeX;

  // Calculate how much to increment in "tile space" for one unit in "main landscape space"
  double factor = static_cast<double>(_tile->m_heightMap->GetNumColumns() - 1) / static_cast<double>(numCells - 1);
  ...
  _tile->m_heightMap->m_cellSizeX = _tile->m_size / static_cast<double>(_tile->m_heightMap->GetNumColumns());
  _tile->m_heightMap->m_cellSizeY = _tile->m_size / static_cast<double>(_tile->m_heightMap->GetNumRows());
  ...
  for (unsigned short z = 0; z < numCells; ++z)
  {
    ...
    double tileZ = m_heightMap->GetRealY(static_cast<double>(z + posZ));

    for (unsigned short x = 0; x < numCells; ++x)
    {
      //			double tileX = posX + (double)x * m_heightMap->m_cellSizeX;
      double tileX = m_heightMap->GetRealX(static_cast<double>(x + posX));
...
void Landscape::Init(LandscapeDef* _def, bool _justMakeTheHeightMap)
{
  ...
  if (!m_heightMap || m_heightMap->m_cellSizeX != _def->m_cellSize || m_heightMap->m_cellSizeY != _def->m_cellSize || m_worldSizeX != _def->
    m_worldSizeX || m_worldSizeZ != _def->m_worldSizeZ)
  {
    delete m_heightMap;
    delete m_normalMap;
    m_heightMap = new SurfaceMap2D<double>(_def->m_worldSizeX, _def->m_worldSizeZ, 0.0, 0.0, _def->m_cellSize, _def->m_cellSize,
                                           m_outsideHeight);
    m_normalMap = new SurfaceMap2D<Vector3>(_def->m_worldSizeX, _def->m_worldSizeZ, 0.0, 0.0, _def->m_cellSize, _def->m_cellSize,
                                            g_upVector);
  }
  ...
  DebugTrace("Generated Heightmap in %f seconds\n", GetHighResTime() - t);
  t = GetHighResTime();
  ...
  DebugTrace("Generated OpenGL setup in %f seconds\n", GetHighResTime() - t);
  t = GetHighResTime();
}
...
bool Landscape::IsInLandscape(const Vector3& _pos)
{
  return (_pos.x >= 0.0 && _pos.z >= 0.0 && _pos.x<GetWorldSizeX() && _pos.z<GetWorldSizeZ());
}
...
bool Landscape::RayHit(const Vector3& _rayStart, const Vector3& _rayDir, Vector3* _result) const
{
  if (!m_heightMap)
    return false;
...
bool Landscape::UnsafeRayHit(const Vector3& _rayStart, const Vector3& _rayDir, Vector3* _result) const
{
...
bool Landscape::RayHitCell(int x0, int z0, const Vector3& _rayStart, const Vector3& _rayDir, Vector3* _result) const
{
...
double Landscape::SphereHit(const Vector3& _centre, double _radius) const
{
  // Make sure the specified radius is +ve and not so large to cause
  // major efficiency problems
  DEBUG_ASSERT(_radius > 0.0 && _radius < 200.0);
