...
void LevelFile::WriteInstantUnits(TextWriter* _out)
{
  ...
  for (int i = 0; i < m_instantUnits.Size(); i++)
  {
    ...
    _out->printf("\t%-15s %2d %7.1f %7.1f %6d %4d %7d %7.1f %7.1f %7.1f %4d %4d %4d\n", Entity::GetTypeName(iu->m_type), iu->m_teamId,
                 ...
                 iu->m_routeId, iu->m_routeWaypointId, iu->m_runAsTask);
  }
...
void LevelFile::WriteLandFlattenAreas(TextWriter* _out)
{
  ...
  for (int i = 0; i < m_landscape.m_flattenAreas.Size(); ++i)
  {
    ...
    _out->printf("\t%6.1f %6.1f %6.1f %6.1f %6d %6.1f\n", area->m_centre.x, area->m_centre.y, area->m_centre.z, area->m_size,
                 area->m_flattenType, area->m_heightThreshold);
  }
...
void LevelFile::Save()
{
  ...
  {
#ifdef TARGET_DEBUG
    sprintf(fullFilename, "levels/%s", m_mapFilename); out = g_app->m_resource->GetTextFileWriter(fullFilename, false);
#else
...
void LevelFile::SaveMissionFile(const char* _filename)
{
...
void LevelFile::WriteLeaderboardStats(TextWriter* _out)
{
  ...
  DebugTrace("%sCompleted all primary objective (time %u)\n", completedAllPrimaryObjectives ? "" : "!", completedAllPrimaryObjectives
             ? static_cast<unsigned int>(g_app->m_location->m_levelFile->m_leaderboardLevelTime)
             : maxTime);

  unsigned int time = completedAllPrimaryObjectives ? g_app->m_location->m_levelFile->m_leaderboardLevelTime : maxTime;

  g_app->m_globalWorld->m_spLevelTimes[index] = min(time, g_app->m_globalWorld->m_spLevelTimes[index]);

  DebugTrace("LeaderboardStats_StartDefinition\n");
  DebugTrace("\tTime %u\n", g_app->m_globalWorld->m_spLevelTimes[index]);
  DebugTrace("\tKills %d\n", g_app->m_location->m_teams[2]->m_teamKills);
  DebugTrace("\tSurvivors %d\n", g_app->m_location->m_teams[0]->m_others.NumUsed());
  ...
  DebugTrace("LeaderboardStats_EndDefinition\n");
}
...
void LevelFile::WriteMultiwiniaOptions(TextWriter* _out)
{
  ...
  for (int i = 0; i < NumLevelEffects; ++i)
  {
    if (m_effects[i])
    {
      ...
      switch (i)
      {
      case LevelEffectSnow:
        _out->printf("snow\t");
        break;
      case LevelEffectDustStorm:
        _out->printf("dustwind\t");
        break;
      case LevelEffectSoulRain:
        _out->printf("soulrain\t");
        break;
      case LevelEffectLightning:
        _out->printf("lightning\t");
        break;
      case LevelEffectSpecialLighting:
        _out->printf("SpecialLighting\t");
        break;
      }
    }
  }
  ...
  for (int i = 0; i < NumLevelOptions; ++i)
  {
    if (m_levelOptions[i] > 0)
    {
      ...
      switch (i)
      {
      case LevelOptionNoHandicap:
        _out->printf("nohandicap\t");
        break;
      case LevelOptionInstantSpawnPointCapture:
        _out->printf("instantspawnpointcapture\t");
        break;
      case LevelOptionTrunkPortReinforcements:
        _out->printf("trunkportreinforcements %d\t", m_levelOptions[i]);
        break;
      case LevelOptionTrunkPortArmour:
        _out->printf("trunkportarmour %d\t", m_levelOptions[i]);
        break;
      case LevelOptionNoGunTurrets:
        _out->printf("nogunturrets\t");
        break;
      case LevelOptionAttackerReinforcements:
        _out->printf("attackerreinforcements %d\t", m_levelOptions[i]);
        break;
      case LevelOptionDefenderReinforcements:
        _out->printf("defenderreinforcements %d\t", m_levelOptions[i]);
        break;
      case LevelOptionDefenderReinforcementType:
        _out->printf("defenderreinforcementtype %s\t", GlobalResearch::GetTypeName(m_levelOptions[i]));
        break;
      case LevelOptionAttackerReinforcementType:
        _out->printf("attackerreinforcementtype %s\t", GlobalResearch::GetTypeName(m_levelOptions[i]));
        break;
      case LevelOptionForceRadarTeamMatch:
        _out->printf("forceradarteammatch\t");
        break;
      case LevelOptionNoRadarControl:
        _out->printf("noradarcontrol\t");
        break;
      case LevelOptionAirstrikeCaptureZones:
        _out->printf("airstrikecapturezones %d\t", m_levelOptions[i]);
        break;
      case LevelOptionFuturewinianTeam:
        _out->printf("futurewinianteam %d\t", m_levelOptions[i]);
      case LevelOptionAttackReinforcementMode:
        _out->printf("attackreinforcementoption %d\t", m_levelOptions[i]);
        break;
      case LevelOptionDefendReinforcementMode:
        _out->printf("defendreinforcementoption %d\t", m_levelOptions[i]);
        break;
      }
    }
  }
  ...
  if (m_defaultPlayTime != -1)
    _out->printf("\tPlayTime\t%d\n", m_defaultPlayTime);

  if (strcmp(m_author, "Author") != 0 && strlen(m_author) > 0)
    _out->printf("\tAuthor\t\t%s\n", m_author);
  if (strcmp(m_description, "LevelDescription") != 0 && strlen(m_description) > 0)
    _out->printf("\tDescription\t%s\n", m_description);
  if (m_difficultyRating)
    _out->printf("\tDifficulty\t%s\n", m_difficultyRating);
  ...
  if (m_coop)
    _out->printf("\tCoopMode\n");
  if (m_forceCoop)
    _out->printf("\tForceCoop\n");
