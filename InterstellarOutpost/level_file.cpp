...
void LevelFile::ParseCameraAnims(TextReader *_in)
{
	while (_in->ReadLine())
	{
		...
		while (_in->ReadLine())
		{
			...
			ASSERT_TEXT(node->m_duration >= 0.0 &&
    ...
    while( _in->ReadLine() )
    {
        ...
        else if( stricmp( "Effects", word ) == 0 )
        {
            while( _in->TokenAvailable() )
            {
                ...
                //m_effects.PutData( _strdup(_in->GetNextToken()) );
            }
        }
...
void LevelFile::ParseLandscapeTiles(TextReader *_in)
{
    while(_in->ReadLine())
	{
		...
		word = _in->GetNextToken();
        def->m_fractalDimension = (double)atof(word);

		word = _in->GetNextToken();
        def->m_heightScale = (double)atof(word);

		word = _in->GetNextToken();
        def->m_desiredHeight = (double)atof(word);
		...
		word = _in->GetNextToken();
		def->m_lowlandSmoothingFactor = (double)atof(word);
...
void LevelFile::ParseLandFlattenAreas(TextReader *_in)
{
	while(_in->ReadLine())
	{
        ...
        def->m_centre.x = (double)atof(word);

        word = _in->GetNextToken();
		def->m_centre.y = (double)atof(word);

        word = _in->GetNextToken();
        def->m_centre.z = (double)atof(word);
        
		word = _in->GetNextToken();
        def->m_size = (double)atof(word);
        ...
        if( _in->TokenAvailable() )
        {
            ...
            word = _in->GetNextToken();
            def->m_heightThreshold = (double)atof(word);
        }
        else
        {
