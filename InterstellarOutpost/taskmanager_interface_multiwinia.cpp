...
void TaskManagerInterfaceMultiwinia::Advance()
{
  ...
  if (tm && tm->m_mostRecentTaskId != -1)
  {
    if (tm->m_mostRecentTaskIdTimer > 0 && g_prefsManager->GetInt("HelpEnabled", 1))
    {
      double timePassed = GetHighResTime() - tm->m_mostRecentTaskIdTimer;
      if (timePassed > 0.2f && g_inputManager->controlEvent(ControlCloseTaskHelp))
      {
    ...
    {
      double timePassed = GetHighResTime() - abs(tm->m_mostRecentTaskIdTimer);
      if (timePassed >= 1.0f)
        tm->m_mostRecentTaskId = -1;
    }
  }
...
void TaskManagerInterfaceMultiwinia::RestoreRenderMatrices() { g_app->m_renderer->SetupMatricesFor2D(); }
...
void TaskManagerInterfaceMultiwinia::RenderCurrentCrateHelp()
{
  ...
  if (task && task->m_state == Task::StateStarted)
  {
    ...
    if (task->m_type == GlobalResearch::TypeGunTurret)
    {
      ...
      {
      case 0:
        fullCaption = LANGUAGEPHRASE("crate_tooltip_gunturret");
        break;
      case 1:
        fullCaption = LANGUAGEPHRASE("crate_tooltip_rocketturret");
        break;
      case 3:
        fullCaption = LANGUAGEPHRASE("crate_tooltip_flameturret");
        break;
      default:
        fullCaption = LANGUAGEPHRASE("crate_tooltip_gunturret");
        break;
...
bool TaskManagerInterfaceMultiwinia::ButtonHeld() { return GetHighResTime() - m_taskManagerDownTime > 0.5; }
...
void TaskManagerInterfaceMultiwinia::RenderScreenZoneHighlight(float x, float y, float w, float h)
{
  ...
  else
  {
    ...
    glVertex2f(x + h / 2, y + h);
    glTexCoord2f(0.0f, 1.0f);
    glVertex2f(x, y + h);

    glTexCoord2f(0.5f, 0.0f);
    glVertex2f(x + w - h / 2, y);
    glTexCoord2f(1.0f, 0.0f);
    glVertex2f(x + w, y);
    glTexCoord2f(1.0f, 1.0f);
    glVertex2f(x + w, y + h);
    glTexCoord2f(0.5f, 1.0f);
    glVertex2f(x + w - h / 2, y + h);
    glEnd();
    ...
    for (int i = 0; i < manager->m_recentTasks.Size(); ++i)
    {
      if (manager->m_recentTasks.ValidIndex(i))
      {
        ...
        if (task)
        {
          ...
          float timePassed = GetHighResTime() - abs(ts->m_timer);
          if (timePassed < 3)
            ts->m_rotation += g_advanceTime * 20.0f * pow(3 - timePassed, 4);
          ...
          if (!minimising)
          {
            ...
            if (task->m_type == GlobalResearch::TypeGunTurret)
            {
              ...
              {
              case 0:
                specialCaption = LANGUAGEPHRASE("crate_tooltip_gunturret");
                break;
              case 1:
                specialCaption = LANGUAGEPHRASE("crate_tooltip_rocketturret");
                break;
              case 3:
                specialCaption = LANGUAGEPHRASE("crate_tooltip_flameturret");
                break;
              default:
                specialCaption = LANGUAGEPHRASE("crate_tooltip_gunturret");
                break;
            ...
            {
            case 0:
              cratePowerupFound = LANGUAGEPHRASE("multiwinia_cratepowerupfound");
              break;
            case 1:
              cratePowerupFound = LANGUAGEPHRASE("multiwinia_reinforcementsgiven");
              break;
            case 2:
              cratePowerupFound = LANGUAGEPHRASE("multiwinia_retributiongiven");
              break;
            }
          ...
          glBegin(GL_QUADS);
          glTexCoord2i(0, 1);
          glVertex2f(iconCentre.x - colourSize / 2 + shadowOffset, iconCentre.y - colourSize / 2 + shadowOffset);
          glTexCoord2i(1, 1);
          glVertex2f(iconCentre.x + colourSize / 2 + shadowOffset, iconCentre.y - colourSize / 2 + shadowOffset);
          glTexCoord2i(1, 0);
          glVertex2f(iconCentre.x + colourSize / 2 + shadowOffset, iconCentre.y + colourSize / 2 + shadowOffset);
          ...
          if (!minimising && ts->m_source == 0)
          {
            ...
            glBegin(GL_QUADS);
            glTexCoord2i(0, 1);
            glVertex2dv((iconCentre + offset).GetData());
            offset.RotateAroundY(0.5f * M_PI);
            glTexCoord2i(1, 1);
            glVertex2dv((iconCentre + offset).GetData());
            offset.RotateAroundY(0.5f * M_PI);
            glTexCoord2i(1, 0);
...
void TaskManagerInterfaceMultiwinia::RenderRunningTasks()
{
  ...
  if (myTeam->m_taskManager->m_currentTaskId != -1)
  {
    int taskIndex = -1;
  ...
  if (g_app->m_location->GetMyTeam() && g_app->m_location->GetMyTeam()->m_taskManager->m_tasks.Size())
  {
    glEnable(GL_TEXTURE_2D);
    glEnable(GL_BLEND);
    glDepthMask(false);
    ...
    glBegin(GL_QUADS);
    glTexCoord2i(0, 1);
    glVertex2f(iconCentre.x - tabShadowSize, iconCentre.y - tabShadowSize);
    glTexCoord2i(1, 1);
    glVertex2f(iconCentre.x + tabShadowSize, iconCentre.y - tabShadowSize);
    glTexCoord2i(1, 0);
    glVertex2f(iconCentre.x + tabShadowSize, iconCentre.y + tabShadowSize);
    ...
    glBegin(GL_QUADS);
    glTexCoord2i(0, 1);
    glVertex2f(iconCentre.x - tabSize, iconCentre.y - tabSize);
    glTexCoord2i(1, 1);
    glVertex2f(iconCentre.x + tabSize, iconCentre.y - tabSize);
    glTexCoord2i(1, 0);
    glVertex2f(iconCentre.x + tabSize, iconCentre.y + tabSize);
  ...
  for (int i = 0; i < numTasks; ++i)
  {
    ...
    if (i == currentTaskId)
      shadowSize *= sizeChangeIfSelected;
    else
      shadowSize /= sizeChangeIfSelected;

    glBegin(GL_QUADS);
    glTexCoord2i(0, 1);
    glVertex2f(iconCentre.x - shadowSize / 2.2f + shadowOffset, iconCentre.y - shadowSize / 2.2f + shadowOffset);
    glTexCoord2i(1, 1);
    glVertex2f(iconCentre.x + shadowSize / 2.2f + shadowOffset, iconCentre.y - shadowSize / 2.2f + shadowOffset);
    glTexCoord2i(1, 0);
    glVertex2f(iconCentre.x + shadowSize / 2.2f + shadowOffset, iconCentre.y + shadowSize / 2.2f + shadowOffset);
  ...
  for (int i = 0; i < numTasks; ++i)
  {
    ...
    if (task && task->m_state == Task::StateStarted)
      thisAlpha = abs(sin(g_gameTime * 4.0f));
    if (thisAlpha < 0.5f)
      thisAlpha = 0.5f;
    ...
    glBegin(GL_QUADS);
    glTexCoord2i(0, 1);
    glVertex2f(iconCentre.x - shadowSize / 2.4f + shadowOffset, iconCentre.y - shadowSize / 2.4f + shadowOffset);
    glTexCoord2i(1, 1);
    glVertex2f(iconCentre.x + shadowSize / 2.4f + shadowOffset, iconCentre.y - shadowSize / 2.4f + shadowOffset);
    glTexCoord2i(1, 0);
    glVertex2f(iconCentre.x + shadowSize / 2.4f + shadowOffset, iconCentre.y + shadowSize / 2.4f + shadowOffset);
  ...
  for (int i = 0; i < numTasks; ++i)
  {
    ...
    if (i == currentTaskId)
      iconSize *= sizeChangeIfSelected;
    else
      iconSize /= sizeChangeIfSelected;

    if (i == currentTaskId)
      compassSize *= sizeChangeIfSelected;
    else
      compassSize /= sizeChangeIfSelected;
    ...
    if (task)
    {
      ...
      ScreenZone* zone = new ScreenZone("SelectTask", captionId, iconX - iconSize / 2, iconY - iconSize / 2, iconSize, iconSize, i);
    ...
    if (task && task->m_id == myTeam->m_taskManager->m_currentTaskId && task->m_state != Task::StateRunning)
    {
...
void TaskManagerInterfaceMultiwinia::RenderExtraTaskInfo(Task* task, Vector2 iconCentre, float iconSize, float alpha, bool targetOrders,
                                                         bool taskManager)
{
  //glColor4f( 1.0f, 1.0f, 1.0f, alpha );
  //glBegin( GL_LINE_LOOP );
  ...
  if (task)
  {
    ...
    {
      Harvester* harvester = static_cast<Harvester*>(g_app->m_location->GetEntitySafe(task->m_objId, Entity::TypeHarvester));
      if (harvester)
      {
    ...
    else if (task->m_type == GlobalResearch::TypeBooster || task->m_type == GlobalResearch::TypeHotFeet || task->m_type ==
      GlobalResearch::TypeSubversion || task->m_type == GlobalResearch::TypeRage || (task->m_type == GlobalResearch::TypeTank && g_app->
        m_multiwinia->m_gameType != Multiwinia::GameTypeTankBattle))
    {
      if (task->m_state == Task::StateRunning)
      {
        if (task->m_lifeTimer > 0.0f)
    ...
    else if (task->m_type == GlobalResearch::TypeArmour)
    {
      ...
      if (armour)
      {
        ...
        if (targetOrders)
          currentState = armour->m_conversionOrder;
        ...
        if (draw)
        {
          ...
          glBegin(GL_QUADS);
          glTexCoord2i(0, 1);
          glVertex2f(iconCentre.x - iconSize / 2.2f, iconCentre.y - iconSize / 2.2f);
          glTexCoord2i(1, 1);
          glVertex2f(iconCentre.x + iconSize / 2.2f, iconCentre.y - iconSize / 2.2f);
          glTexCoord2i(1, 0);
          glVertex2f(iconCentre.x + iconSize / 2.2f, iconCentre.y + iconSize / 2.2f);
...
void TaskManagerInterfaceMultiwinia::RenderCompass(float _screenX, float _screenY, const Vector3& _worldPos, bool _selected, float _size)
{
