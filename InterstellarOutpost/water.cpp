...
constexpr float waveBrightnessScale = 4.0;
constexpr float shoreBrighteningFactor = 250.0;
constexpr float shoreNoiseFactor = 0.25;
#ifdef USE_DIRECT3D
...
#ifdef USE_DIRECT3D
    m_renderVertexBuffer(0), m_prepareVertexBuffer(0), m_updateWaterThread(NULL),
#endif
    m_colourTable(NULL)
#ifdef TARGET_OS_MACOSX
	,m_updateCount(0), m_lastUpdateTime(0.0)
#endif
{

#ifdef USE_DIRECT3D
  memset(m_vertexBuffer, 0, sizeof(m_vertexBuffer)); memset(m_vertexBufferReady, 0, sizeof(m_vertexBufferReady));
#endif
    ...
    if (detail > 0)
    {
      ...
      if (m_renderVerts.Size())
      {
        ...
        for (unsigned i = 0; i < strip->m_numVerts - 2; i++)
        {
          ...
          trilist.PutData(tmp);
          if ((Vector3(m_renderVerts[i].m_pos) - Vector3(m_renderVerts[i + 1].m_pos)).Mag() + (Vector3(m_renderVerts[i].m_pos) -
              Vector3(m_renderVerts[i + 2].m_pos)).Mag() + (Vector3(m_renderVerts[i + 1].m_pos) - Vector3(m_renderVerts[i + 2].m_pos)).Mag()
            >
            20)
          {
          ...
          {
            // standard triangle
            tmp.m_pos = m_renderVerts[i + 1 + (i % 2)].m_pos;
            trilist.PutData(tmp);
            tmp.m_pos = m_renderVerts[i + 2 - (i % 2)].m_pos;
            trilist.PutData(tmp);
          }
        }
...
Water::~Water()
{
  ...
  {
    DebugTrace("Killing water thread: %p\n", m_updateWaterThread);
...
#define MASK_SIZE 128
  ...
  float scaleFactorX = 2.0 * landSizeX / static_cast<float>(MASK_SIZE);
  float scaleFactorZ = 2.0 * landSizeZ / static_cast<float>(MASK_SIZE);
  int low = MASK_SIZE / 4;
  ...
  for (int z = low; z < high; ++z)
  {
    ...
    {
      float landHeight = g_app->m_location->m_landscape.m_heightMap->GetValue((0.0 + static_cast<float>(x)) * scaleFactorX - offX,
                                                                              (0.0 + static_cast<float>(z)) * scaleFactorZ - offZ);
      if (landHeight > 0.0)
  ...
  for (int x = 0; x < MASK_SIZE; ++x)
  {
    for (int z = 0; z < MASK_SIZE; ++z)
    {
      float grayVal = static_cast<float>(landData.GetData(x, z)) * 855.0;
      if (grayVal > 255.0)
        grayVal = 255.0;
  ...
  float depthMapCellSize = (landSizeX * 2.0) / static_cast<float>(finalImage.m_height);

  if (m_waterDepthMap)
    delete m_waterDepthMap;
  m_waterDepthMap = new SurfaceMap2D<float>(landSizeX * 2.0, landSizeZ * 2.0, -landSizeX / 2.0, -landSizeZ / 2.0, depthMapCellSize,
                                            depthMapCellSize, 1.0);
  //if (!g_app->m_editing)
  {
    for (int z = 0; z < finalImage.m_height; ++z)
    {
      for (int x = 0; x < finalImage.m_width; ++x)
      {
        ...
        float depth = static_cast<float>(pixel.g) / 255.0;
        depth = 1.0 - depth;
...
{
#ifdef USE_DIRECT3D
  NetLockMutex l(m_vertexBufferLock); for (int i = 0; i < NumVertexBuffers; i++)
  {
...
void Water::BuildTriangleStrips()
{
  ...
  const float lowX = -landSizeX * 0.5;
  const float lowZ = -landSizeZ * 0.5;
  const float highX = landSizeX * 1.5;
  const float highZ = landSizeZ * 1.5;
  ...
  {
    const Vector3& pos = m_renderVerts[i].m_pos;
...
{
  static D3DVERTEXELEMENT9 s_vertexDesc[] = {{0, 0, D3DDECLTYPE_FLOAT3, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_POSITION, 0},
                                             {0, 12, D3DDECLTYPE_D3DCOLOR, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_COLOR, 0},
                                             {0, 16, D3DDECLTYPE_FLOAT3, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_NORMAL, 0},
                                             {0xFF, 0, D3DDECLTYPE_UNUSED, 0, 0, 0} // D3DDECL_END 
  };

  if (!s_vertexDecl) { OpenGLD3D::g_pd3dDevice->CreateVertexDeclaration(s_vertexDesc, &s_vertexDecl); }

  return s_vertexDecl;
};void Water::ReleaseD3DPoolDefaultResources() { BuildOpenGlState(); }void Water::ReleaseD3DResources()
{
...
void Water::RenderFlatWater()
{
  ...
  const float timeFactor = g_gameTimer.GetAccurateTime() / 30.0;

#ifdef USE_DIRECT3D
  // sets default vertex format (somebody changes it and not returns back, where?)
  extern LPDIRECT3DVERTEXDECLARATION9 s_pCustomVertexDecl; OpenGLD3D::g_pd3dDevice->SetVertexDeclaration(s_pCustomVertexDecl);
#endif

  RenderFlatWaterTiles(landSizeZ + borderZ, -borderZ, -borderX, landSizeX + borderX, -9.0, timeFactor, timeFactor + 30.0, timeFactor,
                       timeFactor + 30.0, 0.0, 1.0, 0.0, 1.0, m_flatWaterTiles->GetNumColumns());
...
void Water::UpdateDynamicWater()
{
  ...
  constexpr float scaleFactor = 7.0;
  ...
  if ((timeNow - m_lastUpdateTime) < 1.0f / 15.0f)
    return; m_lastUpdateTime = timeNow;
#endif
  ...
  for (int i = 0; i < m_waveTableSizeX; ++i)
  {
    ...
    float heightForX = sin(x * 0.01f + timeNow * 0.65f) * 1.2f;
    heightForX += sin(x * 0.03f + timeNow * 1.5f) * 0.9f;
    m_waveTableX[i] = heightForX * scaleFactor;
  }
  for (int i = 0; i < m_waveTableSizeZ; ++i)
  {
    float z = static_cast<float>(i) * m_cellSize;
    float heightForZ = sin(z * 0.02f + timeNow * 0.75f) * 0.9f;
    heightForZ += sin(z * 0.03f + timeNow * 1.85f) * 0.65f;
...
#ifdef USE_DIRECT3D_TRILIST
  int totalNumVertices = 0; for (int i = 0; i < m_strips.Size(); ++i)
  {
  ...
  for (int i = 0; i < m_strips.Size(); ++i)
  {
    ...
    for (int j = strip->m_startRenderVertIndex; j < finalVertIndex; ++j)
    {
      ...
      const double landSizeX = g_app->m_location->m_landscape.GetWorldSizeX();
      const double landSizeZ = g_app->m_location->m_landscape.GetWorldSizeZ();
      const double lowX = -landSizeX * 0.5;
      const double lowZ = -landSizeZ * 0.5;
      int indexX = static_cast<int>((vertex1->m_pos.x - (float)lowX) / m_cellSize + 0.1f);
      int indexZ = static_cast<int>((vertex1->m_pos.z - (float)lowZ) / m_cellSize + 0.1f);
      DEBUG_ASSERT(indexX < m_waveTableSizeX);
      DEBUG_ASSERT(indexZ + 1 < m_waveTableSizeZ);
  ...
  if (m_renderVerts.Size() > 0)
  {
    ...
    if (buffer && !m_vertexBufferReady[m_prepareVertexBuffer] &&
      ...
      !m_vertexBufferReady[next] && !m_vertexBufferReady[nnext])
    {
...
{
#ifdef USE_DIRECT3D
  NetLockMutex l(m_vertexBufferLock); int nextVertexBuffer = (m_renderVertexBuffer + 1) % NumVertexBuffers; if (m_vertexBuffer[
    nextVertexBuffer] && m_vertexBufferReady[nextVertexBuffer])
  {
...
#ifdef USE_DIRECT3D
  IDirect3DVertexDeclaration9* savedDecl; OpenGLD3D::g_pd3dDevice->GetVertexDeclaration(&savedDecl); OpenGLD3D::g_pd3dDevice->
    SetVertexDeclaration(GetVertexDeclWater()); OpenGLD3D::g_pd3dDevice->SetStreamSource(
    0, m_vertexBuffer[m_renderVertexBuffer], 0, sizeof(WaterVertex));
#else
  glEnableClientState(GL_VERTEX_ARRAY);
  glEnableClientState(GL_COLOR_ARRAY);

  ASSERT_TEXT(m_renderVerts.ValidIndex(0), "RenderDynamicWater: Invalid index 0 in DArray\n");
...
#ifdef USE_DIRECT3D
  if (m_renderWaterEffect && g_waterReflectionEffect) { g_waterReflectionEffect->Start(); }
#endif

#ifdef USE_DIRECT3D_TRILIST
  if (m_renderVerts.Size())
  {
    if (g_fixedPipeline&& OpenGLD3D::g_pd3dDevice
    ==
    OpenGLD3D::g_pd3dDeviceActual
    )
    {
...
#ifdef USE_DIRECT3D
  if (m_renderWaterEffect && g_waterReflectionEffect) { g_waterReflectionEffect->Stop(); }
#endif
...
void Water::Render()
{
  ...
  if (g_app->m_editing)
  {
    ...
    glEnable(GL_TEXTURE_2D);
    glBindTexture(GL_TEXTURE_2D, g_app->m_resource->GetTexture("textures\\triangleOutline.bmp", true, false));
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
    ...
    glVertex3f(0, -10.0, land->GetWorldSizeZ());
    glTexCoord2f(size, size);
    glVertex3f(land->GetWorldSizeX(), -10.0, land->GetWorldSizeZ());
    glTexCoord2f(0.0, size);
    glVertex3f(land->GetWorldSizeX(), -10.0, 0);
    glEnd();
    glDisable(GL_TEXTURE_2D);
...
{
  if ( //!g_app->m_editing && 
    RENDER_WATER_DETAIL > 0 && m_renderVerts.Size() > 0
#ifdef USE_DIRECT3D
  ...
  for (int i = 0; i < NumVertexBuffers; i++)
  {
    if (m_vertexBuffer[i] == NULL)
    {
      HRESULT hr = OpenGLD3D::g_pd3dDevice->CreateVertexBuffer(
        bufferSize, D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY | (OpenGLD3D::g_supportsHwVertexProcessing ? 0 : D3DUSAGE_SOFTWAREPROCESSING), 0,
        D3DPOOL_DEFAULT, &m_vertexBuffer[i], NULL);
