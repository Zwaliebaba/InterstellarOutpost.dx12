...
void Camera::AdvanceSphereWorldScriptedMode()
{
  ...
  auto focusPos = Vector3(0, m_height * -400, 0);
  focusPos.x += sin(g_gameTime * 0.5f) * 4000.0f;
  focusPos.y += cos(g_gameTime * 0.4f) * 2000.0f;
  focusPos.z += sin(g_gameTime * 0.3f) * 4000.0f;
...
void Camera::AdvanceSphereWorldIntroMode()
{
...
#ifdef _DEBUG
  if (g_inputManager->controlEvent(ControlDebugCameraFixUp)) { fixMeUp = true; }
#endif
  ...
  float factor2 = 1.0f - factor1;
  m_front = m_front * factor2 + targetFront * factor1;
  m_up.RotateAround(m_front * frameTime * rotateSpeed * sin(runningTime * 0.1f + 0.4f) * 1.6f);
...
void Camera::AdvanceSphereWorldOutroMode()
{
  ...
  float forwardSpeed = sqrt(m_pos.Mag()) * 4;
  ...
  m_up.RotateAround(m_front * frameTime * rotateSpeed * sin(runningTime * 0.1f + 0.4f) * 0.6f);
...
void Camera::AdvanceSphereWorldFocusMode()
{
  ...
  m_trackVector.Normalise();
  float height = sin(g_gameTime * 0.2f) * m_trackHeight;
  float trackRange = abs(m_trackRange) + sin(g_gameTime * 0.3f) * fabs(m_trackRange) * 0.4f;
...
void Camera::AdvanceFreeMovementMode()
{
  ...
  if (!g_app->m_taskManagerInterface->IsVisible() && !g_app->m_shamanInterface->IsVisible())
  {
    ...
    if (keyLeft)
      m_targetPos += accelRight * advanceTime * moveRate;
    if (keyRight)
      m_targetPos -= accelRight * advanceTime * moveRate;
    if (keyForward)
      m_targetPos += accelForward * advanceTime * moveRate;
    if (keyBackward)
      m_targetPos -= accelForward * advanceTime * moveRate;
    ...
    if (keyForward && !keyLeft && !keyRight && !keyBackward && !g_inputManager->controlEvent(ControlCameraSpeedup))
    {
      double timeNow = GetHighResTime();
    ...
    if (!disableMouseControl && !g_app->m_taskManagerInterface->IsVisible() && !chatLog)
    {
      if (mousePos3D.MagSquared() > 1.0f)
      {
        Vector3 desiredFront = mousePos3D - m_pos;
        ...
        float amountToRotate = sqrt(advanceTime) * 1.0f;
...
void Camera::AdvanceBuildingFocusMode()
{
  ...
  m_trackVector.RotateAroundY(g_advanceTime * -0.15f);
  m_trackVector.Normalise();
  float height = m_trackHeight + sin(g_gameTime * 0.3f) * m_trackHeight * 0.5f;
  float trackRange = abs(m_trackRange) + sin(g_gameTime * 0.3f) * fabs(m_trackRange) * 0.3f;
  Vector3 realTargetPos = m_targetPos + m_trackVector * trackRange;
...
bool Camera::GetEntityToTrack(WorldObjectId& selection)
{
  ...
  {
    auto squad = (InsertionSquad*)unit;
...
void Camera::AdvanceAutomaticTracking()
{
  if (!m_resettingCameraPos)
    m_resettingCameraPos = g_inputManager->controlEvent(ControlCameraZoom);
  ...
  */
  ...
  if (m_resettingCameraPos && m_pos.x - m_wantedPos.x < 5.0f && m_pos.y - m_wantedPos.y < 5.0f && m_pos.z - m_wantedPos.z < 5.0f)
  {
    DebugTrace("Stopping camera reset\n");
...
void Camera::UpdateControlVector()
{
  ...
  float angle = cos(g_upVector * GetUp());
  if ((abs(angle) < angleThreshold && !m_skipDirectionCalculation) || !g_inputManager->controlEvent(ControlUnitMove) || recalc)
  {
    m_controlVector = GetRight();
    m_skipDirectionCalculation = false;
  }
  else if (abs(angle) >= angleThreshold)
...
bool Camera::TargetEntity(short _x, short _y)
{
  ...
  if (!targetted)
  {
    ...
    g_app->m_location->m_entityGrid->GetEnemies(s_neighbours, m_trackingEntity->m_pos.x, m_trackingEntity->m_pos.z, range, &numFound,
    ...
    for (int i = 0; i < numFound; i++)
    {
      ...
      if (closestE)
      {
        int x1 = xPos - closestE->m_pos.x, z1 = zPos - closestE->m_pos.z, x2 = xPos - e->m_pos.x, z2 = zPos - e->m_pos.z;
...
bool AimingBuilding()
{
  ...
  if (!g_app || !g_app->m_location || !(team = g_app->m_location->GetMyTeam()))
    return false;
...
bool FiddlingWithOfficer()
{
  ...
  */
  if (!g_app->m_location || !g_app->m_globalWorld || g_inputManager->getInputMode() != INPUT_MODE_GAMEPAD)
    retVal = false;
  else
  {
  ...
  if (firstTime)
  {
    DebugTrace("Return value of fiddlingwithofficer is now %s\n", retVal ? "true" : "false");
    firstTime = false;
  }
...
{
  if (g_app->m_location->GetMyTeam() && g_app->m_location->GetMyTeam()->GetMyEntity())
    return false;
...
{
  if (m_trackingEntity->m_type != Entity::TypeInsertionSquadie)
    return false;
  ...
  Squadie* squadie = static_cast<Squadie*>(m_trackingEntity);
...
void Camera::RotateTowardsEntity(Entity* entity)
{
  ...
  w = abs(w - screenPosX) / w;
  h = abs(h - screenPosY) / h;
...
{
  Entity* obj = g_app->m_location->GetEntity(m_objectId);
  if (!obj)
  {
    RequestMode(ModeFreeMovement);
    return;
...
Camera::Camera()
  : m_targetPosOffset(0.0f),
    m_targettingFOVAngle(0.939692621f),
    m_targettingEntity(NULL),
    m_trackingEntity(NULL),
...
bool Camera::SphereInViewFrustum(const Vector3& _centre, float _radius)
{
  ...
  float angularSize = asin(_radius / distance);
  float angle = acos(dirToPos.Normalise() * m_front);
...
void Camera::AdvanceComponentZoom()
{
  ...
  if (g_app->m_taskManagerInterface->IsVisible() || IsInMode(ModeEntityTrack))
    return;
...
void Camera::AdvanceComponentMouseWheelHeight()
{
  ...
  if (g_app->m_location)
  {
    ...
    m_height += delta * 2.0f * sqrt(fabsf(altitude));
...
void Camera::Advance()
{
  ...
  if (!m_camLookAround && m_mode != ModeEntityTrack)
  ...
  if (m_anim == NULL && (m_debugMode == DebugModeAlways || (m_debugMode == DebugModeAuto && EclGetWindows()->Size() > 0) ||
...
void Camera::RequestRadarAimMode(Building* _building)
{
  ...
  {
    RadarDish* radar = static_cast<RadarDish*>(_building);
...
bool Camera::ChatLogVisible()
{
  ...
  if (g_prefsManager->GetInt("ControlMethod") == 1 && g_inputManager->controlEvent(ControlIconsChatLog))
    return true;

  if (g_prefsManager->GetInt("ControlMethod") == 0 && g_inputManager->controlEvent(ControlGesturesChatLog))
    return true;
