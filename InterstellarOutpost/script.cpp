...
{
#ifdef USE_SEPULVEDA_HELP_TUTORIAL
  g_app->m_sepulveda->DemoGesture(_name, 2.0);
#endif
}
...
void Script::RunCommand_GiveResearch(const char* _name)
{
  ...
  else if (stricmp(_name, "accessallareas") == 0)
  {
    ...
    g_app->m_taskManagerInterface->SetCurrentMessage(TaskManagerInterface::MessageResearch, 998, 4.0);
  }
  else
  {
    ...
    if (researchType != -1)
    {
      ...
      g_app->m_taskManagerInterface->SetCurrentMessage(TaskManagerInterface::MessageResearch, researchType, 4.0);
    }
  }
}
...
void Script::Advance()
{
  if (g_inputManager->controlEvent(ControlSkipCutscene))
  {
    ...
    m_waitForRocket = false;
    m_waitForPlayerNotBusy = false;
    g_app->m_renderer->m_renderDarwinLogo = -1.0;
  ...
  if (m_waitUntil > g_gameTime)
    return;
#ifdef USE_SEPULVEDA_HELP_TUTORIAL
  if (m_waitForSpeech && g_app->m_sepulveda->IsTalking())
    return;
#endif
  if (m_waitForCamera && g_app->m_camera->IsAnimPlaying())
    return;
#ifdef USE_SEPULVEDA_HELP_TUTORIAL
  if (m_waitForPlayerNotBusy && Demo2Tutorial::IsPlayerBusy())
    return;
#endif
...
void Script::TestScript(char* _filename)
{
  ...
  while (m_in->ReadLine())
  {
    ...
    case OpCamMove:
    {
      ...
      float duration = atof(m_in->GetNextToken());
      if (duration < 0.01 || duration > 20.0)
      {
    ...
    case OpSay:
      ...
      break;
    case OpWait:
      if (nextFloat < 0.01 || nextFloat > 20.0)
      {
...
static char* g_opCodeNames[] = {"CamCut", "CamMove", "CamAnim", "CamFov", "CamBuildingFocus", "CamBuildingApproach", "CamLocationFocus",
                                "CamGlobalWorldFocus", "CamReset", "EnterLocation", "ExitLocation", "Say", "ShutUp", "Wait", "WaitSay",
                                "WaitCam", "WaitFade", "WaitRocket", "WaitPlayerNotBusy", "Highlight", "ClearHighlights", "TriggerSound",
                                "StopSound", "DemoGesture", "GiveResearch", "SetMission", "GameOver", "ResetResearch", "RestoreResearch",
                                "RunCredits", "SetCutsceneMode", "GodDishActivate", "GodDishDeactivate", "GodDishSpawnSpam",
                                "GodDishSpawnResearch", "TriggerSpam", "PurityControl", "ShowDarwinLogo", "ShowDemoEndSequence",
                                "PermitEscape", "DestroyBuilding", "ActivateTrunkPort", "ActivateTrunkPortFull"};
