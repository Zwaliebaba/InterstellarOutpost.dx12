...
class MarkerSystem
{
  protected:
    ...
    void FindScreenEdge(const Vector2& line, float* posX, float* posY);
    void CalculateMarkerPosition(float screenX, float screenY, float dx, float dy, float& posX, float& posY);
  ...
  public:
    ...
    void RenderColourCircle(float screenX, float screenY, float screenDX, float screenDY, float size, RGBAColour colour,
                            float percent = 1.0);
    void RenderIcon(float screenX, float screenY, float screenDX, float screenDY, float size, char* iconFilename, RGBAColour colour,
                    float rotation = 0.0f);

    void CalculateScreenBox(float screenX, float screenY, float screenDX, float screenDY, float size, float& minX, float& minY, float& maxX,
                            float& maxY);
    bool IsMOuseInside(float screenX, float screenY, float screenDX, float screenDY, float size);

    bool CalculateScreenPosition(const Vector3& worldPos, float& screenX, float& screenY); // returns true if onscreen
    ...
    void RegisterMarker_Fixed(int teamId, const Vector3& pos, char* icon, bool fromCrate = true);
