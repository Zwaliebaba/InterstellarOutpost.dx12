...
bool GameTimer::IsPaused() { return (m_gameSpeed < 0.01f); }
...
void GameTimer::Tick()
{
  ...
  if (m_requestedSpeed >= 0.0)
  {
    ...
    if (abs(m_gameSpeed - m_requestedSpeed) < 0.01)
    {
...
double GameTimer::GetGameSpeed() { return m_gameSpeed; }

double GameTimer::GetRequestedGameSpeed() { return m_requestedSpeed; }
...
double GameTimer::GetServerAdvancePeriod() { return 0.1 * m_gameSpeed; }
...
void GameTimer::DebugRender()
{
  ...
  if (abs(m_gameSpeed - 1.0f) > 0.01f)
    g_gameFont.DrawText2D(10, 200, 20, "Game Speed : %2.2f", m_gameSpeed);
}

void GameTimer::LockSpeedChange() { m_lockSpeedChange = true; }
