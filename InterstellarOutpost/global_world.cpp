...
GlobalBuilding::GlobalBuilding()
  : m_id(-1),
    ...
    m_online(false),
    m_link(-1),
    m_shape(NULL) { m_shape = g_app->m_resource->GetShape("trunkport.shp"); }
...
{
  char* names[] = {"Darwinian", "Officer", "Squad", "Laser", "Grenade", "Rocket", "Controller", "AirStrike", "Armour", "TaskManager",
                   "Engineer", "Harvester", "Nuke", "Subversion", "PersonalShield", "HotFeet", "GunTurret", "Shaman", "Infection",
                   "MagicalForest", "DarkForest", "AntNest", "Plague", "Egg", "MeteorShower", "FireExtinguisher", "Rage", "Tank",
                   "DropShip"};
  ...
  for (int locationId = 0; locationId < m_numLocations; ++locationId)
  {
    ...
    if (location)
    {
      for (int i = 0; i < m_spirits[locationId].Size(); ++i)
      {
        ...
        position.x += sin(*thisSpirit * 14 + timeOffset) * posOffset;
        position.y += sin(*thisSpirit * 15 + timeOffset) * posOffset;
        position.z += sin(*thisSpirit * 16 + timeOffset) * posOffset;

        double scale = 0.4;

        glColor4f(0.6, 0.2, 0.1, alphaValue);
        glTexCoord2f(0.5, 0.5);
        glVertex3dv((position + camUp * 300 * scale).GetData());
        glTexCoord2f(0.5, 0.5);
        glVertex3dv((position + camRight * 300 * scale).GetData());
        glTexCoord2f(0.5, 0.5);
        glVertex3dv((position - camUp * 300 * scale).GetData());
        glTexCoord2f(0.5, 0.5);
        glVertex3dv((position - camRight * 300 * scale).GetData());
        ...
        glColor4f(0.6, 0.2, 0.1, alphaValue);
        glTexCoord2f(0.5, 0.5);
        glVertex3dv((position + camUp * 100 * scale).GetData());
        glTexCoord2f(0.5, 0.5);
        glVertex3dv((position + camRight * 100 * scale).GetData());
        glTexCoord2f(0.5, 0.5);
        glVertex3dv((position - camUp * 100 * scale).GetData());
        glTexCoord2f(0.5, 0.5);
        glVertex3dv((position - camRight * 100 * scale).GetData());
        ...
        glTexCoord2i(0, 0);
        glVertex3dv((position + camUp * 6000 * scale).GetData());
        glTexCoord2i(1, 0);
        glVertex3dv((position + camRight * 6000 * scale).GetData());
        glTexCoord2i(1, 1);
        glVertex3dv((position - camUp * 6000 * scale).GetData());
        glTexCoord2i(0, 1);
        glVertex3dv((position - camRight * 6000 * scale).GetData());
...
void SphereWorld::RenderHeaven()
{
  ...
  for (int i = 0; i < 50; ++i)
  {
    Vector3 pos(sin(i / g_gameTime + i) * 20, sin(g_gameTime + i) * i, cos(i / g_gameTime + i) * 20);
    ...
    {        
      GlobalLocation *loc = g_app->m_globalWorld->m_locations.GetData(i);
          ...
          for( int j = 0; j < 6; ++j )
          {
              ...
              godRayPos.x += sin( g_gameTime + i + j/2 ) * 1000;
              godRayPos.y += sin( g_gameTime + i + j/2 ) * 1000;
              godRayPos.z += cos( g_gameTime + i + j/2 ) * 1000;
...
void SphereWorld::RenderIslands()
{
  ...
  for (int i = 0; i < g_app->m_globalWorld->m_locations.Size(); ++i)
  {
    ...
    if (loc->m_available)
    {
      ...
      int numRedraws = 5;
      if (!loc->m_missionCompleted && stricmp(loc->m_missionFilename, "null") != 0 && fmodf(g_gameTime, 1.0) < 0.7)
        numRedraws = 10;

      for (int j = 0; j <= numRedraws; ++j)
      {
        glTexCoord2i(0, 0);
        glVertex3dv((islandPos + camUp * 1000 * j).GetData());
        glTexCoord2i(1, 0);
        glVertex3dv((islandPos + camRight * 1000 * j).GetData());
        glTexCoord2i(1, 1);
        glVertex3dv((islandPos - camUp * 1000 * j).GetData());
        glTexCoord2i(0, 1);
        glVertex3dv((islandPos - camRight * 1000 * j).GetData());
  ...
  for (int i = 0; i < g_app->m_globalWorld->m_locations.Size(); ++i)
  {
    ...
    if (loc->m_available)
    {
      ...
      double size = 5.0 * sqrt((g_app->m_camera->GetPos() - islandPos).Mag());
      size = 1000.0;
...
int GlobalWorld::LocationHit(const Vector3& _pos, const Vector3& _dir, double locationRadius)
{
...
{
  DebugTrace("GlobalWorld::LoadGame %s\n", _filename);
  ...
  DebugTrace("game.txt successfully loaded\n");
  ...
  DebugTrace("locations.txt successfully loaded\n");
  ...
  for (int i = 0; i < m_locations.Size(); i++)
  {
    ...
    std::vector<std::string> missionFileNames = g_app->m_resource->ListResources("levels\\", filter, false);
    for (int j = 0; j < missionFileNames.size(); ++j)
    {
      LevelFile levFile(missionFileNames[j].c_str(), loc->m_mapFilename);
...
void GlobalWorld::ParseLevelTimes(TextReader* _in)
{
  ...
  while (_in->ReadLine())
  {
    ...
    DebugTrace("indexWord is %s, timeWord is %s\n", word, timeWord);
  ...
  DebugTrace("Done parsing level times\n");
}

void GlobalWorld::ParseSquaddiesUsed(TextReader* _in)
{
  DebugTrace("Parsing squadies used\n");
  while (_in->ReadLine())
  {
    if (!_in->TokenAvailable())
      continue;
    ...
    DebugTrace("squaddies used indexWord is %s, numWord is %s\n", word, numWord);
...
void GlobalWorld::LoadLocations(const char* _filename)
{
  ...
  while (in->ReadLine())
  {
    ...
    double posX = atof(in->GetNextToken());
    double posY = atof(in->GetNextToken());
    double posZ = atof(in->GetNextToken());
