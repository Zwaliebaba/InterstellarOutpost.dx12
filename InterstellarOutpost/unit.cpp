...
:   m_troopType(troopType),
    ...
    m_accumulatedRadiusSquared(0.0),
    m_numAccumulated(0),
    m_wayPoint(0,0,0),
	m_routeId(-1),
	m_routeWayPointId(-1),
    m_targetDir(1,0,0),
    m_attackAccumulator(0.0),
    m_aiTimer(0.0)
{
...
// with the EntityGrid.
void Unit::RemoveEntity( int _index, double _posX, double _posZ )
{         
    ...
    for (int i = startIndex; i <= endIndex; i++)
    {
        if (m_entities.ValidIndex(i))
        {
            ...
            if( s->m_enabled )
            {
                ...
                START_PROFILE( Entity::GetTypeName( s->m_type ) );
                SyncRandLogText( s->LogState( "UNIT ENTITY BEFORE" ) );
                bool amIdead = s->Advance( this );
                SyncRandLogText( s->LogState( "UNIT ENTITY AFTER" ) );
#ifdef TRACK_SYNC_RAND
				bool WatchEntities();
				WatchEntities();
#endif
                END_PROFILE(  Entity::GetTypeName( s->m_type ) );
...
void Unit::Render( double _predictionTime )
{
	...
	for (int i = lastUpdated + 1; i < size; i++)
	{
        ...
        {
            Entity *entity = m_entities[i];
            if( entity ) entity->Render(_predictionTime);
        }
	}
...
bool Unit::Advance( int _slice )
{
    ...
    m_radius = sqrt( m_accumulatedRadiusSquared );
    ...
    if( m_troopType == Entity::TypeLaserTroop )
    {
        for (int i = 0; i < m_entities.Size(); i++)
        {
            if (m_entities.ValidIndex(i))
            {
                ...
                if( (l->m_pos - l->m_targetPos).Mag() < leadDistance / 5.0 )
                {                
                    Vector3 pos = l->m_pos;
...
//                    l->m_unitTargetPos = targetPos;
                    ...
                    double distance = (targetPos - pos).Mag();
                    double amountToMove = leadDistance;
...
void Unit::Attack( Vector3 pos, bool _withGrenade )
{
    ...
    if( canAttack.Size() > 0 )
    {
        ...
        double timeToWait = (double) reloadTime / (double) NumEntities();
...
void Unit::UpdateEntityPosition( Vector3 pos, double _radius )
{
    m_accumulatedCentre += pos;
    ++m_numAccumulated;

    double distanceFromCentre = (pos - m_centrePos).Mag();
    distanceFromCentre += _radius;
    double distanceFromCentreSquared = distanceFromCentre * distanceFromCentre;
...
{
    //static double *s_offsets = NULL;
    int const numOffsets = 100;
    double const spacedOut = 4.0;
    ...
    if( !s_offsets )
    {
        s_offsets = new double[numOffsets];
        for( int i = 0; i < numOffsets; i++ )
        {
            s_offsets[i] = syncfrand(spacedOut/4.0);
        }
    }
    ...
    switch(_formation)
    {
        ...
        {
            int rowLen = sqrt( NumEntities() );
            
            double x = _index % rowLen;
            x -= rowLen/2.0;
            x *= spacedOut;

            double z = _index / rowLen;
			z -= rowLen/2.0;
			z *= spacedOut;
