...
void Task::Target(const Vector3& _pos)
{
  if (m_startTimer > 0.0)
    return;
  ...
  case GlobalResearch::TypeHarvester:
    TargetHarvester(_pos);
    break;
  case GlobalResearch::TypeAirStrike:
    TargetAirStrike(_pos);
    break;
  case GlobalResearch::TypeNuke:
    TargetNuke(_pos);
    break;
  case GlobalResearch::TypeSubversion:
    TargetSubversion(_pos);
    break;
  case GlobalResearch::TypeBooster:
    TargetBooster(_pos);
    break;
  ...
  case GlobalResearch::TypeMeteorShower:
    TargetMeteorShower(_pos);
    break;
  case GlobalResearch::TypeExtinguisher:
    TargetExtinguisher(_pos);
    break;
  case GlobalResearch::TypeRage:
    TargetRage(_pos);
    break;
...
void Task::TargetSquad(const Vector3& _pos)
{
  ...
  g_app->m_location->SpawnEntities(_pos, m_teamId, unitId, Entity::TypeInsertionSquadie, numEntities, g_zeroVector, 10);
...
void Task::TargetEngineer(const Vector3& _pos)
{
  Vector3 pos = _pos;
  pos.y += 10.0;
...
void Task::TargetShaman(const Vector3& _pos)
{
  Vector3 pos = _pos;
...
void Task::TargetArmour(const Vector3& _pos)
{
#ifndef DEMOBUILD
...
WorldObjectId Task::FindDarwinian(const Vector3& _pos, int _teamId)
{
...
void Task::TargetOfficer(const Vector3& _pos)
{
  //
...
void Task::TargetAirStrike(const Vector3& _pos)
{
  DebugTrace("AirStrike Launched by Team %d\n", m_teamId);
...
void Task::TargetDropShip(const Vector3& _pos)
{
  float inset = 100.0;
...
void Task::TargetNuke(const Vector3& _pos)
{
...
void Task::TargetBooster(const Vector3& _pos)
{
...
void Task::TargetHotFeet(const Vector3& _pos)
{
...
void Task::TargetGunTurret(const Vector3& _pos)
{
  int turretType = m_option;
  if (turretType == -1)
    turretType = GunTurret::GunTurretTypeStandard;
...
void Task::TargetInfection(const Vector3& _pos)
{
...
void Task::TargetMagicalForest(const Vector3& _pos)
{
...
void Task::TargetDarkForest(const Vector3& _pos)
{
...
void Task::TargetAntNest(const Vector3& _pos)
{
...
void Task::TargetPlague(const Vector3& _pos)
{
...
void Task::TargetEggs(const Vector3& _pos)
{
...
void Task::TargetMeteorShower(const Vector3& _pos)
{
  ...
  if (m_state == StateRunning)
  {
    ...
    case GlobalResearch::TypeRage:
    {
      ...
      for (int i = 0; i < team->m_others.Size(); ++i)
      {
        if (team->m_others.ValidIndex(i))
        {
          Darwinian* d = static_cast<Darwinian*>(team->m_others[i]);
          if (d && d->m_type == Entity::TypeDarwinian)
          {
            if (d->m_hotFeetTaskId == m_id || d->m_boosterTaskId == m_id || d->m_subversionTaskId == m_id || d->m_rageTaskId == m_id)
            {
              pos += d->m_pos;
              num++;
            }
    ...
    case GlobalResearch::TypeGunTurret:
    {
      ...
      if (!turret)
        return true;
      if (turret->m_id.GetTeamId() != m_teamId && turret->m_id.GetTeamId() != 255)
      {
        turret->Damage(-1000);
        return true;
      }
...
void Task::Stop()
{
  ...
  case GlobalResearch::TypeHarvester:
  {
    Harvester* entity = static_cast<Harvester*>(g_app->m_location->GetEntity(m_objId));
...
void Task::GetTaskNameTranslated(int _type, UnicodeString& _dest) { return GlobalResearch::GetTypeNameTranslated(_type, _dest); }
...
bool TaskManager::RunTask(Task* _task)
{
  if (CapacityUsed() < Capacity())
  {
    ...
    if (g_app->Multiplayer() && CapacityUsed() >= Capacity() && m_teamId == g_app->m_globalWorld->m_myTeamId)
      g_app->m_location->SetCurrentMessage(LANGUAGEPHRASE("dialog_powerup_limit"), 255, true);

    return true;
  }
  if (g_app->Multiplayer() && m_teamId == g_app->m_globalWorld->m_myTeamId)
    g_app->m_location->SetCurrentMessage(LANGUAGEPHRASE("dialog_powerup_limit"), 255, true);
...
bool TaskManager::TerminateTask(int _id, bool _showMessage)
{
  for (int i = 0; i < m_tasks.Size(); ++i)
  {
    ...
    if (task->m_id == _id)
    {
      if (task->m_type == GlobalResearch::TypeShaman)
        return false; // you can't terminate shamans
...
void TaskManager::Advance()
{
  ...
  if (g_app->Multiplayer())
  {
    for (int i = m_tasks.Size() - 1; i > 0; --i)
    {
      ...
      if (thisTask->m_state == Task::StateRunning && prevTask->m_state == Task::StateStarted)
      {
  ...
  {
    double timeNow = GetHighResTime();
    if (m_mostRecentTaskIdTimer > 0.0f && timeNow > m_mostRecentTaskIdTimer + 3.0f)
...
void TaskManager::SelectTask(WorldObjectId _id)
{
  ...
  {
    Task* task = m_tasks[i];
    if (task->m_objId.GetTeamId() == _id.GetTeamId() && task->m_objId.GetUnitId() == _id.GetUnitId() && task->m_objId.GetIndex() == _id.
      GetIndex())
    {
      SelectTask(task->m_id);
      return;
...
bool TaskManager::TargetTask(int _id, const Vector3& _pos)
{
...
bool TaskManager::IsValidTargetArea(int _id, const Vector3& _pos)
{
  Task* task = GetTask(_id);
  if (!task)
    return false;
  ...
  if (m_verifyTargetting)
  {
    ...
    {
      if (RestrictionZone::IsRestricted(_pos, true))
        return false;
          ...
          if (b)
          {
            ...
            if (b->m_id.GetTeamId() != 255)
            {
              ...
              if (!g_app->m_location->IsFriend(b->m_id.GetTeamId(), m_teamId))
              {
                ...
                if (b->m_type == Building::TypeTrunkPort && dist < 400.0f)
                  invalid = true;

                if (b->m_type == Building::TypeSpawnPoint && dist < 200.0f)
                  invalid = true;
    ...
    if (task->m_type == GlobalResearch::TypeBooster || task->m_type == GlobalResearch::TypeHotFeet || task->m_type ==
      GlobalResearch::TypeSubversion || task->m_type == GlobalResearch::TypeRage)
    {
  ...
  if (task)
  {
    ...
    case GlobalResearch::TypeHarvester:
    {
      ...
      for (int i = 0; i < team->m_units.Size(); ++i)
      {
        if (team->m_units.ValidIndex(i))
        {
          ...
          if (unit->m_troopType == Entity::TypeInsertionSquadie || unit->m_troopType == Entity::TypeEngineer)
          {
            TaskTargetArea tta;
      ...
      if (g_app->IsSinglePlayer())
      {
        for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
        {
          if (g_app->m_location->m_buildings.ValidIndex(i))
          {
            ...
            if (building && building->m_id.GetTeamId() == m_teamId)
            {
    ...
    case GlobalResearch::TypeTank:
    {
      if (!g_app->Multiplayer())
      {
        for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
        {
          ...
          {
            Building* building = g_app->m_location->m_buildings[i];
            if (building && building->m_type == Building::TypeTrunkPort && static_cast<TrunkPort*>(building)->m_openTimer > 0.0)
            {
              TaskTargetArea tta;
              tta.m_centre = building->m_pos;
              tta.m_radius = 120.0;
    ...
    case GlobalResearch::TypeSquad:
      for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
      {
        ...
        {
          Building* building = g_app->m_location->m_buildings[i];
          if (building && g_app->m_location->IsFriend(building->m_id.GetTeamId(), m_teamId))
          {
            bool match = false;
            ...
            if (g_app->Multiplayer())
            {
              if (building->m_type == Building::TypeSpawnPoint || building->m_type == Building::TypeGunTurret || building->m_type ==
                Building::TypeMultiwiniaZone || building->m_type == Building::TypeTrunkPort)
                match = true;
            }
