...
void BenchMark::RequestDXDiag()
{
    ...
    {
        DebugTrace( "Requesting DXDiag...caught exception, continue\n" );
    }
#endif
    DebugTrace( "Requesting DXDiag...result code %d\n", result );
}
            ...
            if( colon )
            {
                *colon = '\x0';
                ...
                {                
                    ...
                    m_dxDiag.PutData( _strdup(fullLine) );                
                }
            }
        }
    }
...
void BenchMark::Begin()
{
    ...
    DebugTrace( "Benchmark started at %s\n", BuildTimeIndex() );
} 
...
void BenchMark::Upload()
{
    if( DoesFileExist( m_filename ) )
    {
        ...
        DebugTrace( "Uploading benchmark...result code %d\n", result );
        //DebugTrace( "CMD: ncftpput.exe %s\n", args );
    }
}
