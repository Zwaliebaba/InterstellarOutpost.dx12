...
EntityGridCache::~EntityGridCache() { m_cache.EmptyAndDelete(); }
  ...
  for (int i = 0; i < entityCacheSize; ++i)
  {
    if (m_cache.ValidIndex(i))
    {
      ...
      if (element->m_teamId == teamId)
      {
        ...
        if (distanceSqd < element->m_halfRangeSqd && distanceSqd < bestDistSqd)
        {
...
std::vector<WorldObjectId>* EntityGridCache::GetEnemies(double worldX, double worldZ, double range, int* numFound, unsigned char myTeam)
{
  *numFound = 0;
  ...
  auto element = new EntityGridCacheElement();
