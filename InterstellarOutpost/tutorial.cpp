...
Tutorial::Tutorial()
:   m_chapter(0),
    m_nextChapterTimer(-1.0),
    m_repeatMessageTimer(-1.0),
...
void Tutorial::RepeatMessage( char *_stringId, float _repeatPeriod, char *_gestureDemo )
{
    ...
    if( _stringId )
    {
        m_repeatMessage = _strdup( _stringId );
    ...
    if( _gestureDemo )
    {
        m_repeatGesture = _strdup( _gestureDemo );    
    }
}
...
void Tutorial::Advance()
{
    ...
    if( g_app->m_location->m_teams[2]->m_teamType == TeamTypeUnused ) return;
