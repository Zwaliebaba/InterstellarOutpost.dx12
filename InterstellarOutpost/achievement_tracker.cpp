...
unsigned char AchievementTracker::GetViralAchievementMask()
{
  unsigned char ret = 0;
  if (m_isMasterPlayer)
    ret |= VIRAL_MASTER_ACHIEVEMENT_MASK;
...
bool AchievementTracker::HasLoaded() { return m_hasLoaded; }

void AchievementTracker::AddToTotalKills(int kills)
{
  m_totalKills += kills;
  if (m_totalKills >= 1000000)
    g_app->GiveAchievement(App::DarwiniaAchievementGenocide);
  Save();
}
...
{
  if (AlreadyVisitedLevel(_mapName))
    return;
