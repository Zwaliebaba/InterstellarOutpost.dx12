...
Multiwinia::~Multiwinia() { s_gameBlueprints.EmptyAndDelete(); }

static RGBAColour defaultColour[] = {RGBAColour(100, 255, 100), // Normally Green AI
...
static RGBAColour teamColours[] = {RGBAColour(100, 255, 100), // Normally Green AI
                                   RGBAColour(255, 50, 50), // Normally Virii
                                   RGBAColour(255, 255, 50), // Normally Player
                                   RGBAColour(120, 180, 255), // Blue
                                   RGBAColour(255, 150, 0), // Orange
                                   ...
                                   RGBAColour(10, 10, 10), RGBAColour(200, 200, 200)};

static RGBAColour groupColours1[] = {RGBAColour(0, 150, 20), RGBAColour(150, 0, 20), RGBAColour(150, 150, 20), RGBAColour(20, 0, 150),
                                     RGBAColour(150, 90, 0), RGBAColour(180, 120, 255), RGBAColour(0, 140, 140), RGBAColour(255, 90, 255),
                                     RGBAColour(10, 10, 10), RGBAColour(200, 200, 200)};

static RGBAColour groupColours2[] = {RGBAColour(90, 200, 100), RGBAColour(255, 80, 80), RGBAColour(200, 200, 90), RGBAColour(90, 90, 200),
                                     RGBAColour(200, 150, 60), RGBAColour(100, 60, 180), RGBAColour(110, 210, 210),
                                     RGBAColour(255, 190, 255), RGBAColour(10, 10, 10), RGBAColour(200, 200, 20)};
...
void Multiwinia::InitialiseTeam(unsigned char _teamId, unsigned char _teamType, int _clientId)
{
  ...
  DEBUG_ASSERT(m_teams[_teamId].m_teamType == TeamTypeUnused);
#endif

  LobbyTeam* team = &m_teams[_teamId];

  DebugTrace("Zeroing achievement mask\n");
...
void Multiwinia::LoadBlueprints()
{
  ...
  while (in->ReadLine())
  {
    ...
    DEBUG_ASSERT(stricmp(effect, "GAMETYPE") == 0);

    auto blueprint = new MultiwiniaGameBlueprint(in->GetNextToken());
    ...
    {
      auto sb = new MultiwiniaGameParameter(param);
...
bool Multiwinia::GameInLobby() { return g_app->m_atMainMenu; }

bool Multiwinia::GameRunning() { return (!GameInLobby() && m_gameType != -1 && g_app->m_location); }

void Multiwinia::SetGameOptions(int _gameType, int* _params)
{
  m_gameType = _gameType;

  ASSERT_TEXT(s_gameBlueprints.ValidIndex(m_gameType), "Invalid Gametype %d\n", m_gameType);
...
{
  return s_gameBlueprints.ValidIndex(m_gameType) && s_gameBlueprints[m_gameType]->m_params.ValidIndex(_optionIndex);
}

void Multiwinia::SetGameOption(int _optionIndex, int _value)
{
  ASSERT_TEXT(ValidGameOption(_optionIndex), "Invalid game option %d for game type: %d\n", _optionIndex, m_gameType);
...
{
  memcpy(g_app->m_globalWorld->m_research->m_researchLevel, _research, GlobalResearch::NumResearchItems * sizeof(int));

  memset(g_app->m_globalWorld->m_research->m_researchProgress, 0, GlobalResearch::NumResearchItems * sizeof(int));
}
...
void Multiwinia::AdvanceSkirmish()
{
  ...
  m_timer -= SERVER_ADVANCE_PERIOD; if (m_timer <= 0.0)
  {
    ...
    {
      if (m_teams[t].m_teamType == TeamTypeUnused)
        continue;
      ...
      if (t == g_app->m_location->GetFuturewinianTeamId() || t == g_app->m_location->GetMonsterTeamId())
      {
        // this check has to be 1, not 0, because both special teams will always have an AI entity 
        if (g_app->m_location->m_teams[t]->NumEntities(255) <= 1)
          continue;
        if (SpawnPoint::s_numSpawnPoints[t + 1] == 0)
          continue;
      }
    ...
    else if (m_timeRemaining <= 0 && !m_unlimitedTime)
    {
      if (winningTeam == MULTIWINIA_DRAW_TEAM && GetGameOption("SuddenDeath") == 1)
      {
        ...
        for (int t = 0; t < NUM_TEAMS; ++t)
        {
          ...
          if (m_teams[t].m_score != highScore) { g_app->m_location->m_teams[t]->Eliminate(); }
        }
      }
      else { DeclareWinner(winningTeam); }
      return;
    }
    ...
    if (m_coopMode)
    {
      ...
      {
        if (i == g_app->m_location->GetFuturewinianTeamId() || i == g_app->m_location->GetMonsterTeamId())
        {
          if (g_app->m_location->m_teams[i]->NumEntities(255) <= 1)
            continue;
        }
      ...
      if (!noWinner) { DeclareWinner(winningTeam); }
    }
    else if (totalTeams - 1 == numEliminated)
    {
      if (notEliminated != -1)
      {
        ...
        if (notEliminated == g_app->m_globalWorld->m_myTeamId) { g_app->GiveAchievement(App::DarwiniaAchievementDominator); }
      }
    }
  }
...
void Multiwinia::AdvanceKingOfTheHill()
{
  ...
  m_timer -= SERVER_ADVANCE_PERIOD; if (m_timer <= 0.0f)
  {
    if (m_timeRemaining > 0)
      m_timeRemaining--;
    m_timer = 1.0f;
    ...
    for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
    {
      if (g_app->m_location->m_buildings.ValidIndex(i))
      {
        ...
        if (building->m_type == Building::TypeMultiwiniaZone)
        {
          ...
          if (zone->m_id.GetTeamId() != 255) { numZonesOwned[zone->m_id.GetTeamId()]++; }
        }
      }
    }
    ...
    for (int t = 0; t < NUM_TEAMS; ++t)
    {
      ...
      if (scoreMode == 0) { m_teams[t].m_score += numZonesOwned[t]; }
      if (scoreMode == 3) { if (numZonesOwned[t] == s_numMultiwiniaZones) { ++m_teams[t].m_score; } }
    ...
    if (m_timeRemaining <= 0)
    {
      if (winningTeamId == MULTIWINIA_DRAW_TEAM && GetGameOption("SuddenDeath") == 1)
      {
        ...
        for (int t = 0; t < NUM_TEAMS; ++t)
        {
          ...
          if (m_teams[t].m_score != highestScore) { g_app->m_location->m_teams[t]->Eliminate(); }
        }
      }
      else
      {
        ...
        if (winningTeamId == g_app->m_globalWorld->m_myTeamId)
        {
          ...
          for (int i = 0; i < NUM_TEAMS; ++i)
          {
            ...
            if (m_teams[i].m_score > secondHighest && m_teams[i].m_score < highestScore) { secondHighest = m_teams[i].m_score; }
          }

          DebugTrace("Highest score is %d, secondHighest is %d\n", highestScore, secondHighest);

          if (highestScore > secondHighest * 1.5f)
          {
            DebugTrace("Giving achievement!\n");
  ...
  m_timer -= SERVER_ADVANCE_PERIOD; if (m_timer <= 0.0f)
  {
      ...
      if (lobbyTeam->m_rocketId == -1)
      {
        for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
        {
          if (g_app->m_location->m_buildings.ValidIndex(i))
          {
            ...
            if (building && building->m_type == Building::TypeEscapeRocket && building->m_id.GetTeamId() == t)
            {
    ...
    for (int t = 0; t < NUM_TEAMS; ++t)
    {
      ...
      if (rocket && rocket->m_type == Building::TypeEscapeRocket)
      {
        ...
        if (rocket->m_state == EscapeRocket::StateFlight && rocket->m_pos.y > 600)
        {
          DeclareWinner(t);
          ...
          if (t == g_app->m_globalWorld->m_myTeamId && rocket->m_damage <= 0.01)
          {
            ...
            for (int i = 0; i < NUM_TEAMS; ++i)
            {
              if (g_app->m_location->m_teams[i]->m_monsterTeam)
                continue;
              if (g_app->m_location->m_teams[i]->m_futurewinianTeam)
                continue;
              if (m_teams[i].m_teamType == TeamTypeUnused)
                continue;
            ...
            if (!noWin) { g_app->GiveAchievement(App::DarwiniaAchievementUncladSkies); }
          }
          break;
        }
      }
  ...
  if (m_reinforcementCountdown <= 0)
  {
    ...
    if (reinforcementCount == -1)
    {
      ...
      if (m_gameType == GameTypeAssault)
      {
        ...
        if (defenderCount == -1)
          defenderCount = reinforcementCount;
        if (attackerCount == -1)
          attackerCount = reinforcementCount;
      }
    }
      ...
      if (g_app->m_location->m_buildings.ValidIndex(i))
      {
        ...
        if (building && building->m_type == Building::TypeTrunkPort)
        {
          ...
          if (teamId != 255)
          {
            ...
            if (!static_cast<TrunkPort*>(building)->PopulationLocked())
            {
              for (int j = 0; j < reinforcementCount; ++j)
              {
                ...
                Darwinian* darwinian = static_cast<Darwinian*>(g_app->m_location->GetEntitySafe(spawned, Entity::TypeDarwinian));
                if (darwinian)
            ...
            if (!m_firstReinforcements)
              tm->m_blockMostRecentPopup = true;
            ...
            if (tm->CountNumTasks(GlobalResearch::TypeArmour) < maxArmour || maxArmour == 0)
            {
              bool attacking = g_app->m_location->m_levelFile->m_attacking[g_app->m_location->GetTeamPosition(teamId)];
              if ((defenderReinforcementType == -1 && !attacking) || (attackerReinforcementType == -1 && attacking))
              {
            ...
            if (m_gameType == GameTypeBlitzkreig)
            {
              int numZones = MultiwiniaZone::GetNumZones(teamId);
              if (numZones >= (s_numMultiwiniaZones + 1) / 2)
                taskType = GlobalResearch::TypeAirStrike;
              int num = g_app->m_location->m_levelFile->m_levelOptions[LevelFile::LevelOptionAirstrikeCaptureZones];
              if (num != -1 && numZones > num)
                taskType = GlobalResearch::TypeAirStrike;
            }

            if (giveTurrets && tm->CountNumTasks(taskType) < maxTurrets)
            {
              tm->RunTask(taskType);
              tm->NotifyRecentTaskSource(1);
            }
    ...
    if (attackerReinforcementType != -1)
    {
      for (int i = 0; i < g_app->m_location->m_levelFile->m_numPlayers; ++i)
      {
        if (m_teams[i].m_teamType == TeamTypeUnused)
          continue;
        if (g_app->m_location->m_levelFile->m_attacking[g_app->m_location->GetTeamPosition(i)] || m_gameType != GameTypeAssault)
        {
          g_app->m_location->m_teams[i]->m_blockUnitChange = true;
          TaskManager* tm = g_app->m_location->m_teams[i]->m_taskManager;
          if (!m_firstReinforcements)
...
void Multiwinia::AdvanceRetribution()
{
  for (int t = 0; t < NUM_TEAMS; ++t)
  {
    ...
    if (team->m_monsterTeam || team->m_futurewinianTeam)
      continue;
...
void Multiwinia::EnsureStartingStatue()
{
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    if (g_app->m_location->m_buildings.ValidIndex(i))
    {
      ...
      if (building)
      {
        ...
        if (building->m_type == Building::TypeMultiwiniaZone && building->m_id.GetTeamId() == 255)
  ...
  if (statues.Size() == 0)
  {
    ...
    Statue* building = static_cast<Statue*>(Building::CreateBuilding(Building::TypeStatue));
    g_app->m_location->m_buildings.PutData(building);
    building->Initialise(static_cast<Building*>(&carryTemplate));
    int id = g_app->m_globalWorld->GenerateBuildingId();
  ...
  m_timer -= SERVER_ADVANCE_PERIOD; if (m_timer <= 0.0f)
  {
    ...
    if (m_timeRemaining <= 0)
    {
      ...
      if (m_coopMode)
      {
        ...
        if (group1Total == group2Total) { winningTeam = MULTIWINIA_DRAW_TEAM; }
        else if (group2Total > group1Total) { winningTeam = group2Player; }
        else { winningTeam = group1Player; }
      }
      ...
      else
      {
        ...
        if (winningTeam == g_app->m_globalWorld->m_myTeamId)
        {
          ...
          for (int i = 0; i < NUM_TEAMS; ++i)
          {
            ...
            if (m_teams[i].m_score > secondHighest && m_teams[i].m_score < highScore) { secondHighest = m_teams[i].m_score; }
          }

          if (highScore > secondHighest * 2 && highScore > 3) { g_app->GiveAchievement(App::DarwiniaAchievementHoarder); }
        }
    ...
    for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
    {
      if (g_app->m_location->m_buildings.ValidIndex(i))
      {
        ...
        if (building)
        {
          if (building->m_type == Building::TypeStatue) { statues.PutData(building->m_id.GetUniqueId()); }
    ...
    if (statues.Size() < statueSpawns.Size())
    {
      if (m_statueSpawner == -1.0)
      {
        ...
        if (m_statueSpawner < 10.0)
          m_statueSpawner = 10.0;

        DebugTrace("Statue Spawn timer = %d secs\n", (int)m_statueSpawner);
      }
      else if (m_statueSpawner > 0.0f) { m_statueSpawner -= 1.0f; }
      else if (m_statueSpawner <= 0.0f)
      {
        ...
        else
        {
          ...
          for (int i = 0; i < statueSpawns.Size(); ++i)
          {
            ...
            if (thisNearest > largestSpawnerDistance && thisNearest > 30)
            {
...
void Multiwinia::AdvanceTankBattle()
{
  ...
  m_timer -= SERVER_ADVANCE_PERIOD; if (m_timer <= 0.0f)
  {
    ...
    for (int t = 0; t < NUM_TEAMS; ++t)
    {
      if (m_teams[t].m_teamType == TeamTypeUnused)
        continue;
      if (t == g_app->m_location->GetMonsterTeamId() || t == g_app->m_location->GetFuturewinianTeamId())
        continue;
        ...
        if (!tankFound)
        {
          ...
          else
          {
            ...
            for (int i = 0; i < g_app->m_location->m_levelFile->m_instantUnits.Size(); ++i)
            {
              ...
              if (iu->m_type == Entity::TypeTank && iu->m_teamId == g_app->m_location->GetTeamPosition(t))
              {
    ...
    if (totalTeams - 1 == numEliminated) { DeclareWinner(notEliminated); }
  }
#endif
#endif
}
...
void Multiwinia::AdvanceAssault()
{
    ...
    for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
    {
      if (g_app->m_location->m_buildings.ValidIndex(i))
      {
        ...
        if (building->m_type == Building::TypePulseBomb)
        {
          ...
          for (int j = 0; j < pb->m_links.Size(); ++j) { m_assaultObjectives.PutDataAtEnd(pb->m_links[j]); }
  ...
  {
    m_timer = 1.0f;
    if (m_timeRemaining > 0 && m_victoryTimer <= 0.0f)
      m_timeRemaining--;
    ...
    if (m_pulseDetonated)
    {
      ...
      {
        if (m_victoryTimer <= 1.0f && g_app->m_renderer->IsFadeComplete() && m_numRotations < g_app->m_location->m_levelFile->m_numPlayers -
          1) { g_app->m_renderer->StartFadeOut(); }
      }

      if (m_victoryTimer <= 0.0f && !PulseWave::s_pulseWave)
      {
        if (m_numRotations == g_app->m_location->m_levelFile->m_numPlayers - 1 || m_assaultSingleRound)
        {
          ...
          else
          {
            ...
            for (int i = 0; i < g_app->m_location->m_levelFile->m_numPlayers; ++i)
            {
              if (m_timeRecords[i] >= PulseBomb::s_defaultTime)
                numFullDefenses++;
          ...
          {
            if (m_defendedAll) { g_app->GiveAchievement(App::DarwiniaAchievementAggravatedAssault); }
          }
        }
        else
        {
...
bool DoesBlitzkriegRouteExist(int fromZoneId, int attackingTeamId, LList<int>& zonesSoFar)
{
  ...
  MultiwiniaZone* currentZone = static_cast<MultiwiniaZone*>(g_app->m_location->GetBuilding(fromZoneId));
  ...
  if (currentZone->m_blitzkriegOwnership < 100.0f && currentZone->m_blitzkriegUpOrDown < 0)
    return false;
...
void Multiwinia::AdvanceBlitkrieg()
{
  ...
  } AdvanceTrunkPorts(); m_timer -= SERVER_ADVANCE_PERIOD; if (m_timer <= 0.0f)
  {
    if (m_timeRemaining > 0)
      m_timeRemaining--;
    m_timer = 1.0f;
    ...
    for (int t = 0; t < NUM_TEAMS; ++t)
    {
      if (!m_eliminated[t] && m_teams[t].m_teamType != TeamTypeUnused && t != g_app->m_location->GetMonsterTeamId() && t != g_app->
        m_location->GetFuturewinianTeamId())
      {
        ...
        if (zone->m_id.GetTeamId() != 255 && zone->m_id.GetTeamId() != t)
        {
    ...
    for (int t = 0; t < NUM_TEAMS; ++t)
    {
        ...
        m_location->GetFuturewinianTeamId())
      {
        ...
        if (zone->m_id.GetTeamId() == 255)
          m_teams[t].m_score = BLITZKRIEGSCORE_UNOCCUPIED;
        else if (zone->m_id.GetTeamId() != t)
          m_teams[t].m_score = BLITZKRIEGSCORE_ENEMYOCCUPIED;
        else if (!zone->m_blitzkriegLocked)
          m_teams[t].m_score = BLITZKRIEGSCORE_UNDERTHREAT;
        else
          m_teams[t].m_score = BLITZKRIEGSCORE_SAFE;
    ...
    if (m_timeRemaining <= 0 && !m_unlimitedTime)
    {
      ...
      for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
      {
        if (g_app->m_location->m_buildings.ValidIndex(i))
        {
          ...
          if (b->m_type == Building::TypeMultiwiniaZone && b->m_id.GetTeamId() != 255) { numFlags[b->m_id.GetTeamId()]++; }
        }
      }
      ...
      for (int i = 0; i < NUM_TEAMS; ++i)
      {
        ...
        else if (realScores[i] == highscore) { winningTeam = MULTIWINIA_DRAW_TEAM; }
      }
    }
    ...
    if (winningTeam == MULTIWINIA_DRAW_TEAM && GetGameOption("SuddenDeath") == 1)
    {
      if (!m_suddenDeathActive)
      {
        for (int t = 0; t < NUM_TEAMS; ++t)
        {
          ...
          if (realScores[t] != highscore) { g_app->m_location->m_teams[t]->Eliminate(); }
        }
    ...
    else if (winningTeam >= 0)
    {
      ...
      if (myTeam && myTeam->m_teamId == m_teams[winningTeam].m_teamId && m_startTime <= m_timeRemaining + 300) // 300 is 5 minutes!
      {
        g_app->GiveAchievement(App::DarwiniaAchievementBlitzMaster);
      }
    }
...
void Multiwinia::AdvanceGracePeriod()
{
  ...
  {
    Team* team = g_app->m_location->m_teams[i];
    if (team->m_teamType == TeamTypeLocalPlayer || team->m_teamType == TeamTypeRemotePlayer || team->m_teamType == TeamTypeSpectator)
    {
      ++numFound;
      if (team->m_ready)
        ++numReady;
  ...
  if (numReady == numFound)
  {
    ...
    difference = abs(static_cast<int>(m_gracePeriodTimer) - m_gracePeriodTimer);
...
void Multiwinia::Advance()
{
  ...
  if (m_victoryTimer > 0.0)
    m_victoryTimer -= SERVER_ADVANCE_PERIOD;
  ...
  case GameTypeCaptureTheStatue:
    AdvanceCaptureStatue();
    break;
  case GameTypeTankBattle:
    AdvanceTankBattle();
    break;
  ...
  if (g_app->m_location->m_levelFile->m_levelOptions[LevelFile::LevelOptionTrunkPortReinforcements] > 0 || g_app->m_location->m_levelFile->
    m_levelOptions[LevelFile::LevelOptionTrunkPortArmour] > 0)
    AdvanceTrunkPorts();
  ...
  if (m_gameType != GameTypeAssault && m_gameType != GameTypeBlitzkreig)
  {
    ...
    if (m_positionTimer <= 0.0)
    {
      ...
      for (int t = 0; t < NUM_TEAMS; ++t)
      {
        if (g_app->m_location->m_teams[t]->m_monsterTeam)
          continue;
        if (g_app->m_location->m_teams[t]->m_futurewinianTeam)
          continue;
        Team* team = g_app->m_location->m_teams[t];
...
double Multiwinia::GetPreciseTimeRemaining() { return m_timeRemaining - (1.0 - m_timer); }
...
bool Multiwinia::GameOver()
{
  ...
  if ( //m_gameType == Multiwinia::GameTypeShaman ||
    m_gameType == GameTypeSkirmish || m_gameType == GameTypeTankBattle)
  {
    if (m_winner != -1)
    ...
    else
    {
      ...
      for (int i = 0; i < NUM_TEAMS; ++i)
      {
        if (!g_app->m_location->m_teams[i])
          continue;
        if (g_app->m_location->m_teams[i]->m_teamType == TeamTypeUnused)
          continue;
        if (g_app->m_location->GetMonsterTeamId() == i)
          continue;
        if (g_app->m_location->GetFuturewinianTeamId() == i)
          continue;
        totalTeams++;
        if (m_eliminated[i])
          numEliminated++;
  ...
  if (m_gameType == GameTypeRocketRiot) // Rocket riot
  {
    if (m_rocketsFound)
    {
      for (int t = 0; t < NUM_TEAMS; ++t)
      {
        ...
        auto rocket = (EscapeRocket*)g_app->m_location->GetBuilding(lobbyTeam->m_rocketId);

        if (rocket && rocket->m_type == Building::TypeEscapeRocket && rocket->m_state == EscapeRocket::StateFlight && rocket->m_pos.y > 600)
        {
          DeclareWinner(t);
          return true;
        }
...
void Multiwinia::RemoveTeams(int _clientId/*, int _reason */)
{
  for (int i = 0; i < NUM_TEAMS; ++i)
  {
    ...
    if (team.m_teamType != TeamTypeUnused && team.m_teamType != TeamTypeCPU && team.m_clientId == _clientId)
    {
      if (GameInLobby())
      {
      ...
      {
        DebugTrace("Team %d replaced with CPU player\n", team.m_teamId);
        ...
        if (g_app)
        {
          if (g_app->m_location)
          {
            ...
            g_app->m_location->SpawnEntities(Vector3(-100, 0, -100), team.m_teamId, -1, Entity::TypeAI, 1, g_zeroVector, 0);
...
bool Multiwinia::IsEliminated(int _teamId)
{
  ...
  case GameTypeCaptureTheStatue:
    return (CountSpawnPoints() > 0 && // cant be eliminated in koth/cts on maps that dont have spawn points
      SpawnPoint::s_numSpawnPoints[_teamId + 1] == 0 && CountDarwinians(_teamId) < 50);
...
{
  if (m_pulseTimer == -1.0f)
    m_pulseTimer = _time;
  else
    if (_time < m_pulseTimer)
      m_pulseTimer = _time;
...
bool Multiwinia::GetScoreLine(int _teamId, wchar_t* _statusLine, size_t bufferBytes)
{
  ...
  if (g_app->m_location->m_teams[teamIndex]->m_monsterTeam || g_app->m_location->m_teams[teamIndex]->m_futurewinianTeam)
  {
    if (currentScore == 0)
      return false;
  }
  ...
  if (scoreChangeInt > 0)
    sprintf(scoreChange, "(+%d)", scoreChangeInt);
  else if (scoreChangeInt < 0)
    sprintf(scoreChange, "(%d)", scoreChangeInt);
  else
    sprintf(scoreChange, "");
  ...
  if (m_gameType == GameTypeCaptureTheStatue)
  {
    ...
    for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
    {
      ...
      {
        Building* building = g_app->m_location->m_buildings[i];
        if (building && building->m_type == Building::TypeStatue && building->m_id.GetTeamId() == _teamId)
          ++carryCount;
      }
    }

    if (carryCount > 0)
      sprintf(scoreChange, "(+%d)", carryCount);
  }

  if (fmodf(g_gameTimer.GetGameTime(), 1.0f) > 0.5f && m_gameType != GameTypeCaptureTheStatue)
    sprintf(scoreChange, "");
  ...
  if (((m_gameType == GameTypeKingOfTheHill && GetGameOption("ScoreMode") != 2) || (m_gameType == GameTypeSkirmish &&
    GetGameOption("ScoreMode") == 1)) && m_timeRemaining > -1)
    sprintf(estScoreMsg, "(est %d)", estimatedScore);

  if (GameInGracePeriod())
  {
    swprintf(_statusLine, bufferWChars, L"%-15ls  %ls", team->GetTeamName().m_unicodestring, team->m_teamType == TeamTypeCPU
             ? LANGUAGEPHRASE("multiwinia_ready").m_unicodestring
             : team->m_ready
             ? LANGUAGEPHRASE("multiwinia_ready").m_unicodestring
             : L" ");
  }
  else if (m_gameType == GameTypeRocketRiot)
  {
    // Rocket riot
    double fuel = 0;
    int passengers = 0;
    ...
    if (passengers > 0)
      swprintf(passengersLine, sizeof(passengersLine) / sizeof(wchar_t), L"%ls %3d/100",
               LANGUAGEPHRASE("multiwinia_rr_passengers").m_unicodestring, passengers);
    if (damage > 0)
      swprintf(damageLine, sizeof(damageLine) / sizeof(wchar_t), L"%ls %3d%%", LANGUAGEPHRASE("multiwinia_rr_damage").m_unicodestring,
               damage);

    if (static_cast<int>(fuelPressure) > 0)
      swprintf(fuelLine, sizeof(fuelLine) / sizeof(wchar_t), L"(+%dpsi)", static_cast<int>(fuelPressure));

    swprintf(_statusLine, bufferWChars, L"%-15ls  %-10ls %ls %3d%% %-10ls %-20ls %hs", team->GetTeamName().m_unicodestring, damageLine,
             LANGUAGEPHRASE("multiwinia_rr_fuel").m_unicodestring, static_cast<int>(fuel), fuelLine, passengersLine, inSyncMsg);
  ...
  else if (m_gameType == GameTypeAssault)
  {
    if (g_app->m_location->IsDefending(teamIndex))
    {
      swprintf(_statusLine, bufferWChars, L"%-15ls  %ls %hs", team->GetTeamName().m_unicodestring,
               LANGUAGEPHRASE("multiwinia_assault_holdtheline").m_unicodestring, inSyncMsg);
    }
    else
    {
      if (m_timeRecords[teamIndex] != -1.0f)
      {
        ...
        int secondsRemaining = static_cast<int>(m_timeRecords[teamIndex]) % 60;
        ...
        swprintf(_statusLine, bufferWChars, L"%-15ls  %ls %hs", team->GetTeamName().m_unicodestring,
                 LANGUAGEPHRASE("multiwinia_defendedfor_score").m_unicodestring, inSyncMsg);
      ...
      {
        swprintf(_statusLine, bufferWChars, L"%-15ls  %ls %hs", team->GetTeamName().m_unicodestring,
                 LANGUAGEPHRASE("multiwinia_assault_ticktock").m_unicodestring, inSyncMsg);
      }
    }
  }
  ...
  {
    swprintf(_statusLine, bufferWChars, L"%-15ls  %ls %hs", team->GetTeamName().m_unicodestring,
             LANGUAGEPHRASE("multiwinia_numtanks").m_unicodestring, inSyncMsg);
    char score[32];
  ...
  {
    if (m_eliminationTimer[teamIndex] < 30 && m_eliminationTimer[teamIndex] > 0)
    {
      ...
      swprintf(_statusLine, bufferWChars, L"%-15ls  %ls, %ls %hs", team->GetTeamName().m_unicodestring,
               LANGUAGEPHRASE(pointsStringId).m_unicodestring, LANGUAGEPHRASE("multiwinia_dom_eliminatedin").m_unicodestring, inSyncMsg);
    ...
    {
      swprintf(_statusLine, bufferWChars, L"%-20ls  %ls %hs", team->GetTeamName().m_unicodestring,
               LANGUAGEPHRASE("multiwinia_dom_eliminated").m_unicodestring, inSyncMsg);
    }
    else if (GetGameOption("ScoreMode") == 1)
    {
      swprintf(_statusLine, bufferWChars, L"%-15ls  %ls %5hs      %hs %hs", team->GetTeamName().m_unicodestring,
               LANGUAGEPHRASE("multiwinia_points").m_unicodestring, scoreChange, estScoreMsg, inSyncMsg);
    ...
    {
      swprintf(_statusLine, bufferWChars, L"%-15ls  %ls %hs", team->GetTeamName().m_unicodestring,
               LANGUAGEPHRASE("multiwinia_dom_spawnpoints").m_unicodestring, inSyncMsg);
  ...
  else if (m_gameType == GameTypeBlitzkreig)
  {
    ...
    else
    {
      ...
      case BLITZKRIEGSCORE_ENEMYOCCUPIED:
        bkStatus = LANGUAGEPHRASE("multiwinia_blitzkrieg_enemyoccupied");
        break;
      case BLITZKRIEGSCORE_UNDERATTACK:
        bkStatus = LANGUAGEPHRASE("multiwinia_blitzkrieg_underattack");
        break;
      case BLITZKRIEGSCORE_UNOCCUPIED:
        bkStatus = LANGUAGEPHRASE("multiwinia_blitzkrieg_unoccupied");
        break;
      case BLITZKRIEGSCORE_UNDERTHREAT:
        bkStatus = LANGUAGEPHRASE("multiwinia_blitzkrieg_underthreat");
        break;
      case BLITZKRIEGSCORE_SAFE:
        bkStatus = LANGUAGEPHRASE("multiwinia_blitzkrieg_safe");
        break;
      }
    }

    swprintf(_statusLine, bufferWChars, L"%-15ls  %ls %hs", team->GetTeamName().m_unicodestring, bkStatus.m_unicodestring, inSyncMsg);
  }
  else
  {
    swprintf(_statusLine, bufferWChars, L"%-15ls  %ls %4hs     %hs %hs", team->GetTeamName().m_unicodestring,
             LANGUAGEPHRASE("multiwinia_points").m_unicodestring, scoreChange, estScoreMsg, inSyncMsg);
...
bool Multiwinia::GameInGracePeriod() { return (m_gracePeriodTimer > 0.0f); }
...
void Multiwinia::Render()
{
  ...
  float scale = static_cast<float>(g_app->m_renderer->ScreenH()) / 800.0f;
  ...
  for (int t = 0; t < NUM_TEAMS; ++t)
  {
    ...
    //if( g_app->m_location->m_teams[t]->m_futurewinianTeam ) continue;
    Team* team = g_app->m_location->m_teams[t];
    if (g_app->m_location->m_teams[t]->m_monsterTeam || g_app->m_location->m_teams[t]->m_futurewinianTeam)
      if (m_teams[t].m_score == 0)
        continue;
  ...
  if (m_coopMode)
  {
    ...
    for (int i = 0; i < NUM_TEAMS; ++i)
    {
      if (g_app->m_location->m_coopGroups[i] == 1)
      {
        if (m_teams[i].m_score > group1Total)
          coopOrderedList[0].PutDataAtStart(i);
        else
          coopOrderedList[0].PutDataAtEnd(i);
        group1Total += m_teams[i].m_score;
      }
      else if (g_app->m_location->m_coopGroups[i] == 2)
      {
        if (m_teams[i].m_score > group2Total)
          coopOrderedList[1].PutDataAtStart(i);
        else
          coopOrderedList[1].PutDataAtEnd(i);
        group2Total += m_teams[i].m_score;
      }
    }
    ...
    if (group1Total > group2Total)
      orderedList.PutDataAtEnd(2);
    else
      orderedList.PutDataAtStart(2);

    if (m_teams[g_app->m_location->GetMonsterTeamId()].m_score > 0)
      orderedList.PutDataAtEnd(g_app->m_location->GetMonsterTeamId());
    if (m_teams[g_app->m_location->GetFuturewinianTeamId()].m_score > 0)
      orderedList.PutDataAtEnd(g_app->m_location->GetFuturewinianTeamId());
  }
  ...
  if (!GameOver() || (m_gameType != GameTypeTankBattle && m_gameType != GameTypeBlitzkreig && (m_gameType != GameTypeAssault || !
  ...
  if (!GameOver())
  {
    ...
    if (m_gameType == GameTypeAssault)
      scoresTitle = LANGUAGEPHRASE("multiwinia_teamscores_assault");
          ...
          for( int i = 0; i < m_assaultObjectives.Size(); ++i )
  {
    ...
    {
      bool special = false;
      if (orderedList[t] == g_app->m_location->GetMonsterTeamId() || orderedList[t] == g_app->m_location->GetFuturewinianTeamId())
        special = true;
      ...
      if (m_coopMode && !special)
      {
        ...
        int scoreTotal = 0;
        if (groupId == 1)
          scoreTotal = group1Total;
        else
          scoreTotal = group2Total;
        //char teamLine[512];
        UnicodeString teamName;
        switch (g_app->m_location->m_groupColourId[groupId - 1])
        {
        case 0:
          teamName = LANGUAGEPHRASE("multiwinia_team_green");
          break;
        case 1:
          teamName = LANGUAGEPHRASE("multiwinia_team_red");
          break;
        case 2:
          teamName = LANGUAGEPHRASE("multiwinia_team_yellow");
          break;
        case 3:
          teamName = LANGUAGEPHRASE("multiwinia_team_blue");
          break;
        case 4:
          teamName = LANGUAGEPHRASE("multiwinia_team_orange");
          break;
        case 5:
          teamName = LANGUAGEPHRASE("multiwinia_team_purple");
          break;
        case 6:
          teamName = LANGUAGEPHRASE("multiwinia_team_cyan");
          break;
        case 7:
          teamName = LANGUAGEPHRASE("multiwinia_team_pink");
          break;
  ...
  else if (m_gameType != GameTypeTankBattle && (m_gameType != GameTypeAssault || !m_assaultSingleRound) && m_gameType !=
    GameTypeBlitzkreig)
  {
    // Scores at game over 
    float screenCentre = g_app->m_renderer->ScreenW() / 2;
    ...
    for (int i = 0; i < orderedList.Size(); ++i)
    {
      ...
      else
      {
        ...
        case GameTypeCaptureTheStatue:
        case GameTypeKingOfTheHill:
          swprintf(scoreString, sizeof(scoreString) / sizeof(wchar_t), L"%-15ls   %-4d", team->GetTeamName().m_unicodestring,
                   lobbyTeam->m_score);
          break;
        ...
        case GameTypeAssault:
        {
          ...
          int seconds = static_cast<int>(m_timeRecords[teamIndex]) % 60;

          minutes = max(minutes, 0);
          seconds = max(seconds, 0);
          wchar_t teamNameString[256];
          swprintf(teamNameString, sizeof(teamNameString) / sizeof(wchar_t), L"%-15ls", team->GetTeamName().m_unicodestring);
          char timeString[256];
        ...
        case GameTypeRocketRiot:
        {
          swprintf(scoreString, sizeof(scoreString) / sizeof(wchar_t), L"%-15ls", team->GetTeamName().m_unicodestring);
        }
        break;
        }
      }
  ...
  if (m_timeRemaining > 0 && !g_app->m_multiwinia->GameInGracePeriod())
  {
    ...
    char caption[256];
    sprintf(caption, "%d%c%02d", minutesRemaining, fmodf(g_gameTimer.GetGameTime(), 1.0f) < 0.5f ? ':' : ' ', secondsRemaining);
  ...
  bool eliminated = g_app->m_globalWorld->m_myTeamId >= 0 && g_app->m_globalWorld->m_myTeamId < NUM_TEAMS && IsEliminated(
    ...
    if (m_pulseDetonated && m_numRotations < g_app->m_location->m_levelFile->m_numPlayers - 1)
    {
      if (!m_assaultSingleRound)
      {
        ...
        if (defender != -1)
        {
          ...
          int secondsRemaining = static_cast<int>(m_timeRecords[defender]) % 60;
          ...
          {
            char time[16];
            sprintf(time, "%d", static_cast<int>(m_victoryTimer));
  ...
  if (GameOver())
  {
    ...
    if (m_gameType != GameTypeAssault || !m_assaultSingleRound)
    {
      if (m_coopMode)
      {
        int winner = 1;
        ...
        {
        case 0:
          winnerString = LANGUAGEPHRASE("multiwinia_team_green");
          break;
        case 1:
          winnerString = LANGUAGEPHRASE("multiwinia_team_red");
          break;
        case 2:
          winnerString = LANGUAGEPHRASE("multiwinia_team_yellow");
          break;
        case 3:
          winnerString = LANGUAGEPHRASE("multiwinia_team_blue");
          break;
        case 4:
          winnerString = LANGUAGEPHRASE("multiwinia_team_orange");
          break;
        case 5:
          winnerString = LANGUAGEPHRASE("multiwinia_team_purple");
          break;
        case 6:
          winnerString = LANGUAGEPHRASE("multiwinia_team_cyan");
          break;
        case 7:
          winnerString = LANGUAGEPHRASE("multiwinia_team_pink");
          break;

        default:
          //winnerString = UnicodeString("BAD TEAM NAME");        
          winnerString = winner == 3 ? LANGUAGEPHRASE("multiwinia_team_virus") : LANGUAGEPHRASE("multiwinia_team_future");
          break;
        }
        ...
        if (winner < 3)
          glColor4ubv(GetColour(g_app->m_location->m_groupColourId[winner - 1]).GetData());
        else if (winner == 3)
          glColor4ubv(m_teams[g_app->m_location->GetMonsterTeamId()].m_colour.GetData());
        else
          glColor4ubv(m_teams[g_app->m_location->GetFuturewinianTeamId()].m_colour.GetData());
...
void Multiwinia::RenderOverlay_GracePeriod()
{
  ...
  if (m_gracePeriodTimer < GRACE_TIME)
  {
    ...
    char t[64];
    sprintf(t, "%d", static_cast<int>(m_gracePeriodTimer + 1.0f));
  ...
  else if (!usReady)
  {
    if (g_inputManager->getInputMode() == INPUT_MODE_KEYBOARD)
      statusMsg = LANGUAGEPHRASE("multiwinia_spacetostart");
    else
      statusMsg = LANGUAGEPHRASE("multiwinia_atostart");
  }
...
void Multiwinia::DeclareWinner(int _teamId)
{
  ...
  if (t)
    g_app->m_achievementTracker->AddToTotalKills(t->m_teamKills);
...
{
  DebugTrace("Checking master achievement\n");
  if (!g_app->EarnedAchievement(App::DarwiniaAchievementMasterPlayer))
  {
    DebugTrace("Player has not yet earned MasterPlayer achievement\n");
    if (!m_coopMode && m_winner == g_app->m_location->GetMyTeam()->m_teamId)
    {
      for (int i = 0; i < NUM_TEAMS; i++)
      {
        if (m_teams[i].m_teamType != TeamTypeCPU && m_teams[i].m_teamType != TeamTypeUnused && m_teams[i].m_teamId != g_app->m_location->
          GetMyTeam()->m_teamId && // This is not us
  ...
  else
  {
    if (m_gameType == GameTypeAssault)
    {
      ...
      if (numPlayers != -1)
      {
        ...
        if (numPlayers == 3)
        {
          if (_colourId == 0 && num > 0)
            taken = true;
          else
            if (_colourId == 1 && num == 2)
              taken = true;
        }
        else
        {
...
int Multiwinia::GetCPUColour()
{
  ...
  for (int i = 0; i < NUM_TEAMS; ++i)
  {
    if (g_app->m_multiwinia->m_teams[i].m_colourId != -1 && g_app->m_multiwinia->m_teams[i].m_colourId < MULTIWINIA_NUM_TEAMCOLOURS / 2)
      taken[g_app->m_multiwinia->m_teams[i].m_colourId] = true;
  }

  int id = syncrand() % (MULTIWINIA_NUM_TEAMCOLOURS / 2);
  while (taken[id]) { id = syncrand() % (MULTIWINIA_NUM_TEAMCOLOURS / 2); }
...
bool Multiwinia::IsValidCoopColour(int _colourId)
{
  ...
  for (int i = 0; i < MULTIWINIA_NUM_TEAMCOLOURS; ++i)
  {
    ...
    if (taken)
      numColoursTaken++;
    if (i == _colourId)
      numOnThisColour = taken;
  }

  if (numColoursTaken >= 2 && numOnThisColour == 0)
    return false;
  if (numOnThisColour >= 2)
    return false;

  return true;
}

RGBAColour Multiwinia::GetDefaultTeamColour(int _teamId) { return defaultColour[_teamId]; }

RGBAColour Multiwinia::GetColour(int _colourId) { return teamColours[_colourId]; }

RGBAColour Multiwinia::GetGroupColour(int _memberId, int _colourId)
{
  if (_colourId == -1)
    return RGBAColour(0.6f, 0.6f, 0.6f, 1.0f);
  if (_memberId == 0)
    return groupColours1[_colourId];
  return groupColours2[_colourId];
}
...
void Multiwinia::SaveIFrame(int _clientId, IFrame* _iframe)
{
  for (int i = 0; i < NUM_TEAMS; i++)
  {
    ...
    {
      DebugTrace("Multiwinia: Captured IFrame from client %d\n", _clientId);
  ...
  for (int i = 0; i < NUM_TEAMS; i++)
  {
    if (m_teams[i].m_clientId == g_app->m_clientToServer->m_clientId)
    {
      if (m_teams[i].m_iframe)
      {
        ...
        {
          if (i != j && m_teams[j].m_iframe && m_teams[j].m_syncDifferences.Size() == 0 && m_teams[i].m_iframe->HashValue() != m_teams[j].
            m_iframe->HashValue())
          {
            m_teams[i].m_iframe->CalculateDiff(*m_teams[j].m_iframe, m_teams[j].m_colour, m_teams[j].m_syncDifferences);
...
MultiwiniaGameBlueprint::~MultiwiniaGameBlueprint() { m_params.EmptyAndDelete(); }
...
char* MultiwiniaGameBlueprint::GetDifficulty(int _type)
{
  ...
  case Multiwinia::GameTypeAssault:
    return "multiwinia_complexity_intermediate";

  case Multiwinia::GameTypeRocketRiot:
  case Multiwinia::GameTypeBlitzkreig:
    return "multiwinia_complexity_advanced";
  }
  return "multiwinia_complexity_basic";
}
...
char* MultiwiniaGameParameter::GetStringId() { return m_stringId; }
