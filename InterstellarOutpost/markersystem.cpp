...
void MarkerSystem::FindScreenEdge(const Vector2& _line, float* _posX, float* _posY)
{
...
bool MarkerSystem::CalculateScreenPosition(const Vector3& worldPos, float& screenX, float& screenY)
{
  int screenH = g_app->m_renderer->ScreenH();
  int screenW = g_app->m_renderer->ScreenW();
...
void MarkerSystem::CalculateScreenBox(float screenX, float screenY, float screenDX, float screenDY, float size, float& minX, float& minY,
...
void MarkerSystem::RenderShadow(float _screenX, float _screenY, float _screenDX, float _screenDY, float _size, float _alpha)
{
  ...
  glBegin(GL_QUADS);
  glTexCoord2i(0, 1);
  glVertex2dv((pos - rightAngle * _size / 2.0f - gradient * _size / 2.0f).GetData());
  glTexCoord2i(0, 0);
  glVertex2dv((pos - rightAngle * _size / 2.0f + gradient * _size / 2.0f).GetData());
  glTexCoord2i(1, 0);
  glVertex2dv((pos + rightAngle * _size / 2.0f + gradient * _size / 2.0f).GetData());
...
void MarkerSystem::RenderIcon(float _screenX, float _screenY, float _screenDX, float _screenDY, float _size, char* iconFilename,
                              RGBAColour colour, float rotation)
{
  Vector2 pos(_screenX, _screenY);
  Vector2 gradient(_screenDX, _screenDY);
  if (gradient.MagSquared() > 0)
  ...
  glBegin(GL_QUADS);
  glTexCoord2f(texOff.x + texPos.x, texOff.z + texPos.z);
  glVertex2dv((pos + rightAngle * _size / 2.0f + gradient * _size / 2.0f).GetData());
  texPos.RotateAroundY(-0.5f * M_PI);
  glTexCoord2f(texOff.x + texPos.x, texOff.z + texPos.z);
  glVertex2dv((pos + rightAngle * _size / 2.0f - gradient * _size / 2.0f).GetData());
  texPos.RotateAroundY(-0.5f * M_PI);
  glTexCoord2f(texOff.x + texPos.x, texOff.z + texPos.z);
...
void MarkerSystem::RenderColourCircle(float screenX, float screenY, float screenDX, float screenDY, float size, RGBAColour colour,
                                      float percent)
{
  Vector2 pos(screenX, screenY);
  Vector2 gradient(screenDX, screenDY);
  ...
  glBegin(GL_QUADS);
  glTexCoord2f(0, 1);
  glVertex2dv((pos - rightAngle * size / 2.0f + gradient * size / 2.0f).GetData());
  glTexCoord2f(0, 1 - percent);
  glVertex2dv((pos - rightAngle * size / 2.0f + gradient * size / 2.0f - gradient * totalHeight).GetData());
  glTexCoord2f(1, 1 - percent);
  glVertex2dv((pos + rightAngle * size / 2.0f + gradient * size / 2.0f - gradient * totalHeight).GetData());
  glTexCoord2f(1, 1);
  glVertex2dv((pos + rightAngle * size / 2.0f + gradient * size / 2.0f).GetData());
  glEnd();
...
{
  auto marker = new LocationMarker();
...
void MarkerSystem::RegisterMarker_Fixed(int teamId, const Vector3& pos, char* icon, bool fromCrate)
{
  auto marker = new LocationMarker();
...
void MarkerSystem::RegisterMarker_Orders(WorldObjectId id)
{
  ...
  if (entity && entity->m_type == Entity::TypeArmour)
    marker->m_pos = static_cast<Armour*>(entity)->m_wayPoint;
}
...
void MarkerSystem::SortMarkers()
{
  ...
  for (int i = 0; i < m_markers.Size() - 1; ++i)
  {
    ...
    if (thisMarker->m_teamId == g_app->m_globalWorld->m_myTeamId)
      thisDist -= 50.0f;
    if (nextMarker->m_teamId == g_app->m_globalWorld->m_myTeamId)
      nextDist -= 50.0f;
    ...
    if (thisMarker->m_objId == m_markerUnderMouse && thisMarker->m_type != LocationMarker::TypeAttachedToOfficer)
      thisDist = 0.0f;
    if (nextMarker->m_objId == m_markerUnderMouse && nextMarker->m_type != LocationMarker::TypeAttachedToOfficer)
      nextDist = 0.0f;

    // Move further away if we're attached to orders (so we always go behind the unit itself)

    if (thisMarker->m_type == LocationMarker::TypeAttachedToOrders)
      thisDist += 50;
    if (nextMarker->m_type == LocationMarker::TypeAttachedToOrders)
      nextDist += 50;
...
void MarkerSystem::Render()
{
  ...
  for (int i = 0; i < m_markers.Size(); ++i)
  {
    ...
    if (!onScreen)
    {
      ...
      lineNormal.Set(g_app->m_renderer->ScreenW() / 2.0f - screenX, g_app->m_renderer->ScreenH() / 2.0f - screenY);
      lineNormal.Normalise();
    }

    if (onScreen)
    {
      if (marker->m_onscreenTimer < 0.0f)
        marker->m_onscreenTimer = 0.0f;
      ...
      if (marker->m_hasBeenSeen && marker->m_onscreenTimer > 6.0f)
      {
      ...
      float toScreenCentre = sqrtf(
        pow(screenX - g_app->m_renderer->ScreenW() / 2.0f, 2) + pow(screenY - g_app->m_renderer->ScreenH() / 2.0f, 2));

      toScreenCentre /= (static_cast<float>(g_app->m_renderer->ScreenH()) * 0.5f);
    ...
    if (!marker->IsPersistant() && marker->m_hasBeenSeen && marker->m_onscreenTimer > 6.0f)
    {
    ...
    {
      bool teamSelectedThisMarker = g_app->m_location->GetMyTeam() && g_app->m_location->GetMyTeam()->GetMyEntity() && g_app->m_location->
        GetMyTeam()->GetMyEntity()->m_id == marker->m_objId;

      if (m_markerUnderMouse == marker->m_objId || g_app->m_gameCursor->m_objUnderMouse == marker->m_objId || teamSelectedThisMarker)
      {
        alpha = 1.0f;
        iconSize *= 1.2f;
        shadowSize *= 1.2f;
      ...
      if (marker->m_type != LocationMarker::TypeAttachedToOfficer && IsMOuseInside(screenX, screenY, lineNormal.x, lineNormal.y, iconSize))
      {
    ...
    case LocationMarker::TypeAttachedToCrate:
    {
      Crate* crate = static_cast<Crate*>(g_app->m_location->GetBuilding(marker->m_objId.GetUniqueId()));
      if (crate && crate->m_type == Building::TypeCrate)
      {
        ...
        RenderIcon(screenX, screenY, lineNormal.x, lineNormal.y, iconSize, "icons\\icon_crate.bmp", RGBAColour(255, 255, 255, 255 * alpha),
                   rotation);
      }
      break;
    }
    ...
    case LocationMarker::TypeAttachedToResearch:
    {
      ResearchItem* research = static_cast<ResearchItem*>(g_app->m_location->GetBuilding(marker->m_objId.GetUniqueId()));
    ...
    case LocationMarker::TypeAttachedToStatue:
    {
      Statue* statue = static_cast<Statue*>(g_app->m_location->GetBuilding(marker->m_objId.GetUniqueId()));
      if (statue && statue->m_type == Building::TypeStatue)
      {
        ...
        if (statue->m_id.GetTeamId() != 255)
        {
          ...
          percent = (statue->m_numLifters - statue->m_minLifters) / static_cast<float>(statue->m_maxLifters - statue->m_minLifters);
          alpha = 1.0f;
        }
      ...
      if (marker->m_teamId != -1)
      {
        ...
        if (task)
        {
          ...
          if (marker->m_fromCrate)
          {
            ...
            if (timePassed < 5)
              marker->m_rotation += pow(5 - timePassed, 2);
          }
          ...
          if (g_app->m_location->IsFriend(task->m_teamId, g_app->m_globalWorld->m_myTeamId))
          {
            ...
            auto tmm = static_cast<TaskManagerInterfaceMultiwinia*>(g_app->m_taskManagerInterface);
    ...
    case LocationMarker::TypeAttachedToOrders:
    {
      if (marker->m_teamId != -1)
      {
        ...
        if (task)
        {
          Armour* armour = static_cast<Armour*>(g_app->m_location->GetEntitySafe(task->m_objId, Entity::TypeArmour));
          ...
          if (marker->IsSelectable() && RaySphereIntersection(rayStart, rayDir, marker->m_pos, 8))
          {
    ...
    case LocationMarker::TypeFixed:
    {
      if (g_app->IsSinglePlayer() || marker->m_teamId != -1)
      {
        ...
        if (marker->m_fromCrate)
        {
          float timePassed = GetHighResTime() - marker->m_birthTime;
          if (timePassed < 2.5f)
            marker->m_rotation += pow(2.5f - timePassed, 2);
        }
    ...
    case LocationMarker::TypeAttachedToOfficer:
    {
      Officer* officer = static_cast<Officer*>(g_app->m_location->GetEntitySafe(marker->m_objId, Entity::TypeOfficer));
      ...
      if (officer && !officer->m_dead && officer->m_enabled)
      {
        ...
        Vector3 rayStart, rayDir;
        g_app->m_camera->GetClickRay(g_target->X(), g_target->Y(), &rayStart, &rayDir);
        if (officer->m_id.GetTeamId() == g_app->m_globalWorld->m_myTeamId && officer->m_flag.RayHit(rayStart, rayDir))
          m_newMarkerUnderMouse = marker->m_objId;

        officer->m_flag.m_isHighlighted = (m_markerUnderMouse == officer->m_id || g_app->m_gameCursor->m_objUnderMouse == officer->m_id);
...
void MarkerSystem::ClearAllMarkers() { m_markers.EmptyAndDelete(); }
...
LocationMarker::LocationMarker()
  : m_type(TypeUnknown),
    ...
    m_rotation(0.0f) { m_birthTime = GetHighResTime(); }

bool LocationMarker::IsPersistant()
{
  return (m_type == TypeAttachedToTask || m_type == TypeAttachedToCrate || m_type == TypeAttachedToStatue || m_type == TypeAttachedToOfficer
    || m_type == TypeAttachedToOrders);
}

bool LocationMarker::ShowOffscreen()
{
  if (m_type == TypeAttachedToStatue || m_type == TypeAttachedToCrate)
    return true;

  if (m_type == TypeAttachedToTask && m_teamId == g_app->m_globalWorld->m_myTeamId)
    return true;
  ...
  if (m_teamId == g_app->m_globalWorld->m_myTeamId)
  {
    ...
    if (m_type == TypeAttachedToOrders)
    {
      ...
      if (attached)
      {
        ...
        Entity* entity = team->GetMyEntity();
        if (!entity && team->m_numDarwiniansSelected == 0)
          return true;
        if (entity == attached)
          return true;
      }
...
bool LocationMarker::AlwaysAtFullAlpha()
{
  if (m_teamId == g_app->m_globalWorld->m_myTeamId)
  {
    if (m_type == TypeAttachedToTask || m_type == TypeAttachedToOrders)
      return true;
  }
...
bool LocationMarker::ShouldFlashAlert()
{
  ...
  if (m_type == TypeFixed && m_teamId != g_app->m_globalWorld->m_myTeamId)
    return true;
