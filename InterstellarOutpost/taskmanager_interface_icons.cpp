...
TaskManagerInterfaceIcons::TaskManagerInterfaceIcons()
  : m_screenId(ScreenTaskManager),
    ...
    m_objectivesChanged(NULL),
    m_progressBarFontSize(-1.0f),
    m_progressBarLongestStrSize(-1.0f),
    m_currentParticlesIndex(-1)
{
  InitProgressBarList();
  for (int i = 0; i < NUM_TASKMANAGER_ICONS_PARTICLES; i++)
    m_progressBarCompleted[i] = false;
  ...
  // Removed use of ['s and ]'s for cycling units as tab down is also a binding for ControlUnitCycleRight
  // This causes the task manager interface to do down once when you hold down tab to bring up the task manager! 
  ...
  m_keyboardShortcuts.PutData(new KeyboardShortcut("ScreenUp", -1, ControlTaskManagerScrollUp));
  m_keyboardShortcuts.PutData(new KeyboardShortcut("ScreenDown", -1, ControlTaskManagerScrollDown));
  m_keyboardShortcuts.PutData(new KeyboardShortcut("ScreenUp", -1, ControlXBoxMenuCycleUp));
  m_keyboardShortcuts.PutData(new KeyboardShortcut("ScreenDown", -1, ControlXBoxMenuCycleDown));
}
...
TaskManagerInterfaceIcons::~TaskManagerInterfaceIcons()
{
  ...
  delete [] m_objectivesChanged;
  m_progressBarList.EmptyAndDelete();
}
...
void TaskManagerInterfaceIcons::Advance()
{
  ...
  if (!IsVisible() && (g_inputManager->controlEvent(ControlIconsTaskManagerDisplay) || g_inputManager->controlEvent(
...
void TaskManagerInterfaceIcons::EvaluteTutorialObjectives()
{
  int size = m_progressBarList.Size();

  for (int i = 0; i < size; i++)
  {
    // N.B.: These objectives can only be done in order. 
    // For example, objective 3 or ANDed against objective 2
    if (m_progressBarCompleted[i])
      continue;
    ...
    GlobalBuilding* gb = NULL;
    Team* myTeam = g_app->m_location->GetMyTeam();
    bool gotAllSpawnPoints = true;

    switch (i)
    {
    case 0: // Capture radar dish on first island
      building = g_app->m_location->GetBuilding(66);
      if (myTeam && building && building->m_type == Building::TypeRadarDish)
        m_progressBarCompleted[0] = building->m_id.GetTeamId() == myTeam->m_teamId;
      break;
    case 1: // Capture solar panels on second island
      /*solarpanel = (SolarPanel*)g_app->m_location->GetBuilding(19);
      if( solarpanel && solarpanel->m_type == Building::TypeSolarPanel ) gotAllSpawnPoints &= solarpanel->IsOperating();
      else gotAllSpawnPoints = false;

      solarpanel = (SolarPanel*)g_app->m_location->GetBuilding(22);
      if( solarpanel && solarpanel->m_type == Building::TypeSolarPanel ) gotAllSpawnPoints &= solarpanel->IsOperating();
      else gotAllSpawnPoints = false;

      solarpanel = (SolarPanel*)g_app->m_location->GetBuilding(25);
      if( solarpanel && solarpanel->m_type == Building::TypeSolarPanel ) gotAllSpawnPoints &= solarpanel->IsOperating();
      else gotAllSpawnPoints = false;
      
      m_progressBarCompleted[1] = m_progressBarCompleted[0] && gotAllSpawnPoints;*/
      gb = g_app->m_globalWorld->GetBuilding(1, g_app->m_locationId);

      m_progressBarCompleted[1] = m_progressBarCompleted[0] && (gb ? gb->m_online : false);
      break;
    case 2: // Conquer 3rd island
      //m_progressBarCompleted[2] = m_progressBarCompleted[1] && !g_app->m_location->m_entityGrid->AreEnemiesPresent(2600.0f, 750.0f, 30.0f, 0 );
      item = static_cast<ResearchItem*>(g_app->m_location->GetBuilding(130));
      if (!item)
        m_progressBarCompleted[2] = m_progressBarCompleted[1];
      else if (item->m_type == Building::TypeResearchItem)
        m_progressBarCompleted[2] = m_progressBarCompleted[1] && !item->NeedsReprogram();
      break;
    case 3: // Capture solar panels on fourth island
      /*solarpanel = (SolarPanel*)g_app->m_location->GetBuilding(41);
      if( solarpanel && solarpanel->m_type == Building::TypeSolarPanel ) gotAllSpawnPoints &= solarpanel->IsOperating();
      else gotAllSpawnPoints = false;

      solarpanel = (SolarPanel*)g_app->m_location->GetBuilding(42);
      if( solarpanel && solarpanel->m_type == Building::TypeSolarPanel ) gotAllSpawnPoints &= solarpanel->IsOperating();
      else gotAllSpawnPoints = false;

      solarpanel = (SolarPanel*)g_app->m_location->GetBuilding(44);
      if( solarpanel && solarpanel->m_type == Building::TypeSolarPanel ) gotAllSpawnPoints &= solarpanel->IsOperating();
      else gotAllSpawnPoints = false;

      solarpanel = (SolarPanel*)g_app->m_location->GetBuilding(45);
      if( solarpanel && solarpanel->m_type == Building::TypeSolarPanel ) gotAllSpawnPoints &= solarpanel->IsOperating();
      else gotAllSpawnPoints = false;

      solarpanel = (SolarPanel*)g_app->m_location->GetBuilding(24);
      if( solarpanel && solarpanel->m_type == Building::TypeSolarPanel ) gotAllSpawnPoints &= solarpanel->IsOperating();
      else gotAllSpawnPoints = false;

      solarpanel = (SolarPanel*)g_app->m_location->GetBuilding(26);
      if( solarpanel && solarpanel->m_type == Building::TypeSolarPanel ) gotAllSpawnPoints &= solarpanel->IsOperating();
      else gotAllSpawnPoints = false;

      m_progressBarCompleted[3] = m_progressBarCompleted[2] && gotAllSpawnPoints;*/

      gotAllSpawnPoints = false;
      gb = g_app->m_globalWorld->GetBuilding(2, g_app->m_locationId);
      if (gb)
        gotAllSpawnPoints = gb->m_online;
      gb = g_app->m_globalWorld->GetBuilding(3, g_app->m_locationId);

      m_progressBarCompleted[3] = m_progressBarCompleted[2] && gotAllSpawnPoints && (gb ? gb->m_online : false);
      break;
    case 4: // Recapture centre island
      spawnpoint = static_cast<SpawnPoint*>(g_app->m_location->GetBuilding(18));
      if (myTeam && spawnpoint && spawnpoint->m_type == Building::TypeSpawnPoint)
        gotAllSpawnPoints &= spawnpoint->m_id.GetTeamId() == 0;
      else
        gotAllSpawnPoints = false;

      spawnpoint = static_cast<SpawnPoint*>(g_app->m_location->GetBuilding(51));
      if (myTeam && spawnpoint && spawnpoint->m_type == Building::TypeSpawnPoint)
        gotAllSpawnPoints &= spawnpoint->m_id.GetTeamId() == 0;
      else
        gotAllSpawnPoints = false;

      spawnpoint = static_cast<SpawnPoint*>(g_app->m_location->GetBuilding(52));
      if (myTeam && spawnpoint && spawnpoint->m_type == Building::TypeSpawnPoint)
        gotAllSpawnPoints &= spawnpoint->m_id.GetTeamId() == 0;
      else
        gotAllSpawnPoints = false;

      spawnpoint = static_cast<SpawnPoint*>(g_app->m_location->GetBuilding(54));
      if (myTeam && spawnpoint && spawnpoint->m_type == Building::TypeSpawnPoint)
        gotAllSpawnPoints &= spawnpoint->m_id.GetTeamId() == 0;
      else
        gotAllSpawnPoints = false;

      m_progressBarCompleted[4] = m_progressBarCompleted[3] && gotAllSpawnPoints;
      break;
...
void TaskManagerInterfaceIcons::AdvanceProgressBar()
{
  //
  // Advance the objectives display

  int size = m_progressBarList.Size();
  int particlesPerObj = NUM_TASKMANAGER_ICONS_PARTICLES / size;

  // First time we've looked at the objectives? If so, make some variables work!
  if (m_objectivesChanged == NULL)
  {
    if (size > 0)
    {
      m_objectivesChanged = new bool[size];
      for (int i = 0; i < size; i++)
        m_objectivesChanged[i] = false;
    }
  }

  EvaluteTutorialObjectives();

  // Degrade the particles
  bool setParticlesIndex = false;
  m_currentParticlesIndex = -1;
  for (int i = 0; i < NUM_TASKMANAGER_ICONS_PARTICLES; i++)
  {
    if (m_objectiveParticles[i].alpha > 0.0f)
    {
      if (!setParticlesIndex)
      {
        m_currentParticlesIndex = i / particlesPerObj;
        setParticlesIndex = true;
      }
      m_objectiveParticles[i].alpha -= frand(0.015f) + 0.015f;
      m_objectiveParticles[i].pos += m_objectiveParticles[i].speed;
    }
  }

  // Check for objective changes
  if (m_objectivesChanged)
  {
    for (int i = 0; i < size; i++)
    {
      if (m_progressBarList.ValidIndex(i))
      {
        if (m_objectivesChanged[i] != m_progressBarCompleted[i])
        {
          float boxSize = g_app->m_renderer->ScreenW() / 25;
          float xPos = (g_app->m_renderer->ScreenW() / 2) - ((m_progressBarList.Size() / 2.0f) * boxSize);
          float yPos = g_app->m_renderer->ScreenH() / 10;
          xPos += boxSize * i;
          xPos += boxSize / 4;
          yPos += boxSize / 4;

          for (int j = 0; j < particlesPerObj; j++)
          {
            float xSpeed, ySpeed, lineSpeed, angle;
            lineSpeed = frand(2.0f);
            angle = frand(2 * M_PI);
            xSpeed = cosf(angle) * lineSpeed;
            ySpeed = sinf(angle) * lineSpeed;

            m_objectiveParticles[(i * particlesPerObj) + j].pos = Vector2(xPos, yPos);
            m_objectiveParticles[(i * particlesPerObj) + j].speed = Vector2(xSpeed, ySpeed);
            m_objectiveParticles[(i * particlesPerObj) + j].alpha = 1.0f;
            m_objectiveParticles[(i * particlesPerObj) + j].size = g_app->m_renderer->ScreenW() / 50;
          }

          // LEANDER : Add a decent DING! sound here
          //g_app->m_soundSystem->TriggerOtherEvent( NULL, "", SoundSourceBlueprint::Type... );

          m_objectivesChanged[i] = m_progressBarCompleted[i];
        }
      }
    }
  }
...
void TaskManagerInterfaceIcons::AdvanceScrolling()
{
  if (m_screenY != m_desiredScreenY)
  {
    ...
    if (abs(m_desiredScreenY - m_screenY) < 0.001f)
...
{
  bool scrollPermitted = abs(m_desiredScreenY - m_screenY) == 0;
...
void TaskManagerInterfaceIcons::SetupRenderMatrices(int _screenId)
{
  ...
  case ScreenTaskManager:
    left += m_screenY * m_screenH;
    break;

  case ScreenObjectives:
    left += (m_screenY - 1.0f) * m_screenH;
    break;
...
void TaskManagerInterfaceIcons::Render()
{
  ...
  if (g_app->m_locationId != -1 && g_app->m_locationId == g_app->m_globalWorld->GetLocationId("launchpad") && !m_visible)
  {
    // Render tutorial location objective boxes if the taskmanager isn't visible
    RenderProgressBar();
  }
...
void TaskManagerInterfaceIcons::InitProgressBarList()
{
  float fontSmall, fontMed, fontLarge;
  GetFontSizes(fontLarge, fontMed, fontSmall);
  m_progressBarFontSize = fontSmall;

  m_progressBarList.Empty();
  m_progressBarList.PutData(new UnicodeString(LANGUAGEPHRASE("prologue_objective_1")));
  m_progressBarList.PutData(new UnicodeString(LANGUAGEPHRASE("prologue_objective_2")));
  m_progressBarList.PutData(new UnicodeString(LANGUAGEPHRASE("prologue_objective_3")));
  m_progressBarList.PutData(new UnicodeString(LANGUAGEPHRASE("prologue_objective_4")));
  m_progressBarList.PutData(new UnicodeString(LANGUAGEPHRASE("prologue_objective_5")));

  m_progressBarLongestStrSize = -1;
  for (int i = 0; i < m_progressBarList.Size(); i++)
  {
    UnicodeString tempStr(*m_progressBarList.GetData(i));
    m_progressBarLongestStrSize = max(m_progressBarLongestStrSize, g_titleFont.GetTextWidthReal(tempStr, m_progressBarFontSize));
  }
}

void TaskManagerInterfaceIcons::RenderProgressBar()
{
#ifdef USE_SEPULVEDA_HELP_TUTORIAL
  ...
  int particleEffectObjective = -1;
  bool completedAll = true;
  for (int i = 0; i < m_progressBarList.Size(); i++)
  {
    completedAll &= m_progressBarCompleted[i];
    if (!m_progressBarCompleted[i])
    {
      currentObjective = i;
      i = m_progressBarList.Size();
    }
  }

  if (!completedAll && currentObjective < 1)
    return;

  float screenW = g_app->m_renderer->ScreenW();
  float screenH = g_app->m_renderer->ScreenH();
  float boxSize = g_app->m_renderer->ScreenW() / 25;
  float xPos = (screenW / 2) - ((m_progressBarList.Size() / 2.0f) * boxSize);
  float yPos = screenH / 10;

  const float oldXPos = xPos, oldYPos = yPos;
  float modifier = boxSize / 4;
  float backgroundSizeInc = boxSize * 0.25f;
  float backgroundXPos = (screenW / 2) - ((m_progressBarLongestStrSize * 1.1f) / 2);
  ...
  glBegin(GL_QUADS);
  glVertex2f(backgroundXPos, yPos - backgroundSizeInc);
  glVertex2f(backgroundXPos + (m_progressBarLongestStrSize * 1.2f), yPos - backgroundSizeInc);
  glVertex2f(backgroundXPos + (m_progressBarLongestStrSize * 1.2f), yPos + (boxSize * 2));
  glVertex2f(backgroundXPos, yPos + (boxSize * 2));
  glEnd();
  ...
  // As long as we haven't completed all progress bar objectives
  if (currentObjective != -1)
  {
    //
    // Current objective highlight box

    float bounceModifier = /*sinf(GetHighResTime()) */ 5.0f;

    glColor4f(1.0f, 1.0f, 1.0f, 0.4f);
    xPos += boxSize * (particleEffectObjective != -1 ? particleEffectObjective : currentObjective);
    glBegin(GL_QUADS);
    glVertex2f(xPos + bounceModifier, yPos + bounceModifier);
    glVertex2f(xPos + boxSize - bounceModifier, yPos + bounceModifier);
    glVertex2f(xPos + boxSize - bounceModifier, yPos + boxSize - bounceModifier);
    glVertex2f(xPos + bounceModifier, yPos + boxSize - bounceModifier);
    glEnd();

    //
    // Current objective highlight box outline
  ...
  for (int i = 0; i < m_progressBarList.Size(); i++)
  {
    ...
    glVertex2f(xPos + boxSize - modifier, yPos + modifier);
    glVertex2f(xPos + boxSize - modifier, yPos + boxSize - modifier);
    glVertex2f(xPos + modifier, yPos + boxSize - modifier);
    glEnd();
    xPos += boxSize;
  }
  ...
  UnicodeString objective = currentObjective == -1
    ? LANGUAGEPHRASE("prologue_objective_complete")
    : *m_progressBarList.GetData(m_currentParticlesIndex != -1 ? m_currentParticlesIndex : currentObjective);
  glColor4f(1.0f, 1.0f, 1.0f, 0.0f);
  g_titleFont.SetRenderOutline(true);
  g_titleFont.DrawText2DCentre(screenW / 2, oldYPos + (boxSize * 1.5), m_progressBarFontSize, objective);
  glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
  ...
  for (int i = 0; i < m_progressBarList.Size(); i++)
  {
    if (m_progressBarCompleted[i])
    {
      ...
      glVertex2f(xPos + boxSize, yPos);
      glTexCoord2f(1.0f, 0.0f);
      glVertex2f(xPos + boxSize, yPos + boxSize);
      glTexCoord2f(0.0f, 0.0f);
      glVertex2f(xPos, yPos + boxSize);
      glEnd();
    }
    xPos += boxSize;
  }
...
void TaskManagerInterfaceIcons::AdvanceScreenZones()
{
  ...
  if (m_currentScreenZone != -1)
  {
    ...
    if (currentZone)
    {
      if (stricmp(currentZone->m_name, "SelectTask") == 0 && g_app->m_location->GetMyTeam() && g_app->m_location->GetMyTeam()->m_taskManager
        ->m_tasks.ValidIndex(currentZone->m_data))
      {
...
void TaskManagerInterfaceIcons::RunScreenZone(const char* _name, int _data)
{
  ...
  if (stricmp(_name, "NewTask") == 0)
  {
    ...
    {
      if (_data == GlobalResearch::TypeSquad && g_app->m_globalWorld)
      {
        ...
        if (index == 12)
          ...
          index--;
        int squads = g_app->m_globalWorld->m_spSquaddiesUsed[index] == 3599999 ? 1 : (g_app->m_globalWorld->m_spSquaddiesUsed[index] + 1);
        g_app->m_globalWorld->m_spSquaddiesUsed[index] = squads;
        DebugTrace("Now have run %d squaddies for location index %d\n", g_app->m_globalWorld->m_spSquaddiesUsed[index], index);
      }
...
void TaskManagerInterfaceIcons::RenderTooltip()
{
  ...
  if (m_screenZones.ValidIndex(m_currentScreenZone))
  {
    ...
    if (timeSoFar < timeRequired)
    {
      ...
      clipped.m_unicodestring[static_cast<int>(clipped.Length() * fraction)] = '\x0';
    }
    ...
    if (g_inputManager->getInputMode() != INPUT_MODE_GAMEPAD)
    {
      ...
      if (selectedShortcut)
      {
        ...
        sprintf(caption, " : %s", selectedShortcut->noun().c_str());
...
void TaskManagerInterfaceIcons::RenderMessages()
{
  if (m_currentMessageType != -1)
  {
    ...
    static bool completedObjs = false;
    if (!completedObjs && m_currentMessageType == MessageObjectivesComplete)
    {
      int index = g_app->m_requestedLocationId;
      if (index == 12)
        index = 0;
        ...
        index--;
      DebugTrace("Mission is completed - %f - %f\n", g_app->m_location->m_levelFile->m_leaderboardLevelTime, g_gameTimer.GetGameTime());
      g_app->m_location->m_levelFile->m_leaderboardLevelTime += g_gameTimer.GetGameTime();
      g_app->m_globalWorld->m_spLevelTimes[index] = min(g_app->m_location->m_levelFile->m_leaderboardLevelTime,
                                                        static_cast<double>(g_app->m_globalWorld->m_spLevelTimes[index]));
      completedObjs = true;
    }
    ...
    {
      //size = sqrt( 2.7f - timeRemaining ) * 30.0f;        
      size += sqrt(2.0f - timeRemaining) * 15.0f;
    }
...
void TaskManagerInterfaceIcons::RenderTargetAreas()
{
  ...
  if (task && task->m_state == Task::StateStarted)
  {
    ...
    if (targetAreas->Size() == 1 && targetAreas->GetPointer(0)->m_centre == g_zeroVector && targetAreas->GetPointer(0)->m_radius > 9999)
    {
      delete targetAreas;
      return;
    }
...
void TaskManagerInterfaceIcons::RenderTaskManager()
{
  ...
  UnicodeString str(LANGUAGEPHRASE("taskmanager_taskmanager"));
  int textLen = g_gameFont.GetTextWidthReal(str, 50) / 2;
  g_gameFont.SetRenderOutline(true);
  glColor4f(0.8f, 0.8f, 0.8f, 0.0f);
  g_gameFont.DrawText2D(m_screenW / 2.0f - textLen, 30, 50, str);
  g_gameFont.SetRenderOutline(false);
  glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
  g_gameFont.DrawText2D(m_screenW / 2.0f - textLen, 30, 50, str);
  ...
  if (abs(m_screenY) < 0.1f)
  {
    ...
    auto zoneLeft = new ScreenZone("ScreenUp", "newcontrols_showresearch", 10, m_screenH - 60, 20, 40, -1);
    auto zoneRight = new ScreenZone("ScreenDown", "newcontrols_showobjectives", m_screenW - 50, m_screenH - 60, 20, 40, -1);
    /*ScreenZone *zoneLeft        = new ScreenZone( "ScreenUp", "newcontrols_showresearch",    m_screenW-60, 10, 40, 20, -1 );
    ScreenZone *zoneRight       = new ScreenZone( "ScreenDown", "newcontrols_showobjectives",  m_screenW-60, m_screenH-50, 40, 20, -1 );
    */
...
void TaskManagerInterfaceIcons::RenderCreateTaskMenu()
{
  ...
  int runnableTaskType[] = {GlobalResearch::TypeSquad, GlobalResearch::TypeEngineer, GlobalResearch::TypeOfficer,
                            GlobalResearch::TypeArmour, GlobalResearch::TypeHarvester};
  ...
  for (int i = 0; i < numRunnableTasks; ++i)
  {
    ...
    if (g_app->m_globalWorld->m_research->HasResearch(taskType))
    {
      ...
      auto zone = new ScreenZone("NewTask", tooltipId, x + 5, y - h / 3, w - 10, h, taskType);
      zone->m_scrollZone = 1;
      ...
      GlobalResearch::GetTypeNameTranslated(taskType, name);
      g_gameFont.SetRenderOutline(true);
      glColor4f(0.8f, 0.8f, 0.8f, 0.0f);
      g_gameFont.DrawText2D(x + 70, y + 5, 18, name);
      g_gameFont.SetRenderOutline(false);
...
void TaskManagerInterfaceIcons::RenderTitleBar()
{
  ...
  glTexCoord2i(1, 0);
  glVertex2i(m_screenW * 2, 0);
  glTexCoord2i(1, 1);
  glVertex2i(m_screenW * 2, panelW);
  glTexCoord2i(0, 1);
  glVertex2i(-m_screenW, panelW);
...
void TaskManagerInterfaceIcons::RenderRunningTasks()
{
  ...
  for (int i = 0; i < numTasks; ++i)
  {
    ...
    auto zone = new ScreenZone("SelectTask", captionId, iconX - iconSize / 2, iconY - iconSize / 2, iconSize, iconSize, i);
    m_newScreenZones.PutData(zone);
    zone->m_scrollZone = 2;
    ...
    if (!invisible)
    {
      ...
      else if (task->m_type == GlobalResearch::TypeBooster && task->m_state == Task::StateRunning)
      {
        g_gameFont.SetRenderShadow(true);
        glColor4f(1.0f, 1.0f, 1.0f, 0.0f);
        g_gameFont.DrawText2DCentre(iconCentre.x + 3, iconCentre.y + 2, 12, "%2.0f", task->m_lifeTimer);
      ...
      {
        RenderCompass(iconCentre.x, iconCentre.y, taskWorldPos,
                      task->m_id == g_app->m_location->GetMyTeam()->m_taskManager->m_currentTaskId, compassSize);
      }

      if (task->m_state != Task::StateStarted && m_visible && task->m_id == g_app->m_location->GetMyTeam()->m_taskManager->m_currentTaskId)
      {
        if (task->m_type == GlobalResearch::TypeSquad)
        {
          ...
          InsertionSquad* squad = static_cast<InsertionSquad*>(unit);
          int currentWeapon = -1;
          if (squad)
          ...
          for (int i = 0; i < availableWeapons.Size(); ++i)
          {
            ...
            ScreenZone* zone = new ScreenZone("SelectWeapon", captionId, weaponX - weaponSize / 2, weaponY - weaponSize / 2, weaponSize,
    ...
    if (m_visible)
    {
      ...
      ScreenZone* zone = new ScreenZone("DeleteTask", captionId, deleteX - deleteSize / 2.0f, deleteY - deleteSize / 2.0f, deleteSize,
...
void TaskManagerInterfaceIcons::RenderObjectives()
{
  ...
  UnicodeString str(LANGUAGEPHRASE("taskmanager_objectives"));
  int textLen = g_gameFont.GetTextWidthReal(str, 50) / 2;
  g_gameFont.SetRenderOutline(true);
  glColor4f(0.8f, 0.8f, 0.8f, 0.0f);
  g_gameFont.DrawText2D(m_screenW / 2.0f - textLen, 30, 50, str);
  ...
  for (int o = 0; o < 2; ++o)
  {
    ...
    g_gameFont.SetRenderShadow(true);
    glColor4ub(255, 255, 150, 30);
    const UnicodeString theString = (o == 0 ? LANGUAGEPHRASE("taskmanager_primarys") : LANGUAGEPHRASE("taskmanager_secondarys"));
    ...
    for (int i = 0; i < numObjectives; ++i)
    {
      ...
      if (o == 1)
        objectiveId *= -1;
      auto zone = new ScreenZone("Objective", "help_explainobjective", boxX + 10, m_screenH + textY - textH / 2, boxW - 20, textH * 1.5f,
                                 objectiveId);
      zone->m_scrollZone = 1;
      m_newScreenZones.PutData(zone);
      ...
      const char* completedString = (completed ? "taskmanager_complete" : "taskmanager_incomplete");
      g_gameFont.SetRenderOutline(true);
      glColor4f(0.8f, 0.8f, 0.8f, 0.0f);
  ...
  // 
  // Time/Kills/Survivors/Squaddies
  // Background box

  float boxY = textY - textH;
  float boxH = 2 * (textH * 1.5f) + textH * 0.75f;
  float titleHeight = 20.0f;
  ...
  g_gameFont.SetRenderShadow(true);
  glColor4ub(255, 255, 150, 30);
  const UnicodeString theString = LANGUAGEPHRASE("taskmanager_statistics");
  g_gameFont.DrawText2DCentre(boxX + boxW / 2.0f, boxY - titleHeight / 2 + 1, 13, theString);
  g_gameFont.DrawText2DCentre(boxX + boxW / 2.0f, boxY - titleHeight / 2 + 1, 13, theString);
  g_gameFont.SetRenderShadow(false);
  ...
  // Actual stats
  char temp[256];
  UnicodeString theUString;
  int hours = 0, mins = 0, seconds = 0;
  float timeNow = g_app->m_location->m_levelFile->m_leaderboardLevelTime + (g_app->m_location->MissionComplete()
    ? 0
    : g_gameTimer.GetGameTime());
  float textW = (boxW / 4);
  int index = g_app->m_locationId;
  if (index == 12)
    index = 0;
  else
    index--;
  ...
  for (int i = 0; i < 4; ++i)
  {
    ...
    {
    case 0:
      theUString = LANGUAGEPHRASE("taskmanager_time");
      break;
    case 1:
      theUString = LANGUAGEPHRASE("taskmanager_kills");
      break;
    case 2:
      theUString = LANGUAGEPHRASE("taskmanager_survivors");
      break;
    case 3:
      theUString = LANGUAGEPHRASE("taskmanager_squaddies");
      break;
    }

    textX = boxX + (textW / 4) + (textW * i);
    g_gameFont.SetRenderOutline(true);
    glColor4f(0.8f, 0.8f, 0.8f, 0.0f);
    g_gameFont.DrawText2D(textX, textY, textH * 0.65f, theUString);
    g_gameFont.SetRenderOutline(false);
    glColor4f(0.8f, 0.8f, 1.0f, 1.0f);
    g_gameFont.DrawText2D(textX, textY, textH * 0.65f, theUString);
    ...
    {
    case 0:
      hours = static_cast<int>(timeNow / 3600.0f);
      mins = static_cast<int>((timeNow - static_cast<float>(hours)) / 60.0f);
      seconds = static_cast<int>(timeNow) % 60;
      sprintf(temp, "%s%d:%s%d:%s%d", hours < 10 ? "0" : "", hours, mins < 10 ? "0" : "", mins, seconds < 10 ? "0" : "", seconds);
      break;
    case 1:
      // Only player team kills count
      sprintf(temp, "%d", g_app->m_location->m_teams[2]->m_teamKills);
      break;
    case 2:
      // Only team 0 Darwinians count as survivors.
      sprintf(temp, "%d", g_app->m_location->m_teams[0]->m_others.NumUsed());
      break;
    case 3:
      sprintf(temp, "%d", g_app->m_globalWorld->m_spSquaddiesUsed[index] == 3599999 ? 0 : g_app->m_globalWorld->m_spSquaddiesUsed[index]);
      break;
    }
    theUString = LANGUAGEPHRASE(temp);

    g_gameFont.SetRenderOutline(true);
    glColor4f(0.8f, 0.8f, 0.8f, 0.0f);
    g_gameFont.DrawText2D(textX, textY + textH * 1.5f, textH * 0.65f, theUString);
    g_gameFont.SetRenderOutline(false);
    glColor4f(0.8f, 0.8f, 1.0f, 1.0f);
    g_gameFont.DrawText2D(textX, textY + textH * 1.5f, textH * 0.65f, theUString);
  }
  ...
  if (m_screenY > 0.9f)
  {
    ...
    auto zoneLeft = new ScreenZone("ScreenUp", "newcontrols_showtaskmanager", m_screenW + 10, m_screenH - 60, 20, 40, -1);
    //ScreenZone *zoneLeft = new ScreenZone( "ScreenUp", "newcontrols_showtaskmanager",    m_screenW-60, m_screenH+10, 40, 20, -1 );
...
void TaskManagerInterfaceIcons::RenderResearch()
{
  ...
  UnicodeString str(LANGUAGEPHRASE("taskmanager_research"));
  int textLen = g_gameFont.GetTextWidthReal(str, 50) / 2;
  g_gameFont.SetRenderOutline(true);
  glColor4f(0.8f, 0.8f, 0.8f, 0.0f);
  g_gameFont.DrawText2D(m_screenW / 2.0f - textLen, 30, 50, str);
  ...
  g_gameFont.SetRenderShadow(true);
  glColor4ub(255, 255, 150, 30);
  const UnicodeString theString(LANGUAGEPHRASE("taskmanager_research"));
  g_gameFont.DrawText2DCentre(boxX + boxW / 2.0f, boxY - (titleHeight / 2), 13, theString);
  g_gameFont.DrawText2DCentre(boxX + boxW / 2.0f, boxY - (titleHeight / 2), 13, theString);
  g_gameFont.SetRenderShadow(false);
  ...
  for (int i = 0; i < GlobalResearch::NumResearchItems; ++i)
  {
    if (g_app->m_globalWorld->m_research->HasResearch(i))
    {
      ...
      auto zone = new ScreenZone("Research", tooltipId, 40, -m_screenH + iconY, m_screenW - 160, iconSize, i);
      zone->m_scrollZone = 1;
      m_newScreenZones.PutData(zone);
      ...
      for (int level = 1; level < 4; ++level)
      {
        ...
        if (researchLevel == level && researchProgress > 0)
        {
          ...
          if (g_app->m_globalWorld->m_research->m_currentResearch == i)
          {
            float alpha = abs(sin(g_gameTime * 2));
            glColor4f(0.9f, 0.9f, 1.0f, alpha);
            glBlendFunc(GL_SRC_ALPHA, GL_ONE);
            glBegin(GL_QUADS);
            glTexCoord2i(0, 0);
  ...
  if (m_screenY < 0.1f)
  {
    ...
    auto zoneRight = new ScreenZone("ScreenDown", "newcontrols_showtaskmanager", -50, m_screenH - 60, 20, 40, -1);
    //ScreenZone *zoneRight = new ScreenZone( "ScreenDown", "newcontrols_showtaskmanager",  m_screenW-60, -50, 40, 20, -1 );
...
bool TaskManagerInterfaceIcons::AdviseCreateControlHelpBlue()
{
  return m_currentScreenZone != -1 && ButtonHeld() && g_inputManager->controlEvent(ControlIconsTaskManagerDisplayPressed) && g_app->
    m_location->GetMyTeam() && g_app->m_location->GetMyTeam()->m_taskManager->CapacityUsed() < g_app->m_location->GetMyTeam()->m_taskManager
    ->Capacity();
}
...
bool TaskManagerInterfaceIcons::AdviseCreateControlHelpGreen()
{
  ...
  return m_currentScreenZone != -1 && g_app->m_location->GetMyTeam() && g_app->m_location->GetMyTeam()->m_taskManager->CapacityUsed() <
    g_app->m_location->GetMyTeam()->m_taskManager->Capacity() && (screenZone = m_screenZones[m_currentScreenZone]) && strcmp(
      screenZone->m_name, "NewTask") == 0;
}
