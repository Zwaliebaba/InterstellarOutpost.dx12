...
{
	InitProgressBarList();
	for( int i = 0; i < NUM_TASKMANAGER_ICONS_PARTICLES; i++ ) m_progressBarCompleted[i] = false;
...
void TaskManagerInterfaceIcons::EvaluteTutorialObjectives()
{
	...
	for( int i = 0; i < size; i++ )
	{
		...
		if( m_progressBarCompleted[i] ) continue;
		...
		switch( i )
		{
			...
			case 2:		// Conquer 3rd island
				...
				item = (ResearchItem*) g_app->m_location->GetBuilding(130);
				if( !item )
			...
			case 4:		// Recapture centre island
				...
				if( myTeam && spawnpoint && spawnpoint->m_type == Building::TypeSpawnPoint ) gotAllSpawnPoints &= spawnpoint->m_id.GetTeamId() == 0;
				else gotAllSpawnPoints = false;
...
void TaskManagerInterfaceIcons::InitProgressBarList()
{
	...
	for( int i = 0; i < m_progressBarList.Size(); i++ )
	{
		...
		m_progressBarLongestStrSize = max(
			m_progressBarLongestStrSize, 
			g_titleFont.GetTextWidthReal(tempStr,m_progressBarFontSize)
		);
	}
}
...
{
#ifdef USE_SEPULVEDA_HELP_TUTORIAL
	if( g_app->m_sepulveda && g_app->m_sepulveda->IsInCutsceneMode() ) return;
#endif
	...
	UnicodeString objective = currentObjective == -1 ? LANGUAGEPHRASE("prologue_objective_complete") : *m_progressBarList.GetData(m_currentParticlesIndex != -1 ? m_currentParticlesIndex : currentObjective);
	...
	for( int i = 0; i < m_progressBarList.Size(); i++ )
	{
		if( m_progressBarCompleted[i] )
		{
			...
			glBegin(GL_QUADS);
				...
				glTexCoord2f(1.0f,1.0f);	glVertex2f(xPos+boxSize,	yPos);
				glTexCoord2f(1.0f,0.0f);	glVertex2f(xPos+boxSize,	yPos+boxSize);
				glTexCoord2f(0.0f,0.0f);	glVertex2f(xPos,			yPos+boxSize);
			glEnd();
		}
		xPos += boxSize;
	}
	...
	for( int i = 0; i < NUM_TASKMANAGER_ICONS_PARTICLES; i++ )
	{
		if( m_objectiveParticles[i].alpha > 0.0f )
		{
			...
			glBegin(GL_QUADS);
				...
				glTexCoord2f(1.0f,0.0f);	glVertex2f(pos.x+size,	pos.y);
				glTexCoord2f(1.0f,1.0f);	glVertex2f(pos.x+size,	pos.y+size);
				glTexCoord2f(0.0f,1.0f);	glVertex2f(pos.x,		pos.y+size);
			glEnd();
		}
	}
	glBlendFunc     ( GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA );
        ...
        if( g_app->m_globalWorld->m_research->HasResearch( _data ) )
        {
			if( _data == GlobalResearch::TypeSquad && g_app->m_globalWorld )
			{
				...
				DebugTrace("Now have run %d squaddies for location index %d\n", 
					g_app->m_globalWorld->m_spSquaddiesUsed[index],
					index);
			}
...
void TaskManagerInterfaceIcons::RenderMessages()
{
    if( m_currentMessageType != -1 )
    {
		...
		if( !completedObjs && m_currentMessageType == MessageObjectivesComplete )
		{
			...
			g_app->m_globalWorld->m_spLevelTimes[index] = min(g_app->m_location->m_levelFile->m_leaderboardLevelTime, (double)g_app->m_globalWorld->m_spLevelTimes[index]);
			completedObjs = true;
		}
...
void TaskManagerInterfaceIcons::RenderTaskManager()
{
    ...
    {        
        ...
        ScreenZone *zoneLeft        = new ScreenZone( "ScreenUp", "newcontrols_showresearch",    10, m_screenH-60, 20, 40, -1 );
        ScreenZone *zoneRight       = new ScreenZone( "ScreenDown", "newcontrols_showobjectives",  m_screenW-50, m_screenH-60, 20, 40, -1 );
...
void TaskManagerInterfaceIcons::RenderCreateTaskMenu()
{
    ...
    for( int i = 0; i < numRunnableTasks; ++i )
    {
        ...
        if( g_app->m_globalWorld->m_research->HasResearch(taskType) )
        {
            ...
            ScreenZone *zone = new ScreenZone( "NewTask", tooltipId, x+5, y-h/3, w-10, h, taskType );
            zone->m_scrollZone = 1;
...
void TaskManagerInterfaceIcons::RenderTitleBar()
{
    ...
    glBegin( GL_QUADS );
        glTexCoord2i(0,0);      glVertex2i( -m_screenW, 0 );
        glTexCoord2i(1,0);      glVertex2i( m_screenW*2, 0 );
        glTexCoord2i(1,1);      glVertex2i( m_screenW*2, panelW );
        glTexCoord2i(0,1);      glVertex2i( -m_screenW, panelW );
                ...
                task->m_id == g_app->m_location->GetMyTeam()->m_taskManager->m_currentTaskId )
            {
                if( task->m_type == GlobalResearch::TypeSquad )
                {
                    ...
                    for( int i = 0; i < availableWeapons.Size(); ++i )
                    {
                        ...
                        ScreenZone *zone = new ScreenZone( "SelectWeapon", captionId,
...
{    
    ...
    for( int o = 0; o < 2; ++o )
    {
        ...
        g_gameFont.SetRenderShadow(true);
        glColor4ub( 255, 255, 150, 30 );
        const UnicodeString theString = ( o == 0 ? 
	...
	float timeNow = g_app->m_location->m_levelFile->m_leaderboardLevelTime + (g_app->m_location->MissionComplete() ? 0 : g_gameTimer.GetGameTime());
	float textW = (boxW / 4);
	...
	for( int i = 0; i < 4; ++i )
	{
		...
		{
			case 0:
				hours = (int)(timeNow / 3600.0f);
				mins = (int)((timeNow - (float) hours)/60.0f);
    ...
    {        
        ...
        ScreenZone *zoneLeft = new ScreenZone( "ScreenUp", "newcontrols_showtaskmanager",    m_screenW+10, m_screenH-60, 20, 40, -1 );
...
void TaskManagerInterfaceIcons::RenderResearch()
{
    ...
    {        
        ...
        ScreenZone *zoneRight = new ScreenZone( "ScreenDown", "newcontrols_showtaskmanager",  -50, m_screenH-60, 20, 40, -1 );
