...
bool Gesture::CalculateFeatureSet()
{
    ...
    double sumAbsoluteAngles = 0.0;
    double sumSquaredAngles = 0.0;
        ...
        for( int j = 0; j <= GESTURE_MAX_FEATURES; ++j )
        {
            ...
            for( int c = 0; c < numSymbols; ++c )
            {
                coVarianceSum += coVarianceMatrix[c].z[i][j] /
                                 (double) m_symbols[c].m_classes.m_nextTrainer-1.0;
            }
...
{
    TextFileWriter *file = g_app->m_resource->GetTextFileWriter( filename, false );
