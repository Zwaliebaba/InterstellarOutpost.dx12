...
bool GameCursor::GetHighlightedObject(WorldObjectId& _id, Vector3& _pos, float& _radius)
{
  ...
  if (somethingHighlighted)
  {
    if (id.GetUnitId() == UNIT_BUILDINGS)
    {
      ...
      if (building && (building->m_type == Building::TypeRadarDish || building->m_type == Building::TypeBridge || (building->m_type ==
          Building::TypeGunTurret && building->m_id.GetTeamId() == g_app->m_globalWorld->m_myTeamId) || building->m_type ==
        Building::TypeFenceSwitch))
...
void GameCursor::CreateMarker(const Vector3& _pos)
{
...
void GameCursor::RenderSelectionCircle()
{
  ...
  if (g_app->m_location && team && team->m_selectionCircleCentre != g_zeroVector)
  {
    for (int i = 0; i < 30; ++i)
    {
      ...
      Vector3 angle(cosf(thisAngle) * predictedCircleRadius, 0, sinf(thisAngle) * predictedCircleRadius);
...
void RenderSprite(char* filename, float scaleFactor)
{
  ...
  glBegin(GL_QUADS);
  glTexCoord2i(0, 1);
  glVertex2f(centreX - iconSize, centreY - iconSize);
  glTexCoord2i(1, 1);
  glVertex2f(centreX + iconSize, centreY - iconSize);
  glTexCoord2i(1, 0);
  glVertex2f(centreX + iconSize, centreY + iconSize);
...
void GameCursor::RenderSelectedTaskIcon(Task* task)
{
  ...
  if (task->m_state != Task::StateStarted || fmodf(GetHighResTime(), 0.4f) < 0.25f)
  ...
  if (task->m_state == Task::StateStarted)
  {
    ...
    else if (task->m_type == GlobalResearch::TypeHotFeet || task->m_type == GlobalResearch::TypeBooster || task->m_type ==
      GlobalResearch::TypeSubversion || task->m_type == GlobalResearch::TypePlague || task->m_type == GlobalResearch::TypeExtinguisher ||
      task->m_type == GlobalResearch::TypeMeteorShower || task->m_type == GlobalResearch::TypeRage || task->m_type ==
      GlobalResearch::TypeNuke)
    {
      ...
      if (fmodf(GetHighResTime(), 0.4f) > 0.25f)
        colour.a *= 0.5f;
...
bool GameCursor::RenderCrosshair()
{
  ...
  {
    if (unit->m_troopType == Entity::TypeInsertionSquadie && INPUT_MODE_GAMEPAD == g_inputManager->getInputMode())
  ...
  else if (unit && unit->m_troopType == Entity::TypeInsertionSquadie && INPUT_MODE_GAMEPAD == g_inputManager->getInputMode() && g_app->
    m_camera->m_trackingEntity)
  {
    // Squaddies
...
void GameCursor::RenderTryingToEnterEffect()
{
  if (m_objUnderMouse.IsValid())
  {
    ...
    if (entity && entity->m_type == Entity::TypeArmour && myTeam->m_numDarwiniansSelected > 0)
    {
      tryingToEnter = true;
      entrancePos = entity->m_pos;
      validEnter = static_cast<Armour*>(entity)->GetNumPassengers() < static_cast<Armour*>(entity)->Capacity();
    }
    ...
    if (m_objUnderMouse.GetUnitId() == UNIT_BUILDINGS)
    {
      ...
      if (building && building->m_type == Building::TypeRadarDish)
      {
        ...
        if (myTeam->m_numDarwiniansSelected > 0 || (selected && selected->m_type == Entity::TypeOfficer))
        {
          tryingToEnter = true;
          Vector3 front;
          static_cast<RadarDish*>(building)->GetEntrance(entrancePos, front);
          validEnter = static_cast<RadarDish*>(building)->Connected();
        }
      }
    }
...
{
  if (!g_app->m_atMainMenu && g_app->m_location && !g_app->m_location->GetMyTeam())
  {
        ...
        if (b && b->m_type == Building::TypeRadarDish)
        {
          ...
          for (int j = 0; check && j < rd->m_inTransit.Size(); j++)
          {
            if (rd->m_inTransit.ValidIndex(j))
            {
              ...
              if (id->GetUniqueId() == selectedId.GetUniqueId() || id->GetUnitId() == selectedId.GetUnitId())
              {
                skipSelectionEffect = true;
                check = false;
              }
  ...
  if (!somethingHighlighted && !somethingSelected)
  {
    ...
    if (darwinianId.IsValid() && myTeam->m_numDarwiniansSelected == 0 && myTeam->m_selectionCircleCentre == g_zeroVector)
    {
...
void GameCursor::RenderRing(const Vector3& centrePos, float innerRadius, float outerRadius, int numSteps, bool forceAttachToGround)
{
...
void GameCursor::RenderCyliner(const Vector3& centrePos, float radius, float height, int numSteps, RGBAColour colour,
                               bool forceAttachToGround)
{
  if (g_app->m_multiwinia->GameOver())
    return;
  Vector3 groundPos = centrePos;
...
void GameCursor::RenderUnitHighlightEffect(const Vector3& pos, float radius, RGBAColour colour)
{
  ...
  g_app->m_camera->SetupProjectionMatrix(nearPlaneStart * 1.03f, g_app->m_renderer->GetFarPlane());
...
{
  if (g_app->m_hideInterface && EclGetWindows()->Size() <= 0)
    return;
...
void GameCursor::RenderFormation(const Vector3& pos, const Vector3& front)
{
  float nearPlaneStart = g_app->m_renderer->GetNearPlane();
  g_app->m_camera->SetupProjectionMatrix(nearPlaneStart * 1.03f, g_app->m_renderer->GetFarPlane());
  ...
  for (int i = 0; i < formationSize; ++i)
  {
    ...
    glBegin(GL_QUADS);
    glTexCoord2i(0, 0);
    glVertex3dv((formationPos - camUp).GetData());
    glTexCoord2i(1, 0);
    glVertex3dv((formationPos + camRight).GetData());
    glTexCoord2i(1, 1);
    glVertex3dv((formationPos + camUp).GetData());
    glTexCoord2i(0, 1);
    glVertex3dv((formationPos - camRight).GetData());
...
void GameCursor::Render()
{
  ...
  if (EclGetWindows()->Size() == 0)
  {
    ...
    else if (!g_app->m_location)
    {
      ...
      bool locAvailable = highlightedLocation && strcmp(highlightedLocation->m_missionFilename, "null") != 0 && highlightedLocation->
        m_available;
      g_app->m_renderer->SetupMatricesFor2D();
    ...
    else if (g_app->m_location)
    {
      ...
      if (g_app->m_taskManagerInterface->IsVisible())
      {
        ...
        if (somethingHighlighted)
        {
          RGBAColour teamCol = g_app->m_location->GetMyTeam() ? g_app->m_location->GetMyTeam()->m_colour : RGBAColour(255, 255, 100, 255);
      ...
      else if (task && task->m_state == Task::StateStarted && task->m_type != GlobalResearch::TypeOfficer && !somethingHighlighted)
      {
        ...
        m_cursorPlacement->SetSize(40.0f); Vector3 front = (landNormal ^ g_upVector).Normalise(); m_cursorPlacement->
          Render3D(mousePos, front, landNormal); if (!validPlacement)
        ...
        if (task->m_type == GlobalResearch::TypeHotFeet || task->m_type == GlobalResearch::TypeBooster || task->m_type ==
          GlobalResearch::TypeSubversion || task->m_type == GlobalResearch::TypePlague || task->m_type == GlobalResearch::TypeExtinguisher
          || task->m_type == GlobalResearch::TypeMeteorShower || task->m_type == GlobalResearch::TypeRage)
        {
          float area = POWERUP_EFFECT_RANGE;
          if (task->m_type == GlobalResearch::TypeExtinguisher)
            area *= 2.0f;
          if (task->m_type == GlobalResearch::TypeMeteorShower)
            area *= 6.0f;
      ...
      else if (g_app->m_camera->IsInMode(Camera::ModeEntityTrack))
      {
        ...
        if (g_app->m_camera->m_trackingEntity && g_inputManager->controlEvent(ControlUnitPrimaryFireTarget))
        {
          float posX, posY;
          g_app->m_camera->Get2DScreenPos(g_app->m_camera->m_trackingEntity->m_pos + g_app->m_camera->m_targetVector, &posX, &posY);
      ...
      else
      {
        ...
        if (somethingHighlighted && !((somethingSelected || darwiniansSelected) && highlightedId.GetUnitId() == UNIT_BUILDINGS))
        {
          float camDist = (g_app->m_camera->GetPos() - highlightedWorldPos).Mag();
        ...
        if (darwiniansSelected && somethingHighlighted && highlightedId.GetUnitId() != UNIT_BUILDINGS)
        {
        ...
        else if ((somethingSelected || darwiniansSelected) && selectedId.GetUnitId() != UNIT_BUILDINGS)
        {
          ...
          if (somethingSelected)
          {
            if (selectedId.GetIndex() == -1)
              ...
              entityType = g_app->m_location->GetEntity(selectedId)->m_type;
          ...
          if ((entityType == Entity::TypeInsertionSquadie || entityType == Entity::TypeOfficer || g_app->m_location->GetMyTeam()->
            m_numDarwiniansSelected > 0) && highlightedBuilding)
          {
          ...
          if ((!somethingHighlighted || highlightedId.GetUnitId() == UNIT_BUILDINGS) && g_app->m_location->m_teams[g_app->m_globalWorld->
            m_myTeamId]->m_numDarwiniansSelected == 0 && !g_app->m_camera->m_camLookAround)
          {
            Vector3 landNormal = g_app->m_location->m_landscape.m_normalMap->GetValue(mousePos.x, mousePos.z);
#ifdef RENDER_CURSOR_3D
            Vector3 targetFront = (mousePos - selectedWorldPos).Normalise(); Vector3 targetRight = (targetFront ^ landNormal).Normalise();
            targetFront = (targetRight ^ landNormal).Normalise(); m_cursorMoveHere->SetSize(30.0f); m_cursorMoveHere->Render3D(
              mousePos, targetFront, landNormal);
#else
        ...
        if (!somethingSelected && !somethingHighlighted)
        {
          ...
          if (team)
          {
            ...
            if (darwinianId.IsValid() && team->m_numDarwiniansSelected == 0 && team->m_selectionCircleCentre == g_zeroVector)
            {
              // Highlighting a darwinian, ready to promote
              Entity* entity = g_app->m_location->GetEntity(darwinianId);
              if (entity->IsSelectable() || (task && task->m_state == Task::StateStarted && task->m_type == GlobalResearch::TypeOfficer))
              {
              ...
              else
              {
                ...
                Vector3 landNormal = g_app->m_location->m_landscape.m_normalMap->GetValue(mousePos.x, mousePos.z);
#ifdef RENDER_CURSOR_3D
                Vector3 front = (landNormal ^ g_upVector).Normalise(); m_cursorHighlight->SetAnimation(false); m_cursorHighlight->
                  SetSize(30.0f); m_cursorHighlight->Render3D(mousePos, front, landNormal);
#else
                float posX, posY;
                g_app->m_camera->Get2DScreenPos(mousePos, &posX, &posY);
                g_app->m_renderer->SetupMatricesFor2D();
            ...
            else if (team->m_numDarwiniansSelected > 0)
            {
              ...
              Vector3 landNormal = g_app->m_location->m_landscape.m_normalMap->GetValue(mousePos.x, mousePos.z);
#ifdef RENDER_CURSOR_3D
              Vector3 targetFront = (mousePos - team->m_selectedDarwinianCentre).Normalise(); Vector3 targetRight = (targetFront ^
                landNormal).Normalise(); targetFront = (targetRight ^ landNormal).Normalise(); m_cursorMoveHere->SetSize(30.0f);
  ...
  if (g_app->m_location && g_app->m_location->GetMyTeam() && g_app->m_location->GetMyTeam()->m_selectionCircleCentre != g_zeroVector)
  {
    Team* team = g_app->m_location->GetMyTeam();
...
void GameCursor::FindScreenEdge(const Vector2& _line, float* _posX, float* _posY)
{
  //    y = mx + c
  //    c = y - mx
  //    x = (y - c) / m
...
void GameCursor::RenderSelectionArrow(float _screenX, float _screenY, float _screenDX, float _screenDY, float _size, float _alpha)
{
  ...
  glBegin(GL_QUADS);
  glTexCoord2i(0, 1);
  glVertex2dv((pos - rightAngle * _size / 2.0f).GetData());
  glTexCoord2i(0, 0);
  glVertex2dv((pos - rightAngle * _size / 2.0f + gradient * _size).GetData());
  glTexCoord2i(1, 0);
  glVertex2dv((pos + rightAngle * _size / 2.0f + gradient * _size).GetData());
  glTexCoord2i(1, 1);
  glVertex2dv((pos + rightAngle * _size / 2.0f).GetData());
...
void GameCursor::RenderOffscreenArrow(Vector3& fromPos, Vector3& toPos, float alpha)
{
  if (g_app->Multiplayer())
    return;
  if (!g_app->m_location->m_landscape.IsInLandscape(toPos))
    return;
  ...
  for (int i = 0; i < numSteps; ++i)
  {
    ...
    glBegin(GL_QUADS);
    glTexCoord2i(0, 0);
    glVertex3dv((thisPos - camUp).GetData());
    glTexCoord2i(1, 0);
    glVertex3dv((thisPos + camRight).GetData());
    glTexCoord2i(1, 1);
    glVertex3dv((thisPos + camUp).GetData());
    glTexCoord2i(0, 1);
    glVertex3dv((thisPos - camRight).GetData());
...
void GameCursor::RenderSelectionArrows(WorldObjectId _id, Vector3& _pos, bool darwinians)
{
  ...
  if (unit && unit->NumAliveEntities() == 0)
    return;
  if (unit && unit->m_troopType == Entity::TypeInsertionSquadie && g_inputManager->getInputMode() == INPUT_MODE_GAMEPAD)
    return;
...
float MouseCursor::GetSize()
{
  ...
  if (m_animating)
    size += abs(sinf(g_gameTime * 4.0f)) * size * 0.6f;
...
void MouseCursor::SetColour(const RGBAColour& _col) { m_colour = _col; }
...
void MouseCursor::Render3D(const Vector3& _pos, bool _cameraScale)
{
...
void MouseCursor::Render3D(const Vector3& _pos, const Vector3& _front, const Vector3& _up, bool _cameraScale)
{
  ...
  if (m_shadowed)
  {
    ...
    glTexCoord2i(1, 0);
    glVertex3dv((pos - _front * scale + rightAngle * scale).GetData());
    glTexCoord2i(0, 0);
    glVertex3dv((pos - _front * scale).GetData());
    glEnd();
  }
