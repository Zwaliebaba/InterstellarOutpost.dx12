...
#ifdef USE_DIRECT3D
	static const int NumVertexBuffers = 10;
	IDirect3DVertexBuffer9	*m_vertexBuffer[NumVertexBuffers];
	bool					 m_vertexBufferReady[NumVertexBuffers];

	NetMutex				m_vertexBufferLock;
	NetEvent				m_terminating;
	HANDLE					m_updateWaterThread;
	int						m_renderVertexBuffer;
	int						m_prepareVertexBuffer;
#endif
...
#ifdef USE_DIRECT3D
	static DWORD WINAPI	UpdateWaterThread( void *_self );
	void			AllocateVertexBuffersIfNecessary();
#endif
