...
void Location::Init(const char* _missionFilename, const char* _mapFilename)
{
  ...
  else if (g_app->m_multiwinia->m_coopMode)
  {
    ...
    {
      if (i != GetMonsterTeamId() && i != GetFuturewinianTeamId() && m_coopGroups[i] == 0)
        SetupCoop(i);
    }
  }
  else
  {
    ...
    if (!g_app->m_multiwinia->m_coopMode && g_app->m_multiwinia->m_gameType != Multiwinia::GameTypeAssault)
    {
      for (int i = 0; i < m_levelFile->m_numPlayers; ++i)
      {
        if (i != GetMonsterTeamId() && i != GetFuturewinianTeamId() && g_app->m_multiwinia->m_teams[i].m_teamType == TeamTypeCPU)
        {
  ...
  if (m_levelFile->m_numPlayers < 4)
  {
    ...
    for (int i = 0; i < NUM_TEAMS; ++i)
    {
      if (i == GetMonsterTeamId() || i == GetFuturewinianTeamId())
        continue;
  ...
  DebugTrace("Location::Init Step 7 took %f seconds\n", GetHighResTime() - t);
  t = GetHighResTime();
  ...
  if (g_app->Multiplayer())
  {
    DebugTrace("Location::Init Step 8 took %f seconds\n", GetHighResTime() - t);
    t = GetHighResTime();
    InitMonsterTeam(GetMonsterTeamId());
    DebugTrace("Location::Init Step 9 took %f seconds\n", GetHighResTime() - t);
    t = GetHighResTime();
    InitFuturewinianTeam(GetFuturewinianTeamId());
    DebugTrace("Location::Init Step 10 took %f seconds\n", GetHighResTime() - t);
    t = GetHighResTime();
  ...
  DebugTrace("Location::Init Step 11 took %f seconds\n", GetHighResTime() - t);
  t = GetHighResTime();
}
...
void Location::LoadLevel(const char* missionFilename, const char* mapFilename)
{
...
void Location::InitBuildings()
{
  ...
  for (int i = 0; i < m_levelFile->m_buildings.Size(); i++)
  {
    ...
    if (existing)
    {
      ASSERT_TEXT(false,
                  ...
                  Building::GetTypeName(existing->m_type), Building::GetTypeName(building->m_type));
    }
    ...
    if (building->m_id.GetTeamId() != 255 && building->m_id.GetTeamId() != -1)
      building->SetTeamId(GetTeamFromPosition(building->m_id.GetTeamId()));

    newBuilding->Initialise(building);
    newBuilding->SetDetail(1);
    if (building->m_id.GetUniqueId() > highestId)
      highestId = building->m_id.GetUniqueId();

    if (building->m_type == Building::TypeControlStation)
      stations.PutData(building->m_id.GetUniqueId());
    else
      if (building->m_type == Building::TypePulseBomb)
        pulseBomb = building->m_id;
  }
  ...
  if (g_app->m_multiwinia->m_gameType == Multiwinia::GameTypeAssault && pulseBomb.GetUniqueId() != -1)
  {
    ...
    {
      PulseBomb* pulse = static_cast<PulseBomb*>(pb);
          ...
          if (b && b->m_type == Building::TypeControlStation)
          {
            for (int j = 0; j < pulse->m_links.Size(); j++)
            {
              if (pulse->m_links.ValidIndex(j))
              {
                ...
                {
                  ControlStation* cs = static_cast<ControlStation*>(b);
...
WorldObjectId Location::SpawnEntities(const Vector3& _pos, unsigned char _teamId, int _unitId, unsigned char _type, int _numEntities,
                                      const Vector3& _vel, double _spread, double _range, int _routeId, int _routeWaypointId)
{
  if (_teamId == GetMonsterTeamId() && m_teams[_teamId]->m_teamType == TeamTypeUnused)
  {
    // no monster team yet, set it up now
    InitMonsterTeam(_teamId);
  }

  if (_teamId == GetFuturewinianTeamId() && m_teams[_teamId]->m_teamType == TeamTypeUnused)
    InitFuturewinianTeam(_teamId);

  DEBUG_ASSERT(_teamId < NUM_TEAMS && _teamId >= 0 && m_teams[_teamId]->m_teamType > TeamTypeUnused);
...
Vector3 Location::FindValidSpawnPosition(const Vector3& _pos, double _spread)
{
  int tries = 10;
...
int Location::SpawnSpirit(const Vector3& _pos, const Vector3& _vel, unsigned char _teamId, WorldObjectId _id)
{
...
{
  const unsigned char teamId = _id.GetTeamId();
  const int unitId = _id.GetUnitId();
  const int index = _id.GetIndex();
  const int uniqueId = _id.GetUniqueId();

  if (teamId >= NUM_TEAMS || m_teams[teamId]->m_teamType == TeamTypeUnused)
    return NULL;
...
Entity* Location::GetEntity(const Vector3& _rayStart, const Vector3& _rayDir)
{
...
Building* Location::GetBuilding(const Vector3& _rayStart, const Vector3& _rayDir)
{
...
bool Location::IsVisible(const Vector3& _from, const Vector3& _to)
{
...
double Location::GetWalkingDistance(const Vector3& from, const Vector3& to, bool checkObstructions)
{
  ...
  {
    Vector3 position = from + (to - from) * (i / static_cast<double>(numSteps));
    ...
    if (m_obstructionGrid && checkObstructions)
    {
      ...
      for (int j = 0; j < buildingIds->Size(); ++j)
      {
        ...
        if (building)
        {
          if (building->m_type == Building::TypeWall && building->DoesSphereHit(position, stepSize / 2.0))
            return -1;

          if (building->m_type == Building::TypeLaserFence && static_cast<LaserFence*>(building)->IsEnabled() && building->DoesSphereHit(
            position, stepSize / 2.0))
            return -1;
        }
      }
    }
...
bool Location::IsWalkable(const Vector3& _from, const Vector3& _to, bool _evaluateCliffs, bool permitSteepDownhill, int* _laserFenceId)
{
  double waterLevel = 0.0;
  ...
  Vector3 diff = (_to - _from) / static_cast<double>(numSteps);
  double distanceUnderWater = 0.0;
  ...
  for (int i = 0; i < numSteps; ++i)
  {
    ...
    if (m_obstructionGrid)
    {
      ...
      for (int j = 0; j < buildingIds->Size(); ++j)
      {
        ...
        if (building)
        {
          if (building->m_type == Building::TypeWall && building->DoesSphereHit(position, stepSize * 0.66))
            return false;

          if (building->m_type == Building::TypeLaserFence && static_cast<LaserFence*>(building)->IsEnabled() && building->
            DoesSphereHit(position, stepSize / 2.0))
          {
            if (_laserFenceId)
...
#ifdef TRACK_SYNC_RAND
      if (e->m_type != WorldObject::EffectSnow)
        SyncRandLogText(e->LogState("EFFECT AFTER: ")); bool WatchEntities(); WatchEntities();
#endif
  ...
  for (int i = startIndex; i <= endIndex; ++i)
  {
    if (m_buildings.ValidIndex(i))
    {
...
#ifdef TRACK_SYNC_RAND
      bool WatchEntities(); WatchEntities();
#endif
      ...
      if (removeBuilding)
      {
        ...
        if (building->m_type == Building::TypeAITarget || building->m_type == Building::TypeWall)
          regenGrid = true;
...
void Location::Advance(int _slice)
{
  ...
  if (_slice == 3)
  {
    ...
    if (m_levelFile->m_effects[LevelFile::LevelEffectSnow])
      AdvanceChristmas();
    if (m_levelFile->m_effects[LevelFile::LevelEffectSoulRain])
      AdvanceSoulRain();
    if (m_levelFile->m_effects[LevelFile::LevelEffectDustStorm])
      AdvanceDustWind();
    if (m_levelFile->m_effects[LevelFile::LevelEffectLightning])
      AdvanceLightning();
  ...
  if (_slice == 7)
    m_entityGridCache->Advance();

  if (g_app->Multiplayer() && _slice == 8)
  {
    AdvanceCrates();
    if (m_spawnManiaTimer > 0.0)
      m_spawnManiaTimer -= SERVER_ADVANCE_PERIOD;
    if (m_bitzkreigTimer > 0.0)
      m_bitzkreigTimer -= SERVER_ADVANCE_PERIOD;
  }
}
...
void Location::AdvanceSoulRain()
{
  ...
  if (m_spawnSync <= 0.0)
  {
    ...
    auto spawnPos = Vector3(FRAND(sizeX), posY, FRAND(sizeZ));
...
void Location::AdvanceDustWind()
{
  ...
  if (DustBall::s_vortexTimer <= 0.0)
  {
    ...
    {
      auto spawnPos = Vector3(FRAND(worldSizeX), FRAND(500.0), FRAND(worldSizeZ));
  ...
  if (m_christmasTimer < -90.0)
  {
    ...
    {
      double posY = syncfrand(1000.0);
      auto spawnPos = Vector3(FRAND(worldSizeX), posY, FRAND(worldSizeZ));
  ...
  if (DustBall::s_vortexPos != g_zeroVector)
  {
    ...
    {
      Darwinian* d = static_cast<Darwinian*>(GetEntitySafe(s_neighbours[i], Entity::TypeDarwinian));
      if (d)
      {
        if (d->m_onGround)
        {
...
void Location::CreateRandomCrate()
{
  ...
  Crate crateTemplate; int dropMode = g_app->m_multiwinia->GetGameOption("CrateDropMode"); bool random = (dropMode == 0 || dropMode == -1);
  if (dropMode == 1)
  {
    bool success = false;
    double totalScore = 0;
    ...
    for (int i = 0; i < NUM_TEAMS; ++i)
    {
      if (i == GetMonsterTeamId() || i == GetFuturewinianTeamId() || g_app->m_multiwinia->m_teams[i].m_teamType == TeamTypeUnused)
        continue;
    ...
    if (lowDropChance > weightedDropChance)
      weightedDropChance = lowDropChance;
    if (syncfrand(1.0) > weightedDropChance) { random = true; }
    else
    {
      DebugTrace("Begining Weighted Crate Drop\n");
      int dropTeam = -1;
      ...
      if (dropTeam != -1)
      {
        DebugTrace("Weighted Crate Drop team determined - %d\n", dropTeam);
        ...
        for (int i = 0; i < m_buildings.Size(); ++i)
        {
          ...
          {
            Building* b = m_buildings[i];
            if (b->m_id.GetTeamId() == dropTeam) { buildingIndexList.PutData(i); }
          }
        }

        if (buildingIndexList.Size() > 0)
        {
          DebugTrace("Weighted Crate Drop - Building list generated\n");
          int sanity = 0;
          while (sanity++ < 1000)
          {
            ...
            // if( !RestrictionZone::IsRestricted( b->m_pos ) )
            {
              ...
              {
                DebugTrace("Weighted Crate Drop - Valid Position found\n");
    ...
    if (!success)
    {
      DebugTrace("Weighted Crate Drop - no valid pos found, falling back to random\n");
      random = true;
    }
  } bool nocrate = false; if (random)
  {
    int sanity = 0;
...
// *** Render Landscape
void Location::RenderLandscape() { m_landscape.Render(); }
...
void Location::RenderBuildings()
{
  ...
  if (g_prefsManager->GetInt("RenderSpecialLighting") == 1 || (g_app->Multiplayer() && g_app->m_multiwinia->m_gameType ==
      Multiwinia::GameTypeRocketRiot) || m_levelFile->m_effects[LevelFile::LevelEffectSpecialLighting] || g_app->m_gameMode ==
    App::GameModePrologue)
    SetupSpecialLights();

#ifdef CHEATMENU_ENABLED
  if (g_inputManager->controlEvent(ControlRTLoaderPixelWaveIncrease))
    g_prefsManager->SetInt("RenderSpecialLighting", 1); if (g_inputManager->controlEvent(ControlRTLoaderPixelWaveDecrease))
...
{
  auto b1 = (Location::DepthSortedBuilding*)elem1;
  auto b2 = (Location::DepthSortedBuilding*)elem2;

  if (b1->m_distance < b2->m_distance)
    return -1;
  if (b1->m_distance > b2->m_distance)
    return +1;
  return 0;
...
void Location::RenderWeapons()
{
  ...
  for (int i = 0; i < m_effects.Size(); ++i)
  {
    if (m_effects.ValidIndex(i))
    {
      ...
      if (g_app->m_camera->PosInViewFrustum(w->m_pos) || w->m_type == WorldObject::EffectPulseWave || w->m_type ==
        WorldObject::EffectSpamInfection || w->m_type == WorldObject::EffectLightning)
      {
...
void Location::RenderCurrentMessage()
{
  ...
  if (m_message.Length() > 0)
  {
    ...
    {
      //size = sqrt( 2.7 - timeRemaining ) * 30.0;        
      size += sqrt(2.0 - timeRemaining) * 15.0;
    }

    double outlineAlpha = alpha * alpha * alpha;

    double scale = static_cast<double>(g_app->m_renderer->ScreenW()) / 1280.0;
...
void Location::RenderChatMessages()
{
  ...
  float scale = static_cast<float>(g_app->m_renderer->ScreenW()) / 1280.0f;
  if (g_app->m_renderer->ScreenW() <= 800)
  {
    scale = static_cast<float>(g_app->m_renderer->ScreenW()) / 1024.0f;
  ...
  for (int i = 0; i < m_chatMessages.Size(); ++i)
  {
    ...
    if (!stopRendering)
    {
      ...
      for (int j = 0; j < wrapped->Size(); ++j)
      {
        ...
        if (i == 0 && j == wrapped->Size() - 1)
          mod = fontSize * 0.25f;
        if ((maxed || i == m_chatMessages.Size() - 1) && j == 0)
          bottomMod = fontSize * 0.25f;
        glBegin(GL_QUADS);
...
int Location::GetTeamPosition(unsigned char _teamId)
{
  ...
  if (_teamId == GetMonsterTeamId() || _teamId == GetFuturewinianTeamId())
  ...
  if (m_teamPosition[_teamId] == -1)
  {
    ...
    if (unused.Size() == 0)
      return -1; // no spaces left, more teams than available slots
...
int Location::GetTeamFromPosition(int _positionId)
{
  ...
  for (int i = 0; i < m_levelFile->m_numPlayers; ++i)
  {
    if (m_teamPosition[i] == -1)
      GetTeamPosition(i);
    if (m_teamPosition[i] == _positionId)
      return i;
  }
...
void Location::RotatePositions()
{
  ...
  {
    index++;
    if (index >= m_levelFile->m_numPlayers)
      index = 0;
...
void Location::SetSoloTeam(int _teamId, int _groupId)
{
  ...
  if (colourId == -1)
  {
    ...
    for (int i = 0; i < NUM_TEAMS; ++i)
    {
      ...
      if (g_app->m_multiwinia->m_teams[i].m_colourId != -1)
        used.PutData(g_app->m_multiwinia->m_teams[i].m_colourId);
    }
  ...
  if (_groupId == 1)
    m_teamPosition[_teamId] = m_levelFile->m_coopGroup1Positions[0];
  else
    if (_groupId == 2)
      m_teamPosition[_teamId] = m_levelFile->m_coopGroup2Positions[0];
  ...
  if (findUnassignedTeam)
  {
    for (int i = 0; i < NUM_TEAMS; ++i)
    {
      ...
      if (g_app->m_multiwinia->m_teams[i].m_colourId == -1)
      {
        ...
        int colourId = 0;
        if (g_app->m_multiwinia->m_teams[_teamId].m_colourId != -1)
          colourId = g_app->m_multiwinia->m_teams[_teamId].m_colourId;
        else { while (taken[colourId]) { colourId = syncrand() % MULTIWINIA_NUM_TEAMCOLOURS / 2; } }
    ...
    if (m_coopGroups[_teamId] == 0)
    {
      ...
      for (int t = 0; t < NUM_TEAMS; ++t)
      {
        if (t == _teamId)
          continue;
        if (t == GetMonsterTeamId() || t == GetFuturewinianTeamId())
          continue;
        bool paired = false;
        for (int i = 0; i < NUM_TEAMS; ++i)
        {
        ...
        if (!paired)
        {
          ...
          int colourId = 0;
          while (taken[colourId]) { colourId = syncrand() % MULTIWINIA_NUM_TEAMCOLOURS / 2; }
...
void Location::SetCoopTeam(int _teamId, int _groupId)
{
  ...
  if (setGroup == 1)
  {
    if (m_levelFile->m_coopGroup1Positions[0] == position)
      position2 = m_levelFile->m_coopGroup1Positions[1];
    else
      position2 = m_levelFile->m_coopGroup1Positions[0];
  }
  else
  {
    if (m_levelFile->m_coopGroup2Positions[0] == position)
      position2 = m_levelFile->m_coopGroup2Positions[1];
    else
      position2 = m_levelFile->m_coopGroup2Positions[0];
  }
  ...
  else
  {
    ...
    for (int t = 0; t < NUM_TEAMS; ++t)
    {
      if (t == _teamId)
        continue;
      if (g_app->m_multiwinia->m_teams[t].m_teamType == TeamTypeUnused)
        continue;
      ...
      {
        bool notLoner = true;
        if (m_coopGroups[i] == 1 && m_levelFile->m_coopGroup1Positions.Size() == 1)
          notLoner = false;
        else
          if (m_coopGroups[i] == 2 && m_levelFile->m_coopGroup2Positions.Size() == 1)
            notLoner = false;
...
void Location::GetCurrentTeamSpread(int& _attackers, int& _defenders)
{
  for (int i = 0; i < m_levelFile->m_numPlayers; ++i)
  {
    if (m_teamPosition[i] != -1 && m_levelFile->m_attacking[m_teamPosition[i]])
      _attackers++;
    else
      if (g_app->m_multiwinia->m_teams[i].m_colourId == ASSAULT_ATTACKER_COLOURID)
        _attackers++;

    if (m_teamPosition[i] != -1 && !m_levelFile->m_attacking[m_teamPosition[i]])
      _defenders++;
    else
      if (g_app->m_multiwinia->m_teams[i].m_colourId == ASSAULT_DEFENDER_COLOURID)
        _defenders++;
  }
}
...
void Location::SetupAssault()
{
  ...
  for (int i = 0; i < m_levelFile->m_numPlayers; ++i)
  {
    ...
    {
      if (numAttackers == totalAttackers)
        team->m_colourId = ASSAULT_DEFENDER_COLOURID;
      else if (numDefenders == totalDefenders)
        team->m_colourId = ASSAULT_ATTACKER_COLOURID;
      else
        team->m_colourId = syncrand() % 2;
    }

    if (team->m_colourId == ASSAULT_DEFENDER_COLOURID)
      m_teamPosition[i] = GetFreeDefenderPosition();
    else
      m_teamPosition[i] = GetFreeAttackerPosition();
    ...
    {
      int group = -1;
      if (m_levelFile->m_coopGroup1Positions.FindData(m_teamPosition[i]) != -1)
        group = 1;
      else
        group = 2;
      ...
      int position = 1;
      if (colourId == ASSAULT_DEFENDER_COLOURID && numDefenders > 0)
        position = 2;
      else
        if (colourId == ASSAULT_ATTACKER_COLOURID && numAttackers > 0)
          position = 2;
...
int Location::GetFreeAttackerPosition()
{
  ...
  {
    if (m_teamPosition[i] != -1 && m_levelFile->m_attacking[m_teamPosition[i]])
      taken.PutData(m_teamPosition[i]);
    if (m_levelFile->m_attacking[i])
      positions.PutData(i);
  }
  ...
  for (int i = 0; i < positions.Size(); ++i)
  {
    ...
    if (!invalid)
      valid.PutData(positions[i]);
  }

  if (valid.Size() > 0)
    return valid[syncrand() % valid.Size()];
  return -1;
}
...
void Location::SetupCoop(int _teamId)
{
  ...
  if (m_levelFile->m_coopGroup1Positions.Size() == 1)
    group1Solo = true;
  if (m_levelFile->m_coopGroup2Positions.Size() == 1)
    group2Solo = true;
  ...
  for (int i = 0; i < m_levelFile->m_coopGroup1Positions.Size(); ++i)
  {
    ...
    {
      setGroup = 1;
      if (totalTeam1Players == 2 || (totalTeam1Players == 1 && group1Solo))
      {
        setGroup = 2;
  ...
  if (setGroup == 0)
  {
    for (int i = 0; i < m_levelFile->m_coopGroup2Positions.Size(); ++i)
    {
      ...
      {
        setGroup = 2;
        if (totalTeam2Players == 2 || (totalTeam2Players == 1 && group2Solo))
        {
          setGroup = 1;
  ...
  for (int i = 0; i < m_levelFile->m_numPlayers; ++i)
  {
    if (i == _teamId)
      continue;
    if (m_teamPosition[i] == m_teamPosition[_teamId])
      m_teamPosition[i] = -1;
    GetTeamPosition(i);
  }

  if (setGroup == 1 && m_levelFile->m_coopGroup1Positions.Size() == 1)
    loner = true;
  else
    if (setGroup == 2 && m_levelFile->m_coopGroup2Positions.Size() == 1)
      loner = true;
...
void Location::InitialiseTeam(unsigned char _teamId)
{
  ...
  if (g_app->m_gameMode == App::GameModeMultiwinia)
  {
    ...
    if (_teamId == GetFuturewinianTeamId() || _teamId == GetMonsterTeamId())
      numPowerups = 0;
    if (numPowerups > 0)
    {
      for (int i = 0; i < numPowerups; ++i)
      {
        ...
        while (true)
        {
          ...
          if (powerupId != Crate::CrateRewardRandomiser && powerupId != Crate::CrateRewardCrateMania && powerupId !=
            Crate::CrateRewardDarwinians && powerupId != Crate::CrateRewardBitzkreig && powerupId != Crate::CrateRewardSpawnMania &&
            //                        powerupId != Crate::CrateRewardFormationIncrease &&
            powerupId != Crate::CrateRewardPlague && powerupId != Crate::CrateRewardRocketDarwinians && powerupId !=
            Crate::CrateRewardSpaceShip && powerupId != Crate::CrateRewardSpam && powerupId != Crate::CrateRewardTriffids && powerupId !=
            Crate::CrateRewardSpeedUpTime && powerupId != Crate::CrateRewardSlowDownTime && powerupId != Crate::CrateRewardForest &&
    ...
    for (int i = 0; i < m_buildings.Size(); ++i)
    {
      ...
      {
        Building* b = m_buildings[i];
        if ((b->m_type == Building::TypeSpawnPoint || b->m_type == Building::TypeSolarPanel) && b->m_id.GetTeamId() == _teamId)
        {
          for (int j = 0; j < b->m_ports.Size(); ++j)
          {
            ...
            Darwinian* d = static_cast<Darwinian*>(g_app->m_location->GetEntitySafe(id, Entity::TypeDarwinian));
            if (d)
            {
          ...
          {
            SpawnPoint* sp = static_cast<SpawnPoint*>(b);
            sp->EvaluatePorts();
          ...
          {
            SolarPanel* sp = static_cast<SolarPanel*>(b);
            sp->EvaluatePorts();
            sp->RecalculateOwnership();
  ...
  for (int i = 0; i < m_levelFile->m_instantUnits.Size(); i++)
  {
    ...
    if (iu->m_runAsTask)
    {
      ...
      case Entity::TypeArmour:
        taskType = GlobalResearch::TypeArmour;
        break;
      case Entity::TypeInsertionSquadie:
        taskType = GlobalResearch::TypeSquad;
        break;
      case Entity::TypeHarvester:
        taskType = GlobalResearch::TypeHarvester;
        break;
      case Entity::TypeShaman:
        taskType = GlobalResearch::TypeShaman;
        break;
      case Entity::TypeEngineer:
        taskType = GlobalResearch::TypeEngineer;
        break;
      case Entity::TypeTank:
        taskType = GlobalResearch::TypeTank;
        break;
      }
    ...
    else
    {
      ...
      WorldObjectId spawnedId = SpawnEntities(pos, _teamId, unitId, iu->m_type, iu->m_number, g_zeroVector, iu->m_spread, -1.0,
      ...
      {
        Darwinian* darwinian = static_cast<Darwinian*>(g_app->m_location->GetEntitySafe(spawnedId, Entity::TypeDarwinian));
        if (darwinian)
          darwinian->GiveOrders(targetPos);
      }
      if (iu->m_type == Entity::TypeOfficer)
      {
        Officer* officer = static_cast<Officer*>(g_app->m_location->GetEntitySafe(spawnedId, Entity::TypeOfficer));
      ...
      {
        Armour* armour = static_cast<Armour*>(g_app->m_location->GetEntitySafe(spawnedId, Entity::TypeArmour));
  ...
  if (_teamType == TeamTypeLocalPlayer)
  {
    for (int i = 0; i < m_levelFile->m_runningPrograms.Size(); ++i)
    {
      ...
      if (program->m_type == Entity::TypeEngineer)
      {
        ...
        Engineer* engineer = static_cast<Engineer*>(GetEntitySafe(objId, Entity::TypeEngineer));
        ...
        auto task = new Task(team->m_teamId);
      ...
      if (program->m_type == Entity::TypeInsertionSquadie)
      {
        ...
        int unitId;
        InsertionSquad* squad = static_cast<InsertionSquad*>(GetMyTeam()->NewUnit(Entity::TypeInsertionSquadie, program->m_count, &unitId,
                                                                                  pos));
...
bool Location::SelectDarwinians(unsigned char teamId, const Vector3& pos, double radius)
{
  ...
  for (int i = 0; i < numFound; ++i)
  {
    ...
    Darwinian* darwinian = static_cast<Darwinian*>(GetEntitySafe(id, Entity::TypeDarwinian));
    if (darwinian && darwinian->m_state != Darwinian::StateOperatingPort && darwinian->m_state != Darwinian::StateBeingSacraficed &&
      darwinian->m_state != Darwinian::StateOnFire && darwinian->m_state != Darwinian::StateInsideArmour && !darwinian->
      m_underPlayerControl)
    {
      darwinian->m_underPlayerControl = true;
      if (myTeam)
        g_app->m_soundSystem->TriggerEntityEvent(darwinian, "Selected");
    }
  }
...
void Location::IssueDarwinianOrders(unsigned char teamId, const Vector3& pos, bool directRoute)
{
  ...
  for (int i = 0; i < nearbyBuildings->Size(); ++i)
  {
    ...
    if (building->m_type == Building::TypeRadarDish)
    {
      ...
      Teleport* teleport = static_cast<Teleport*>(building);
  ...
  for (int i = 0; i < numFound; ++i)
  {
    Armour* a = static_cast<Armour*>(g_app->m_location->GetEntitySafe(s_neighbours[i], Entity::TypeArmour));
    if (a)
    {
      ...
      if (entity->m_type == Entity::TypeDarwinian)
      {
        ...
        if (darwinian->m_underPlayerControl)
        {
          if (armourId.IsValid())
          {
            ...
            Armour* a = static_cast<Armour*>(g_app->m_location->GetEntitySafe(armourId, Entity::TypeArmour));
  ...
  if (teamId == g_app->m_globalWorld->m_myTeamId && g_app->IsSinglePlayer())
  {
    OfficerOrders orders;
    orders.m_pos = centrePos;
    orders.m_wayPoint = targetPos;
    if (teleportEntrance != g_zeroVector)
      orders.m_wayPoint = teleportEntrance;
...
// *** UpdateTeam
void Location::UpdateTeam(unsigned char teamId, const TeamControls& teamControls)
{
  if (g_app->Multiplayer())
  {
    if (g_app->GamePaused() || g_app->m_multiwinia->GameOver())
      return;
  }
  Team* team = m_teams[teamId];
  ...
  if (unit)
  {
    if (unit->m_troopType == Entity::TypeInsertionSquadie) { unitMove = (teamControls.m_leftButtonPressed && !primaryFire); }
    ...
    if (unitMoved && teamControls.m_endSetTarget && teamId == g_app->m_globalWorld->m_myTeamId)
  ...
  {
    if (teamControls.m_endSetTarget && teamId == g_app->m_globalWorld->m_myTeamId)
      g_app->m_gameCursor->CreateMarker(teamControls.m_mousePos);
    ...
    case Entity::TypeEngineer:
    {
      Engineer* eng = static_cast<Engineer*>(entity);
      if (unitMove)
      {
      ...
      if (secondaryFire)
        eng->BeginBridge(teamControls.m_mousePos);
      break;
    }
    ...
    case Entity::TypeOfficer:
    {
      ...
      if (officer->m_orders == Officer::OrderPrepareGoto && officer->m_vel.Mag() < 5.0f)
    ...
    */
    ...
    case Entity::TypeShaman:
    {
      Shaman* shaman = static_cast<Shaman*>(entity);
      if (unitMove)
        shaman->SetWaypoint(teamControls.m_mousePos);
      if (primaryFire)
        shaman->ChangeMode(teamControls.m_mousePos);
      if (secondaryFire)
        shaman->TeleportToPos(teamControls.m_mousePos);

      if (!primaryFire && !secondaryFire)
        shaman->m_renderTeleportTargets = false;
      break;
    }

    case Entity::TypeHarvester:
    {
      Harvester* harvester = static_cast<Harvester*>(entity);
      if (unitMove)
      {
    ...
    case Entity::TypeTank:
    {
      ...
      if (unitMove)
        tank->SetWayPoint(teamControls.m_mousePos);
      if (!teamControls.m_consoleController)
      {
        if (primaryFire)
          tank->PrimaryFire();
  ...
  else
  {
    ...
    if (b)
    {
      ...
      {
        GunTurret* gt = static_cast<GunTurret*>(b);
        gt->SetTarget(teamControls.m_mousePos);

        if (unitMove || primaryFire || teamControls.m_directUnitMove || teamControls.m_leftButtonPressed)
          gt->PrimaryFire();
      }
    }
  }
...
int Location::GetUnitId(const Vector3& startRay, const Vector3& direction, unsigned char team, double* _range)
{
  if (team == 255)
    return -1;
        ...
        for (int i = 0; i < theUnit->m_entities.Size(); ++i)
        {
          if (theUnit->m_entities.ValidIndex(i))
          {
            ...
            if (entityHit && !entity->m_dead)
            {
              ...
              double rangeSqd = pow(centrePosX - rayHitX, 2) + pow(centrePosY - rayHitY, 2);
  ...
  if (_range && unitId != -1)
    *_range = sqrt(closestRangeSqd);

  return unitId;
}

WorldObjectId Location::GetEntityId(const Vector3& startRay, const Vector3& direction, unsigned char teamId, double* _range,
                                    bool _ignoreDarwinians)
{
  if (teamId == 255)
  ...
  for (int i = 0; i < numEntities; ++i)
  {
    if (m_teams[teamId]->m_others.ValidIndex(i))
    {
      ...
      if (ent->m_type == Entity::TypeDarwinian && _ignoreDarwinians)
        continue;
      ...
      if (!ent->m_dead)
      {
        ...
        {
          Armour* armour = static_cast<Armour*>(ent);
...
int Location::GetBuildingId(const Vector3& rayStart, const Vector3& rayDir, unsigned char teamId, double _maxDistance, double* _range)
{
  double closestRangeSqd = FLT_MAX;
  int buildingId = -1;
  ...
  for (int i = 0; i < m_buildings.Size(); i++)
  {
    ...
    {
      Building* building = m_buildings.GetData(i);
      bool teamMatch = (teamId == 255 || building->m_id.GetTeamId() == 255 || teamId == building->m_id.GetTeamId());
      ...
      if (building->m_type != Building::TypeControlTower && teamMatch)
      {
        ...
        if (building->m_type == Building::TypeRadarDish || building->m_type == Building::TypeGunTurret)
        {
...
int Location::ThrowWeapon(const Vector3& _pos, const Vector3& _target, int _type, unsigned char _fromTeamId, double _force,
                          double _lifeTime, bool _explodeOnImpact)
{
  double distance = (_target - _pos).Mag();
  double force = sqrt(distance) * 8.0;

  int grenadeResearch = g_app->m_globalWorld->m_research->CurrentLevel(GlobalResearch::TypeGrenade);
  if (_fromTeamId == 1 && !g_app->Multiplayer())
    grenadeResearch = 4;
  ...
  case WorldObject::EffectThrowableGrenade:
  {
    ...
    static_cast<Grenade*>(weapon)->m_explodeOnImpact = _explodeOnImpact;
  }
  break;

  case WorldObject::EffectThrowableAirstrikeMarker:
    weapon = new AirStrikeMarker(_pos, front, force);
    break;
  case WorldObject::EffectThrowableControllerGrenade:
  {
    ...
    {
      ControllerGrenade* controller = static_cast<ControllerGrenade*>(weapon);
  ...
  if (_type == WorldObject::EffectThrowableGrenade && _lifeTime != 0.0)
    static_cast<Grenade*>(weapon)->m_life = _lifeTime;
...
void Location::FireRocket(const Vector3& _pos, const Vector3& _target, unsigned char _teamId, double _lifeTime, int _fromBuildingId,
                          bool _firework)
{
...
void Location::FireTurretShell(const Vector3& _pos, const Vector3& _vel, int _teamId, int _fromBuildingId)
{
...
void Location::FireLaser(const Vector3& _pos, const Vector3& _vel, unsigned char _teamId, bool _mindControl)
{
  ...
  if (_mindControl)
    lifetime *= 2.0; // subversion lasers are half speed, so they need double life to get the same range
...
int Location::PromoteDarwinian(unsigned char teamId, int _entityId)
{
  ...
  if (static_cast<Darwinian*>(entity)->IsOnFire())
    return -1;

  //
  // Spawn an Officer

  WorldObjectId spawnedId = g_app->m_location->SpawnEntities(entity->m_pos,
                                                             g_app->IsSinglePlayer() ? g_app->m_globalWorld->m_myTeamId : teamId, -1,
                                                             Entity::TypeOfficer, 1, entity->m_vel, 0);
  Officer* officer = static_cast<Officer*>(g_app->m_location->GetEntity(spawnedId));
...
int Location::DemoteOfficer(unsigned char teamId, int _entityId)
{
  ...
  static_cast<Officer*>(entity)->m_demoted = true;
...
int Location::Bang(const Vector3& _pos, double _range, double _damage, int _teamId, int _fromBuildingId)
{
  int numCores = _range * _damage * 0.01f;
  numCores += frand(numCores);
  for (int p = 0; p < numCores; ++p)
  {
    Vector3 vel(sfrand(30.0), 10.0 + frand(10.0), sfrand(30.0));
    double size = 120.0 + frand(120.0);
    g_app->m_particleSystem->CreateParticle(_pos + g_upVector * _range * 0.3, vel, Particle::TypeExplosionCore, size);
  }
  ...
  {
    double angle = 2.0 * M_PI * p / static_cast<double>(numDebris);
    ...
    g_app->m_particleSystem->CreateParticle(_pos + g_upVector * _range * 0.2, vel, Particle::TypeExplosionDebris, size);
  }
  ...
  for (int i = 0; i < numFound; ++i)
  {
    ...
    if (success)
    {
      ...
      if (entity->m_type != Entity::TypeInsertionSquadie && entity->m_type != Entity::TypeOfficer && entity->m_type != Entity::TypeShaman)
      {
        entity->m_vel += push;
...
void Location::CreateShockwave(const Vector3& _pos, double _size, unsigned char _teamId, int _fromBuildingId)
{
...
{
  float fogCol[] = {g_app->m_backgroundColour.r / 255.0, g_app->m_backgroundColour.g / 255.0, g_app->m_backgroundColour.b / 255.0, 0};

  float fogEnd = 4000;
  if (m_landscape.GetWorldSizeX() + 1000 > fogEnd)
    fogEnd = m_landscape.GetWorldSizeX() + 1000;
  if (m_landscape.GetWorldSizeZ() + 1000 > fogEnd)
    fogEnd = m_landscape.GetWorldSizeZ() + 1000;
...
bool Location::ChristmasModEnabled() { return m_christmasMode; }

bool Location::IsFriend(unsigned char _teamId1, unsigned char _teamId2)
{
  if (_teamId1 == 255 || _teamId2 == 255)
    return false;
  ...
  bool friends[NUM_TEAMS][NUM_TEAMS] = {true, false, true, false, false, false, // Green
...
void Location::FlushOpenGlState()
{
  ...
  for (int i = 0; i < m_buildings.Size(); ++i)
  {
    if (m_buildings.ValidIndex(i))
    {
      ...
      {
        Tree* tree = static_cast<Tree*>(building);
...
void Location::RecalculateAITargets()
{
  for (int i = 0; i < m_buildings.Size(); ++i)
  {
    if (g_app->m_location->m_buildings.ValidIndex(i))
    {
      ...
      if (building && building->m_type == Building::TypeAITarget && !building->m_destroyed)
      {
        AITarget* aiTarget = static_cast<AITarget*>(building);
  ...
  for (int i = 0; i < g_app->m_location->m_buildings.Size(); ++i)
  {
    if (g_app->m_location->m_buildings.ValidIndex(i))
    {
      ...
      {
        AITarget* a = static_cast<AITarget*>(building);
        if (a->m_turretTarget)
          continue;
        for (int n = 0; n < a->m_neighbours.Size(); ++n)
        {
          ...
          if (info->m_neighbourId == a->GetBuildingLink())
            continue; // never cull manual links

          AITarget* c = static_cast<AITarget*>(g_app->m_location->GetBuilding(info->m_neighbourId));
          DEBUG_ASSERT(c && c->m_type == Building::TypeAITarget);
          if (c->m_turretTarget)
            continue; // dont cull for gunturrets
          ...
          for (int x = 0; x < a->m_neighbours.Size(); ++x)
          {
            if (x != n)
            {
              ...
              AITarget* b = static_cast<AITarget*>(g_app->m_location->GetBuilding(info2->m_neighbourId));
              ...
              if (distanceBtoC > 0.0 && distanceAtoC > (distanceAtoB + distanceBtoC) * 0.8)
              {
                delete info;
...
void Location::SetLastProcessedSlice(int _slice) { m_lastSliceProcessed = _slice; }

int Location::GetLastProcessedSlice() { return m_lastSliceProcessed; }

#ifdef TRACK_SYNC_RAND
#include <map>
#include <string>

typedef std::map<Entity*, std::string> EntityStateMap;EntityStateMap s_trackedEntities;void TrackEntity(Entity* _entity)
{
