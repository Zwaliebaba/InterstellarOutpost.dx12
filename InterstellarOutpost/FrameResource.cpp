...
FrameResource::FrameResource(ID3D12Device10 *pDevice, ID3D12PipelineState *pPso, ID3D12PipelineState *pShadowMapPso,
                             ID3D12DescriptorHeap *pDsvHeap, ID3D12DescriptorHeap *pCbvSrvHeap, D3D12_VIEWPORT *pViewport,
                             UINT frameResourceIndex)
  : m_fenceValue(0)
{
  m_pipelineState.attach(pPso);
  m_pipelineStateShadowMap.attach(pShadowMapPso);

  for (UINT i = 0; i < CommandListCount; i++)
  {
    check_hresult(pDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&m_commandAllocators[i])));
    check_hresult(pDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_commandAllocators[i].get(),
    ...
    check_hresult(m_commandLists[i]->Close());
  }
  ...
  for (UINT i = 0; i < NumContexts; i++)
  {
    ...
    check_hresult(pDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&m_shadowCommandAllocators[i])));
    check_hresult(pDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&m_sceneCommandAllocators[i])));

    check_hresult(pDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_shadowCommandAllocators[i].get(),
                                             m_pipelineStateShadowMap.get(), IID_PPV_ARGS(&m_shadowCommandLists[i])));
    check_hresult(pDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_sceneCommandAllocators[i].get(),
                                             m_pipelineState.get(), IID_PPV_ARGS(&m_sceneCommandLists[i])));
    ...
    check_hresult(m_shadowCommandLists[i]->Close());
    check_hresult(m_sceneCommandLists[i]->Close());
  }

  // Describe and create the shadow map texture.
  CD3DX12_RESOURCE_DESC shadowTexDesc(D3D12_RESOURCE_DIMENSION_TEXTURE2D, 0, static_cast<UINT>(pViewport->Width),
                                      static_cast<UINT>(pViewport->Height), 1, 1, DXGI_FORMAT_R32_TYPELESS, 1, 0,
  ...
  {
    auto shadowTexDesc1 = CD3DX12_RESOURCE_DESC1(shadowTexDesc);
    check_hresult(pDevice->CreateCommittedResource3(&DefaultHeapProperties, D3D12_HEAP_FLAG_NONE, &shadowTexDesc1,
                                                    D3D12_BARRIER_LAYOUT_DEPTH_STENCIL_WRITE, &clearValue, nullptr, 0, nullptr,
                                                    IID_PPV_ARGS(&m_shadowTexture)));
  }
  else
  {
    check_hresult(pDevice->CreateCommittedResource(&DefaultHeapProperties, D3D12_HEAP_FLAG_NONE, &shadowTexDesc,
  ...
  CD3DX12_CPU_DESCRIPTOR_HANDLE depthHandle(pDsvHeap->GetCPUDescriptorHandleForHeapStart(), 1 + frameResourceIndex,
  ...
  const UINT textureCount = _countof(
      SampleAssets::
          Textures);// Diffuse + normal textures near the start of the heap.  Ideally, track descriptor heap contents/offsets at a higher level.
  ...
  const UINT constantBufferSize = (sizeof(SceneConstantBuffer) + (D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT - 1)) &
                                  ~(D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT - 1);// must be a multiple 256 bytes

  if (D3D12Multithreading::IsEnhancedBarriersEnabled())
  {
    auto constantBufferDesc = CD3DX12_RESOURCE_DESC1::Buffer(constantBufferSize);
    check_hresult(pDevice->CreateCommittedResource3(&UploadHeapProperties, D3D12_HEAP_FLAG_NONE, &constantBufferDesc,
  ...
  else
  {
    ...
    check_hresult(pDevice->CreateCommittedResource(&UploadHeapProperties, D3D12_HEAP_FLAG_NONE, &constantBufferDesc,
  ...
  check_hresult(m_shadowConstantBuffer->Map(0, &readRange, reinterpret_cast<void **>(&mp_shadowConstantBufferWO)));
  check_hresult(m_sceneConstantBuffer->Map(0, &readRange, reinterpret_cast<void **>(&mp_sceneConstantBufferWO)));
...
// this frame resource.
void FrameResource::WriteConstantBuffers(D3D12_VIEWPORT *pViewport, Camera *pSceneCamera, Camera lightCams[NumLights],
...
void FrameResource::Init()
{
  ...
  for (int i = 0; i < CommandListCount; i++)
  {
    check_hresult(m_commandAllocators[i]->Reset());
    check_hresult(m_commandLists[i]->Reset(m_commandAllocators[i].get(), m_pipelineState.get()));
  }
  ...
  for (int i = 0; i < NumContexts; i++)
  {
    check_hresult(m_shadowCommandAllocators[i]->Reset());
    check_hresult(m_shadowCommandLists[i]->Reset(m_shadowCommandAllocators[i].get(), m_pipelineStateShadowMap.get()));

    check_hresult(m_sceneCommandAllocators[i]->Reset());
    check_hresult(m_sceneCommandLists[i]->Reset(m_sceneCommandAllocators[i].get(), m_pipelineState.get()));
  }
}
...
void FrameResource::SwapBarriers()
{
  if (D3D12Multithreading::IsEnhancedBarriersEnabled())
  {
    D3D12_TEXTURE_BARRIER ShadowTextureBarriers[] = {
        CD3DX12_TEXTURE_BARRIER(D3D12_BARRIER_SYNC_DEPTH_STENCIL,        // SyncBefore
    ...
    D3D12_BARRIER_GROUP ShadowTextureBarrierGroups[] = {
        CD3DX12_BARRIER_GROUP(_countof(ShadowTextureBarriers), ShadowTextureBarriers)};
    m_commandLists[CommandListMid]->Barrier(_countof(ShadowTextureBarrierGroups), ShadowTextureBarrierGroups);
  }
  else
  {
    // Transition the shadow map from writeable to readable.
    auto shadowToReadableBarrier = CD3DX12_RESOURCE_BARRIER::Transition(m_shadowTexture.get(), D3D12_RESOURCE_STATE_DEPTH_WRITE,
                                                                        D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
    m_commandLists[CommandListMid]->ResourceBarrier(1, &shadowToReadableBarrier);
  }
}
...
void FrameResource::Finish()
{
  if (D3D12Multithreading::IsEnhancedBarriersEnabled())
  {
    D3D12_TEXTURE_BARRIER FinishTexBarrier[] = {
        CD3DX12_TEXTURE_BARRIER(D3D12_BARRIER_SYNC_PIXEL_SHADING,        // SyncBefore
  ...
  {
    auto shadowToWritableBarrier =
        CD3DX12_RESOURCE_BARRIER::Transition(m_shadowTexture.get(), D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE,
                                             D3D12_RESOURCE_STATE_DEPTH_WRITE);
    m_commandLists[CommandListPost]->ResourceBarrier(1, &shadowToWritableBarrier);
  }
}

// Sets up the descriptor tables for the worker command list to use
// resources provided by frame resource.
void FrameResource::Bind(ID3D12GraphicsCommandList *pCommandList, BOOL scenePass, D3D12_CPU_DESCRIPTOR_HANDLE *pRtvHandle,
