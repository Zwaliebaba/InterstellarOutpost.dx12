    ...
    if( g_prefsManager->GetInt( "TextLanguageTest", 0 ) == 1 )
    {
        ...
        for( int i = 0; i < phrases->Size(); ++i )
        {
            if( phrases->ValidIndex(i) )
            {
                ...
                BuildCaption( phrase->m_string, result );
				if( result.WcsLen() <= 0 )
                {
...
//#ifndef NDEBUG
				if( phrase->m_string.StrStr(UnicodeString("gesture"))  ||
                    phrase->m_string.StrStr(UnicodeString("Gesture"))  ||
                    phrase->m_string.StrStr(UnicodeString("[TAB]")) ) 
                {
                    char iconsId[256];
...
Sepulveda::~Sepulveda()
{
	delete m_mouseCursor; m_mouseCursor = NULL;
...
void Sepulveda::BuildCaption(UnicodeString const &_baseString, UnicodeString &_dest)
{
//	return buildCaption( _baseString, _dest );
	...
	UnicodeString temp(SEPULVEDA_MAX_PHRASE_LENGTH, _baseString.m_unicodestring, SEPULVEDA_MAX_PHRASE_LENGTH);
	temp.m_unicodestring[SEPULVEDA_MAX_PHRASE_LENGTH-1] = L'\x0';
	_dest = temp;
}
...
void Sepulveda::Advance()
{
	...
	if( m_caption.m_unicodestring[0] != L'\0' )
    {
	...
	if( m_caption.m_unicodestring[0] == L'\0' && m_msgQueue.Size() > 0 )
    {
    ...
    if( captionGone && m_caption.m_unicodestring[0] == L'\0' )
    {
...
void Sepulveda::Say( char *_stringId )
{
    ...
    {
        char phrase[256];
        sprintf( phrase, "%s_demogame", _stringId );
        if( ISLANGUAGEPHRASE( phrase ) )
        {
            actualStringId = newStr( phrase );
        }
...
{
	return( m_caption.m_unicodestring[0] != L'\0' || m_msgQueue.Size() > 0 );
}
...
void Sepulveda::RenderTextBoxCutsceneMode()
{
	...
	if( m_caption.m_unicodestring[0] != L'\0' )
    {
		...
		int totalChars = m_caption.WcsLen();
        ...
        LList<UnicodeString *> *wrapped = WordWrapText( m_caption, textBoxW-15, g_editorFont.GetTextWidth(1,textH), true );

        for( int i = 0; i < wrapped->Size(); i++ )
        {
            UnicodeString thisLine(*(wrapped->GetData(i)));

			if( thisChar + thisLine.WcsLen() < finishingChar )
            {                
            ...
            else if( thisChar < finishingChar )
            {
				...
				UnicodeString thisLineCopy = thisLine;
				thisLineCopy.m_unicodestring[finishingCharIndex] = L'\0';
    ...
    else if( m_msgQueue.Size() > 0 )
    {
		...
		swprintf( theString, sizeof(theString)/sizeof(wchar_t), L"%ls (%d)", LANGUAGEPHRASE("sepulveda_pressreturn").m_unicodestring, m_msgQueue.Size() );
        g_gameFont.DrawText2DRight( textBoxX+textBoxW, 590.0f, 7.0f, UnicodeString(theString) );
    }
	else if( m_caption.m_unicodestring[0] != L'\0' )
    {
...
void Sepulveda::RenderTextBoxTaskManagerMode()
{
    ...
    float yPos = m_screenH - 76;
    float alpha = 1.0f;
    float textX = textBoxX+15;
        
	if( m_caption.m_unicodestring[0] != L'\0' )
    {
        float textTime = CalculateTextTime() * 1.5f;
        float finishTime = m_timeSync + textTime;
        float fractionDone = ( GetHighResTime() - m_timeSync ) / ( finishTime - m_timeSync );
        if( fractionDone < 0.0f ) fractionDone = 0.0f;
        if( fractionDone > 1.0f ) fractionDone = 1.0f;
        ...
        for( int i = 0; i < wrapped->Size(); i++ )
        {
            ...
            else if( thisChar < finishingChar )
            {
				UnicodeString thisLineCopy = thisLine;
                int finishingCharIndex = ( finishingChar - thisChar );
				thisLineCopy.m_unicodestring[ finishingCharIndex ] = '\x0';
                g_gameFont.SetRenderOutline(true);
                glColor4f( m_fade, m_fade, m_fade, 0.0f );        
                g_gameFont.DrawText2D( textX, yPos, textH, thisLineCopy );
                glColor4f( 1.0f, 1.0f, 1.0f, m_fade );        
...
void Sepulveda::RenderTextBox()
{
    ...
    float startingAlpha = 0.8f;
	if( m_caption.m_unicodestring[0] == L'\0' && m_history.Size() ) startingAlpha *= GetCaptionAlpha(m_history[0]->m_timeSync);
    ...
    if( m_caption.m_unicodestring[0] != L'\0' )
    {
        ...
        for( int i = wrapped->Size()-1; i >= 0; --i )
        {
            ...
            if( yPos > 450.0f && yPos < 570.0f+textH )
            {
                ...
                {
                    glColor4f( 1.0f, 1.0f, 1.0f, 1.0f );        
                    g_gameFont.DrawText2D( textX-10, yPos, textH, UnicodeString(">") );
                }
	...
	if ( m_caption.m_unicodestring[0] == L'\0' ) firstItemIndex = 0;
    yPos += m_scrollbarOffset;
    for( int i = firstItemIndex; i < g_app->m_sepulveda->m_history.Size(); ++i )
    {
        ...
        {
            UnicodeString builtCaption;
			BuildCaption(LANGUAGEPHRASE( caption->m_stringId ), builtCaption);
			LList<UnicodeString*> *wrapped = WordWrapText( builtCaption, textBoxW-15, g_editorFont.GetTextWidth(1,textH) );
			
            bool rendered = false;

            for( int j = wrapped->Size()-1; j >= 0; --j )
            {
                UnicodeString thisLine(*(wrapped->GetData(j)));
                if( yPos > 450.0f && yPos < 570.0f+textH )
                {
                    if( j == 0 ) 
                    {
    ...
    s_highestY = yPos;
    s_highestY = max( s_highestY, 440.0f );
...
void Sepulveda::Render()
{
    ...
    START_PROFILE( "Render Sepulveda" );
...
void Sepulveda::RenderScrollBar()
{
            ...
            g_inputManager->controlEvent( ControlEclipseLMousePressed ) ) // TODO: Is this right?
    {
        ...
        m_scrollbarOffset = max( m_scrollbarOffset, 0.0f );
    }
...
void SepulvedaHighlight::Render()
{
    ...
    for( int i = 0; i < 3; ++i )
    {
        ...
        pos.x += sin( g_gameTime + i ) * 10;
        pos.z += cos( g_gameTime + i ) * 10;

        float blue = 0.5f + abs( sin(g_gameTime * i) ) * 0.5f;
